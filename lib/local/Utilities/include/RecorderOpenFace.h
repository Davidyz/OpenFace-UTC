...
public:
  ...
  // sequence or not, in_filename should be just the name and not contain
  // extensions
  ...
  void SetObservationHOG(bool good_frame,
                         const cv::Mat_<double> &hog_descriptor, int num_cols,
                         int num_rows, int num_channels);

  void SetObservationVisualization(const cv::Mat &vis_track);

  // Write out all observations for current face (except for tracked
  // image/video)
  void WriteObservation();

  // Separate method for writing tracked video observation, this is done because
  // video observation is written once a frame/image, other observations can
  // happen multiple times a frame/image
  void WriteObservationTracked();

  std::string GetCSVFile() { return csv_filename; }

private:
  // Blocking copy, assignment and move operators, as it does not make sense to
  ...
  // A thread that will write image and video output (the slowest parts of
  // output_
  ...
  std::string default_record_directory =
      "processed"; // By default we are writing in the processed directory in
                   // the working directory, if no output parameters provided
  std::string
