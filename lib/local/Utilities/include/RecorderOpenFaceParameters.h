...
public:
  // Constructors
  RecorderOpenFaceParameters(std::vector<std::string> &arguments, bool sequence,
                             bool is_from_webcam, float fx = -1, float fy = -1,
  ...
  RecorderOpenFaceParameters(bool sequence, bool is_from_webcam,
                             ...
                             bool record_bad = true, float fx = -1,
                             float fy = -1, float cx = -1, float cy = -1,
                             double fps_vid_out = 30);
  ...
  std::string imageFormatVisualization() const {
...
private:
  // If we are recording results from a sequence each row refers to a frame, if
  // we are recording an image each row is a face
  bool is_sequence;
  ...
  // Should the algined faces be recorded even if the detection failed (blank
  // images)
