...
public:
  ...
  // Webcam
  bool OpenWebcam(int device_id, int image_width = 640, int image_height = 480,
                  float fx = -1, float fy = -1, float cx = -1, float cy = -1);

  // Image sequence in the directory
  bool OpenImageSequence(std::string directory, float fx = -1, float fy = -1,
                         float cx = -1, float cy = -1);

  // Video file
  bool OpenVideoFile(std::string video_file, float fx = -1, float fy = -1,
                     float cx = -1, float cy = -1);
...
private:
  ...
  // A thread that will write video output, so that the rest of the application
