...
public:
  ...
  bool OpenDirectory(std::string directory, std::string bbox_directory = "",
                     float fx = -1, float fy = -1, float cx = -1,
                     float cy = -1);

  // Video file
  bool OpenImageFiles(const std::vector<std::string> &image_files,
                      float fx = -1, float fy = -1, float cx = -1,
                      float cy = -1);
  ...
  // Parameters describing the sequence and it's progress (what's the proportion
  // of images opened)
  double GetProgress();
...
private:
  ...
  // Could optionally read the bounding box locations from files (each image
  // could have multiple bounding boxes)
