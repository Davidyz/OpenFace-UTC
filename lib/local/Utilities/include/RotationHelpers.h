...
static cv::Vec3f RotationMatrix2Euler(const cv::Matx33f &rotation_matrix) {
  float q0 = sqrt(1 + rotation_matrix(0, 0) + rotation_matrix(1, 1) +
                  rotation_matrix(2, 2)) /
             2.0f;
  ...
  float roll =
      atan2(2.0f * (q0 * q3 - q1 * q2), q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3);
...
static void Project(cv::Mat_<float> &dest, const cv::Mat_<float> &mesh,
...
static cv::Matx22f AlignShapesKabsch2D(const cv::Mat_<float> &align_from,
                                       const cv::Mat_<float> &align_to) {
...
//=============================================================================
// Basically Kabsch's algorithm but also allows the collection of points to be
// different in scale from each other
static cv::Matx22f AlignShapesWithScale(cv::Mat_<float> &src,
                                        cv::Mat_<float> dst) {
  int n = src.rows;
