...
bool RecorderCSV::Open(std::string output_file_name, bool is_sequence,
                       bool output_2D_landmarks, bool output_3D_landmarks,
                       bool output_model_params, bool output_pose,
                       bool output_AUs, bool output_gaze,
                       int num_face_landmarks, int num_model_modes,
  ...
  // Different headers if we are writing out the results on a sequence or an
  // individual image
  if (this->is_sequence) {
  ...
  // Outputting model parameters (rigid and non-rigid), the first parameters are
  // the 6 rigid shape parameters, they are followed by the non rigid shape
  // parameters
...
void RecorderCSV::WriteLine(
    int face_id, int frame_num, double time_stamp,
    bool landmark_detection_success, double landmark_confidence,
    const cv::Mat_<float> &landmarks_2D, const cv::Mat_<float> &landmarks_3D,
    const cv::Mat_<float> &pdm_model_params,
    const cv::Vec6f &rigid_shape_params, cv::Vec6f &pose_estimate,
    const cv::Point3f &gazeDirection0, const cv::Point3f &gazeDirection1,
  ...
  if (output_gaze) {
    ...
    output_file << "," << gazeDirection0.x << "," << gazeDirection0.y << ","
                << gazeDirection0.z << "," << gazeDirection1.x << ","
                << gazeDirection1.y << "," << gazeDirection1.z;
...
void RecorderCSV::Close() { output_file.close(); }
