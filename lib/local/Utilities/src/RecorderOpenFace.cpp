...
void CreateDirectory(std::string output_path) {
  // Removing trailing separators, as that causes issues with directory creation
  // in unix
...
void RecorderOpenFace::PrepareRecording(const std::string &in_filename) {
  ...
  // Create the filename for the general output file that contains all of the
  // meta information about the recording
  ...
  if (!metadata_file.is_open()) {
    std::cout << "ERROR: could not open the output file:" << of_det_name
              << ", either the path of the output directory is wrong or you do "
                 "not have the permissions to write to it"
              << std::endl;
    exit(1);
  }

  // Populate relative and full path names in the meta file, unless it is a
  // webcam
  ...
  metadata_file << "Camera parameters:" << params.getFx() << ","
                << params.getFy() << "," << params.getCx() << ","
                << params.getCy() << std::endl;
  ...
  if (params.outputHOG()) {
    // Output the data based on record_root, but do not include record_root in
    // the meta file, as it is also in that directory
  ...
  if (params.outputTracked()) {
    if (params.isSequence()) {
      ...
      this->media_filename =
          (fs::path(record_root) / this->media_filename).string();
    } else {
  ...
  if (params.outputAlignedFaces()) {
    ...
    metadata_file << "Output aligned directory:"
                  << this->aligned_output_directory << std::endl;
    this->aligned_output_directory =
        (fs::path(record_root) / this->aligned_output_directory).string();
...
RecorderOpenFace::RecorderOpenFace(const std::string in_filename,
  ...
  for (size_t i = 0; i < arguments.size(); ++i) {
    if (!output_found && arguments[i].compare("-of") == 0) {
      record_root = (fs::path(record_root) / fs::path(arguments[i + 1]))
                        .remove_filename()
                        .string();
      out_name = fs::path(fs::path(arguments[i + 1]))
                     .replace_extension("")
                     .filename()
                     .string();
      valid[i] = false;
...
void RecorderOpenFace::WriteObservation() {
  ...
  // Write out the CSV file (it will always be there, even if not outputting
  // anything more but frame/face numbers)
  if (!csv_recorder.isOpen()) {
    // As we are writing out the header, work out some things like number of
    // landmarks, names of AUs etc.
    ...
    metadata_file << "Landmarks 2D: " << params.output2DLandmarks()
                  << std::endl;
    metadata_file << "Landmarks 3D: " << params.output3DLandmarks()
                  << std::endl;
    metadata_file << "Pose: " << params.outputPose() << std::endl;
    metadata_file << "Shape parameters: " << params.outputPDMParams()
                  << std::endl;

    csv_filename = (fs::path(record_root) / csv_filename).string();
    csv_recorder.Open(csv_filename, params.isSequence(),
                      params.output2DLandmarks(), params.output3DLandmarks(),
                      params.outputPDMParams(), params.outputPose(),
                      params.outputAUs(), params.outputGaze(),
                      num_face_landmarks, num_model_modes, num_eye_landmarks,
                      au_names_class, au_names_reg);
  }

  this->csv_recorder.WriteLine(
      face_id, frame_number, timestamp, landmark_detection_success,
      landmark_detection_confidence, landmarks_2D, landmarks_3D,
      pdm_params_local, pdm_params_global, head_pose, gaze_direction0,
      gaze_direction1, gaze_angle, eye_landmarks2D, eye_landmarks3D,
      au_intensities, au_occurences);
  ...
  if (params.outputAlignedFaces()) {
    ...
    if (!aligned_writing_thread_started) {
      ...
      int capacity =
          (1024 * 1024 * ALIGNED_QUEUE_CAPACITY) /
              (aligned_face.size().width * aligned_face.size().height *
               aligned_face.channels()) +
          1;
      aligned_face_queue.set_capacity(capacity);

      // Start the alignment output thread
      aligned_writing_thread =
          std::thread(&RecorderOpenFace::AlignedImageWritingTask, this);
    }
    ...
    std::string out_file =
        (fs::path(aligned_output_directory) /
         fs::path(std::string(name) + params.imageFormatAligned()))
            .string();

    if (params.outputBadAligned() || landmark_detection_success) {
      aligned_face_queue.push(
          std::pair<std::string, cv::Mat>(out_file, aligned_face));
    }
...
void RecorderOpenFace::WriteObservationTracked() {
  ...
  if (params.outputTracked()) {
    ...
    if (!tracked_writing_thread_started) {
      ...
      int capacity = (1024 * 1024 * TRACKED_QUEUE_CAPACITY) /
                         (vis_to_out.size().width * vis_to_out.size().height *
                          vis_to_out.channels()) +
                     1;
      ...
      if (params.isSequence()) {
        ...
        try {
          video_writer.open(
              media_filename,
              cv::VideoWriter::fourcc(output_codec[0], output_codec[1],
                                      output_codec[2], output_codec[3]),
              params.outputFps(), vis_to_out.size(), true);

          if (!video_writer.isOpened()) {
            WARN_STREAM(
                "Could not open VideoWriter, OUTPUT FILE WILL NOT BE WRITTEN.");
          }
        } catch (cv::Exception e) {
          WARN_STREAM("Could not open VideoWriter, OUTPUT FILE WILL NOT BE "
                      "WRITTEN. Currently using codec "
                      << output_codec
                      << ", try using an other one (-oc option)");
        }
      }

      // Start the video and tracked image writing thread
      video_writing_thread = std::thread(&RecorderOpenFace::VideoWritingTask,
                                         this, params.isSequence());
    }
    ...
    if (params.isSequence()) {
      ...
      vis_to_out_queue.push(
          std::pair<std::string, cv::Mat>(media_filename, vis_to_out));
    }
...
void RecorderOpenFace::SetObservationHOG(bool good_frame,
                                         const cv::Mat_<double> &hog_descriptor,
                                         int num_cols, int num_rows,
                                         int num_channels) {
  this->hog_recorder.SetObservationHOG(good_frame, hog_descriptor, num_cols,
...
void RecorderOpenFace::SetObservationLandmarks(
    const cv::Mat_<float> &landmarks_2D, const cv::Mat_<float> &landmarks_3D,
    const cv::Vec6f &pdm_params_global, const cv::Mat_<float> &pdm_params_local,
    double confidence, bool success) {
...
void RecorderOpenFace::SetObservationActionUnits(
...
void RecorderOpenFace::SetObservationGaze(
    const cv::Point3f &gaze_direction0, const cv::Point3f &gaze_direction1,
...
RecorderOpenFace::~RecorderOpenFace() { this->Close(); }
