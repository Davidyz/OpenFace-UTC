...
public:
  ...
  // TODO this could have optimize for demo mode (also could appropriately
  // update sigma, reg_factor as well)
  void optimiseForVideo() {
    ...
    if (params->curr_landmark_detector ==
  ...
  bool IsCECLM() {
    return params->curr_landmark_detector ==
           ::LandmarkDetector::FaceModelParameters::CECLM_DETECTOR;
  }
  ...
  System::String ^
      GetMTCNNLocation() {
        return gcnew System::String(
            params->mtcnn_face_detector_location.c_str());
      }
      ...
      GetHaarLocation() {
        return gcnew System::String(
            params->haar_face_detector_location.c_str());
      }
  ...
  ::LandmarkDetector::FaceModelParameters *getParams() { return params; }

  !FaceModelParameters() { delete params; }

  ~FaceModelParameters() { this->!FaceModelParameters(); }
};
...
public:
  ...
  bool isLoaded() { return clnf->loaded_successfully; }

  ::LandmarkDetector::CLNF *getCLNF() { return clnf; }

  void Reset() { clnf->Reset(); }

  void Reset(double x, double y) { clnf->Reset(x, y); }

  bool HasEyeModel() { return clnf->eye_model; }

  double GetConfidence() { return clnf->detection_certainty; }

  bool DetectLandmarksInVideo(OpenCVWrappers::RawImage ^ rgb_image,
                              FaceModelParameters ^ modelParams,
                              OpenCVWrappers::RawImage ^ gray_image) {
    return ::LandmarkDetector::DetectLandmarksInVideo(
        rgb_image->Mat, *clnf, *modelParams->getParams(), gray_image->Mat);
  }

  bool DetectLandmarksInVideo(OpenCVWrappers::RawImage ^ rgb_image,
                              FaceModelParameters ^ modelParams) {
    return ::LandmarkDetector::DetectLandmarksInVideo(
        rgb_image->Mat, *clnf, *modelParams->getParams(), cv::Mat());
  }

  bool DetectFaceLandmarksInImage(OpenCVWrappers::RawImage ^ rgb_image,
                                  FaceModelParameters ^ modelParams,
                                  OpenCVWrappers::RawImage ^ gray_image) {
  ...
  bool DetectFaceLandmarksInImage(OpenCVWrappers::RawImage ^ rgb_image,
                                  ...
                                  FaceModelParameters ^ modelParams,
                                  OpenCVWrappers::RawImage ^ gray_image) {
    cv::Rect_<float> bbox(bounding_box->Left, bounding_box->Top,
                          bounding_box->Width, bounding_box->Height);
    return ::LandmarkDetector::DetectLandmarksInImage(
        rgb_image->Mat, bbox, *clnf, *modelParams->getParams(),
        gray_image->Mat);
  }
  ...
  bool DetectFaceLandmarksInImage(OpenCVWrappers::RawImage ^ rgb_image,
    ...
    cv::Rect_<float> bbox(bounding_box->Left, bounding_box->Top,
                          bounding_box->Width, bounding_box->Height);
    return ::LandmarkDetector::DetectLandmarksInImage(
        rgb_image->Mat, bbox, *clnf, *modelParams->getParams(), cv::Mat());
  }

  void GetPoseWRTCamera(List<float> ^ pose, float fx, float fy, float cx,
                        float cy) {
  ...
  List<bool> ^
      ...
      List<System::Tuple<float, float> ^> ^
      CalculateVisibleLandmarks() {
        std::vector<cv::Point2f> vecLandmarks =
            ::LandmarkDetector::CalculateVisibleLandmarks(*clnf);

        auto landmarks = gcnew
        System::Collections::Generic::List<System::Tuple<float, float> ^>();
      ...
      List<System::Tuple<float, float> ^> ^
      CalculateAllLandmarks() {
        std::vector<cv::Point2f> vecLandmarks =
            ::LandmarkDetector::CalculateAllLandmarks(*clnf);
      ...
      CalculateAllEyeLandmarks() {
        std::vector<cv::Point2f> vecLandmarks =
            ::LandmarkDetector::CalculateAllEyeLandmarks(*clnf);
      ...
      CalculateAllEyeLandmarks3D(float fx, float fy, float cx, float cy) {
        std::vector<cv::Point3f> vecLandmarks =
            ::LandmarkDetector::Calculate3DEyeLandmarks(*clnf, fx, fy, cx, cy);

        auto landmarks = gcnew System::Collections::Generic::List<
            System::Tuple<float, float, float> ^>();
        for (cv::Point3f p : vecLandmarks) {
          landmarks->Add(
              gcnew System::Tuple<float, float, float>(p.x, p.y, p.z));
        }
      ...
      CalculateVisibleEyeLandmarks() {
        std::vector<cv::Point2f> vecLandmarks =
            ::LandmarkDetector::CalculateVisibleEyeLandmarks(*clnf);
      ...
      List<System::Tuple<float, float, float> ^> ^
      Calculate3DLandmarks(float fx, float fy, float cx, float cy) {
        ...
        for (int i = 0; i < shape3D.cols; ++i) {
          landmarks_3D->Add(gcnew System::Tuple<float, float, float>(
              shape3D.at<float>(0, i), shape3D.at<float>(1, i),
              shape3D.at<float>(2, i)));
        }

        return landmarks_3D;
      }

      List<System::Tuple<System::Windows::Point, System::Windows::Point> ^> ^
      CalculateBox(float fx, float fy, float cx, float cy) {
        ...
        for (std::pair<cv::Point2f, cv::Point2f> line : vecLines) {
          lines->Add(gcnew System::Tuple<System::Windows::Point,
                                         System::Windows::Point>(
              System::Windows::Point(line.first.x, line.first.y),
              System::Windows::Point(line.second.x, line.second.y)));
        }
  ...
  int GetNumModes() { return clnf->pdm.NumberOfModes(); }
  ...
  List<float> ^
      ...
      // Getting the rigid shape parameters describing face scale rotation and
      // translation (scale,rotx,roty,rotz,tx,ty)
      List<float> ^
      GetRigidParams() {
