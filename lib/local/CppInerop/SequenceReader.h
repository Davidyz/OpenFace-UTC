...
public:
  ...
  SequenceReader(System::String ^ filename, bool directory, float fx, float fy,
                 float cx, float cy) {
    m_sequence_capture = new Utilities::SequenceCapture();
  ...
  SequenceReader(int webcam_id, int width, int height, float fx, float fy,
                 float cx, float cy) {
    m_sequence_capture = new Utilities::SequenceCapture();

    bool success = m_sequence_capture->OpenWebcam(webcam_id, width, height, fx,
                                                  fy, cx, cy);
  ...
  double GetTimestamp() { return m_sequence_capture->time_stamp; }
  ...
  float GetFx() { return m_sequence_capture->fx; }

  float GetFy() { return m_sequence_capture->fy; }

  float GetCx() { return m_sequence_capture->cx; }

  float GetCy() { return m_sequence_capture->cy; }

  bool IsOpened() { return m_sequence_capture->IsOpened(); }

  bool IsWebcam() { return m_sequence_capture->IsWebcam(); }

  double GetFPS() { return m_sequence_capture->fps; }

  void Close() { m_sequence_capture->Close(); }
  ...
  ~SequenceReader() { this->!SequenceReader(); }
...
private:
  ...
  static Dictionary<System::String ^, List<System::Tuple<int, int> ^> ^> ^
      GetListingFromFile(std::string filename) {
        // Check what cameras have been written (using OpenCVs XML packages)
        cv::FileStorage fs_read(filename, cv::FileStorage::READ);

        auto managed_camera_list_initial = gcnew
        Dictionary<System::String ^, List<System::Tuple<int, int> ^> ^>();
        ...
        for (size_t idx = 0; idx < camera_node_list.size(); idx++) {
          ...
          auto resolutions = gcnew
          System::Collections::Generic::List<System::Tuple<int, int> ^>();
          ...
          managed_camera_list_initial[gcnew System::String(
              camera_name.c_str())] = resolutions;
        }
        fs_read.release();
        return managed_camera_list_initial;
      }

      static void WriteCameraListingToFile(
  ...
  // A utility for listing the currently connected cameras together with their
  // ID, name, subset of supported resolutions and a thumbnail
public:
  static List<
      System::Tuple<int, System::String ^, List<System::Tuple<int, int> ^> ^,
                    OpenCVWrappers::RawImage ^> ^> ^
      GetCameras(System::String ^ root_directory_m) {
        auto managed_camera_list =
            gcnew List<System::Tuple<int, System::String ^,
        ...
        // std::cout << "Number of cameras found: " << cameras.size() <<
        // std::endl;
        //// Print information about the devices
        // for (auto const &device : cameras) {
        //	std::cout << "== VIDEO DEVICE (id:" << device.first << ") ==" <<
        //std::endl; 	std::cout << "Name: " << device.second.deviceName <<
        //std::endl; 	std::cout << "Path: " << device.second.devicePath <<
        //std::endl;
        // }
        ...
        std::string root_directory =
            msclr::interop::marshal_as<std::string>(root_directory_m);
        auto camera_resolution_list =
            GetListingFromFile(root_directory + "camera_list.xml");
        ...
        for (size_t i = 0; i < num_cameras; ++i) {
          ...
          // Before trying the resolutions, check if the resolutions have
          // already been computed for the camera of interest
          std::string device_name = cameras[i].deviceName;
          System::String ^ device_name_m =
              gcnew System::String(device_name.c_str());
          if (camera_resolution_list->ContainsKey(device_name_m)) {
            ...
            for (size_t i = 0; i < common_resolutions.size(); ++i) {
              auto resolution = gcnew System::Tuple<int, int>(
                  common_resolutions[i].first, common_resolutions[i].second);
            ...
            WriteCameraListingToFile(camera_resolution_list,
                                     root_directory + "camera_list.xml");
          }

          OpenCVWrappers::RawImage ^ sample_img_managed =
              gcnew OpenCVWrappers::RawImage(sample_img);
          managed_camera_list->Add(
              gcnew System::Tuple<int, System::String ^,
                                  List<System::Tuple<int, int> ^> ^,
                                  OpenCVWrappers::RawImage ^>(
