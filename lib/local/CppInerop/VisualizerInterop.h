...
public:
  Visualizer(bool vis_track, bool vis_hog, bool vis_aligned, bool vis_aus) {
    m_visualizer =
        new Utilities::Visualizer(vis_track, vis_hog, vis_aligned, vis_aus);
  }
  ...
  void SetObservationGaze(System::Tuple<float, float, float> ^ gaze_direction0,
    ...
    m_visualizer->SetObservationGaze(gaze_direction0_cv, gaze_direction1_cv,
                                     landmarks_2D_cv, landmarks_3D_cv,
                                     confidence);
  }
  ...
  void SetObservationHOG(OpenCVWrappers::RawImage ^ observation_HOG,
                         int num_cols, int num_rows) {
    m_visualizer->SetObservationHOG(observation_HOG->Mat, num_cols, num_rows);
  }

  void SetObservationLandmarks(List<System::Tuple<float, float> ^> ^
                                   landmarks_2D,
                               double confidence, List<bool> ^ visibilities) {
    // Construct an OpenCV matrix from the landmarks
  ...
  void SetImage(OpenCVWrappers::RawImage ^ canvas, float fx, float fy, float cx,
                float cy) {
  ...
  OpenCVWrappers::RawImage ^
      GetHOGVis() {
        OpenCVWrappers::RawImage ^ hog_image =
            gcnew OpenCVWrappers::RawImage(m_visualizer->GetHOGVis());
        return hog_image;
      }
      ...
      ^
      GetVisImage() {
        OpenCVWrappers::RawImage ^ vis_image =
            gcnew OpenCVWrappers::RawImage(m_visualizer->GetVisImage());
        return vis_image;
      }
