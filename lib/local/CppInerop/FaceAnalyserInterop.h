...
public:
  ...
  void PostProcessOutputFile(System::String ^ file) {
    face_analyser->PostprocessOutputFile(
        msclr::interop::marshal_as<std::string>(file));
  }

  void AddNextFrame(OpenCVWrappers::RawImage ^ frame,
                    List<System::Tuple<float, float> ^> ^ landmarks,
                    bool success, bool online) {
    ...
    //(captured_image, face_model.detected_landmarks,
  ...
  System::Tuple<Dictionary<System::String ^, double> ^,
                Dictionary<System::String ^, double> ^> ^
      PredictStaticAUsAndComputeFeatures(OpenCVWrappers::RawImage ^ frame,
        ...
        for (int i = 0; i < landmarks->Count; ++i) {
          landmarks_mat.at<float>(i, 0) = landmarks[i]->Item1;
          landmarks_mat.at<float>(i + landmarks->Count, 0) =
              landmarks[i]->Item2;
        }

        face_analyser->PredictStaticAUsAndComputeFeatures(frame->Mat,
                                                          landmarks_mat);
        ...
        return gcnew System::Tuple<Dictionary<System::String ^, double> ^,
                                   Dictionary<System::String ^, double> ^>(
            au_intensities, au_occurences);
      }

      List<System::String ^> ^
      GetClassActionUnitsNames() {
      ...
      ^
      GetLatestAlignedFace() {
        OpenCVWrappers::RawImage ^ face_aligned_image =
            gcnew OpenCVWrappers::RawImage(*aligned_face);
      ...
      ^
      GetLatestHOGFeature() {
        OpenCVWrappers::RawImage ^ HOG_feature =
            gcnew OpenCVWrappers::RawImage(*hog_features);
        return HOG_feature;
      }

      // As the number of HOG rows and columns might not be known in advance,
      // have methods for querying them
      int GetHOGRows() {
    return *num_rows;
  }

  int GetHOGCols() { return *num_cols; }

  // The number of channels is always the same
  int GetHOGChannels() { return 31; }

  void Reset() { face_analyser->Reset(); }
  ...
  ~FaceAnalyserManaged() { this->!FaceAnalyserManaged(); }
};
} // namespace FaceAnalyser_Interop
