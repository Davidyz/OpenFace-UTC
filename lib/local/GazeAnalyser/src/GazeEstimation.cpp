...
cv::Point3f RaySphereIntersect(cv::Point3f rayOrigin, cv::Point3f rayDir,
                               cv::Point3f sphereOrigin, float sphereRadius) {
...
void GazeAnalysis::EstimateGaze(const LandmarkDetector::CLNF &clnf_model,
                                cv::Point3f &gaze_absolute, float fx, float fy,
                                float cx, float cy, bool left_eye) {
  ...
  for (size_t i = 0; i < clnf_model.hierarchical_models.size(); ++i) {
    if (left_eye &&
        clnf_model.hierarchical_model_names[i].compare("left_eye_28") == 0) {
      part = i;
    }
    if (!left_eye &&
  ...
  cv::Mat eyeLdmks3d =
      clnf_model.hierarchical_models[part].GetShape(fx, fy, cx, cy);
  ...
  cv::Mat eyeballCentreMat =
      (faceLdmks3d.row(36 + eyeIdx * 6) + faceLdmks3d.row(39 + eyeIdx * 6)) /
          2.0f +
...
cv::Vec2f GazeAnalysis::GetGazeAngle(cv::Point3f &gaze_vector_1,
                                     cv::Point3f &gaze_vector_2) {
