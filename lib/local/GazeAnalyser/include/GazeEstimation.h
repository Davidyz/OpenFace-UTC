...
void EstimateGaze(const LandmarkDetector::CLNF &clnf_model,
                  cv::Point3f &gaze_absolute, float fx, float fy, float cx,
                  float cy, bool left_eye);

// Getting the gaze angle in radians with respect to the world coordinates
// (camera plane), when looking ahead straight at camera plane the gaze angle
// will be (0,0)
