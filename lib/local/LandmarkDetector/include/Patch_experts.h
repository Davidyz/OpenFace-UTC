...
public:
  ...
  // The collection of CEN patch experts (for intensity images), the experts are
  // laid out scale->view->landmark
  ...
  void Response(std::vector<cv::Mat_<float>> &patch_expert_responses,
                ...
                const cv::Mat_<float> &params_local, int window_size,
                int scale);
...
private:
  ...
  bool
  Read_CCNF_patch_experts(std::string patchesFileLocation,
                          std::vector<cv::Vec3d> &centers,
                          std::vector<cv::Mat_<int>> &visibility,
                          std::vector<std::vector<CCNF_patch_expert>> &patches,
                          double &patchScaling);
  bool Read_CEN_patch_experts(
      std::string expert_location, std::vector<cv::Vec3d> &centers,
      std::vector<cv::Mat_<int>> &visibility,
