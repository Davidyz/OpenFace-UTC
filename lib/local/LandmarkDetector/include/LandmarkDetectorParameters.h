...
struct FaceModelParameters {
  ...
  // Landmark detection validator boundary for correct detection, the regressor
  // output 1 (perfect alignment) 0 (bad alignment),
  ...
  // How often should face detection be used to attempt reinitialisation, every
  // n frames (set to negative not to reinit)
  ...
  // only capable of detecting faces at least 70px across MTCNN detector is much
  // more accurate that the other two, and is even suitable for profile faces,
  // but it is somewhat slower
