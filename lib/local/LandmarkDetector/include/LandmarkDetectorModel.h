...
private:
  ...
  float NU_RLMS(cv::Vec6f &final_global, cv::Mat_<float> &final_local,
                ...
                const cv::Matx22f &sim_ref_to_img, int resp_size, int view_idx,
                bool rigid, int scale, cv::Mat_<float> &landmark_lhoods,
                const FaceModelParameters &parameters, bool compute_lhood);

  // Generating the weight matrix for the Weighted least squares
  void GetWeightMatrix(cv::Mat_<float> &WeightMatrix, int scale, int view_id,
                       const FaceModelParameters &parameters);
};
//===========================================================================
} // namespace LandmarkDetector
