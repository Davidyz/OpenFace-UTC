...
public:
  ...
  // For frontal faces can apply mirrored and non-mirrored experts at the same
  // time
  void ResponseSparse(const cv::Mat_<float> &area_of_interest_left,
...
void interpolationMatrix(cv::Mat_<float> &mapMatrix, int response_height,
                         int response_width, int input_width, int input_height);
