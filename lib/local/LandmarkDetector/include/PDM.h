...
// from motion or PCA) Only describes the model but does not contain an instance
// of it (no local or global parameters are stored here) Contains the utility
// functions to help manipulate the model
...
public:
  ...
  void Clamp(cv::Mat_<float> &params_local, cv::Vec6f &params_global,
  ...
  // Given the current parameters, and the computed delta_p compute the updated
  // parameters
