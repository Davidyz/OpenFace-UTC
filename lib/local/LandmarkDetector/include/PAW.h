...
/**
    A Piece-wise Affine Warp
        The ideas for this piece-wise affine triangular warping are taken from
   the Active appearance models revisited by Iain Matthews and Simon Baker in
   IJCV 2004 This is used for both validation of landmark detection, and for
   avatar animation
...
public:
  ...
  // Triangle index, indicating which triangle each of destination pixels lies
  // in
  ...
  PAW(const cv::Mat_<float> &destination_shape,
      const cv::Mat_<int> &triangulation);

  // The final optional argument allows for optimisation if the triangle indices
  // from previous frame are known (for tracking in video)
  PAW(const cv::Mat_<float> &destination_shape,
  ...
  // The actual warping
  void Warp(const cv::Mat &image_to_warp, cv::Mat &destination_image,
...
private:
  ...
  static bool sameSide(float x0, float y0, float x1, float y1, float x2,
                       float y2, float x3, float y3);
  static bool pointInTriangle(float x0, float y0, float x1, float y1, float x2,
                              float y2, float x3, float y3);
  static int findTriangle(const cv::Point_<float> &point,
