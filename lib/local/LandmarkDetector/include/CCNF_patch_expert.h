...
public:
  ...
  // allows us not to recompute the dft of the template each time, improving the
  // speed of tracking
  ...
  void Response(const cv::Mat_<float> &im, cv::Mat_<double> &im_dft,
...
public:
  ...
  void Read(std::ifstream &stream, std::vector<int> window_sizes,
            std::vector<std::vector<cv::Mat_<float>>> sigma_components);

  // actual work (can pass in an image and a potential depth image, if the CCNF
  // is trained with depth)
