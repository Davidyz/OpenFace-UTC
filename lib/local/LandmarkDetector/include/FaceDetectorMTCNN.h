...
public:
  ...
  // convolution is used (through matrix multiplication) or an FFT optimization
...
private:
  ...
  // Keeping some pre-allocated im2col data as malloc is a significant time cost
  ...
  // CNN: 0 - convolutional, 1 - max pooling, 2 - fully connected, 3 - prelu, 4
  // - sigmoid
...
public:
  ...
  // Given an image, orientation and detected landmarks output the result of the
  // appropriate regressor
  bool DetectFaces(std::vector<cv::Rect_<float>> &o_regions,
                   const cv::Mat &input_img, std::vector<float> &o_confidences,
                   int min_face = 60, float t1 = 0.6, float t2 = 0.7,
                   float t3 = 0.7);
