...
Patch_experts::Patch_experts(const Patch_experts &other)
    : patch_scaling(other.patch_scaling), centers(other.centers),
      ...
      cen_expert_intensity(other.cen_expert_intensity),
      early_term_weights(other.early_term_weights),
      early_term_biases(other.early_term_biases),
      early_term_cutoffs(other.early_term_cutoffs),
...
// Returns indices to landmarks that need to have patch responses computed
// (omits mirrored frontal landmarks for CEN as they will be computed together
// with their mirrored pair)
std::vector<int> Patch_experts::Collect_visible_landmarks(
    std::vector<std::vector<cv::Mat_<int>>> visibilities, int scale,
    int view_id, int n) {
    ...
    if (visibilities[scale][view_id].rows == n) {
      if (visibilities[scale][view_id].at<int>(i, 0) != 0) {
        ...
        if (!cen_expert_intensity.empty()) {
          ...
          if (view_id == 0) {
            // If the patch expert does not have values, means it's a mirrored
            // version and will be done in another part of a loop
...
// Additionally returns the transform from the image coordinates to the response
// coordinates (and vice versa). The computation also requires the current
// landmark locations to compute response around, the PDM corresponding to the
// desired model, and the parameters describing its instance Also need to
// provide the size of the area of interest and the desired scale of analysis
void Patch_experts::Response(
    std::vector<cv::Mat_<float>> &patch_expert_responses,
    cv::Matx22f &sim_ref_to_img, cv::Matx22f &sim_img_to_ref,
    const cv::Mat_<float> &grayscale_image, const PDM &pdm,
    const cv::Vec6f &params_global, const cv::Mat_<float> &params_local,
    int window_size, int scale) {

  int view_id = GetViewIdx(params_global, scale);

  int n = pdm.NumberOfPoints();

  // Compute the current landmark locations (around which responses will be
  // computed)
  ...
  // similarity and inverse similarity transform to and from image and reference
  // shape
  ...
  sim_img_to_ref =
      Utilities::AlignShapesWithScale(image_shape_2D, reference_shape_2D);
  ...
  if (use_ccnf) {
    ...
    for (size_t w_size = 0; w_size < this->sigma_components.size(); ++w_size) {
      if (!this->sigma_components[w_size].empty()) {
        if (window_size * window_size ==
            this->sigma_components[w_size][0].rows) {
    ...
    for (int lmark = 0; lmark < n; lmark++) {
      ...
      if (visibilities[scale][view_id].at<int>(lmark, 0)) {
        ...
        ccnf_expert_intensity[scale][view_id][lmark].ComputeSigmas(
            sigma_components, window_size);
      }
    }
  }
  ...
  if (use_cen) {
    ...
    interpolationMatrix(interp_mat, resp_size, resp_size,
                        area_of_interest_width, area_of_interest_height);
  }

  // We do not want to create threads for invisible landmarks, so construct an
  // index of visible ones
  std::vector<int> vis_lmk =
      Collect_visible_landmarks(visibilities, scale, view_id, n);

  // calculate the patch responses for every landmark (this is the heavy lifting
  // of landmark detection)
  parallel_for_(cv::Range(0, vis_lmk.size()), [&](const cv::Range &range) {
    for (int i = range.start; i < range.end; i++) {

      // Work out how big the area of interest has to be to get a response of
      // window size
      ...
      if (use_cen) {
        area_of_interest_width =
            window_size +
            cen_expert_intensity[scale][view_id][ind].width_support - 1;
        area_of_interest_height =
            window_size +
            cen_expert_intensity[scale][view_id][ind].height_support - 1;
      } else if (use_ccnf) {
        area_of_interest_width =
            window_size + ccnf_expert_intensity[scale][view_id][ind].width - 1;
        area_of_interest_height =
            window_size + ccnf_expert_intensity[scale][view_id][ind].height - 1;
      } else {
        area_of_interest_width =
            window_size + svr_expert_intensity[scale][view_id][ind].width - 1;
        area_of_interest_height =
            window_size + svr_expert_intensity[scale][view_id][ind].height - 1;
      }

      // scale and rotate to mean shape to reference frame
      cv::Mat sim = (cv::Mat_<float>(2, 3) << a1, -b1,
                     landmark_locations.at<float>(ind, 0) -
                         a1 * (area_of_interest_width - 1.0f) / 2.0f +
                         b1 * (area_of_interest_width - 1.0f) / 2.0f,
                     b1, a1,
                     landmark_locations.at<float>(ind + n, 0) -
                         a1 * (area_of_interest_width - 1.0f) / 2.0f -
                         b1 * (area_of_interest_width - 1.0f) / 2.0f);

      // Extract the region of interest around the current landmark location
      cv::Mat_<float> area_of_interest(area_of_interest_height,
                                       area_of_interest_width, 0.0f);

      cv::warpAffine(grayscale_image, area_of_interest, sim,
                     area_of_interest.size(),
                     cv::WARP_INVERSE_MAP + cv::INTER_LINEAR);

      // Get intensity response either from the SVR, CCNF, or CEN patch experts
      // (prefer CEN as they are the most accurate so far)
      if (!cen_expert_intensity.empty()) {

        int im2col_size =
            (area_of_interest_width * area_of_interest_height - 1) / 2;
        ...
        if (view_id == 0) {
          ...
          if (!cen_expert_intensity[scale][view_id][ind].biases.empty()) {
            ...
            if (mirror_id == ind) {
              ...
              cen_expert_intensity[scale][view_id][ind].ResponseSparse(
                  area_of_interest, empty, patch_expert_responses[ind], empty,
                  interp_mat, prealloc_mat, empty);
            } else {

              // Grab mirrored area of interest

              // scale and rotate to mean shape to reference frame
              cv::Mat sim_r = (cv::Mat_<float>(2, 3) << a1, -b1,
                               landmark_locations.at<float>(mirror_id, 0) -
                                   a1 * (area_of_interest_width - 1.0f) / 2.0f +
                                   b1 * (area_of_interest_width - 1.0f) / 2.0f,
                               b1, a1,
                               landmark_locations.at<float>(mirror_id + n, 0) -
                                   a1 * (area_of_interest_width - 1.0f) / 2.0f -
                                   b1 * (area_of_interest_width - 1.0f) / 2.0f);

              // Extract the region of interest around the current landmark
              // location
              cv::Mat_<float> area_of_interest_r(area_of_interest_height,
                                                 area_of_interest_width, 0.0f);

              cv::warpAffine(grayscale_image, area_of_interest_r, sim_r,
                             area_of_interest_r.size(),
                             cv::WARP_INVERSE_MAP + cv::INTER_LINEAR);

              cv::Mat_<float> prealloc_mat_right =
                  preallocated_im2col[mirror_id][im2col_size];

              cen_expert_intensity[scale][view_id][ind].ResponseSparse(
                  area_of_interest, area_of_interest_r,
                  patch_expert_responses[ind],
                  patch_expert_responses[mirror_id], interp_mat, prealloc_mat,
                  prealloc_mat_right);
          ...
          if (!cen_expert_intensity[scale][view_id][ind].biases.empty()) {
            ...
            // cen_expert_intensity[scale][view_id][ind].Response(area_of_interest,
            // patch_expert_responses[ind]);
          } else {
            cv::Mat_<float> empty(0, 0, 0.0f);
            cen_expert_intensity[scale][mirror_views.at<int>(
                view_id)][mirror_inds.at<int>(ind)]
                .ResponseSparse(empty, area_of_interest, empty,
                                patch_expert_responses[ind], interp_mat, empty,
                                prealloc_mat);
          }
        }
        ...
        ccnf_expert_intensity[scale][view_id][ind].ResponseOpenBlas(
            area_of_interest, patch_expert_responses[ind], prealloc_mat);

        preallocated_im2col[ind][im2col_size] = prealloc_mat;

        // Below is an alternative way to compute the same, but that uses FFT
        // instead of OpenBLAS
        // ccnf_expert_intensity[scale][view_id][ind].Response(area_of_interest,
        // patch_expert_responses[ind]);
        ...
        svr_expert_intensity[scale][view_id][ind].Response(
            area_of_interest, patch_expert_responses[ind]);
      }
    }
  });
}
...
//===========================================================================
bool Patch_experts::Read(
    std::vector<std::string> intensity_svr_expert_locations,
  ...
  for (int scale = 0; scale < num_intensity_svr; ++scale) {
    ...
    std::cout << "Reading the intensity SVR patch experts from: " << location
              << "....";
    bool success_read = Read_SVR_patch_experts(
        location, centers[scale], visibilities[scale],
        svr_expert_intensity[scale], patch_scaling[scale]);
    if (!success_read) {
      return false;
    }
  }
  ...
  for (int scale = 0; scale < num_intensity_ccnf; ++scale) {
    ...
    bool success_read = Read_CCNF_patch_experts(
        location, centers[scale], visibilities[scale],
        ccnf_expert_intensity[scale], patch_scaling[scale]);
  ...
  for (int scale = 0; scale < num_intensity_cen; ++scale) {
    ...
    std::cout << "Reading the intensity CEN patch experts from: " << location
              << "....";
    bool success_read = Read_CEN_patch_experts(
        location, centers[scale], visibilities[scale],
        cen_expert_intensity[scale], patch_scaling[scale]);
    if (!success_read) {
      return false;
    }
...
//=========================================//
bool Patch_experts::Read_SVR_patch_experts(
    std::string expert_location, std::vector<cv::Vec3d> &centers,
    std::vector<cv::Mat_<int>> &visibility,
    std::vector<std::vector<Multi_SVR_patch_expert>> &patches, double &scale) {
  ...
  if (patchesFile.is_open()) {
    ...
    // the visibility of points for each of the views (which verts are visible
    // at a specific view
...
//=========================================//
bool Patch_experts::Read_CCNF_patch_experts(
    std::string patchesFileLocation, std::vector<cv::Vec3d> &centers,
    std::vector<cv::Mat_<int>> &visibility,
    std::vector<std::vector<CCNF_patch_expert>> &patches,
    double &patchScaling) {

  std::ifstream patchesFile(patchesFileLocation.c_str(),
  ...
  if (patchesFile.is_open()) {
    ...
    // Read the possible SigmaInvs (without beta), this will be followed by
    // patch reading (this assumes all of them have the same type, and number of
    // betas)
    int num_win_sizes;
    int num_sigma_comp;
...
//=========================================//
bool Patch_experts::Read_CEN_patch_experts(
    std::string expert_location, std::vector<cv::Vec3d> &centers,
    std::vector<cv::Mat_<int>> &visibility,
    std::vector<std::vector<CEN_patch_expert>> &patches, double &scale) {

  std::ifstream patchesFile(expert_location.c_str(),
                            std::ios::in | std::ios::binary);
  ...
  if (patchesFile.is_open()) {
    ...
    std::cout << "Could not find CEN patch experts, for instructions of how to "
                 "download them, see "
