...
// Getting a head pose estimate from the currently detected landmarks, with
// appropriate correction due to the PDM assuming an orthographic camera which
// is only correct close to the centre of the image This method returns a
// corrected pose estimate with respect to world coordinates with camera at
// origin (0,0,0) The format returned is [Tx, Ty, Tz, Eul_x, Eul_y, Eul_z]
cv::Vec6f LandmarkDetector::GetPose(const CLNF &clnf_model, float fx, float fy,
                                    float cx, float cy) {
  if (!clnf_model.detected_landmarks.empty() &&
      clnf_model.params_global[0] != 0) {
    ...
    cv::Vec3f vec_rot(clnf_model.params_global[1], clnf_model.params_global[2],
                      clnf_model.params_global[3]);

    cv::solvePnP(landmarks_3D, landmarks_2D, camera_matrix, cv::Mat(), vec_rot,
                 vec_trans, true);

    cv::Vec3f euler = Utilities::AxisAngle2Euler(vec_rot);

    return cv::Vec6f(vec_trans[0], vec_trans[1], vec_trans[2], euler[0],
                     euler[1], euler[2]);
  } else {
...
// Getting a head pose estimate from the currently detected landmarks, with
// appropriate correction due to perspective projection This method returns a
// corrected pose estimate with respect to a point camera (NOTE not the world
// coordinates), which is useful to find out if the person is looking at a
// camera The format returned is [Tx, Ty, Tz, Eul_x, Eul_y, Eul_z]
cv::Vec6f LandmarkDetector::GetPoseWRTCamera(const CLNF &clnf_model, float fx,
  ...
  if (!clnf_model.detected_landmarks.empty() &&
    ...
    // Here we correct for the camera orientation, for this need to determine
    // the angle the camera makes with the head pose
    float z_x =
        cv::sqrt(vec_trans[0] * vec_trans[0] + vec_trans[2] * vec_trans[2]);
    float eul_x = atan2(vec_trans[1], z_x);

    float z_y =
        cv::sqrt(vec_trans[1] * vec_trans[1] + vec_trans[2] * vec_trans[2]);
    float eul_y = -atan2(vec_trans[0], z_y);

    cv::Matx33f camera_rotation =
        Utilities::Euler2RotationMatrix(cv::Vec3f(eul_x, eul_y, 0));
    ...
    cv::Vec3f euler_corrected =
        Utilities::RotationMatrix2Euler(corrected_rotation);

    return cv::Vec6f(vec_trans[0], vec_trans[1], vec_trans[2],
                     euler_corrected[0], euler_corrected[1],
                     euler_corrected[2]);
  } else {
    return cv::Vec6f(0, 0, 0, 0, 0, 0);
  }
...
// If landmark detection in video succeeded create a template for use in simple
// tracking
void UpdateTemplate(const cv::Mat_<uchar> &grayscale_image, CLNF &clnf_model) {
  cv::Rect_<float> bounding_box;
  clnf_model.pdm.CalcBoundingBox(bounding_box, clnf_model.params_global,
                                 clnf_model.params_local);

  // Make sure the box is not out of bounds
  cv::Rect_<int> bbox_tmp((int)bounding_box.x, (int)bounding_box.y,
                          (int)bounding_box.width, (int)bounding_box.height);
  bounding_box =
      bbox_tmp & cv::Rect(0, 0, grayscale_image.cols, grayscale_image.rows);

  clnf_model.face_template = grayscale_image(bounding_box).clone();
}

// This method uses basic template matching in order to allow for better
// tracking of fast moving faces
void CorrectGlobalParametersVideo(const cv::Mat_<uchar> &grayscale_image,
                                  CLNF &clnf_model,
                                  const FaceModelParameters &params) {
  cv::Rect_<float> init_box;
  clnf_model.pdm.CalcBoundingBox(init_box, clnf_model.params_global,
                                 clnf_model.params_local);

  cv::Rect roi(init_box.x - init_box.width / 2,
               init_box.y - init_box.height / 2, init_box.width * 2,
               init_box.height * 2);
  ...
  cv::Mat_<uchar> image;
  if (scaling < 1) {
    cv::resize(clnf_model.face_template, clnf_model.face_template, cv::Size(),
               scaling, scaling);
  ...
  cv::Mat corr_out;
  cv::matchTemplate(image, clnf_model.face_template, corr_out,
                    cv::TM_CCOEFF_NORMED);
  ...
  cv::Rect_<float> out_bbox(max_loc[1] / scaling + off_x,
                            max_loc[0] / scaling + off_y,
                            clnf_model.face_template.rows / scaling,
                            clnf_model.face_template.cols / scaling);
...
bool LandmarkDetector::DetectLandmarksInVideo(const cv::Mat &rgb_image,
                                              CLNF &clnf_model,
                                              FaceModelParameters &params,
                                              cv::Mat &grayscale_image) {
  // First need to decide if the landmarks should be "detected" or "tracked"
  // Detected means running face detection and a larger search area, tracked
  // means initialising from previous step and using a smaller search area

  if (grayscale_image.empty()) {
    Utilities::ConvertToGrayscale_8bit(rgb_image, grayscale_image);
  }

  // Indicating that this is a first detection in video sequence or after
  // restart
  ...
  if (clnf_model.tracking_initialised) {
    ...
    // The area of interest search size will depend if the previous track was
    // successful
    ...
    // Before the expensive landmark detection step apply a quick template
    // tracking approach
    if (params.use_face_template && !clnf_model.face_template.empty() &&
        clnf_model.detection_success) {
  ...
  // This is used for both detection (if it the tracking has not been
  // initialised yet) or if the tracking failed (however we do this every n
  // frames, for speed) This also has the effect of an attempt to reinitialise
  // just after the tracking has failed, which is useful during large motions
  if ((!clnf_model.tracking_initialised &&
       (clnf_model.failures_in_a_row + 1) % (params.reinit_video_every * 6) ==
           0) ||
      (clnf_model.tracking_initialised && !clnf_model.detection_success &&
       params.reinit_video_every > 0 &&
       clnf_model.failures_in_a_row % params.reinit_video_every == 0)) {

    cv::Rect_<float> bounding_box;

    // If the face detector has not been initialised and we're using it, then
    // read it in
    if (clnf_model.face_detector_HAAR.empty() &&
      ...
      clnf_model.haar_face_detector_location =
          params.haar_face_detector_location;
    }
    ...
    if (params.curr_face_detector == FaceModelParameters::HOG_SVM_DETECTOR) {
      float confidence;
      face_detection_success = LandmarkDetector::DetectSingleFaceHOG(
          bounding_box, grayscale_image, clnf_model.face_detector_HOG,
          confidence, preference_det);
    } else if (params.curr_face_detector ==
               FaceModelParameters::HAAR_DETECTOR) {
      face_detection_success = LandmarkDetector::DetectSingleFace(
          bounding_box, grayscale_image, clnf_model.face_detector_HAAR,
          preference_det);
    } else if (params.curr_face_detector ==
               FaceModelParameters::MTCNN_DETECTOR) {
      float confidence;
      face_detection_success = LandmarkDetector::DetectSingleFaceMTCNN(
          bounding_box, rgb_image, clnf_model.face_detector_MTCNN, confidence,
          preference_det);
    }

    // Attempt to detect landmarks using the detected face (if unseccessful the
    ...
    if (face_detection_success) {
      ...
      // Keep track of old model values so that they can be restored if
      // redetection fails
      ...
      cv::Mat_<float> detected_landmarks_init =
          clnf_model.detected_landmarks.clone();
      cv::Mat_<float> landmark_likelihoods_init =
          clnf_model.landmark_likelihoods.clone();

      // Use the detected bounding box and empty local parameters
      clnf_model.params_local.setTo(0);
      clnf_model.pdm.CalcParams(clnf_model.params_global, bounding_box,
                                clnf_model.params_local);

      // Make sure the search size is large
      params.window_sizes_current = params.window_sizes_init;

      // TODO rem (should the multi-hyp version be only for CEN and not CLNF?),
      // otherwise poss too slow, and poss not accurate
      ...
      // Perform multi-hypothesis detection here (as face detector can pick up
      // multiple of them)
      params.multi_view = true;
      bool landmark_detection_success = DetectLandmarksInImage(
          rgb_image, bounding_box, clnf_model, params, grayscale_image);
      params.multi_view = false;

      // If landmark reinitialisation unsucessful continue from previous
      // estimates if it's initial detection however, do not care if it was
      // successful as the validator might be wrong, so continue trackig
      // regardless
      if (!initial_detection && !landmark_detection_success) {
        ...
        clnf_model.pdm.CalcShape2D(clnf_model.detected_landmarks,
                                   clnf_model.params_local,
                                   clnf_model.params_global);
...
bool LandmarkDetector::DetectLandmarksInVideo(
    const cv::Mat &rgb_image, const cv::Rect_<double> bounding_box,
    CLNF &clnf_model, FaceModelParameters &params, cv::Mat &grayscale_image) {
  if (bounding_box.width > 0) {
    // calculate the local and global parameters from the generated 2D shape
    // (mapping from the 2D to 3D because camera params are unknown)
    clnf_model.params_local.setTo(0);
    clnf_model.pdm.CalcParams(clnf_model.params_global, bounding_box,
                              clnf_model.params_local);
...
bool DetectLandmarksInImageMultiHypBasic(
  ...
  std::vector<cv::Vec6f> best_global_parameters_h(
      clnf_model.hierarchical_models.size());
  std::vector<cv::Mat_<float>> best_local_parameters_h(
      clnf_model.hierarchical_models.size());
  std::vector<cv::Mat_<float>> best_detected_landmarks_h(
      clnf_model.hierarchical_models.size());
  std::vector<cv::Mat_<float>> best_landmark_likelihoods_h(
      clnf_model.hierarchical_models.size());

  for (size_t hypothesis = 0; hypothesis < rotation_hypotheses.size();
       ++hypothesis) {
    // Reset the potentially set clnf_model parameters
    clnf_model.params_local.setTo(0.0);

    for (size_t part = 0; part < clnf_model.hierarchical_models.size();
         ++part) {
    ...
    clnf_model.pdm.CalcParams(clnf_model.params_global, bounding_box,
                              clnf_model.params_local,
                              rotation_hypotheses[hypothesis]);
    ...
    if (hypothesis == 0 || best_likelihood < clnf_model.model_likelihood) {
      ...
      for (size_t part = 0; part < clnf_model.hierarchical_models.size();
           ++part) {
        best_likelihood_h[part] =
            clnf_model.hierarchical_models[part].model_likelihood;
        best_global_parameters_h[part] =
            clnf_model.hierarchical_models[part].params_global;
        best_local_parameters_h[part] =
            clnf_model.hierarchical_models[part].params_local.clone();
        best_detected_landmarks_h[part] =
            clnf_model.hierarchical_models[part].detected_landmarks.clone();
        best_landmark_likelihoods_h[part] =
            clnf_model.hierarchical_models[part].landmark_likelihoods.clone();
      }
    }
  }
  ...
  for (size_t part = 0; part < clnf_model.hierarchical_models.size(); ++part) {
    clnf_model.hierarchical_models[part].params_global =
        best_global_parameters_h[part];
    clnf_model.hierarchical_models[part].params_local =
        best_local_parameters_h[part].clone();
    clnf_model.hierarchical_models[part].detected_landmarks =
        best_detected_landmarks_h[part].clone();
    clnf_model.hierarchical_models[part].landmark_likelihoods =
        best_landmark_likelihoods_h[part].clone();
  }
...
bool DetectLandmarksInImageMultiHypEarlyTerm(
    const cv::Mat_<uchar> &grayscale_image,
    std::vector<cv::Vec3d> rotation_hypotheses,
    const cv::Rect_<double> bounding_box, CLNF &clnf_model,
    FaceModelParameters &params) {
  ...
  for (size_t hypothesis = 0; hypothesis < rotation_hypotheses.size();
    ...
    float lhood =
        clnf_model.model_likelihood *
            clnf_model.patch_experts.early_term_weights[clnf_model.view_used] +
        clnf_model.patch_experts.early_term_biases[clnf_model.view_used];

    // If likelihood higher than cutoff continue on this model
    if (lhood >
        clnf_model.patch_experts.early_term_cutoffs[clnf_model.view_used]) {
...
// This is the one where the actual work gets done, other DetectLandmarksInImage
// calls lead to this one
bool LandmarkDetector::DetectLandmarksInImage(
    const cv::Mat &rgb_image, const cv::Rect_<double> bounding_box,
    CLNF &clnf_model, FaceModelParameters &params, cv::Mat &grayscale_image) {
  ...
  if (params.multi_view) {
    ...
    // It is possible to add other orientation hypotheses easilly by just
    // pushing to this vector
  ...
  // Either use basic multi-hypothesis testing or clever testing if early
  // termination parameters are present
  if (clnf_model.patch_experts.early_term_biases.size() == 0) {
    success = DetectLandmarksInImageMultiHypBasic(
        grayscale_image, rotation_hypotheses, bounding_box, clnf_model, params);
  } else {
    success = DetectLandmarksInImageMultiHypEarlyTerm(
        grayscale_image, rotation_hypotheses, bounding_box, clnf_model, params);
  }
  return success;
}

bool LandmarkDetector::DetectLandmarksInImage(const cv::Mat &rgb_image,
                                              CLNF &clnf_model,
                                              FaceModelParameters &params,
                                              cv::Mat &grayscale_image) {
  if (grayscale_image.empty()) {
  ...
  if (clnf_model.face_detector_HAAR.empty() &&
      params.curr_face_detector == FaceModelParameters::HAAR_DETECTOR) {
    clnf_model.face_detector_HAAR.load(params.haar_face_detector_location);
    clnf_model.haar_face_detector_location = params.haar_face_detector_location;
  }

  if (clnf_model.face_detector_MTCNN.empty() &&
      params.curr_face_detector == FaceModelParameters::MTCNN_DETECTOR) {
    clnf_model.face_detector_MTCNN.Read(params.mtcnn_face_detector_location);
    clnf_model.mtcnn_face_detector_location =
        params.mtcnn_face_detector_location;
  ...
  if (params.curr_face_detector == FaceModelParameters::HOG_SVM_DETECTOR) {
    float confidence;
    LandmarkDetector::DetectSingleFaceHOG(bounding_box, grayscale_image,
                                          clnf_model.face_detector_HOG,
                                          confidence);
  } else if (params.curr_face_detector == FaceModelParameters::HAAR_DETECTOR) {
    LandmarkDetector::DetectSingleFace(bounding_box, rgb_image,
                                       clnf_model.face_detector_HAAR);
  } else if (params.curr_face_detector == FaceModelParameters::MTCNN_DETECTOR) {
    float confidence;
    LandmarkDetector::DetectSingleFaceMTCNN(
        bounding_box, rgb_image, clnf_model.face_detector_MTCNN, confidence);
  }
  ...
  if (bounding_box.width == 0) {
    ...
    return DetectLandmarksInImage(rgb_image, bounding_box, clnf_model, params,
                                  grayscale_image);
  }
}
