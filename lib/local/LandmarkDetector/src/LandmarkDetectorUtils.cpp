...
void crossCorr_m(const cv::Mat_<float> &img, cv::Mat_<double> &img_dft,
                 const cv::Mat_<float> &_templ,
                 std::map<int, cv::Mat_<double>> &_templ_dfts,
                 cv::Mat_<float> &corr) {
  ...
  if (_templ_dfts.find(dftsize.width) == _templ_dfts.end()) {
    ...
    cv::Mat_<double> dst(dftTempl,
                         cv::Rect(0, 0, dftsize.width, dftsize.height));
    ...
    if (dst.cols > _templ.cols) {
      cv::Mat_<double> part(dst, cv::Range(0, _templ.rows),
                            cv::Range(_templ.cols, dst.cols));
      part.setTo(0);
    }
  ...
  cv::Size bsz(std::min(blocksize.width, corr.cols),
...
void matchTemplate_m(const cv::Mat_<float> &input_img,
                     cv::Mat_<double> &img_dft, cv::Mat &_integral_img,
                     cv::Mat &_integral_img_sq, const cv::Mat_<float> &templ,
  ...
  int numType = method == cv::TM_CCORR || method == cv::TM_CCORR_NORMED     ? 0
                : method == cv::TM_CCOEFF || method == cv::TM_CCOEFF_NORMED ? 1
                                                                            : 2;
  ...
  if (result.empty()) {
    cv::Size corrSize(input_img.cols - templ.cols + 1,
                      input_img.rows - templ.rows + 1);
  ...
  if (method == cv::TM_CCOEFF) {
    ...
    templSum2 = templNorm + templMean[0] * templMean[0] +
                templMean[1] * templMean[1] + templMean[2] * templMean[2] +
...
// Useful utility for grabing a bounding box around a set of 2D landmarks (as a
// 1D 2n x 1 vector of xs followed by doubles or as an n x 2 vector)
void ExtractBoundingBox(const cv::Mat_<float> &landmarks, float &min_x,
...
std::vector<cv::Point2f> CalculateAllLandmarks(const cv::Mat_<float> &shape2D) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (shape2D.cols == 1) {
      featurePoint =
          cv::Point2f(shape2D.at<float>(i), shape2D.at<float>(i + n));
    } else {
      featurePoint =
          cv::Point2f(shape2D.at<float>(i, 0), shape2D.at<float>(i, 1));
    }
...
std::vector<cv::Point2f> CalculateVisibleLandmarks(const CLNF &clnf_model) {
  ...
  if (clnf_model.detection_success) {
    ...
    // Because we only draw visible points, need to find which points patch
    // experts consider visible at a certain orientation
    return CalculateVisibleLandmarks(
        clnf_model.detected_landmarks,
        clnf_model.patch_experts.visibilities[0][idx]);
  } else {
...
std::vector<cv::Point3f> Calculate3DEyeLandmarks(const CLNF &clnf_model,
  ...
  for (size_t i = 0; i < clnf_model.hierarchical_models.size(); ++i) {
    ...
    if (clnf_model.hierarchical_model_names[i].compare("left_eye_28") == 0 ||
      ...
      for (int lmk = 0; lmk < num_landmarks; ++lmk) {
        cv::Point3f curr_lmk(lmks.at<float>(0, lmk), lmks.at<float>(1, lmk),
                             lmks.at<float>(2, lmk));
...
bool DetectFaces(std::vector<cv::Rect_<float>> &o_regions,
                 const cv::Mat_<uchar> &intensity, float min_width,
                 cv::Rect_<float> roi) {
...
bool DetectFaces(std::vector<cv::Rect_<float>> &o_regions,
                 ...
                 cv::CascadeClassifier &classifier, float min_width,
                 cv::Rect_<float> roi) {

  std::vector<cv::Rect> face_detections;
  if (min_width == -1) {
    classifier.detectMultiScale(intensity, face_detections, 1.2, 2, 0,
                                cv::Size(50, 50));
  } else {
    classifier.detectMultiScale(intensity, face_detections, 1.2, 2, 0,
                                cv::Size(min_width, min_width));
  }
  ...
  for (size_t face = 0; face < face_detections.size(); ++face) {
    ...
    // CLNF detector expects the bounding box to encompass from eyebrow to chin
    // in y, and from cheeck outline to cheeck outline in x, so we need to
    // compensate

    // The scalings were learned using the Face Detections on LFPW, Helen, AFW
    // and iBUG datasets, using ground truth and detections from openCV
    ...
    // Shift face down as OpenCV Haar Cascade detects the forehead as well, and
    // we're not interested
    region.y = face_detections[face].y + face_detections[face].height * 0.2166f;

    if (min_width != -1) {
      if (region.width < min_width ||
          region.x < ((float)intensity.cols) * roi.x ||
          region.y < ((float)intensity.cols) * roi.y ||
          region.x + region.width >
              ((float)intensity.cols) * (roi.x + roi.width) ||
          region.y + region.height >
              ((float)intensity.rows) * (roi.y + roi.height))
        continue;
    }
...
bool DetectSingleFace(cv::Rect_<float> &o_region,
                      const cv::Mat_<uchar> &intensity_image,
                      cv::CascadeClassifier &classifier, cv::Point preference,
                      float min_width, cv::Rect_<float> roi) {
  // The tracker can return multiple faces
  std::vector<cv::Rect_<float>> face_detections;

  bool detect_success = LandmarkDetector::DetectFaces(
      face_detections, intensity_image, classifier, min_width, roi);
    ...
    if (face_detections.size() > 1) {
      ...
      for (size_t i = 0; i < face_detections.size(); ++i) {
        ...
        if (use_preferred) {
          dist =
              sqrt((preference.x) *
                       (face_detections[i].width / 2 + face_detections[i].x) +
                   (preference.y) *
...
bool DetectFacesHOG(std::vector<cv::Rect_<float>> &o_regions,
                    const cv::Mat_<uchar> &intensity,
                    std::vector<float> &confidences, float min_width,
                    cv::Rect_<float> roi) {
  dlib::frontal_face_detector detector = dlib::get_frontal_face_detector();

  return DetectFacesHOG(o_regions, intensity, detector, confidences, min_width,
                        roi);
}
...
bool DetectFacesHOG(std::vector<cv::Rect_<float>> &o_regions,
                    ...
                    std::vector<float> &o_confidences, float min_width,
                    cv::Rect_<float> roi) {
  ...
  cv::resize(intensity, upsampled_intensity,
             cv::Size((int)(intensity.cols * scaling),
                      (int)(intensity.rows * scaling)));
  ...
  // Convert from int bounding box do a double one with corrections
  for (size_t face = 0; face < face_detections.size(); ++face) {
    // CLNF expects the bounding box to encompass from eyebrow to chin in y, and
    // from cheeck outline to cheeck outline in x, so we need to compensate

    cv::Rect_<float> region;
    // Move the face slightly to the right (as the width was made smaller)
    region.x = (face_detections[face].rect.get_rect().tl_corner().x() +
                0.0389f * face_detections[face].rect.get_rect().width()) /
               scaling;
    // Shift face down as OpenCV Haar Cascade detects the forehead as well, and
    // we're not interested
    region.y = (face_detections[face].rect.get_rect().tl_corner().y() +
                0.1278f * face_detections[face].rect.get_rect().height()) /
               scaling;

    // Correct for scale
    region.width =
        (face_detections[face].rect.get_rect().width() * 0.9611) / scaling;
    region.height =
        (face_detections[face].rect.get_rect().height() * 0.9388) / scaling;
...
bool DetectSingleFaceHOG(cv::Rect_<float> &o_region,
  ...
  bool detect_success = LandmarkDetector::DetectFacesHOG(
      face_detections, intensity_img, detector, confidences, min_width, roi);
  ...
  if (detect_success) {
    ...
    if (use_preferred) {
      best_so_far =
          sqrt((preference.x -
                (face_detections[0].width / 2 + face_detections[0].x)) *
                   (preference.x -
                    (face_detections[0].width / 2 + face_detections[0].x)) +
               (preference.y -
      ...
      best_so_far =
          (face_detections[0].width + face_detections[0].height) / 2.0;
    } else {
    ...
    // if not detected
    o_region = cv::Rect_<float>(0, 0, 0, 0);
    // A completely unreliable detection (shouldn't really matter what is
    // returned here)
    confidence = -2;
  }
  return detect_success;
...
bool DetectSingleFaceMTCNN(cv::Rect_<float> &o_region, const cv::Mat &image,
                           LandmarkDetector::FaceDetectorMTCNN &detector,
                           float &confidence, cv::Point preference) {
  // The tracker can return multiple faces
  std::vector<cv::Rect_<float>> face_detections;
