...
CLNF::CLNF(const CLNF &other)
    : pdm(other.pdm), params_local(other.params_local.clone()),
      ...
      preference_det(other.preference_det),
  ...
  // Make sure the matrices are allocated properly
  for (std::map<int, cv::Mat_<float>>::const_iterator it =
           other.kde_resp_precalc.begin();
       it != other.kde_resp_precalc.end(); it++) {
    // Make sure the matrix is copied.
    this->kde_resp_precalc.insert(
        std::pair<int, cv::Mat_<float>>(it->first, it->second.clone()));
  }
}
...
CLNF &CLNF::operator=(const CLNF &other) {
  if (this != &other) // protect against invalid self-assignment
  {
    ...
    // Load the CascadeClassifier (as it does not have a proper copy
    // constructor)
    if (!haar_face_detector_location.empty()) {
      this->face_detector_HAAR.load(haar_face_detector_location);
    }
...
bool CLNF::Read_CLNF(std::string clnf_location) {
  ...
  bool read_success =
      patch_experts.Read(intensity_expert_locations, ccnf_expert_locations,
                         cen_expert_locations, early_term_loc);
...
void CLNF::Read(std::string main_location) {
  ...
  std::cout << "Reading the landmark detector/tracker from: " << main_location
            << std::endl;
  ...
  while (!locations.eof()) {
    ...
    if (module.compare("LandmarkDetector") == 0) {
      std::cout << "Reading the landmark detector module from: " << location
                << std::endl;
      ...
      if (part_name.compare("left_eye") == 0 ||
          part_name.compare("right_eye") == 0) {
...
void CLNF::Reset(double x, double y) {
  ...
  // Now in the following frame when face detection takes place this is the
  // point at which it will be preffered
...
bool CLNF::DetectLandmarks(const cv::Mat_<uchar> &image,
  ...
  if (params.refine_hierarchical && hierarchical_models.size() > 0) {
    ...
    parallel_for_(
        cv::Range(0, hierarchical_models.size()), [&](const cv::Range &range) {
          for (int part_model = range.start; part_model < range.end;
            ...
            int n_part_points =
                hierarchical_models[part_model].pdm.NumberOfPoints();

            std::vector<std::pair<int, int>> mappings =
                this->hierarchical_mapping[part_model];

            cv::Mat_<float> part_model_locs(n_part_points * 2, 1, 0.0f);

            // Extract the corresponding landmarks
            for (size_t mapping_ind = 0; mapping_ind < mappings.size();
                 ++mapping_ind) {
              part_model_locs.at<float>(mappings[mapping_ind].second) =
                  detected_landmarks.at<float>(mappings[mapping_ind].first);
              part_model_locs.at<float>(mappings[mapping_ind].second +
                                        n_part_points) =
                  detected_landmarks.at<float>(mappings[mapping_ind].first +
                                               this->pdm.NumberOfPoints());
            }

            // Fit the part based model PDM
            hierarchical_models[part_model].pdm.CalcParams(
                hierarchical_models[part_model].params_global,
                hierarchical_models[part_model].params_local, part_model_locs);

            // Only do this if we don't need to upsample
            if (params_global[0] > 0.9 * hierarchical_models[part_model]
                                             .patch_experts.patch_scaling[0]) {
              parts_used = true;

              this->hierarchical_params[part_model].window_sizes_current =
                  this->hierarchical_params[part_model].window_sizes_init;
    ...
    if (parts_used) {
      ...
      for (size_t part_model = 0; part_model < hierarchical_models.size();
           ++part_model) {
        std::vector<std::pair<int, int>> mappings =
            this->hierarchical_mapping[part_model];
        ...
        for (size_t mapping_ind = 0; mapping_ind < mappings.size();
             ++mapping_ind) {
          detected_landmarks.at<float>(mappings[mapping_ind].first) =
              hierarchical_models[part_model].detected_landmarks.at<float>(
                  mappings[mapping_ind].second);
          detected_landmarks.at<float>(mappings[mapping_ind].first +
                                       pdm.NumberOfPoints()) =
              hierarchical_models[part_model].detected_landmarks.at<float>(
                  mappings[mapping_ind].second +
...
//=============================================================================
bool CLNF::Fit(const cv::Mat_<float> &im, const std::vector<int> &window_sizes,
  ...
  // Converting from image space to patch expert space (normalised for rotation
  // and scale)
  ...
  // Optimise the model across a number of areas of interest (usually in
  // descending window size and ascending scale size)
  for (int scale = 0; scale < num_scales; scale++) {
    ...
    patch_experts.Response(patch_expert_responses, sim_ref_to_img,
                           sim_img_to_ref, im, pdm, params_global, params_local,
                           window_size, scale);

    if (parameters.refine_parameters == true) {
      int scale_max = scale >= 2 ? 2 : scale;

      // Adapt the parameters based on scale (wan't to reduce regularisation as
      // scale increases, but increase sigma and Tikhonov)
      tmp_parameters.reg_factor =
          parameters.reg_factor -
          15 * log(patch_experts.patch_scaling[scale_max] / 0.25) / log(2);

      if (tmp_parameters.reg_factor <= 0)
        tmp_parameters.reg_factor = 0.001;

      tmp_parameters.sigma =
          parameters.sigma +
          0.25 * log(patch_experts.patch_scaling[scale_max] / 0.25) / log(2);
      tmp_parameters.weight_factor =
          parameters.weight_factor +
          2 * parameters.weight_factor *
              log(patch_experts.patch_scaling[scale_max] / 0.25) / log(2);
    }
    ...
    this->NU_RLMS(params_global, params_local, patch_expert_responses,
                  cv::Vec6f(params_global), params_local.clone(), current_shape,
                  sim_img_to_ref, sim_ref_to_img, window_size, view_id, true,
                  scale, this->landmark_likelihoods, tmp_parameters, false);

    // non-rigid optimisation

    // If we are terminating next iteration, make sure to record the model
    // likelihood
    if (scale == num_scales - 1 || window_sizes[scale + 1] == 0 ||
        params_global[0] < 0.30) {
      this->model_likelihood = this->NU_RLMS(
          params_global, params_local, patch_expert_responses,
          cv::Vec6f(params_global), params_local.clone(), current_shape,
          sim_img_to_ref, sim_ref_to_img, window_size, view_id, false, scale,
          this->landmark_likelihoods, tmp_parameters, true);
    } else {
      this->NU_RLMS(params_global, params_local, patch_expert_responses,
                    cv::Vec6f(params_global), params_local.clone(),
                    current_shape, sim_img_to_ref, sim_ref_to_img, window_size,
                    view_id, false, scale, this->landmark_likelihoods,
                    tmp_parameters, false);
    }
    ...
    if (active_scale < num_scales - 1 &&
        0.9 * patch_experts.patch_scaling[active_scale + 1] < params_global[0])
...
void CLNF::NonVectorisedMeanShift_precalc_kde(
    ...
    const std::vector<cv::Mat_<float>> &patch_expert_responses,
  ...
  if (kde_resp_precalc.find(resp_size) == kde_resp_precalc.end()) {
    kde_resp = cv::Mat_<float>(
        (int)((resp_size / step_size) * (resp_size / step_size)),
        resp_size * resp_size);
  ...
  for (int i = 0; i < n; i++) {
    ...
    // Pick the row from precalculated kde that approximates the current dx, dy
    // best
    int closest_col =
        (int)(dy / step_size +
              0.5); // Plus 0.5 is there, as C++ rounds down with int cast
    int closest_row =
        (int)(dx / step_size +
              0.5); // Plus 0.5 is there, as C++ rounds down with int cast

    int idx =
        closest_row * ((int)(resp_size / step_size + 0.5)) +
        closest_col; // Plus 0.5 is there, as C++ rounds down with int cast
    ...
    for (int ii = 0; ii < resp_size; ii++) {
      for (int jj = 0; jj < resp_size; jj++) {
        ...
        // the KDE evaluation of that point multiplied by the probability at the
        // current, xi, yi
        float v = (*p++) * (*kde_it++);
...
void CLNF::GetWeightMatrix(cv::Mat_<float> &WeightMatrix, int scale,
                           int view_id, const FaceModelParameters &parameters) {
  int n = pdm.NumberOfPoints();
    ...
    for (int p = 0; p < n; p++) {
      if (!patch_experts.cen_expert_intensity.empty()) {
        ...
        WeightMatrix.at<float>(p, p) =
            WeightMatrix.at<float>(p, p) +
            patch_experts.cen_expert_intensity[scale][view_id][p].confidence;
        ...
        WeightMatrix.at<float>(p, p) =
            WeightMatrix.at<float>(p, p) +
            patch_experts.ccnf_expert_intensity[scale][view_id][p]
                .patch_confidence;
        ...
        for (size_t pc = 0;
             pc < patch_experts.svr_expert_intensity[scale][view_id][p]
                      .svr_patch_experts.size();
             pc++) {
          // for the x dimension
          WeightMatrix.at<float>(p, p) =
              WeightMatrix.at<float>(p, p) +
              patch_experts.svr_expert_intensity[scale][view_id][p]
                  .svr_patch_experts.at(pc)
                  .confidence;
        }
        // for the y dimension
        WeightMatrix.at<float>(p + n, p + n) = WeightMatrix.at<float>(p, p);
...
//=============================================================================
float CLNF::NU_RLMS(cv::Vec6f &final_global, cv::Mat_<float> &final_local,
                    const std::vector<cv::Mat_<float>> &patch_expert_responses,
                    ...
                    const cv::Matx22f &sim_ref_to_img, int resp_size,
                    int view_id, bool rigid, int scale,
  ...
  if (rigid) {
    ...
    cv::Mat(parameters.reg_factor / E)
        .copyTo(regularisations(cv::Rect(6, 0, m, 1)));
  ...
  for (int iter = 0; iter < parameters.num_optimisation_iteration; iter++) {
    ...
    // calculate the appropriate Jacobians in 2D, even though the actual
    // behaviour is in 3D, using small angle approximation and oriented shape
    if (rigid) {
      pdm.ComputeRigidJacobian(current_local, current_global, J, WeightMatrix,
                               J_w_t);
    } else {
      pdm.ComputeJacobian(current_local, current_global, J, WeightMatrix,
                          J_w_t);
    }
    ...
    NonVectorisedMeanShift_precalc_kde(mean_shifts, patch_expert_responses, dxs,
                                       dys, resp_size, a, scale, view_id,
                                       kde_resp_precalc);

    // Now transform the mean shifts to the the image reference frame, as
    // opposed to one of ref shape (object space)
    ...
    // projection of the meanshifts onto the jacobians (using the weighted
    // Jacobian, see Baltrusaitis 2013)
    ...
    char N[2];
    N[0] = 'N';
    sgemm_(N, N, &J.cols, &J_w_t.rows, &J_w_t.cols, &alpha1, (float *)J.data,
           &J.cols, (float *)J_w_t.data, &J_w_t.cols, &beta1,
           (float *)Hessian.data, &J.cols);

    // Above is a fast (but ugly) version of
    // cv::Mat_<float> Hessian = J_w_t * J + regTerm;

    // Solve for the parameter update (from Baltrusaitis 2013 based on eq (36)
    // Saragih 2011)
...
// Getting a 3D shape model from the current detected landmarks (in camera
// space)
cv::Mat_<float> CLNF::GetShape(float fx, float fy, float cx, float cy) const {
  ...
  cv::Matx33f R = Utilities::Euler2RotationMatrix(
      cv::Vec3f((float)params_global[1], (float)params_global[2],
                (float)params_global[3]));

  shape3d = shape3d.reshape(1, 3);

  shape3d = shape3d.t() * cv::Mat(R).t();

  // from the weak perspective model can determine the average depth of the
  // object
  float Zavg = fx / (float)params_global[0];

  cv::Mat_<float> outShape(n, 3, 0.0f);

  // this is described in the paper in section 3.4 (equation 10) (of the CLM-Z
  // paper)
  for (int i = 0; i < n; i++) {
...
cv::Mat_<int> CLNF::GetVisibilities() const {
  ...
  cv::Mat_<int> visibilities_to_ret =
      this->patch_experts.visibilities[scale][view_id].clone();
