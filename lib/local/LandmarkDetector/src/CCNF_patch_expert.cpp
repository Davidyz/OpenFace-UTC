...
CCNF_neuron::CCNF_neuron(const CCNF_neuron &other)
  ...
  for (std::map<int, cv::Mat_<double>>::const_iterator it =
           other.weights_dfts.begin();
       it != other.weights_dfts.end(); it++) {
    // Make sure the matrix is copied.
    this->weights_dfts.insert(
        std::pair<int, cv::Mat>(it->first, it->second.clone()));
  }
}

// Copy constructor
CCNF_patch_expert::CCNF_patch_expert(const CCNF_patch_expert &other)
    : neurons(other.neurons), window_sizes(other.window_sizes),
      betas(other.betas) {
  ...
  for (std::vector<cv::Mat_<float>>::const_iterator it = other.Sigmas.begin();
       it != other.Sigmas.end(); it++) {
    // Make sure the matrix is copied.
...
void CCNF_patch_expert::ComputeSigmas(
    std::vector<cv::Mat_<float>> sigma_components, int window_size) {
  for (size_t i = 0; i < window_sizes.size(); ++i) {
    if (window_sizes[i] == window_size)
      return;
  }
  // Each of the landmarks will have the same connections, hence constant number
  // of sigma components
  ...
  cv::Mat_<float> q1 =
      sum_alphas * cv::Mat_<float>::eye(window_size * window_size,
                                        window_size * window_size);

  cv::Mat_<float> q2 = cv::Mat_<float>::zeros(window_size * window_size,
                                              window_size * window_size);
...
// adding a bias term
void im2colContrastNormBias(const cv::Mat_<float> &input,
                            const unsigned int width, const unsigned int height,
                            cv::Mat_<float> &output) {
  const unsigned int m = input.rows;
  const unsigned int n = input.cols;

  // determine how many blocks there will be with a sliding window of width x
  // height in the input
...
//===========================================================================
void CCNF_neuron::Response(const cv::Mat_<float> &im, cv::Mat_<double> &im_dft,
  ...
  if (neuron_type == 3) {
    // Perform normalisation across whole patch (ignoring the invalid values
    // indicated by <= 0
  ...
  if (neuron_type == 3) {
    ...
    matchTemplate_m(
        I, im_dft, integral_img, integral_img_sq, weights, weights_dfts, resp,
        cv::TM_CCOEFF); // the linear multiplication, efficient calc of response
  } else {
    matchTemplate_m(I, im_dft, integral_img, integral_img_sq, weights,
                    weights_dfts, resp,
                    cv::TM_CCOEFF_NORMED); // the linear multiplication,
                                           // efficient calc of response
  }
...
//===========================================================================
void CCNF_patch_expert::Read(
    std::ifstream &stream, std::vector<int> window_sizes,
  ...
  // Combine the neuron weights to one weight matrix for more efficient
  // computation
  weight_matrix = cv::Mat_<float>(
      neurons.size(), 1 + neurons[0].weights.rows * neurons[0].weights.cols);
  for (size_t i = 0; i < neurons.size(); i++) {
    cv::Mat_<float> w_tmp = neurons[i].weights.t();
    cv::Mat_<float> weights_flat =
        w_tmp.reshape(1, neurons[i].weights.rows * neurons[i].weights.cols);
    weights_flat = weights_flat.t();

    // Incorporate neuron weights directly
    weights_flat = weights_flat * neurons[i].norm_weights;
    weights_flat.copyTo(weight_matrix(
        cv::Rect(1, i, neurons[i].weights.rows * neurons[i].weights.cols, 1)));
    // Incorporate bias as well
    weight_matrix.at<float>(i, 0) = neurons[i].bias;
  }

  // In case we are using OpenBLAS, make sure it is not multi-threading as we
  // are multi-threading outside of it
...
void CCNF_patch_expert::Response(const cv::Mat_<float> &area_of_interest,
  ...
  // the placeholder for the DFT of the image, the integral image, and squared
  // integral image so they don't get recalculated for every response
  ...
  for (size_t i = 0; i < neurons.size(); i++) {
    // Do not bother with neuron response if the alpha is tiny and will not
    // contribute much to overall result
    if (neurons[i].alpha > 1e-4) {

      neurons[i].Response(area_of_interest, area_of_interest_dft,
                          integral_image, integral_image_sq, neuron_response);
  ...
  cv::Mat_<float> resp_vec_f =
      response.reshape(1, response_height * response_width);
...
void CCNF_patch_expert::ResponseOpenBlas(
    const cv::Mat_<float> &area_of_interest, cv::Mat_<float> &response,
  ...
  // the placeholder for the column normalized representation of the image,
  // don't get recalculated for every response
  ...
  cv::Mat_<float> neuron_resp_full(weight_matrix.rows, normalized_input.cols,
                                   0.0f);
  ...
  sgemm_(N, N, &normalized_input.cols, &weight_matrix.rows, &weight_matrix.cols,
         ...
         (float *)weight_matrix.data, &weight_matrix.cols, &beta1,
         (float *)neuron_resp_full.data, &normalized_input.cols);
  ...
  sgemm_(N, N, &resp_vec_f.cols, &Sigmas[s_to_use].rows, &Sigmas[s_to_use].cols,
         ...
         (float *)Sigmas[s_to_use].data, &Sigmas[s_to_use].cols, &beta1,
         (float *)out.data, &resp_vec_f.cols);
