...
DetectionValidator::DetectionValidator(const DetectionValidator &other)
    : orientations(other.orientations), paws(other.paws),
      cnn_subsampling_layers(other.cnn_subsampling_layers),
      cnn_layer_types(other.cnn_layer_types),
      cnn_convolutional_layers_im2col_precomp(
          other.cnn_convolutional_layers_im2col_precomp),
  ...
  for (size_t v = 0; v < other.cnn_convolutional_layers.size(); ++v) {
    this->cnn_convolutional_layers[v].resize(
        other.cnn_convolutional_layers[v].size());

    for (size_t l = 0; l < other.cnn_convolutional_layers[v].size(); ++l) {
      this->cnn_convolutional_layers[v][l].resize(
          other.cnn_convolutional_layers[v][l].size());

      for (size_t i = 0; i < other.cnn_convolutional_layers[v][l].size(); ++i) {
        this->cnn_convolutional_layers[v][l][i].resize(
            other.cnn_convolutional_layers[v][l][i].size());

        for (size_t k = 0; k < other.cnn_convolutional_layers[v][l][i].size();
             ++k) {
          // Make sure the matrix is copied.
          this->cnn_convolutional_layers[v][l][i][k] =
              other.cnn_convolutional_layers[v][l][i][k].clone();
        }
      }
    }
  }
  ...
  for (size_t v = 0; v < other.cnn_fully_connected_layers_weights.size(); ++v) {
    this->cnn_fully_connected_layers_weights[v].resize(
        other.cnn_fully_connected_layers_weights[v].size());

    for (size_t l = 0; l < other.cnn_fully_connected_layers_weights[v].size();
         ++l) {
      // Make sure the matrix is copied.
      this->cnn_fully_connected_layers_weights[v][l] =
          other.cnn_fully_connected_layers_weights[v][l].clone();
    }
  }
  ...
  for (size_t v = 0; v < other.cnn_fully_connected_layers_biases.size(); ++v) {
    this->cnn_fully_connected_layers_biases[v].resize(
        other.cnn_fully_connected_layers_biases[v].size());

    for (size_t l = 0; l < other.cnn_fully_connected_layers_biases[v].size();
         ++l) {
      // Make sure the matrix is copied.
      this->cnn_fully_connected_layers_biases[v][l] =
          other.cnn_fully_connected_layers_biases[v][l].clone();
    }
  }
...
void DetectionValidator::Read(std::string location) {
  ...
  std::ifstream detection_validator_stream(location,
  ...
  if (detection_validator_stream.is_open()) {
    ...
    if (validator_type != 3) {
      std::cout << "ERROR: Using old face validator, no longer supported"
                << std::endl;
    }
    ...
    for (int i = 0; i < n; i++) {
      ...
      orientations[i] = cv::Vec3d(orientation_tmp.at<double>(0),
      ...
      if (validator_type == 3) {
        ...
        for (int layer = 0; layer < network_depth; ++layer) {
          ...
          if (layer_type == 0) {
            ...
            cv::Mat_<float> weight_matrix(num_in_maps *
                                              kernels_rearr[0][0].rows *
            ...
            for (int k = 0; k < num_kernels; ++k) {
              for (int i = 0; i < num_in_maps; ++i) {
                ...
                k_flat.copyTo(weight_matrix(cv::Rect(
                    k, i * kernels_rearr[0][0].rows * kernels_rearr[0][0].cols,
                    1, kernels_rearr[0][0].rows * kernels_rearr[0][0].cols)));
              }
            }
            ...
            cnn_convolutional_layers_im2col_precomp[i].push_back(
                cv::Mat_<float>());
...
float DetectionValidator::Check(const cv::Vec3d &orientation,
                                const cv::Mat_<uchar> &intensity_img,
                                cv::Mat_<float> &detected_landmarks) {

  int id = GetViewId(orientation);

  // The warped (cropped) image, corresponding to a face lying withing the
  // detected lanmarks
  ...
  ExtractBoundingBox(detected_landmarks_local, min_x_f, max_x_f, min_y_f,
                     max_y_f);

  cv::Mat_<float> xs =
      detected_landmarks_local(cv::Rect(0, 0, 1, detected_landmarks.rows / 2));
  cv::Mat_<float> ys = detected_landmarks_local(
      cv::Rect(0, detected_landmarks.rows / 2, 1, detected_landmarks.rows / 2));
  ...
  if (min_y < 0)
    min_y = 0;
  if (max_x > intensity_img.cols - 1)
    max_x = intensity_img.cols - 1;
  if (max_y > intensity_img.rows - 1)
    max_y = intensity_img.rows - 1;
  xs = xs - min_x;
  ys = ys - min_y;

  // If the ROI is non existent return failure (this could happen if all
  // landmarks are outside of the image)
  if (max_x - min_x <= 1 || max_y - min_y <= 1) {
    return 0.0f;
  }

  cv::Mat_<float> intensity_img_float_local;
  intensity_img(cv::Rect(min_x, min_y, max_x - min_x, max_y - min_y))
      .convertTo(intensity_img_float_local, CV_32F);
  ...
  // Convert it to a more interpretable signal (0 low confidence, 1 high
  // confidence)
  dec = 0.5 * (1.0 - dec);
...
double DetectionValidator::CheckCNN(const cv::Mat_<float> &warped_img,
                                    int view_id) {
  ...
  for (size_t layer = 0; layer < cnn_layer_types[view_id].size(); ++layer) {
    ...
    if (layer_type == 0) {
      ...
      convolution_direct_blas(
          outputs, input_maps,
          cnn_convolutional_layers_weights[view_id][cnn_layer],
          cnn_convolutional_layers[view_id][cnn_layer][0][0].rows,
          cnn_convolutional_layers[view_id][cnn_layer][0][0].cols,
          cnn_convolutional_layers_im2col_precomp[view_id][cnn_layer]);
    ...
    if (layer_type == 2) {
      ...
      fully_connected(
          outputs, input_maps,
          cnn_fully_connected_layers_weights[view_id][fully_connected_layer]
              .t(),
          cnn_fully_connected_layers_biases[view_id][fully_connected_layer]);
...
void DetectionValidator::NormaliseWarpedToVector(
    const cv::Mat_<float> &warped_img, cv::Mat_<float> &feature_vec,
    int view_id) {
