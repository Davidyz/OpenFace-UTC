...
// Instead of including cblas.h (the definitions from OpenBLAS and other BLAS
// libraries differ, declare the required OpenBLAS functionality here)
#ifdef __cplusplus
extern "C" {
...
CNN::CNN(const CNN &other)
    : cnn_layer_types(other.cnn_layer_types),
      ...
      conv_layer_pre_alloc_im2col(other.conv_layer_pre_alloc_im2col) {

  this->cnn_convolutional_layers_weights.resize(
      other.cnn_convolutional_layers_weights.size());
  for (size_t l = 0; l < other.cnn_convolutional_layers_weights.size(); ++l) {
    // Make sure the matrix is copied.
    this->cnn_convolutional_layers_weights[l] =
        other.cnn_convolutional_layers_weights[l].clone();
  }
  ...
  for (size_t l = 0; l < other.cnn_convolutional_layers.size(); ++l) {
    this->cnn_convolutional_layers[l].resize(
        other.cnn_convolutional_layers[l].size());

    for (size_t i = 0; i < other.cnn_convolutional_layers[l].size(); ++i) {
      this->cnn_convolutional_layers[l][i].resize(
          other.cnn_convolutional_layers[l][i].size());

      for (size_t k = 0; k < other.cnn_convolutional_layers[l][i].size(); ++k) {
        // Make sure the matrix is copied.
        this->cnn_convolutional_layers[l][i][k] =
            other.cnn_convolutional_layers[l][i][k].clone();
      }
    }
  }

  this->cnn_fully_connected_layers_weights.resize(
      other.cnn_fully_connected_layers_weights.size());

  for (size_t l = 0; l < other.cnn_fully_connected_layers_weights.size(); ++l) {
    // Make sure the matrix is copied.
    this->cnn_fully_connected_layers_weights[l] =
        other.cnn_fully_connected_layers_weights[l].clone();
  }

  this->cnn_fully_connected_layers_biases.resize(
      other.cnn_fully_connected_layers_biases.size());

  for (size_t l = 0; l < other.cnn_fully_connected_layers_biases.size(); ++l) {
    // Make sure the matrix is copied.
    this->cnn_fully_connected_layers_biases[l] =
        other.cnn_fully_connected_layers_biases[l].clone();
  }
...
std::vector<cv::Mat_<float>> CNN::Inference(const cv::Mat &input_img,
  ...
  for (size_t layer = 0; layer < cnn_layer_types.size(); ++layer) {
    ...
    if (layer_type == 0) {
      ...
      // Either perform direct convolution through matrix multiplication or use
      // an FFT optimized version, which one is optimal depends on the kernel
      // and input sizes
      if (direct) {
        if (thread_safe) {
          cv::Mat_<float> pre_alloc;
          convolution_direct_blas(
              outputs, input_maps, cnn_convolutional_layers_weights[cnn_layer],
              cnn_convolutional_layers[cnn_layer][0][0].rows,
              cnn_convolutional_layers[cnn_layer][0][0].cols, pre_alloc);
        } else {
          convolution_direct_blas(
              outputs, input_maps, cnn_convolutional_layers_weights[cnn_layer],
              cnn_convolutional_layers[cnn_layer][0][0].rows,
              cnn_convolutional_layers[cnn_layer][0][0].cols,
              conv_layer_pre_alloc_im2col[cnn_layer]);
        }

      } else {
        convolution_fft2(outputs, input_maps,
                         cnn_convolutional_layers[cnn_layer],
                         cnn_convolutional_layers_bias[cnn_layer],
                         cnn_convolutional_layers_dft[cnn_layer]);
      }
      // vector<cv::Mat_<float> > outs;
      // convolution_fft(outs, input_maps, cnn_convolutional_layers[cnn_layer],
      // cnn_convolutional_layers_bias[cnn_layer],
      // cnn_convolutional_layers_dft[cnn_layer]);
    ...
    if (layer_type == 1) {
      ...
      max_pooling(outputs, input_maps, stride_x, stride_y, kernel_size_x,
                  kernel_size_y);
      max_pool_layer++;
    }
    if (layer_type == 2) {
      fully_connected(outputs, input_maps,
                      cnn_fully_connected_layers_weights[fully_connected_layer],
                      cnn_fully_connected_layers_biases[fully_connected_layer]);
    ...
    for (int layer = 0; layer < network_depth; ++layer) {
      ...
      if (layer_type == 0) {
        ...
        std::vector<std::map<int, std::vector<cv::Mat_<double>>>>
            cnn_convolutional_layers_dft_curr_layer;
        cnn_convolutional_layers_dft_curr_layer.resize(num_kernels);
        cnn_convolutional_layers_dft.push_back(
            cnn_convolutional_layers_dft_curr_layer);

        // Rearrange the flattened kernels into weight matrices for direct
        // convolution computation
        cv::Mat_<float> weight_matrix(num_in_maps * kernels_rearr[0][0].rows *
                                          kernels_rearr[0][0].cols,
                                      num_kernels);
        for (int k = 0; k < num_kernels; ++k) {
          for (int i = 0; i < num_in_maps; ++i) {
            ...
            k_flat.copyTo(weight_matrix(cv::Rect(
                k, i * kernels_rearr[0][0].rows * kernels_rearr[0][0].cols, 1,
                kernels_rearr[0][0].rows * kernels_rearr[0][0].cols)));
          }
        }
        ...
        weight_matrix.copyTo(
            W(cv::Rect(0, 0, weight_matrix.cols, weight_matrix.rows)));
...
void FaceDetectorMTCNN::Read(const std::string &location) {
  ...
  std::cout << "Reading the MTCNN face detector from: " << location
            << std::endl;
...
non_maximum_supression(const std::vector<cv::Rect_<float>> &original_bb,
  ...
  // Sort the input bounding boxes by the detection score, using the nice trick
  // of multimap always being sorted internally
  ...
  while (idxs.size() > 0) {
    ...
    for (auto pos = std::begin(idxs); pos != std::end(idxs);) {
      ...
      // Remove the bounding boxes with less confidence but with significant
...
void select_subset(const std::vector<int> &to_keep,
                   std::vector<cv::Rect_<float>> &bounding_boxes,
...
// boxes as well
void generate_bounding_boxes(std::vector<cv::Rect_<float>> &o_bounding_boxes,
                             std::vector<float> &o_scores,
                             std::vector<cv::Rect_<float>> &o_corrections,
                             const cv::Mat_<float> &heatmap,
                             const std::vector<cv::Mat_<float>> &corrections,
  ...
  for (int x = 0; x < heatmap.cols; ++x) {
    for (int y = 0; y < heatmap.rows; ++y) {
      if (heatmap.at<float>(y, x) >= threshold) {
        ...
        o_corrections.push_back(
            cv::Rect_<float>(corr_x, corr_y, corr_width, corr_height));
...
void apply_correction(std::vector<cv::Rect_<float>> &total_bboxes,
                      const std::vector<cv::Rect_<float>> corrections,
                      bool add1) {
  ...
  for (size_t i = 0; i < total_bboxes.size(); ++i) {
    ...
    float new_max_x =
        curr_box.x + curr_box.width + curr_box.width * corrections[i].width;
    float new_max_y =
        curr_box.y + curr_box.height + curr_box.height * corrections[i].height;
    total_bboxes[i] = cv::Rect_<float>(
        new_min_x, new_min_y, new_max_x - new_min_x, new_max_y - new_min_y);
  }
}

// The actual MTCNN face detection step
bool FaceDetectorMTCNN::DetectFaces(std::vector<cv::Rect_<float>> &o_regions,
                                    const cv::Mat &img_in,
                                    std::vector<float> &o_confidences,
                                    int min_face_size, float t1, float t2,
                                    float t3) {
  ...
  // scale(which is 12 / min), and work down from there to smallest scale(no
  // smaller than 12x12px)
  int min_dim = std::min(height_orig, width_orig);

  int face_support = 12;
  int num_scales = floor(log((double)min_face_size / (double)min_dim) /
                         log(pyramid_factor)) +
                   1;
  ...
  std::vector<std::vector<cv::Rect_<float>>> proposal_boxes_cross_scale(
      num_scales);
  std::vector<std::vector<float>> scores_cross_scale(num_scales);
  std::vector<std::vector<cv::Rect_<float>>> proposal_corrections_cross_scale(
      num_scales);

  for (int i = 0; i < num_scales; ++i) {
    double scale = ((double)face_support / (double)min_face_size) *
                   cv::pow(pyramid_factor, i);
    ...
    std::vector<cv::Mat_<float>> pnet_out =
        PNet.Inference(normalised_img, true, false);
    ...
    // Extract the probabilities from PNet response
    std::vector<cv::Mat_<float>> corrections_heatmap(pnet_out.begin() + 2,
                                                     pnet_out.end());
    ...
    generate_bounding_boxes(proposal_boxes, scores, proposal_corrections,
                            prob_heatmap, corrections_heatmap, scale, t1,
                            face_support);
  ...
  for (int i = 0; i < num_scales; ++i) {
    std::vector<int> to_keep = non_maximum_supression(
        proposal_boxes_cross_scale[i], scores_cross_scale[i], 0.5, false);
    select_subset(to_keep, proposal_boxes_cross_scale[i], scores_cross_scale[i],
                  proposal_corrections_cross_scale[i]);

    proposal_boxes_all.insert(proposal_boxes_all.end(),
                              proposal_boxes_cross_scale[i].begin(),
                              proposal_boxes_cross_scale[i].end());
    scores_all.insert(scores_all.end(), scores_cross_scale[i].begin(),
                      scores_cross_scale[i].end());
    proposal_corrections_all.insert(proposal_corrections_all.end(),
                                    proposal_corrections_cross_scale[i].begin(),
                                    proposal_corrections_cross_scale[i].end());
  }
  ...
  std::vector<int> to_keep =
      non_maximum_supression(proposal_boxes_all, scores_all, 0.7, false);
  select_subset(to_keep, proposal_boxes_all, scores_all,
                proposal_corrections_all);
  ...
  for (size_t k = 0; k < proposal_boxes_all.size(); ++k) {
    ...
    int end_x_in =
        cv::min((int)(proposal_boxes_all[k].x + width_target - 1), width_orig);
    int end_y_in = cv::min((int)(proposal_boxes_all[k].y + height_target - 1),
                           height_orig);
    ...
    int end_x_out =
        cv::min(width_target - (proposal_boxes_all[k].x +
                                proposal_boxes_all[k].width - width_orig),
                width_target);
    int end_y_out =
        cv::min(height_target - (proposal_boxes_all[k].y +
                                 proposal_boxes_all[k].height - height_orig),
                height_target);

    cv::Mat tmp(height_target, width_target, CV_32FC3,
                cv::Scalar(0.0f, 0.0f, 0.0f));

    img_float(cv::Rect(start_x_in, start_y_in, end_x_in - start_x_in,
                       end_y_in - start_y_in))
        .copyTo(tmp(cv::Rect(start_x_out, start_y_out, end_x_out - start_x_out,
                             end_y_out - start_y_out)));
    ...
    std::vector<cv::Mat_<float>> rnet_out =
        RNet.Inference(prop_img, true, false);

    float prob =
        1.0 /
        (1.0 + cv::exp(rnet_out[0].at<float>(0) - rnet_out[0].at<float>(1)));
    scores_all[k] = prob;
  ...
  for (size_t k = 0; k < proposal_boxes_all.size(); ++k) {
    ...
    // Perform RNet on the proposal image
    std::vector<cv::Mat_<float>> onet_out =
        ONet.Inference(prop_img, true, false);

    float prob =
        1.0 /
        (1.0 + cv::exp(onet_out[0].at<float>(0) - onet_out[0].at<float>(1)));
    scores_all[k] = prob;
  ...
  for (size_t k = 0; k < proposal_boxes_all.size(); ++k) {
    proposal_boxes_all[k].x =
        proposal_boxes_all[k].width * -0.0075 + proposal_boxes_all[k].x;
    proposal_boxes_all[k].y =
        proposal_boxes_all[k].height * 0.2459 + proposal_boxes_all[k].y;
    ...
    o_regions.push_back(cv::Rect_<float>(
        proposal_boxes_all[k].x, proposal_boxes_all[k].y,
        proposal_boxes_all[k].width, proposal_boxes_all[k].height));
