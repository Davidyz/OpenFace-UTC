...
// Copy constructor
PAW::PAW(const PAW &other)
    : destination_landmarks(other.destination_landmarks.clone()),
      source_landmarks(other.source_landmarks.clone()),
      triangulation(other.triangulation.clone()),
      triangle_id(other.triangle_id.clone()),
      pixel_mask(other.pixel_mask.clone()),
      coefficients(other.coefficients.clone()), alpha(other.alpha.clone()),
      beta(other.beta.clone()), map_x(other.map_x.clone()),
      map_y(other.map_y.clone()) {
...
PAW::PAW(const cv::Mat_<float> &destination_shape,
  ...
  cv::Mat_<float> ys =
      destination_shape(cv::Rect(0, num_points, 1, num_points));
  ...
  for (int tri = 0; tri < num_tris; ++tri) {
    ...
    alpha.at<float>(tri, 0) =
        (ys.at<float>(j) * c2 - xs.at<float>(j) * c1) / c5;
    ...
    cv::Vec3f xs_three(triangle_points[0], triangle_points[2],
                       triangle_points[4]);
    cv::Vec3f ys_three(triangle_points[1], triangle_points[3],
                       triangle_points[5]);
  ...
  for (int y = 0; y < pixel_mask.rows; y++) {
    for (int x = 0; x < pixel_mask.cols; x++) {
      curr_tri = findTriangle(cv::Point_<float>(x + min_x, y + min_y),
                              destination_points, curr_tri);
...
PAW::PAW(const cv::Mat_<float> &destination_shape,
         const cv::Mat_<int> &triangulation, float in_min_x, float in_min_y,
...
//=============================================================================
// cropping from the source image to the destination image using the shape in s,
// used to determine if shape fitting converged successfully
void PAW::Warp(const cv::Mat &image_to_warp, cv::Mat &destination_image,
               const cv::Mat_<float> &landmarks_to_warp) {
    ...
    for (int x = 0; x < pixel_mask.cols; x++) {
      ...
      if (*mp == 0) {
        ...
        *yp = -1;
        ...
        // If it is different from the previous triangle point to new
        // coefficients This will always be the case in the first iteration,
        // hence a will not point to nothing
        if (j != k) {
        ...
        // look at the first coefficient (and increment). first coefficient is
        // an x offset
        float xo = *ap++;
...
// Is the point (x0,y0) on same side as a half-plane defined by (x1,y1), (x2,
// y2), and (x3, y3)
bool PAW::sameSide(float x0, float y0, float x1, float y1, float x2, float y2,
                   float x3, float y3) {
...
// if point (x0, y0) is on same side for all three half-planes it is in a
// triangle
bool PAW::pointInTriangle(float x0, float y0, float x1, float y1, float x2,
...
int PAW::findTriangle(const cv::Point_<float> &point,
                      const std::vector<std::vector<float>> &control_points,
                      int guess) {
  ...
  if (guess != -1) {
    ...
    bool in_triangle = pointInTriangle(
        x0, y0, control_points[guess][0], control_points[guess][1],
  ...
  for (int i = 0; i < num_tris; ++i) {
    ...
    bool in_triangle =
        pointInTriangle(x0, y0, control_points[i][0], control_points[i][1],
