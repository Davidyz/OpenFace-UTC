    ...
    for (int k = 0; k < prelu_weights.rows; ++k) {
      ...
      for (int i = 0; i < w; ++i) {
        ...
        *iter = in_val >= 0 ? in_val : in_val * neg_mult;
        ...
        // To deal with OpenCV 3.4s debug mode not allowing to go over iteration
        // boundaries
        if (i + 1 < w) {
          iter++;
        }
...
void fully_connected(std::vector<cv::Mat_<float>> &outputs,
                     ...
                     cv::Mat_<float> weights, cv::Mat_<float> biases) {
  outputs.clear();

  if (input_maps.size() > 1) {
    // Concatenate all the maps
    cv::Size orig_size = input_maps[0].size();
    cv::Mat_<float> input_concat((int)input_maps.size(),
                                 input_maps[0].cols * input_maps[0].rows);
    ...
    if (input_concat.rows == weights.cols) {
      ...
      // Flatten the input
      input_concat =
          input_concat.reshape(0, input_concat.rows * input_concat.cols);
...
void max_pooling(std::vector<cv::Mat_<float>> &outputs,
                 const std::vector<cv::Mat_<float>> &input_maps, int stride_x,
                 int stride_y, int kernel_size_x, int kernel_size_y) {
  ...
  for (size_t in = 0; in < input_maps.size(); ++in) {
    ...
    int out_x = (int)round((float)(input_maps[in].cols - kernel_size_x) /
                           (float)stride_x) +
                1;
    int out_y = (int)round((float)(input_maps[in].rows - kernel_size_y) /
                           (float)stride_y) +
                1;
...
void convolution_single_kern_fft(
    const std::vector<cv::Mat_<float>> &input_imgs,
    std::vector<cv::Mat_<double>> &img_dfts,
    const std::vector<cv::Mat_<float>> &_templs,
    std::map<int, std::vector<cv::Mat_<double>>> &_templ_dfts,
    cv::Mat_<float> &result) {
  // Assume result is defined properly
  if (result.empty()) {
    cv::Size corrSize(input_imgs[0].cols - _templs[0].cols + 1,
                      input_imgs[0].rows - _templs[0].rows + 1);
  ...
  if (_templ_dfts.find(dftsize.width) == _templ_dfts.end()) {
    ...
    for (size_t k = 0; k < _templs.size(); ++k) {
      ...
      cv::Mat_<double> dst(dftTempl[k],
                           cv::Rect(0, 0, dftsize.width, dftsize.height));

      cv::Mat_<double> dst1(dftTempl[k],
                            cv::Rect(0, 0, _templs[k].cols, _templs[k].rows));

      if (dst1.data != src.data)
        src.convertTo(dst1, dst1.depth());

      if (dst.cols > _templs[k].cols) {
        cv::Mat_<double> part(dst, cv::Range(0, _templs[k].rows),
                              cv::Range(_templs[k].cols, dst.cols));
        part.setTo(0);
      }
  ...
  cv::Size bsz(std::min(blocksize.width, result.cols),
               std::min(blocksize.height, result.rows));
  cv::Mat src;
  ...
  if (img_dfts.empty()) {
    for (size_t k = 0; k < input_imgs.size(); ++k) {
      ...
      cv::Size dsz(bsz.width + _templs[k].cols - 1,
                   bsz.height + _templs[k].rows - 1);
...
void convolution_fft2(
  ...
  for (size_t k = 0; k < kernels.size(); ++k) {
    ...
    convolution_single_kern_fft(input_maps, input_image_dft, kernels[k],
                                precomp_dfts[k], output);
...
void im2col_t(const cv::Mat_<float> &input, const unsigned int width,
              const unsigned int height, cv::Mat_<float> &output) {
...
void im2col(const cv::Mat_<float> &input, const unsigned int width,
            const unsigned int height, cv::Mat_<float> &output) {
  ...
  for (unsigned int i = 0; i < yB; i++) {
    ...
    for (unsigned int j = 0; j < xB; j++) {
      ...
      for (unsigned int yy = 0; yy < height; ++yy) {
        ...
        for (unsigned int xx = 0; xx < width; ++xx) {
          unsigned int colIdx = xx * height + yy;
          // output.at<float>(rowIdx, colIdx) = Mi[j + xx]; //input.at<float>(i
          // + yy, j + xx);
          Mo[colIdx] = Mi[j + xx];
        }
      }
...
void im2col_multimap(const std::vector<cv::Mat_<float>> &inputs,
  ...
  // Allocate the output size
  if (output.cols != width * height * inputs.size() + 1 ||
      (unsigned int)output.rows < xB * yB) {
      ...
      for (unsigned int yy = 0; yy < height; ++yy) {
        for (unsigned int in_maps = 0; in_maps < num_maps; ++in_maps) {
          ...
          for (unsigned int xx = 0; xx < width; ++xx) {
            ...
            // output.at<float>(rowIdx, colIdx) = Mi[j + xx];
            // //input.at<float>(i + yy, j + xx);
            Mo[colIdx] = Mi[j + xx];
          }
        }
      }
...
// A fast convolution implementation, can provide a pre-allocated im2col as
// well, if empty, it is created
void convolution_direct_blas(std::vector<cv::Mat_<float>> &outputs,
                             const std::vector<cv::Mat_<float>> &input_maps,
                             const cv::Mat_<float> &weight_matrix, int height_k,
                             int width_k, cv::Mat_<float> &pre_alloc_im2col) {
  outputs.clear();
  ...
  // Instead of re-allocating data use the first rows of already allocated data
  // and re-allocate only if not enough rows are present, this is what makes
  // this non thread safe, as same memory would be used
  ...
  // cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, weight_t.cols, yB *
  // xB, pre_alloc_im2col.cols, 1, m2, weight_t.cols, m1, pre_alloc_im2col.cols,
  // 0.0, m3, weight_t.cols);
  float alpha = 1.0f;
  float beta = 0.0f;
  // Call fortran directly (faster)
  char N[2];
  N[0] = 'N';
  sgemm_(N, N, &m2_cols, &num_rows, &pre_alloc_im2col.cols, &alpha, m2,
         &m2_cols, m1, &pre_alloc_im2col.cols, &beta, m3, &m2_cols);
