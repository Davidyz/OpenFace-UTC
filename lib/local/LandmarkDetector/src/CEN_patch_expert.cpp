...
void im2colBias(const cv::Mat_<float> &input, const unsigned int width,
    ...
    for (unsigned int i = 0; i < yB; i++) {
      ...
      for (unsigned int yy = 0; yy < height; ++yy)
        for (unsigned int xx = 0; xx < width; ++xx) {
          ...
          output.at<float>(rowIdx, colIdx + 1) =
...
void CEN_patch_expert::Response(const cv::Mat_<float> &area_of_interest,
  ...
  for (size_t layer = 0; layer < activation_function.size(); ++layer) {
    ...
    char N[2];
    N[0] = 'N';
    sgemm_(N, N, &resp.cols, &weight.rows, &weight.cols, &alpha1, m1,
           &resp.cols, m2, &weight.cols, &beta1, m3, &resp.cols);

    // The above is a faster version of this, by calling the fortran version
    // directly
    // cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, resp.cols,
    // weight.rows, weight.cols, 1, m1, resp.cols, m2, weight.cols, 0.0, m3,
    // resp.cols);
...
void im2colBiasSparse(const cv::Mat_<float> &input, const unsigned int width,
                      const unsigned int height, cv::Mat_<float> &output) {
...
void LandmarkDetector::interpolationMatrix(cv::Mat_<float> &mapMatrix,
                                           int response_height,
                                           int response_width, int input_width,
                                           int input_height) {
  int m = input_height;
  int n = input_width;
  ...
  // Find a mapping from indices in the computed sparse response and the
...
void CEN_patch_expert::ResponseInternal(cv::Mat_<float> &response) {
  for (size_t layer = 0; layer < activation_function.size(); ++layer) {
    ...
    char N[2];
    N[0] = 'N';
    sgemm_(N, N, &resp.cols, &weights[layer].rows, &weights[layer].cols,
           &alpha1, m1, &resp.cols, m2, &weights[layer].cols, &beta1, m3,
           &resp.cols);
    ...
    // Alternative is to multiply the responses directly using OpenCV (much
    // slower)
...
void CEN_patch_expert::ResponseSparse(
    ...
    cv::Mat_<float> &response_left, cv::Mat_<float> &response_right,
    cv::Mat_<float> &mapMatrix, cv::Mat_<float> &im2col_prealloc_left,
  ...
  if (right_provided) {
    ...
    im2colBiasSparseContrastNorm(area_of_interest_right, width_support,
                                 height_support, im2col_prealloc_right);
  }

  // Extract im2col but in a sparse way and contrast normalize
  if (left_provided) {
    response_height = area_of_interest_left.rows - height_support + 1;
    im2colBiasSparseContrastNorm(area_of_interest_left, width_support,
                                 height_support, im2col_prealloc_left);
  }
  ...
  if (left_provided && right_provided) {
    ...
    response_right =
        response(cv::Rect(response.cols / 2, 0, response.cols / 2, 1));
  } else if (left_provided) {
