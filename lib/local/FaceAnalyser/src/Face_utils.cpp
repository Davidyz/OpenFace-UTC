...
void extract_rigid_points(cv::Mat_<float> &source_points,
                          cv::Mat_<float> &destination_points) {
...
void AlignFace(cv::Mat &aligned_face, const cv::Mat &frame,
  ...
  cv::Mat_<float> destination_landmarks =
      similarity_normalised_shape.reshape(1, 2).t();
  ...
  cv::Matx22f scale_rot_matrix =
      Utilities::AlignShapesWithScale(source_landmarks, destination_landmarks);
...
// Aligning a face to a common reference frame
void AlignFaceMask(cv::Mat &aligned_face, const cv::Mat &frame,
                   const cv::Mat_<float> &detected_landmarks,
                   cv::Vec6f params_global, const LandmarkDetector::PDM &pdm,
                   const cv::Mat_<int> &triangulation, bool rigid,
                   double sim_scale, int out_width, int out_height) {
  // Will warp to scaled mean shape
  cv::Mat_<float> similarity_normalised_shape = pdm.mean_shape * sim_scale;
  ...
  cv::Matx22f warp_matrix_2d(warp_matrix(0, 0), warp_matrix(0, 1),
                             warp_matrix(1, 0), warp_matrix(1, 1));

  destination_landmarks = cv::Mat(detected_landmarks.reshape(1, 2).t()) *
                          cv::Mat(warp_matrix_2d).t();

  destination_landmarks.col(0) =
      destination_landmarks.col(0) + warp_matrix(0, 2);
  destination_landmarks.col(1) =
      destination_landmarks.col(1) + warp_matrix(1, 2);
  ...
  LandmarkDetector::PAW paw(destination_landmarks, triangulation, 0, 0,
                            aligned_face.cols - 1, aligned_face.rows - 1);

  // Mask each of the channels (a bit of a roundabout way, but OpenCV 3.1 in
  // debug mode doesn't seem to be able to handle a more direct way using split
  // and merge)
  ...
  for (size_t i = 0; i < aligned_face_channels.size(); ++i) {
    cv::multiply(aligned_face_channels[i], paw.pixel_mask,
                 aligned_face_channels[i], 1.0, CV_8U);
  }

  if (aligned_face.channels() == 3) {
    cv::Mat planes[] = {aligned_face_channels[0], aligned_face_channels[1],
                        aligned_face_channels[2]};
...
void Extract_FHOG_descriptor(cv::Mat_<double> &descriptor, const cv::Mat &image,
                             int &num_rows, int &num_cols, int cell_size) {
...
// Extract summary statistics (mean, stdev, min, max) from each dimension of a
// descriptor, each row is a descriptor
void ExtractSummaryStatistics(const cv::Mat_<double> &descriptors,
                              cv::Mat_<double> &sum_stats, bool use_mean,
                              bool use_stdev, bool use_max_min) {
...
void AddDescriptor(cv::Mat_<double> &descriptors,
                   cv::Mat_<double> new_descriptor, int curr_frame,
                   int num_frames_to_keep) {
  if (descriptors.empty()) {
    descriptors =
        cv::Mat_<double>(num_frames_to_keep, new_descriptor.cols, 0.0);
  }
