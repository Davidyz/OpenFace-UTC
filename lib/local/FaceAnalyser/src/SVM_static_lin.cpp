...
void SVM_static_lin::Read(std::ifstream &stream,
  ...
  if (this->means.empty()) {
    ...
    if (cv::norm(m_tmp - this->means > 0.00001)) {
      std::cout << "Something went wrong with the SVM static classifiers"
                << std::endl;
    }
  }
...
void SVM_static_lin::Predict(std::vector<double> &predictions,
