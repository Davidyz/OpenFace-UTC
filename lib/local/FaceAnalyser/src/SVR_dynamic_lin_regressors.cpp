...
void SVR_dynamic_lin_regressors::Read(
    std::ifstream &stream, const std::vector<std::string> &au_names) {
  ...
  if (this->means.empty()) {
    ...
    if (cv::norm(m_tmp - this->means > 0.00001)) {
      std::cout << "Something went wrong with the SVR dynamic regressors"
                << std::endl;
    }
  }
...
void SVR_dynamic_lin_regressors::Predict(
    std::vector<double> &predictions, std::vector<std::string> &names,
    const cv::Mat_<double> &fhog_descriptor,
    const cv::Mat_<double> &geom_params, const cv::Mat_<double> &running_median,
    const cv::Mat_<double> &running_median_geom) {
  if (AU_names.size() > 0) {
    cv::Mat_<double> preds;
    if (fhog_descriptor.cols == this->means.cols) {
      preds = (fhog_descriptor - this->means - running_median) *
      ...
      preds = (input - this->means - run_med) * this->support_vectors +
              this->biases;
    }
