...
FaceAnalyser::FaceAnalyser(
  ...
  // Initialise the histograms that will represent bins from 0 - 1 (as HoG
  // values are only stored as those)
  num_bins_hog = 1000;
  max_val_hog = 1;
  min_val_hog = -0.005;
...
std::vector<std::string> FaceAnalyser::GetAUClassNames() const {
  ...
  std::vector<std::string> au_class_names_stat =
      AU_SVM_static_appearance_lin.GetAUNames();
  std::vector<std::string> au_class_names_dyn =
      AU_SVM_dynamic_appearance_lin.GetAUNames();
...
std::vector<std::string> FaceAnalyser::GetAURegNames() const {
  ...
  std::vector<std::string> au_reg_names_stat =
      AU_SVR_static_appearance_lin_regressors.GetAUNames();
  std::vector<std::string> au_reg_names_dyn =
      AU_SVR_dynamic_appearance_lin_regressors.GetAUNames();
...
FaceAnalyser::GetDynamicAUReg() const {
  ...
  for (size_t i = 0; i < au_reg_names_stat.size(); ++i) {
    au_dynamic_reg.push_back(
        std::pair<std::string, bool>(au_reg_names_stat[i], false));
  }
  for (size_t i = 0; i < au_reg_names_dyn.size(); ++i) {
    au_dynamic_reg.push_back(
        std::pair<std::string, bool>(au_reg_names_dyn[i], true));
  }
...
void FaceAnalyser::GetLatestHOG(cv::Mat_<double> &hog_descriptor, int &num_rows,
                                int &num_cols) {
...
int GetViewId(const std::vector<cv::Vec3d> orientations_all,
...
void FaceAnalyser::PredictStaticAUsAndComputeFeatures(
  ...
  AlignFaceMask(aligned_face_for_au, frame, detected_landmarks, params_global,
                pdm, triangulation, true, align_scale_au, align_width_au,
                align_height_au);

  // If the aligned face for AU matches the output requested one, just reuse it,
  // else compute it
  if (align_scale_out == align_scale_au && align_width_out == align_width_au &&
      align_height_out == align_height_au && align_mask) {
    aligned_face_for_output = aligned_face_for_au.clone();
  } else {
    if (align_mask) {
      AlignFaceMask(aligned_face_for_output, frame, detected_landmarks,
                    params_global, pdm, triangulation, true, align_scale_out,
                    align_width_out, align_height_out);
    } else {
      AlignFace(aligned_face_for_output, frame, detected_landmarks,
                params_global, pdm, true, align_scale_out, align_width_out,
                align_height_out);
    }
  }
  ...
  Extract_FHOG_descriptor(hog_descriptor, aligned_face_for_au,
                          this->num_hog_rows, this->num_hog_cols);
  ...
  // First convert the face image to double representation as a row vector, TODO
  // rem
  // cv::Mat_<uchar> aligned_face_cols(1, aligned_face_for_au.cols *
  // aligned_face_for_au.rows * aligned_face_for_au.channels(),
  // aligned_face_for_au.data, 1); cv::Mat_<double> aligned_face_cols_double;
...
void FaceAnalyser::AddNextFrame(const cv::Mat &frame,
                                ...
                                bool success, double timestamp_seconds,
                                bool online) {
  ...
  if (success) {
    ...
    // If the aligned face for AU matches the output requested one, just reuse
    // it, else compute it
    if (align_scale_out == align_scale_au &&
        align_width_out == align_width_au &&
        align_height_out == align_height_au && align_mask) {
      aligned_face_for_output = aligned_face_for_au.clone();
    } else {
      if (align_mask) {
    ...
    aligned_face_for_output =
        cv::Mat(align_height_out, align_width_out, CV_8UC3);
  ...
  if (aligned_face_for_output.channels() == 3 && out_grayscale) {
    cvtColor(aligned_face_for_output, aligned_face_for_output,
             cv::COLOR_BGR2GRAY);
  }
  ...
  //		{
  //			if (dyns[n].first.compare(AU_predictions_reg[i].first) ==
  //0)
  //			{
  //				stat = !dyns[i].second;
  //			}
  ...
  if (frames_tracking % 2 == 1) {
    UpdateRunningMedian(this->hog_desc_hist[orientation_to_use],
                        ...
                        this->hog_desc_median, hog_descriptor, update_median,
                        this->num_bins_hog, this->min_val_hog,
                        this->max_val_hog);
  ...
  // A small speedup
  if (frames_tracking % 2 == 1) {
    UpdateRunningMedian(this->geom_desc_hist, this->geom_hist_sum,
                        this->geom_descriptor_median, geom_descriptor_frame,
                        update_median, this->num_bins_geom, this->min_val_geom,
                        this->max_val_geom);
  }
  ...
  if (online) {
    AU_predictions_reg_corrected = CorrectOnlineAUs(
        AU_predictions_reg, orientation_to_use, true, false, success, true);
    AU_predictions_reg = AU_predictions_reg_corrected;
  }

  // Useful for prediction corrections (calibration after the whole video is
  // processed)
...
// Perform prediction on initial n frames anew as the current neutral face
// estimate is better now
  ...
  if (!postprocessed) {
    ...
    while (all_ind < all_frames_size && success_ind < max_init_frames) {
      ...
      if (valid_preds[all_ind]) {
        ...
        for (size_t au = 0; au < AU_predictions_reg.size(); ++au) {
          // Find the appropriate AU (if not found add it)
          AU_predictions_reg_all_hist[AU_predictions_reg[au].first][all_ind] =
              AU_predictions_reg[au].second;
        }
        ...
        for (size_t au = 0; au < AU_predictions_class.size(); ++au) {
          // Find the appropriate AU (if not found add it)
          AU_predictions_class_all_hist[AU_predictions_class[au].first]
                                       [all_ind] =
                                           AU_predictions_class[au].second;
        }
...
void FaceAnalyser::ExtractAllPredictionsOfflineReg(
  ...
  std::vector<std::string> dyn_au_names =
      AU_SVR_dynamic_appearance_lin_regressors.GetAUNames();

  // Allow these AUs to be person calirated based on expected number of neutral
  // frames (learned from the data)
  for (auto au_iter = AU_predictions_reg_all_hist.begin();
    ...
    au_predictions.push_back(
        std::pair<std::string, std::vector<double>>(au_name, au_vals));
    ...
    if (au_good.empty() || !dynamic) {
      ...
      // If it is a dynamic AU regressor we can also do some prediction shifting
      // to make it more accurate The shifting proportion is learned and is
      // callen cutoff
      ...
      if (au_id != -1 &&
          AU_SVR_dynamic_appearance_lin_regressors.GetCutoffs()[au_id] != -1) {
        double cutoff =
            AU_SVR_dynamic_appearance_lin_regressors.GetCutoffs()[au_id];
  ...
  for (auto au_iter = au_predictions.begin(); au_iter != au_predictions.end();
       ++au_iter) {
    std::string au_name = au_iter->first;
...
void FaceAnalyser::ExtractAllPredictionsOfflineClass(
  ...
  for (auto au_iter = AU_predictions_class_all_hist.begin();
    ...
    if ((int)au_vals.size() > (window_size - 1) / 2) {
      for (size_t i = (window_size - 1) / 2;
           i < au_vals.size() - (window_size - 1) / 2; ++i) {
        double sum = 0;
        int div_by = 0;
        for (int w = -(window_size - 1) / 2;
             w <= (window_size - 1) / 2 && (i + w < au_vals_tmp.size()); ++w) {
          sum += au_vals_tmp[i + w];
          div_by++;
        }
        sum = sum / div_by;
...
void FaceAnalyser::Reset() {
  ...
  for (size_t i = 0; i < hog_desc_hist.size(); ++i) {
    this->hog_desc_hist[i] =
        cv::Mat_<int>(hog_desc_hist[i].rows, hog_desc_hist[i].cols, (int)0);
    this->hog_hist_sum[i] = 0;

    this->face_image_hist[i] =
        cv::Mat_<int>(face_image_hist[i].rows, face_image_hist[i].cols, (int)0);
    this->face_image_hist_sum[i] = 0;

    // 0 callibration predictions
    this->au_prediction_correction_count[i] = 0;
    this->au_prediction_correction_histogram[i] =
        cv::Mat_<int>(au_prediction_correction_histogram[i].rows,
                      au_prediction_correction_histogram[i].cols, (int)0);
  }

  this->geom_descriptor_median.setTo(cv::Scalar(0));
  this->geom_desc_hist =
      cv::Mat_<int>(geom_desc_hist.rows, geom_desc_hist.cols, (int)0);
  geom_hist_sum = 0;

  // Reset the predictions
  AU_prediction_track =
      cv::Mat_<double>(AU_prediction_track.rows, AU_prediction_track.cols, 0.0);

  geom_desc_track =
      cv::Mat_<double>(geom_desc_track.rows, geom_desc_track.cols, 0.0);

  dyn_scaling = std::vector<std::vector<double>>(
      dyn_scaling.size(), std::vector<double>(dyn_scaling[0].size(), 5.0));
...
void FaceAnalyser::UpdateRunningMedian(cv::Mat_<int> &histogram,
  ...
  if (update) {
    ...
    converted_descriptor.setTo(cv::Scalar(num_bins - 1),
                               converted_descriptor > num_bins - 1);
    ...
    for (int i = 0; i < histogram.rows; ++i) {
      ...
      for (int j = 0; j < histogram.cols; ++j) {
        ...
        if (cummulative_sum >= cutoff_point) {
          median.at<double>(i) = min_val +
                                 ((double)j) * (length / ((double)num_bins)) +
...
void FaceAnalyser::ExtractMedian(cv::Mat_<int> &histogram, int hist_count,
                                 cv::Mat_<double> &median, int num_bins,
                                 double min_val, double max_val) {
...
FaceAnalyser::PredictCurrentAUs(int view) {
  ...
  if (!hog_desc_frame.empty()) {
    ...
    AU_SVR_static_appearance_lin_regressors.Predict(
        svr_lin_stat_preds, svr_lin_stat_aus, hog_desc_frame,
        geom_descriptor_frame);

    for (size_t i = 0; i < svr_lin_stat_preds.size(); ++i) {
      predictions.push_back(std::pair<std::string, double>(
          svr_lin_stat_aus[i], svr_lin_stat_preds[i]));
    }

    std::vector<std::string> svr_lin_dyn_aus;
    std::vector<double> svr_lin_dyn_preds;

    AU_SVR_dynamic_appearance_lin_regressors.Predict(
        svr_lin_dyn_preds, svr_lin_dyn_aus, hog_desc_frame,
        geom_descriptor_frame, this->hog_desc_median,
        this->geom_descriptor_median);

    for (size_t i = 0; i < svr_lin_dyn_preds.size(); ++i) {
      predictions.push_back(std::pair<std::string, double>(
          svr_lin_dyn_aus[i], svr_lin_dyn_preds[i]));
    }
  }
...
std::vector<std::pair<std::string, double>> FaceAnalyser::CorrectOnlineAUs(
    std::vector<std::pair<std::string, double>> predictions_orig, int view,
    bool dyn_shift, bool dyn_scale, bool update_track, bool clip_values) {
  // Correction that drags the predicion to 0 (assuming the bottom 10% of
  // predictions are of neutral expresssions)
  ...
  if (update_track) {
    UpdatePredictionTrack(au_prediction_correction_histogram[view],
                          au_prediction_correction_count[view], correction,
                          predictions, 0.10, 200, -3, 5, 10);
  }
  ...
  if (dyn_scale) {
    ...
    // Also makes sense as till the maximum expression is seen, it is hard to
    // tell how expressive a persons face is
...
// (classification)
std::vector<std::pair<std::string, double>>
FaceAnalyser::PredictCurrentAUsClass(int view) {
  ...
  if (!hog_desc_frame.empty()) {
    ...
    AU_SVM_static_appearance_lin.Predict(svm_lin_stat_preds, svm_lin_stat_aus,
                                         hog_desc_frame, geom_descriptor_frame);

    for (size_t i = 0; i < svm_lin_stat_aus.size(); ++i) {
      predictions.push_back(std::pair<std::string, double>(
          svm_lin_stat_aus[i], svm_lin_stat_preds[i]));
    }

    std::vector<std::string> svm_lin_dyn_aus;
    std::vector<double> svm_lin_dyn_preds;

    AU_SVM_dynamic_appearance_lin.Predict(svm_lin_dyn_preds, svm_lin_dyn_aus,
                                          hog_desc_frame, geom_descriptor_frame,
    ...
    for (size_t i = 0; i < svm_lin_dyn_aus.size(); ++i) {
      predictions.push_back(std::pair<std::string, double>(
          svm_lin_dyn_aus[i], svm_lin_dyn_preds[i]));
    }
  }
...
std::vector<std::pair<std::string, double>>
FaceAnalyser::GetCurrentAUsClass() const {
  return AU_predictions_class;
}

std::vector<std::pair<std::string, double>>
FaceAnalyser::GetCurrentAUsReg() const {
  return AU_predictions_reg;
}

std::vector<std::pair<std::string, double>>
FaceAnalyser::GetCurrentAUsCombined() const {
...
void FaceAnalyser::Read(std::string model_loc) {
  ...
  std::cout << "Reading the AU analysis module from: " << model_loc
            << std::endl;
  ...
  // The other module locations should be defined as relative paths from the
  // main model
  ...
  while (!locations.eof()) {
    ...
    if (module.compare("AUPredictor") == 0) {
      ...
      // The triangulation used for masking out the non-face parts of aligned
      // image
...
void FaceAnalyser::ReadAU(std::string au_model_location) {
  ...
  if (!locations.is_open()) {
    std::cout << "Couldn't open the AU prediction files at: "
              << au_model_location.c_str() << " aborting" << std::endl;
    std::cout.flush();
    return;
  }
...
void FaceAnalyser::UpdatePredictionTrack(
    cv::Mat_<int> &prediction_corr_histogram, int &prediction_correction_count,
  ...
  if (prediction_corr_histogram.empty()) {
    prediction_corr_histogram =
        cv::Mat_<int>((int)predictions.size(), num_bins, (int)0);
  }

  for (int i = 0; i < prediction_corr_histogram.rows; ++i) {
    // Find the bins corresponding to the current descriptor
    int index = (int)((predictions[i].second - min_val) * ((double)num_bins) /
                      (length));
    if (index < 0) {
      index = 0;
...
void FaceAnalyser::GetSampleHist(cv::Mat_<int> &prediction_corr_histogram,
                                 ...
                                 std::vector<double> &sample, double ratio,
...
void FaceAnalyser::ReadRegressor(std::string fname,
                                 const std::vector<std::string> &au_names) {
  std::ifstream regressor_stream(fname.c_str(),
...
double FaceAnalyser::GetCurrentTimeSeconds() { return current_time_seconds; }
...
void FaceAnalyser::PostprocessOutputFile(std::string output_file) {
  ...
  ExtractAllPredictionsOfflineReg(predictions_reg, certainties, successes,
                                  timestamps, dynamic);
  ExtractAllPredictionsOfflineClass(predictions_class, certainties, successes,
                                    timestamps, dynamic);
  ...
  // Read the header and find all _r and _c parts in a file and use their
  // indices
    ...
    for (int t = 1; t < (int)tokens.size(); ++t) {
      if (t >= begin_ind && t < end_ind) {
        if (t - begin_ind < num_reg) {
          outfile << ","
                  << predictions_reg[inds_reg[t - begin_ind]].second[i - 1];
        } else {
          outfile << ","
                  << predictions_class[inds_class[t - begin_ind - num_reg]]
                         .second[i - 1];
        }
      } else {
        rtrim(tokens[t]);
