...
public:
  ...
  void AddNextFrame(const cv::Mat &frame,
                    const cv::Mat_<float> &detected_landmarks, bool success,
  ...
  // from a static image
  void
  PredictStaticAUsAndComputeFeatures(const cv::Mat &frame,
                                     const cv::Mat_<float> &detected_landmarks);

  void Reset();

  void GetLatestHOG(cv::Mat_<double> &hog_descriptor, int &num_rows,
                    int &num_cols);
  void GetLatestAlignedFace(cv::Mat &image);

  void GetLatestNeutralHOG(cv::Mat_<double> &hog_descriptor, int &num_rows,
                           int &num_cols);
...
private:
  ...
  // The geometry descriptor (rigid followed by non-rigid shape parameters from
  // CLNF)
  ...
  std::vector<std::pair<std::string, double>>
  CorrectOnlineAUs(std::vector<std::pair<std::string, double>> predictions_orig,
                   int view, bool dyn_shift = false, bool dyn_scale = false,
                   bool update_track = true, bool clip_values = false);
  ...
  // for AU and emotion inference using a set of histograms (the histograms are
  // evenly spaced from min_val to max_val) Descriptor has to be a row vector
  // TODO this duplicates some other code
  void UpdateRunningMedian(cv::Mat_<int> &histogram, int &hist_sum,
                           cv::Mat_<double> &median,
                           const cv::Mat_<double> &descriptor, bool update,
                           int num_bins, double min_val, double max_val);
  void ExtractMedian(cv::Mat_<int> &histogram, int hist_count,
  ...
  // That is they don't always predict 0 for a neutral expression Keeping track
  // of the predictions we can correct for this, by assuming that at least
  // "ratio" of frames are neutral and subtract that value of prediction, only
  // perform the correction after min_frames
  void UpdatePredictionTrack(
      cv::Mat_<int> &prediction_corr_histogram,
      int &prediction_correction_count, std::vector<double> &correction,
      const std::vector<std::pair<std::string, double>> &predictions,
      double ratio = 0.25, int num_bins = 200, double min_val = -3,
      double max_val = 5, int min_frames = 10);
  void GetSampleHist(cv::Mat_<int> &prediction_corr_histogram,
                     ...
                     int num_bins = 200, double min_val = 0,
                     double max_val = 5);
  ...
  // allows us to scale the model to the most extreme seen) They have to be view
  // specific
