...
class static_set_kernel_1 : public enumerable<const T> {
  ...
  friend void deserialize(static_set_kernel_1 &item, std::istream &in) {
    try {
      item.clear();
      size_t size;
      deserialize(size, in);
...
public:
  ...
  void load(remover<T> &source);

  void load(asc_remover<T, compare> &source);

  bool is_member(const T &item) const;

  inline void swap(static_set_kernel_1 &item);
...
inline void swap(static_set_kernel_1<T, compare> &a,
...
template <typename T, typename compare>
static_set_kernel_1<T, compare>::static_set_kernel_1()
    : set_size(0), d(0), cur(0), at_start_(true) {}
...
template <typename T, typename compare>
void static_set_kernel_1<T, compare>::clear() {
  if (set_size > 0) {
    set_size = 0;
    delete[] d;
    d = 0;
...
template <typename T, typename compare>
void static_set_kernel_1<T, compare>::load(remover<T> &source) {
  if (source.size() > 0) {
    d = new T[source.size()];
...
template <typename T, typename compare>
void static_set_kernel_1<T, compare>::load(asc_remover<T, compare> &source) {
  if (source.size() > 0) {
    d = new T[source.size()];
...
template <typename T, typename compare>
bool static_set_kernel_1<T, compare>::is_member(const T &item) const {
...
template <typename T, typename compare>
size_t static_set_kernel_1<T, compare>::size() const {
  return set_size;
}
...
template <typename T, typename compare>
void static_set_kernel_1<T, compare>::swap(
...
template <typename T, typename compare>
bool static_set_kernel_1<T, compare>::at_start() const {
  return at_start_;
}
...
template <typename T, typename compare>
void static_set_kernel_1<T, compare>::reset() const {
  at_start_ = true;
  cur = 0;
}
...
template <typename T, typename compare>
bool static_set_kernel_1<T, compare>::current_element_valid() const {
  return (cur != 0);
}
...
template <typename T, typename compare>
const T &static_set_kernel_1<T, compare>::element() const {
  return *cur;
}
...
template <typename T, typename compare>
const T &static_set_kernel_1<T, compare>::element() {
  return *cur;
}
...
template <typename T, typename compare>
bool static_set_kernel_1<T, compare>::move_next() const {
  // if at_start() && size() > 0
