...
class static_set : public enumerable<const T> {
  ...
  with std::less and T is swappable by a global swap() and T must have a default
  constructor
...
inline void swap(static_set<T, compare> &a, static_set<T, compare> &b) {
  a.swap(b);
}
/*!
...
template <typename T, typename compare>
void deserialize(static_set<T, compare> &item, std::istream &in);
/*!
