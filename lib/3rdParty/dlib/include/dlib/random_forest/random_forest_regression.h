...
public:
  ...
  dense_feature_extractor() = default;

  void setup(const std::vector<sample_type> &x, const std::vector<double> &y) {
    DLIB_CASSERT(x.size() > 0);
    DLIB_CASSERT(x.size() == y.size());
    for (auto &el : x)
      DLIB_CASSERT(el.size() == x[0].size(),
                   "All the vectors in a training set have to have the same "
                   "dimensionality.");
  ...
  double extract_feature_value(const sample_type &item,
                               const feature &f) const {
    DLIB_ASSERT(max_num_feats() != 0);
    return item(f);
  }

  size_t max_num_feats() const { return num_feats; }

  friend void serialize(const dense_feature_extractor &item,
                        std::ostream &out) {
...
template <typename feature_extractor>
void deserialize(internal_tree_node<feature_extractor> &item,
                 std::istream &in) {
...
public:
  ...
  random_forest_regression_function() = default;
  ...
  random_forest_regression_function(
      ...
      : fe(std::move(fe_)), trees(std::move(trees_)),
        leaves(std::move(leaves_)) {
    DLIB_ASSERT(trees.size() > 0);
    DLIB_ASSERT(trees.size() == leaves.size(),
                "Every set of tree nodes has to have leaves");
#ifdef ENABLE_ASSERTS
    for (size_t i = 0; i < trees.size(); ++i) {
      DLIB_ASSERT(trees[i].size() > 0, "A tree can't have 0 leaves.");
      for (auto &node : trees[i]) {
        DLIB_ASSERT(trees[i].size() + leaves[i].size() > node.left,
                    "left node index in tree is too big. There is no "
                    "associated tree node or leaf.");
        DLIB_ASSERT(trees[i].size() + leaves[i].size() > node.right,
                    "right node index in tree is too big. There is no "
                    "associated tree node or leaf.");
      }
    }
#endif
  ...
  double operator()(const sample_type &x) const {
    ...
    for (size_t i = 0; i < trees.size(); ++i) {
      ...
      while (idx < tree.size()) {
        auto feature_value =
            fe.extract_feature_value(x, tree[idx].split_feature);
  ...
  friend void serialize(const random_forest_regression_function &item,
                        std::ostream &out) {
  ...
  friend void deserialize(random_forest_regression_function &item,
                          std::istream &in) {
  ...
  /*!
      CONVENTION
          ...
          - Any .left or .right index in trees that is larger than the number of
            nodes in the tree references a leaf. Moreover, the index of the leaf
  is computed by subtracting the number of nodes in the tree.
  !*/
...
public:
  ...
  random_forest_regression_trainer() = default;

  const feature_extractor_type &get_feature_extractor() const { return fe_; }

  void set_feature_extractor(const feature_extractor_type &feat_extractor) {
    fe_ = feat_extractor;
  }

  void set_seed(const std::string &seed) { random_seed = seed; }

  const std::string &get_random_seed() const { return random_seed; }
  ...
  size_t get_min_samples_per_leaf() const { return min_samples_per_leaf; }

  void be_verbose() { verbose = true; }

  void be_quiet() { verbose = false; }

  trained_function_type train(const std::vector<sample_type> &x,
...
private:
  trained_function_type do_train(const std::vector<sample_type> &x,
                                 const std::vector<double> &y,
                                 std::vector<double> &oob_values,
                                 bool compute_oob_values) const {
    DLIB_CASSERT(x.size() == y.size());
    DLIB_CASSERT(x.size() > 0);
    ...
    std::vector<std::vector<internal_tree_node<feature_extractor>>> all_trees(
        num_trees);
    std::vector<std::vector<float>> all_leaves(num_trees);

    const double sumy = sum(mat(y));

    const size_t feats_per_node = std::max(
        1.0, std::round(fe.max_num_feats() * feature_subsampling_frac));
    ...
    auto build_tree = [&](long i) {
      ...
      while (ranges_to_process.size() > 0) {
        ...
        auto best_split =
            find_best_split_among_feats(fe, range, feats, x, y, idxs);

        range_t left_split(best_split.left_sum, range.begin,
                           best_split.split_idx);
        range_t right_split(best_split.right_sum, best_split.split_idx,
                            range.end);
        ...
        if (left_split.size() > min_samples_per_leaf &&
            right_split.size() != 0) {
        ...
        // This could happen if the features are all uniform so there just
        // isn't any way to split them anymore.
        if (right_split.size() > min_samples_per_leaf &&
            left_split.size() != 0) {
          // allocate an interior leaf node for it.
      ...
      if (compute_oob_values) {
        ...
        for (size_t i = 0; i < oob_values.size(); ++i) {
          ...
          if (j == idxs.size() || idxs[j].second != i) {
            ...
            while (idx < tree.size()) {
              auto feature_value =
                  fe.extract_feature_value(x[i], tree[idx].split_feature);
              if (feature_value < tree[idx].split_threshold)
                idx = tree[idx].left;
              else
                idx = tree[idx].right;
    ...
    if (compute_oob_values) {
      ...
      // If there are some elements that didn't get hits, we set their oob
      // values to the mean oob value.
      if (cnt != 0) {
    ...
    return trained_function_type(std::move(fe), std::move(all_trees),
                                 std::move(all_leaves));
  }

  struct range_t {
    range_t(double sumy, uint32_t begin, uint32_t end)
  ...
  static best_split_details
  find_best_split(const range_t &range, const std::vector<double> &y,
                  const std::vector<std::pair<float, uint32_t>> &idxs)
  /*!
      requires
          - max(mat(idxs)) < y.size()
          - range.sumy == sum of y[idxs[j].second] for all valid j in range
  [range.begin, range.end). ensures
          - finds a threshold T such that there exists an i satisfying the
  following:
              - y[idxs[j].second] < T for all j <= i
              - y[idxs[j].second] > T for all j > i
            Therefore, the threshold T partitions the contents of y into two
  groups, relative to the ordering established by idxs.  Moreover the
  partitioning of y values into two groups has the additional requirement that
  it is optimal in the sense that the sum of the squared deviations from each
  ...
  !*/
  {
    ...
    for (size_t i = range.begin; i + 1 < range.end; ++i) {
      ...
      const double score = left_sum * left_sum / left_size +
                           right_sum * right_sum / (size - left_size);
  ...
  static best_split_details find_best_split_among_feats(
      const feature_extractor &fe, const range_t &range,
    ...
    auto compare_first = [](const std::pair<float, uint32_t> &a,
                            const std::pair<float, uint32_t> &b) {
    ...
    for (auto &feat : feats) {
      ...
      std::sort(idxs.begin() + range.begin, idxs.begin() + range.end,
                compare_first);
    ...
    for (auto i = range.begin; i < range.end; ++i)
      idxs[i].first =
          fe.extract_feature_value(x[idxs[i].second], best.split_feature);
    std::sort(idxs.begin() + range.begin, idxs.begin() + range.end,
              compare_first);
