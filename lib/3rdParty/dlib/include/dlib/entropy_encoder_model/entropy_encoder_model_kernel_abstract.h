...
class entropy_encoder_model {
  ...
  entropy_encoder/entropy_encoder_kernel_abstract.h
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          can use it to write symbols to an entropy_encoder and it will
  calculate the cumulative counts/probabilities and manage contexts for you.
          ...
          Also note that this object does not perform any buffering of symbols.
  It writes them to its associated entropy_encoder immediately. This makes it
  safe to use multiple entropy_encoder_model objects with a single
  entropy_encoder without them trampling each other.
  !*/

public:
  typedef entropy_encoder entropy_encoder_type;

  entropy_encoder_model(entropy_encoder &coder);
  /*!
      ensures
  ...
  void encode(unsigned long symbol);
  /*!
      requires
...
private:
  ...
  entropy_encoder_model(entropy_encoder_model<alphabet_size, entropy_encoder>
                            &); // copy constructor
  entropy_encoder_model<alphabet_size, entropy_encoder> &
  operator=(entropy_encoder_model<alphabet_size, entropy_encoder>
                &); // assignment operator
};
