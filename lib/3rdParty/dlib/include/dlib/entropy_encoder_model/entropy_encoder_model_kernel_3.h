...
template <unsigned long alphabet_size, typename entropy_encoder, typename cc,
          typename cc_high>
class entropy_encoder_model_kernel_3 {
  /*!
      REQUIREMENTS ON cc
          cc is an implementation of
  conditioning_class/conditioning_class_kernel_abstract.h
          cc::get_alphabet_size() == alphabet_size+1

      REQUIREMENTS ON cc_high
          cc_high is an implementation of
  conditioning_class/conditioning_class_kernel_abstract.h
      ...
      INITIAL VALUE
          ...
          - order_2 == pointer to an array of alphabet_size*alphabet_size
  elements
          - order_2[i] == 0
      ...
      CONVENTION
          ...
          This is an order-2-1-0 model. The last symbol in the order-2, order-1
  and order-0 contexts is an escape into the lower context.
...
public:
  ...
  entropy_encoder_model_kernel_3(entropy_encoder &coder);
...
private:
  ...
  entropy_encoder_model_kernel_3(
      entropy_encoder_model_kernel_3 &); // copy constructor
  entropy_encoder_model_kernel_3 &
  operator=(entropy_encoder_model_kernel_3 &); // assignment operator
};
...
entropy_encoder_model_kernel_3<alphabet_size, entropy_encoder, cc, cc_high>::
    entropy_encoder_model_kernel_3(entropy_encoder &coder_)
    : coder(coder_), order_0(gs), order_1(0), previous_symbol(0), order_2(0),
      previous_symbol2(0) {
...
template <unsigned long alphabet_size, typename entropy_encoder, typename cc,
          typename cc_high>
entropy_encoder_model_kernel_3<alphabet_size, entropy_encoder, cc,
                               cc_high>::~entropy_encoder_model_kernel_3() {
...
template <unsigned long alphabet_size, typename entropy_encoder, typename cc,
          typename cc_high>
void entropy_encoder_model_kernel_3<alphabet_size, entropy_encoder, cc,
                                    cc_high>::clear() {
  previous_symbol = 0;
  previous_symbol2 = 0;
  order_0.clear();
...
template <unsigned long alphabet_size, typename entropy_encoder, typename cc,
          typename cc_high>
void entropy_encoder_model_kernel_3<alphabet_size, entropy_encoder, cc,
                                    cc_high>::encode(unsigned long symbol) {
  ...
  // order-2 context stuff
  {
    ...
    if (order_2[temp] != 0) {
      if (order_2[temp]->get_range(symbol, low_count, high_count,
                                   total_count)) {
      ...
      order_2[temp]->get_range(alphabet_size, low_count, high_count,
                               total_count);
      coder.encode(low_count, high_count, total_count);
