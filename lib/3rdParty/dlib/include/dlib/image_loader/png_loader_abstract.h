...
public:
  ...
  /*!
      ensures
          - if (this object contains a grayscale image with an alpha channel)
  then
              - returns true
          - else
              - returns false
  ...
  /*!
      ensures
          - returns the number of bits per channel in the image contained by
  this object.  The possible values are 8 or 16.
  !*/

  template <typename image_type> void get_image(image_type &img) const;
  /*!
      requires
...
void load_png(image_type &image, const std::string &file_name);
/*!
    requires
