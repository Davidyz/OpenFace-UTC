// Copyright (C) 2011  Davis E. King (davis@dlib.net), Nils Labugt, Changjiang
// Yang (yangcha@leidos.com) License: Boost Software License   See LICENSE.txt
...
enum type { BMP, JPG, PNG, DNG, GIF, UNKNOWN };
...
void load_image(image_type &image, const std::string &file_name) {
  ...
  switch (im_type) {
  case image_file_type::BMP:
    load_bmp(image, file_name);
    return;
  case image_file_type::DNG:
    load_dng(image, file_name);
    return;
#ifdef DLIB_PNG_SUPPORT
  case image_file_type::PNG:
    load_png(image, file_name);
    return;
#endif
#ifdef DLIB_JPEG_SUPPORT
  case image_file_type::JPG:
  ...
  case image_file_type::GIF: {
    ...
    GifFileType *gif =
        DGifOpenFileName(file_name.c_str() DLIB_GIFLIB_HANDLE_DIFF_VERSIONS);
    try {
      if (gif == 0)
      ...
      if (gif->SavedImages == 0)
        throw image_load_error("Unsupported GIF format 1.");

      ColorMapObject *cmo = gif->SColorMap
                                ? gif->SColorMap
                                : gif->SavedImages->ImageDesc.ColorMap;

      if (cmo == 0)
        throw image_load_error("Unsupported GIF format 2.");
      if (cmo->Colors == 0)
        throw image_load_error("Unsupported GIF format 3.");
      if (gif->SavedImages->ImageDesc.Width != gif->SWidth)
        throw image_load_error("Unsupported GIF format 4.");
      if (gif->SavedImages->ImageDesc.Height != gif->SHeight)
        throw image_load_error("Unsupported GIF format 5.");
      if (gif->SavedImages->RasterBits == 0)
        throw image_load_error("Unsupported GIF format 6.");
      if (gif->Image.Top != 0)
        throw image_load_error("Unsupported GIF format 7.");
      if (gif->Image.Left != 0)
        throw image_load_error("Unsupported GIF format 8.");
      ...
      if (gif->Image.Interlace) {
        ...
        for (int i = 0; i < 4; ++i) {
          for (long r = interlaced_offset[i]; r < img.nr();
               r += interlaced_jumps[i]) {
  ...
  if (im_type == image_file_type::JPG) {
    ...
    sout << "Unable to load image in file " + file_name + ".\n" +
                "You must #define DLIB_JPEG_SUPPORT and link to libjpeg to "
                "read JPEG files.\n" +
                "Do this by following the instructions at "
                "http://dlib.net/compile.html.\n\n";
#ifdef _MSC_VER
    sout << "Note that you must cause DLIB_JPEG_SUPPORT to be defined for your "
            "entire project.\n";
    sout << "So don't #define it in one file. Instead, add it to the "
            "C/C++->Preprocessor->Preprocessor Definitions\n";
    sout << "field in Visual Studio's Property Pages window so it takes effect "
            "for your entire application.";
#else
    sout << "Note that you must cause DLIB_JPEG_SUPPORT to be defined for your "
            "entire project.\n";
    sout << "So don't #define it in one file. Instead, use a compiler switch "
            "like -DDLIB_JPEG_SUPPORT\n";
    ...
    std::ostringstream sout;
    sout << "Unable to load image in file " + file_name + ".\n" +
                "You must #define DLIB_PNG_SUPPORT and link to libpng to read "
                "PNG files.\n" +
                "Do this by following the instructions at "
                "http://dlib.net/compile.html.\n\n";
#ifdef _MSC_VER
    sout << "Note that you must cause DLIB_PNG_SUPPORT to be defined for your "
            "entire project.\n";
    sout << "So don't #define it in one file. Instead, add it to the "
            "C/C++->Preprocessor->Preprocessor Definitions\n";
    sout << "field in Visual Studio's Property Pages window so it takes effect "
    ...
    std::ostringstream sout;
    sout << "Unable to load image in file " + file_name + ".\n" +
                "You must #define DLIB_GIF_SUPPORT and link to libgif to read "
                "GIF files.\n\n";
#ifdef _MSC_VER
    sout << "Note that you must cause DLIB_GIF_SUPPORT to be defined for your "
            "entire project.\n";
    sout << "So don't #define it in one file. Instead, add it to the "
            "C/C++->Preprocessor->Preprocessor Definitions\n";
    sout << "field in Visual Studio's Property Pages window so it takes effect "
    ...
    sout << "Note that you must cause DLIB_GIF_SUPPORT to be defined for your "
            "entire project.\n";
    sout << "So don't #define it in one file. Instead, use a compiler switch "
            "like -DDLIB_GIF_SUPPORT\n";
    sout << "so it takes effect for your entire application.";
#endif
    throw image_load_error(sout.str());
  } else {
    throw image_load_error(
        "Unknown image file format: Unable to load image in file " + file_name);
  }
}
