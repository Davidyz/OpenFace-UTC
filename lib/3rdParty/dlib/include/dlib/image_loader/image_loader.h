...
public:
  image_load_error(const std::string &str) : error(EIMAGE_LOAD, str) {}
};

// ----------------------------------------------------------------------------------------

template <typename image_type>
void load_bmp(image_type &image_, std::istream &in_) {
  ...
  try {
    ...
    */
    ...
    if (biSize > 40) {
      if (in.sgetn(reinterpret_cast<char *>(buf), biSize - 40) !=
          static_cast<long>(biSize - 40)) {
    ...
    case 1: {
      ...
      while (bytes_read_so_far != bfOffBits) {
        const long to_read = (long)std::min(bfOffBits - bytes_read_so_far,
      ...
      for (long row = biHeight - 1; row >= 0; --row) {
        for (unsigned long col = 0; col < biWidth; col += 8) {
          ...
          for (int i = 0; i < 8 && col + i < biWidth; ++i) {
            ...
            assign_pixel(image[bottomUp ? biHeight - row - 1 : row][col + i],
                         p);
          }
        }
      ...
      for (long row = biHeight - 1; row >= 0; --row) {
        for (unsigned long col = 0; col < biWidth; col += 2) {
          ...
          if (col + 1 < biWidth) {
            ...
            p.blue = blue[pixel2];
            assign_pixel(image[bottomUp ? biHeight - row - 1 : row][col + 1],
                         p);
          }
        }
        if (in.sgetn(reinterpret_cast<char *>(buf), padding) != padding)
    ...
    case 8: {
      ...
      // check for this case.  It shouldn't happen but some BMP writers screw up
      // the files so we have to do this.
  ...
  try {
    ...
    if (version != 1)
      throw image_load_error(
          "You need the new version of the dlib library to read this dng file");
    ...
    } else // if this is a grayscale_float type image
    {
      ...
      for (long r = 0; r < image.nr(); ++r) {
        for (long c = 0; c < image.nc(); ++c) {
          ...
          if (!is_same_type<typename image_traits<image_type>::pixel_type,
                            long double>::value) {
            double temp = cur;
...
template <typename image_type>
void load_bmp(image_type &image, const std::string &file_name) {
