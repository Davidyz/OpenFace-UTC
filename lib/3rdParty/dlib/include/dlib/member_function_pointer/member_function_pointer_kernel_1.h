...
template <typename PARAM1 = void, typename PARAM2 = void,
          typename PARAM3 = void, typename PARAM4 = void>
class member_function_pointer;

// ----------------------------------------------------------------------------------------

#define DLIB_MFP_SC                                                            \
  DLIB_ASSERT(cb != 0, "\tvoid member_function_pointer::set"                   \
...
template <unsigned long num_args> class mfp_kernel_1_base_class {
  /*
      ...
      any sort of implicit casting between derived member function pointer
     classes that take different numbers of arguments.
  */
protected:
  ...
  template <typename T> class mp_null : public mp_base_base {
  public:
    typedef void (T::*mfp_pointer_type)();

    mp_null(void *, mfp_pointer_type)
        : mp_base_base(0, mfp_null), callback(0) {}
    mp_null() : mp_base_base(0, mfp_null), callback(0) {}

    const mfp_pointer_type callback;
  };

  template <typename mp_impl> class mp_impl_T : public mp_impl {
    /*
  ...
  public:
    ...
    void safe_clone(stack_based_memory_block<mem_size> &buf) {
      // This is here just to validate the assumption that our block of memory
      // we have made in mp_memory is the right size to store the data for this
    ...
    void clone(void *ptr) const {
      new (ptr) mp_impl_T(this->o, this->callback);
    }
  ...
  struct dummy_base {
    virtual void nonnull(){};
    virtual ~dummy_base(){};
    int a;
  };
  struct dummy : virtual public dummy_base {
    void nonnull(){};
  };
...
public:
  mfp_kernel_1_base_class(const mfp_kernel_1_base_class &item) {
    item.mp()->clone(mp_memory.get());
  }

  mfp_kernel_1_base_class() { mp_null_impl().safe_clone(mp_memory); }

  bool operator==(const mfp_kernel_1_base_class &item) const {
  ...
  mfp_kernel_1_base_class &operator=(const mfp_kernel_1_base_class &item) {
    mfp_kernel_1_base_class(item).swap(*this);
    return *this;
  }

  ~mfp_kernel_1_base_class() { destroy_mp_memory(); }

  void clear() { mfp_kernel_1_base_class().swap(*this); }

  bool is_set() const { return mp()->is_set(); }
...
public:
  ...
  void swap(mfp_kernel_1_base_class &item) {
...
protected:
  ...
  // try to compile this code with sizeof(mp_null_impl) == 0 (which is a bug in
  // visual studio). Fortunately, no actual real instances of this template seem
  // to end up with that screwed up value so everything works fine if we just
  // add 1 so that this degenerate case doesn't cause trouble.  Note that we
  // know it all works fine because safe_clone() checks the size of this memory
  // block whenever the member function pointer is used.
  ...
  const mp_base_base *mp() const {
    return static_cast<const mp_base_base *>(mp_memory.get());
  }
};
...
class member_function_pointer<void, void, void, void>
    : public mfp_kernel_1_base_class<0> {
  ...
  template <typename T> class mp_impl : public mp_base {
  public:
    typedef void (T::*mfp_pointer_type)();
    void call() const { (static_cast<T *>(this->o)->*callback)(); }

    mp_impl(void *object, mfp_pointer_type cb)
        : mp_base(object, mfp_nonconst), callback(cb) {}
    const mfp_pointer_type callback;
  };

  template <typename T> class mp_impl_const : public mp_base {
  public:
    typedef void((T::*mfp_pointer_type)() const);
    void call() const { (static_cast<const T *>(this->o)->*callback)(); }

    mp_impl_const(void *object, mfp_pointer_type cb)
        : mp_base(object, mfp_const), callback(cb) {}
    const mfp_pointer_type callback;
  };
...
public:
  ...
  void operator()() const {
    DLIB_MFP_OC;
    static_cast<const mp_base *>(mp_memory.get())->call();
  }

  // the reason for putting disable_if on this function is that it avoids an
  // overload resolution bug in visual studio.
  template <typename T>
  typename disable_if<is_const_type<T>, void>::type
  set(T &object, typename mp_impl<T>::mfp_pointer_type cb) {
    DLIB_MFP_SC;
    destroy_mp_memory();
    mp_impl_T<mp_impl<T>>(&object, cb).safe_clone(mp_memory);
  }

  template <typename T>
  void set(const T &object, typename mp_impl_const<T>::mfp_pointer_type cb) {
    DLIB_MFP_SC;
    destroy_mp_memory();
    mp_impl_T<mp_impl_const<T>>((void *)&object, cb).safe_clone(mp_memory);
  }
};
...
class member_function_pointer<PARAM1, void, void, void>
    : public mfp_kernel_1_base_class<1> {
  class mp_base : public mp_base_base {
  public:
    mp_base(void *ptr, mfp_type type_) : mp_base_base(ptr, type_) {}
  ...
  public:
    ...
    void call(PARAM1 p1) const {
      (static_cast<const T *>(this->o)->*callback)(p1);
    }
...
public:
  ...
  void operator()(PARAM1 p1) const {
    DLIB_MFP_OC;
    static_cast<const mp_base *>(mp_memory.get())->call(p1);
  }
...
class member_function_pointer<PARAM1, PARAM2, void, void>
    : public mfp_kernel_1_base_class<2> {
  class mp_base : public mp_base_base {
  public:
    mp_base(void *ptr, mfp_type type_) : mp_base_base(ptr, type_) {}
  ...
  public:
    ...
    void call(PARAM1 p1, PARAM2 p2) const {
      (static_cast<T *>(this->o)->*callback)(p1, p2);
    }
  ...
  public:
    ...
    void call(PARAM1 p1, PARAM2 p2) const {
      (static_cast<const T *>(this->o)->*callback)(p1, p2);
    }
...
public:
  ...
  void operator()(PARAM1 p1, PARAM2 p2) const {
    DLIB_MFP_OC;
    static_cast<const mp_base *>(mp_memory.get())->call(p1, p2);
  }
...
class member_function_pointer<PARAM1, PARAM2, PARAM3, void>
    : public mfp_kernel_1_base_class<3> {
  class mp_base : public mp_base_base {
  public:
    mp_base(void *ptr, mfp_type type_) : mp_base_base(ptr, type_) {}
  ...
  public:
    ...
    void call(PARAM1 p1, PARAM2 p2, PARAM3 p3) const {
      (static_cast<T *>(this->o)->*callback)(p1, p2, p3);
    }
  ...
  public:
    ...
    void call(PARAM1 p1, PARAM2 p2, PARAM3 p3) const {
      (static_cast<const T *>(this->o)->*callback)(p1, p2, p3);
    }
...
public:
  ...
  void operator()(PARAM1 p1, PARAM2 p2, PARAM3 p3) const {
    DLIB_MFP_OC;
    static_cast<const mp_base *>(mp_memory.get())->call(p1, p2, p3);
  }
...
class member_function_pointer : public mfp_kernel_1_base_class<4> {
  ...
  public:
    ...
    void call(PARAM1 p1, PARAM2 p2, PARAM3 p3, PARAM4 p4) const {
      (static_cast<T *>(this->o)->*callback)(p1, p2, p3, p4);
    }
  ...
  public:
    ...
    void call(PARAM1 p1, PARAM2 p2, PARAM3 p3, PARAM4 p4) const {
      (static_cast<const T *>(this->o)->*callback)(p1, p2, p3, p4);
    }
