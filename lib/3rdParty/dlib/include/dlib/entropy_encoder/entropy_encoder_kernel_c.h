...
template <typename encoder> class entropy_encoder_kernel_c : public encoder {
...
std::ostream &entropy_encoder_kernel_c<encoder>::get_stream() const {
...
template <typename encoder>
void entropy_encoder_kernel_c<encoder>::encode(uint32 low_count,
                                               uint32 high_count,
                                               uint32 total) {
  // make sure requires clause is not broken
  DLIB_CASSERT((0 < total) && (total < 65536) && (low_count < high_count) &&
                   (high_count <= total) && (this->stream_is_set() == true),
               "\tvoid entropy_encoder::encode()"
                   << "\n\trefer to the ensures clause for this function for "
                      "further information"
                   << "\n\tthis:            " << this << "\n\ttotal:           "
                   << total << "\n\tlow_count:       " << low_count
                   << "\n\thigh_count:      " << high_count
                   << "\n\tis_stream_set(): "
                   << (this->stream_is_set() ? "true" : "false"));
...
template <typename encoder> void entropy_encoder_kernel_c<encoder>::flush() {
  // make sure requires clause is not broken
