...
class entropy_encoder_kernel_2 {
  /*!
   ...
   encoding. this number is used as a 32bit fixed point real number. the point
   is fixed just before the first bit, so when we interpret high as a real
   number then it is always in the range [0,1)
...
private:
  ...
  entropy_encoder_kernel_2 &
  operator=(entropy_encoder_kernel_2 &); // assignment operator
