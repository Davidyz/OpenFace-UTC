...
/*!
    ensures
        - if (#get_local_hostname() == 0) then
            - #hostname == a string containing the hostname of the local
computer
...
/*!
    ...
    ensures
        - if (#hostname_to_ip() == 0) then
            - #ip == string containing the nth ip address associated with the
hostname
...
int create_listener(listener *&new_listener, unsigned short port,
                    const std::string &ip = "");
/*!
    requires
        - 0 <= port <= 65535
    ensures
        - if (#create_listener() == 0) then
            - #new_listener == a pointer to a listener object that is listening
on the specified port and ip for an incoming connection
            - if (ip == "") then
...
int create_listener(std::unique_ptr<listener> &new_listener,
                    unsigned short port, const std::string &ip = "");
/*!
...
/*!
    ...
    ensures
        - if (#create_connection() == 0) then
            - #new_connection  == a pointer to a connection object that is
   connected to foreign_ip on port foreign_port and is using the local interface
...
int create_connection(std::unique_ptr<connection> &new_connection,
...
class connection {
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          NOTE:
              A connection object must ALWAYS be closed (delete the pointer to
  the connection) or it will cause a resource leak.

              Note also that all errors indicated by a return code of
  OTHER_ERROR are fatal so if one occurs the connection should just be closed.

      CLOSING A CONNECTION
          Note that if ~connection() or shutdown() is called before the remote
  client has received all sent data it is possible that the data will be lost.
  To avoid this you should call the close_gracefully() function to close your
          connections (unless you actually do want to immediately dispose of a
          connection and don't care about the data).
          (example: close_gracefully(con); // close con gracefully but force it
  closed
                                             // if it takes more than 500
  milliseconds.)
      ...
      THREAD SAFETY
          ...
          - You can safely call shutdown or shutdown_outgoing in conjunction
  with the read/write functions. This is helpful if you want to unblock another
  thread that is blocking on a read/write operation.  Shutting down the
  connection will cause the read/write functions to return a value of SHUTDOWN.
...
public:
  ...
  void *user_data;
  /*!
      This pointer is provided so that the client programmer may easily
  associate some data with a connection object.  You can really do whatever you
  want with it.  Initially user_data is 0.
  !*/
  ...
  /*!
      ...
      ensures
          ...
          - returns 0 if the connection has ended/terminated and there is no
  more data.
  ...
  /*!
      ...
      ensures
          - read() will not read more than num bytes of data into #buf
          - if (timeout > 0) then read() blocks until ONE of the following
  happens:
              - there is some data available and it has been written into #buf
              - the remote end of the connection is closed
              - an error has occurred
          ...
          - returns 0 if the connection has ended/terminated and there is no
  more data.
          - returns TIMEOUT if timeout milliseconds elapsed before we got any
  data.
          - returns OTHER_ERROR if there was an error.
          - returns SHUTDOWN if the connection has been shutdown locally
  !*/
  ...
  unsigned short get_foreign_port() const;
  /*!
      ensures
  ...
  const std::string &get_foreign_ip() const;
  /*!
      ensures
  ...
  /*!
      ensures
          - if (#shutdown() == 0 && connection was still open) then
              - terminates the connection but does not free the resources for
  the connection object

          - any read() or write() calls on this connection will return
  immediately with the code SHUTDOWN.
  ...
  /*!
      ensures
          - if (#shutdown_outgoing() == 0 && outgoing channel was still open)
  then
...
class listener {
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          Calling accept returns a pointer to any new incoming connections on
  its port.
          ...
          NOTE:
              A listener object must ALWAYS be closed (delete the pointer to it)
  or it will cause a resource leak.

              Note also that all errors indicated by a return code of
  OTHER_ERROR are fatal so if one occurs the listener should be closed.
...
public:
  ...
  int accept(std::unique_ptr<connection> &new_connection,
  ...
  const std::string &get_listening_ip() const;
  /*!
      ensures
