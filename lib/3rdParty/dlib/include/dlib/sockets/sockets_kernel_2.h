...
class connection {
  /*!
      ...
      CONVENTION
          ...
          connection_local_ip     == a string containing the IP address of the
                                     local interface being used by this
  connection

          sd                      == if shutdown() has been called then true
                                     else false
          sdo                     == if shutdown_outgoing() has been called then
  true else false sdr                     == the return value of shutdown() if
  it has been called.  if it hasn't been called then 0
  ...
  friend int create_connection(connection *&new_connection,
...
private:
  ...
  connection(int sock, int foreign_port, const std::string &foreign_ip,
             int local_port, const std::string &local_ip);
  /*!
      requires
          - sock is a socket handle
          - sock is the handle for the connection between
  foreign_ip:foreign_port and local_ip:local_port ensures
...
class listener {
  ...
  friend int create_listener(listener *&new_listener, unsigned short port,
                             const std::string &ip);
...
private:
  ...
  listener(int sock, int port, const std::string &ip);
  /*!
      requires
          - sock is a socket handle
          - sock is listening on the port and ip(may be "") indicated in the
  above parameters ensures
          - *this is initialized correctly with the above parameters
  !*/
