...
template <typename T> struct gate_traits {};
...
public:
  quantum_register() { set_num_bits(1); }

  int num_bits() const { return num_bits_in_register; }
  ...
  void append(const quantum_register &reg) {
    num_bits_in_register += reg.num_bits_in_register;
    state = tensor_product(state, reg.state);
  }

  template <typename rand_type> bool measure_bit(int bit, rand_type &rnd) {
    // make sure requires clause is not broken
    ...
    // Next we set all the states where this bit doesn't have the given value to
    // 0
  ...
  bool measure_and_remove_bit(int bit, rand_type &rnd) {
    // make sure requires clause is not broken
    ...
    for (long r = 0; r < state.nr(); ++r) {
      const unsigned long field = r;
      // if this basis vector is one that matches the measured state then keep
      // it
  ...
  double probability_of_bit(int bit) const {
    // make sure requires clause is not broken
    DLIB_CASSERT(0 <= bit && bit < num_bits(),
    ...
    // now find the total probability of all the states that have the given bit
    // set
    double prob = 0;
    for (long r = 0; r < state.nr(); ++r) {
      const unsigned long field = r;
  ...
  void swap(quantum_register &item) {
...
inline void swap(quantum_register &a, quantum_register &b) { a.swap(b); }

// ----------------------------------------------------------------------------------------

template <typename T> class gate_exp {
public:
  ...
  const matrix<qc_scalar_type> mat() const {
  ...
  void apply_gate_to(quantum_register &reg) const {
    ...
    // we don't have to do the full matrix multiply.  Or check if only a small
    // number are non-zero.
  ...
  qc_scalar_type compute_state_element(const matrix_exp<exp> &reg,
                                       long row_idx) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(reg.nr() == dims && reg.nc() == 1 && 0 <= row_idx &&
                    row_idx < dims,
...
template <typename T, typename U> class composite_gate;

template <typename T, typename U> struct gate_traits<composite_gate<T, U>> {
...
public:
  ...
  composite_gate(const composite_gate &g)
      : gate_exp<composite_gate>(*this), lhs(g.lhs), rhs(g.rhs) {}

  composite_gate(const gate_exp<T> &lhs_, const gate_exp<U> &rhs_)
  ...
  const qc_scalar_type operator()(long r, long c) const {
    return lhs(r / U::dims, c / U::dims) * rhs(r % U::dims, c % U::dims);
  }
  ...
  qc_scalar_type compute_state_element(const matrix_exp<exp> &reg,
    ...
    // make sure requires clause is not broken
    DLIB_ASSERT(
        reg.nr() == dims && reg.nc() == 1 && 0 <= row_idx && row_idx < dims,
    ...
    for (long c = 0; c < T::dims; ++c) {
      if (lhs(row_idx / U::dims, c) != qc_scalar_type(0)) {
        result += lhs(row_idx / U::dims, c) *
                  rhs.compute_state_element(
                      subm(reg, c * U::dims, 0, U::dims, 1), row_idx % U::dims);
      }
    }
...
template <long bits> class gate;
template <long bits> struct gate_traits<gate<bits>> {
...
template <long bits> class gate : public gate_exp<gate<bits>> {
public:
...
namespace qc_helpers {
// This is the maximum number of bits used for cached sub-matrices in
// composite_gate expressions
const int qc_block_chunking_size = 8;

template <typename T> struct is_composite_gate {
  const static bool value = false;
};
template <typename T, typename U>
struct is_composite_gate<composite_gate<T, U>> {
  const static bool value = true;
};

// These overloads all deal with intelligently composing chains of
// composite_gate expressions such that the resulting expression has the form:
//    (gate_exp,(gate_exp,(gate_exp,(gate_exp()))))
// and each gate_exp contains a cached gate matrix for a gate of at most
// qc_block_chunking_size bits. This facilitates the optimizations in the
// compute_state_element() function.
template <typename T, typename U, typename V, typename enabled = void>
struct combine_gates;

// This is a base case of this recursive template.  It takes care of converting
// small composite_gates into cached gate objects.
template <typename T, typename U, typename V>
struct combine_gates<T, U, V,
                     typename enable_if_c<(T::num_bits + U::num_bits <=
                                           qc_block_chunking_size)>::type> {
  typedef composite_gate<gate<T::num_bits + U::num_bits>, V> result_type;

  static const result_type eval(const composite_gate<T, U> &lhs,
...
struct combine_gates<
    T, U, V,
    typename enable_if_c<(is_composite_gate<U>::value == true)>::type> {
  typedef typename combine_gates<typename U::lhs_type, typename U::rhs_type,
                                 V>::result_type inner_type;
  typedef composite_gate<T, inner_type> result_type;

  static const result_type eval(const composite_gate<T, U> &lhs,
                                const gate_exp<V> &rhs) {
    return composite_gate<T, inner_type>(
        lhs.lhs,
        combine_gates<typename U::lhs_type, typename U::rhs_type, V>::eval(
            lhs.rhs, rhs));
  }
};

// This is a base case of this recursive template.  It takes care of adding new
// gates when the left hand side is too big to just turn it into a cached gate
// object.
template <typename T, typename U, typename V>
struct combine_gates<
    T, U, V,
    typename enable_if_c<(T::num_bits + U::num_bits > qc_block_chunking_size &&
...
template <typename T, typename U>
const composite_gate<T, U> operator,(const gate_exp<T> &lhs,
...
const typename qc_helpers::combine_gates<T, U, V>::result_type operator,(
    const composite_gate<T, U> &lhs, const gate_exp<V> &rhs) {
  return qc_helpers::combine_gates<T, U, V>::eval(lhs, rhs);
}

// If you are getting an error here then it means that you are trying to combine
// a gate expression with an integer somewhere (and that is an error).
template <typename T> void operator,(const gate_exp<T> &, int) {
  COMPILE_TIME_ASSERT(sizeof(T) > 100000000);
}
template <typename T> void operator,(int, const gate_exp<T> &) {
  COMPILE_TIME_ASSERT(sizeof(T) > 100000000);
}
...
template <int control_bit, int target_bit> class cnot;

template <int control_bit1, int control_bit2, int target_bit> class toffoli;
} // namespace quantum_gates
...
struct gate_traits<
    quantum_gates::toffoli<control_bit1, control_bit2, target_bit>> {
  static const long num_bits =
      tmax<tabs<control_bit1 - target_bit>::value,
           tabs<control_bit2 - target_bit>::value>::value +
      1;
  static const long dims = qc_helpers::exp_2_n<num_bits>::value;
};
...
public:
  ...
  cnot() : gate_exp<cnot>(*this) {
    ...
    // make the masks so that their only on bit corresponds to the given
    // control_bit and target_bit bits
...
template <int control_bit1, int control_bit2, int target_bit>
class toffoli
    : public gate_exp<toffoli<control_bit1, control_bit2, target_bit>> {
public:
  COMPILE_TIME_ASSERT(control_bit1 != target_bit &&
  ...
  toffoli() : gate_exp<toffoli>(*this) {
    const int min_bit =
        std::min(std::min(control_bit1, control_bit2), target_bit);
    ...
    // make the masks so that their only on bit corresponds to the given
    // control_bit1 and target_bit bits
