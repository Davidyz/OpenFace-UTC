...
public:
  ...
  void set_num_bits(int new_num_bits);
  /*!
      requires
          - 1 <= new_num_bits <= 30
      ensures
          ...
          - #state_vector().nr() == 2^new_num_bits
            (i.e. the size of the state_vector is exponential in the number of
  bits in a register)
          - for all valid i:
  ...
  /*!
      ensures
          ...
          - #this->state_vector() == tensor_product(this->state_vector(),
  reg.state_vector())
          - The original bits in *this become the high order bits of the
  resulting register and all the bits in reg end up as the low order bits in the
            resulting register.
  !*/
  ...
  /*!
      requires
          - 0 <= bit < num_bits()
      ensures
          - returns the probability of measuring the given bit and it being in
  the 1 state.
          - The returned value is also equal to the sum of
  norm(state_vector()(i)) for all i where the bit'th bit in i is set to 1. (note
  that the lowest order bit is bit 0)
  !*/
  ...
  const matrix<qc_scalar_type, 0, 1> &state_vector() const;
  /*!
      ensures
          - returns a const reference to the state vector that describes the
  state of the quantum bits in this register.
  !*/

  matrix<qc_scalar_type, 0, 1> &state_vector();
  /*!
      ensures
          - returns a non-const reference to the state vector that describes the
  state of the quantum bits in this register.
  !*/
...
template <typename T> class gate_exp {
  /*!
      REQUIREMENTS ON T
          T must be some object that inherits from gate_exp and implements its
  own version of operator() and compute_state_element().
...
public:
  ...
  ref().compute_state_element(reg,row_idx)
  !*/
...
class composite_gate : public gate_exp<composite_gate<T, U>> {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          As an example, suppose you have 3 registers, reg_high, reg_low, and
  reg_all.  Also suppose that reg_all is what you get when you append reg_high
  and reg_low, so reg_all.state_vector() ==
  tensor_product(reg_high.state_vector(),reg_low.state_vector()).

          Then applying a composite gate to reg_all would give you the same
  thing as applying the lhs gate to reg_high and the rhs gate to reg_low and
  then appending the two resulting registers.  So the lhs gate of a
  composite_gate applies to the high order bits of a regitser and the rhs gate
  applies to the lower order bits.
  !*/
public:
  ...
  /*!
      ...
      ensures
          - Let M represent the matrix for this gate, then this function
            ...
            (i.e. returns the row_idx row of what you get when you apply this
            gate to the given column vector in reg)
          - This function works by calling rhs.compute_state_element() and using
  elements of the matrix in lhs.
  !*/
...
public:
  ...
  template <typename T> explicit gate(const gate_exp<T> &g);
  /*!
      requires
          - T::num_bits == num_bits
      ensures
  ...
  qc_scalar_type &operator()(long r, long c);
  /*!
      requires
          - 0 <= r < dims
          - 0 <= c < dims
...
class cnot : public gate_exp<cnot<control_bit, target_bit>> {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents the controlled-not quantum gate.  It is a gate
  that operates on abs(control_bit-target_bit)+1 qubits.

          In terms of the computational basis vectors, this gate maps input
          vectors to output vectors in the following way:
              - if (the input vector corresponds to a state where the
  control_bit qubit is 1) then
                  - this gate outputs the computational basis vector that
                    corresponds to the state where the target_bit has been
  flipped with respect to the input vector
              - else
...
class toffoli
  ...
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents the toffoli variant of a controlled-not quantum
  gate. It is a gate that operates on
  max(abs(control_bit2-target_bit),abs(control_bit1-target_bit))+1 qubits.

          In terms of the computational basis vectors, this gate maps input
          vectors to output vectors in the following way:
              - if (the input vector corresponds to a state where the
  control_bit1 and control_bit2 qubits are 1) then
                  - this gate outputs the computational basis vector that
                    corresponds to the state where the target_bit has been
  flipped with respect to the input vector
              - else
