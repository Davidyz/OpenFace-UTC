...
class xml_parser {
  typedef dlib::map<std::string, std::string,
                    memory_manager<char>::kernel_2a>::kernel_1b map;
  typedef dlib::stack<std::string, memory_manager<char>::kernel_2a>::kernel_1a
      stack;
...
public:
  ...
  inline void add_document_handler(document_handler &item);

  inline void add_error_handler(error_handler &item);

  inline void swap(xml_parser &item);
...
private:
  ...
  public:
    ...
    bool is_in_list(const std::string &key) const {
    ...
    const std::string &operator[](const std::string &key) const {
      if (is_in_list(key))
        ...
        throw xml_attribute_list_error("No XML attribute named " + key +
                                       " is present in tag.");
    }

    bool at_start() const { return list.at_start(); }

    void reset() const { return list.reset(); }

    bool current_element_valid() const { return list.current_element_valid(); }

    const type &element() const { return list.element(); }

    type &element() { return list.element(); }

    bool move_next() const { return list.move_next(); }
  ...
  */
  ...
  inline void get_next_token(std::istream &in, std::string &token_text,
  ...
  inline int parse_element(const std::string &token, std::string &name,
                           attrib_list &atts);
  /*!
      requires
  ...
  inline int parse_pi(const std::string &token, std::string &target,
                      std::string &data);
  /*!
      requires
  ...
  inline int parse_element_end(const std::string &token, std::string &name);
  /*!
      requires
  ...
  inline int change_entity(std::istream &in);
  /*!
      ensures
...
inline void swap(xml_parser &a, xml_parser &b) { a.swap(b); }
...
void xml_parser::parse(std::istream &in) {
  ...
  try {
    ...
    while (token_kind != eof) {
      bool is_empty =
          false; // this becomes true when this token is an empty_element
      ...
      if ((token_kind != chars) && (token_kind != chars_cdata) &&
          (token_kind != dtd) && (token_kind != comment) &&
          (chars_buf.size() != 0)) {
        // notify all the document_handlers
        for (unsigned long i = 0; i < dh_list.size(); ++i) {
...
void xml_parser::add_document_handler(document_handler &item) {
...
void xml_parser::add_error_handler(error_handler &item) {
...
void xml_parser::swap(xml_parser &item) {
...
void xml_parser::get_next_token(std::istream &in, std::string &token_text,
                                int &token_kind, unsigned long &line_number) {
...
int xml_parser::parse_element(const std::string &token, std::string &name,
                              attrib_list &atts) {
  name.erase();
  atts.list.clear();
  ...
  while (ch != '>' && ch != '/') {
    ...
    std::istream::int_type delimiter =
        ch;        // this should be either a ' or " character
...
int xml_parser::parse_pi(const std::string &token, std::string &target,
                         std::string &data) {
  target.erase();
  data.erase();
...
int xml_parser::parse_element_end(const std::string &token, std::string &name) {
  name.erase();
...
int xml_parser::change_entity(std::istream &in) {
  ...
  if (buf[1] != 'a' && buf[1] != 'l' && buf[1] != 'g' && buf[1] != 'q')
    return -1;

  buf[2] = in.get();
  // if this is an undefined entity reference then return error
  if (buf[2] != 'm' && buf[2] != 't' && buf[2] != 'p' && buf[2] != 'u')
    return -1;

  buf[3] = in.get();
  // if this is an undefined entity reference then return error
  if (buf[3] != 'p' && buf[3] != ';' && buf[3] != 'o')
    return -1;
  ...
  // if this should be &apos;
  if (buf[1] == 'a' && buf[2] == 'p' && buf[3] == 'o' && buf[4] == 's' &&
      buf[5] == ';')
    return '\'';

  // if this should be &quot;
  if (buf[1] == 'q' && buf[2] == 'u' && buf[3] == 'o' && buf[4] == 't' &&
      buf[5] == ';')
    return '"';
...
public:
  xml_parse_error(const std::string &a) : error(a) {}
};
...
public:
  ...
  default_xml_error_handler(const std::string &filename_)
      : filename(filename_) {}

  virtual void error(const unsigned long) {
    // just ignore non-fatal errors
  }

  virtual void fatal_error(const unsigned long line_number) {
    std::ostringstream sout;
    if (filename.size() != 0)
      sout << "There is a fatal error on line " << line_number
...
inline void parse_xml(std::istream &in, document_handler &dh,
                      error_handler &eh) {
  if (!in)
...
inline void parse_xml(std::istream &in, error_handler &eh,
                      document_handler &dh) {
  if (!in)
    throw xml_parse_error("Unexpected end of file during xml parsing.");
  xml_parser parser;
...
inline void parse_xml(const std::string &filename, document_handler &dh,
                      error_handler &eh) {
...
inline void parse_xml(const std::string &filename, error_handler &eh,
                      document_handler &dh) {
  std::ifstream in(filename.c_str());
  if (!in)
    throw xml_parse_error("Unable to open file '" + filename + "'.");
