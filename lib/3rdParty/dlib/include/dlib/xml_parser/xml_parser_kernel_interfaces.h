...
class xml_attribute_list_error : public dlib::error {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This is an exception object thrown by attribute_list objects if you
...
public:
  inline virtual ~attribute_list() = 0;

  virtual bool is_in_list(const std::string &key) const = 0;
  /*!
      ensures
...
public:
  inline virtual ~document_handler() = 0;
  ...
  virtual void start_element(const unsigned long line_number,
                             ...
                             const dlib::attribute_list &atts) = 0;
  /*!
      requires
          - is called when an opening element tag is encountered.
          - line_number == the line number where the opening tag for this
  element was encountered.
          - name == the name of the element encountered
          - atts == a list containing all the attributes in this element and
  ...
  /*!
      requires
          - is called when a closing element tag is encountered. (note that this
            ...
            trigger a start_element() callback as well as an end_element()
  callback)
  ...
  /*!
      requires
          ...
          - data == all the normal non-markup data and CDATA between the next
  and last tag in the document.
  !*/
...
public:
  inline virtual ~error_handler() = 0;

  virtual void error(const unsigned long line_number) = 0;
  /*!
      requires
  ...
  virtual void fatal_error(const unsigned long line_number) = 0;
  /*!
      requires
          - is called when an error that requires the parser to abort its
  parsing is encountered (i.e. fatal errors in the input)
          - line_number == the line number where this fatal error was
  encountered
      ...
      the following events trigger a fatal_error:
          ...
          Hitting EOF before the closing tag for the document is also a
  fatal_error.
  !*/
