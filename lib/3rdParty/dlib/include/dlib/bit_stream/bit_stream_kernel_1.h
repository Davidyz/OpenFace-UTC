...
public:
  bit_stream_kernel_1() : write_mode(false), read_mode(false) {}
  ...
  inline bool is_in_write_mode() const;

  inline bool is_in_read_mode() const;

  inline void write(int bit);

  bool read(int &bit);

  void swap(bit_stream_kernel_1 &item);
...
inline void swap(bit_stream_kernel_1 &a, bit_stream_kernel_1 &b);
