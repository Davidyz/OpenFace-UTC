...
class byte_orderer {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          functions does the exact same thing as the host_to_big() and
  big_to_host() functions and is provided simply so that client code can use the
  most self documenting name appropriate.

          Also note that this object is capable of correctly flipping the
  contents of arrays when the arrays are declared on the stack.  e.g.  You can
          say things like:
          int array[10];
...
public:
  ...
  template <typename T> void host_to_network(T &item) const;
  /*!
      ensures
  ...
  template <typename T> void network_to_host(T &item) const;
  /*!
      ensures
  ...
  template <typename T> void host_to_big(T &item) const;
  /*!
      ensures
          - #item == the value of item converted from host byte order
  ...
  template <typename T> void big_to_host(T &item) const;
  /*!
      ensures
  ...
  template <typename T> void host_to_little(T &item) const;
  /*!
      ensures
          - #item == the value of item converted from host byte order
  ...
  template <typename T> void little_to_host(T &item) const;
  /*!
      ensures
