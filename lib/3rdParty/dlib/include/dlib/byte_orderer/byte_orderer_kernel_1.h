...
public:
  ...
  byte_orderer() {
    // This will probably never be false but if it is then it means chars are
    // not 8bits on this system.  Which is a problem for this object.
  ...
  bool host_is_big_endian() const { return !little_endian; }

  bool host_is_little_endian() const { return little_endian; }

  template <typename T> inline void host_to_network(T &item) const {
    if (little_endian)
      flip(item);
  }

  template <typename T> inline void network_to_host(T &item) const {
    if (little_endian)
      flip(item);
  }
