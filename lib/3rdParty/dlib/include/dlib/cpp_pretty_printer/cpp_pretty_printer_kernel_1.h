...
template <typename stack, typename tok> class cpp_pretty_printer_kernel_1 {
  /*!
...
private:
  ...
  cpp_pretty_printer_kernel_1(
      const cpp_pretty_printer_kernel_1 &); // copy constructor
  cpp_pretty_printer_kernel_1 &
  operator=(const cpp_pretty_printer_kernel_1 &); // assignment operator
};
...
cpp_pretty_printer_kernel_1<stack, tok>::cpp_pretty_printer_kernel_1() {}
...
template <typename stack, typename tok>
void cpp_pretty_printer_kernel_1<stack, tok>::print(
    std::istream &in, std::ostream &out, const std::string &title) const {
  using namespace std;

  if (!out)
    throw std::ios::failure(
        "error occurred in cpp_pretty_printer_kernel_1::print");
  ...
  // true if we have seen the #pragma or #if or #define or #elif keywords and
  // have not seen an end of line.
...
template <typename stack, typename tok>
void cpp_pretty_printer_kernel_1<stack, tok>::print_and_number(
    std::istream &in, std::ostream &out, const std::string &title) const {
  using namespace std;
  ostringstream sout;
  print(in, sout, title);
...
template <typename stack, typename tok>
void cpp_pretty_printer_kernel_1<stack, tok>::number(std::istream &in,
                                                     std::ostream &out) const {
  if (!out)
    throw std::ios::failure(
        "error occurred in cpp_pretty_printer_kernel_1::number");
...
template <typename stack, typename tok>
const std::string
cpp_pretty_printer_kernel_1<stack, tok>::htmlify(const std::string &str) const {
