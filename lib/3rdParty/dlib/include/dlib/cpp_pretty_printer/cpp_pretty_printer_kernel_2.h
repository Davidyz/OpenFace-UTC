...
template <typename stack, typename tok> class cpp_pretty_printer_kernel_2 {
  /*!
      ...
      INFO
          ...
          for printing on a black and white printer.  It also places the
  document title prominently at the top of the pretty printed source file.
  !*/
...
public:
  ...
  void print_and_number(std::istream &in, std::ostream &out,
                        const std::string &title) const;
...
private:
  ...
  const std::string htmlify(const std::string &str) const;
  /*!
      ensures
  ...
  void number(std::istream &in, std::ostream &out) const;
  /*!
      ensures
  ...
  cpp_pretty_printer_kernel_2(
      const cpp_pretty_printer_kernel_2 &); // copy constructor
  cpp_pretty_printer_kernel_2 &
  operator=(const cpp_pretty_printer_kernel_2 &); // assignment operator
};
...
template <typename stack, typename tok>
cpp_pretty_printer_kernel_2<stack, tok>::cpp_pretty_printer_kernel_2() {}
...
template <typename stack, typename tok>
void cpp_pretty_printer_kernel_2<stack, tok>::print(
    std::istream &in, std::ostream &out, const std::string &title) const {
  using namespace std;

  if (!out)
    throw std::ios::failure(
        "error occurred in cpp_pretty_printer_kernel_2::print");

  t.set_stream(in);

  out << "<html><!-- "
      << "Created using the cpp_pretty_printer from the dlib C++ library.  See "
         "http://dlib.net for updates."
      << " --><head>"
  ...
  unsigned long scope =
      0; // counts the number of new scopes we have entered
  ...
  // true if we have seen a ) and we have only seen white_space or comments
  // since
  ...
  while (type != tok::END_OF_FILE) {
    ...
    case tok::IDENTIFIER: // ------------------------------------------
      ...
      } else if (!in_initialization_list && !recently_seen_preprocessor &&
                 scope == 0 && paren_count == 0) {
    ...
    case tok::KEYWORD: // ---------------------------------------------
      ...
      if (token == "#include") {
        ...
        recently_seen_new_scope = true;
        recently_seen_class_keyword = true;
      } else if (token == "#pragma" || token == "#define" || token == "#elif" ||
                 token == "#if") {
...
template <typename stack, typename tok>
void cpp_pretty_printer_kernel_2<stack, tok>::print_and_number(
    std::istream &in, std::ostream &out, const std::string &title) const {
  using namespace std;
  ostringstream sout;
  print(in, sout, title);
...
template <typename stack, typename tok>
void cpp_pretty_printer_kernel_2<stack, tok>::number(std::istream &in,
                                                     std::ostream &out) const {
  if (!out)
    throw std::ios::failure(
        "error occurred in cpp_pretty_printer_kernel_2::number");
...
template <typename stack, typename tok>
const std::string
cpp_pretty_printer_kernel_2<stack, tok>::htmlify(const std::string &str) const {
