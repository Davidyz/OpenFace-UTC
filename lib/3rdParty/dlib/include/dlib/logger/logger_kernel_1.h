...
class log_level {
public:
  log_level(int priority_, const char *name_) : priority(priority_) {
...
void set_all_logging_output_hooks(
    T &object,
    void (T::*hook_)(const std::string &logger_name, const log_level &l,
...
class logger {
  ...
  loggers.
          ...
          - logger::gd::next_thread_name == the next thread name that will be
  given out to a thread when we find that it isn't already in thread_names.
  !*/
  ...
  class logger_stream {
    ...
    !*/
  public:
    logger_stream(const log_level &l_, logger &log_)
        : l(l_), log(log_), been_used(false),
    ...
    template <typename T> inline logger_stream &operator<<(const T &item) {
      if (!enabled) {
        return *this;
      } else {
...
public:
  ...
  const std::string &name() const { return logger_name; }

  logger_stream operator<<(const log_level &l) const {
    return logger_stream(l, const_cast<logger &>(*this));
  }
  ...
  void set_output_stream(std::ostream &out_) {
    auto_mutex M(gd.m);
    gd.loggers.reset();
    while (gd.loggers.move_next()) {
      if (gd.loggers.element()->is_child_of(*this)) {
  ...
  print_header_type logger_header() const { return print_header; }

  void set_logger_header(print_header_type ph) {
    auto_mutex M(gd.m);
    gd.loggers.reset();
    while (gd.loggers.move_next()) {
      if (gd.loggers.element()->is_child_of(*this))
...
private:
  ...
  struct global_data {
    ...
    // Make a very simple streambuf that writes characters into a
    // std::vector<char>.  We can use this as the output target for hooks.  The
    // reason we don't just use a std::ostringstream instead is that this way we
    // can be guaranteed that logging doesn't perform memory allocations. This
    // is because a std::vector never frees memory.  I.e. its capacity() doesn't
    // go down when you resize it back to 0.  It just stays the same.
    ...
    const log_level level(const std::string &name) const;
    /*!
        ensures
    ...
    void set_level(const std::string &name, const log_level &new_level);
    /*!
        ensures
    ...
    struct auto_flush_container {
      bool val;
      map<std::string, std::unique_ptr<auto_flush_container>>::kernel_1b_c
          table;
    } auto_flush_table;

    bool auto_flush(const std::string &name) const;
    /*!
        ensures
            - returns the auto_flush value loggers with the given name are
    supposed to have
    !*/

    void set_auto_flush(const std::string &name, bool enabled);
    /*!
        ensures
            - for all children C of name:
    ...
    struct output_streambuf_container {
      std::streambuf *val;
      map<std::string, std::unique_ptr<output_streambuf_container>>::kernel_1b_c
          table;
    } streambuf_table;

    std::streambuf *output_streambuf(const std::string &name);
    /*!
        ensures
    ...
    void set_output_stream(const std::string &name, std::ostream &out_);
    /*!
        ensures
            - for all children C of name:
    ...
    void set_output_stream(const std::string &name, std::streambuf &buf);
    /*!
        ensures
            - for all children C of name:
    ...
    struct output_hook_container {
      hook_mfp val;
      map<std::string, std::unique_ptr<output_hook_container>>::kernel_1b_c
          table;
    } hook_table;

    hook_mfp output_hook(const std::string &name);
    /*!
        ensures
    ...
    void set_output_hook(const std::string &name, const hook_mfp &hook);
    /*!
        ensures
            - for all children C of name:
                - #output_hook(C) == hook
    ...
    struct logger_header_container {
      ...
      map<std::string, std::unique_ptr<logger_header_container>>::kernel_1b_c
          table;
    } header_table;

    print_header_type logger_header(const std::string &name);
    /*!
        ensures
            - returns the header function loggers with the given name are
    supposed to have
    !*/

    void set_logger_header(const std::string &name, print_header_type ph);
    /*!
        ensures
            - for all children C of name:
                - #logger_header(C) == ph
  ...
  friend void set_all_logging_levels(const log_level &new_level);

  friend void set_all_logging_headers(const print_header_type &new_header);

  friend void set_all_logging_output_streams(std::ostream &out);
