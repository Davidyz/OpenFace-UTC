...
class graph : noncopyable {
  ...
  is when they reference nodes or edges that have been removed from a graph.
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents an undirected graph which is a set of nodes
  with undirected edges connecting various nodes.
...
public:
  ...
  struct node_type {
    ...
    unsigned long number_of_neighbors() const;
    /*!
        ensures
    ...
    /*!
        requires
            - edge_index < number_of_neighbors()
        ensures
            - returns a non-const reference to the edge_index'th neighbor of
    *this
    !*/
    ...
    the edge connecting to neighbor this->neighbor(edge_index)
    !*/
  };
  ...
  /*!
      ...
      ensures
          - if (there is an edge connecting node(node_index1) and
  node(node_index2)) then
              - returns true
          - else
              - returns false
  !*/
...
template <typename T, typename E, typename mem_manager>
inline void swap(graph<T, E, mem_manager> &a, graph<T, E, mem_manager> &b) {
  a.swap(b);
}
/*!
...
template <typename T, typename E, typename mem_manager>
void serialize(const graph<T, E, mem_manager> &item, std::ostream &out);
/*!
    provides deserialization support
!*/

template <typename T, typename E, typename mem_manager>
void deserialize(graph<T, E, mem_manager> &item, std::istream &in);
/*!
    provides deserialization support
!*/
