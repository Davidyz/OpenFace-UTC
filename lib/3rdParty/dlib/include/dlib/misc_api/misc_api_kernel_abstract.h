...
void set_current_dir(const std::string &new_dir);
/*!
    ensures
...
class locally_change_current_dir : noncopyable {
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          to a new directory and then automatically switching back to the
  original directory upon this object's destruction.
  !*/
public:
  explicit locally_change_current_dir(const std::string &new_dir);
  /*!
      ensures
          - calls set_current_dir(new_dir)
          - #old_dir() == The value of get_current_dir() prior to switching to
  new_dir.
  !*/

  const std::string &old_dir() const;
  /*!
      ensures
          - returns the directory we switch back to once this object is
  destructed.
  !*/
...
/*!
    ...
    throws
        - dir_create_error
            ...
            directory and it didn't already exist.  The type member of the
exception will bet set to EDIR_CREATE and the name member will be set to dir.
!*/
