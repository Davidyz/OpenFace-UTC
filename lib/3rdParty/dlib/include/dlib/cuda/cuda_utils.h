...
#define CHECK_CUDA(call)                                                       \
  do {                                                                         \
    ...
    if (error != cudaSuccess) {                                                \
      ...
      sout << "Error while calling " << #call << " in file " << __FILE__       \
           << ":" << __LINE__ << ". ";                                         \
...
__inline__ __device__ size_t pack_idx(size_t dim_size3, size_t dim_size2,
                                      size_t dim_size1, size_t idx4,
                                      size_t idx3, size_t idx2, size_t idx1)
/*!
    ensures
        - Converts a 4D array index into a 1D index assuming row major layout.
To understand precisely what this function does, imagine we had an array
          declared like this:
            int ARRAY[anything][dim_size3][dim_size2][dim_size1];
          Then we could index it like this:
            ARRAY[idx4][idx3][idx2][idx1]
          or equivalently like this:
            ((int*)ARRAY)[pack_idx(dim_size3,dim_size2,dim_size1,
idx4,idx3,idx2,idx1)]
!*/
{
...
unpack_idx(size_t idx, size_t dim_size3, size_t dim_size2, size_t dim_size1,
           size_t &idx4, size_t &idx3, size_t &idx2, size_t &idx1)
/*!
    ensures
        - This function computes the inverse of pack_idx().  Therefore,
          if PACKED == pack_idx(dim_size3,dim_size2,dim_size1,
idx4,idx3,idx2,idx1) then unpack_idx(PACKED,dim_size3,dim_size2,dim_size1,
IDX4,IDX3,IDX2,IDX1) results in:
            - IDX1 == idx1
            - IDX2 == idx2
            - IDX3 == idx3
            - IDX4 == idx4
...
__inline__ __device__ void warp_reduce_atomic_add(float &out, float val)
/*!
    ensures
...
void launch_kernel(Kernel K, T... args)
/*!
    ensures
        - launches the given kernel K(args...).  The point of this function is
to automatically set the kernel launch parameters to something reasonable based
on the properties of the kernel and the current GPU card.
!*/
{
...
template <typename Kernel, typename... T>
void launch_kernel(Kernel K, max_jobs m, T... args)
/*!
    ensures
        - This function is just like launch_kernel(K,args...) except that you
can additionally supply a max_jobs number that tells it how many possible total
threads could be used.  This is useful when launching potentially small jobs
that might not need the number of threads suggested by launch_kernel().
!*/
{
  ...
  int num_blocks, num_threads;
  CHECK_CUDA(cudaOccupancyMaxPotentialBlockSize(&num_blocks, &num_threads, K));
  // Check if the job is really small and we don't really need to launch a
  // kernel with this many blocks and threads.
  ...
  if (m.num_y == 1) {
    ...
    */
    ...
    const int ratio = static_cast<int>(
        std::round(put_in_range(1, y_threads, m.num_x / (double)m.num_y)));
    x_threads *= ratio;
    y_threads /= ratio;
...
class grid_stride_range {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This is a tool for making a for loop that loops over an entire block
  of memory inside a kernel, but doing so in a way that parallelizes
...
public:
  __device__ grid_stride_range(size_t ibegin_, size_t iend_)
      : ibegin(ibegin_), iend(iend_) {}
  ...
  __device__ iterator end() const { return iterator(iend); }
  ...
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          Therefore, if you launch a cuda kernel with a statement like:
              dim3 blocks(1,10);
              dim3 threads(32,32);  // You need to have x and y not equal to 1
  to get parallelism over both loops.
              add_arrays<<<blocks,threads>>>(a,b,out,nr,nc);
          You can perform a nested 2D parallel for loop rather than doing just a
          1D for loop.

          So the code in the kernel would look like this if you wanted to add
  two 2D matrices:
              __global__ void add_arrays(
                  const float* a,
                  const float* b,
...
public:
  __device__ grid_stride_range_y(size_t ibegin_, size_t iend_)
      : ibegin(ibegin_), iend(iend_) {}
