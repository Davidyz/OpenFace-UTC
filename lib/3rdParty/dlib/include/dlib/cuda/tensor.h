...
class tensor {
public:
  tensor() : m_n(0), m_k(0), m_nr(0), m_nc(0), m_size(0) {}
  ...
  void async_copy_to_device() const { data().async_copy_to_device(); }
  ...
  tensor &operator=(float val) {
    ...
    // data then we do this initialization on the device side since this avoids
    // a host to device transfer that would likely immediately follow.
  ...
  template <typename EXP> tensor &operator=(const matrix_exp<EXP> &item) {
    ...
    static_assert(
        (is_same_type<float, typename EXP::type>::value == true),
...
#ifdef DLIB_USE_CUDA
  virtual const cuda::tensor_descriptor &
  get_cudnn_tensor_descriptor() const = 0;
#endif

  friend void memcpy(tensor &dest, const tensor &src) {
    DLIB_CASSERT(dest.size() == src.size());
    memcpy(dest.data(), dest.get_alias_offset(), src.data(),
           src.get_alias_offset(), src.size());
  }
...
protected:
  ...
  virtual size_t get_alias_offset() const {
    return 0;
...
inline bool is_vector(const tensor &t) {
  return t.size() == (size_t)t.num_samples() || t.size() == (size_t)t.k() ||
         t.size() == (size_t)t.nr() || t.size() == (size_t)t.nc();
}

// ----------------------------------------------------------------------------------------

inline const matrix_op<op_pointer_to_mat<float>>
mat(const tensor &t, long long nr, long long nc) {
  DLIB_ASSERT(nr >= 0 && nc >= 0, "\tconst matrix_exp mat(tensor, nr, nc)"
                                      << "\n\t nr and nc must be >= 0"
                                      << "\n\t nr: " << nr
                                      << "\n\t nc: " << nc);
...
inline const matrix_op<op_pointer_to_mat<float>> mat(const tensor &t) {
  if (t.size() != 0)
...
inline const matrix_op<op_pointer_to_mat<float>>
image_plane(const tensor &t, long long sample = 0, long long k = 0) {
  DLIB_ASSERT(0 <= sample && sample < t.num_samples() && 0 <= k && k < t.k() &&
                  t.size() != 0,
  ...
  typedef op_pointer_to_mat<float> op;
  return matrix_op<op>(
      op(t.host() + ((sample * t.k() + k) * t.nr()) * t.nc(), t.nr(), t.nc()));
}

// ----------------------------------------------------------------------------------------

inline bool have_same_dimensions(const tensor &a, const tensor &b) {
  return a.num_samples() == b.num_samples() && a.k() == b.k() &&
         a.nr() == b.nr() && a.nc() == b.nc();
}
...
public:
  ...
  virtual float *device_write_only() {
    return data_instance.device_write_only();
  }
...
#ifdef DLIB_USE_CUDA
  virtual const cuda::tensor_descriptor &get_cudnn_tensor_descriptor() const {
    return cudnn_descriptor;
  }
#endif
...
inline void serialize(const tensor &item, std::ostream &out) {
  ...
  for (auto d : item) {
    ...
    // that any CUDA enabled platform isn't going to use IEEE floats.  But if
    // one does we can just update the serialization code here to handle it if
    // such a platform is encountered.
    bo.host_to_little(d);
    static_assert(
        sizeof(d) == 4,
        "This serialization code assumes we are writing 4 byte floats");
...
inline void deserialize(resizable_tensor &item, std::istream &in) {
  ...
  for (auto &d : item) {
    ...
    if (sbuf->sgetn((char *)&d, sizeof(d)) != sizeof(d)) {
      ...
      throw serialization_error(
          "Error reading data while deserializing dlib::resizable_tensor.");
    }
    bo.little_to_host(d);
  }
}
...
class alias_tensor_instance : public tensor {
  alias_tensor_instance() : data_instance(0), _annotation(0), data_offset(0) {}
...
public:
  ...
  virtual const float *host() const {
    return data_instance->host() + data_offset;
  }
  virtual float *host() { return data_instance->host() + data_offset; }
  virtual float *host_write_only() {
    return data_instance->host() + data_offset;
  }
  virtual const float *device() const {
    return data_instance->device() + data_offset;
  }
...
public:
  ...
  long long num_samples() const { return inst.m_n; }

  long long k() const { return inst.m_k; }

  long long nr() const { return inst.m_nr; }

  long long nc() const { return inst.m_nc; }

  size_t size() const { return inst.m_size; }
  ...
  alias_tensor_instance operator()(tensor &t, size_t offset = 0) const {
    ...
    // Note that t might already be an aliasing tensor so we need to take that
    // into account.
