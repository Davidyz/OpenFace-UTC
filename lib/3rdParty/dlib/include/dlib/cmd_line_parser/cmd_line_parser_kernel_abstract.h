...
class cmd_line_parser : public enumerable<cmd_line_parser_option<charT>> {
  ...
  guarantee.
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          command_line     ::= <program_name> { <options> | <arg> } [ --
  {<word>} ] program_name     ::= <word> arg              ::= any <word> that
  does not start with - option_arg       ::= <sword> option_name      ::= <char>
          long_option_name ::= <char> {<char> | - }
          options          ::= <bword> - <option_name> {<option_name>}
  {<option_arg>}  | <bword> -- <long_option_name> [=<option_arg>] {<bword>
  <option_arg>} char             ::= any character other than - or = word ::=
  any string from argv where argv is the second parameter to main() sword ::=
  any suffix of a string from argv where argv is the second parameter to main()
          bword            ::= This is an empty string which denotes the
  begining of a <word>.
          ...
          Options with arguments:
              ...
              arg1 and arg2 are associated with the option o and
  general_argument is not.
          ...
          Arguments not associated with an option:
              ...
              Example 2:
                  command line: program general_arg1 -- -o arg1 arg2
  general_arg2 Then the following is true (the -- causes everything following it
  to be treated as a general argument).
...
public:
  ...
  /*!
      requires
          - argv == an array of strings that was obtained from the second
  argument of the function main(). (i.e. argv[0] should be the <program> token,
  argv[1] should be an <options> or <arg> token, etc.)
      ...
      throws
          ...
          - cmd_line_parse_error
              This exception is thrown if there is an error parsing the command
  line. If this exception is thrown then #parsed_line() == false and all options
  will have their count() set to 0 but otherwise there will be no effect (i.e.
  all registered options will remain registered).
  !*/

  void parse(int argc, charT **argv);
  /*!
      This just calls this->parse(argc,argv) and performs the necessary
  const_cast on argv.
  !*/

  bool parsed_line() const;
  /*!
      ensures
  ...
  bool option_is_defined(const string_type &name) const;
  /*!
      ensures
  ...
  void add_option(const string_type &name, const string_type &description,
  ...
  const option_type &option(const string_type &name) const;
  /*!
      requires
          - option_is_defined(name) == true
      ensures
  ...
  unsigned long number_of_arguments() const;
  /*!
      requires
          - parsed_line() == true
      ensures
  ...
  const string_type &operator[](unsigned long N) const;
  /*!
      requires
          - parsed_line() == true
  ...
  void swap(cmd_line_parser &item);
  /*!
      ensures
          - swaps *this and item
  !*/

  void print_options(std::basic_ostream<char_type> &out) const;
  /*!
      ensures
  ...
  string_type get_group_name() const;
  /*!
      ensures
          - returns the current group name.  This is the group new options will
  be added into when added via add_option().
          - The group name of an option is used by print_options().  In
  particular, it groups all options with the same group name together and
  displays them under a title containing the text of the group name.  This
  allows you to group similar options together in the output of print_options().
          - A group name of "" (i.e. the empty string) means that no group name
  is set.
  !*/

  void set_group_name(const string_type &group_name);
  /*!
      ensures
  ...
  class cmd_line_check_error : public dlib::error {
    /*!
        ...
        command line error.  The interpretation of the member variables is
    defined below in each check_*() routine.
    !*/
  ...
  template <typename T>
  void check_option_arg_type(const string_type &option_name) const;
  /*!
      requires
          - parsed_line() == true
      ...
      throws
          ...
          - cmd_line_check_error
              This exception is thrown if the ensures clause could not be
  satisfied. The exception's members will be set as follows:
  ...
  template <typename T>
  void check_option_arg_range(const string_type &option_name, const T &first,
                              const T &last) const;
  /*!
      requires
          - parsed_line() == true
      ...
      ensures
          - all the arguments for the given option are convertible
            by string_cast<T>() to an object of type T and the resulting value
  is in the range first to last inclusive. throws
          - std::bad_alloc
          - cmd_line_check_error
              This exception is thrown if the ensures clause could not be
  satisfied. The exception's members will be set as follows:
  ...
  void check_option_arg_range(const string_type &option_name,
                              const T (&arg_set)[length]) const;
  /*!
      requires
          - parsed_line() == true
          - option_is_defined(option_name) == true
  ...
  void check_option_arg_range(const string_type &option_name,
                              const char_type *(&arg_set)[length]) const;
  /*!
      requires
          - parsed_line() == true
          - option_is_defined(option_name) == true
      ensures
          - for each argument to the given option:
              - there is a string in the arg_set array that is equal to this
  argument. throws
          - std::bad_alloc
          - cmd_line_check_error
              This exception is thrown if the ensures clause could not be
  ...
  template <size_t length>
  void check_one_time_options(const char_type *(&option_set)[length]) const;
  /*!
      requires
          - parsed_line() == true
          - for all valid i:
  ...
  satisfied. The exception's members will be set as follows:
                  ...
                  - opt == the option that occurred more than once on the
  command line.
  !*/

  void check_incompatible_options(const string_type &option_name1,
                                  const string_type &option_name2) const;
  /*!
      requires
          - parsed_line() == true
      ...
      ensures
          - option(option_name1).count() == 0 || option(option_name2).count() ==
  0 (i.e. at most, only one of the options is currently present) throws
          - std::bad_alloc
          - cmd_line_check_error
              This exception is thrown if the ensures clause could not be
  satisfied. The exception's members will be set as follows:
  ...
  template <size_t length>
  void check_incompatible_options(const char_type *(&option_set)[length]) const;
  /*!
      requires
          - parsed_line() == true
          - for all valid i:
              - option_is_defined(option_set[i]) == true
      ensures
          - At most only one of the options in the array option_set has a
  count() greater than 0.  (i.e. at most, only one of the options is currently
  present) throws
          - std::bad_alloc
          - cmd_line_check_error
              This exception is thrown if the ensures clause could not be
  ...
  void check_sub_option(const string_type &parent_option,
                        const string_type &sub_option) const;
  /*!
      requires
          - parsed_line() == true
  ...
  template <size_t length>
  void check_sub_options(const char_type *(&parent_option_set)[length],
                         const string_type &sub_option) const;
  /*!
      requires
          - parsed_line() == true
      ...
      ensures
          - if (option(sub_option).count() > 0) then
              - At least one of the options in the array parent_option_set has a
  count() greater than 0. (i.e. at least one of the options in parent_option_set
  ...
  satisfied. The exception's members will be set as follows:
                  ...
                  - required_opts == a vector containing everything from
  parent_option_set.
  !*/

  template <size_t length>
  void check_sub_options(const string_type &parent_option,
                         const char_type *(&sub_option_set)[length]) const;
  /*!
      requires
          - parsed_line() == true
          - option_is_defined(parent_option) == true
  ...
  satisfied. The exception's members will be set as follows:
                  - type == EMISSING_REQUIRED_OPTION
                  - opt == the first option from the sub_option_set that is
  present.
                  - required_opts == a vector that contains only parent_option.
  !*/

  template <size_t parent_length, size_t sub_length>
  void check_sub_options(const char_type *(&parent_option_set)[parent_length],
                         const char_type *(&sub_option_set)[sub_length]) const;
  /*!
      requires
          - parsed_line() == true
          - for all valid i:
      ...
      ensures
          - for all valid j:
              - if (option(sub_option_set[j]).count() > 0) then
                  - At least one of the options in the array parent_option_set
  has a count() greater than 0. (i.e. at least one of the options in
  parent_option_set is currently present) throws
          - std::bad_alloc
          - cmd_line_check_error
              This exception is thrown if the ensures clause could not be
  satisfied. The exception's members will be set as follows:
...
template <typename charT>
inline void swap(cmd_line_parser<charT> &a, cmd_line_parser<charT> &b) {
  a.swap(b);
}
/*!
