...
public:
  option_parse_error(const std::string &option_string, const std::string &str)
      : error(EOPTION_PARSE, "Error parsing argument for option '" +
                                 option_string + "', offending string is '" +
                                 str + "'.") {}
};
...
T impl_config_reader_get_option(const config_reader_type &cr,
  ...
  if (pos == std::string::npos) {
    if (cr.is_key_defined(option_name)) {
      try {
        return string_cast<T>(cr[option_name]);
      } catch (string_cast_error &) {
        throw option_parse_error(full_option_name, cr[option_name]);
      }
    }
  } else {
    ...
    if (cr.is_block_defined(block_name)) {
      return impl_config_reader_get_option(cr.block(block_name),
                                           ...
                                           full_option_name, default_value);
    }
  }
...
typename enable_if<is_config_reader<cr_type>, T>::type
get_option(const cr_type &cr, const std::string &option_name, T default_value) {
  return impl_config_reader_get_option(cr, option_name, option_name,
                                       default_value);
}
...
typename disable_if<is_config_reader<parser_type>, T>::type
get_option(const parser_type &parser, const std::string &option_name,
           T default_value) {
  ...
  if (parser.option(option_name)) {
    try {
      ...
      throw option_parse_error(option_name,
                               parser.option(option_name).argument());
    }
  }
  return default_value;
}
...
typename disable_if<is_config_reader<parser_type>, T>::type
get_option(const parser_type &parser, const cr_type &cr,
           const std::string &option_name, T default_value) {
  // make sure requires clause is not broken
  DLIB_ASSERT(parser.option_is_defined(option_name) == true &&
                  parser.option(option_name).number_of_arguments() == 1,
              "\t T get_option()"
...
template <typename parser_type, typename cr_type, typename T>
typename disable_if<is_config_reader<parser_type>, T>::type
get_option(const cr_type &cr, const parser_type &parser,
           const std::string &option_name, T default_value) {
  // make sure requires clause is not broken
  DLIB_ASSERT(parser.option_is_defined(option_name) == true &&
                  parser.option(option_name).number_of_arguments() == 1,
...
template <typename T>
inline std::string get_option(const T &cr, const std::string &option_name,
...
inline std::string get_option(const T &parser, const U &cr,
                              const std::string &option_name,
                              const char *default_value) {
