...
template <typename clp_base> class cmd_line_parser_check_1 : public clp_base {
...
public:
  ...
  class cmd_line_check_error : public dlib::error {
    ...
    cmd_line_check_error(error_type t, const string_type &opt_,
                         const string_type &arg_)
        : dlib::error(t), opt(opt_), opt2(), arg(arg_), required_opts() {
      set_info_string();
    }

    cmd_line_check_error(error_type t, const string_type &opt_,
                         const string_type &opt2_,
                         int // this is just to make this constructor different
                             // from the one above
                         )
        : dlib::error(t), opt(opt_), opt2(opt2_), arg(), required_opts() {
      set_info_string();
    }

    cmd_line_check_error(error_type t, const string_type &opt_,
                         const std::vector<string_type> &vect)
        : dlib::error(t), opt(opt_), opt2(), arg(), required_opts(vect) {
      set_info_string();
    }

    cmd_line_check_error(error_type t, const string_type &opt_)
        : dlib::error(t), opt(opt_), opt2(), arg(), required_opts() {
      set_info_string();
    }
    ...
    void set_info_string() {
      ...
      case EINVALID_OPTION_ARG:
        sout << "Command line error: '" << narrow(arg)
             << "' is not a valid argument to "
             << "the '" << narrow(opt) << "' option.";
        break;
      case EMISSING_REQUIRED_OPTION:
        if (required_opts.size() == 1) {
          sout << "Command line error: The '" << narrow(opt)
               << "' option requires the presence of "
      ...
      case EINCOMPATIBLE_OPTIONS:
        sout << "Command line error: The '" << narrow(opt) << "' and '"
             << narrow(opt2)
...
inline void swap(cmd_line_parser_check_1<clp_base> &a,
...
template <typename clp_base>
template <typename T>
void cmd_line_parser_check_1<clp_base>::check_option_arg_type(
...
template <typename clp_base>
template <typename T>
void cmd_line_parser_check_1<clp_base>::check_option_arg_range(
    ...
    for (unsigned long i = 0; i < number_of_arguments; ++i) {
      for (unsigned long j = 0; j < count; ++j) {
        ...
        if (temp < first || last < temp) {
          throw cmd_line_check_error(EINVALID_OPTION_ARG, option_name,
                                     opt.argument(i, j));
        }
      }
    }
...
template <typename clp_base>
template <size_t length>
void cmd_line_parser_check_1<clp_base>::check_incompatible_options(
    ...
    for (size_t j = i + 1; j < length; ++j) {
      if (this->option(option_set[i]).count() > 0 &&
        ...
        throw cmd_line_check_error(
            EINCOMPATIBLE_OPTIONS, option_set[i], option_set[j],
...
void cmd_line_parser_check_1<clp_base>::check_incompatible_options(
  ...
  if (this->option(option_name1).count() > 0 &&
    ...
    throw cmd_line_check_error(
        EINCOMPATIBLE_OPTIONS, option_name1, option_name2,
        0 // this argument has no meaning and is only here to make this
          // call different from the other constructor
    );
  }
...
template <typename clp_base>
void cmd_line_parser_check_1<clp_base>::check_sub_option(
...
template <typename clp_base>
template <size_t length>
void cmd_line_parser_check_1<clp_base>::check_sub_options(
  ...
  if (this->option(parent_option).count() == 0) {
    ...
    if (i != length) {
      ...
      vect.resize(1);
      vect[0] = parent_option;
      throw cmd_line_check_error(EMISSING_REQUIRED_OPTION, sub_option_set[i],
                                 vect);
    }
  }
}
...
void cmd_line_parser_check_1<clp_base>::check_sub_options(
  ...
  if (this->option(sub_option).count() > 0) {
    ...
    if (!parents_present) {
      std::vector<string_type> vect(parent_option_set,
                                    parent_option_set + length);
      throw cmd_line_check_error(EMISSING_REQUIRED_OPTION, sub_option, vect);
    }
  }
}
...
void cmd_line_parser_check_1<clp_base>::check_sub_options(
  ...
  if (!parents_present) {
    ...
    if (i != sub_length) {
      std::vector<string_type> vect(parent_option_set,
                                    parent_option_set + parent_length);
      throw cmd_line_check_error(EMISSING_REQUIRED_OPTION, sub_option_set[i],
                                 vect);
    }
  }
...
template <typename clp_base>
template <size_t length>
void cmd_line_parser_check_1<clp_base>::check_one_time_options(
    const char_type *(&option_set)[length]) const {
  size_t i = 0;
  for (; i < length; ++i) {
  ...
  if (i != length) {
    throw cmd_line_check_error(EMULTIPLE_OCCURANCES, option_set[i]);
  }
}
