...
template <typename clp_base> class cmd_line_parser_print_1 : public clp_base {

public:
  void
  print_options(std::basic_ostream<typename clp_base::char_type> &out) const;

  void print_options() const { print_options(std::cout); }
};

template <typename clp_base>
inline void swap(cmd_line_parser_print_1<clp_base> &a,
...
template <typename clp_base>
void cmd_line_parser_print_1<clp_base>::print_options(
    std::basic_ostream<typename clp_base::char_type> &out) const {
  ...
  try {
    ...
    while (this->move_next()) {
      ...
      if (this->element().number_of_arguments() == 1) {
        ...
        for (unsigned long i = 0; i < this->element().number_of_arguments();
             ++i) {
          len += 7;
          if (i + 1 > 9)
            ++len;
    ...
    // Make a separate ostringstream for each option group.  We are going to
    // write the output for each group to a separate ostringstream so that we
    // can keep them grouped together in the final output.
    ...
    // Only print out a generic Options: group name if there is an unnamed
    // option present.
