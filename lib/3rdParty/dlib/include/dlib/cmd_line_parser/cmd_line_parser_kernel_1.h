...
template <typename charT, typename map, typename sequence, typename sequence2>
class cmd_line_parser_kernel_1
    : public enumerable<cmd_line_parser_option<charT>> {
  /*!
      REQUIREMENTS ON map
...
public:
  ...
  class cmd_line_parse_error : public dlib::error {
    void set_info_string() {
      ...
      case EINVALID_OPTION:
        sout << "Command line error: '" << narrow(item)
             << "' is not a valid option.";
        break;
      case ETOO_FEW_ARGS:
        if (num > 1) {
          sout << "Command line error: The '" << narrow(item)
               << "' option requires " << num << " arguments.";
        } else {
          sout << "Command line error: The '" << narrow(item)
               << "' option requires " << num << " argument.";
        }
        break;
      case ETOO_MANY_ARGS:
        sout << "Command line error: The '" << narrow(item)
  ...
  public:
    cmd_line_parse_error(error_type t, const std::basic_string<charT> &_item)
        : dlib::error(t), item(_item), num(0) {
      set_info_string();
    }

    cmd_line_parse_error(error_type t, const std::basic_string<charT> &_item,
                         unsigned long _num)
        : dlib::error(t), item(_item), num(_num) {
      set_info_string();
    }

    cmd_line_parse_error() : dlib::error(), item(), num(0) {
      set_info_string();
    }
...
private:
  ...
  public:
    const std::basic_string<charT> &name() const { return name_; }

    const std::basic_string<charT> &group_name() const { return group_name_; }

    const std::basic_string<charT> &description() const { return description_; }

    unsigned long number_of_arguments() const { return number_of_arguments_; }

    unsigned long count() const { return num_present; }

    const std::basic_string<charT> &argument(unsigned long arg,
                                             unsigned long N) const {
      // make sure requires clause is not broken
      DLIB_CASSERT(
          N < count() && arg < number_of_arguments(),
          "\tconst string_type& cmd_line_parser_option::argument(unsigned "
          "long,unsigned long)"
...
public:
  ...
  string_type get_group_name() const { return group_name; }

  const cmd_line_parser_option<charT> &option(const string_type &name) const;

  unsigned long number_of_arguments() const;

  const string_type &operator[](unsigned long index) const;

  void swap(cmd_line_parser_kernel_1 &item);

  // functions from the enumerable interface
  bool at_start() const { return options.at_start(); }

  void reset() const { options.reset(); }

  bool current_element_valid() const { return options.current_element_valid(); }

  const cmd_line_parser_option<charT> &element() const {
    return *static_cast<cmd_line_parser_option<charT> *>(
  ...
  bool move_next() const { return options.move_next(); }
...
private:
  ...
  cmd_line_parser_kernel_1 &
  operator=(cmd_line_parser_kernel_1 &); // assignment operator
};

// ----------------------------------------------------------------------------------------

template <typename charT, typename map, typename sequence, typename sequence2>
inline void swap(cmd_line_parser_kernel_1<charT, map, sequence, sequence2> &a,
...
template <typename charT, typename map, typename sequence, typename sequence2>
cmd_line_parser_kernel_1<charT, map, sequence,
                         sequence2>::cmd_line_parser_kernel_1()
    : have_parsed_line(false) {}
...
template <typename charT, typename map, typename sequence, typename sequence2>
void cmd_line_parser_kernel_1<charT, map, sequence, sequence2>::clear() {
  have_parsed_line = false;
  argv.clear();
...
template <typename charT, typename map, typename sequence, typename sequence2>
void cmd_line_parser_kernel_1<charT, map, sequence, sequence2>::parse(
    int argc_, const charT **argv) {
  using namespace std;
      ...
      if (argv[i][0] == _dT(charT, '-') && !escape) {
        ...
        if (argv[i][1] == _dT(charT, '-')) {
          ...
          typename string_type::size_type pos =
              temp.find_first_of(_dT(charT, '='));
          // This variable will be 1 if there is an argument supplied via the =
          // sign and 0 otherwise.
          ...
          if (argc + extra_argument <= o->number_of_arguments() + i) {
            ...
            throw cmd_line_parse_error(ETOO_FEW_ARGS, temp,
                                       o->number_of_arguments());
          }
        ...
        // --------------------------------------------------------------------
          ...
          for (unsigned long k = 0; k < num; ++k) {
            string_type name;
            // Doing this instead of name = temp[k] seems to avoid a bug in g++
            // (Ubuntu/Linaro 4.5.2-8ubuntu4) 4.5.2 which results in name[0]
            ...
            if (argc + delta <= o->number_of_arguments() + i) {
              // there are too few arguments
              std::ostringstream sout;
              throw cmd_line_parse_error(ETOO_FEW_ARGS, name,
                                         o->number_of_arguments());
            }
            ...
            if (o->number_of_arguments() > 0) {
              ...
              if (delta == 1) {
                temp = &argv[i][2 + k];
                k = (unsigned long)num; // this ensures that the argument to
                                        // this option isn't going to be treated
                                        // as a list of options

                stemp[0] = temp;
              }
              for (unsigned long j = 0; j < o->number_of_arguments() - delta;
                   ++j) {
...
template <typename charT, typename map, typename sequence, typename sequence2>
bool cmd_line_parser_kernel_1<charT, map, sequence, sequence2>::parsed_line()
    const {
...
bool cmd_line_parser_kernel_1<charT, map, sequence, sequence2>::
    option_is_defined(const string_type &name) const {
...
template <typename charT, typename map, typename sequence, typename sequence2>
void cmd_line_parser_kernel_1<charT, map, sequence, sequence2>::set_group_name(
...
template <typename charT, typename map, typename sequence, typename sequence2>
void cmd_line_parser_kernel_1<charT, map, sequence, sequence2>::add_option(
    const string_type &name, const string_type &description,
...
template <typename charT, typename map, typename sequence, typename sequence2>
const cmd_line_parser_option<charT> &
cmd_line_parser_kernel_1<charT, map, sequence, sequence2>::option(
...
template <typename charT, typename map, typename sequence, typename sequence2>
unsigned long
cmd_line_parser_kernel_1<charT, map, sequence, sequence2>::number_of_arguments()
    const {
  return argv.size();
}
...
cmd_line_parser_kernel_1<charT, map, sequence, sequence2>::operator[](
...
template <typename charT, typename map, typename sequence, typename sequence2>
void cmd_line_parser_kernel_1<charT, map, sequence, sequence2>::swap(
