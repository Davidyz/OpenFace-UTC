...
public:
  ...
  explicit circular_buffer(unsigned long s) { resize(s); }
  ...
  T &operator[](unsigned long i) {
    DLIB_ASSERT(i < size(), "\t T& circular_buffer::operator[](i)"
  ...
  const T &operator[](unsigned long i) const {
    DLIB_ASSERT(i < size(), "\t const T& circular_buffer::operator[](i)"
                                << "\n\t You have supplied an invalid index"
                                << "\n\t this:   " << this << "\n\t i:      "
                                << i << "\n\t size(): " << size());
    return data[(i + offset) % data.size()];
  ...
  void assign(unsigned long size, const T &value) {
    offset = 0;
...
template <typename T> void swap(circular_buffer<T> &a, circular_buffer<T> &b) {
  a.swap(b);
}
...
template <typename T>
void serialize(const circular_buffer<T> &item, std::ostream &out) {
  try {
...
template <typename T>
void deserialize(circular_buffer<T> &item, std::istream &in) {
  try {
    unsigned long size;
    deserialize(size, in);
    item.resize(size);
...
template <typename T>
const matrix_op<op_array_to_mat<circular_buffer<T>>>
mat(const circular_buffer<T> &m) {
