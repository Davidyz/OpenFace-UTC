...
template <typename sb_base> class sliding_buffer_kernel_c : public sb_base {
...
template <typename sb_base>
inline void swap(sliding_buffer_kernel_c<sb_base> &a,
...
template <typename sb_base>
void sliding_buffer_kernel_c<sb_base>::set_size(unsigned long exp_size) {
...
template <typename sb_base>
unsigned long
sliding_buffer_kernel_c<sb_base>::get_element_id(unsigned long index) const {
  // make sure requires clause is not broken
  DLIB_CASSERT(
      index < this->size(),
...
template <typename sb_base>
unsigned long sliding_buffer_kernel_c<sb_base>::get_element_index(
    unsigned long element_id) const {
  // make sure requires clause is not broken
  DLIB_CASSERT(
      element_id < this->size(),
...
template <typename sb_base>
const typename sb_base::type &
sliding_buffer_kernel_c<sb_base>::operator[](unsigned long index) const {
  // make sure requires clause is not broken
...
template <typename sb_base>
typename sb_base::type &
sliding_buffer_kernel_c<sb_base>::operator[](unsigned long index) {
  // make sure requires clause is not broken
  DLIB_CASSERT(index < this->size(),
...
template <typename sb_base>
const typename sb_base::type &
sliding_buffer_kernel_c<sb_base>::element() const {
  // make sure requires clause is not broken
  DLIB_CASSERT(
...
template <typename sb_base>
typename sb_base::type &sliding_buffer_kernel_c<sb_base>::element() {
  // make sure requires clause is not broken
  DLIB_CASSERT(
      this->current_element_valid() == true,
