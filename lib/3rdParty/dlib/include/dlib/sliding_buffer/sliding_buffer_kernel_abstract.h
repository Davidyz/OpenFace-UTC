...
template <typename T> class sliding_buffer : public enumerable<T> {
  /*!
      REQUIREMENTS ON T
      ...
      ENUMERATION ORDER
          The enumerator will iterate over the elements of the sliding_buffer in
  the order (*this)[0], (*this)[1], (*this)[2], ...
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          if instead of calling rotate_left we call rotate_right(3) instead we
  would have "ta!some da" 9876543210
...
public:
  ...
  /*!
      ...
      ensures
          - returns the index of the element with the given element_id.
            ( (*this)[get_element_index(element_id)] will always refer to the
  same element no matter where it has been rotated to)
          - returned value is < size()
  !*/
  ...
  void swap(sliding_buffer<T> &item);
  /*!
      ensures
          - swaps *this and item
  !*/
...
template <typename T> void swap(sliding_buffer<T> &a, sliding_buffer<T> &b) {
  a.swap(b);
}
/*!
...
template <typename T>
void deserialize(sliding_buffer<T> &item, std::istream &in);
/*!
