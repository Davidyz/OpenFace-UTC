...
template <typename decoder> class entropy_decoder_kernel_c : public decoder {
...
std::istream &entropy_decoder_kernel_c<decoder>::get_stream() const {
...
void entropy_decoder_kernel_c<decoder>::decode(uint32 low_count,
  ...
  DLIB_CASSERT((low_count <= _get_target) && (_get_target < high_count) &&
               ...
               "\tvoid entropy_decoder::decode()"
                   ...
                   << "\n\tget_target_called(): "
                   << (this->get_target_called() ? "true" : "false"));
...
template <typename decoder>
uint32 entropy_decoder_kernel_c<decoder>::get_target(uint32 total) {
  // make sure requires clause is not broken
  DLIB_CASSERT(
      (total > 0) && (total < 65536) && (this->stream_is_set() == true),
