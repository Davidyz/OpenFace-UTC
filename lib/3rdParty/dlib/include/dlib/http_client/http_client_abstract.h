...
typedef bool (*fnOnDownload)(long already_downloaded, long total_to_download,
                             void *userInfo);
...
class Browser {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This object represents a possibility for the end user to download
  webpages (HTTP/1.0) from the internet like a normal webbrowser would do.
  !*/
...
public:
  ...
  void set_user_agent(const std::string &new_agent) {
  ...
  /*!
      Clear out any header and/or cookie which would obstruct getting a next
  page. At this moment this is cleared:
          - the Content-Type header
  !*/

  void set_callback_function(fnOnDownload od, void *_user_info);
  /*!
      Set a callback function for one of the following events:
      - OnDownload: this will tell you how much is downloaded and how much will
  need to be downloaded
  !*/

  void set_cookie(const std::string &cookie_name,
  ...
  /*!
      Set the maximum time how long a request can take. Setting this to 0
  disables this behavior.
  !*/

  string_to_stringvector get_returned_headers() const;
  /*!
      Returns all the headers which are returned in the download of the webpage.
  !*/

  short get_http_return() const;
  /*!
  ...
  const std::string &
  post_url(const std::string &url, const string_to_stringmap &postvars,
  ...
  const std::string &post_url(const std::string &url,
