...
class timeout {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This object provides a simple way to implement a timeout.  An example
  will make its use clear.  Suppose we want to read from a socket but we want to
          terminate the connection if the read takes longer than 10 seconds.
  ...
  the
              // connection.  If read completes in less than 10 seconds then the
  t
              // object will be destructed on the next line due to the } and
  then the
          ...
          example, the following statement will print "Hello world!" after
  1000ms: timeout t([](){ cout << "Hello world!" << endl; },  1000);
...
public:
  template <typename T>
  timeout(T callback_function, unsigned long ms_to_timeout);
  /*!
      requires
      ...
      ensures
          ...
          - if (this object isn't destructed in ms_to_timeout milliseconds) then
              - callback_function() will be called in ms_to_timeout
  milliseconds. throws
          - std::bad_alloc
          - dlib::thread_error
  !*/
  ...
  template <typename T>
  timeout(T &object, void (T::*callback_function)(),
  ...
  timeout(T &object, void (T::*callback_function)(U callback_function_argument),
          unsigned long ms_to_timeout, U callback_function_argument);
  /*!
      requires
          - callback_function does not throw
      ensures
  ...
  template <typename T>
  timeout(T &object, int (T::*callback_function)(),
          unsigned long ms_to_timeout);
  /*!
      requires
          - callback_function does not throw
  ...
  template <typename T, typename U>
  timeout(T &object, int (T::*callback_function)(U callback_function_argument),
          unsigned long ms_to_timeout, U callback_function_argument);
  /*!
      requires
          - callback_function does not throw
  ...
  /*!
      ...
      ensures
          ...
          - if (the callback_function hasn't been called yet) then
              - the callback_function specified in the constructor will not be
  called
  !*/
