...
class timeout {
  ...
  template <typename T> class functor : public bind {
  public:
  ...
  template <typename T, typename R> class zero : public bind {
  public:
    T *object;
  ...
  template <typename T, typename R, typename U> class one : public bind {
  public:
    T *object;
...
public:
  // This typedef is here for backwards compatibility with previous versions of
  // dlib.
  ...
  template <typename T>
  timeout(T &object, int (T::*callback_function)(), unsigned long ms_to_timeout)
      : t(*this, &timeout::trigger_timeout) {
