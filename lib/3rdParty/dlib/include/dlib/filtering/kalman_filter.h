...
public:
  ...
  void set_observation_model(const matrix<double, measurements, states> &H_) {
    H = H_;
  }
  void set_transition_model(const matrix<double, states, states> &A_) {
    A = A_;
  }
  void set_process_noise(const matrix<double, states, states> &Q_) { Q = Q_; }
  void
  set_measurement_noise(const matrix<double, measurements, measurements> &R_) {
    R = R_;
  }
  void
  set_estimation_error_covariance(const matrix<double, states, states> &P_) {
    P = P_;
  }
  ...
  const matrix<double, states, states> &get_process_noise() const { return Q; }
  ...
  void update(const matrix<double, measurements, 1> &z) {
    ...
    const matrix<double, states, measurements> K =
        P * trans(H) * pinv(H * P * trans(H) + R);
    ...
    if (got_first_meas) {
      ...
      // Since we don't have a previous state estimate at the start of
      // filtering, we will just set the current state to whatever is indicated
      // by the measurement
      x = pinv(H) * z;
      got_first_meas = true;
    }
  ...
  const matrix<double, states, 1> &get_current_state() const { return x; }
  ...
  friend inline void deserialize(kalman_filter &item, std::istream &in) {
    ...
    if (version != 1)
      throw dlib::serialization_error("Unknown version number found while "
                                      "deserializing kalman_filter object.");
...
public:
  momentum_filter(double meas_noise, double acc, double max_meas_dev)
      : measurement_noise(meas_noise), typical_acceleration(acc),
    ...
    kal.set_transition_model({1, 1, 0, 1});
    kal.set_process_noise(
        {0, 0, 0, typical_acceleration * typical_acceleration});
  ...
  double get_measurement_noise() const { return measurement_noise; }

  double get_typical_acceleration() const { return typical_acceleration; }

  double get_max_measurement_deviation() const {
    return max_measurement_deviation;
  }

  void reset() {
    *this = momentum_filter(measurement_noise, typical_acceleration,
  ...
  double operator()(const double measured_position) {
    ...
    // Check if measured_position has suddenly jumped in value by a whole lot.
    // This could happen if the velocity term experiences a much larger than
    // normal acceleration, e.g.  because the underlying object is doing a
    // maneuver.  If this happens then we clamp the state so that the predicted
    // next value is no more than max_deviation away from measured_position at
    // all times.
...
private:
  ...
  double max_measurement_deviation =
      3; // nominally number of standard deviations
...
public:
  ...
  rect_filter(const momentum_filter &filt)
      : left(filt), top(filt), right(filt), bottom(filt) {}

  drectangle operator()(const drectangle &r) {
    return drectangle(left(r.left()), top(r.top()), right(r.right()),
                      bottom(r.bottom()));
  }
  ...
  friend void deserialize(rect_filter &item, std::istream &in) {
    ...
    if (version != 123)
      throw dlib::serialization_error("Unknown version number found while "
                                      "deserializing rect_filter object.");
