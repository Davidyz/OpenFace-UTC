...
class rls_filter {
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          To accomplish this, this object maintains a fixed size buffer of
  recent points.  Each prediction is a linear combination of the points in this
          ...
          a point, recursive least squares updates the linear combination
  weights, and then it inserts the point into the history buffer.  After that,
  the next prediction is based on these updated weights and the current history
          buffer.
  !*/
...
public:
  ...
  explicit rls_filter(unsigned long size, double forget_factor = 0.8,
                      double C = 100);
  /*!
      requires
          - 0 < forget_factor <= 1
  ...
  determines the trade-off between trying to fit the data points given to
  update() or allowing more errors but hopefully improving the generalization of
  the predictions.  Larger values encourage exact fitting while smaller values
  ...
  /*!
      ensures
          - This object uses exponential forgetting in its implementation of
  recursive least squares.  Therefore, this function returns the "forget
  factor".
          - if (get_forget_factor() == 1) then
              - In this case, exponential forgetting is disabled.
              - The recursive least squares algorithm will implicitly take all
  previous calls to update(z) into account when estimating the optimal weights
  for linearly combining the history buffer into a prediction of the next point.
          - else
              - Old calls to update(z) are eventually forgotten.  That is, the
  smaller the forget factor, the less recursive least squares will care about
                attempting to find linear combination weights which would have
  make good predictions on old points.  It will care more about fitting recent
                points.  This is appropriate if the statistical properties of
  the time series we are modeling are not constant.
  !*/

  unsigned long get_window_size() const;
  /*!
      ensures
          - returns the size of the history buffer.  This is the number of
  points which are linearly combine to make the predictions returned by
  ...
  template <typename EXP> void update(const matrix_exp<EXP> &z);
  /*!
      requires
          ...
          - if (get_predicted_next_state().size() != 0) then
              - z.size() == get_predicted_next_state().size()
                ...
                to this function)
      ensures
          - Updates the state of this filter based on the current measurement in
  z.
  ...
  const matrix<double, 0, 1> &get_predicted_next_state() const;
  /*!
      ensures
...
void serialize(const rls_filter &item, std::ostream &out);
/*!
    provides serialization support
!*/

void deserialize(rls_filter &item, std::istream &in);
/*!
