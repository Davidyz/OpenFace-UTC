...
class rls_filter {
  /*!
      CONVENTION
          ...
          - data[i][0] == most recent measurement of i-th variable given to
  update.
          - data[i].back() == oldest measurement of i-th variable given to
  update (or zero if we haven't seen this much data yet).
...
public:
  ...
  explicit rls_filter(unsigned long size_, double forget_factor = 0.8,
                      double C = 100) {
    // make sure requires clause is not broken
    DLIB_ASSERT(0 < forget_factor && forget_factor <= 1 && 0 < C && size_ >= 2,
  ...
  double get_c() const { return filter.get_c(); }

  double get_forget_factor() const { return filter.get_forget_factor(); }

  unsigned long get_window_size() const { return size; }
  ...
  template <typename EXP> void update(const matrix_exp<EXP> &z) {
    // make sure requires clause is not broken
    DLIB_ASSERT(is_col_vector(z) == true && z.size() != 0 &&
  ...
  const matrix<double, 0, 1> &get_predicted_next_state() const { return next; }
  ...
  friend inline void deserialize(rls_filter &item, std::istream &in) {
    ...
    if (version != 1)
      throw dlib::serialization_error("Unknown version number found while "
                                      "deserializing rls_filter object.");
