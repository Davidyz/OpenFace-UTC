...
template <typename map_base> class map_kernel_c : public map_base {
...
inline void swap(map_kernel_c<map_base> &a, map_kernel_c<map_base> &b) {
  a.swap(b);
}
...
template <typename map_base>
void map_kernel_c<map_base>::add(domain &d, range &r) {
  // make sure requires clause is not broken
  DLIB_CASSERT(
      (!this->is_in_domain(d)) &&
...
template <typename map_base>
void map_kernel_c<map_base>::remove_any(domain &d, range &r) {
  // make sure requires clause is not broken
...
template <typename map_base>
void map_kernel_c<map_base>::remove(const domain &d, domain &d_copy, range &r) {
  // make sure requires clause is not broken
  DLIB_CASSERT((this->is_in_domain(d)) &&
                   ...
                   (static_cast<const void *>(&d) !=
                    static_cast<void *>(&d_copy)),
               "\tvoid map::remove"
                   << "\n\tcan't remove something that isn't in the map or if "
                      "the paremeters actually"
...
template <typename map_base>
void map_kernel_c<map_base>::destroy(const domain &d) {
  // make sure requires clause is not broken
  DLIB_CASSERT(this->is_in_domain(d),
               "\tvoid map::destroy"
...
template <typename map_base>
typename map_base::range_type &
map_kernel_c<map_base>::operator[](const domain &d) {
  // make sure requires clause is not broken
  DLIB_CASSERT(this->is_in_domain(d),
               "\trange& map::operator[]"
...
template <typename map_base>
const typename map_base::range_type &
map_kernel_c<map_base>::operator[](const domain &d) const {
  // make sure requires clause is not broken
  DLIB_CASSERT(this->is_in_domain(d),
