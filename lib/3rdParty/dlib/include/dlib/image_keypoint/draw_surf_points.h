...
// ----------------------------------------------------------------------------------------
inline void draw_surf_points(image_window &win,
  ...
  for (unsigned long i = 0; i < sp.size(); ++i) {
    ...
    // SURF descriptors are rotated by sp[i].angle.  So we want to include a
    // visual indication of this rotation on our overlay.
    direction = rotate_point(center, direction, sp[i].angle);

    win.add_overlay(
        image_display::overlay_circle(center, radius, rgb_pixel(0, 255, 0)));
