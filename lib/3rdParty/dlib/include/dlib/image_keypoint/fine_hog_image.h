...
template <unsigned long cell_size_, unsigned long block_size_,
          unsigned long pixel_stride_, unsigned char num_orientation_bins_,
          int gradient_type_>
...
public:
  ...
  fine_hog_image() : num_block_rows(0), num_block_cols(0) {}
  ...
  void copy_configuration(const fine_hog_image &) {}
      ...
      for (unsigned long c = 0; c < block_size; ++c) {
        for (unsigned long rr = 0; rr < cell_size; ++rr) {
          for (unsigned long cc = 0; cc < cell_size; ++cc) {
            const histogram_count &hist =
                hist_counts[row + r * cell_size + rr][col + c * cell_size + cc];
  ...
  template <typename image_type> void load_impl(const image_type &img) {
    ...
    for (long r = 0; r < hist_counts.nr(); ++r) {
      for (long c = 0; c < hist_counts.nc(); ++c) {
        ...
        // obtain the angle of the gradient.  Make sure it is scaled between 0
        // and 1.
        ...
        unsigned char quantized_angle_lower =
            static_cast<unsigned char>(std::floor(angle));
        unsigned char quantized_angle_upper =
            static_cast<unsigned char>(std::ceil(angle));
        ...
        // Stick into gradient counts.  Note that we linearly interpolate
        // between neighboring histogram buckets.
    ...
    num_block_rows =
        (hist_counts.nr() - block_size * cell_size + 1) / (long)pixel_stride;
    num_block_cols =
        (hist_counts.nc() - block_size * cell_size + 1) / (long)pixel_stride;
  }
...
template <unsigned long T1, unsigned long T2, unsigned long T3,
          unsigned char T4, int T5>
void serialize(const fine_hog_image<T1, T2, T3, T4, T5> &item,
               std::ostream &out) {
...
template <unsigned long T1, unsigned long T2, unsigned long T3,
          unsigned char T4, int T5>
void deserialize(fine_hog_image<T1, T2, T3, T4, T5> &item, std::istream &in) {
