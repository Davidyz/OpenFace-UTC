...
inline void serialize(const surf_point &item, std::ostream &out) {
  try {
...
inline void deserialize(surf_point &item, std::istream &in) {
  try {
...
inline double gaussian(double x, double y, double sig) {
  DLIB_ASSERT(sig > 0, "\tdouble gaussian()"
...
double compute_dominant_angle(const integral_image_type &img,
                              ...
                              const double &scale) {
  DLIB_ASSERT(get_rect(img).contains(
                  centered_rect(center, (unsigned long)(17 * scale),
                                (unsigned long)(17 * scale))) == true &&
                  scale > 0,
...
void compute_surf_descriptor(const integral_image_type &img,
  ...
  DLIB_ASSERT(get_rect(img).contains(
                  centered_rect(center, (unsigned long)(32 * scale),
                                (unsigned long)(32 * scale))) == true &&
                  scale > 0,
  ...
  for (long r = -10; r < 10; r += 5) {
    for (long c = -10; c < 10; c += 5) {
      ...
      // that we include 1 pixels worth of padding around the outside of each
      // 5x5 cell.  This is to help neighboring cells interpolate their counts
      ...
      for (long y = r - 1; y < r + 5 + 1; ++y) {
        ...
        for (long x = c - 1; x < c + 5 + 1; ++x) {
          ...
          const double weight =
              1.0 / (4 + std::abs(center_r - y) + std::abs(center_c - x));
...
get_surf_points(const image_type &img, long max_points = 10000,
  ...
  typedef typename pixel_traits<
      typename image_traits<image_type>::pixel_type>::basic_pixel_type bp_type;
  ...
  surf_point sp;
  for (unsigned long i = 0; i < std::min((size_t)max_points, points.size());
       ++i) {
    // ignore points that are close to the edge of the image
    const double border = 32;
    const unsigned long border_size =
        static_cast<unsigned long>(border * points[i].scale);
    if (get_rect(int_img).contains(
            centered_rect(points[i].center, border_size, border_size))) {
      sp.angle =
          compute_dominant_angle(int_img, points[i].center, points[i].scale);
      compute_surf_descriptor(int_img, points[i].center, points[i].scale,
                              sp.angle, sp.des);
      sp.p = points[i];
