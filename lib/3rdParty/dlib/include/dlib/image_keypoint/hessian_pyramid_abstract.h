...
public:
  ...
  /*!
      requires
          ...
          - num_intervals > 0
          - initial_step_size > 0
          - integral_image_type == an object such as dlib::integral_image or
  another type that implements the interface defined in
  image_transforms/integral_image_abstract.h ensures
  ...
  /*!
      ...
      ensures
          - Each interval of the pyramid has a certain sized border region where
  we can't compute the Hessian values since they are too close to the edge of
  the input image.  This function returns the size of that border.
  !*/
  ...
  /*!
      ...
      ensures
          - Each octave has a step size value.  This value determines how many
            ...
            As the octave gets larger (i.e. as it goes to the top of the
  pyramid) the step size gets bigger and thus the pyramid narrows.
  !*/
  ...
  /*!
      requires
          ...
          - Let BS == get_border_size(interval): then
              ...
              - BS <= c < nc(octave)-BS
      ensures
          - returns the determinant of the Hessian from the given octave and
  interval of the pyramid.  The specific point sampled at this pyramid level is
            the one that corresponds to the input image point
  (point(r,c)*get_step_size(octave)).
  !*/
  ...
  /*!
      ...
      ensures
          - returns the sign of the laplacian for the given octave and interval
            ...
            the one that corresponds to the input image point
  (point(r,c)*get_step_size(octave)).
          - The laplacian is the trace of the Hessian at the given point.  So
  this function returns either +1 or -1 depending on this number's sign.  This
            value can be used to distinguish bright blobs on dark backgrounds
  from the reverse.
  !*/
};
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          meanings:
              - center == the x/y location of the center of the interest point
                ...
                column in the image)
              - scale == the scale at which the point was detected.  This is a
  number
                >= 1.  If it is 1 then it means the interest point was detected
  at the lowest scale in the image pyramid.  Larger numbers indicate that the
  interest point is from high up in the image pyramid.  For example, a scale of
  4 would mean the interest point was located at a point in the pyramid where
  the image had been shrunk by a factor of 4.
...
void serialize(const interest_point &item, std::ostream &out);
/*!
    provides serialization support
!*/

void deserialize(interest_point &item, std::istream &in);
/*!
    provides serialization support
!*/
...
/*!
    ...
    ensures
        - extracts interest points from the pyramid pyr and stores them into
          result_points (note that result_points is cleared before these new
interest points are added to it).
        - Only interest points with determinant values in the pyramid larger
than threshold are output.
!*/
