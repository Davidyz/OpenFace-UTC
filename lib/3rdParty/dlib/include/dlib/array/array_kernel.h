...
public:
  // These typedefs are here for backwards compatibility with old versions of
  // dlib.
  ...
  array()
      : array_size(0), max_array_size(0), array_elements(0), pos(0),
        last_pos(0), _at_start(true) {}

  array(const array &) = delete;
  array &operator=(array &) = delete;

  array(array &&item) : array() { swap(item); }
...
template <typename T, typename mem_manager>
void serialize(const array<T, mem_manager> &item, std::ostream &out) {
  try {
...
template <typename T, typename mem_manager>
void deserialize(array<T, mem_manager> &item, std::istream &in) {
  try {
...
template <typename T, typename mem_manager> array<T, mem_manager>::~array() {
  if (array_elements) {
...
template <typename T, typename mem_manager>
void array<T, mem_manager>::clear() {
  reset();
  last_pos = 0;
  array_size = 0;
  if (array_elements) {
...
template <typename T, typename mem_manager>
const T &array<T, mem_manager>::operator[](size_t pos) const {
  // make sure requires clause is not broken
  DLIB_ASSERT(pos < this->size(), "\tconst T& array::operator[]"
                                      << "\n\tpos must < size()"
                                      << "\n\tpos: " << pos << "\n\tsize(): "
                                      << this->size() << "\n\tthis: " << this);
...
template <typename T, typename mem_manager>
T &array<T, mem_manager>::operator[](size_t pos) {
  // make sure requires clause is not broken
  DLIB_ASSERT(pos < this->size(), "\tT& array::operator[]"
                                      << "\n\tpos must be < size()"
                                      << "\n\tpos: " << pos << "\n\tsize(): "
                                      << this->size() << "\n\tthis: " << this);
...
template <typename T, typename mem_manager>
void array<T, mem_manager>::set_size(size_t size) {
  // make sure requires clause is not broken
...
template <typename T, typename mem_manager>
size_t array<T, mem_manager>::size() const {
  return array_size;
}
...
template <typename T, typename mem_manager>
void array<T, mem_manager>::set_max_size(size_t max) {
  reset();
  array_size = 0;
  last_pos = 0;
  if (max != 0) {
    ...
    if (max != max_array_size) {
      ...
      try {
        array_elements = pool.allocate_array(max);
      } catch (...) {
        array_elements = 0;
        max_array_size = 0;
        throw;
...
template <typename T, typename mem_manager>
size_t array<T, mem_manager>::max_size() const {
...
template <typename T, typename mem_manager> void array<T, mem_manager>::sort() {
  if (this->size() > 1) {
...
template <typename T, typename mem_manager>
void array<T, mem_manager>::resize(size_t new_size) {
...
template <typename T, typename mem_manager> T &array<T, mem_manager>::back() {
  // make sure requires clause is not broken
  DLIB_ASSERT(this->size() > 0, "\tT& array::back()"
...
const T &array<T, mem_manager>::back() const {
  // make sure requires clause is not broken
  DLIB_ASSERT(this->size() > 0, "\tconst T& array::back()"
                                    << "\n\tsize() must be bigger than 0"
                                    << "\n\tsize(): " << this->size()
                                    << "\n\tthis:   " << this);
...
void array<T, mem_manager>::pop_back(T &item) {
  // make sure requires clause is not broken
  DLIB_ASSERT(this->size() > 0, "\tvoid array::pop_back()"
                                    << "\n\tsize() must be bigger than 0"
                                    << "\n\tsize(): " << this->size()
                                    << "\n\tthis:   " << this);
...
template <typename T, typename MM> struct is_array<array<T, MM>> {
