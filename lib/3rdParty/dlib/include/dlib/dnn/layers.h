...
public:
  ...
  con_(num_con_outputs o)
      : learning_rate_multiplier(1), weight_decay_multiplier(1),
        bias_learning_rate_multiplier(1), bias_weight_decay_multiplier(0),
        num_filters_(o.num_outputs), padding_y_(_padding_y),
  ...
  void set_num_filters(long num) {
    ...
    if (num != num_filters_) {
      DLIB_CASSERT(get_layer_params().size() == 0,
                   "You can't change the number of filters in con_ if the "
                   "parameter tensor has already been allocated.");
      num_filters_ = num;
    }
  }
  ...
  con_(const con_ &item)
      : params(item.params), filters(item.filters), biases(item.biases),
        ...
        num_filters_(item.num_filters_), padding_y_(item.padding_y_),
        padding_x_(item.padding_x_) {
    // this->conv is non-copyable and basically stateless, so we have to write
    // our own copy to avoid trying to copy it and getting an error.
  }

  con_ &operator=(const con_ &item) {
    if (this == &item)
      return *this;
  ...
  template <typename SUBNET> void setup(const SUBNET &sub) {
    ...
    filters =
        alias_tensor(num_filters_, sub.get_output().k(), filt_nr, filt_nc);
  ...
  void forward(const SUBNET &sub, resizable_tensor &output) {
    conv.setup(sub.get_output(), filters(params, 0), _stride_y, _stride_x,
               padding_y_, padding_x_);
    conv(false, output, sub.get_output(), filters(params, 0));
  ...
  void backward(const tensor &gradient_input, SUBNET &sub,
                tensor &params_grad) {
    conv.get_gradient_for_data(true, gradient_input, filters(params, 0),
    ...
    if (learning_rate_multiplier != 0) {
      ...
      conv.get_gradient_for_filters(false, gradient_input, sub.get_output(),
                                    filt);
  ...
  friend std::ostream &operator<<(std::ostream &out, const con_ &item) {
    out << "con\t ("
        << "num_filters=" << item.num_filters_ << ", nr=" << item.nr()
        << ", nc=" << item.nc() << ", stride_y=" << _stride_y
        << ", stride_x=" << _stride_x << ", padding_y=" << item.padding_y_
...
private:
  ...
  // These are here only because older versions of con (which you might
  // encounter serialized to disk) used different padding settings.
  int padding_y_;
  int padding_x_;
};
...
public:
  ...
  static_assert(_stride_y > 0, "The filter stride must be > 0");
  static_assert(_stride_x > 0, "The filter stride must be > 0");
  static_assert(0 <= _padding_y && _padding_y < _nr,
                "The padding must be smaller than the filter size.");
  static_assert(0 <= _padding_x && _padding_x < _nc,
                "The padding must be smaller than the filter size.");
  ...
  void set_num_filters(long num) {
    ...
    if (num != num_filters_) {
      DLIB_CASSERT(get_layer_params().size() == 0,
                   "You can't change the number of filters in cont_ if the "
                   "parameter tensor has already been allocated.");
      num_filters_ = num;
    }
  }
  ...
  cont_ &operator=(const cont_ &item) {
    if (this == &item)
      return *this;
  ...
  void forward(const SUBNET &sub, resizable_tensor &output) {
    ...
    unsigned int gnr =
        _stride_y * (sub.get_output().nr() - 1) + filt.nr() - 2 * padding_y_;
    unsigned int gnc =
        _stride_x * (sub.get_output().nc() - 1) + filt.nc() - 2 * padding_x_;
  ...
  void backward(const tensor &gradient_input, SUBNET &sub,
    ...
    if (learning_rate_multiplier != 0) {
      auto filt = filters(params_grad, 0);
      conv.get_gradient_for_filters(false, sub.get_output(), gradient_input,
                                    filt);
      auto b = biases(params_grad, filters.size());
      tt::assign_conv_bias_gradient(b, gradient_input);
    }
  ...
  friend std::ostream &operator<<(std::ostream &out, const cont_ &item) {
    out << "cont\t ("
        << "num_filters=" << item.num_filters_ << ", nr=" << _nr
        << ", nc=" << _nc << ", stride_y=" << _stride_y
        << ", stride_x=" << _stride_x << ", padding_y=" << item.padding_y_
        << ", padding_x=" << item.padding_x_ << ")";
    out << " learning_rate_mult=" << item.learning_rate_multiplier;
    out << " weight_decay_mult=" << item.weight_decay_multiplier;
...
public:
  ...
  template <typename SUBNET>
  void forward(const SUBNET &sub, resizable_tensor &output) {
    output.set_size(sub.get_output().num_samples(), sub.get_output().k(),
  ...
  friend void deserialize(upsample_ &, std::istream &in) {
    ...
    if (version != "upsample_")
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::upsample_.");
...
public:
  ...
  void forward(const SUBNET &sub, resizable_tensor &output) {
    ...
    output.set_size(sub.get_output().num_samples(), sub.get_output().k(), NR_,
                    NC_);
    tt::resize_bilinear(output, sub.get_output());
  }
  ...
  friend void deserialize(resize_to_ &item, std::istream &in) {
    ...
    if (version != "resize_to_")
      throw serialization_error(
          ...
          "' found while deserializing dlib::resize_to_.");
...
class max_pool_ {
  ...
  static_assert(_stride_y > 0, "The filter stride must be > 0");
  static_assert(_stride_x > 0, "The filter stride must be > 0");
  static_assert(0 <= _padding_y && ((_nr == 0 && _padding_y == 0) ||
                                    (_nr != 0 && _padding_y < _nr)),
                "The padding must be smaller than the filter size, unless the "
                "filters size is 0.");
  static_assert(0 <= _padding_x && ((_nc == 0 && _padding_x == 0) ||
                                    (_nc != 0 && _padding_x < _nc)),
                "The padding must be smaller than the filter size, unless the "
                "filters size is 0.");

public:
  max_pool_() : padding_y_(_padding_y), padding_x_(_padding_x) {}
  ...
  max_pool_(const max_pool_ &item)
      : padding_y_(item.padding_y_), padding_x_(item.padding_x_) {
    // this->mp is non-copyable so we have to write our own copy to avoid trying
    // to copy it and getting an error.
  }

  max_pool_ &operator=(const max_pool_ &item) {
    if (this == &item)
      return *this;
  ...
  void forward(const SUBNET &sub, resizable_tensor &output) {
    mp.setup_max_pooling(_nr != 0 ? _nr : sub.get_output().nr(),
                         _nc != 0 ? _nc : sub.get_output().nc(), _stride_y,
                         _stride_x, padding_y_, padding_x_);
  ...
  void backward(const tensor &computed_output, const tensor &gradient_input,
    ...
    mp.get_gradient(gradient_input, computed_output, sub.get_output(),
                    sub.get_gradient_input());
  }
  ...
  friend void deserialize(max_pool_ &item, std::istream &in) {
    ...
    if (version == "max_pool_2") {
      ...
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::max_pool_.");
    }

    if (item.padding_y_ != _padding_y)
      throw serialization_error(
          "Wrong padding_y found while deserializing dlib::max_pool_");
    if (item.padding_x_ != _padding_x)
      throw serialization_error(
          "Wrong padding_x found while deserializing dlib::max_pool_");
    if (_nr != nr)
      throw serialization_error(
          "Wrong nr found while deserializing dlib::max_pool_");
    if (_nc != nc)
      throw serialization_error(
          "Wrong nc found while deserializing dlib::max_pool_");
    if (_stride_y != stride_y)
      throw serialization_error(
          "Wrong stride_y found while deserializing dlib::max_pool_");
    if (_stride_x != stride_x)
      throw serialization_error(
          "Wrong stride_x found while deserializing dlib::max_pool_");
  }

  friend std::ostream &operator<<(std::ostream &out, const max_pool_ &item) {
    out << "max_pool ("
        << "nr=" << _nr << ", nc=" << _nc << ", stride_y=" << _stride_y
        << ", stride_x=" << _stride_x << ", padding_y=" << item.padding_y_
        << ", padding_x=" << item.padding_x_ << ")";
    return out;
  }
...
public:
  ...
  avg_pool_() : padding_y_(_padding_y), padding_x_(_padding_x) {}
  ...
  avg_pool_(const avg_pool_ &item)
      : padding_y_(item.padding_y_), padding_x_(item.padding_x_) {
    // this->ap is non-copyable so we have to write our own copy to avoid trying
    // to copy it and getting an error.
  }

  avg_pool_ &operator=(const avg_pool_ &item) {
    if (this == &item)
      return *this;
  ...
  void backward(const tensor &computed_output, const tensor &gradient_input,
    ...
    ap.get_gradient(gradient_input, computed_output, sub.get_output(),
                    sub.get_gradient_input());
  }
  ...
  friend void deserialize(avg_pool_ &item, std::istream &in) {
    ...
    if (version == "avg_pool_2") {
      ...
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::avg_pool_.");
    }

    if (item.padding_y_ != _padding_y)
      throw serialization_error(
          "Wrong padding_y found while deserializing dlib::avg_pool_");
    if (item.padding_x_ != _padding_x)
      throw serialization_error(
          "Wrong padding_x found while deserializing dlib::avg_pool_");
    if (_nr != nr)
      throw serialization_error(
          "Wrong nr found while deserializing dlib::avg_pool_");
    if (_nc != nc)
      throw serialization_error(
          "Wrong nc found while deserializing dlib::avg_pool_");
    if (_stride_y != stride_y)
      throw serialization_error(
          "Wrong stride_y found while deserializing dlib::avg_pool_");
    if (_stride_x != stride_x)
      throw serialization_error(
          "Wrong stride_x found while deserializing dlib::avg_pool_");
  }
...
enum layer_mode { CONV_MODE = 0, FC_MODE = 1 };

const double DEFAULT_BATCH_NORM_EPS = 0.0001;

template <layer_mode mode> class bn_ {
public:
  explicit bn_(unsigned long window_size, double eps_ = DEFAULT_BATCH_NORM_EPS)
      : num_updates(0), running_stats_window_size(window_size),
        learning_rate_multiplier(1), weight_decay_multiplier(0),
        bias_learning_rate_multiplier(1), bias_weight_decay_multiplier(1),
        eps(eps_) {
    DLIB_CASSERT(
        window_size > 0,
        "The batch normalization running stats window size can't be 0.");
  }
  ...
  template <typename SUBNET> void setup(const SUBNET &sub) {
    if (mode == FC_MODE) {
      gamma = alias_tensor(1, sub.get_output().k(), sub.get_output().nr(),
                           sub.get_output().nc());
    } else {
  ...
  void forward(const SUBNET &sub, resizable_tensor &output) {
    ...
    if (sub.get_output().num_samples() > 1) {
      ...
      if (mode == FC_MODE)
        tt::batch_normalize(eps, output, means, invstds, decay, running_means,
                            running_variances, sub.get_output(), g, b);
      else
        tt::batch_normalize_conv(eps, output, means, invstds, decay,
                                 running_means, running_variances,
                                 sub.get_output(), g, b);
    } else // we are running in testing mode so we just linearly scale the input
           // tensor.
    {
      if (mode == FC_MODE)
        tt::batch_normalize_inference(eps, output, sub.get_output(), g, b,
                                      running_means, running_variances);
      else
        tt::batch_normalize_conv_inference(eps, output, sub.get_output(), g, b,
                                           running_means, running_variances);
    }
  }
  ...
  void backward(const tensor &gradient_input, SUBNET &sub,
    ...
    if (mode == FC_MODE)
      tt::batch_normalize_gradient(eps, gradient_input, means, invstds,
                                   sub.get_output(), g,
                                   sub.get_gradient_input(), g_grad, b_grad);
    else
      tt::batch_normalize_conv_gradient(
          eps, gradient_input, means, invstds, sub.get_output(), g,
          sub.get_gradient_input(), g_grad, b_grad);
  }
...
public:
  visitor_bn_running_stats_window_size(unsigned long new_window_size_)
      : new_window_size(new_window_size_) {}

  template <typename T> void set_window_size(T &) const {
    // ignore other layer detail types
  }

  template <layer_mode mode> void set_window_size(bn_<mode> &l) const {
...
void set_all_bn_running_stats_window_sizes(net_type &net,
                                           unsigned long new_window_size) {
  visit_layers(net,
               impl::visitor_bn_running_stats_window_size(new_window_size));
}
...
template <unsigned long num_outputs_, fc_bias_mode bias_mode> class fc_ {
  static_assert(num_outputs_ > 0,
                "The number of outputs from a fc_ layer must be > 0");

public:
  fc_(num_fc_outputs o)
      : num_outputs(o.num_outputs), num_inputs(0), learning_rate_multiplier(1),
        weight_decay_multiplier(1), bias_learning_rate_multiplier(1),
  ...
  void set_num_outputs(long num) {
    ...
    if (num != (long)num_outputs) {
      DLIB_CASSERT(get_layer_params().size() == 0,
                   "You can't change the number of filters in fc_ if the "
                   "parameter tensor has already been allocated.");
      num_outputs = num;
    }
  }

  fc_bias_mode get_bias_mode() const { return bias_mode; }

  template <typename SUBNET> void setup(const SUBNET &sub) {
    num_inputs =
        sub.get_output().nr() * sub.get_output().nc() * sub.get_output().k();
  ...
  template <typename SUBNET>
  void forward(const SUBNET &sub, resizable_tensor &output) {
    DLIB_CASSERT((long)num_inputs == sub.get_output().nr() *
                                         sub.get_output().nc() *
                                         sub.get_output().k(),
                 "The size of the input tensor to this fc layer doesn't match "
                 "the size the fc layer was trained with.");
  ...
  alias_tensor_instance get_biases() {
    static_assert(bias_mode == FC_HAS_BIAS,
...
class dropout_ {
public:
  explicit dropout_(float drop_rate_ = 0.5)
      : drop_rate(drop_rate_), rnd(std::rand()) {
    DLIB_CASSERT(0 <= drop_rate && drop_rate <= 1);
  }

  // We have to add a copy constructor and assignment operator because the rnd
  // object is non-copyable.
  dropout_(const dropout_ &item)
      : drop_rate(item.drop_rate), mask(item.mask), rnd(std::rand()) {}

  dropout_ &operator=(const dropout_ &item) {
    if (this == &item)
      return *this;
  ...
  friend void deserialize(dropout_ &item, std::istream &in) {
    ...
    if (version != "dropout_")
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::dropout_.");
    deserialize(item.drop_rate, in);
    deserialize(item.mask, in);
  }

  void clean() { mask.clear(); }
...
class multiply_ {
public:
  explicit multiply_(float val_ = 0.5) : val(val_) {}

  multiply_(const dropout_ &item) : val(1 - item.get_drop_rate()) {}

  float get_multiply_value() const { return val; }
  ...
  friend void deserialize(multiply_ &item, std::istream &in) {
    ...
    if (version != "multiply_")
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::multiply_.");
...
class affine_ {
public:
  affine_() : mode(FC_MODE) {}

  affine_(layer_mode mode_) : mode(mode_) {}

  template <layer_mode bnmode> affine_(const bn_<bnmode> &item) {
    gamma = item.gamma;
    beta = item.beta;
    mode = bnmode;
    ...
    g = pointwise_multiply(
        mat(sg), 1.0f / sqrt(mat(item.running_variances) + item.get_eps()));
  ...
  void backward_inplace(const tensor &gradient_input, tensor &data_grad,
    ...
    // We are computing the gradient of dot(gradient_input, computed_output*g +
    // b)
    if (mode == FC_MODE) {
      if (is_same_object(gradient_input, data_grad))
  ...
  friend void deserialize(affine_ &item, std::istream &in) {
    ...
    if (version != "affine_")
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::affine_.");
    deserialize(item.params, in);
    deserialize(item.gamma, in);
    deserialize(item.beta, in);
    int mode;
...
public:
  ...
  void forward(const SUBNET &sub, resizable_tensor &output) {
    ...
    output.set_size(std::max(t1.num_samples(), t2.num_samples()),
                    std::max(t1.k(), t2.k()), std::max(t1.nr(), t2.nr()),
                    std::max(t1.nc(), t2.nc()));
    tt::add(output, t1, t2);
  }
  ...
  void backward(const tensor &gradient_input, SUBNET &sub,
    ...
    tt::add(layer<tag>(sub).get_gradient_input(),
            layer<tag>(sub).get_gradient_input(), gradient_input);
  }
...
public:
  ...
  void backward(const tensor &gradient_input, SUBNET &sub,
    ...
    // multiplied together.
    tt::multiply_zero_padded(true, sub.get_gradient_input(), t2,
                             gradient_input);
    tt::multiply_zero_padded(true, layer<tag>(sub).get_gradient_input(), t1,
                             gradient_input);
  }
...
public:
  ...
  void backward(const tensor &gradient_input, SUBNET &sub,
    ...
    // The gradient just flows backwards to the two layers that forward()
    // read from.
    tt::scale_channels(true, layer<tag>(sub).get_gradient_input(),
                       gradient_input, scales);

    if (reshape_src.num_samples() != src.num_samples()) {
      reshape_scales = alias_tensor(src.num_samples() * src.k());
      reshape_src =
          alias_tensor(src.num_samples() * src.k(), src.nr() * src.nc());
    }
  ...
  friend void deserialize(scale_ &item, std::istream &in) {
    ...
    if (version != "scale_")
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::scale_.");
...
public:
  ...
  friend void deserialize(relu_ &, std::istream &in) {
    ...
    if (version != "relu_")
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::relu_.");
  }
...
public:
  ...
  float get_initial_param_value() const { return initial_param_value; }
  ...
  friend void deserialize(prelu_ &item, std::istream &in) {
    ...
    if (version != "prelu_")
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::prelu_.");
    deserialize(item.params, in);
...
public:
  ...
  friend void deserialize(sig_ &, std::istream &in) {
    ...
    if (version != "sig_")
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::sig_.");
  }
...
public:
  ...
  friend void deserialize(htan_ &, std::istream &in) {
    ...
    if (version != "htan_")
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::htan_.");
  }
...
public:
  ...
  friend void deserialize(softmax_ &, std::istream &in) {
    ...
    if (version != "softmax_")
      throw serialization_error("Unexpected version '" + version +
                                "' found while deserializing dlib::softmax_.");
  }
...
struct concat_helper_impl {
  ...
  constexpr static size_t tag_count() {
    return 1 + concat_helper_impl<TAG_TYPES...>::tag_count();
  }
...
template <template <typename> class... TAG_TYPES> class concat_ {
  static void list_tags(std::ostream &out) {
    impl::concat_helper_impl<TAG_TYPES...>::list_tags(out);
  };

public:
  constexpr static size_t tag_count() {
    return impl::concat_helper_impl<TAG_TYPES...>::tag_count();
  };

  template <typename SUBNET> void setup(const SUBNET &) {
    // do nothing
  }
  template <typename SUBNET>
  void forward(const SUBNET &sub, resizable_tensor &output) {
  ...
  friend void deserialize(concat_ &item, std::istream &in) {
    ...
    if (count_tags != tag_count())
      throw serialization_error("Invalid count of tags " +
                                std::to_string(count_tags) + ", expecting " +
...
// inception layer will use tags internally. If user will use tags too, some
// conflicts possible to exclude them, here are new tags specially for
// inceptions
...
class l2normalize_ {
public:
  explicit l2normalize_(double eps_ = DEFAULT_L2_NORM_EPS) : eps(eps_) {}
  ...
  void backward_inplace(const tensor &computed_output,
    ...
    if (is_same_object(gradient_input, data_grad)) {
      ...
      tt::scale_rows2(0, data_grad, gradient_input, computed_output, temp,
                      norm);
    } else {
      tt::dot_prods(temp, gradient_input, computed_output);
      tt::scale_rows2(1, data_grad, gradient_input, computed_output, temp,
                      norm);
    }
  }
...
template <typename SUBNET> using l2normalize = add_layer<l2normalize_, SUBNET>;
...
public:
  ...
  template <typename SUBNET> void setup(const SUBNET &sub) {
    DLIB_CASSERT((long)sub.get_output().size() >=
                     sub.get_output().num_samples() *
                         (_offset + _k * _nr * _nc),
                 "The tensor we are trying to extract from the input tensor is "
                 "too big to fit into the input tensor.");

    aout = alias_tensor(sub.get_output().num_samples(), _k * _nr * _nc);
    ain =
        alias_tensor(sub.get_output().num_samples(),
