...
namespace impl {
template <typename training_label_type> struct dnn_job_t {
  dnn_job_t() = default;
  ...
  std::vector<int>
      have_data; // have_data[i] is true if there is data in labels[i] and t[i].
  bool test_only = false;
};

template <typename training_label_type>
void swap(dnn_job_t<training_label_type> &a,
          dnn_job_t<training_label_type> &b) {
...
public:
  ...
  explicit dnn_trainer(net_type &net_) : job_pipe(0), net(net_) {
    ...
    devices.push_back(std::make_shared<device_data>(dlib::cuda::get_device(),
                                                    net, default_solver));

    init();
  }

  dnn_trainer(net_type &net_, const solver_type &solver_)
      : job_pipe(0), net(net_) {
    devices.push_back(
        std::make_shared<device_data>(dlib::cuda::get_device(), net, solver_));
  ...
  dnn_trainer(net_type &net_, const solver_type &solver_,
    ...
    for (auto id : temp) {
      DLIB_CASSERT(0 <= id && id < total_devices,
                   "Invalid CUDA device id given to dnn_trainer.");
      // Switch to this device so that any tensor objects that get allocated
      // when we create the device context happen on this device.
      dlib::cuda::set_device(id);
      devices.push_back(
          std::make_shared<device_data>(id, net, solver_, clone_net()));
    }
  ...
  net_type &
  get_net(force_flush_to_disk force_flush = force_flush_to_disk::yes) {
  ...
  unsigned long get_mini_batch_size() const { return mini_batch_size; }
  ...
  unsigned long get_max_num_epochs() const { return max_num_epochs; }
  ...
  void train(const std::vector<input_type> &data,
    ...
    // instead use class members is so we can include the state of the loops in
    // the stuff written by sync_to_disk()
    for (;
      ...
      clear_average_loss();
      for (; epoch_pos < data.size() && learning_rate >= min_learning_rate;
        ...
        if (verbose) {
          ...
          if (now_time - last_time > seconds(20)) {
            ...
            std::cout << "epoch: "
                      << rpad(cast_to_string(iter), epoch_string_pad) << "  "
                      << "learning rate: "
                      << rpad(cast_to_string(learning_rate), lr_string_pad)
                      << "  "
                      << "average loss: "
                      << rpad(cast_to_string(get_average_loss()), string_pad)
                      << "  ";
            print_progress();
          }
        }
        ...
        send_job(false, data.begin() + epoch_pos,
                 data.begin() +
                     std::min(epoch_pos + mini_batch_size, data.size()),
      ...
      if (verbose) {
        ...
        std::cout << "Epoch: "
                  << rpad(cast_to_string(epoch_iteration + 1), epoch_string_pad)
                  << "  "
                  << "learning rate: "
                  << rpad(cast_to_string(learning_rate), lr_string_pad) << "  "
                  << "average loss: "
  ...
  void train(const std::vector<input_type> &data) {
    ...
    const bool has_unsupervised_loss =
        std::is_same<no_label_type, training_label_type>::value;
    static_assert(has_unsupervised_loss,
                  "You can only call this version of train() when using an "
                  "unsupervised loss.");
  ...
  const std::string &get_synchronization_file() { return sync_filename; }
  ...
  double get_min_learning_rate() const { return min_learning_rate; }
...
private:
  ...
  void record_loss(double loss) {
    // This kind of budgeting causes our gradient checking to use a fixed amount
    // of computational resources, regardless of the size of
  ...
  template <typename T>
  double compute_parameter_gradients(size_t device, job_t &next_job,
                                     const T &) {
    if (next_job.have_data[device]) {
      ...
      if (next_job.test_only)
        return dev.net.compute_loss(next_job.t[device],
                                    next_job.labels[device].begin());
      else
        return dev.net.compute_parameter_gradients(
            next_job.t[device], next_job.labels[device].begin());
    } else {
      return 0;
    }
  }
  ...
  void thread() try {
    ...
    // We make separate thread pools with just one thread in them because we
    // want to make sure each device is always executed on the same thread.  We
    // care about this because there are thread_local context variables for some
    // cuda components and they get allocated for each combination of thread and
    // device. So if we make sure the same device always uses the same thread
    // this will reduce the number of contexts we allocate from
    // num_devices*num_devices to just num_devices.
    ...
    while (job_pipe.dequeue(next_job)) {
      if (next_job.test_only) {
        ...
        for (size_t i = 0; i < devices.size(); ++i)
          tp[i]->add_task_by_value(
              [&, i](double &loss) {
                loss = compute_parameter_gradients(i, next_job,
        ...
        if (learning_rate_shrink != 1) {
          test_steps_without_progress =
              count_steps_without_decrease(test_previous_loss_values);
          if (test_steps_without_progress >=
              test_iter_without_progress_thresh) {
            test_steps_without_progress =
                count_steps_without_decrease_robust(test_previous_loss_values);
            if (test_steps_without_progress >=
                test_iter_without_progress_thresh) {
              // optimization has flattened out, so drop the learning rate.
              learning_rate = learning_rate_shrink * learning_rate;
              test_steps_without_progress = 0;
              // Empty out some of the previous loss values so that
              // test_steps_without_progress will decrease below
              // test_iter_without_progress_thresh.
              for (unsigned long cnt = 0;
                   cnt < test_previous_loss_values_dump_amount +
                             test_iter_without_progress_thresh / 10 &&
                   test_previous_loss_values.size() > 0;
                   ++cnt)
                test_previous_loss_values.pop_front();
            }
          }
      ...
      if (devices.size() > 1) {
        ...
        if (averagers.size() == 0 || sync_file_reloaded) {
          averagers = std::vector<tt::multi_device_tensor_averager>(
              net_type::num_computational_layers);
          ...
          for (size_t i = 0; i < all_tensors.size(); ++i) {
            ...
            visit_layer_parameter_gradients(
                devices[i]->net,
                [&](size_t j, tensor &t) { all_tensors[i][j] = &t; });
          }
          // Now set each averager to average the tensors at the same layer in
          // each network.
      ...
      for (size_t i = 0; i < devices.size(); ++i)
        tp[i]->add_task_by_value([&, i]() {
          if (next_job.have_data[i])
            update_parameters(i);
        });
      ...
      // sure they aren't drifting apart due to any non-deterministic behavior
      // on the GPU.  It's also important to do this on the first iteration
      // because the different networks may be initialized differently when
      // tensor data is first passed through them.  So this code block deals
      // with these issues.
      ...
      // dropping.  If it isn't then we will reduce the learning rate.  Note
      // that we have a "budget" that prevents us from calling
      ...
      if (gradient_check_budget > iter_without_progress_thresh &&
        ...
        steps_without_progress =
            count_steps_without_decrease(previous_loss_values);
        if (steps_without_progress >= iter_without_progress_thresh) {
          ...
          steps_without_progress =
              count_steps_without_decrease_robust(previous_loss_values);
          if (steps_without_progress >= iter_without_progress_thresh) {
            // optimization has flattened out, so drop the learning rate.
            learning_rate = learning_rate_shrink * learning_rate;
            steps_without_progress = 0;
            // Empty out some of the previous loss values so that
            // steps_without_progress will decrease below
            // iter_without_progress_thresh.
            for (unsigned long cnt = 0;
                 cnt < previous_loss_values_dump_amount +
                           iter_without_progress_thresh / 10 &&
                 previous_loss_values.size() > 0;
                 ++cnt)
              previous_loss_values.pop_front();
          }
        }
      } else if (lr_schedule.size() !=
                 0) // or use the learning rate schedule if we have one.
      {
  ...
  // result in user errors.  However, we use these functions as part of the
  // automatic sync code in this object.
  ...
  friend void deserialize(dnn_trainer &item, std::istream &in) {
    ...
    if (num_layers != dnn_trainer::num_layers) {
      ...
      sout << "Error deserializing dlib::dnn_trainer.  The saved sync file is "
              "for a network with "
           << std::endl;
      sout << "a different number of layers.  We expected the number of layers "
              "to be "
           << dnn_trainer::num_layers << " but" << std::endl;
      sout << "instead the file contains " << num_layers << " layers."
           << std::endl;
    ...
    if (item.devices.size() > 1) {
      ...
      for (size_t i = 1; i < item.devices.size(); ++i) {
        // Switch to this device so that any tensor objects that get allocated
        // when we copy this stuff happen on this device.
  ...
  void sync_to_disk(bool do_it_now = false) {
    ...
    if (std::chrono::system_clock::now() - last_sync_time >
            time_between_syncs ||
        do_it_now) {
      wait_for_thread_to_pause();
      ...
      if (loss_increased_since_last_disk_sync()) {
        ...
        if (verbose)
          std::cout << "Loss has been increasing, reloading saved state from "
                    << newest_syncfile() << std::endl;
      } else {
  ...
  bool loss_increased_since_last_disk_sync() {
    size_t gradient_updates_since_last_sync =
        main_iteration_counter - main_iteration_counter_at_last_disk_sync;
    ...
    for (auto x : previous_loss_values) {
      // If we get a NaN value of loss assume things have gone horribly wrong
      // and we should reload the state of the trainer.
      if (std::isnan(x))
        return true;
    }

    // if we haven't seen much data yet then just say false.  Or, alternatively,
    ...
    if (prob > prob_loss_increasing_thresh &&
        prob_loss_increasing_thresh <= prob_loss_increasing_thresh_max_value) {
      ...
      prob_loss_increasing_thresh =
          std::max(prob_loss_increasing_thresh,
  ...
  // per device state.  All the containers have the same number of objects in
  // them.
  struct device_data {
    device_data(int device_id_, net_type &net_, const solver_type &solver_)
        : device_id(device_id_), net(net_),
          solvers(num_computational_layers, solver_) {}

    device_data(int device_id_, net_type &net_, const solver_type &solver_,
                clone_net)
        : device_id(device_id_), net_copy(std::make_shared<net_type>(net_)),
          net(*net_copy), solvers(num_computational_layers, solver_) {}
  ...
  template <typename data_iterator, typename label_iterator>
  void send_job(bool test_only, data_iterator dbegin, data_iterator dend,
                label_iterator lbegin) {
    propagate_exception();
  ...
  void print_progress() {
    if (lr_schedule.size() == 0) {
      if (test_previous_loss_values.size() == 0)
        std::cout << "steps without apparent progress: "
                  << steps_without_progress;
      else
        std::cout << "steps without apparent progress: train="
                  << steps_without_progress
                  << ", test=" << test_steps_without_progress;
    } else {
      std::ostringstream sout;
  ...
  void print_periodic_verbose_status() {
    if (verbose) {
      ...
      if (now_time - last_time > seconds(40)) {
        last_time = now_time;
        std::cout << "step#: "
                  << rpad(cast_to_string(train_one_step_calls),
                          epoch_string_pad)
                  << "  "
                  << "learning rate: "
                  << rpad(cast_to_string(learning_rate), lr_string_pad) << "  ";
        if (test_previous_loss_values.size() == 0) {
          ...
          std::cout << "train loss: "
                    ...
                    << "  ";
          std::cout << "test loss: "
                    << rpad(cast_to_string(get_average_test_loss()), string_pad)
                    << "  ";
        }
        print_progress();
        clear_average_loss();
...
std::ostream &operator<<(std::ostream &out,
  ...
  std::ostringstream sout;
  net_type temp = trainer.get_net(); // make a copy so that we can clean it
                                     // without mutating the trainer's net.
  temp.clean();
  serialize(temp, sout);
  out << "  net size: " << sout.str().size() / 1024.0 / 1024.0 << "MB" << endl;
  // Don't include the loss params in the hash since we print them on the next
  // line. They also aren't really part of the "architecture" of the network.
  out << "  net architecture hash: "
      << md5(cast_to_string(trainer.get_net().subnet())) << endl;
  out << "  loss: " << trainer.get_net().loss_details() << endl;

  out << "  synchronization file:                       "
      << trainer.get_synchronization_file() << endl;
  out << "  trainer.get_solvers()[0]:                   "
      << trainer.get_solvers()[0] << endl;
  auto sched = trainer.get_learning_rate_schedule();
  if (sched.size() != 0) {
    out << "  using explicit user-supplied learning rate schedule" << endl;
  } else {
    out << "  learning rate:                              "
        << trainer.get_learning_rate() << endl;
    out << "  learning rate shrink factor:                "
        << trainer.get_learning_rate_shrink_factor() << endl;
    out << "  min learning rate:                          "
        << trainer.get_min_learning_rate() << endl;
