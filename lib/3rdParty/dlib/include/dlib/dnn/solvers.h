...
class sgd {
public:
  explicit sgd(float weight_decay_, float momentum_ = 0.9) {
  ...
  const tensor &operator()(const float learning_rate,
                           const fc_<N, FC_HAS_BIAS> &l,
                           const tensor &params_grad) {
    update_considering_bias(learning_rate, l, params_grad,
  ...
  const tensor &operator()(const float learning_rate,
                           const con_<_num_filters, _nr, _nc, _stride_y,
                                      _stride_x, _padding_y, _padding_x> &l,
                           const tensor &params_grad) {
    update_considering_bias(learning_rate, l, params_grad,
  ...
  template <long _num_filters, long _nr, long _nc, int _stride_y, int _stride_x,
            int _padding_y, int _padding_x>
  const tensor &operator()(const float learning_rate,
                           const cont_<_num_filters, _nr, _nc, _stride_y,
                                       _stride_x, _padding_y, _padding_x> &l,
                           const tensor &params_grad) {
    update_considering_bias(learning_rate, l, params_grad,
                            params_grad.size() - l.num_filters());
  ...
  const tensor &operator()(const float learning_rate, const bn_<mode> &l,
                           const tensor &params_grad) {
    update_considering_bias(learning_rate, l, params_grad,
                            params_grad.size() / 2);
    return v;
...
private:
  template <typename layer_type>
  void update_considering_bias(const float learning_rate, const layer_type &l,
                               const tensor &params_grad,
    ...
    if (l.get_bias_learning_rate_multiplier() == 1 &&
        l.get_bias_weight_decay_multiplier() == 1) {
      tt::affine_transform(v, v, params, params_grad, momentum, -wd * lr, -lr);
    } else {

      tt::affine_transform_range(0, bias_offset, v, v, params, params_grad,
                                 momentum, -wd * lr, -lr);
      ...
      tt::affine_transform_range(bias_offset, v.size(), v, v, params,
                                 params_grad, momentum, -wd * lr, -lr);
    }
  }
...
class adam {
public:
  adam(float weight_decay_, float momentum1_, float momentum2_) {
    weight_decay = weight_decay_;
  ...
  friend std::ostream &operator<<(std::ostream &out, const adam &item) {
    out << "adam: weight_decay=" << item.get_weight_decay()
