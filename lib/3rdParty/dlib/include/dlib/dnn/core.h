...
#ifdef _MSC_VER
// Tell Visual Studio not to recursively inline functions very much because
// otherwise it takes hours to compile the DNN code sometimes.  It's crazy.
// Hopefully we can remove this some day when the visual studio compiler is more
// efficient.
...
namespace impl {
template <typename T,
          typename int_<decltype(&T::get_learning_rate_multiplier)>::type = 0>
double get_learning_rate_multiplier(const T &obj, special_) {
  return obj.get_learning_rate_multiplier();
}

template <typename T> double get_learning_rate_multiplier(const T &, general_) {
  return 1;
}
} // namespace impl
template <typename T> double get_learning_rate_multiplier(const T &obj) {
  return impl::get_learning_rate_multiplier(obj, special_());
}
...
namespace impl {
template <typename T,
          typename int_<decltype(&T::get_weight_decay_multiplier)>::type = 0>
double get_weight_decay_multiplier(const T &obj, special_) {
  return obj.get_weight_decay_multiplier();
}

template <typename T> double get_weight_decay_multiplier(const T &, general_) {
  return 1;
}
} // namespace impl
template <typename T> double get_weight_decay_multiplier(const T &obj) {
  return impl::get_weight_decay_multiplier(obj, special_());
}

// ----------------------------------------------------------------------------------------

namespace impl {
// The reason we return an int for this version rather than doing the more
// straight forward thing (like we do above) is to avoid a bug in visual studio
// 2015.
template <typename T>
auto call_clean_method_if_exists(T &obj, special_) ->
    typename int_<decltype(&T::clean)>::type {
  obj.clean();
  return 0;
}

template <typename T> void call_clean_method_if_exists(T &, general_) {}
} // namespace impl
template <typename T> void call_clean_method_if_exists(T &obj) {
  impl::call_clean_method_if_exists(obj, special_());
}
/*!
    ensures
...
class repeat_input_layer {
  /*!
      None of the declarations in this object are really used. The only reason
  it exists is to allow the repeat object to use a special input layer in its
...
public:
  ...
  void to_tensor(forward_iterator, forward_iterator, resizable_tensor &) const {
  }

  friend void serialize(const repeat_input_layer &, std::ostream &) {}
  friend void deserialize(repeat_input_layer &, std::istream &) {}
  friend std::ostream &operator<<(std::ostream &out,
                                  const repeat_input_layer &) {
    return out;
  }
};

inline std::string tensor_to_str(const tensor &t, int &min_length) {
  if (t.size() == 0)
    return "";
  ...
  while (sout.tellp() < 28)
    sout << " ";
  sout << "nr:" << t.nr() << ",";
  while (sout.tellp() < 28 + 8)
    sout << " ";
  sout << "nc:" << t.nc() << ")";
  while (sout.tellp() < min_length)
    sout << " ";
...
// Tell us if T is one of the special layer types (i.e. add_layer, repeat,
// add_tag_layer, or add_skip_layer).
...
auto tuple_subset(const Tuple &item, compile_time_integer_list<indices...>)
    -> decltype(std::make_tuple(std::get<indices>(item)...)) {
  return std::make_tuple(std::get<indices>(item)...);
}

template <typename Head, typename... Tail>
std::tuple<Tail...> basic_tuple_tail(const std::tuple<Head, Tail...> &item) {
  return tuple_subset(
      item, typename make_compile_time_integer_range<sizeof...(Tail)>::type());
}

template <typename T> std::tuple<T> tuple_flatten(const T &t) {
  return std::make_tuple(t);
}

template <typename... T>
auto tuple_flatten(const std::tuple<T...> &item) -> decltype(tuple_flatten(
    item, typename make_compile_time_integer_range<sizeof...(T)>::type())) {
  return tuple_flatten(
      item, typename make_compile_time_integer_range<sizeof...(T)>::type());
}
...
template <typename T> struct tuple_head_helper {
  typedef T type;
  static const type &get(const T &item) { return item; }
};
...
template <typename T> struct alwaysbool {
  typedef bool type;
};
// one more structure for VS 2015 UP3 support workaround
template <typename T> struct alwaysbool2 {
  typedef bool type;
};
...
constexpr auto backward_requires_forward_output(layer_type &layer, SUBNET &sub)
    ->
...
template <typename layer_type, typename SUBNET>
constexpr auto backward_requires_forward_output(layer_type &layer, SUBNET &sub)
    -> typename alwaysbool<decltype(layer.backward_inplace(
        rt(), rt(), sub.get_gradient_input(), rt()))>::type {
  return true;
}
...
constexpr auto backward_requires_forward_output(layer_type &layer, SUBNET &sub)
    -> typename alwaysbool<decltype(layer.backward_inplace(
        rt(), sub.get_gradient_input(), rt()))>::type {
  return false;
}

template <typename layer_type, typename SUBNET>
constexpr auto has_inplace_backward(layer_type &layer, SUBNET &sub) ->
    typename alwaysbool2<decltype(layer.backward(rt(), rt(), sub,
                                                 rt()))>::type {
  return false;
}
...
template <typename layer_type, typename SUBNET>
constexpr auto has_inplace_backward(layer_type &layer, SUBNET &sub) ->
    typename alwaysbool2<decltype(layer.backward_inplace(
        rt(), rt(), sub.get_gradient_input(), rt()))>::type {
  return true;
}
...
template <typename layer_type, typename SUBNET>
constexpr auto is_inplace_layer(layer_type &layer, const SUBNET &sub) ->
...
template <typename layer_type, typename SUBNET>
constexpr auto is_inplace_layer(layer_type &layer, const SUBNET &sub) ->
    typename alwaysbool<decltype(layer.forward_inplace(sub.get_output(),
                                                       rt()))>::type {
  return true;
}

template <typename layer_type, typename SUBNET>
auto call_layer_backward(layer_type &layer, const tensor &computed_output,
                         const tensor &gradient_input, SUBNET &sub,
                         tensor &params_grad)
    -> decltype(layer.backward(computed_output, gradient_input, sub,
                               params_grad)) {
  layer.backward(computed_output, gradient_input, sub, params_grad);
}

template <typename layer_type, typename SUBNET>
auto call_layer_backward(layer_type &layer, const tensor &,
                         const tensor &gradient_input, SUBNET &sub,
                         tensor &params_grad)
...
auto call_layer_backward(layer_type &layer, const tensor &computed_output,
                         const tensor &gradient_input, SUBNET &sub,
                         tensor &params_grad)
    -> decltype(layer.backward_inplace(computed_output, gradient_input,
                                       sub.get_gradient_input(), params_grad)) {
  layer.backward_inplace(computed_output, gradient_input,
                         sub.get_gradient_input(), params_grad);
}
...
auto call_layer_backward(layer_type &layer, const tensor &,
                         const tensor &gradient_input, SUBNET &sub,
                         tensor &params_grad)
    -> decltype(layer.backward_inplace(gradient_input, sub.get_gradient_input(),
                                       params_grad)) {
  layer.backward_inplace(gradient_input, sub.get_gradient_input(), params_grad);
}

template <typename layer_type, typename SUBNET>
auto call_layer_forward(layer_type &layer, const SUBNET &sub,
                        tensor & /*data_output*/
...
template <typename... T>
typename impl::tuple_head_helper<std::tuple<T...>>::type
tuple_head(const std::tuple<T...> &item) {
  return impl::tuple_head_helper<std::tuple<T...>>::get(item);
}

template <typename... T>
auto tuple_tail(const std::tuple<T...> &item)
    -> decltype(impl::basic_tuple_tail(impl::tuple_flatten(item))) {
  return impl::basic_tuple_tail(impl::tuple_flatten(item));
}

inline std::tuple<> tuple_tail(const std::tuple<> &item) { return item; }
// ----------------------------------------------------------------------------------------
...
public:
  ...
  sstack(T *data_, size_t s) : data(data_), mysize(s) {}
...
template <typename T> sstack<T> make_sstack(std::vector<T> &item) {
...
class subnet_wrapper {
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          way, layers that have had their output/gradient overwritten by
  in-place layers can only be accessed from the in-place layers that sit
  directly on top of them since those in-place layers are the only layers that
...
public:
  ...
  subnet_wrapper(T &l_, unsigned int sef)
      : l(l_), _sample_expansion_factor(sef) {}
  ...
  unsigned int sample_expansion_factor() const {
    return _sample_expansion_factor;
  }
...
template <typename T>
class subnet_wrapper<
    T, true, typename std::enable_if<is_nonloss_layer_type<T>::value>::type> {
...
public:
  ...
  subnet_wrapper(T &l_, unsigned int = 0)
      : l(l_), subnetwork(l.subnet(), l.sample_expansion_factor()) {}
  ...
  const subnet_wrapper<typename T::subnet_type, false> &subnet() const {
    return subnetwork;
  }
  subnet_wrapper<typename T::subnet_type, false> &subnet() {
    return subnetwork;
  }
  unsigned int sample_expansion_factor() const {
    return l.sample_expansion_factor();
  }
...
template <typename T>
class subnet_wrapper<
    T, false, typename std::enable_if<is_nonloss_layer_type<T>::value>::type> {
...
void serialize(const add_layer<LAYER_DETAILS, SUBNET, enabled> &item,
               std::ostream &out);
template <typename LAYER_DETAILS, typename SUBNET, typename enabled>
void deserialize(add_layer<LAYER_DETAILS, SUBNET, enabled> &item,
                 std::istream &in);
...
public:
  ...
  const static size_t num_computational_layers =
      subnet_type::num_computational_layers + 1;

  add_layer()
      : subnetwork(new subnet_type()), this_layer_setup_called(false),
  ...
  template <typename T, typename U, typename E> friend class add_layer;
  template <typename T, bool is_first, typename E>
  friend class dimpl::subnet_wrapper;
  template <unsigned long T, typename U, typename E> friend class add_tag_layer;
  ...
  add_layer(const add_layer<T, U, E> &item)
      : details(item.layer_details()),
        ...
        get_output_and_gradient_input_disabled(
            item.get_output_and_gradient_input_disabled),
        x_grad(item.x_grad), cached_output(item.cached_output) {
    if (this_layer_operates_inplace())
      subnetwork->disable_output_and_gradient_getters();
  }
  ...
  add_layer(const LAYER_DETAILS &layer_det, T &&...args)
      : details(layer_det),
        ...
        this_layer_setup_called(false), gradient_input_is_stale(true),
        get_output_and_gradient_input_disabled(false) {
    if (this_layer_operates_inplace())
      subnetwork->disable_output_and_gradient_getters();
  }

  template <typename T, typename... U> struct disable_forwarding_constr {
    const static bool value = std::is_constructible<LAYER_DETAILS, T>::value;
  };
  template <typename... T, typename... U>
  struct disable_forwarding_constr<std::tuple<T...>, U...> {
    const static bool value = disable_forwarding_constr<
  ...
  template <typename... T> struct disable_forwarding_constr<add_layer<T...>> {
    const static bool value = true;
  };

  template <typename... T,
            typename = typename std::enable_if<!disable_forwarding_constr<
  ...
  add_layer(const std::tuple<LD, U...> &layer_det, T &&...args)
      : details(tuple_head(layer_det)),
        subnetwork(
            new subnet_type(tuple_tail(layer_det), std::forward<T>(args)...)),
        this_layer_setup_called(false), gradient_input_is_stale(true),
        get_output_and_gradient_input_disabled(false) {
    if (this_layer_operates_inplace())
      subnetwork->disable_output_and_gradient_getters();
  ...
  template <typename... T, typename LD, typename... U>
  add_layer(std::tuple<>, const std::tuple<LD, U...> &layer_det, T &&...args)
      : add_layer(layer_det, args...) {}

  add_layer(std::tuple<>) : add_layer() {}

  template <typename... T>
  add_layer(std::tuple<>, LAYER_DETAILS &&layer_det, T &&...args)
      : add_layer(layer_det, args...) {}

  template <typename forward_iterator>
  void to_tensor(forward_iterator ibegin, forward_iterator iend,
                 resizable_tensor &data) const {
    subnetwork->to_tensor(ibegin, iend, data);
  }

  template <typename forward_iterator>
  const tensor &operator()(forward_iterator ibegin, forward_iterator iend) {
    to_tensor(ibegin, iend, temp_tensor);
    return forward(temp_tensor);
  }

  const tensor &operator()(const input_type &x) { return (*this)(&x, &x + 1); }
...
public:
  const tensor &get_output() const {
    if (get_output_and_gradient_input_disabled)
      throw dlib::error(
          "Accessing this layer's get_output() is disabled because an in-place "
          "layer has been stacked on top of it.");
    return private_get_output();
  }
  tensor &get_gradient_input() {
    if (get_output_and_gradient_input_disabled)
      throw dlib::error(
          "Accessing this layer's get_gradient_input() is disabled because an "
          "in-place layer has been stacked on top of it.");
    return private_get_gradient_input();
  }

  const tensor &get_final_data_gradient() const {
  ...
  void back_propagate_error(const tensor &x, const tensor &gradient_input) {
    ...
    impl::call_layer_backward(details, private_get_output(), gradient_input,
                              wsub, static_cast<tensor &>(params_grad));
  ...
  void update_parameters(sstack<solver_type> solvers, double learning_rate) {
    ...
    if (params_grad.size() != 0 && get_learning_rate_multiplier(details) != 0) {
      const tensor &step = solvers.top()(
          learning_rate, details, static_cast<const tensor &>(params_grad));
      tt::add(details.get_layer_params(), details.get_layer_params(), step);
    }
    subnetwork->update_parameters(solvers.pop(), learning_rate);
  }

  const tensor &get_parameter_gradient() const { return params_grad; }

  tensor &get_parameter_gradient() { return params_grad; }
  ...
  void print(std::ostream &out, unsigned long idx, int &min_length) const {
    out << "layer<" << idx << ">\t"
        << impl::tensor_to_str(private_get_output(), min_length)
        << layer_details() << "\n";
...
private:
  bool this_layer_operates_inplace() {
    ...
    // the layer it's on top of doesn't need its own output tensor (since
    // in-place layers overwrite that tensor)
    return impl::is_inplace_layer(details, *subnetwork) &&
           !subnetwork->this_layer_requires_forward_output();
  }
  ...
  // Note that if this_layer_operates_inplace()==true then x_grad and
  // cached_output are not used at all.  Instead, this layer uses these
  // variables from the lower layer.
...
// This version of add_layer handles the special case where the subnetwork being
// given is just an input layer object.
...
public:
  ...
  template <typename T, typename U, typename E>
  add_layer(const add_layer<T, U, E> &item)
      : input_layer(item.subnet()), details(item.layer_details()),
        this_layer_setup_called(item.this_layer_setup_called),
        gradient_input_is_stale(item.gradient_input_is_stale),
        get_output_and_gradient_input_disabled(false),
  ...
  add_layer(const LAYER_DETAILS &layer_det)
      : details(layer_det), this_layer_setup_called(false),
        gradient_input_is_stale(true),
        get_output_and_gradient_input_disabled(false),
        _sample_expansion_factor(0) {}

  add_layer(const INPUT_LAYER &il)
      : input_layer(il), this_layer_setup_called(false),
        gradient_input_is_stale(true),
        get_output_and_gradient_input_disabled(false),
        _sample_expansion_factor(0) {}

  add_layer(LAYER_DETAILS &&layer_det)
      : details(std::move(layer_det)), this_layer_setup_called(false),
        gradient_input_is_stale(true),
        get_output_and_gradient_input_disabled(false),
        _sample_expansion_factor(0) {}

  add_layer(LAYER_DETAILS layer_det, INPUT_LAYER il)
      : details(std::move(layer_det)), input_layer(std::move(il)),
        this_layer_setup_called(false), gradient_input_is_stale(true),
        get_output_and_gradient_input_disabled(false),
        _sample_expansion_factor(0) {}

  add_layer(std::tuple<>, const LAYER_DETAILS &layer_det)
      : add_layer(layer_det) {}

  add_layer(std::tuple<>, LAYER_DETAILS &&layer_det) : add_layer(layer_det) {}

  add_layer(std::tuple<>, LAYER_DETAILS layer_det, INPUT_LAYER il)
      : add_layer(layer_det, il) {}

  add_layer(const std::tuple<LAYER_DETAILS> &layer_det)
      : add_layer(tuple_head(layer_det)) {}

  add_layer(const std::tuple<LAYER_DETAILS> &layer_det, INPUT_LAYER il)
  ...
  void to_tensor(forward_iterator ibegin, forward_iterator iend,
    ...
    DLIB_CASSERT(data.num_samples() >= std::distance(ibegin, iend),
                 "The input layer can't produce fewer output tensors than "
                 "there are inputs.");
    DLIB_CASSERT(data.num_samples() % std::distance(ibegin, iend) == 0,
                 "The number of tensors produced by the input layer must be an "
                 "integer multiple of the number of input objects.");
  ...
  const tensor &forward(const tensor &x) {
    DLIB_CASSERT(sample_expansion_factor() != 0,
...
public:
  ...
  const tensor &get_final_data_gradient() const { return grad_final; }
  ...
  friend void deserialize(add_layer &item, std::istream &in) {
    ...
    if (version >= 3)
      ...
      item._sample_expansion_factor =
          1; // all layer types set this to 1 in older dlib versions, so that's
             // what we put here.
  }
...
private:
  ...
  // The following 2 objects don't logically contribute to the state of this
  // class. They are only here to prevent them from being reallocated over and
  // over in member functions.
  resizable_tensor params_grad;
  resizable_tensor temp_tensor;
};
...
template <template <typename SUBNET> class tag> struct tag_id {
...
public:
  typedef SUBNET subnet_type;
  typedef typename subnet_type::input_type input_type;
  typedef int layer_details_type; // not really used anywhere, but required by
                                  // subnet_wrapper.
  const static size_t num_layers = subnet_type::num_layers + 1;
  const static size_t num_computational_layers =
      subnet_type::num_computational_layers;
  ...
  template <typename T>
  add_tag_layer(const add_tag_layer<ID, T> &item) : subnetwork(item.subnet()) {}

  template <typename... T>
  add_tag_layer(T... args) : subnetwork(std::move(args)...) {}
  ...
  const tensor &operator()(const input_type &x) { return subnetwork(x); }

  const tensor &forward(const tensor &x) { return subnetwork.forward(x); }

  const tensor &get_output() const { return subnetwork.get_output(); }

  tensor &get_gradient_input() { return subnetwork.get_gradient_input(); }
  ...
  void print(std::ostream &out, unsigned long idx, int &min_length) const {
    out << "layer<" << idx << ">\t"
        << impl::tensor_to_str(private_get_output(), min_length) << "tag" << ID
        << "\n";
    subnet().print(out, idx + 1, min_length);
  }
...
private:
  ...
  bool this_layer_requires_forward_output() { return true; }
  ...
  void disable_output_and_gradient_getters() {
    ...
    // before an inplace layer basically means you don't want the following
    // layer to operate in place.  So the inplace layer should turn itself into
    // an out-of-place layer and not call disable_output_and_gradient_getters().
    DLIB_CASSERT(false, "This should never happen");
  }
  ...
  // This member doesn't logically contribute to the state of the object since
  // it is always empty. It's just here so we can have the
  // get_parameter_gradient() methods which have to return something.  So they
...
template <typename... T> struct decorator_repeat_group {
  decorator_repeat_group(T &&...args) : data(std::forward<T>(args)...) {}

  std::tuple<T...> data;
};
template <typename... T>
decorator_repeat_group<T...> repeat_group(T &&...args) {
...
public:
  ...
  const static size_t comp_layers_in_each_group =
      (REPEATED_LAYER<SUBNET>::num_computational_layers -
       SUBNET::num_computational_layers);
  const static size_t comp_layers_in_repeated_group =
      comp_layers_in_each_group * num;
  const static size_t num_computational_layers =
      comp_layers_in_repeated_group + SUBNET::num_computational_layers;

  const static size_t layers_in_each_group =
      (REPEATED_LAYER<SUBNET>::num_layers - SUBNET::num_layers);
  const static size_t layers_in_repeated_group = layers_in_each_group * num;
  const static size_t num_layers =
      subnet_type::num_layers + layers_in_repeated_group;

  typedef REPEATED_LAYER<impl::repeat_input_layer> repeated_layer_type;

  repeat() : details(num) {}

  size_t num_repetitions() const { return num; }

  const repeated_layer_type &get_repeated_layer(size_t i) const {
    DLIB_CASSERT(i < num_repetitions());
    return details[i];
  }

  repeated_layer_type &get_repeated_layer(size_t i) {
    DLIB_CASSERT(i < num_repetitions());
    return details[i];
  }
  ...
  template <template <typename> class T, typename U>
  repeat(const repeat<num, T, U> &item) : subnetwork(item.subnetwork) {
    for (auto &&d : item.details)
      details.emplace_back(d);
  }

  template <typename T, typename... U>
  repeat(T arg1, U... args2)
      : details(num, std::move(arg1)), subnetwork(std::move(args2)...) {}

  template <typename... T, typename... U>
  repeat(decorator_repeat_group<T...> &&arg1, U... args2)
      : details(num, arg1.data), subnetwork(std::move(args2)...) {}

  template <typename T, typename... U>
  repeat(std::tuple<>, T arg1, U... args2)
      : details(num, std::move(arg1)), subnetwork(std::move(args2)...) {}
...
private:
  tensor &private_get_output() const { return details[0].private_get_output(); }
  tensor &private_get_gradient_input() {
    return details[0].private_get_gradient_input();
  }

public:
  const tensor &get_output() const { return details[0].get_output(); }
  tensor &get_gradient_input() { return details[0].get_gradient_input(); }

  const tensor &get_parameter_gradient() const {
  ...
  void back_propagate_error(const tensor &x, const tensor &gradient_input) {
    if (details.size() > 1) {
      ...
      for (size_t i = 1; i < details.size(); ++i) {
        if (i + 1 < details.size())
          details[i].back_propagate_error(
              details[i + 1].get_output(),
              details[i - 1].get_final_data_gradient());
        else
          details[i].back_propagate_error(
              subnetwork.get_output(),
              details[i - 1].get_final_data_gradient());
      }
    } else {
    ...
    subnetwork.back_propagate_error(x,
                                    details.back().get_final_data_gradient());
  }

  template <typename solver_type>
  void update_parameters(sstack<solver_type> solvers, double learning_rate) {
    for (size_t i = 0; i < details.size(); ++i)
      details[i].update_parameters(solvers.pop(comp_layers_in_each_group * i),
                                   learning_rate);
    subnetwork.update_parameters(
        solvers.pop(comp_layers_in_each_group * details.size()), learning_rate);
  }
  ...
  friend void deserialize(repeat &item, std::istream &in) {
    ...
    if (version != 1)
      throw serialization_error(
          "Unexpected version found while deserializing dlib::repeat.");
    deserialize(item.details, in);
    deserialize(item.subnetwork, in);
  }
...
template <size_t num, template <typename> class REPEATED_LAYER, typename SUBNET>
struct is_nonloss_layer_type<repeat<num, REPEATED_LAYER, SUBNET>>
    : std::true_type {};

// ----------------------------------------------------------------------------------------

// This version of add_tag_layer handles the special case where the subnetwork
// being given is just an input layer object.
...
public:
  ...
  add_tag_layer()
      : cached_output_ptr(nullptr), gradient_input_is_stale(true),
        _sample_expansion_factor(0) {}
  ...
  add_tag_layer(const add_tag_layer<ID, T, E> &item)
      : input_layer(item.subnet()), cached_output(item.cached_output),
        cached_output_ptr(nullptr), grad_final(item.grad_final),
        gradient_input_is_stale(item.gradient_input_is_stale),
        _sample_expansion_factor(0) {}

  template <typename... T>
  add_tag_layer(T... args)
      : input_layer(std::move(args)...), cached_output_ptr(nullptr),
        gradient_input_is_stale(true), _sample_expansion_factor(0) {}

  add_tag_layer(std::tuple<>)
      : cached_output_ptr(nullptr), gradient_input_is_stale(true),
        _sample_expansion_factor(0) {}
  ...
  const tensor &forward(const tensor &x) {
    ...
    // layer then we don't want it to be creating copies of x.  This is because,
    // we can just hold a pointer to x since the way repeat is constructed
    // guarantees that x will have a lifetime larger than this pointer.
  ...
  template <typename solver_type>
  void update_parameters(sstack<solver_type> /*solvers*/,
...
template <typename LOSS_DETAILS, typename SUBNET> class add_loss_layer;

class no_label_type {
private:
  // We don't want anyone making these no_label_type objects.  They are here
  // only to allow add_loss_layer::training_label_type and
  // dnn_trainer::training_label_type to exist which avoids needing to overload
  // add_loss_layer and dnn_trainer for supervised an unsupervised losses.  It
  // also can be a type to use in template metaprogramming to indicate "no
  // label".  So here we make the constructor private with the exception that
  // add_loss_layer objects can make it (again, just to simplify
  // add_loss_layer's implementation).
  no_label_type(){};
...
template <typename LOSS_DETAILS, typename SUBNET> class add_loss_layer {
  ...
  template <typename T>
  struct get_loss_layer_training_label_type<
      T, typename std::enable_if<sizeof(typename T::training_label_type) !=
                                 0>::type> {
  ...
  template <typename T>
  struct get_loss_layer_output_label_type<
      T, typename std::enable_if<sizeof(typename T::output_label_type) !=
                                 0>::type> {
...
public:
  ...
  typedef typename subnet_type::input_type input_type;
  const static size_t num_layers = subnet_type::num_layers + 1;
  // Note that the loss layer doesn't count as an additional computational
  // layer.
  const static size_t num_computational_layers =
      subnet_type::num_computational_layers;
  typedef typename get_loss_layer_training_label_type<LOSS_DETAILS>::type
      training_label_type;
  typedef typename get_loss_layer_output_label_type<LOSS_DETAILS>::type
      output_label_type;

  static_assert(
      is_nonloss_layer_type<SUBNET>::value,
  ...
  template <typename T, typename U>
  add_loss_layer(const add_loss_layer<T, U> &item)
      : loss(item.loss_details()), subnetwork(item.subnet()) {}

  template <typename... T>
  add_loss_layer(const LOSS_DETAILS &layer_det, T &&...args)
      : loss(layer_det), subnetwork(std::forward<T>(args)...) {}

  template <typename... T>
  add_loss_layer(LOSS_DETAILS &&layer_det, T &&...args)
      : loss(std::move(layer_det)), subnetwork(std::forward<T>(args)...) {}
  ...
  template <typename... T,
            typename = typename std::enable_if<!disable_forwarding_constr<
                typename std::remove_reference<T>::type...>::value>::type>
  add_loss_layer(T &&...args) : subnetwork(std::forward<T>(args)...) {}
  ...
  void operator()(const tensor &x, output_iterator obegin) {
    subnetwork.forward(x);
    const dimpl::subnet_wrapper<subnet_type> wsub(subnetwork);
  ...
  void operator()(forward_iterator ibegin, forward_iterator iend,
  ...
  std::vector<output_label_type> process_batch(const iterable_type &data,
                                               size_t batch_size, T &&...args) {
    std::vector<output_label_type> results(
  ...
  std::vector<output_label_type> operator()(const iterable_type &data,
  ...
  double compute_loss(const tensor &x, label_iterator lbegin) {
    subnetwork.forward(x);
    dimpl::subnet_wrapper<subnet_type> wsub(subnetwork);
  ...
  double compute_loss(forward_iterator ibegin, forward_iterator iend,
  ...
  template <typename label_iterator>
  double compute_parameter_gradients(const tensor &x, label_iterator lbegin) {
    subnetwork.forward(x);
    dimpl::subnet_wrapper<subnet_type> wsub(subnetwork);
  ...
  template <typename forward_iterator, typename label_iterator>
  double compute_parameter_gradients(forward_iterator ibegin,
...
private:
  ...
  // These two objects don't logically contribute to the state of this object.
  // They are here to prevent them from being reallocated over and over.
  output_label_type temp_label;
  resizable_tensor temp_tensor;
};

template <typename LOSS_DETAILS, typename SUBNET>
void serialize(const add_loss_layer<LOSS_DETAILS, SUBNET> &item,
               std::ostream &out) {
  int version = 1;
  serialize(version, out);
...
struct layer_helper {
  static_assert(i < T::num_layers, "Call to layer() attempted to access "
                                   "non-existing layer in neural network.");
  static T &makeT();
  using next_type =
...
template <unsigned int i, size_t N, template <typename> class L, typename S>
struct layer_helper<
    i, repeat<N, L, S>,
    typename std::enable_if<(
        i != 0 && i >= repeat<N, L, S>::layers_in_repeated_group)>::type> {
  const static size_t layers_in_repeated_group =
      repeat<N, L, S>::layers_in_repeated_group;

  static repeat<N, L, S> &makeT();
  using next_type =
      typename std::remove_reference<decltype(makeT().subnet())>::type;
  using type =
      typename layer_helper<i - layers_in_repeated_group, next_type>::type;
  static type &layer(repeat<N, L, S> &n) {
    return layer_helper<i - layers_in_repeated_group, next_type>::layer(
        n.subnet());
  }
};
template <unsigned int i, size_t N, template <typename> class L, typename S>
struct layer_helper<
    ...
    typename std::enable_if<(
        i != 0 && i < repeat<N, L, S>::layers_in_repeated_group)>::type> {
  const static size_t layers_in_each_group =
      repeat<N, L, S>::layers_in_each_group;
...
struct layer_helper<0, repeat<N, L, S>, void> {
  ...
  static type &layer(repeat<N, L, S> &n) { return n.get_repeated_layer(0); }
};

template <unsigned int i, size_t N, template <typename> class L, typename S>
struct layer_helper<
    i, const repeat<N, L, S>,
    typename std::enable_if<(
        i != 0 && i >= repeat<N, L, S>::layers_in_repeated_group)>::type> {
  const static size_t layers_in_repeated_group =
      repeat<N, L, S>::layers_in_repeated_group;
  ...
  using next_type =
      const typename std::remove_reference<decltype(makeT().subnet())>::type;
  using type = const typename layer_helper<i - layers_in_repeated_group,
                                           next_type>::type;
...
struct layer_helper<
  ...
  using type =
      const typename layer_helper<i % layers_in_each_group, next_type>::type;
  static type &layer(const repeat<N, L, S> &n) {
    return layer_helper<i % layers_in_each_group, next_type>::layer(
        n.get_repeated_layer(i / layers_in_each_group));
  }
...
template <typename T> struct layer_helper<0, T, void> {
  using type = T;
  static type &layer(T &n) { return n; }
};
...
struct layer_helper_match<
    ...
    typename std::enable_if<std::is_same<
        const T, const Match<typename T::subnet_type>>::value>::type> {
  using type = typename layer_helper<i, T>::type;
  static type &layer(T &n) { return layer_helper<i, T>::layer(n); }
};
...
struct layer_helper_match<
    Match, T, i,
    typename std::enable_if<std::is_same<
        const T, const Match<typename T::input_type>>::value>::type> {
  using type = typename layer_helper<i, T>::type;
  static type &layer(T &n) { return layer_helper<i, T>::layer(n); }
};
...
namespace dimpl {
template <typename T> T &get_input_details(T &net) { return net; }

template <typename T, bool is_first, typename enabled>
auto get_input_details(dimpl::subnet_wrapper<T, is_first, enabled> &net)
    -> decltype(net.layer_details()) & {
  return net.layer_details();
}

template <typename T, bool is_first, typename enabled>
auto get_input_details(const dimpl::subnet_wrapper<T, is_first, enabled> &net)
    -> decltype(net.layer_details()) & {
  return net.layer_details();
}
} // namespace dimpl
...
auto input_layer(net_type &net) -> decltype(dimpl::get_input_details(
    layer<net_type::num_layers - 1>(net))) & {
  // Calling input_layer() on a subnet_wrapper is a little funny since the
  // behavior of .subnet() returns another subnet_wrapper rather than an input
  // details object as it does in add_layer.
...
public:
  ...
  template <typename T>
  add_skip_layer(const add_skip_layer<TAG_TYPE, T> &item)
      : subnetwork(item.subnet()) {}

  template <typename... T>
  add_skip_layer(T... args) : subnetwork(std::move(args)...) {}
  ...
  void print(std::ostream &out, unsigned long idx, int &min_length) const {
    out << "layer<" << idx << ">\t"
        << impl::tensor_to_str(private_get_output(), min_length) << "skip" << id
        << "\n";
    subnet().print(out, idx + 1, min_length);
  }
...
namespace timpl {
inline void fill_with_gassuan_random_numbers(tensor &t, dlib::rand &rnd,
                                             double sigma = 1) {
  float *data = t.host();
...
public:
  test_layer_subnet(dlib::rand &rnd_) : rnd(rnd_) {
...
inline std::ostream &operator<<(std::ostream &out,
                                const layer_test_results &item) {
  out << item.log;
  return out;
}

template <typename layer_details_type>
layer_test_results impl_test_layer(layer_details_type l, const float base_eps) {
  ...
  for (int iter = 0; iter < 10; ++iter) {
    ...
    // happened before we start assuming we know how many data elements there
    // are (since we do a lazy layer creation thing based on calls to subnet()
    // inside test_layer_subnet).
    l.setup(subnetwork);
    ...
    // The f() we are computing gradients of is this thing.  It's value at the
    // current parameter and data values is:
    // sout << "f(data,params): " << dot(output, input_grad) << std::endl;

    // We are going to save a copy of the subnetwork.get_gradient_input() data
    // before we do backpropagation since the backward() function is supposed to
    // *add* to the gradients rather than overwrite them.  We will use this
    // saved data to check if that is the case.
    ...
    // Now tell the layer to compute all the gradients.  In the rest of this
    // function we will just be checking that these gradients were computed
    // correctly by comparing them to a central differences approximation.
    resizable_tensor params_grad;
    params_grad.copy_size(l.get_layer_params());
    // But first, set the params grad to something crazy so that it's very
    // obvious if it doesn't get fully assigned.
    params_grad = std::numeric_limits<float>::infinity();
    impl::call_layer_backward(l, output, input_grad, subnetwork, params_grad);

    static_assert(impl::is_inplace_layer(l, subnetwork) ==
                      impl::has_inplace_backward(l, subnetwork),
                  "Layer not defined correctly.  forward and backward methods "
                  "must either both be in-place or both out-of-place. ");

    // Make sure the outputs of forward() and backward() are the same when they
    // are run in in-place mode.
    if (impl::is_inplace_layer(l, subnetwork)) {
      ...
      impl::call_layer_forward(ll, subnetwork2,
                               subnetwork2.get_mutable_output());
      const auto forward_error =
          max(abs(mat(ip_out) - mat(subnetwork2.get_output())));
      if (forward_error > 0.00001) {
        using namespace std;
        sout << "This layer is supposed to support in-place computations but "
                "the output of forward_inplace()\n";
        sout
      ...
      // Now call backward() and make sure it works as well.  Recall that when
      // an in-place layer works in-place it assigns to it's outputs but when
      // it's not running in-place it adds.  So we initialize to a non-zero
      // value to check that this is the behavior that really executes.
      subnetwork2.get_gradient_input() = 9;
      impl::call_layer_backward(ll, ip_out, input_grad, subnetwork2,
                                params_grad1);
      ...
      impl::call_layer_backward(ll, ip_out, subnetwork2.get_gradient_input(),
                                subnetwork2, params_grad2);
      data_grad2 = subnetwork2.get_gradient_input();
      if (params_grad.size() != 0) {
        const auto backward_param_error =
            max(abs(mat(params_grad1) - mat(params_grad2)));
        if (backward_param_error > 0.00001) {
          using namespace std;
          sout << "This layer is supposed to support in-place computations but "
                  "the output of backward_inplace()\n";
          sout << "changes when invoked in-place vs. out-of-place. The error "
                  "was: "
               << backward_param_error << endl;
          return layer_test_results(sout.str());
        }
      }
      const auto backward_data_error =
          max(abs(mat(data_grad1) - 9 - mat(data_grad2)));
    ...
    for (unsigned long i = 0; i < params_grad.size(); ++i) {
      ...
      // Compute a reference derivative via a central differences approximation
      // and compare it to the one output by the layer and make sure they match.
      double reference_derivative =
          (dot(out2, input_grad) - dot(out3, input_grad)) / (2 * eps);
      double output_derivative = params_grad.host()[i];
      double relative_error;
      if (reference_derivative * output_derivative != 0)
        relative_error =
            (reference_derivative - output_derivative) / (reference_derivative);
      else
      ...
      if (std::abs(relative_error) > 0.05 && std::abs(absolute_error) > 0.006) {
        using namespace std;
        sout << "Gradient error in parameter #" << i
    ...
    for (unsigned long i = 0; i < num_data_inputs; ++i) {
      ...
      if (std::abs(relative_error) > 0.05 && std::abs(absolute_error) > 0.006) {
        using namespace std;
        sout << "Gradient error in data variable #" << i
             << ".  Relative error: " << relative_error << endl;
        sout << "expected derivative: " << reference_derivative << endl;
        sout << "output derivative:   " << output_derivative << endl;
        sout << "iteration:           " << iter << endl;
  ...
  if (rs_params.mean() > 0.003) {
    using namespace std;
    sout << "Average parameter gradient error is somewhat large at: "
         << rs_params.mean() << endl;
    return layer_test_results(sout.str());
  }
  if (rs_data.mean() > 0.003) {
    using namespace std;
    sout << "Average data gradient error is somewhat large at: "
         << rs_data.mean() << endl;
    return layer_test_results(sout.str());
  }
...
template <typename layer_details_type>
layer_test_results test_layer(layer_details_type l) {
  ...
  // However, if none of the step sizes worked then try this one and probably
  // result in returning an error.
...
namespace impl {
template <size_t i, size_t num> struct vlp_loop {
  template <typename T, typename U>
  static typename std::enable_if<!is_add_layer<U>::value>::type
  invoke_functor(T &&, size_t &, U &&) {
    // intentionally left empty
  }

  template <typename T, typename U>
  static typename std::enable_if<is_add_layer<U>::value>::type
  invoke_functor(T &&v, size_t &comp_i, U &&l) {
    v(comp_i, l.layer_details().get_layer_params());
    ++comp_i;
  }

  template <typename net_type, typename visitor>
  static void visit(size_t comp_i, net_type &net, visitor &&v) {
    invoke_functor(v, comp_i, layer<i>(net));
    vlp_loop<i + 1, num>::visit(comp_i, net, v);
  }
};

template <size_t num> struct vlp_loop<num, num> {
  template <typename net_type, typename visitor>
  static void visit(size_t, net_type &, visitor &&) {
...
template <typename net_type, typename visitor>
void visit_layer_parameters(net_type &net, visitor v) {
  size_t comp_i = 0;
...
namespace impl {
template <size_t i, size_t num> struct vlpg_loop {
  template <typename T, typename U>
  static typename std::enable_if<!is_add_layer<U>::value>::type
  invoke_functor(T &&, size_t &, U &&) {
    // intentionally left empty
...
template <size_t num> struct vlpg_loop<num, num> {
  template <typename net_type, typename visitor>
  static void visit(size_t, net_type &, visitor &&) {
    // Base case of recursion.  Don't do anything.
  }
...
template <typename net_type, typename visitor>
void visit_layer_parameter_gradients(net_type &net, visitor v) {
  size_t comp_i = 0;
...
namespace impl {
template <size_t i, size_t num> struct vl_loop {
  template <typename net_type, typename visitor>
  static void visit(net_type &net, visitor &&v) {
    v(i, layer<i>(net));
    vl_loop<i + 1, num>::visit(net, v);
  }
};

template <size_t num> struct vl_loop<num, num> {
  template <typename net_type, typename visitor>
  static void visit(net_type &, visitor &&) {
    // Base case of recursion.  Don't do anything.
  }
};

template <size_t i, size_t num> struct vl_loop_backwards {
  template <typename net_type, typename visitor>
  static void visit(net_type &net, visitor &&v) {
...
template <size_t num> struct vl_loop_backwards<num, num> {
  template <typename net_type, typename visitor>
  static void visit(net_type &, visitor &&) {
    // Base case of recursion.  Don't do anything.
  }
...
template <typename net_type, typename visitor>
void visit_layers(net_type &net, visitor v) {
  impl::vl_loop<0, net_type::num_layers>::visit(net, v);
}

template <typename net_type, typename visitor>
void visit_layers_backwards(net_type &net, visitor v) {
  impl::vl_loop_backwards<0, net_type::num_layers>::visit(net, v);
}

template <size_t begin, size_t end, typename net_type, typename visitor>
void visit_layers_range(net_type &net, visitor v) {
...
template <size_t begin, size_t end, typename net_type, typename visitor>
void visit_layers_backwards_range(net_type &net, visitor v) {
  static_assert(begin <= end, "Invalid range");
  static_assert(end <= net_type::num_layers, "Invalid range");
...
namespace impl {
template <size_t i, unsigned long tag_id> struct vl_until_tag {
  template <typename net_type, typename next_net_type, typename visitor>
  static void visit(net_type &net, next_net_type &next_net, visitor &&v) {
    v(next_net);
    vl_until_tag<i + 1, tag_id>::visit(net, layer<i + 1>(net), v);
  }

  template <typename net_type, typename SUBNET, typename visitor>
  static void visit(net_type &net,
  ...
  template <typename net_type, typename SUBNET, typename visitor>
  static void visit(net_type &net, add_tag_layer<tag_id, SUBNET> &next_net,
                    visitor &&v) {
    v(next_net);
  }
};
...
template <unsigned long tag_id, typename net_type, typename visitor>
void visit_layers_until_tag(net_type &net, visitor v) {
