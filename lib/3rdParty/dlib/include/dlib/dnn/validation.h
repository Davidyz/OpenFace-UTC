...
namespace dlib {
namespace impl {
inline std::set<std::string> get_labels(const std::vector<mmod_rect> &rects1,
...
const matrix<double, 1, 3> test_object_detection_function(
  ...
  for (unsigned long i = 0; i < images.size(); ++i) {
    ...
    detector.loss_details().to_label(temp, detector.subnet(), &hits,
                                     adjust_threshold);
    ...
    for (auto &label : impl::get_labels(truth_dets[i], hits)) {
      ...
      correct_hits += impl::number_of_truth_hits(
          truth_boxes, ignore, boxes, overlap_tester, all_dets,
