...
public:
  ...
  double compute_loss_value_and_gradient(const tensor &input_tensor,
    ...
    DLIB_CASSERT(input_tensor.num_samples() % sub.sample_expansion_factor() ==
                 0);
  ...
  friend void deserialize(loss_binary_hinge_ &, std::istream &in) {
    ...
    if (version != "loss_binary_hinge_")
      throw serialization_error("Unexpected version found while deserializing "
                                "dlib::loss_binary_hinge_.");
  }

  friend std::ostream &operator<<(std::ostream &out,
                                  const loss_binary_hinge_ &) {
...
public:
  ...
  double compute_loss_value_and_gradient(const tensor &input_tensor,
    ...
    DLIB_CASSERT(output_tensor.nr() == 1 && output_tensor.nc() == 1 &&
                 output_tensor.k() == 1);
    DLIB_CASSERT(grad.nr() == 1 && grad.nc() == 1 && grad.k() == 1);
...
template <typename T> T safe_log(T input, T epsilon = 1e-10) {
  // Prevent trying to calculate the logarithm of a very small number (let alone
  // zero)
...
public:
  ...
  double compute_loss_value_and_gradient(const tensor &input_tensor,
    ...
    for (long i = 0; i < output_tensor.num_samples(); ++i) {
      ...
      // The network must produce a number of outputs that is equal to the
      // number of labels when using this type of loss.
      DLIB_CASSERT(y < output_tensor.k(),
  ...
  friend void deserialize(loss_multiclass_log_ &, std::istream &in) {
    ...
    if (version != "loss_multiclass_log_")
      throw serialization_error("Unexpected version found while deserializing "
                                "dlib::loss_multiclass_log_.");
  }

  friend std::ostream &operator<<(std::ostream &out,
                                  const loss_multiclass_log_ &) {
...
public:
  ...
  loss_multimulticlass_log_(
    ...
    for (auto &l : labels) {
      ...
      DLIB_CASSERT(l.second.size() >= 2,
                   "Each classifier must have at least two possible labels.");
  ...
  private:
    ...
    classifier_output(const matrix_exp<EXP> &class_probs,
                      const std::shared_ptr<std::vector<std::string>> &_labels)
        : class_probs(class_probs), _labels(_labels) {}
  ...
  template <typename SUB_TYPE, typename label_iterator>
  void to_label(const tensor &input_tensor, const SUB_TYPE &sub,
                label_iterator iter_begin) const {
    const tensor &output_tensor = sub.get_output();
    DLIB_CASSERT(sub.sample_expansion_factor() == 1);
    DLIB_CASSERT(output_tensor.nr() == 1 && output_tensor.nc() == 1);
    DLIB_CASSERT(input_tensor.num_samples() == output_tensor.num_samples());
    ...
    DLIB_CASSERT(number_of_labels() != 0,
                 "You must give the loss_multimulticlass_log_'s constructor "
                 "label data before you can use it!");
    DLIB_CASSERT(output_tensor.k() == (long)number_of_labels(),
                 "The output tensor must have " << number_of_labels()
                                                << " channels.");
    ...
    for (auto &l : possible_labels) {
      ...
      tt::copy_tensor(false, scratch, 0, output_tensor, k_offset,
                      labels.size());

      tt::softmax(scratch, scratch);

      for (long i = 0; i < scratch.num_samples(); ++i)
        (*iter++)[classifier_name] =
            classifier_output(rowm(mat(scratch), i), l.second);
  ...
  friend void serialize(const loss_multimulticlass_log_ &item,
                        std::ostream &out) {
  ...
  friend void deserialize(loss_multimulticlass_log_ &item, std::istream &in) {
    ...
    if (version != "loss_multimulticlass_log_")
      throw serialization_error("Unexpected version found while deserializing "
                                "dlib::loss_multimulticlass_log_.");
  ...
  friend std::ostream &operator<<(std::ostream &out,
                                  const loss_multimulticlass_log_ &item) {
    out << "loss_multimulticlass_log, labels={";
    for (auto i = item.possible_labels.begin();
         i != item.possible_labels.end();) {
...
private:
  ...
  // We make it true that:
  // possible_labels[classifier][label_idx_lookup[classifier][label]] == label
  std::map<std::string, std::map<std::string, long>> label_idx_lookup;

  // Scratch doesn't logically contribute to the state of this object.  It's
  // just temporary scratch space used by this class.
...
public:
    ...
    friend inline void deserialize(detector_window_details &item,
      ...
      if (version != 1 && version != 2)
        throw serialization_error(
            "Unexpected version found while deserializing "
            "dlib::mmod_options::detector_window_details");
  ...
  mmod_options(
      ...
      const unsigned long
          target_size, // We want the length of the longest dimension of the
                       // detector window to be this.
      const unsigned long
          min_target_size, // But we require that the smallest dimension of the
                           // detector window be at least this big.
      const double min_detector_window_overlap_iou = 0.75) {
    DLIB_CASSERT(0 < min_target_size && min_target_size <= target_size);
    DLIB_CASSERT(0.5 < min_detector_window_overlap_iou &&
                 min_detector_window_overlap_iou < 1);

    // Figure out what detector windows we will need.
    for (auto &label : get_labels(boxes)) {
      for (auto ratio : find_covering_aspect_ratios(
               boxes, test_box_overlap(min_detector_window_overlap_iou),
               label)) {
        ...
        detector_window_details p((unsigned long)std::round(detector_width),
                                  (unsigned long)std::round(detector_height),
                                  label);
    ...
    DLIB_CASSERT(detector_windows.size() != 0,
                 "You can't call mmod_options's constructor with a set of "
                 "boxes that is empty (or only contains ignored boxes).");
  ...
  mmod_options(use_image_pyramid assume_image_pyramid,
    ...
    // Figure out what detector windows we will need.
    for (auto &label : get_labels(boxes)) {
      for (auto rectangle : find_covering_rectangles(
               boxes, test_box_overlap(min_detector_window_overlap_iou),
               label)) {
        detector_windows.push_back(detector_window_details(
            rectangle.width(), rectangle.height(), label));
      }
    }
...
private:
  void set_overlap_nms(const std::vector<std::vector<mmod_rect>> &boxes) {
    ...
    // Relax the non-max-suppression a little so that it doesn't accidentally
    // make it impossible for the detector to output boxes matching the training
    // data. This could be a problem with the tightest possible nms test since
    // there is some small variability in how boxes get positioned between the
    // training data and the coordinate system used by the detector when it
    // runs.  So relaxing it here takes care of that.
    auto iou_thresh = advance_toward_1(overlaps_nms.get_iou_thresh());
    auto percent_covered_thresh =
        advance_toward_1(overlaps_nms.get_percent_covered_thresh());
    overlaps_nms = test_box_overlap(iou_thresh, percent_covered_thresh);
  }

  static double advance_toward_1(double val) {
    if (val < 1)
      val += (1 - val) * 0.1;
    return val;
  }

  static size_t count_overlaps(const std::vector<rectangle> &rects,
  ...
  find_rectangles_overlapping_all_others(std::vector<rectangle> rects,
                                         const test_box_overlap &overlaps) {
    std::vector<rectangle> exemplars;
    dlib::rand rnd;

    while (rects.size() > 0) {
      // Pick boxes at random and see if they overlap a lot of other boxes.  We
      // will try 500 different boxes each iteration and select whichever hits
      // the most others to add to our exemplar set.
  ...
  get_labels(const std::vector<std::vector<mmod_rect>> &rects) {
  ...
  find_covering_aspect_ratios(const std::vector<std::vector<mmod_rect>> &rects,
                              const test_box_overlap &overlaps,
                              const std::string &label) {
    std::vector<rectangle> boxes;
    // Make sure all the boxes have the same size and position, so that the only
    // thing our checks for overlap will care about is aspect ratio (i.e. scale
    // and x,y position are ignored).
    for (auto &bb : rects) {
      for (auto &&b : bb) {
        if (!b.ignore && b.label == label)
          boxes.push_back(
              move_rect(set_rect_area(b.rect, 400 * 400), point(0, 0)));
      }
    }
  ...
  static std::vector<dlib::rectangle>
  find_covering_rectangles(const std::vector<std::vector<mmod_rect>> &rects,
                           const test_box_overlap &overlaps,
                           const std::string &label) {
    std::vector<rectangle> boxes;
    // Make sure all the boxes have the same position, so that the we only check
...
inline void deserialize(mmod_options &item, std::istream &in) {
  ...
  if (version == 1) {
    ...
    item.detector_windows = {
        mmod_options::detector_window_details(width, height)};
  } else {
  ...
  if (version >= 3) {
    ...
    item.assume_image_pyramid =
        static_cast<use_image_pyramid>(assume_image_pyramid);
  }
}
...
class loss_mmod_ {
  struct intermediate_detection {
    ...
    intermediate_detection(rectangle rect_) : rect(rect_) {}

    intermediate_detection(rectangle rect_, double detection_confidence_,
                           size_t tensor_offset_, long channel)
        : rect(rect_), detection_confidence(detection_confidence_),
          tensor_offset(tensor_offset_), tensor_channel(channel) {}
...
public:
  ...
  const mmod_options &get_options() const { return options; }
  ...
  void to_label(const tensor &input_tensor, const SUB_TYPE &sub,
    ...
    for (long i = 0; i < output_tensor.num_samples(); ++i) {
      tensor_to_dets(input_tensor, output_tensor, i, dets_accum,
                     adjust_threshold, sub);
      ...
      for (unsigned long i = 0; i < dets_accum.size(); ++i) {
        ...
        final_dets.push_back(mmod_rect(
            dets_accum[i].rect, dets_accum[i].detection_confidence,
  ...
  double compute_loss_value_and_gradient(const tensor &input_tensor,
    ...
    for (long i = 0; i < output_tensor.num_samples(); ++i) {
      tensor_to_dets(input_tensor, output_tensor, i, dets,
                     -options.loss_per_false_alarm + det_thresh_speed_adjust,
                     sub);
      ...
      if (dets.size() >= max_num_initial_dets) {
        det_thresh_speed_adjust =
            std::max(det_thresh_speed_adjust,
                     dets[max_num_initial_dets].detection_confidence +
                         options.loss_per_false_alarm);
      }

      // The loss will measure the number of incorrect detections.  A detection
      // is incorrect if it doesn't hit a truth rectangle or if it is a
      // duplicate detection on a truth rectangle.
      ...
      for (auto &&x : *truth) {
        if (!x.ignore) {
          ...
          const size_t idx =
              (k * output_tensor.nr() + p.y()) * output_tensor.nc() + p.x();
          loss -= out_data[idx];
          // compute gradient
          g[idx] = -scale;
      ...
      // Measure the loss augmented score for the detections which hit a truth
      // rect.
      ...
      for (unsigned long i = 0;
           i < dets.size() && final_dets.size() < max_num_dets; ++i) {
        if (overlaps_any_box_nms(final_dets, dets[i].rect))
          continue;

        const auto &det_label =
            options.detector_windows[dets[i].tensor_channel].label;

        const std::pair<double, unsigned int> hittruth =
            find_best_match(*truth, dets[i].rect, det_label);
        ...
        if (truth_match > options.truth_match_iou_threshold) {
          // if this is the first time we have seen a detect which hit
          // (*truth)[hittruth.second]
          ...
          if (hit_truth_table[hittruth.second] == false) {
            ...
            truth_score_hits[hittruth.second] +=
                score + options.loss_per_false_alarm;
          }
        }
      }
      ...
      for (size_t i = 0; i < hit_truth_table.size(); ++i) {
        if (!hit_truth_table[i] && !(*truth)[i].ignore) {
          ...
          const std::pair<double, unsigned int> hittruth =
              find_best_match(*truth, (*truth)[i], i);
          ...
          if (options.overlaps_nms(best_matching_truth_box, (*truth)[i])) {
            ...
            g[idx] = 0;
            std::cout << "Warning, ignoring object.  We encountered a truth "
                         "rectangle located at "
                      << (*truth)[i].rect;
            std::cout << " that is suppressed by non-max-suppression ";
            std::cout << "because it is overlapped by another truth rectangle "
                         "located at "
                      << best_matching_truth_box << " (IoU:"
                      << box_intersection_over_union(best_matching_truth_box,
                                                     (*truth)[i])
                      << ", Percent covered:"
                      << box_percent_covered(best_matching_truth_box,
                                             (*truth)[i])
                      << ")." << std::endl;
          }
        }
      ...
      // Now figure out which detections jointly maximize the loss and detection
      // score sum.  We need to take into account the fact that allowing a true
      // detection in the output, while initially reducing the loss, may allow
      // us to increase the loss later with many duplicate detections.
      for (unsigned long i = 0;
           i < dets.size() && final_dets.size() < max_num_dets; ++i) {
        if (overlaps_any_box_nms(final_dets, dets[i].rect))
          continue;
        ...
        const double truth_match = hittruth.first;
        if (truth_match > options.truth_match_iou_threshold) {
          if (truth_score_hits[hittruth.second] >
              options.loss_per_missed_target) {
    ...
    // Here we scale the loss so that it's roughly equal to the number of
    // mistakes in an image.  Note that this scaling is different than the
    // scaling we applied to the gradient but it doesn't matter since the loss
    // value isn't used to update parameters.  It's used only for display and to
    // check if we have converged.  So it doesn't matter that they are scaled
    // differently and this way the loss that is displayed is readily
  ...
  friend std::ostream &operator<<(std::ostream &out, const loss_mmod_ &item) {
    ...
    for (size_t i = 0; i < opts.detector_windows.size(); ++i) {
      out << opts.detector_windows[i].width << "x"
          << opts.detector_windows[i].height;
...
private:
  ...
  void tensor_to_dets(const tensor &input_tensor, const tensor &output_tensor,
                      long i, std::vector<intermediate_detection> &dets_accum,
                      double adjust_threshold, const net_type &net) const {
    DLIB_CASSERT(net.sample_expansion_factor() == 1,
                 net.sample_expansion_factor());
    DLIB_CASSERT(output_tensor.k() == (long)options.detector_windows.size());
    const float *out_data = output_tensor.host() + output_tensor.k() *
                                                       output_tensor.nr() *
                                                       output_tensor.nc() * i;
    ...
    for (long k = 0; k < output_tensor.k(); ++k) {
      for (long r = 0; r < output_tensor.nr(); ++r) {
        for (long c = 0; c < output_tensor.nc(); ++c) {
          double score =
              out_data[(k * output_tensor.nr() + r) * output_tensor.nc() + c];
          if (score > adjust_threshold) {
            dpoint p = output_tensor_to_input_tensor(net, point(c, r));
            drectangle rect =
                centered_drect(p, options.detector_windows[k].width,
                               options.detector_windows[k].height);
            rect = input_layer(net).tensor_space_to_image_space(input_tensor,
                                                                rect);

            dets_accum.push_back(intermediate_detection(
                rect, score,
                (k * output_tensor.nr() + r) * output_tensor.nc() + c, k));
          }
        }
      }
    }
  ...
  size_t
  find_best_detection_window(rectangle rect, const std::string &label,
                             use_image_pyramid assume_image_pyramid) const {
    ...
    // Figure out which detection window in options.detector_windows is most
    // similar to rect (in terms of aspect ratio, if assume_image_pyramid ==
    // use_image_pyramid::yes).
    size_t best_i = 0;
    ...
    for (size_t i = 0; i < options.detector_windows.size(); ++i) {
      ...
      if (options.assume_image_pyramid == use_image_pyramid::yes) {
        det_window =
            centered_rect(point(0, 0), options.detector_windows[i].width,
                          options.detector_windows[i].height);
        det_window =
            move_rect(set_rect_area(det_window, 400 * 400), point(0, 0));
      } else {
        det_window = rectangle(options.detector_windows[i].width,
                               options.detector_windows[i].height);
      }
  ...
  template <typename net_type>
  bool image_rect_to_feat_coord(point &tensor_p, const tensor &input_tensor,
                                const rectangle &rect, const std::string &label,
                                const net_type &net, size_t &det_idx,
                                use_image_pyramid assume_image_pyramid) const {
    using namespace std;
    if (!input_layer(net).image_contained_point(input_tensor, center(rect))) {
      std::ostringstream sout;
      sout << "Encountered a truth rectangle located at " << rect
           << " that is outside the image." << endl;
      sout << "The center of each truth rectangle must be within the image."
           << endl;
    ...
    double scale = 1.0;
    if (options.assume_image_pyramid == use_image_pyramid::yes) {
      // Compute the scale we need to be at to get from rect to our detection
      // window. Note that we compute the scale as the max of two numbers.  It
      // doesn't actually matter which one we pick, because if they are very
      // different then it means the box can't be matched by the sliding window.
      // But picking the max causes the right error message to be selected in
      // the logic below.
      scale = std::max(
          options.detector_windows[det_idx].width / (double)rect.width(),
    ...
    const rectangle mapped_rect = input_layer(net).image_space_to_tensor_space(
        input_tensor, std::min(1.0, scale), rect);

    // compute the detection window that we would use at this position.
    tensor_p = center(mapped_rect);
    rectangle det_window =
        centered_rect(tensor_p, options.detector_windows[det_idx].width,
                      options.detector_windows[det_idx].height);
    det_window =
        input_layer(net).tensor_space_to_image_space(input_tensor, det_window);

    // make sure the rect can actually be represented by the image pyramid we
    // are using.
    if (box_intersection_over_union(rect, det_window) <=
        options.truth_match_iou_threshold) {
      std::cout << "Warning, ignoring object.  We encountered a truth "
                   "rectangle with a width and height of "
                << rect.width() << " and " << rect.height() << ".  ";
      std::cout << "The image pyramid and sliding windows can't output a "
                   "rectangle of this shape.  ";
      const double detector_area = options.detector_windows[det_idx].width *
                                   options.detector_windows[det_idx].height;
      if (mapped_rect.area() / detector_area <=
          options.truth_match_iou_threshold) {
        std::cout << "This is because the rectangle is smaller than the best "
                     "matching detection window, which has a width ";
        std::cout << "and height of " << options.detector_windows[det_idx].width
                  << " and " << options.detector_windows[det_idx].height << "."
                  << std::endl;
      } else {
        std::cout << "This is either because (1) the final layer's features "
                     "have too large of a stride across the image, limiting "
                     "the possible locations the sliding window can search ";
        std::cout << "or (2) because the rectangle's aspect ratio is too "
                     "different from the best matching detection window, ";
        std::cout << "which has a width and height of "
                  << options.detector_windows[det_idx].width << " and "
                  << options.detector_windows[det_idx].height << "."
                  << std::endl;
      }
      return true;
    ...
    if (!get_rect(output_tensor).contains(tensor_p)) {
      std::cout << "Warning, ignoring object.  We encountered a truth "
                   "rectangle located at "
                << rect << " that is too close to the edge ";
      std::cout << "of the image to be captured by the CNN features."
                << std::endl;
      return true;
    }
  ...
  bool overlaps_ignore_box(const std::vector<mmod_rect> &boxes,
  ...
  find_best_match(const std::vector<mmod_rect> &boxes, const rectangle &rect,
  ...
  std::pair<double, unsigned int>
  find_best_match(const std::vector<mmod_rect> &boxes, const rectangle &rect,
                  const size_t excluded_idx) const {
    double match = 0;
    unsigned int best_idx = 0;
    for (unsigned long i = 0; i < boxes.size(); ++i) {
  ...
  template <typename T>
  inline bool overlaps_any_box_nms(const std::vector<T> &rects,
                                   const rectangle &rect) const {
    for (auto &&r : rects) {
...
public:
  ...
  loss_metric_(float margin_, float dist_thresh_)
  ...
  double compute_loss_value_and_gradient(const tensor &input_tensor,
    ...
    for (long r = 0; r < temp.num_samples(); ++r) {
      ...
      for (long c = r + 1; c < temp.num_samples(); ++c) {
        ...
        if (x_label == y_label) {
          ...
          // Figure out what distance threshold, when applied to the negative
          // pairs, causes there to be an equal number of positive and negative
          // pairs.
    ...
    DLIB_CASSERT(num_pos_samps >= 1, "Make sure each mini-batch contains both "
                                     "positive pairs and negative pairs");
    DLIB_CASSERT(num_neg_samps >= 1, "Make sure each mini-batch contains both "
                                     "positive pairs and negative pairs");

    std::sort(temp_threshs.begin(), temp_threshs.end());
    const float neg_thresh =
        std::sqrt(temp_threshs[std::min(num_pos_samps, num_neg_samps) - 1]);
    ...
    for (long r = 0; r < temp.num_samples(); ++r) {
      ...
      for (long c = 0; c < temp.num_samples(); ++c) {
        ...
        if (x_label == y_label) {
          // Things with the same label should have distances < dist_thresh
          // between them.  If not then we experience non-zero loss.
          ...
          // Things with different labels should have distances > dist_thresh
          // between them.  If not then we experience non-zero loss.
  ...
  friend void deserialize(loss_metric_ &item, std::istream &in) {
    ...
    if (version == "loss_metric_") {
      ...
      throw serialization_error("Unexpected version found while deserializing "
                                "dlib::loss_metric_.  Instead found " +
                                version);
    }
  }
  ...
  friend std::ostream &operator<<(std::ostream &out, const loss_metric_ &item) {
    out << "loss_metric (margin=" << item.margin
        << ", distance_threshold=" << item.dist_thresh << ")";
    return out;
  }

  friend void to_xml(const loss_metric_ &item, std::ostream &out) {
    out << "<loss_metric margin='" << item.margin << "' distance_threshold='"
        << item.dist_thresh << "'/>";
  }
...
public:
  ...
  double compute_loss_value_and_gradient(const tensor &input_tensor,
    ...
    count_ranking_inversions(rel_scores, nonrel_scores, rel_counts,
                             nonrel_counts);
    const unsigned long total_pairs = rel_scores.size() * nonrel_scores.size();
    DLIB_CASSERT(total_pairs > 0,
                 "You can't give a ranking mini-batch that contains only one "
                 "class.  Both classes must be represented.");
...
public:
  ...
  friend void deserialize(loss_mean_squared_ &, std::istream &in) {
    ...
    if (version != "loss_mean_squared_")
      throw serialization_error("Unexpected version found while deserializing "
                                "dlib::loss_mean_squared_.");
  }

  friend std::ostream &operator<<(std::ostream &out,
                                  const loss_mean_squared_ &) {
...
public:
  ...
  friend void serialize(const loss_epsilon_insensitive_ &item,
                        std::ostream &out) {
  ...
  friend void deserialize(loss_epsilon_insensitive_ &item, std::istream &in) {
    ...
    if (version != "loss_epsilon_insensitive_")
      throw serialization_error("Unexpected version found while deserializing "
                                "dlib::loss_epsilon_insensitive_.");
    deserialize(item.eps, in);
  }

  friend std::ostream &operator<<(std::ostream &out,
                                  const loss_epsilon_insensitive_ &item) {
...
public:
  ...
  friend void serialize(const loss_mean_squared_multioutput_ &,
                        std::ostream &out) {
  ...
  friend std::ostream &operator<<(std::ostream &out,
                                  const loss_mean_squared_multioutput_ &) {
    out << "loss_mean_squared_multioutput";
    return out;
  }

  friend void to_xml(const loss_mean_squared_multioutput_ & /*item*/,
                     std::ostream &out) {
    out << "<loss_mean_squared_multioutput/>";
  }
};
...
public:
  ...
  template <typename SUB_TYPE, typename label_iterator>
  static void to_label(const tensor &input_tensor, const SUB_TYPE &sub,
                       label_iterator iter) {
    DLIB_CASSERT(sub.sample_expansion_factor() == 1);
    ...
    DLIB_CASSERT(
        output_tensor.k() >=
        1); // Note that output_tensor.k() should match the number of labels.
    ...
    const auto find_label = [&](long sample, long r, long c) {
      ...
      for (long k = 1; k < output_tensor.k(); ++k) {
        const float value =
            out_data[tensor_index(output_tensor, sample, k, r, c)];
        if (value > max_value) {
  ...
  double compute_loss_value_and_gradient(const tensor &input_tensor,
    ...
    for (long idx = 0; idx < output_tensor.num_samples(); ++idx) {
      ...
      DLIB_CASSERT(truth_matrix_ptr->nr() == output_tensor.nr() &&
                   ...
                   "truth size = " << truth_matrix_ptr->nr() << " x "
                                   << truth_matrix_ptr->nc()
                                   << ", "
                                      "output size = "
                                   << output_tensor.nr() << " x "
    ...
    // The loss we output is the average loss over the mini-batch, and also over
    // each element of the matrix output.
    const double scale = 1.0 / (output_tensor.num_samples() *
                                output_tensor.nr() * output_tensor.nc());
    double loss = 0;
    ...
    for (long i = 0; i < output_tensor.num_samples(); ++i, ++truth) {
      for (long r = 0; r < output_tensor.nr(); ++r) {
        for (long c = 0; c < output_tensor.nc(); ++c) {
          ...
          // number of labels when using this type of loss.
          DLIB_CASSERT(
              static_cast<long>(y) < output_tensor.k() || y == label_to_ignore,
              "y: " << y << ", output_tensor.k(): " << output_tensor.k());
          for (long k = 0; k < output_tensor.k(); ++k) {
  ...
  friend void serialize(const loss_multiclass_log_per_pixel_ &,
                        std::ostream &out) {
  ...
  friend void deserialize(loss_multiclass_log_per_pixel_ &, std::istream &in) {
    ...
    if (version != "loss_multiclass_log_per_pixel_")
      throw serialization_error("Unexpected version found while deserializing "
                                "dlib::loss_multiclass_log_per_pixel_.");
  }
  ...
  friend void to_xml(const loss_multiclass_log_per_pixel_ & /*item*/,
                     std::ostream &out) {
    out << "<loss_multiclass_log_per_pixel/>";
  }

private:
  static size_t tensor_index(const tensor &t, long sample, long k, long row,
                             long column) {
    // See:
...
public:
  ...
  double compute_loss_value_and_gradient(const tensor &input_tensor,
    ...
    // The loss we output is the weighted average loss over the mini-batch, and
    // also over each element of the matrix output.
    const double scale = 1.0 / (output_tensor.num_samples() *
                                output_tensor.nr() * output_tensor.nc());
    double loss = 0;
    float *const g = grad.host();
    for (long i = 0; i < output_tensor.num_samples(); ++i, ++truth) {
      for (long r = 0; r < output_tensor.nr(); ++r) {
        for (long c = 0; c < output_tensor.nc(); ++c) {
          ...
          // number of labels when using this type of loss.
          DLIB_CASSERT(
              static_cast<long>(y) < output_tensor.k() || weight == 0.f,
              "y: " << y << ", output_tensor.k(): " << output_tensor.k());
          for (long k = 0; k < output_tensor.k(); ++k) {
            const size_t idx = tensor_index(output_tensor, i, k, r, c);
            if (k == y) {
  ...
  friend void serialize(const loss_multiclass_log_per_pixel_weighted_ &,
                        std::ostream &out) {
    serialize("loss_multiclass_log_per_pixel_weighted_", out);
  }

  friend void deserialize(loss_multiclass_log_per_pixel_weighted_ &,
                          std::istream &in) {
    std::string version;
    deserialize(version, in);
    if (version != "loss_multiclass_log_per_pixel_weighted_")
      throw serialization_error(
          "Unexpected version found while deserializing "
          "dlib::loss_multiclass_log_per_pixel_weighted_.");
  }
  ...
  friend void to_xml(const loss_multiclass_log_per_pixel_weighted_ & /*item*/,
                     std::ostream &out) {
    out << "<loss_multiclass_log_per_pixel_weighted/>";
  }
...
public:
  ...
  void to_label(const tensor &input_tensor, const SUB_TYPE &sub,
    ...
    for (long i = 0; i < output_tensor.num_samples(); ++i, ++iter) {
      ...
      for (long r = 0; r < output_tensor.nr(); ++r) {
        for (long c = 0; c < output_tensor.nc(); ++c) {
          iter->operator()(r, c) =
              out_data[tensor_index(output_tensor, i, 0, r, c)];
        }
      }
    }
  }
  ...
  friend void serialize(const loss_mean_squared_per_pixel_ &,
                        std::ostream &out) {
  ...
  friend void deserialize(loss_mean_squared_per_pixel_ &, std::istream &in) {
    ...
    if (version != "loss_mean_squared_per_pixel_")
      throw serialization_error("Unexpected version found while deserializing "
                                "dlib::loss_mean_squared_per_pixel_.");
  }

  friend std::ostream &operator<<(std::ostream &out,
                                  const loss_mean_squared_per_pixel_ &) {
    out << "loss_mean_squared_per_pixel";
    return out;
  }

  friend void to_xml(const loss_mean_squared_per_pixel_ & /*item*/,
                     std::ostream &out) {
    out << "<loss_mean_squared_per_pixel/>";
  }
...
public:
  ...
  double compute_loss_value_and_gradient(const tensor &input_tensor,
    ...
    const long network_output_dims =
        output_tensor.size() / output_tensor.num_samples();
    ...
    for (long i = 0; i < output_tensor.num_samples(); ++i) {
      DLIB_CASSERT(truth->size() == network_output_dims,
                   "The network must output a vector with the same "
                   "dimensionality as the training labels. "
