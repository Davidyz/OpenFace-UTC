...
template <typename T> class input {
  ...
  static_assert(always_false,
                "Unsupported type given to input<>.  input<> only supports "
...
public:
  ...
  input_rgb_image() : avg_red(122.782), avg_green(117.001), avg_blue(104.298) {}

  input_rgb_image(float avg_red_, float avg_green_, float avg_blue_)
      : avg_red(avg_red_), avg_green(avg_green_), avg_blue(avg_blue_) {}

  template <size_t NR, size_t NC>
  inline input_rgb_image(const input_rgb_image_sized<NR, NC> &item);
  ...
  void to_tensor(forward_iterator ibegin, forward_iterator iend,
    ...
    for (auto i = ibegin; i != iend; ++i) {
      DLIB_CASSERT(i->nr() == nr && i->nc() == nc,
                   "\t input_rgb_image::to_tensor()"
                       << "\n\t All matrices given to to_tensor() must have "
                          "the same dimensions."
                       << "\n\t nr: " << nr << "\n\t nc: " << nc
                       << "\n\t i->nr(): " << i->nr()
                       << "\n\t i->nc(): " << i->nc());
    ...
    // initialize data to the right size to contain the stuff in the iterator
    // range.
  ...
  friend std::ostream &operator<<(std::ostream &out,
    ...
    out << "input_rgb_image(" << item.avg_red << "," << item.avg_green << ","
        << item.avg_blue << ")";
    return out;
  }

  friend void to_xml(const input_rgb_image &item, std::ostream &out) {
    out << "<input_rgb_image r='" << item.avg_red << "' g='" << item.avg_green
        << "' b='" << item.avg_blue << "'/>";
  }
...
template <size_t NR, size_t NC> class input_rgb_image_sized {
public:
  ...
  input_rgb_image_sized()
      : avg_red(122.782), avg_green(117.001), avg_blue(104.298) {}

  input_rgb_image_sized(const input_rgb_image &item)
      : avg_red(item.get_avg_red()), avg_green(item.get_avg_green()),
        avg_blue(item.get_avg_blue()) {}

  input_rgb_image_sized(float avg_red_, float avg_green_, float avg_blue_)
      : avg_red(avg_red_), avg_green(avg_green_), avg_blue(avg_blue_) {}
    ...
    for (auto i = ibegin; i != iend; ++i) {
      DLIB_CASSERT(i->nr() == NR && i->nc() == NC,
                   "\t input_rgb_image_sized::to_tensor()"
                       << "\n\t All input images must have " << NR
                       << " rows and " << NC << " columns, but we got one with "
                       << i->nr() << " rows and " << i->nc() << " columns.");
    }
  ...
  friend void deserialize(input_rgb_image_sized &item, std::istream &in) {
    ...
    if (nr != NR || nc != NC) {
      ...
      sout << "Wrong image dimensions found while deserializing "
              "dlib::input_rgb_image_sized.\n";
      sout << "Expected " << NR << " rows and " << NC << " columns, but found "
  ...
  friend std::ostream &operator<<(std::ostream &out,
                                  const input_rgb_image_sized &item) {
    out << "input_rgb_image_sized(" << item.avg_red << "," << item.avg_green
        << "," << item.avg_blue << ") nr=" << NR << " nc=" << NC;
    return out;
  }

  friend void to_xml(const input_rgb_image_sized &item, std::ostream &out) {
    out << "<input_rgb_image_sized r='" << item.avg_red << "' g='"
        << item.avg_green << "' b='" << item.avg_blue << "' nr='" << NR
        << "' nc='" << NC << "'/>";
  }
...
template <size_t NR, size_t NC>
input_rgb_image::input_rgb_image(const input_rgb_image_sized<NR, NC> &item)
    : avg_red(item.get_avg_red()), avg_green(item.get_avg_green()),
      avg_blue(item.get_avg_blue()) {}
...
public:
  ...
  template <typename mm> input(const input<array2d<T, mm>> &) {}
    ...
    for (auto i = ibegin; i != iend; ++i) {
      for (size_t k = 0; k < K; ++k) {
        ...
        DLIB_CASSERT(arr[k].nr() == nr && arr[k].nc() == nc,
                     "\t input::to_tensor()"
                         << "\n\t When using std::array<matrix> as input, all "
                            "matrices in a batch must have the same dimensions."
                         << "\n\t nr: " << nr << "\n\t nc: " << nc
                         << "\n\t k:  " << k << "\n\t arr[k].nr(): "
                         << arr[k].nr() << "\n\t arr[k].nc(): " << arr[k].nc());
      }
...
template <typename T, typename MM> class input<array2d<T, MM>> {
public:
  ...
  void to_tensor(forward_iterator ibegin, forward_iterator iend,
    ...
    for (auto i = ibegin; i != iend; ++i) {
      DLIB_CASSERT(i->nr() == nr && i->nc() == nc,
                   "\t input::to_tensor()"
                       << "\n\t All array2d objects given to to_tensor() must "
...
public:
  ...
  input_rgb_image_pyramid(float avg_red_, float avg_green_, float avg_blue_)
      : avg_red(avg_red_), avg_green(avg_green_), avg_blue(avg_blue_) {}
  ...
  unsigned long get_pyramid_outer_padding() const {
    return pyramid_outer_padding;
  }
  void set_pyramid_outer_padding(unsigned long value) {
    pyramid_outer_padding = value;
  }
  ...
  void to_tensor(forward_iterator ibegin, forward_iterator iend,
    ...
    impl::compute_tiled_image_pyramid_details(
        pyr, nr, nc, pyramid_padding, pyramid_outer_padding, rects, NR, NC);
    ...
    // copy the first raw image into the top part of the tiled pyramid.  We need
    // to do this for each of the input images/samples in the tensor.
    for (auto i = ibegin; i != iend; ++i) {
      auto &img = *i;
    ...
    for (size_t i = 1; i < rects.size(); ++i) {
      alias_tensor src(data.num_samples(), data.k(), rects[i - 1].height(),
                       rects[i - 1].width());
      alias_tensor dest(data.num_samples(), data.k(), rects[i].height(),
                        rects[i].width());

      auto asrc =
          src(data, data.nc() * rects[i - 1].top() + rects[i - 1].left());
      auto adest = dest(data, data.nc() * rects[i].top() + rects[i].left());

      tt::resize_bilinear(adest, data.nc(), data.nr() * data.nc(), asrc,
                          data.nc(), data.nr() * data.nc());
    }
  }

  friend void serialize(const input_rgb_image_pyramid &item,
                        std::ostream &out) {
  ...
  friend void deserialize(input_rgb_image_pyramid &item, std::istream &in) {
    ...
    if (version != "input_rgb_image_pyramid" &&
        version != "input_rgb_image_pyramid2")
      throw serialization_error("Unexpected version found while deserializing "
  ...
  friend std::ostream &operator<<(std::ostream &out,
                                  const input_rgb_image_pyramid &item) {
    out << "input_rgb_image_pyramid(" << item.avg_red << "," << item.avg_green
        << "," << item.avg_blue << ")";
  ...
  friend void to_xml(const input_rgb_image_pyramid &item, std::ostream &out) {
    out << "<input_rgb_image_pyramid r='" << item.avg_red << "' g='"
        << item.avg_green << "' b='" << item.avg_blue << "' pyramid_padding='"
        << item.pyramid_padding << "' pyramid_outer_padding='"
        << item.pyramid_outer_padding << "'/>";
  }
