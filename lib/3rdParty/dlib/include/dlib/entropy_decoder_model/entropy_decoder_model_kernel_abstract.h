...
class entropy_decoder_model {
  ...
  entropy_decoder/entropy_decoder_kernel_abstract.h
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          can use it to read symbols from an entropy_decoder and it will
  ...
  It reads them from its associated entropy_decoder simply as it needs them.
...
public:
  ...
  entropy_decoder_model(entropy_decoder &coder);
  /*!
      ensures
  ...
  void decode(unsigned long &symbol);
  /*!
      ensures
...
private:
  ...
  entropy_decoder_model(
      entropy_decoder_model<alphabet_size> &); // copy constructor
  entropy_decoder_model<alphabet_size> &
  operator=(entropy_decoder_model<alphabet_size> &); // assignment operator
};
