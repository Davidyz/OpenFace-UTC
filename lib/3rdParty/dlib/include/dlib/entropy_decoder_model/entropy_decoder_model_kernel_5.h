...
template <unsigned long alphabet_size, typename entropy_decoder,
          unsigned long total_nodes, unsigned long order>
  ...
  /*!
      ...
      CONVENTION
          ...
          - if (next_node < total_nodes) then
              - next_node == the next node in root that has not yet been
  allocated
          ...
          - for every node in the tree:
            {
              - NOTATION:
                  ...
                  - A set of nodes is in the same "context set" if all the
  node's contexts are of length n and all the node's contexts share the same
  prefix of length n-1.
                  - The "child context set" of a node is a set of nodes with
                    contexts that are one symbol longer and prefixed by the
  node's context.  For example, if a node has a context "abc" then the nodes for
  contexts "abca", "abcb", "abcc", etc. are all in the child context set of the
  node.
              ...
              - if (parent_context != 0) then
                  - parent_context == pointer to the parent context of this
  node.
              - else
              ...
              - if (this is not the root node) then
                  ...
                  - count == the number of times this symbol has been seen in
  its parent context.
              - else
...
public:
  ...
  entropy_decoder_model_kernel_5(entropy_decoder &coder);
...
private:
  ...
  inline void scale_counts(node *n);
  /*!
      ensures
  ...
  entropy_decoder_model_kernel_5(
      entropy_decoder_model_kernel_5<alphabet_size, entropy_decoder,
                                     total_nodes, order> &); // copy constructor
  entropy_decoder_model_kernel_5<alphabet_size, entropy_decoder, total_nodes,
                                 order> &
  operator=(entropy_decoder_model_kernel_5<alphabet_size, entropy_decoder,
                                           total_nodes, order>
                &); // assignment operator
};
...
template <unsigned long alphabet_size, typename entropy_decoder,
          unsigned long total_nodes, unsigned long order>
entropy_decoder_model_kernel_5<
    alphabet_size, entropy_decoder, total_nodes,
    order>::entropy_decoder_model_kernel_5(entropy_decoder &coder_)
    : coder(coder_), next_node(1), cur_order(0), stack_size(0) {
...
template <unsigned long alphabet_size, typename entropy_decoder,
          unsigned long total_nodes, unsigned long order>
entropy_decoder_model_kernel_5<alphabet_size, entropy_decoder, total_nodes,
                               order>::~entropy_decoder_model_kernel_5() {
  delete[] root;
}
...
template <unsigned long alphabet_size, typename entropy_decoder,
          unsigned long total_nodes, unsigned long order>
void entropy_decoder_model_kernel_5<alphabet_size, entropy_decoder, total_nodes,
                                    order>::clear() {
  next_node = 1;
  root->child_context = 0;
  root->escapes = 0;
...
template <unsigned long alphabet_size, typename entropy_decoder,
          unsigned long total_nodes, unsigned long order>
void entropy_decoder_model_kernel_5<alphabet_size, entropy_decoder, total_nodes,
                                    order>::decode(unsigned long &symbol) {
  node *temp = cur;
  cur = 0;
  ...
  while (stack_size > 0) {
    ...
    if (nc->total) {
      ...
      temp /=
          (temp2 |
           1); // this oring by 1 is just to make sure that temp2 is never zero
      temp += 2;
      if (temp > 50000)
        temp = 50000;
...
template <unsigned long alphabet_size, typename entropy_decoder,
          unsigned long total_nodes, unsigned long order>
bool entropy_decoder_model_kernel_5<alphabet_size, entropy_decoder, total_nodes,
                                    order>::space_left() const {
...
template <unsigned long alphabet_size, typename entropy_decoder,
          unsigned long total_nodes, unsigned long order>
void entropy_decoder_model_kernel_5<alphabet_size, entropy_decoder, total_nodes,
                                    order>::exclude(unsigned short symbol) {
  exc_used = true;
...
template <unsigned long alphabet_size, typename entropy_decoder,
          unsigned long total_nodes, unsigned long order>
bool entropy_decoder_model_kernel_5<alphabet_size, entropy_decoder, total_nodes,
                                    order>::is_excluded(unsigned short symbol) {
  unsigned long temp = 1;
  temp <<= symbol & 0x1F;
