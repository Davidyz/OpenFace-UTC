...
public:
  ...
  rand(time_t seed_value) {
    init();
    set_seed(cast_to_string(seed_value));
  }

  rand(const std::string &seed_value) {
    init();
    set_seed(seed_value);
  }
  ...
  uint16 get_random_16bit_number() { return static_cast<uint16>(mt()); }

  inline uint32 get_random_32bit_number() { return mt(); }
  ...
  double get_double_in_range(double begin, double end) {
    DLIB_ASSERT(begin <= end);
    return begin + get_random_double() * (end - begin);
  }

  long long get_integer_in_range(long long begin, long long end) {
    DLIB_ASSERT(begin <= end);
    if (begin == end)
      return begin;
  ...
  long long get_integer(long long end) {
    DLIB_ASSERT(end >= 0);
  ...
  double get_random_gaussian() {
    ...
    // Generate a pair of Gaussian random numbers using the Box-Muller
    // transformation.
    do {
  ...
  friend void serialize(const rand &item, std::ostream &out);

  friend void deserialize(rand &item, std::istream &in);
...
inline void swap(rand &a, rand &b) { a.swap(b); }

template <> struct is_rand<rand> {
  static const bool value = true;
};

inline void serialize(const rand &item, std::ostream &out) {
  int version = 1;
...
inline void deserialize(rand &item, std::istream &in) {
  int version;
