...
class sequence_kernel_2 : public enumerable<T>, public remover<T> {
  /*!
      INITIAL VALUE
          sequence_size   == 0
          at_start_       == true
...
public:
  ...
  sequence_kernel_2()
      : sequence_size(0), at_start_(true), current_enumeration_node(0) {}
...
private:
  void delete_nodes(node *current_node, unsigned long sequence_size);
  /*!
      requires
  ...
  void move_to_pos(node *&current_node, unsigned long &current_pos,
                   unsigned long pos, unsigned long size) const;
  /*!
      requires
...
inline void swap(sequence_kernel_2<T, mem_manager> &a,
                 sequence_kernel_2<T, mem_manager> &b) {
  a.swap(b);
}

template <typename T, typename mem_manager>
void deserialize(sequence_kernel_2<T, mem_manager> &item, std::istream &in) {
  try {
    item.clear();
    unsigned long size;
    deserialize(size, in);
...
template <typename T, typename mem_manager>
void sequence_kernel_2<T, mem_manager>::clear() {
  if (sequence_size != 0) {
    delete_nodes(current_node, sequence_size);
    sequence_size = 0;
  }
...
template <typename T, typename mem_manager>
void sequence_kernel_2<T, mem_manager>::add(unsigned long pos, T &item) {
...
template <typename T, typename mem_manager>
void sequence_kernel_2<T, mem_manager>::remove(unsigned long pos, T &item) {
  move_to_pos(current_node, current_pos, pos, sequence_size);
  node &c_node = *current_node;
...
template <typename T, typename mem_manager>
const T &
sequence_kernel_2<T, mem_manager>::operator[](unsigned long pos) const {
  move_to_pos(current_node, current_pos, pos, sequence_size);
...
template <typename T, typename mem_manager>
void sequence_kernel_2<T, mem_manager>::cat(
  ...
  if (item.sequence_size > 0) {
    if (sequence_size > 0) {
      ...
      item.move_to_pos(item.current_node, item.current_pos,
                       item.sequence_size - 1, item.sequence_size);
...
template <typename T, typename mem_manager>
T &sequence_kernel_2<T, mem_manager>::operator[](unsigned long pos) {
  move_to_pos(current_node, current_pos, pos, sequence_size);
  return current_node->item;
}
...
template <typename T, typename mem_manager>
size_t sequence_kernel_2<T, mem_manager>::size() const {
...
template <typename T, typename mem_manager>
void sequence_kernel_2<T, mem_manager>::swap(
    sequence_kernel_2<T, mem_manager> &item) {
...
template <typename T, typename mem_manager>
bool sequence_kernel_2<T, mem_manager>::at_start() const {
  return at_start_;
}
...
template <typename T, typename mem_manager>
void sequence_kernel_2<T, mem_manager>::reset() const {
  at_start_ = true;
  current_enumeration_node = 0;
}
...
template <typename T, typename mem_manager>
bool sequence_kernel_2<T, mem_manager>::current_element_valid() const {
...
template <typename T, typename mem_manager>
const T &sequence_kernel_2<T, mem_manager>::element() const {
...
template <typename T, typename mem_manager>
T &sequence_kernel_2<T, mem_manager>::element() {
  return current_enumeration_node->item;
}
...
template <typename T, typename mem_manager>
bool sequence_kernel_2<T, mem_manager>::move_next() const {
...
template <typename T, typename mem_manager>
void sequence_kernel_2<T, mem_manager>::remove_any(T &item) {
  remove(0, item);
}
...
template <typename T, typename mem_manager>
void sequence_kernel_2<T, mem_manager>::delete_nodes(
    node *current_node, unsigned long sequence_size) {
  node *temp;
  while (sequence_size) {
...
template <typename T, typename mem_manager>
void sequence_kernel_2<T, mem_manager>::move_to_pos(node *&current_node,
