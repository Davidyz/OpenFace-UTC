...
template <typename seq_base> class sequence_sort_1 : public seq_base {
...
private:
  void sort_this_sequence(seq_base &sequence);
  /*!
      ensures
          - each element in the sequence is < the element behind it
  !*/

  void selection_sort(seq_base &sequence);
  /*!
      ensures
...
inline void swap(sequence_sort_1<seq_base> &a, sequence_sort_1<seq_base> &b) {
  a.swap(b);
}
...
template <typename seq_base> void sequence_sort_1<seq_base>::sort() {
  if (this->size() > 1) {
...
template <typename seq_base>
void sequence_sort_1<seq_base>::sort_this_sequence(seq_base &sequence) {
  if (sequence.size() < 30) {
    ...
    dlib::median(partition_element, sequence[sequence.size() - 1],
...
template <typename seq_base>
void sequence_sort_1<seq_base>::selection_sort(seq_base &sequence) {
  if (sequence.size() > 2) {
    T temp[29];
