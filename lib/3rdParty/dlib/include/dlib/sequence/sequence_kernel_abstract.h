...
class sequence : public enumerable<T>, public remover<T> {
...
inline void swap(sequence<T, mem_manager> &a, sequence<T, mem_manager> &b) {
  a.swap(b);
}
/*!
...
template <typename T, typename mem_manager>
void deserialize(sequence<T, mem_manager> &item, std::istream &in);
/*!
