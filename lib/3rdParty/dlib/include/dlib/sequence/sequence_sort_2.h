...
template <typename seq_base> class sequence_sort_2 : public seq_base {
...
inline void swap(sequence_sort_2<seq_base> &a, sequence_sort_2<seq_base> &b) {
  a.swap(b);
}
...
template <typename seq_base> void sequence_sort_2<seq_base>::sort() {
  if (this->size() > 1) {
