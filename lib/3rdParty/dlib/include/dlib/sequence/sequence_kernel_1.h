...
class sequence_kernel_1 : public enumerable<T>, public remover<T> {
  ...
  current_element
          ...
          - for all nodes:
              ...
              - left points to the left subtree or 0 if there is no left
  subtree.
              - right points to the right subtree or 0 if there is no right
  subtree.

              - all elements in a left subtree have a position in the sequence <
  that of the root of the current tree.
...
public:
  ...
  sequence_kernel_1()
      : tree_root(0), tree_size(0), stack(ppool.allocate_array(50)),
        current_element(0), at_start_(true), stack_pos(0) {}

  virtual ~sequence_kernel_1();

  inline void clear();

  void add(unsigned long pos, T &item);

  void remove(unsigned long pos, T &item);

  void cat(sequence_kernel_1 &item);

  const T &operator[](unsigned long pos) const;

  T &operator[](unsigned long pos);

  inline void swap(sequence_kernel_1 &item);

  // functions from the remover interface
  inline void remove_any(T &item);
...
private:
  ...
  bool remove_least_element_in_tree(node *&t, T &item);
  /*!
      requires
  ...
  bool add_to_tree(node *&t, unsigned long pos, T &item);
  /*!
      requires
  ...
  bool remove_from_tree(node *&t, unsigned long pos, T &item);
  /*!
      requires
  ...
  const T &return_reference(const node *t, unsigned long pos) const;
  /*!
      requires
          - there is an item in the tree associated with pos
      ensures
          - returns a const reference to the item in the tree associated with
  pos
  !*/

  T &return_reference(node *t, unsigned long pos);
  /*!
      requires
          - there is an item in the tree associated with pos
      ensures
...
inline void swap(sequence_kernel_1<T, mem_manager> &a,
                 sequence_kernel_1<T, mem_manager> &b) {
  a.swap(b);
}

template <typename T, typename mem_manager>
void deserialize(sequence_kernel_1<T, mem_manager> &item, std::istream &in) {
  try {
    item.clear();
    unsigned long size;
    deserialize(size, in);
...
// ----------------------------------------------------------------------------------------
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::swap(
...
template <typename T, typename mem_manager>
size_t sequence_kernel_1<T, mem_manager>::size() const {
  return tree_size;
}
...
template <typename T, typename mem_manager>
const T &
sequence_kernel_1<T, mem_manager>::operator[](unsigned long pos) const {
...
template <typename T, typename mem_manager>
T &sequence_kernel_1<T, mem_manager>::operator[](unsigned long pos) {
  return return_reference(tree_root, pos);
}
...
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::add(unsigned long pos, T &item) {
...
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::remove(unsigned long pos, T &item) {
...
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::cat(
    sequence_kernel_1<T, mem_manager> &item) {
  for (unsigned long i = 0; i < item.tree_size; ++i) {
    add_to_tree(tree_root, tree_size, return_reference(item.tree_root, i));
...
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::clear() {
  if (tree_size > 0) {
    delete_nodes(tree_root);
    tree_root = 0;
    tree_size = 0;
...
template <typename T, typename mem_manager>
bool sequence_kernel_1<T, mem_manager>::at_start() const {
  return at_start_;
}
...
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::reset() const {
  at_start_ = true;
  current_element = 0;
  stack_pos = 0;
}
...
template <typename T, typename mem_manager>
bool sequence_kernel_1<T, mem_manager>::current_element_valid() const {
...
template <typename T, typename mem_manager>
const T &sequence_kernel_1<T, mem_manager>::element() const {
...
template <typename T, typename mem_manager>
T &sequence_kernel_1<T, mem_manager>::element() {
  return current_element->item;
}
...
template <typename T, typename mem_manager>
bool sequence_kernel_1<T, mem_manager>::move_next() const {
...
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::remove_any(T &item) {
  remove(0, item);
}
...
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::rotate_left(node *&t) {
...
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::rotate_right(node *&t) {
  // set the new balance numbers
  if (t->left->balance == -1) {
    t->balance = 0;
    t->left->balance = 0;
...
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::double_rotate_right(node *&t) {
...
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::double_rotate_left(node *&t) {
  node *temp = t;
  t = t->right->left;
...
template <typename T, typename mem_manager>
bool sequence_kernel_1<T, mem_manager>::remove_least_element_in_tree(node *&t,
                                                                     T &item) {
...
template <typename T, typename mem_manager>
bool sequence_kernel_1<T, mem_manager>::add_to_tree(node *&t, unsigned long pos,
                                                    T &item) {
...
template <typename T, typename mem_manager>
bool sequence_kernel_1<T, mem_manager>::remove_from_tree(node *&t,
                                                         unsigned long pos,
                                                         T &item) {
  ...
  // if item is on the right
    ...
    if (tree.balance == 1) {
      tree.balance -=
          remove_from_tree(tree.right, pos - tree.left_size - 1, item);
      return !tree.balance;
    } else {
      tree.balance -=
          remove_from_tree(tree.right, pos - tree.left_size - 1, item);
...
template <typename T, typename mem_manager>
T &sequence_kernel_1<T, mem_manager>::return_reference(node *t,
                                                       unsigned long pos) {
  while (true) {
...
template <typename T, typename mem_manager>
const T &
sequence_kernel_1<T, mem_manager>::return_reference(const node *t,
...
template <typename T, typename mem_manager>
bool sequence_kernel_1<T, mem_manager>::keep_node_balanced(node *&t) {
  // make a reference to the current node so we don't have to dereference
  // a pointer a bunch of times
  node &tree = *t;
...
template <typename T, typename mem_manager>
void sequence_kernel_1<T, mem_manager>::delete_nodes(node *t) {
  if (t->left)
    delete_nodes(t->left);
  if (t->right)
