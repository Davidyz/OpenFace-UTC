...
template <typename seq_base> class sequence_compare : public seq_base {
...
public:
  bool operator<(const sequence_compare &rhs) const;
  /*!
      ensures
...
inline void swap(sequence_compare<seq_base> &a, sequence_compare<seq_base> &b) {
  a.swap(b);
}
/*!
