...
template <unsigned long alphabet_size> class conditioning_class {
  /*!
      ...
      NOTATION:
          ...
          Basically what this is saying is just that you shoudln't assume you
  know what order the symbols are placed in when calculating the cumulative
          sums.  The specific mapping provided by the MAP() function is
  unspecified.

      THREAD SAFETY
          This object can be used safely in a multithreaded program as long as
  the global state is not shared between conditioning classes which run on
          different threads.

      GLOBAL_STATE_TYPE
          The global_state_type obejct allows instances of the
  conditioning_class object to share any kind of global state the implementer
  desires. However, the global_state_type object exists primarily to facilitate
  the sharing of a memory pool between many instances of a conditioning_class
...
public:
  ...
  conditioning_class(global_state_type &global_state);
  /*!
      ensures
          - #*this is properly initialized
          ...
          - if (sufficient memory is available to complete this operation) then
              ...
              - else
                  ...
                  - if (get_count(alphabet_size-1) == 1) then
                      - #get_count(alphabet_size-1) == 1
                  - else
                      - #get_count(alphabet_size-1) ==
  get_count(alphabet_size-1)/2
  ...
  /*!
      ensures
          - returns the number of bytes of memory allocated by all
  conditioning_class objects that share the global state given by
  get_global_state()
  !*/
...
private:
  ...
  conditioning_class<alphabet_size> &
  operator=(conditioning_class<alphabet_size> &); // assignment operator
};
