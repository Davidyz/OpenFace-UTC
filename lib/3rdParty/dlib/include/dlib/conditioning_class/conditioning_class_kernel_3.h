...
template <unsigned long alphabet_size> class conditioning_class_kernel_3 {
  /*!
      INITIAL VALUE
          total == 1
...
public:
  ...
  conditioning_class_kernel_3(global_state_type &global_state_);
...
private:
  ...
  conditioning_class_kernel_3(
      conditioning_class_kernel_3<alphabet_size> &); // copy constructor
  conditioning_class_kernel_3 &operator=(
      conditioning_class_kernel_3<alphabet_size> &); // assignment operator
...
template <unsigned long alphabet_size>
conditioning_class_kernel_3<alphabet_size>::conditioning_class_kernel_3(
    global_state_type &global_state_)
    : total(1), counts(new data[alphabet_size]), global_state(global_state_) {
  ...
  // update memory usage
  global_state.memory_usage +=
      sizeof(data) * alphabet_size + sizeof(conditioning_class_kernel_3);
}
...
template <unsigned long alphabet_size>
void conditioning_class_kernel_3<alphabet_size>::clear() {
  total = 1;
  data *start = counts;
  data *end = counts + alphabet_size;
  start->count = 1;
...
template <unsigned long alphabet_size>
typename conditioning_class_kernel_3<alphabet_size>::global_state_type &
conditioning_class_kernel_3<alphabet_size>::get_global_state() {
  return global_state;
}
...
template <unsigned long alphabet_size>
unsigned long
conditioning_class_kernel_3<alphabet_size>::get_memory_usage() const {
...
template <unsigned long alphabet_size>
bool conditioning_class_kernel_3<alphabet_size>::increment_count(
    unsigned long symbol, unsigned short amount) {
  // if we are going over a total of 65535 then scale down all counts by 2
  if (static_cast<unsigned long>(total) + static_cast<unsigned long>(amount) >=
      65536) {
    total = 0;
    data *start = counts;
    data *end = counts + alphabet_size;
...
template <unsigned long alphabet_size>
unsigned long conditioning_class_kernel_3<alphabet_size>::get_count(
...
template <unsigned long alphabet_size>
unsigned long conditioning_class_kernel_3<alphabet_size>::get_alphabet_size() {
...
template <unsigned long alphabet_size>
unsigned long conditioning_class_kernel_3<alphabet_size>::get_total() const {
  return total;
}
...
template <unsigned long alphabet_size>
unsigned long conditioning_class_kernel_3<alphabet_size>::get_range(
...
template <unsigned long alphabet_size>
void conditioning_class_kernel_3<alphabet_size>::get_symbol(
    unsigned long target, unsigned long &symbol, unsigned long &low_count,
    unsigned long &high_count) const {
