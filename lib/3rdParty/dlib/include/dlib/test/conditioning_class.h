...
template <typename cc, typename cc2>
void conditioning_class_kernel_test()
/*!
    requires
        - cc is an implementation of
conditioning_class/conditioning_class_kernel_abstract.h the alphabet_size for cc
is 256
        - cc2 is an implementation of
conditioning_class/conditioning_class_kernel_abstract.h the alphabet_size for
cc2 is 2 ensures
...
!*/
{
  ...
  for (int g = 0; g < 2; ++g) {
    ...
    for (unsigned long i = 0; i < alphabet_size - 1; ++i) {
      ...
      DLIB_TEST_MSG(test.get_range(i, low_count, high_count, total_count) == 0,
                    i);
    ...
    for (unsigned long i = 0; i < alphabet_size; ++i) {
      ...
      if (i == alphabet_size - 1) {
        DLIB_TEST(test.get_range(i, low_count, high_count, total_count) ==
                  1 + amount);
    ...
    for (unsigned long i = 0; i < alphabet_size; ++i) {
      ...
      if (i == alphabet_size - 1) {
        DLIB_TEST(test.get_range(symbol, low_count, high_count, total_count) ==
    ...
    for (int k = 0; k < 10; ++k) {
      ...
      for (unsigned long i = 0; i < alphabet_size; ++i) {
        ...
        DLIB_TEST(test.get_count(alphabet_size - 1) ==
                  counts[alphabet_size - 1]);
        DLIB_TEST_MSG(test.get_total() == total,
                      "test.get_total() == " << test.get_total() << endl
                                             << "total == " << total);

        unsigned long target = test.get_total() / 2;
        unsigned long symbol = i, low_count = 0, high_count = 0,
                      total_count = 0;
    ...
    for (unsigned long h = 0; h < 10; ++h) {
      ...
      for (unsigned long j = 0; j < alphabet_size; ++j) {
        unsigned long total;
        unsigned long count =
            test.get_range(j, low_counts[j], high_counts[j], total);
        ...
        for (unsigned long k = low_counts[j]; k < high_counts[j]; ++k) {
          ...
          DLIB_TEST_MSG(
              ...
              "j == " << j << endl
                      ...
                      << "high_counts[symbol] == " << high_counts[symbol]
                      << endl
  ...
  for (int g = 0; g < 2; ++g) {
    ...
    for (unsigned long i = 0; i < alphabet_size; ++i) {
      ...
      if (!oom) {
        if (i == alphabet_size - 1) {
          DLIB_TEST(test.get_range(symbol, low_count, high_count,
                                   total_count) == temp * amount + 1 + amount);
          DLIB_TEST(high_count - low_count == temp * amount + 1 + amount);
        } else {
          DLIB_TEST(test.get_range(symbol, low_count, high_count,
                                   total_count) == temp * amount + amount);
          DLIB_TEST(high_count - low_count == temp * amount + amount);
        }
        DLIB_TEST(total_count == test.get_total());
          ...
          for (unsigned long k = low_counts[j]; k < high_counts[j]; ++k) {
            ...
            DLIB_TEST_MSG(
                ...
                "j == " << j << endl
                        ...
                        << "low_counts[j] == " << low_counts[j] << endl
                        << "high_counts[j] == " << high_counts[j] << endl
                        << "low_counts[symbol] == " << low_counts[symbol]
                        << endl
                        << "high_counts[symbol] == " << high_counts[symbol]
                        << endl
                        << "low_count == " << low_count << endl
