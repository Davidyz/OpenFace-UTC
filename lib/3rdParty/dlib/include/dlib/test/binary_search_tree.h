...
/*!
    requires
        - bst is an implementation of
          binary_search_tree/binary_search_tree_kernel_abstract.h is
instantiated to map int to int ensures
...
!*/
{
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    DLIB_TEST_MSG(test.height() > 13 && test.height() <= 26,
                  "this is somewhat of an implementation dependent "
                      << "but really it should be in this range or the "
                         "implementation is just crap");

    a = 0;
    unsigned long count = 0;
    while (test.move_next()) {
      DLIB_TEST_MSG(a <= test.element().key(),
                    "the numers are out of order but they should be in order");
      a = test.element().key();
      ++count;
    ...
    while (test2.move_next()) {
      DLIB_TEST_MSG(a <= test2.element().key(),
                    "the numers are out of order but they should be in order");
      a = test2.element().key();
      ++count;
    ...
    DLIB_TEST_MSG(test2.height() > 13 && test2.height() <= 26,
                  "this is somewhat of an implementation dependent "
                      << "but really it should be in this range or the "
                         "implementation is just crap");
    ...
    for (int i = 0; i < 40; ++i) {
      ...
      DLIB_TEST_MSG(test.current_element_valid(),
                    "size: " << test.size() << "   num: " << num);
      test.remove_current_element(a, b);
      DLIB_TEST_MSG(test.current_element_valid(),
                    "size: " << test.size() << "   num: " << num);
      test.remove_current_element(a, b);
