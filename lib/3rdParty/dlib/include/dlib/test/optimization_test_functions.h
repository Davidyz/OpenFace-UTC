...
/*
    ...
    Most of the code in this file is converted from the set of Fortran 90
   routines created by John Burkardt.

    The original Fortran can be found here:
   http://orion.math.iastate.edu/burkardt/f_src/testopt/testopt.html
...
public:
  ...
  double operator()(const column_vector &x) const { return chebyquad(x); }
...
public:
  ...
  double operator()(const column_vector &x) const { return brown(x); }
...
template <typename T> matrix<T, 2, 1> rosen_big_start() {
  matrix<T, 2, 1> x;
  x = -1.2, -1;
  return x;
}

// This is a variation on the Rosenbrock test function but with large residuals.
// The minimum is at 1, 1 and the objective value is 1.
template <typename T> T rosen_big_residual(int i, const matrix<T, 2, 1> &m) {
  using std::pow;
  const T x = m(0);
  const T y = m(1);
...
template <typename T> T rosen_big(const matrix<T, 2, 1> &m) {
  using std::pow;
  return 0.5 *
         (pow(rosen_big_residual(1, m), 2) + pow(rosen_big_residual(2, m), 2));
}

template <typename T> matrix<T, 2, 1> rosen_big_solution() {
  matrix<T, 2, 1> x;
...
template <typename T> matrix<T, 2, 1> rosen_start() {
  matrix<T, 2, 1> x;
  x = -1.2, -1;
  return x;
}

template <typename T> T rosen(const matrix<T, 2, 1> &m) {
  const T x = m(0);
  const T y = m(1);
...
template <typename T> T rosen_residual(int i, const matrix<T, 2, 1> &m) {
  const T x = m(0);
  const T y = m(1);
...
const matrix<T, 2, 1> rosen_derivative(const matrix<T, 2, 1> &m) {
  ...
  res(0) = -400 * x * (y - x * x) -
           2 * (1 - x);       // derivative of rosen() with respect to x
...
template <typename T> matrix<T, 2, 1> rosen_solution() {
  matrix<T, 2, 1> x;
  // solution from original documentation.
  x = 1, 1;
  return x;
...
template <typename T> struct rosen_function_model {
  typedef matrix<T, 2, 1> column_vector;
  typedef matrix<T, 2, 2> general_matrix;

  T operator()(column_vector x) const { return static_cast<T>(rosen(x)); }
