...
template <typename base> class sync_extension : public base {
  ...
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          the rmutex and rsignaler are associated with the object instance
  itself, not with whatever the object represents.
  !*/
...
public:
  ...
  template <typename T> sync_extension(const T &one);
  /*!
      ensures
          - #*this is properly initialized
  ...
  template <typename T, typename U> sync_extension(const T &one, const T &two);
  /*!
      ensures
          - #*this is properly initialized
          - the argument one will be passed on to the constructor for the parent
  ...
  /*!
      ...
      ensures
          ...
          - calling thread will wake if another thread calls signal() or
  broadcast() on *this
  ...
  thread will wake once *this is free to be locked
          - when wait returns the calling thread again has a lock on #*this
...
inline void swap(sync_extension<base> &a, sync_extension<base> &b) {
  a.swap(b);
}
/*!
