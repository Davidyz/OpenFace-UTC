...
template <typename hash_set_base>
...
template <typename hash_set_base>
inline void swap(hash_set_kernel_c<hash_set_base> &a,
...
template <typename hash_set_base>
void hash_set_kernel_c<hash_set_base>::add(T &item) {
...
template <typename hash_set_base>
void hash_set_kernel_c<hash_set_base>::remove(const T &item, T &item_copy) {
  // make sure requires clause is not broken
  DLIB_CASSERT(
      this->is_member(item) &&
...
template <typename hash_set_base>
void hash_set_kernel_c<hash_set_base>::destroy(const T &item) {
  // make sure requires clause is not broken
  DLIB_CASSERT(
      this->is_member(item),
...
template <typename hash_set_base>
void hash_set_kernel_c<hash_set_base>::remove_any(T &item) {
  // make sure requires clause is not broken
  DLIB_CASSERT(
      this->size() != 0,
...
template <typename hash_set_base>
const typename hash_set_base::type &
hash_set_kernel_c<hash_set_base>::element() const {
  DLIB_CASSERT(
...
template <typename hash_set_base>
const typename hash_set_base::type &
hash_set_kernel_c<hash_set_base>::element() {
  DLIB_CASSERT(
      this->current_element_valid() == true,
      "\tT& hash_set::element()"
          << "\n\tyou can't access the current element if it doesn't exist"
