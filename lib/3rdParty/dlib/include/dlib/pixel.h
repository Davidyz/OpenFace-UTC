...
struct rgb_pixel {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This is a simple struct that represents an RGB colored graphical
  pixel.
  !*/

  rgb_pixel() {}

  rgb_pixel(unsigned char red_, unsigned char green_, unsigned char blue_)
...
struct bgr_pixel {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This is a simple struct that represents an BGR colored graphical
  pixel. (the reason it exists in addition to the rgb_pixel is so you can lay it
  down on top of a memory region that organizes its color data in the BGR format
  and still be able to read it)
  !*/

  bgr_pixel() {}

  bgr_pixel(unsigned char blue_, unsigned char green_, unsigned char red_)
...
struct rgb_alpha_pixel {
  ...
  rgb_alpha_pixel(unsigned char red_, unsigned char green_, unsigned char blue_,
...
struct hsi_pixel {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This is a simple struct that represents an HSI colored graphical
  pixel.
  !*/

  hsi_pixel() {}

  hsi_pixel(unsigned char h_, unsigned char s_, unsigned char i_)
      : h(h_), s(s_), i(i_) {}
...
struct lab_pixel {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This is a simple struct that represents an Lab colored graphical
  pixel.
  !*/

  lab_pixel() {}

  lab_pixel(unsigned char l_, unsigned char a_, unsigned char b_)
      : l(l_), a(a_), b(b_) {}
    ...
    ensures
        ...
        - else if (P1 and P2 are not the same type of pixel) then
            - assigns pixel src to pixel dest and does any necessary color space
              conversions.
            - When converting from a grayscale color space with more than 255
values the pixel intensity is saturated at pixel_traits<P1>::max() or
pixel_traits<P1>::min() as appropriate.
            - if (the dest pixel has an alpha channel and the src pixel doesn't)
then
                - #dest.alpha == 255
            - else if (the src pixel has an alpha channel but the dest pixel
doesn't) then
                - #dest == the original dest value blended with the src value
according to the alpha channel in src. (i.e.  #dest == src*(alpha/255) +
dest*(1-alpha/255))
!*/
...
inline void assign_pixel_intensity(P &dest, const T &new_intensity);
/*!
    requires
        - pixel_traits<P> must be defined
        - pixel_traits<T> must be defined
    ensures
        - This function changes the intensity of the dest pixel. So if the pixel
in question is a grayscale pixel then it simply assigns that pixel with the
          value of get_pixel_intensity(new_intensity).  However, if the pixel is
not a grayscale pixel then it converts the pixel to the HSI color space and sets
          the I channel to the given intensity and then converts this HSI value
back to the original pixel's color space.
        - Note that we don't necessarily have #get_pixel_intensity(dest) ==
get_pixel_intensity(new_intensity) due to vagaries of how converting to and from
HSI works out.
...
inline void serialize(const rgb_pixel &item, std::ostream &out);
/*!
...
inline void deserialize(rgb_pixel &item, std::istream &in);
/*!
...
inline void serialize(const bgr_pixel &item, std::ostream &out);
/*!
...
inline void deserialize(bgr_pixel &item, std::istream &in);
/*!
...
inline void serialize(const rgb_alpha_pixel &item, std::ostream &out);
/*!
...
inline void deserialize(rgb_alpha_pixel &item, std::istream &in);
/*!
...
inline void serialize(const hsi_pixel &item, std::ostream &out);
/*!
...
inline void serialize(const lab_pixel &item, std::ostream &out);
/*!
...
inline void deserialize(hsi_pixel &item, std::istream &in);
/*!
    provides deserialization support for the hsi_pixel struct
!*/
// ----------------------------------------------------------------------------------------

inline void deserialize(lab_pixel &item, std::istream &in);
/*!
...
template <typename T> struct grayscale_pixel_traits {
  constexpr static bool rgb = false;
  constexpr static bool rgb_alpha = false;
...
struct pixel_traits<unsigned char>
    : public grayscale_pixel_traits<unsigned char> {};
template <>
struct pixel_traits<unsigned short>
    : public grayscale_pixel_traits<unsigned short> {};
template <>
struct pixel_traits<unsigned int>
    : public grayscale_pixel_traits<unsigned int> {};
template <>
struct pixel_traits<unsigned long>
    : public grayscale_pixel_traits<unsigned long> {};
...
template <typename T> struct float_grayscale_pixel_traits {
  constexpr static bool rgb = false;
  constexpr static bool rgb_alpha = false;
  constexpr static bool grayscale = true;
  constexpr static bool hsi = false;
...
template <>
struct pixel_traits<long double>
    : public float_grayscale_pixel_traits<long double> {};

// These are here mainly so you can easily copy images into complex arrays. This
// is useful when you want to do a FFT on an image or some similar operation.
template <>
struct pixel_traits<std::complex<float>>
    : public float_grayscale_pixel_traits<float> {};
template <>
struct pixel_traits<std::complex<double>>
    : public float_grayscale_pixel_traits<double> {};
template <>
struct pixel_traits<std::complex<long double>>
    : public float_grayscale_pixel_traits<long double> {};
...
template <typename P>
typename enable_if_c<pixel_traits<P>::grayscale>::type assign(P &dest,
                                                              const P &src) {
  dest = src;
}
...
template <typename T>
typename unsigned_type<T>::type make_unsigned(const T &val) {
...
typename enable_if_c<
    pixel_traits<T>::is_unsigned == pixel_traits<P>::is_unsigned, bool>::type
...
template <typename T, typename P>
typename enable_if_c<
    !pixel_traits<P>::is_unsigned && pixel_traits<T>::is_unsigned, bool>::type
...
template <typename P1, typename P2>
typename enable_if_c<pixel_traits<P1>::grayscale &&
                     pixel_traits<P2>::grayscale>::type
assign(P1 &dest, const P2 &src) {
  /*
      The reason for these weird comparison functions is to avoid getting
     compiler warnings about comparing signed types to unsigned and stuff like
     that.
  */
...
template <typename P1, typename P2>
typename enable_if_c<pixel_traits<P1>::rgb_alpha &&
                     pixel_traits<P2>::rgb_alpha>::type
assign(P1 &dest, const P2 &src) {
  dest.red = src.red;
  dest.green = src.green;
  dest.blue = src.blue;
...
assign(P1 &dest, const P2 &src) {
  const unsigned int temp = ((static_cast<unsigned int>(src.red) +
                              ...
                              static_cast<unsigned int>(src.blue)) /
                             3);
...
assign(P1 &dest, const P2 &src) {
  ...
  const unsigned char avg =
      static_cast<unsigned char>((static_cast<unsigned int>(src.red) +
                                  static_cast<unsigned int>(src.green) +
                                  static_cast<unsigned int>(src.blue)) /
                                 3);
...
assign_pixel_intensity_helper(P &dest, const T &new_intensity) {
  ...
  dest.alpha = 255;
  rgb_pixel temp;
  assign_pixel(temp, dest); // put dest into an rgb_pixel to avoid the somewhat
                            // complicated assign_pixel(hsi,rgb_alpha).
  assign_pixel(p, temp);
...
template <typename P>
inline typename enable_if_c<pixel_traits<P>::grayscale, P>::type
