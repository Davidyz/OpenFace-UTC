...
public:
  iosockstream() : std::iostream(0) {}

  iosockstream(const network_address &addr) : std::iostream(0) { open(addr); }
  ...
  void open(const network_address &addr) {
    ...
    // telling the iostream::tie() function to tie the stream to itself even
    // though that should work fine.  The reason we do it this way is because
    // there is a bug in visual studio 2012 that causes a program to crash when
    // a stream is tied to itself and then used.  See
  ...
  void terminate_connection_after_timeout(unsigned long timeout) {
    auto_mutex lock(class_mutex);
    if (con) {
      con_timeout.reset(new dlib::timeout(
          *this, &iosockstream::terminate_connection, timeout, con));
    }
  }
