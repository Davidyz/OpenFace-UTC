...
template <typename image_type> cv::Mat toMat(image_type &img) {
  ...
  if (pixel_traits<type>::num == 1) {
    return cv::Mat(num_rows(img), num_columns(img),
                   cv::DataType<basic_pixel_type>::type, image_data(img),
                   width_step(img));
  } else {
    ...
    return cv::Mat(num_rows(img), num_columns(img), thetype, image_data(img),
                   width_step(img));
  }
}
