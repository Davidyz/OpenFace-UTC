...
template <typename pixel_type> class cv_image {
public:
  ...
  cv_image(const cv::Mat img) {
    DLIB_CASSERT(
        img.depth() == cv::DataType<typename pixel_traits<
                           pixel_type>::basic_pixel_type>::depth &&
            img.channels() == pixel_traits<pixel_type>::num,
        "The pixel type you gave doesn't match pixel used by the open cv Mat "
        "object."
            << "\n\t img.depth():    " << img.depth()
            << "\n\t img.cv::DataType<typename "
               "pixel_traits<pixel_type>::basic_pixel_type>::depth: "
            << cv::DataType<
                   typename pixel_traits<pixel_type>::basic_pixel_type>::depth
  ...
  cv_image(const IplImage img) { init(&img); }
  ...
  inline const pixel_type &operator()(const long row, const long column) const {
    DLIB_ASSERT(0 <= column && column < nc(),
                "\tcont pixel_type& cv_image::operator()(const long rown const "
                "long column)"
...
// Define the global functions that make cv_image a proper "generic image"
// according to
// ../image_processing/generic_image.h
template <typename T> struct image_traits<cv_image<T>> {
  typedef T pixel_type;
};

template <typename T> inline long num_rows(const cv_image<T> &img) {
  return img.nr();
}
template <typename T> inline long num_columns(const cv_image<T> &img) {
  return img.nc();
}

template <typename T> inline void *image_data(cv_image<T> &img) {
  if (img.size() != 0)
    return &img[0][0];
  else
    return 0;
...
template <typename T> inline const void *image_data(const cv_image<T> &img) {
  if (img.size() != 0)
    return &img[0][0];
  else
    return 0;
...
template <typename T> inline long width_step(const cv_image<T> &img) {
