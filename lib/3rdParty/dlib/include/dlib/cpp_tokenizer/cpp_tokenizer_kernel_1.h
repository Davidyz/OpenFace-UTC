...
template <typename tok, typename queue, typename set>
class cpp_tokenizer_kernel_1 {
  /*!
      ...
      INITIAL VALUE
          ...
          - buffer.size() == 0
          - tokenizer.get_identifier_head() == "$_" +
tokenizer.lowercase_letters() + tokenizer.uppercase_letters()
          - tokenizer.get_identifier_body() == "$_" +
tokenizer.lowercase_letters() + tokenizer.uppercase_letters() +
tokenizer.numbers()
          - have_peeked == false
...
public:
  ...
  void get_token(int &type, std::string &token);

  int peek_type() const;

  const std::string &peek_token() const;

  void swap(cpp_tokenizer_kernel_1<tok, queue, set> &item);

private:
  void buffer_token(int type, const std::string &token)
  /*!
      ensures
  ...
  void buffer_token(int type, char token)
  /*!
      ensures
          - stores the token and its type into buffer
  !*/
  ...
  cpp_tokenizer_kernel_1(
      const cpp_tokenizer_kernel_1<tok, queue, set> &); // copy constructor
  cpp_tokenizer_kernel_1<tok, queue, set> &operator=(
      const cpp_tokenizer_kernel_1<tok, queue, set> &); // assignment operator
...
template <typename tok, typename queue, typename set>
inline void swap(cpp_tokenizer_kernel_1<tok, queue, set> &a,
...
template <typename tok, typename queue, typename set>
cpp_tokenizer_kernel_1<tok, queue, set>::cpp_tokenizer_kernel_1()
    : have_peeked(false) {
...
template <typename tok, typename queue, typename set>
void cpp_tokenizer_kernel_1<tok, queue, set>::clear() {
  tokenizer.clear();
  buffer.clear();
  have_peeked = false;
...
template <typename tok, typename queue, typename set>
void cpp_tokenizer_kernel_1<tok, queue, set>::set_stream(std::istream &in) {
...
template <typename tok, typename queue, typename set>
bool cpp_tokenizer_kernel_1<tok, queue, set>::stream_is_set() const {
...
template <typename tok, typename queue, typename set>
std::istream &cpp_tokenizer_kernel_1<tok, queue, set>::get_stream() const {
...
template <typename tok, typename queue, typename set>
void cpp_tokenizer_kernel_1<tok, queue, set>::get_token(int &type,
                                                        std::string &token) {
  using namespace std;
  ...
  if (!have_peeked) {
    ...
    case tok::NUMBER: {
      ...
      if (tokenizer.peek_type() == tok::IDENTIFIER && token == "0" &&
          (tokenizer.peek_token()[0] == 'x' ||
           tokenizer.peek_token()[0] == 'X')) {
        // this is a hex number so accumulate all the numbers and identifiers
        // that follow because they have to be part of the number
        std::string temp;
        tokenizer.get_token(type, temp);
        token = "0" + temp;
      ...
      // or this could be a floating point value or something with an 'e' or 'E'
      // in it.
      else if ((tokenizer.peek_type() == tok::CHAR &&
                tokenizer.peek_token()[0] == '.') ||
               (tokenizer.peek_type() == tok::IDENTIFIER &&
                std::tolower(tokenizer.peek_token()[0]) == 'e')) {
        std::string temp;
        tokenizer.get_token(type, temp);
        token += temp;
        ...
        if (tokenizer.peek_type() == tok::CHAR) {
          if (tokenizer.peek_token()[0] == '/') {
            ...
            while (type != tok::END_OF_FILE) {
              ...
              if (type == tok::END_OF_LINE && token[token.size() - 1] != '\\') {
                token += '\n';
                break;
              } else {
                token += temp;
...
template <typename tok, typename queue, typename set>
int cpp_tokenizer_kernel_1<tok, queue, set>::peek_type() const {
  const_cast<cpp_tokenizer_kernel_1<tok, queue, set> *>(this)->get_token(
...
template <typename tok, typename queue, typename set>
const std::string &cpp_tokenizer_kernel_1<tok, queue, set>::peek_token() const {
  const_cast<cpp_tokenizer_kernel_1<tok, queue, set> *>(this)->get_token(
      next_type, next_token);
  have_peeked = true;
  return next_token;
...
template <typename tok, typename queue, typename set>
void cpp_tokenizer_kernel_1<tok, queue, set>::swap(
