...
template <typename sample_type_, typename result_type_ = double>
class any_decision_function {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          elements which are some kind of function object with an operator()
  with the following signature: result_type operator()(const sample_type&) const

          It is intended to be used to contain dlib::decision_function objects
  and other types which represent learned decision functions.  It allows you to
  write code which contains and processes these decision functions without
  needing to know the specific types of decision functions used.
  !*/
...
public:
  ...
  any_decision_function(const any_decision_function &item);
  /*!
      ensures
  ...
  template <typename T> any_decision_function(const T &item);
  /*!
      ensures
          - #contains<T>() == true
          - #cast_to<T>() == item
  ...
  /*!
      ensures
          - if (contains<T>() == true) then
              - returns a non-const reference to the object contained within
  *this
          - else
              - throws bad_any_cast
  !*/
  ...
  /*!
      ensures
          ...
          - else
              ...
              - Any previous object stored in this any_decision_function object
  is destructed and its state is lost.
              - returns a non-const reference to the newly created T object.
  !*/

  any_decision_function &operator=(const any_decision_function &item);
  /*!
      ensures
          - copies the state of item into *this.
          - Note that *this and item will contain independent copies of the
  ...
  void swap(any_decision_function &item);
  /*!
      ensures
          - swaps *this and item
  !*/
...
inline void swap(any_decision_function<sample_type, result_type> &a,
...
template <typename T, typename sample_type, typename result_type>
T &any_cast(any_decision_function<sample_type, result_type> &a) {
  return a.cast_to<T>();
}
/*!
    ensures
...
template <typename T, typename sample_type, typename result_type>
const T &any_cast(const any_decision_function<sample_type, result_type> &a) {
  return a.cast_to<T>();
}
/*!
    ensures
