...
public:
  ...
  result_type operator()(const sample_type &item) const {
    ...
    DLIB_ASSERT(is_empty() == false,
                "\t result_type any_decision_function::operator()"
                    << "\n\t You can't call operator() on an empty "
                       "any_decision_function"
                    << "\n\t this: " << this);
  ...
  void swap(any_decision_function &item) { data.swap(item.data); }
...
private:
  struct base {
    ...
    virtual result_type evaluate(const sample_type &samp) const = 0;
  };
  ...
  template <typename T> struct derived : public base {
    ...
    virtual result_type evaluate(const sample_type &samp) const {
      return item(samp);
    }
  };
...
T &any_cast(any_decision_function<U, V> &a) {
  return a.template cast_to<T>();
}

template <typename T, typename U, typename V>
const T &any_cast(const any_decision_function<U, V> &a) {
  return a.template cast_to<T>();
}
