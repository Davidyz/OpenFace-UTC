...
template <typename sample_type_, typename scalar_type_ = double>
class any_trainer {
  /*!
      INITIAL VALUE
          - is_empty() == true
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          elements which are some kind of object with a .train() method
  compatible with the following signature:
          ...
          any_trainer is intended to be used to contain objects such as the
  svm_nu_trainer and other similar types which represent supervised machine
  learning algorithms. It allows you to write code which contains and processes
  these trainer objects without needing to know the specific types of trainer
  objects used.
  !*/
...
public:
  ...
  any_trainer(const any_trainer &item);
  /*!
      ensures
  ...
  template <typename T> any_trainer(const T &item);
  /*!
      ensures
          - #contains<T>() == true
          - #cast_to<T>() == item
  ...
  trained_function_type train(const std::vector<sample_type> &samples,
                              const std::vector<scalar_type> &labels) const
      /*!
          requires
              - is_empty() == false
          ensures
  ...
  /*!
      ensures
          ...
          - else
              ...
              - Any previous object stored in this any_trainer object is
  destructed and its state is lost.
              - returns a non-const reference to the newly created T object.
  !*/

  any_trainer &operator=(const any_trainer &item);
  /*!
      ensures
          - copies the state of item into *this.
          - Note that *this and item will contain independent copies of the
...
inline void swap(any_trainer<sample_type, scalar_type> &a,
...
template <typename T, typename sample_type, typename scalar_type>
T &any_cast(any_trainer<sample_type, scalar_type> &a) {
  return a.cast_to<T>();
}
/*!
    ensures
...
template <typename T, typename sample_type, typename scalar_type>
const T &any_cast(const any_trainer<sample_type, scalar_type> &a) {
  return a.cast_to<T>();
}
/*!
    ensures
