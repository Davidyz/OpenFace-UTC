...
class any {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object is basically a type-safe version of a void*.  In
  particular, it is a container which can contain only one object but the object
  may be of any type.

          It is somewhat like the type_safe_union except you don't have to
  declare the set of possible content types beforehand.  So in some sense this
  is like a less type-strict version of the type_safe_union.
  !*/
...
public:
  ...
  template <typename T> any(const T &item);
  /*!
      ensures
          - #contains<T>() == true
          - #cast_to<T>() == item
  ...
  /*!
      ensures
          ...
          - else
              ...
              - Any previous object stored in this any object is destructed and
  its state is lost.
  ...
  /*!
      ensures
          ...
          - does not invalidate pointers or references to the object contained
            inside *this or item.  Moreover, a pointer or reference to the
  object in *this will now refer to the contents of #item and vice versa.
  !*/
};
