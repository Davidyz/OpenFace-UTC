...
class tuple {
  /*!
      INITIAL VALUE
          Each object in the tuple is default initialized by its own
  constructor. The tuple object itself does not modify them or add any
  additional state.
      ...
      EXAMPLE
          We can declare a tuple that contains an int, a float, and a char like
  so: tuple<int,float,char> ex;

          Then we can access each of these by their index number.  The index
  number is just the order each type has in the template argument list.  So we
  have: ex.get<0>() = 5;     // assign the int the value 5 ex.get<1>() = 3.14;
  // assign the float the value 3.14 ex.get<2>() = 'c';   // assign the char the
  value 'c'
...
public:
  ...
  template <long index> struct get_type {
    typedef(the type of the Tindex template argument) type;
  };

  template <long index> const get_type<index>::type &get() const;
  /*!
      requires
          - 0 <= index <= 31
      ensures
  ...
  template <long index> get_type<index>::type &get();
  /*!
      requires
          - 0 <= index <= 31
      ensures
  ...
  template <typename Q> const long index() const;
  /*!
      requires
  ...
  template <typename Q> const Q &get() const;
  /*!
      requires
          - Q is a type of object contained in this tuple and there is
            only one object of that type in the tuple
  ...
  friend void serialize(const tuple &item, std::ostream &out);
  /*!
      provides serialization support
  !*/

  friend void deserialize(tuple &item, std::istream &in);
  /*!
