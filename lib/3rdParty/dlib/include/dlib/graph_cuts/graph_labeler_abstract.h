...
template <typename vector_type> class graph_labeler {
  /*!
  ...
  dlib/svm/sparse_vector_abstract.h.
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          have the same label.  If we could do this then we could find the
  optimal labeling using the find_max_factor_graph_potts() routine.  Therefore,
          the graph_labeler is just an object which contains the necessary data
          to compute these score functions and then call
  find_max_factor_graph_potts(). Additionally, this object uses linear functions
  to represent these score functions.
  ...
  object from multiple threads.  This is because the const members are purely
  read-only operations.  However, any operation that modifies a graph_labeler is
              not threadsafe.
  !*/
...
public:
  ...
  graph_labeler(const vector_type &edge_weights,
  ...
  const vector_type &get_edge_weights() const;
  /*!
      ensures
  ...
  const vector_type &get_node_weights() const;
  /*!
      ensures
          - Recall that the score function for a node is a linear function of
            ...
            which we dot product with the vector in the graph to compute the
  score. Therefore, this function returns that W vector which defines the node
            score function.
  !*/

  template <typename graph_type>
  void operator()(const graph_type &sample, std::vector<bool> &labels) const;
  /*!
      requires
  ...
  objects capable of representing column vectors or some kind of sparse vector
          ...
          - for all valid i and j:
              ...
              - it must be legal to call dot(edge(sample,i,j),
  get_edge_weights())
              - it must be legal to call dot(sample.node(i).data,
  get_node_weights()) ensures
          - Computes a labeling for each node in the given graph and stores the
  result in #labels.
          - #labels.size() == sample.number_of_nodes()
          - for all valid i:
              - #labels[i] == the label of the node sample.node(i).
          - The labels are computed by creating a graph, G, with scalar values
  on each node and edge.  The scalar values are calculated according to the
  following:
              - for all valid i:
                  - G.node(i).data == dot(get_node_weights(),
  sample.node(i).data)
              - for all valid i and j:
                  - edge(G,i,j) == dot(get_edge_weights(), edge(sample,i,j))
            Then the labels are computed by calling
  find_max_factor_graph_potts(G,#labels).
  !*/

  template <typename graph_type>
  std::vector<bool> operator()(const graph_type &sample) const;
  /*!
      requires
          - graph_type is an implementation of
  dlib/graph/graph_kernel_abstract.h
...
void serialize(const graph_labeler<vector_type> &item, std::ostream &out);
/*!
...
template <typename vector_type>
void deserialize(graph_labeler<vector_type> &item, std::istream &in);
/*!
