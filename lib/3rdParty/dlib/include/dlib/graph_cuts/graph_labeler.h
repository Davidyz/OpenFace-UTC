...
public:
  ...
  graph_labeler(const vector_type &edge_weights_,
                const vector_type &node_weights_)
      : edge_weights(edge_weights_), node_weights(node_weights_) {
  ...
  const vector_type &get_edge_weights() const { return edge_weights; }

  const vector_type &get_node_weights() const { return node_weights; }
  ...
  void operator()(const graph_type &sample, std::vector<bool> &labels) const {
    ...
    for (unsigned long i = 0; i < sample.number_of_nodes(); ++i) {
      if (is_matrix<vector_type>::value &&
        ...
        DLIB_ASSERT((unsigned long)get_node_weights().size() ==
                        (unsigned long)sample.node(i).data.size(),
                    "\t void graph_labeler::operator()"
                        << "\n\t The size of the node weight vector must match "
                           "the one in the node."
      ...
      for (unsigned long n = 0; n < sample.node(i).number_of_neighbors(); ++n) {
        if (is_matrix<vector_type>::value &&
          ...
          // check that dot() is legal.
          DLIB_ASSERT((unsigned long)get_edge_weights().size() ==
                          (unsigned long)sample.node(i).edge(n).size(),
                      "\t void graph_labeler::operator()"
                          << "\n\t The size of the edge weight vector must "
                             "match the one in graph's edge."
        ...
        DLIB_ASSERT(
            sample.node(i).edge(n).size() == 0 ||
                min(sample.node(i).edge(n)) >= 0,
            "\t void graph_labeler::operator()"
...
void deserialize(graph_labeler<vector_type> &item, std::istream &in) {
  ...
  if (version != 1) {
    throw dlib::serialization_error("While deserializing graph_labeler, found "
                                    "unexpected version number of " +
