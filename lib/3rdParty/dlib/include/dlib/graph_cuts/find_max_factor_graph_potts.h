...
template <typename potts_problem, typename T = void> class flows_container {
  /*
...
public:
  void setup(const potts_problem &p) {
  ...
  edge_type &operator()(const long r, const long c) { return flows[r][c]; }

  const edge_type &operator()(const long r, const long c) const {
    return flows[r][c];
  }
};
...
class flows_container<
  ...
  */
  typedef typename potts_problem::value_type edge_type;
  const static unsigned long max_number_of_neighbors =
      potts_problem::max_number_of_neighbors;
...
template <typename potts_problem> class potts_flow_graph {
public:
  typedef typename potts_problem::value_type edge_type;
...
public:
  potts_flow_graph(potts_problem &g_) : g(g_) {
    flows.setup(g);
  ...
  public:
    out_edge_iterator() : idx(0), cnt(0) {}

    out_edge_iterator(unsigned long idx_, unsigned long cnt_)
        : idx(idx_), cnt(cnt_) {}
  ...
  class in_edge_iterator {
    ...
    unsigned long cnt; // count over the neighbors of idx
  public:
    in_edge_iterator() : idx(0), cnt(0) {}

    in_edge_iterator(unsigned long idx_, unsigned long cnt_)
        : idx(idx_), cnt(cnt_) {}
  ...
  unsigned long number_of_nodes() const { return g.number_of_nodes() + 2; }

  out_edge_iterator out_begin(const unsigned long &it) const {
    return out_edge_iterator(it, 0);
  }

  in_edge_iterator in_begin(const unsigned long &it) const {
    return in_edge_iterator(it, 0);
  }

  out_edge_iterator out_end(const unsigned long &it) const {
  ...
  in_edge_iterator in_end(const unsigned long &it) const {
    if (it >= g.number_of_nodes())
  ...
  unsigned long node_id(const iterator_type &it) const {
  ...
  void adjust_flow(const unsigned long &it1, const unsigned long &it2,
  ...
  void set_label(const unsigned long &it, node_label value) {
    if (it < g.number_of_nodes())
  ...
  node_label get_label(const unsigned long &it) const {
    if (it < g.number_of_nodes())
...
template <typename label_image_type, typename image_potts_model>
...
public:
  ...
  potts_grid_problem(label_image_type &label_img_,
                     const image_potts_model &image_potts_model_)
      : label_img(label_img_), model(image_potts_model_) {
  ...
  unsigned long number_of_neighbors(unsigned long) const { return 4; }

  unsigned long get_neighbor_idx(long node_id1, long node_id2) const {
  ...
  unsigned long get_neighbor(long node_id, long idx) const {
    switch (idx) {
    case 0: {
      long temp = node_id + 1;
...
typename potts_model::value_type potts_model_score(const potts_model &prob) {
    ...
    for (unsigned long jj = 0; jj < prob.number_of_neighbors(i); ++jj) {
      ...
      DLIB_ASSERT(prob.factor_value_disagreement(i, j) >= 0,
                  "\t value_type potts_model_score(prob)"
                      ...
                      << prob.factor_value_disagreement(i, j));
      DLIB_ASSERT(prob.factor_value_disagreement(i, j) ==
...
potts_model_score(const potts_grid_problem &prob,
  ...
  dlib::impl::potts_grid_problem<image_type, potts_grid_problem> model(
      const_cast<image_type &>(labels), prob);
...
template <typename potts_model>
void find_max_factor_graph_potts(potts_model &prob) {
#ifdef ENABLE_ASSERTS
  for (unsigned long node_i = 0; node_i < prob.number_of_nodes(); ++node_i) {
    for (unsigned long jj = 0; jj < prob.number_of_neighbors(node_i); ++jj) {
      ...
      DLIB_ASSERT(
          prob.get_neighbor_idx(node_i, prob.get_neighbor(node_i, jj)) == jj,
          "\t void find_max_factor_graph_potts(prob)"
              << "\n\t The get_neighbor_idx() and get_neighbor() functions "
              ...
              << prob.get_neighbor_idx(node_i, node_j));

      DLIB_ASSERT(prob.get_neighbor(
                      node_j, prob.get_neighbor_idx(node_j, node_i)) == node_i,
                  "\t void find_max_factor_graph_potts(prob)"
                      << "\n\t The get_neighbor_idx() and get_neighbor() "
                         "functions must be inverses of each other."
                      << "\n\t node_i: " << node_i << "\n\t node_j: " << node_j
                      << "\n\t prob.get_neighbor_idx(node_j,node_i): "
                      << prob.get_neighbor_idx(node_j, node_i)
                      << "\n\t "
                         "prob.get_neighbor(node_j,prob.get_neighbor_idx(node_"
                         "j,node_i)): "
                      << prob.get_neighbor(
                             node_j, prob.get_neighbor_idx(node_j, node_i)));

      DLIB_ASSERT(prob.factor_value_disagreement(node_i, node_j) >= 0,
                  "\t void find_max_factor_graph_potts(prob)"
                      << "\n\t Invalid inputs were given to this function."
                      << "\n\t node_i: " << node_i << "\n\t node_j: " << node_j
                      << "\n\t prob.factor_value_disagreement(node_i,node_j): "
                      << prob.factor_value_disagreement(node_i, node_j));
      DLIB_ASSERT(prob.factor_value_disagreement(node_i, node_j) ==
                      prob.factor_value_disagreement(node_j, node_i),
                  "\t void find_max_factor_graph_potts(prob)"
                      << "\n\t Invalid inputs were given to this function."
                      << "\n\t node_i: " << node_i << "\n\t node_j: " << node_j
                      << "\n\t prob.factor_value_disagreement(node_i,node_j): "
...
void find_max_factor_graph_potts(const potts_grid_problem &prob,
  ...
  dlib::impl::potts_grid_problem<image_type, potts_grid_problem> model(labels,
                                                                       prob);
...
namespace impl {
template <typename pixel_type1, typename pixel_type2, typename model_type>
struct potts_grid_image_pair_model {
  ...
  potts_grid_image_pair_model(const model_type &model_, const image_type1 &img1,
                              const image_type2 &img2)
      : model(model_), nr_(img1.nr()), nc_(img1.nc()) {
    data1 = &img1[0][0];
    data2 = &img2[0][0];
  }
...
template <typename image_type, typename model_type>
struct potts_grid_image_single_model {
  const typename image_type::type *data1;
  const model_type &model;
  const long nr_;
  const long nc_;
  potts_grid_image_single_model(const model_type &model_,
...
impl::potts_grid_image_pair_model<pixel_type1, pixel_type2, pair_image_model>
make_potts_grid_problem(const pair_image_model &model,
  ...
  typedef impl::potts_grid_image_pair_model<pixel_type1, pixel_type2,
                                            pair_image_model>
      potts_type;
...
template <typename single_image_model, typename pixel_type,
          typename mem_manager>
impl::potts_grid_image_single_model<array2d<pixel_type, mem_manager>,
                                    single_image_model>
make_potts_grid_problem(const single_image_model &model,
                        const array2d<pixel_type, mem_manager> &img) {
  typedef impl::potts_grid_image_single_model<array2d<pixel_type, mem_manager>,
                                              single_image_model>
      potts_type;
