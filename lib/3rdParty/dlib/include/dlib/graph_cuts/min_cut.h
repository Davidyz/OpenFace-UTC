...
typename disable_if<is_directed_graph<flow_graph>,
                    typename flow_graph::edge_type>::type
graph_cut_score(const flow_graph &g) {
...
typename enable_if<is_directed_graph<directed_graph>,
                   typename directed_graph::edge_type>::type
graph_cut_score(const directed_graph &g) {
  return graph_cut_score(
      dlib::impl::general_flow_graph<const directed_graph>(g));
}
...
public:
  ...
  template <typename directed_graph>
  typename enable_if<is_directed_graph<directed_graph>>::type
  operator()(directed_graph &g, const unsigned long source_node,
  ...
  template <typename flow_graph>
  typename disable_if<is_directed_graph<flow_graph>>::type
  operator()(flow_graph &g, const unsigned long source_node,
             const unsigned long sink_node) const {
#ifdef ENABLE_ASSERTS
    DLIB_ASSERT(source_node != sink_node && source_node < g.number_of_nodes() &&
...
private:
  unsigned long distance_to_origin(const unsigned long no_parent,
                                   unsigned long p, unsigned long) const {
  ...
  template <typename flow_graph>
  void adopt(flow_graph &g, const unsigned long source,
             const unsigned long sink) const {
    typedef typename flow_graph::out_edge_iterator out_edge_iterator;
    typedef typename flow_graph::in_edge_iterator in_edge_iterator;
  ...
  template <typename flow_graph>
  void augment(flow_graph &g, const unsigned long &source,
               const unsigned long &sink, const unsigned long &source_side,
               const unsigned long &sink_side) const {
    typedef typename flow_graph::edge_type edge_type;
  ...
  template <typename flow_graph>
  bool grow(flow_graph &g, unsigned long &source_side, unsigned long &sink_side,
            typename flow_graph::in_edge_iterator &in_begin,
            typename flow_graph::out_edge_iterator &out_begin) const
  /*!
      ensures
          - if (an augmenting path was found) then
              - returns true
              - (#source_side, #sink_side) == the point where the two trees
  meet. #source_side is part of the source tree and #sink_side is part of the
  sink tree.
          - else
              - returns false
  !*/
  ...
  inline bool is_closer(unsigned long p, unsigned long q) const {
