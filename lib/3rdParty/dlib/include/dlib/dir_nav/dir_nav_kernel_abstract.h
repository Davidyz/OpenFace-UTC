...
template <typename queue_of_dir> void get_filesystem_roots(queue_of_dir &roots);
/*!
    requires
        - queue_of_dirs == an implementation of queue/queue_kernel_abstract.h
with T set to directory or a std::vector<directory> or
dlib::std_vector_c<directory>. ensures
        - #roots == a queue containing directories that represent all the roots
          of the filesystem on this machine.   (e.g. in windows you have c:\,
d:\ etc.) throws
        - std::bad_alloc
!*/
...
public:
  ...
  std::chrono::time_point<std::chrono::system_clock> last_modified() const;
  /*!
      ensures
  ...
  bool operator<(const file &item) const;
  /*!
      ensures
...
public:
  ...
  directory(const directory &item);
  /*!
      ensures
          - #*this == item
      throws
  ...
  /*!
      ensures
          - returns the character used to separate directories and file names in
  a path.  (i.e.  \ on windows and / in unix)
  !*/
  ...
  /*!
      requires
          - queue_of_files == an implementation of queue/queue_kernel_abstract.h
  with T set to file or a std::vector<file> or dlib::std_vector_c<file>. ensures
          - #files == A queue containing all the files present in this
  directory. (Note that symbolic links will not have been resolved in the names
      ...
      throws
          ...
          - listing_error
              ...
              directory or if some error occurred that prevented us from
  successfully getting the contents of this directory. If this exception is
  thrown then the call to get_files() has no effect on *this and
  #files.size()==0.
  !*/
  ...
  /*!
      ensures
          - This function simply calls get_files(temp_vector) and then returns
  temp_vector.
  !*/
  ...
  with T set to directory or a std::vector<directory> or
  dlib::std_vector_c<directory>. ensures
          - #dirs == a queue containing all the directories present in this
  directory. (note that symbolic links will not have been resolved in the names
      ...
      throws
          ...
          - listing_error
              ...
              directory or if some error occurred that prevented us from
  successfully getting the contents of this directory. If this exception is
  thrown then the call to get_dirs() has no effect on *this and #dirs.size()==0.
  !*/

  std::vector<directory> get_dirs() const;
  /*!
      ensures
          - This function simply calls get_dirs(temp_vector) and then returns
  temp_vector.
  !*/
  ...
  /*!
      ensures
          - returns full_name()
            (i.e. provides an implicit conversion to string from
  dlib::directory)
  !*/
  ...
  bool operator<(const directory &item) const;
  /*!
      ensures
          - if (full_name() < item.full_name()) then
              - returns true
