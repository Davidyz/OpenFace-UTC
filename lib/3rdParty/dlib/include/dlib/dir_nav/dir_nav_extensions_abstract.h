...
get_files_in_directory_tree(const directory &top_of_tree, const T &add_file,
...
/*!
    ...
    ensures
        - performs a recursive search through the directory top_of_tree and all
          ...
          returns true then they will be included in the returned
std::vector<file> object.
...
public:
  match_ending(const std::string &ending);
  /*!
      ensures
...
public:
  match_endings(const std::string &ending_list);
  /*!
      ensures
...
directory get_parent_directory(const directory &dir);
/*!
    ensures
...
directory get_parent_directory(const file &f);
/*!
    ensures
        - if (f.full_name() != "") then
...
std::string select_oldest_file(const std::string &filename1,
                               const std::string &filename2);
/*!
    ensures
        - Checks the last modification times of the two given files and returns
the filename of the oldest file, i.e., the file that has gone longest since
being modified.  Ties are broken arbitrarily.
        - For the purpose of comparison, a file that doesn't exist is presumed
to have a last modification time of -infinity (i.e. very far in the past).
!*/

// ----------------------------------------------------------------------------------------

std::string select_newest_file(const std::string &filename1,
                               const std::string &filename2);
/*!
    ensures
        - Checks the last modification times of the two given files and returns
the filename that was most recently modified.  Ties are broken arbitrarily.
        - For the purpose of comparison, a file that doesn't exist is presumed
to have a last modification time of -infinity (i.e. very far in the past).
!*/
