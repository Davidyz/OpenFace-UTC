...
void get_all_sub_dirs(const directory &top_of_tree, unsigned long max_depth,
...
get_files_in_directory_tree(const directory &top_of_tree, const T &add_file,
  ...
  implementation_details::get_all_sub_dirs(top_of_tree, max_depth, dirs,
                                           dirs_temp);

  // now just loop over all the directories and pick out the files we want to
  // keep
...
public:
  match_ending(const std::string &ending_) : ending(ending_) {}
...
public:
  match_endings(const std::string &endings_) {
