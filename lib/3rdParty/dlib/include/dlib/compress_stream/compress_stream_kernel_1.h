...
template <typename fce, typename fcd, typename crc32>
...
private:
  ...
  compress_stream_kernel_1 &
  operator=(compress_stream_kernel_1 &); // assignment operator
};
...
template <typename fce, typename fcd, typename crc32>
void compress_stream_kernel_1<fce, fcd, crc32>::compress(
    std::istream &in_, std::ostream &out_) const {
...
template <typename fce, typename fcd, typename crc32>
void compress_stream_kernel_1<fce, fcd, crc32>::decompress(
    std::istream &in_, std::ostream &out_) const {
  ...
  while (true) {
    ...
    if (symbol != eof_symbol) {
      ...
      // we read eof from the encoded data.  now we just have to check the
