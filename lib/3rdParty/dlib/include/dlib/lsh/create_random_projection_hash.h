...
projection_hash create_random_projection_hash(const vector_type &v,
  ...
  DLIB_ASSERT(0 < bits && bits <= 32 && v.size() > 1,
  ...
  for (unsigned long i = 0; i < v.size(); ++i) {
    DLIB_ASSERT(v[0].size() == v[i].size() && v[i].size() > 0 &&
                    is_col_vector(v[i]),
                "\t projection_hash create_random_projection_hash()"
                    << "\n\t Invalid arguments were given to this function."
                    << "\n\t m(0).size(): " << v[0].size() << "\n\t m(" << i
...
create_max_margin_projection_hash(const vector_type &v, const int bits,
  ...
  // We set this up to use enough samples to cover the vector space used by
  // elements of v.
  ...
  for (int itr = 0; itr < offset.size(); ++itr) {
    ...
    decision_function<linear_kernel<sample_type>> df =
        trainer.train(training_samples, training_labels);
    offset(itr) = -df.b;
