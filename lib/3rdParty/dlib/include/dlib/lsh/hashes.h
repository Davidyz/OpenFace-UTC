...
public:
  hash_similar_angles_64() : seed(0) {}

  hash_similar_angles_64(const uint64 seed_) : seed(seed_) {}

  uint64 get_seed() const { return seed; }

  typedef uint64 result_type;

  template <typename sparse_vector_type>
  typename disable_if<is_matrix<sparse_vector_type>, uint64>::type
  operator()(const sparse_vector_type &v) const {
    ...
    for (int i = 0; i < 64; ++i) {
      ...
      scalar_type val = 0;
      for (typename sparse_vector_type::const_iterator j = v.begin();
           j != v.end(); ++j)
  ...
  template <typename EXP> uint64 operator()(const matrix_exp<EXP> &v) const {
    typedef typename EXP::type T;
    uint64 temp = 0;
    for (unsigned long i = 0; i < 64; ++i) {
      if (dot(matrix_cast<T>(gaussian_randm(v.size(), 1, i + seed * 64)), v) >
          0)
        temp |= 1;
      temp <<= 1;
    }
...
public:
  hash_similar_angles_128() : seed(0), hasher1(0), hasher2(1) {}

  hash_similar_angles_128(const uint64 seed_)
...
public:
  hash_similar_angles_256() : seed(0), hasher1(0), hasher2(1) {}

  hash_similar_angles_256(const uint64 seed_)
      : seed(seed_), hasher1(2 * seed), hasher2(2 * seed + 1) {}
...
public:
  hash_similar_angles_512() : seed(0), hasher1(0), hasher2(1) {}

  hash_similar_angles_512(const uint64 seed_)
      : seed(seed_), hasher1(2 * seed), hasher2(2 * seed + 1) {}
