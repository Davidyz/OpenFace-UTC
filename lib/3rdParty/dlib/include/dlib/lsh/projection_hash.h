...
public:
  ...
  projection_hash(const matrix_exp<EXP1> &proj_,
  ...
  const matrix<double> &get_projection_matrix() const { return proj; }

  const matrix<double, 0, 1> &get_offset_matrix() const { return offset; }
  ...
  unsigned long operator()(const matrix_exp<EXP> &v) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(is_col_vector(v) && v.size() == get_projection_matrix().nc() &&
                    v.size() > 0,
                "\t unsigned long projection_hash::operator()(v)"
                    << "\n\t Invalid arguments were given to this function."
                    << "\n\t is_col_vector(v):             " << is_col_vector(v)
                    << "\n\t get_projection_matrix().nc(): "
                    << get_projection_matrix().nc()
                    << "\n\t v.size():                     " << v.size());
...
private:
  template <typename EXP>
  unsigned long do_hash(const matrix_exp<EXP> &v) const {
    unsigned long h = 0;
...
inline void serialize(const projection_hash &item, std::ostream &out) {
  serialize(item.get_projection_matrix(), out);
  serialize(item.get_offset_matrix(), out);
}

inline void deserialize(projection_hash &item, std::istream &in) {
  matrix<double> proj;
  matrix<double, 0, 1> offset;
  deserialize(proj, in);
