...
public:
  ...
  explicit upper_bound_function(const std::vector<function_evaluation> &_points,
                                const double relative_noise_magnitude = 0.001,
                                const double solver_eps = 0.0001)
      : relative_noise_magnitude(relative_noise_magnitude),
        solver_eps(solver_eps), points(_points) {
    DLIB_CASSERT(relative_noise_magnitude >= 0);
    DLIB_CASSERT(solver_eps > 0);
    ...
    if (points.size() > 1) {
      ...
      for (auto &p : points)
        DLIB_CASSERT(p.x.size() == dims,
                     "All the vectors given to upper_bound_function must have "
  ...
  void add(const function_evaluation &point) {
    ...
    DLIB_CASSERT(point.x.size() == dimensionality(),
                 "All the vectors given to upper_bound_function must have the "
                 "same dimensionality.");

    if (points.size() < 4) {
      points.push_back(point);
      *this =
          upper_bound_function(points, relative_noise_magnitude, solver_eps);
      return;
    }
  ...
  long num_points() const { return points.size(); }
  ...
  const std::vector<function_evaluation> &get_points() const { return points; }
  ...
  double operator()(const matrix<double, 0, 1> &x) const {
    ...
    for (size_t i = 0; i < points.size(); ++i) {
      const double local_bound =
          points[i].y +
          std::sqrt(offsets[i] + dot(slopes, squared(x - points[i].x)));
...
private:
  void learn_params() {
    ...
    // We are going to normalize the data so the values aren't extreme.  First,
    // we collect statistics on our data.
    ...
    // compute normalization vectors for the data.  The only reason we do this
    // is to make the optimization well conditioned.  In particular, scaling the
    // y values will prevent numerical errors in the 1-diff*diff computation
    // below that would otherwise result when diff is really big.  Also, scaling
    // the xvalues to be about 1 will similarly make the optimization more
    // stable and it also has the added benefit of keeping the
    // relative_noise_magnitude's scale constant regardless of the size of x
    // values.
    ...
    for (size_t i = 0; i < xscale.size(); ++i)
      xscale[i] =
          1.0 /
          (x_rs[i].stddev() *
           yscale); // make it so that xscale[i]*yscale ==  1/x_rs[i].stddev()
    ...
    // save the active constraints for later so we can use them inside add() to
    ...
    // std::cout << "active_constraints.size(): " << active_constraints.size()
    // << std::endl;
  ...
  std::vector<std::pair<size_t, size_t>> active_constraints,
