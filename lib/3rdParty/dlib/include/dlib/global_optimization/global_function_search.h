...
struct outstanding_function_eval_request {
  ...
  double anchor_objective_value =
      std::numeric_limits<double>::quiet_NaN(); // objective value at center of
                                                // TR step

  bool operator==(const outstanding_function_eval_request &item) const {
...
struct funct_info {
  ...
  funct_info(const function_spec &spec, size_t function_idx,
             const std::shared_ptr<std::mutex> &m)
      : spec(spec), function_idx(function_idx), m(m) {
    best_x = zeros_matrix(spec.lower);
  }

  upper_bound_function build_upper_bound_with_all_function_evals() const;

  static double find_nn(const std::vector<function_evaluation> &evals,
...
public:
  ...
  explicit global_function_search(const std::vector<function_spec> &functions_);

  global_function_search(const std::vector<function_spec> &functions_,
...
private:
  ...
  bool has_outstanding_trust_region_request() const;
