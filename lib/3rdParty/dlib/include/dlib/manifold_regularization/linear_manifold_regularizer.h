...
class undirected_adjacency_list {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This object is simply a tool for turning a vector of sample_pair
  objects into an adjacency list with floating point weights on each edge.
  !*/
public:
  ...
  const_iterator begin(unsigned long idx) const
  /*!
      requires
          - idx < size()
      ensures
  ...
  const_iterator end(unsigned long idx) const
  /*!
      requires
          - idx < size()
      ensures
  ...
  void build(const vector_type &edges, const weight_function_type &weight_funct)
  /*!
      requires
  ...
  a floating point number >= 0 ensures
          ...
          - The weight in each neighbor is set to the output of the
  weight_funct() for the associated edge.
  !*/
  {
    ...
    for (unsigned long i = 0; i < edges.size(); ++i) {
      ...
      const unsigned long min_size =
          std::max(edges[i].index1(), edges[i].index2()) + 1;
    ...
    // Now setup the iterators in blocks.  Also setup a version of blocks that
    // holds non-const iterators so we can use it below when we populate data.
    ...
    for (unsigned long i = 0; i < edges.size(); ++i) {
      ...
      DLIB_ASSERT(weight >= 0, "\t void linear_manifold_regularizer::build()"
                                   << "\n\t You supplied a weight_funct() that "
                                      "generated a negative weight."
                                   << "\n\t weight: " << weight);

      *mutable_blocks[edges[i].index1()]++ =
          neighbor(edges[i].index2(), weight);
      *mutable_blocks[edges[i].index2()]++ =
          neighbor(edges[i].index1(), weight);
    }
  }

  double sum_of_edge_weights() const { return sum_edge_weights; }
  ...
  /*!
      ...
      CONVENTION
          ...
          - blocks == a vector of iterators that point into data.
            For all valid i:
              - The iterator range [blocks[i], blocks[i+1]) contains all the
  edges for the i'th node in the graph
  !*/
...
public:
  ...
  void build(const vector_type1 &samples, const vector_type2 &edges,
    ...
    // make sure requires clause is not broken
    DLIB_ASSERT(edges.size() > 0 && contains_duplicate_pairs(edges) == false &&
  ...
  long dimensionality() const { return reg_mat.nr(); }
  ...
  general_matrix get_transformation_matrix(
    ...
    // This isn't how it's defined in the referenced paper but normalizing these
    // kinds of sums is typical of most machine learning algorithms.  Moreover,
    // doing this makes the argument to this function more invariant to the size
    // of the edge set.  So it should make it easier for the user.
    intrinsic_regularization_strength /= sum_edge_weights;

    return inv_lower_triangular(
        chol(identity_matrix<scalar_type>(reg_mat.nr()) +
             intrinsic_regularization_strength * reg_mat));
  }

private:
  template <typename vector_type>
  void make_mr_matrix(const vector_type &samples,
