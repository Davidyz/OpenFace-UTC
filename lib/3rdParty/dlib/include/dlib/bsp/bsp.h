...
namespace impl1 {
inline void null_notify(unsigned short) {}

struct bsp_con {
  bsp_con(const network_address &dest)
      : con(connect(dest)), buf(con), stream(&buf), terminated(false) {
    con->disable_nagle();
  }

  bsp_con(std::unique_ptr<connection> &conptr)
      : buf(conptr), stream(&buf), terminated(false) {
...
typedef dlib::map<unsigned long, std::unique_ptr<bsp_con>>::kernel_1a_c
    map_id_to_con;

void connect_all(map_id_to_con &cons, const std::vector<network_address> &hosts,
...
void send_out_connection_orders(map_id_to_con &cons,
...
struct hostinfo {
  hostinfo() {}
  hostinfo(const network_address &addr_, unsigned long node_id_)
      : addr(addr_), node_id(node_id_) {}

  network_address addr;
  unsigned long node_id;
};

inline void serialize(const hostinfo &item, std::ostream &out) {
  dlib::serialize(item.addr, out);
  dlib::serialize(item.node_id, out);
}

inline void deserialize(hostinfo &item, std::istream &in) {
...
void connect_all_hostinfo(map_id_to_con &cons,
                          const std::vector<hostinfo> &hosts,
                          unsigned long node_id, std::string &error_string);

// ------------------------------------------------------------------------------------

template <typename port_notify_function_type>
void listen_and_connect_all(unsigned long &node_id, map_id_to_con &cons,
                            unsigned short port,
  ...
  if (status == PORTINUSE) {
    throw socket_error("Unable to create listening port " +
                       cast_to_string(port) + ".  The port is already in use");
  } else if (status != 0) {
    throw socket_error("Unable to create listening port " +
                       cast_to_string(port));
  }
  ...
  thread_function thread(connect_all_hostinfo, dlib::ref(cons2),
                         dlib::ref(targets), node_id, dlib::ref(error_string));
...
private:
  struct msg_wrap {
    msg_wrap(const msg_data &data_, const dlib::uint64 &sequence_number_)
...
public:
  ...
  ~thread_safe_message_queue() { disable(); }
  ...
  void push_and_consume(msg_data &item) {
    ...
    // do this here so that we don't have to worry about different threads
    // touching the shared_ptr.
    item.data.reset();
    sig.signal();
  }

  bool pop(msg_data &item)
  /*!
      ensures
  ...
  bool pop(msg_data &item, const dlib::uint64 &max_epoch)
  /*!
      ensures
          - if (this function returns true) then
              - #item == the next thing from the queue that has an epoch <=
  max_epoch
          - else
              - this object is disabled
  !*/
...
public:
  ...
  unsigned long node_id() const { return _node_id; }

  unsigned long number_of_nodes() const { return _cons.size() + 1; }
  ...
  template <typename T> void receive(T &item) {
    if (!try_receive(item))
      throw dlib::socket_error("bsp_context::receive(): no messages to "
                               "receive, all nodes currently blocked.");
  }
    ...
    if (receive_data(temp, sending_node_id)) {
      ...
      if (sin.peek() != EOF)
        throw serialization_error(
            ...
            "This probably means you are calling a receive method with a "
            "different type "
...
private:
  ...
  bsp_context(unsigned long node_id_, impl1::map_id_to_con &cons_);
  ...
  bool receive_data(std::shared_ptr<std::vector<char>> &item,
                    unsigned long &sending_node_id);

  void notify_control_node(char val);

  void broadcast_byte(char val);

  void send_data(const std::vector<char> &item, unsigned long target_node_id);
  /*!
      requires
  ...
  friend void bsp_connect(const std::vector<network_address> &hosts,
                          funct_type funct);
  ...
  friend void
  bsp_listen_dynamic_port(unsigned short listening_port,
                          port_notify_function_type port_notify_function,
                          funct_type funct);
