...
public:
  ...
  size_t size() const noexcept { return disjoint_subsets_.size(); }

  unsigned long find_set(unsigned long item) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(item < size(), "\t unsigned long disjoint_subsets::find_set()"
  ...
  unsigned long merge_sets(unsigned long a, unsigned long b) {
    // make sure requires clause is not broken
    DLIB_ASSERT(a != b && a < size() && b < size() && find_set(a) == a &&
                    find_set(b) == b,
  ...
  unsigned long get_number_of_sets() const noexcept { return number_of_sets; }

  unsigned long get_size_of_set(unsigned long item) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(item < size() && find_set(item) == item,
...
private:
  /*
      See the book Introduction to Algorithms by Cormen, Leiserson, Rivest and
     Stein for a discussion of how this algorithm works.
  */
