...
#define TIME_THIS_TO(_tt_op, _tt_out)                                          \
  {                                                                            \
    ...
    if (_tt_thetime >= std::chrono::minutes(1))                                \
      _tt_out << "\ntime: "                                                    \
              << duration_cast<duration<double, std::ratio<60>>>(_tt_thetime)  \
                     .count()                                                  \
              << "min\n";                                                      \
    else if (_tt_thetime >= std::chrono::seconds(1))                           \
      _tt_out << "\ntime: "                                                    \
              << duration_cast<duration<double>>(_tt_thetime).count()          \
              << "sec\n";                                                      \
    else if (_tt_thetime >= std::chrono::milliseconds(1))                      \
      _tt_out                                                                  \
          << "\ntime: "                                                        \
          << duration_cast<duration<double, std::milli>>(_tt_thetime).count()  \
          << "ms\n";                                                           \
    else if (_tt_thetime >= std::chrono::microseconds(1))                      \
      _tt_out                                                                  \
          << "\ntime: "                                                        \
          << duration_cast<duration<double, std::micro>>(_tt_thetime).count()  \
          << "us\n";                                                           \
    else                                                                       \
      _tt_out                                                                  \
          << "\ntime: "                                                        \
          << duration_cast<duration<double, std::nano>>(_tt_thetime).count()   \
          << "ns\n";                                                           \
  }
