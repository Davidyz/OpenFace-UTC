...
class stack : public enumerable<T>, public remover<T> {
...
inline void swap(stack<T, mem_manager> &a, stack<T, mem_manager> &b) {
  a.swap(b);
}
/*!
...
template <typename T, typename mem_manager>
void deserialize(stack<T, mem_manager> &item, std::istream &in);
/*!
