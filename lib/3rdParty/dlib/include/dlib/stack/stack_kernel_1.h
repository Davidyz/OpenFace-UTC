...
class stack_kernel_1 : public enumerable<T>, public remover<T> {
...
public:
  ...
  stack_kernel_1()
      : top(0), stack_size(0), current_element(0), _at_start(true) {}
  ...
  inline void swap(stack_kernel_1 &item);
...
private:
  void delete_elements_in_stack(node *&top);
  /*!
      requires
...
inline void swap(stack_kernel_1<T, mem_manager> &a,
                 stack_kernel_1<T, mem_manager> &b) {
  a.swap(b);
}

template <typename T, typename mem_manager>
void deserialize(stack_kernel_1<T, mem_manager> &item, std::istream &in) {
  try {
    item.clear();
    unsigned long size;
    deserialize(size, in);
...
template <typename T, typename mem_manager>
void stack_kernel_1<T, mem_manager>::clear() {
  if (stack_size != 0) {
    delete_elements_in_stack(top);
    stack_size = 0;
  }
...
template <typename T, typename mem_manager>
T &stack_kernel_1<T, mem_manager>::current() {
  return top->item;
}
...
template <typename T, typename mem_manager>
const T &stack_kernel_1<T, mem_manager>::current() const {
  return top->item;
}
...
template <typename T, typename mem_manager>
void stack_kernel_1<T, mem_manager>::swap(
...
template <typename T, typename mem_manager>
void stack_kernel_1<T, mem_manager>::push(T &item) {
...
template <typename T, typename mem_manager>
void stack_kernel_1<T, mem_manager>::pop(T &item) {
  node *old_node = top;
  top = top->next;
...
template <typename T, typename mem_manager>
void stack_kernel_1<T, mem_manager>::delete_elements_in_stack(node *&top) {
  node *temp;
  while (top != 0) {
    temp = top->next;
    pool.deallocate(top);
...
template <typename T, typename mem_manager>
size_t stack_kernel_1<T, mem_manager>::size() const {
  return stack_size;
}
...
template <typename T, typename mem_manager>
bool stack_kernel_1<T, mem_manager>::at_start() const {
  return _at_start;
}
...
template <typename T, typename mem_manager>
void stack_kernel_1<T, mem_manager>::reset() const {
  _at_start = true;
  current_element = 0;
}
...
template <typename T, typename mem_manager>
bool stack_kernel_1<T, mem_manager>::current_element_valid() const {
...
template <typename T, typename mem_manager>
const T &stack_kernel_1<T, mem_manager>::element() const {
...
template <typename T, typename mem_manager>
T &stack_kernel_1<T, mem_manager>::element() {
  return current_element->item;
}
...
template <typename T, typename mem_manager>
bool stack_kernel_1<T, mem_manager>::move_next() const {
  if (!_at_start) {
    if (current_element) {
...
template <typename T, typename mem_manager>
void stack_kernel_1<T, mem_manager>::remove_any(T &item) {
  pop(item);
}
