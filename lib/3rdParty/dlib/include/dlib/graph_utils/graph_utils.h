...
template <typename T>
typename enable_if<is_graph<T>, typename T::edge_type>::type &
edge(T &g, unsigned long idx_i, unsigned long idx_j) {
  ...
  // a return here
  DLIB_CASSERT(false, "\tT::edge_type& edge(g, idx_i, idx_j)"
                          << "\n\t you have requested an invalid edge"
                          << "\n\t idx_i: " << idx_i
                          << "\n\t idx_j: " << idx_j);
}
...
template <typename T>
const typename enable_if<is_graph<T>, typename T::edge_type>::type &
edge(const T &g, unsigned long idx_i, unsigned long idx_j) {
  // make sure requires clause is not broken
  DLIB_ASSERT(g.has_edge(idx_i, idx_j) == true,
              "\tT::edge_type& edge(g, idx_i, idx_j)"
                  << "\n\t you have requested an invalid edge"
...
template <typename T>
typename enable_if<is_directed_graph<T>, typename T::edge_type>::type &
edge(T &g, unsigned long parent_idx, unsigned long child_idx) {
  // make sure requires clause is not broken
  ...
  // put this here just so compilers don't complain about a lack of
  // a return here
  DLIB_CASSERT(false, "\t T::edge_type& edge(g, parent_idx, child_idx)"
                          << "\n\t you have requested an invalid edge"
                          << "\n\t parent_idx: " << parent_idx
                          << "\n\t child_idx: " << child_idx);
}
...
template <typename T>
const typename enable_if<is_directed_graph<T>, typename T::edge_type>::type &
edge(const T &g, unsigned long parent_idx, unsigned long child_idx) {
  // make sure requires clause is not broken
  DLIB_ASSERT(g.has_edge(parent_idx, child_idx) == true,
              "\t T::edge_type& edge(g, parent_idx, child_idx)"
                  << "\n\t you have requested an invalid edge"
  ...
  // put this here just so compilers don't complain about a lack of
  // a return here
  DLIB_ASSERT(false, "\t T::edge_type& edge(g, parent_idx, child_idx)"
                         << "\n\t you have requested an invalid edge"
                         << "\n\t parent_idx: " << parent_idx
                         << "\n\t child_idx: " << child_idx);
}
...
inline bool is_same_object(const T &a, const U &b) {
...
template <typename T>
bool search_for_directed_cycles(const T &node, std::vector<bool> &visited,
                                std::vector<bool> &temp)
/*!
    requires
        - visited.size() >= number of nodes in the graph that contains the given
node
        - temp.size() >= number of nodes in the graph that contains the given
node
        - for all i in temp:
            - temp[i] == false
    ensures
        - checks the connected subgraph containing the given node for directed
cycles and returns true if any are found and false otherwise.
...
template <typename T>
typename enable_if<is_directed_graph<typename T::graph_type>, bool>::type
search_for_undirected_cycles(
    const T &node, std::vector<bool> &visited,
...
template <typename T>
typename enable_if<is_graph<typename T::graph_type>, bool>::type
search_for_undirected_cycles(
    const T &node, std::vector<bool> &visited,
    unsigned long prev = std::numeric_limits<unsigned long>::max())
/*!
...
template <typename graph_type1, typename graph_type2>
typename enable_if<is_graph<graph_type1>>::type
copy_graph_structure(const graph_type1 &src, graph_type2 &dest) {
...
template <typename graph_type1, typename graph_type2>
typename enable_if<is_directed_graph<graph_type1>>::type
copy_graph_structure(const graph_type1 &src, graph_type2 &dest) {
...
template <typename graph_type1, typename graph_type2>
typename enable_if<is_graph<graph_type1>>::type
copy_graph(const graph_type1 &src, graph_type2 &dest) {
  COMPILE_TIME_ASSERT(is_graph<graph_type1>::value);
  COMPILE_TIME_ASSERT(is_graph<graph_type2>::value);
  if (graph_helpers::is_same_object(src, dest))
    return;
...
typename enable_if<is_graph<typename T::graph_type>>::type
find_connected_nodes(const T &n, S &visited) {
...
template <typename T, typename S>
typename enable_if<is_directed_graph<typename T::graph_type>>::type
find_connected_nodes(const T &n, S &visited) {
  if (visited.is_member(n.index()) == false) {
    unsigned long temp = n.index();
    visited.add(temp);
...
template <typename T> bool graph_is_connected(const T &g) {
...
template <typename T> bool graph_has_symmetric_edges(const T &graph) {
...
template <typename T> bool graph_contains_directed_cycle(const T &graph) {
  using namespace std;
...
template <typename T> bool graph_contains_undirected_cycle(const T &graph) {
  using namespace std;
  using namespace graph_helpers;
  std::vector<bool> visited(graph.number_of_nodes(), false);
...
template <typename directed_graph_type, typename graph_type>
void create_moral_graph(const directed_graph_type &g, graph_type &moral_graph) {
  // make sure requires clause is not broken
  DLIB_ASSERT(graph_contains_directed_cycle(g) == false,
              "\tvoid create_moral_graph(g, moral_graph)"
                  << "\n\tYou can only make moral graphs if g doesn't have "
                     "directed cycles");
...
template <typename graph_type, typename sets_of_int>
bool is_clique(const graph_type &g, const sets_of_int &clique) {
  // make sure requires clause is not broken
...
template <typename graph_type, typename sets_of_int>
bool is_maximal_clique(const graph_type &g, const sets_of_int &clique) {
  // make sure requires clause is not broken
  DLIB_ASSERT(graph_contains_length_one_cycle(g) == false,
...
template <typename T>
typename enable_if<is_directed_graph<T>, bool>::type
graph_contains_length_one_cycle(const T &graph) {
  for (unsigned long i = 0; i < graph.number_of_nodes(); ++i) {
...
template <typename T>
typename enable_if<is_graph<T>, bool>::type
graph_contains_length_one_cycle(const T &graph) {
  for (unsigned long i = 0; i < graph.number_of_nodes(); ++i) {
...
struct pair {
  ...
  bool operator<(const pair &p) const {
    return num_neighbors < p.num_neighbors;
  }
};

template <typename T, typename S, typename V>
void search_graph_for_triangulate(const T &n, S &visited, V &order_visited) {
  ...
  for (unsigned long i = 0; i < neighbors.size(); ++i) {
    search_graph_for_triangulate(n.neighbor(neighbors[i].index), visited,
                                 order_visited);
  }
}
} // end namespace graph_helpers

template <typename graph_type, typename set_of_sets_of_int>
void triangulate_graph_and_find_cliques(graph_type &g,
...
template <typename graph_type, typename join_tree_type>
void create_join_tree(const graph_type &g, join_tree_type &join_tree) {
  // make sure requires clause is not broken
  DLIB_ASSERT(graph_contains_length_one_cycle(g) == false,
  ...
  for (unsigned long i = 0; i < join_tree.number_of_nodes(); ++i) {
    for (unsigned long j = i + 1; j < join_tree.number_of_nodes(); ++j) {
      set_intersection(join_tree.node(i).data, join_tree.node(j).data, e);
  ...
  while (good.size() < join_tree.number_of_nodes()) {
    ...
    while (good.move_next()) {
      ...
      for (unsigned long i = 0;
           i < join_tree.node(good.element()).number_of_neighbors(); ++i) {
        const unsigned long idx =
            join_tree.node(good.element()).neighbor(i).index();
        if (!good.is_member(idx)) {
          const unsigned long overlap =
              join_tree.node(good.element()).edge(i).size();
    ...
    // now remove all the edges from best_bad_idx to the nodes in good except
    // for the edge to best_good_idx.
    for (unsigned long i = 0;
         i < join_tree.node(best_bad_idx).number_of_neighbors(); ++i) {
      const unsigned long idx =
          join_tree.node(best_bad_idx).neighbor(i).index();
...
namespace graph_helpers {
template <typename T, typename U>
bool validate_join_tree(const T &n, U &deads, unsigned long parent = 0xffffffff)
/*!
    this function makes sure that a join tree satisfies the following criterion
for paths starting at the given node:
        - for all valid i and j such that i and j are both <
#join_tree.number_of_nodes()
            - let X be the set of numbers that is contained in both
#join_tree.node(i).data and #join_tree.node(j).data
            - It is the case that all nodes on the unique path between
#join_tree.node(i) and #join_tree.node(j) contain the numbers from X in their
sets.

    returns true if validation passed and false if there is a problem with the
tree
!*/
{
  n.data.reset();
...
template <typename graph_type, typename join_tree_type>
bool is_join_tree(const graph_type &g, const join_tree_type &join_tree) {
  ...
  COMPILE_TIME_ASSERT(is_graph<graph_type>::value ||
                      is_directed_graph<graph_type>::value);
  COMPILE_TIME_ASSERT(is_graph<join_tree_type>::value);
  ...
  for (unsigned long i = 0; i < join_tree.number_of_nodes(); ++i) {
    ...
    for (unsigned long j = 0; j < join_tree.node(i).number_of_neighbors();
      ...
      set_intersection(join_tree.node(i).data,
                       join_tree.node(i).neighbor(j).data, e);
