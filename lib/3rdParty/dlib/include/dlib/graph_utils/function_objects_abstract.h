...
struct squared_euclidean_distance {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This is a simple function object that computes squared euclidean
  distance between two dlib::matrix objects.

      THREAD SAFETY
          This object has no mutable members.  Therefore, it is safe to call
          operator() on a single instance of this object simultaneously from
  multiple threads.
  !*/
  ...
  squared_euclidean_distance(const double l, const double u);
  /*!
      ensures
          - #lower == l
          - #upper == u
  ...
  double operator()(const sample_type &a, const sample_type &b) const;
  /*!
      requires
...
struct cosine_distance {
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          between two vectors and returns 1 - this quantity.   Moreover, this
  object works for both sparse and dense vectors.
  ...
  /*!
      ...
      ensures
          ...
          - returns 1 - cos(theta)
            (e.g. this function returns 0 when a and b have an angle of 0
  between each other, 1 if they have a 90 degree angle, and a maximum of 2 if
  the vectors have a 180 degree angle between each other).
          - zero length vectors are considered to have angles of 0 between all
  other vectors.
  !*/
};
...
struct negative_dot_product_distance {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This is a simple function object that computes the dot product between
  two vectors and returns the negation of this value.  Moreover, this object
...
struct use_weights_of_one {
  ...
  template <typename edge_type> double operator()(const edge_type &) const;
  /*!
      ensures
          - returns 1
  !*/
...
struct use_gaussian_weights {
  ...
  template <typename edge_type> double operator()(const edge_type &e) const;
  /*!
      requires
