...
void remove_duplicate_edges(vector_type &pairs) {
...
iterator iterator_of_worst(iterator begin, const iterator &end)
/*!
    ensures
...
void find_percent_shortest_edges_randomly(
  ...
  DLIB_ASSERT(0 < percent && percent <= 1 && num > 0,
              "\t void find_percent_shortest_edges_randomly()"
  ...
  if (edges.size() > 0) {
    ...
    // now sort all the edges by distance and take the percent with the smallest
    // distance
    std::sort(edges.begin(), edges.end(), &order_by_distance<sample_pair>);

    const unsigned long out_size =
        std::min<unsigned long>((unsigned long)(num * percent), edges.size());
...
void find_approximate_k_nearest_neighbors(
    const vector_type &samples, const distance_function_type &dist_funct,
    const unsigned long k, unsigned long num, const T &random_seed,
    std::vector<sample_pair, alloc> &out) {
  // make sure requires clause is not broken
  ...
  // we add each edge twice in the following loop.  So multiply num by 2 to
  // account for that.
  num *= 2;
  ...
  // now copy edges into temp when they aren't duplicates and also only move in
  // the k shortest for each index.
  itr = edges.begin();
  while (itr != edges.end()) {
    // first find the bounding range for all the edges connected to node
    // itr->index1()
    beg = itr;
    ...
    for (; beg != itr && count < k; ++beg) {
      if (beg->index2() != prev_index2) {
        temp.push_back(
            sample_pair(beg->index1(), beg->index2(), beg->distance()));
        ++count;
      }
...
void find_k_nearest_neighbors(const vector_type &samples,
                              ...
                              std::vector<sample_pair, alloc> &out) {
  // make sure requires clause is not broken
  DLIB_ASSERT(k > 0, "\t void find_k_nearest_neighbors()"
                         << "\n\t Invalid inputs were given to this function."
                         << "\n\t samples.size(): " << samples.size()
                         << "\n\t k:              " << k);
  ...
  edges.resize(samples.size() * k,
               sample_pair(samples.size(), samples.size(),
                           std::numeric_limits<double>::infinity()));

  // Hold the length for the longest edge for each node.  Initially they are all
  // infinity.
  std::vector<double> worst_dists(samples.size(),
                                  std::numeric_limits<double>::infinity());
  ...
  // Loop over all combinations of samples.   We will maintain the iterator
  // ranges so that within the inner for loop we have:
  //   [begin_i, end_i) == the range in edges that contains neighbors of
  //   samples[i] [begin_j, end_j) == the range in edges that contains neighbors
  //   of samples[j]
  ...
  if (edges[0].index1() < samples.size()) {
    // now put edges into out while avoiding duplicates and any remaining
    // invalid edges.
...
typename enable_if_c<
    (is_same_type<sample_pair, typename vector_type::value_type>::value ||
     is_same_type<ordered_sample_pair,
                  typename vector_type::value_type>::value),
    unsigned long>::type
max_index_plus_one(const vector_type &pairs) {
  if (pairs.size() == 0) {
    return 0;
  } else {
...
void remove_percent_longest_edges(vector_type &pairs, double percent) {
  ...
  const unsigned long num =
      static_cast<unsigned long>((1.0 - percent) * pairs.size());
...
void find_neighbor_ranges(
  ...
  // setup neighbors so that [neighbors[i].first, neighbors[i].second) is the
  // range within edges that contains all node i's edges.
  ...
  if (neighbors.size() != 0)
    neighbors[cur_node] =
        std::make_pair(start_idx, (unsigned long)edges.size());
}
...
void convert_unordered_to_ordered(
  ...
  for (unsigned long i = 0; i < edges.size(); ++i) {
    out_edges.push_back(ordered_sample_pair(
        edges[i].index1(), edges[i].index2(), edges[i].distance()));
    if (edges[i].index1() != edges[i].index2())
      out_edges.push_back(ordered_sample_pair(
          edges[i].index2(), edges[i].index1(), edges[i].distance()));
  }
}
