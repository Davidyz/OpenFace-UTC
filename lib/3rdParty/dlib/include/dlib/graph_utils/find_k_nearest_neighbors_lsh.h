...
template <typename vector_type, typename hash_function_type> class hash_block {
public:
  hash_block(const vector_type &samples_, const hash_function_type &hash_funct_,
             std::vector<typename hash_function_type::result_type> &hashes_)
      : samples(samples_), hash_funct(hash_funct_), hashes(hashes_) {}

  void operator()(long i) const { hashes[i] = hash_funct(samples[i]); }
...
public:
  scan_find_k_nearest_neighbors_lsh(
      const vector_type &samples_, const distance_function_type &dist_funct_,
      const hash_function_type &hash_funct_, const unsigned long k_,
      ...
      : samples(samples_), dist_funct(dist_funct_), hash_funct(hash_funct_),
        k(k_), edges(edges_), k_oversample(k_oversample_), hashes(hashes_) {
    edges.clear();
  ...
  void operator()(unsigned long i) const {
    ...
    std::priority_queue<sample_pair, std::vector<sample_pair>,
                        dlib::impl::compare_sample_pair_with_distance>
        best_samples;
...
void find_k_nearest_neighbors_lsh(const vector_type &samples,
  ...
  typedef impl::scan_find_k_nearest_neighbors_lsh<
      vector_type, distance_function_type, hash_function_type, alloc>
      scan_type;
  scan_type temp(samples, dist_funct, hash_funct, k, edges, k_oversample,
                 hashes);
