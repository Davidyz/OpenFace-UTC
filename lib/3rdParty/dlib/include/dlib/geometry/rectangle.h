...
public:
  ...
  rectangle operator+(const rectangle &rhs) const {
    ...
    return rectangle(std::min(l, rhs.l), std::min(t, rhs.t), std::max(r, rhs.r),
                     std::max(b, rhs.b));
  }

  rectangle intersect(const rectangle &rhs) const {
    return rectangle(std::max(l, rhs.l), std::max(t, rhs.t), std::min(r, rhs.r),
                     std::min(b, rhs.b));
  }
  ...
  bool contains(const rectangle &rect) const { return (rect + *this == *this); }
  ...
  inline bool operator<(const dlib::rectangle &b) const {
    if (left() < b.left())
      ...
      return false;
    else if (right() < b.right())
      return true;
    else if (right() > b.right())
      return false;
    else if (bottom() < b.bottom())
      return true;
    else if (bottom() > b.bottom())
      return false;
    else
...
inline const rectangle centered_rect(long x, long y, unsigned long width,
...
inline point center(const dlib::rectangle &rect) {
  point temp(rect.left() + rect.right() + 1, rect.top() + rect.bottom() + 1);
...
inline size_t nearest_rect(const std::vector<rectangle> &rects,
                           const point &p) {
...
inline void clip_line_to_rectangle(const rectangle &box, point &p1, point &p2) {
  ...
  if (p1.x() == p2.x()) {
    ...
    // test the resulting points for ones that are inside the rectangle and
    // output those.
...
inline const rectangle centered_rect(const rectangle &rect, unsigned long width,
                                     unsigned long height) {
  return centered_rect((rect.left() + rect.right()) / 2,
                       (rect.top() + rect.bottom()) / 2, width, height);
}

// ----------------------------------------------------------------------------------------

inline const rectangle shrink_rect(const rectangle &rect, long num) {
  return rectangle(rect.left() + num, rect.top() + num, rect.right() - num,
                   rect.bottom() - num);
}
...
inline const rectangle shrink_rect(const rectangle &rect, long width,
                                   long height) {
  return rectangle(rect.left() + width, rect.top() + height,
...
inline const rectangle resize_rect(const rectangle &rect, unsigned long width,
                                   unsigned long height) {
  return rectangle(rect.left(), rect.top(), rect.left() + width - 1,
                   rect.top() + height - 1);
}

// ----------------------------------------------------------------------------------------

inline const rectangle resize_rect_width(const rectangle &rect,
                                         unsigned long width) {
  return rectangle(rect.left(), rect.top(), rect.left() + width - 1,
                   rect.bottom());
}
...
inline const rectangle resize_rect_height(const rectangle &rect,
                                          unsigned long height) {
  return rectangle(rect.left(), rect.top(), rect.right(),
                   rect.top() + height - 1);
}
...
inline const rectangle move_rect(const rectangle &rect, const point &p) {
  return rectangle(p.x(), p.y(), p.x() + rect.width() - 1,
                   p.y() + rect.height() - 1);
}
...
inline rectangle set_rect_area(const rectangle &rect, unsigned long area) {
  ...
  if (rect.area() == 0) {
    ...
    return centered_rect(rect, (long)std::round(rect.width() * scale),
                         (long)std::round(rect.height() * scale));
  }
}
...
inline rectangle set_aspect_ratio(const rectangle &rect, double ratio) {
  DLIB_ASSERT(ratio > 0, "\t rectangle set_aspect_ratio()"
                             << "\n\t ratio: " << ratio);
...
template <typename T> inline const rectangle get_rect(const T &m) {
