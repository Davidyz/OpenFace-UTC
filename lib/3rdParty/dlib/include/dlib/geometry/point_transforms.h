...
public:
  ...
  const matrix<double, 2, 2> get_m() const {
    ...
    temp = cos_angle, -sin_angle, sin_angle, cos_angle;
    return temp;
  }
...
public:
  ...
  point_transform(const double &angle,
  ...
  const dlib::vector<double, 2> get_b() const { return translate; }
...
public:
  ...
  point_transform_affine(const matrix<double, 2, 2> &m_,
  ...
  const matrix<double, 2, 2> &get_m() const { return m; }

  const dlib::vector<double, 2> &get_b() const { return b; }

  inline friend void serialize(const point_transform_affine &item,
                               std::ostream &out) {
    serialize(item.m, out);
    serialize(item.b, out);
  }

  inline friend void deserialize(point_transform_affine &item,
                                 std::istream &in) {
...
public:
  ...
  rectangle_transform(const point_transform_affine &tform_) : tform(tform_) {}
  ...
  drectangle operator()(const drectangle &r) const {
    ...
    // The new rectangle wouold ideally have this area if we could actually
    // rotrate the box.
    double new_area =
        length(tform(tl) - tform(tr)) * length(tform(tl) - tform(bl));

    // But if we rotate the coners of the rectangle and then find the rectangle
    // that contains them we get this, which might have a much larger area than
    // we want.
    drectangle temp;
    temp += tform(tl);
    temp += tform(tr);
    temp += tform(bl);
    temp += tform(br);
    // so we adjust the area to match the target area and have the same center
    // as the above box.
    double scale = std::sqrt(new_area / temp.area());

    return centered_rect(center(temp),
                         static_cast<long>(temp.width() * scale + 0.5),
                         static_cast<long>(temp.height() * scale + 0.5));
  }
  ...
  const point_transform_affine &get_tform() const { return tform; }

  inline friend void serialize(const rectangle_transform &item,
                               std::ostream &out) {
...
inline point_transform_affine operator*(const point_transform_affine &lhs,
...
inline point_transform_affine inv(const point_transform_affine &trans) {
...
find_affine_transform(const std::vector<dlib::vector<T, 2>> &from_points,
  ...
  DLIB_ASSERT(
      from_points.size() == to_points.size() && from_points.size() >= 3,
...
find_similarity_transform(const std::vector<dlib::vector<T, 2>> &from_points,
                          const std::vector<dlib::vector<T, 2>> &to_points) {
  // make sure requires clause is not broken
  DLIB_ASSERT(from_points.size() == to_points.size() && from_points.size() >= 2,
              "\t point_transform_affine "
  ...
  // We use the formulas from the paper: Least-squares estimation of
  // transformation parameters between two point patterns by Umeyama.  They are
  // equations 34 through 43.
...
public:
  point_transform_projective() { m = identity_matrix<double>(3); }

  point_transform_projective(const matrix<double, 3, 3> &m_) : m(m_) {}
  ...
  const matrix<double, 3, 3> &get_m() const { return m; }

  inline friend void serialize(const point_transform_projective &item,
                               std::ostream &out) {
    serialize(item.m, out);
  }

  inline friend void deserialize(point_transform_projective &item,
                                 std::istream &in) {
    deserialize(item.m, in);
  }
...
inline point_transform_projective
operator*(const point_transform_projective &lhs,
...
inline point_transform_projective inv(const point_transform_projective &trans) {
...
/*!
    ensures
        - Uses the system of equations approach to finding a projective
transform. This is "Method 3" from Estimating Projective Transformation Matrix
by Zhengyou Zhang.
        - It should be emphasized that the find_projective_transform_basic()
          ...
          transformations, doesn't really work well when the minimum error
solution doesn't have zero error.  In this case, it can deviate by a large
amount from the proper minimum mean squared error transformation.  Therefore,
          ...
          non-linear optimizer which will optimize the correct mean squared
error criterion.

          A great essay on this subject is Homography Estimation by Elan
Dubrofsky.
!*/
{
  // make sure requires clause is not broken
  DLIB_ASSERT(from_points.size() == to_points.size() && from_points.size() >= 4,
              "\t point_transform_projective "
              "find_projective_transform_basic(from_points, to_points)"
                  << "\n\t Invalid inputs were given to this function."
                  << "\n\t from_points.size(): " << from_points.size()
                  << "\n\t to_points.size():   " << to_points.size());
...
struct obj {
  ...
  !*/
  obj(const std::vector<dlib::vector<double, 2>> &from_points_,
      const std::vector<dlib::vector<double, 2>> &to_points_)
      : from_points(from_points_), to_points(to_points_) {}
  const std::vector<dlib::vector<double, 2>> &from_points;
  const std::vector<dlib::vector<double, 2>> &to_points;

  double operator()(const matrix<double, 9, 1> &p) const {
...
struct obj_der {
  ...
  obj_der(const std::vector<dlib::vector<double, 2>> &from_points_,
          const std::vector<dlib::vector<double, 2>> &to_points_)
      : from_points(from_points_), to_points(to_points_) {}
  const std::vector<dlib::vector<double, 2>> &from_points;
  const std::vector<dlib::vector<double, 2>> &to_points;

  matrix<double, 9, 1> operator()(const matrix<double, 9, 1> &p) const {
    ...
    for (unsigned long i = 0; i < from_points.size(); ++i) {
      ...
      grad(2, 0) +=
          -(from.x() * w(0) * residual(0) + from.x() * w(1) * residual(1));
      grad(2, 1) +=
          -(from.y() * w(0) * residual(0) + from.y() * w(1) * residual(1));
...
inline point_transform_projective find_projective_transform(
  ...
  // transform and then compare it with the projective transform estimated using
  // the direct SVD method.  Use whichever one works better as the starting
  // point for a BFGS based optimizer.  If the best solution has large mean
  // squared error and is also close to affine then
  // find_projective_transform_basic() might give a very bad initial guess.  So
  // also checking for a good affine transformation can produce a much better
  // final result in many cases.
  point_transform_projective tran1 =
      find_projective_transform_basic(from_points, to_points);
  ...
  if (error1 < error2)
    ...
    params =
        reshape_to_column_vector(point_transform_projective(tran2).get_m());
  ...
  find_min(bfgs_search_strategy(), objective_delta_stop_strategy(1e-6, 100), o,
           der, params, 0);
...
inline matrix<double, 2, 2> rotation_matrix(double angle) {
...
public:
  ...
  point_transform_affine3d(const matrix<double, 3, 3> &m_,
  ...
  const dlib::vector<double, 3> &get_b() const { return b; }

  inline friend void serialize(const point_transform_affine3d &item,
                               std::ostream &out) {
    serialize(item.m, out);
    serialize(item.b, out);
  }

  inline friend void deserialize(point_transform_affine3d &item,
                                 std::istream &in) {
    deserialize(item.m, in);
    deserialize(item.b, in);
  }
...
inline point_transform_affine3d operator*(const point_transform_affine3d &lhs,
                                          const point_transform_affine &rhs) {
  matrix<double, 3, 3> m;
  m = 0;
  ...
  return point_transform_affine3d(lhs.get_m() * m,
                                  lhs.get_m() * b + lhs.get_b());
}
...
inline point_transform_affine3d operator*(const point_transform_affine3d &lhs,
  ...
  return point_transform_affine3d(lhs.get_m() * rhs.get_m(),
                                  lhs.get_m() * rhs.get_b() + lhs.get_b());
}

// ----------------------------------------------------------------------------------------

inline point_transform_affine3d inv(const point_transform_affine3d &trans) {
...
inline point_transform_affine3d
translate_point(const vector<double, 3> &delta) {
  return point_transform_affine3d(identity_matrix<double>(3), delta);
}

inline point_transform_affine3d translate_point(double x, double y, double z) {
...
public:
  camera_transform() {
    *this = camera_transform(vector<double>(1, 1, 1), vector<double>(0, 0, 0),
                             vector<double>(0, 0, 1), 90, 1);
  }

  camera_transform(const vector<double> &camera_pos_,
    ...
    // Minus because images have y axis going down but we want the 3d projection
    // to appear using a normal coordinate system with y going up.
  ...
  inline dpoint operator()(const vector<double> &p, double &scale,
  ...
  inline friend void serialize(const camera_transform &item,
                               std::ostream &out) {
