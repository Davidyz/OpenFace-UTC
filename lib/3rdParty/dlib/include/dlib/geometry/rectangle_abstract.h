...
public:
  ...
  rectangle(unsigned long width_, unsigned long height_);
  /*!
      requires
  ...
  void set_bottom(long bottom_);
  /*!
      ensures
          - #bottom() == bottom_
  !*/

  const point tl_corner() const;
  /*!
      ensures
  ...
  bool operator<(const dlib::rectangle &a, const dlib::rectangle &b) const;
  /*!
      ensures
...
void serialize(const rectangle &item, std::ostream &out);
/*!
    provides serialization support
!*/

void deserialize(rectangle &item, std::istream &in);
/*!
    provides deserialization support
!*/

std::ostream &operator<<(std::ostream &out, const rectangle &item);
/*!
    ensures
        - writes item to out in the form "[(left, top) (right, bottom)]"
!*/

std::istream &operator>>(std::istream &in, rectangle &item);
/*!
    ensures
        - reads a rectangle from the input stream in and stores it in #item.
          The data in the input stream should be of the form [(left, top)
(right, bottom)]
!*/

// ----------------------------------------------------------------------------------------

point center(const dlib::rectangle &rect);
/*!
    ensures
...
dlib::vector<double, 2> dcenter(const dlib::rectangle &rect);
/*!
    ensures
...
inline const rectangle centered_rect(const point &p, unsigned long width,
...
inline std::vector<rectangle> centered_rects(const std::vector<point> &pts,
                                             unsigned long width,
                                             unsigned long height);
/*!
    ensures
...
const rectangle centered_rect(long x, long y, unsigned long width,
                              unsigned long height);
/*!
    ensures
        - returns a rectangle R such that:
...
inline const rectangle centered_rect(const rectangle &rect, unsigned long width,
                                     unsigned long height);
/*!
    ensures
        - returns centered_rect( (rect.tl_corner() + rect.br_corner())/2, width,
height) (i.e. returns a rectangle centered on rect but with the given width and
height)
!*/

// ----------------------------------------------------------------------------------------

inline rectangle set_rect_area(const rectangle &rect, unsigned long area);
/*!
    requires
        - area > 0
    ensures
...
inline unsigned long area(const rectangle &a);
/*!
    ensures
        - returns a.area()
!*/
...
/*!
    ensures
        - returns rectangle(rect.left()+num, rect.top()+num, rect.right()-num,
rect.bottom()-num) (i.e. shrinks the given rectangle by shrinking its border by
num)
!*/
...
inline const rectangle shrink_rect(const rectangle &rect, long width,
                                   long height);
/*!
    ensures
        - returns rectangle(rect.left()+width, rect.top()+height,
rect.right()-width, rect.bottom()-height) (i.e. shrinks the given rectangle by
...
inline const rectangle grow_rect(const rectangle &rect, long width,
                                 long height);
/*!
    ensures
...
const rectangle translate_rect(const rectangle &rect, const point &p);
/*!
    ensures
        - returns a rectangle R such that:
...
const rectangle translate_rect(const rectangle &rect, long x, long y);
/*!
    ensures
        - returns a rectangle R such that:
            - R.left()   == rect.left()   + x
...
const rectangle resize_rect(const rectangle &rect, unsigned long width,
                            unsigned long height);
/*!
    ensures
        - returns a rectangle R such that:
...
const rectangle resize_rect_width(const rectangle &rect, unsigned long width);
/*!
    ensures
        - returns a rectangle R such that:
            - R.width() == width
...
const rectangle resize_rect_height(const rectangle &rect, unsigned long height);
/*!
    ensures
        - returns a rectangle R such that:
            - R.height() == height
...
const rectangle move_rect(const rectangle &rect, const point &p);
/*!
    ensures
        - returns a rectangle R such that:
            - R.width() == rect.width()
...
const rectangle move_rect(const rectangle &rect, long x, long y);
/*!
    ensures
        - returns a rectangle R such that:
            - R.width() == rect.width()
...
inline size_t nearest_rect(const std::vector<rectangle> &rects, const point &p);
/*!
    requires
        - rects.size() > 0
    ensures
...
void clip_line_to_rectangle(const rectangle &box, point &p1, point &p2);
/*!
    ensures
        - clips the line segment that goes from points p1 to p2 so that it is
entirely within the given box.  In particular, we will have:
            - box.contains(#p1) == true
            - box.contains(#p2) == true
            - The line segment #p1 to #p2 is entirely contained within the line
segment p1 to p2.  Moreover, #p1 to #p2 is the largest such line segment that
...
template <typename T> const rectangle get_rect(const T &m);
/*!
    requires
        - It must be possible to determine the number of "rows" and "columns" in
m. Either by calling num_rows(m) and num_columns(m) or by calling m.nr() and
          m.nc() to obtain the number of rows and columns respectively.
Moreover, these routines should return longs. ensures
