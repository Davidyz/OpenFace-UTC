...
class static_map_kernel_1 : public enumerable<map_pair<domain, range>> {
  ...
  friend void deserialize(static_map_kernel_1 &item, std::istream &in) {
    try {
      item.clear();
      unsigned long size;
      deserialize(size, in);
...
public:
  ...
  void load(pair_remover<domain, range> &source);

  void load(asc_pair_remover<domain, range, compare> &source);

  inline const range *operator[](const domain &d) const;

  inline range *operator[](const domain &d);

  inline void swap(static_map_kernel_1 &item);
  ...
  inline bool current_element_valid() const;

  inline const map_pair<domain, range> &element() const;

  inline map_pair<domain, range> &element();

  inline bool move_next() const;

private:
  bool binary_search(const domain &item, unsigned long &pos) const;
  /*!
      ensures
  ...
  void sort_arrays(unsigned long left, unsigned long right);
  /*!
      requires
  ...
  void qsort_partition(unsigned long &partition_element,
                       const unsigned long left, const unsigned long right);
  /*!
      requires
          - left < right
          - left and right are within the bounts of the array
  ...
  unsigned long median(unsigned long one, unsigned long two,
                       unsigned long three);
  /*!
      requires
...
template <typename domain, typename range, typename compare>
inline void swap(static_map_kernel_1<domain, range, compare> &a,
...
template <typename domain, typename range, typename compare>
static_map_kernel_1<domain, range, compare>::static_map_kernel_1()
    : map_size(0), d(0), r(0), at_start_(true) {
  mp.d = 0;
}
...
template <typename domain, typename range, typename compare>
void static_map_kernel_1<domain, range, compare>::clear() {
  if (map_size > 0) {
    map_size = 0;
    delete[] d;
    delete[] r;
...
template <typename domain, typename range, typename compare>
void static_map_kernel_1<domain, range, compare>::load(
    pair_remover<domain, range> &source) {
  if (source.size() > 0) {
    domain *old_d = d;
    d = new domain[source.size()];
    try {
      r = new range[source.size()];
    } catch (...) {
      delete[] d;
      d = old_d;
      throw;
...
template <typename domain, typename range, typename compare>
const range *static_map_kernel_1<domain, range, compare>::operator[](
    const domain &d_item) const {
  unsigned long pos;
...
template <typename domain, typename range, typename compare>
range *
static_map_kernel_1<domain, range, compare>::operator[](const domain &d_item) {
  unsigned long pos;
  if (binary_search(d_item, pos))
    return r + pos;
  else
...
template <typename domain, typename range, typename compare>
size_t static_map_kernel_1<domain, range, compare>::size() const {
  return map_size;
}
...
template <typename domain, typename range, typename compare>
void static_map_kernel_1<domain, range, compare>::swap(
...
template <typename domain, typename range, typename compare>
bool static_map_kernel_1<domain, range, compare>::at_start() const {
  return (at_start_);
}
...
template <typename domain, typename range, typename compare>
void static_map_kernel_1<domain, range, compare>::reset() const {
  mp.d = 0;
  at_start_ = true;
}
...
template <typename domain, typename range, typename compare>
bool static_map_kernel_1<domain, range, compare>::current_element_valid()
    const {
  return (mp.d != 0);
}
...
static_map_kernel_1<domain, range, compare>::element() const {
  return mp;
}
...
static_map_kernel_1<domain, range, compare>::element() {
  return mp;
}
...
template <typename domain, typename range, typename compare>
bool static_map_kernel_1<domain, range, compare>::move_next() const {
  // if at_start() && size() > 0
...
template <typename domain, typename range, typename compare>
bool static_map_kernel_1<domain, range, compare>::binary_search(
    const domain &item, unsigned long &pos) const {
...
template <typename domain, typename range, typename compare>
void static_map_kernel_1<domain, range, compare>::sort_arrays(
...
template <typename domain, typename range, typename compare>
void static_map_kernel_1<domain, range, compare>::qsort_partition(
    unsigned long &partition_element, const unsigned long left,
    const unsigned long right) {
  partition_element = right;

  unsigned long med =
      median(partition_element, left, ((right - left) >> 1) + left);
  ...
  while (true) {
    ...
    while (!(comp(d[right_scan], d[partition_element])) &&
...
template <typename domain, typename range, typename compare>
unsigned long static_map_kernel_1<domain, range, compare>::median(
