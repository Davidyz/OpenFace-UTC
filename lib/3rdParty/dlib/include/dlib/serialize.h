...
are as follows:
        ...
        /!*
            ensures
                ...
                - if (serializable_type implements the enumerable interface)
then
                    - item.at_start() == true
            throws
                - serialization_error
                    This exception is thrown if there is some problem which
prevents us from successfully writing item to the output stream.
                - any other exception
        *!/
        ...
        /!*
            ensures
                ...
                - Reads all the bytes associated with the serialized
serializable_type contained inside the input stream and no more.  This means you
                  can serialize multiple objects to an output stream and then
read them all back in, one after another, using deserialize().
                - if (serializable_type implements the enumerable interface)
then
                    - item.at_start() == true
            throws
                - serialization_error
                    This exception is thrown if there is some problem which
prevents us from successfully deserializing item from the input stream. If this
exception is thrown then item will have an initial value for its type.
                - any other exception
        *!/
    ...
    That overwrites the contents of your_file.dat with the serialized data from
some_object and another_object.  Then to recall the objects from the file you
can do: deserialize("your_file.dat") >> some_object >> another_object;

    Finally, you can chain as many objects together using the << and >>
operators as you like.
    ...
    Note that you can deserialize an integer value to any integral type (except
for a char type) if its value will fit into the target integer type.  I.e.  the
types short, int, long, unsigned short, unsigned int, unsigned long, and
dlib::uint64 can all receive serialized data from each other so long as the
actual serialized value fits within the receiving integral type's range.

    Also note that for any container to be serializable the type of object it
contains must be serializable.

    FILE STREAMS
        If you are serializing to and from file streams it is important to
        remember to set the file streams to binary mode using the
std::ios::binary flag.


    INTEGRAL SERIALIZATION FORMAT
        All C++ integral types (except the char types) are serialized to the
following format: The first byte is a control byte.  It tells you if the
serialized number is positive or negative and also tells you how many of the
following bytes are part of the number.  The absolute value of the number is
stored in little endian byte order and follows the control byte.

        The control byte:
            The high order bit of the control byte is a flag that tells you if
the encoded number is negative or not.  It is set to 1 when the number is
            negative and 0 otherwise.
            The 4 low order bits of the control byte represent an unsigned
number and tells you how many of the following bytes are part of the encoded
            number.

    bool SERIALIZATION FORMAT
        A bool value is serialized as the single byte character '1' or '0' in
ASCII. Where '1' indicates true and '0' indicates false.

    FLOATING POINT SERIALIZATION FORMAT
        To serialize a floating point value we convert it into a float_details
object and then serialize the exponent and mantissa values using dlib's integral
serialization format.  Therefore, the output is first the exponent and then the
mantissa.  Note that the mantissa is a signed integer (i.e. there is not a
separate sign bit).
!*/
...
void check_serialized_version(const std::string &expected_version,
                              std::istream &in);
/*!
    ensures
        - Deserializes a string from in and if it doesn't match expected_version
we throw serialization_error.
!*/

// ----------------------------------------------------------------------------------------

/*!A ramdump information !*/
template <typename T> struct ramdump_t {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This is a type decoration used to indicate that serialization should
  be done by simply dumping the memory of some object to disk as fast as
          possible without any sort of conversions.  This means that the data
  written will be "non-portable" in the sense that the format output by a RAM
  dump may depend on things like the endianness of your CPU or settings of
  certain compiler switches.
          ...
          Also, not all objects have a ramdump mode.  If you try to use ramdump
  on an object that does not define a serialization dump for ramdump you will
...
void serialize(const ramdump_t<const T> &item_, std::ostream &out) {
  ...
  // against a serialize() call that takes just a const ramdump_t<T>.  Doing
  // this saves you from needing to write multiple overloads of serialize() to
  // handle these different const placement cases.
...
pack_int(T item, std::ostream &out)
/*!
    requires
...
template <typename T>
typename enable_if_c<std::numeric_limits<T>::is_signed, bool>::type
unpack_int(T &item, std::istream &in)
/*!
    requires
        - T is a signed integral type
    ensures
...
template <typename T>
typename disable_if_c<std::numeric_limits<T>::is_signed, bool>::type
pack_int(T item, std::ostream &out)
/*!
    requires
...
template <typename T> inline bool pack_byte(const T &ch, std::ostream &out) {
  std::streambuf *sbuf = out.rdbuf();
  return (sbuf->sputc((char)ch) == EOF);
}

template <typename T> inline bool unpack_byte(T &ch, std::istream &in) {
  std::streambuf *sbuf = in.rdbuf();
...
inline void deserialize_floating_point(T &item, std::istream &in) {
  // check if the serialized data uses the older ASCII based format.  We can
  // check this easily because the new format starts with the integer control
  // byte which always has 0 bits in the positions corresponding to the bitmask
  // 0x70.  Moreover, since the previous format used ASCII numbers we know that
  // no valid bytes can have bit values of one in the positions indicated 0x70.
  // So this test looks at the first byte and checks if the serialized data uses
...
void serialize(const std::map<domain, range, compare, alloc> &item,
               std::ostream &out);

template <typename domain, typename range, typename compare, typename alloc>
void deserialize(std::map<domain, range, compare, alloc> &item,
                 std::istream &in);
...
void serialize(const std::pair<first_type, second_type> &item,
               std::ostream &out) {
  try {
...
template <typename domain, typename compare, typename alloc>
void serialize(const std::set<domain, compare, alloc> &item,
               std::ostream &out) {
  try {
    const unsigned long size = static_cast<unsigned long>(item.size());
...
template <typename alloc>
void serialize(const std::vector<unsigned char, alloc> &item,
               std::ostream &out) {
  try {
    const unsigned long size = static_cast<unsigned long>(item.size());
    serialize(size, out);
...
inline void deserialize(std::string &item, std::istream &in) {
  ...
  if (size != 0) {
    ...
    if (!in)
      throw serialization_error(
          "Error deserializing object of type std::string");
  }
}
...
inline void serialize(const std::wstring &item, std::ostream &out) {
  ...
  for (unsigned long i = 0; i < item.size(); ++i)
    serialize(item[i], out);
  if (!out)
    throw serialization_error("Error serializing object of type std::wstring");
}
...
inline void deserialize(std::wstring &item, std::istream &in) {
  ...
  if (!in)
    throw serialization_error(
        "Error deserializing object of type std::wstring");
}
...
inline void serialize(const ustring &item, std::ostream &out) {
  ...
  for (unsigned long i = 0; i < item.size(); ++i)
    serialize(item[i], out);
  if (!out)
    throw serialization_error("Error serializing object of type ustring");
}
...
inline void deserialize(ustring &item, std::istream &in) {
  ...
  if (!in)
    throw serialization_error("Error deserializing object of type ustring");
}
...
inline void serialize(const char (&array)[length], std::ostream &out) {
  if (length != 0 && array[length - 1] == '\0') {
    // If this is a null terminated string then don't serialize the trailing
    // null. We do this so that the serialization format for C-strings is the
    // same as std::string.
...
inline void deserialize(T (&array)[length], std::istream &in) {
  ...
  if (size != length)
    throw serialization_error(
        "Error deserializing a C style array, lengths do not match");
}
...
inline void deserialize(char (&array)[length], std::istream &in) {
  ...
  if (size == length) {
    ...
    // In this case we are deserializing a C-style array so we need to add the
    // null terminator.
    in.read(array, size);
    array[size] = '\0';
    if (!in)
...
public:
  ...
  template <typename T> inline proxy_serialize &operator<<(const T &item) {
    serialize(item, *fout);
    return *this;
  }
...
public:
  explicit proxy_deserialize(const std::string &filename) : filename(filename) {
  ...
  template <typename T> inline proxy_deserialize &operator>>(T &item) {
    return doit(item);
  }
...
private:
  template <typename T> inline proxy_deserialize &doit(T &&item) {
    try {
      if (fin->peek() == EOF)
      ...
      if (looks_like_a_compressed_file())
        suffix = "\n *** THIS LOOKS LIKE A COMPRESSED FILE.  DID YOU FORGET TO "
                 "DECOMPRESS IT? *** \n";

      if (objects_read == 0) {
        throw serialization_error(
            "An error occurred while trying to read the first"
            " object from the file " +
            filename + ".\nERROR: " + e.info + "\n" + suffix);
      } else if (objects_read == 1) {
        throw serialization_error(
            "An error occurred while trying to read the second"
            " object from the file " +
            filename + ".\nERROR: " + e.info + "\n" + suffix);
      } else if (objects_read == 2) {
        throw serialization_error(
            "An error occurred while trying to read the third"
            " object from the file " +
            filename + ".\nERROR: " + e.info + "\n" + suffix);
      } else {
        throw serialization_error(
  ...
  // We don't need to look at the file header.  However, it's here because
  // people keep posting questions to the dlib forums asking why they get file
  // load errors. Then it turns out that the problem is they have a compressed
  // file that NEEDS TO BE DECOMPRESSED by bzip2 or whatever and the reason they
  // are getting deserialization errors is because they didn't decompress the
  // file.  So we are going to check if this file looks like a compressed file
  // and if so then emit an error message telling them to unzip the file. :(
  char file_header[4] = {0, 0, 0, 0};

  bool looks_like_a_compressed_file() const {
    if (file_header[0] == 'B' && file_header[1] == 'Z' &&
        file_header[2] == 'h' &&
...
/*!A is_protocol_buffer
    This is a template that tells you if a type is a Google protocol buffer
object.
!*/

template <typename T, typename U = void> struct is_protocol_buffer {
...
struct is_protocol_buffer<
    T, typename enable_if<
           is_convertible<T *, ::google::protobuf::MessageLite *>>::type> {
  static const bool value = true;
};

template <typename T>
typename enable_if<is_protocol_buffer<T>>::type serialize(const T &item,
                                                          std::ostream &out) {
  ...
  // to record this information ourselves.  So we save the size as a little
  // endian 32bit integer prefixed onto the front of the message.
  ...
  if (!item.SerializeToString(&temp))
    throw dlib::serialization_error(
        "Error while serializing a Google Protocol Buffer object.");
  if (temp.size() > std::numeric_limits<uint32>::max())
    throw dlib::serialization_error("Error while serializing a Google Protocol "
                                    "Buffer object, message too large.");
...
template <typename T>
typename enable_if<is_protocol_buffer<T>>::type deserialize(T &item,
                                                            std::istream &in) {
  // Note that Google protocol buffer messages are not self delimiting
  // (see https://developers.google.com/protocol-buffers/docs/techniques)
  // This means they don't record their length or where they end, so we have
...
inline void check_serialized_version(const std::string &expected_version,
                                     std::istream &in) {
  std::string version;
  ...
  if (version != expected_version) {
    throw serialization_error(
        ...
        "' found while deserializing object. Expected version to be '" +
        expected_version + "'.");
  }
}
