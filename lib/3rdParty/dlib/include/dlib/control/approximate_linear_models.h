...
public:
  ...
  policy(const matrix<double, 0, 1> &weights_, const feature_extractor &fe_)
      : w(weights_), fe(fe_) {}
  ...
  const matrix<double, 0, 1> &get_weights() const { return w; }
...
template <typename feature_extractor>
inline void serialize(const policy<feature_extractor> &item,
                      std::ostream &out) {
  int version = 1;
...
inline void deserialize(policy<feature_extractor> &item, std::istream &in) {
  ...
  if (version != 1)
    throw serialization_error(
        "Unexpected version found while deserializing dlib::policy object.");
  feature_extractor fe;
  matrix<double, 0, 1> w;
  deserialize(fe, in);
  deserialize(w, in);
