...
public:
  ...
  explicit lspi(const feature_extractor &fe_) : fe(fe_) { init(); }
  ...
  unsigned long get_max_iterations() { return max_iterations; }
    ...
    do {
      ...
      for (unsigned long i = 0; i < samples.size(); ++i) {
        ...
        fe.get_features(samples[i].next_state,
                        fe.find_best_action(samples[i].next_state, w), f2);
      ...
      if (feature_extractor::force_last_weight_to_1)
        w = join_cols(pinv(colm(A, range(0, A.nc() - 2))) *
                          (b - colm(A, A.nc() - 1)),
                      mat(1.0));
      else
        w = pinv(A) * b;
      ...
      if (verbose)
        std::cout << "iteration: " << iter << "\tchange: " << change
                  << std::endl;
