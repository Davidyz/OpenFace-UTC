...
namespace dlib {
template <long S_, long I_, unsigned long horizon_> class mpc {
...
public:
  ...
  mpc(const matrix<double, S, S> &A_, const matrix<double, S, I> &B_,
      const matrix<double, S, 1> &C_, const matrix<double, S, 1> &Q_,
      const matrix<double, I, 1> &R_, const matrix<double, I, 1> &lower_,
    ...
    DLIB_ASSERT(A.nr() == A.nc() && A.nr() == B.nr() && A.nr() == C.nr() &&
                    A.nr() == Q.nr(),
                "\t mpc::mpc()"
                    << "\n\t invalid inputs were given to this function"
                    << "\n\t A.nr(): " << A.nr() << "\n\t A.nc(): " << A.nc()
                    << "\n\t B.nr(): " << B.nr() << "\n\t C.nr(): " << C.nr()
                    << "\n\t Q.nr(): " << Q.nr());
    DLIB_ASSERT(B.nc() == R.nr() && B.nc() == lower.nr() &&
                    B.nc() == upper.nr(),
                "\t mpc::mpc()"
                    << "\n\t invalid inputs were given to this function"
                    << "\n\t B.nr(): " << B.nr() << "\n\t B.nc(): " << B.nc()
                    << "\n\t lower.nr(): " << lower.nr()
                    << "\n\t upper.nr(): " << upper.nr());
    DLIB_ASSERT(min(Q) >= 0 && min(R) > 0 && min(upper - lower) >= 0,
                "\t mpc::mpc()"
                    << "\n\t invalid inputs were given to this function"
                    << "\n\t min(Q): " << min(Q) << "\n\t min(R): " << min(R)
  ...
  const matrix<double, S, S> &get_A() const { return A; }
  const matrix<double, S, I> &get_B() const { return B; }
  const matrix<double, S, 1> &get_C() const { return C; }
  const matrix<double, S, 1> &get_Q() const { return Q; }
  const matrix<double, I, 1> &get_R() const { return R; }
  const matrix<double, I, 1> &get_lower_constraints() const { return lower; }
  const matrix<double, I, 1> &get_upper_constraints() const { return upper; }

  void set_target(const matrix<double, S, 1> &val, const unsigned long time) {
  ...
  void set_target(const matrix<double, S, 1> &val) {
    for (unsigned long i = 0; i < horizon; ++i)
      target[i] = val;
  }

  void set_last_target(const matrix<double, S, 1> &val) {
    set_target(val, horizon - 1);
  }

  const matrix<double, S, 1> &get_target(const unsigned long time) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(time < horizon,
  ...
  matrix<double, I, 1> operator()(const matrix<double, S, 1> &current_state) {
    // make sure requires clause is not broken
    ...
    // Shift the inputs over by one time step so we can use them to warm start
    // the optimizer.
...
private:
  // These temporary variables here just to avoid reallocating them on each call
  // to operator().
  ...
  void solve_linear_mpc(const matrix<double, S, 1> &initial_state) {
    ...
    for (; iter < max_iterations; ++iter) {
      ...
      // Check the stopping condition, which is the magnitude of the largest
      ...
      // We will start out by doing a little bit of coordinate descent because
      // it allows us to optimize individual variables exactly.  Since we are
      // warm starting each iteration with a really good solution this helps
      // speed things up a lot.
      const unsigned long smo_iters = 50;
      if (iter < smo_iters) {
        if (Q_diag[max_t](max_v) == 0)
          continue;

        // Take the optimal step but just for one variable.
        controls[max_t](max_v) =
            -(df[max_t](max_v) -
              Q_diag[max_t](max_v) * controls[max_t](max_v)) /
            Q_diag[max_t](max_v);
        controls[max_t](max_v) =
            put_in_range(lower(max_v), upper(max_v), controls[max_t](max_v));
        ...
        for (unsigned long i = 0; i < horizon; ++i) {
          ...
          controls[i] = dlib::clamp(v[i] + (std::sqrt(lambda) - 1) /
                                               (std::sqrt(lambda) + 1) *
                                               (v[i] - v_old[i]),
                                    lower, upper);
        }
