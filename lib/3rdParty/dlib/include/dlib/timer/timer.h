...
struct timer_base : public threaded_object {
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          It exists so that we can access them from outside any templated
  functions.
  !*/
...
class timer_global_clock : private threaded_object {
  ...
  !*/
  typedef binary_search_tree<uint64, timer_base *,
                             memory_manager<char>::kernel_2b>::kernel_2a_c
      time_map;

public:
  ~timer_global_clock();

  void add(timer_base *r);
  /*!
      requires
          - m is locked
      ensures
  ...
  void adjust_delay(timer_base *r, unsigned long new_delay);
  /*!
      requires
          - m is locked
      ensures
...
template <typename T> class timer : private timer_base {
  /*!
      INITIAL VALUE
          - running   == false
          - delay     == 1000
      ...
      CONVENTION
          ...
          - if (!running) then
              - in_global_clock == false
          - else
              - next_time_to_run == the next time this timer should run
  according to the timestamper in the global_clock
  !*/
...
template <typename T>
timer<T>::timer(T &ao_, af_type af_)
    : ao(ao_), af(af_), gc(get_global_clock()) {
  delay = 1000;
  next_time_to_run = 0;
  running = false;
  in_global_clock = false;
...
template <typename T>
typename timer<T>::af_type timer<T>::action_function() const {
  return af;
}

// ----------------------------------------------------------------------------------------

template <typename T> const T &timer<T>::action_object() const { return ao; }

// ----------------------------------------------------------------------------------------

template <typename T> T &timer<T>::action_object() { return ao; }

// ----------------------------------------------------------------------------------------

template <typename T> bool timer<T>::is_running() const {
  auto_mutex M(gc->m);
  return running;
}
...
template <typename T> unsigned long timer<T>::delay_time() const {
  auto_mutex M(gc->m);
  return delay;
}
...
template <typename T>
void timer<T>::set_delay_time(unsigned long milliseconds) {
  auto_mutex M(gc->m);
...
template <typename T> void timer<T>::thread() {
...
template <typename T> void timer<T>::stop_and_wait() {
  gc->m.lock();
  running = false;
  gc->remove(this);
  gc->m.unlock();
