...
public:
  ...
  operator+(const descr<OtherSize1, OtherSize2> &other) const {
    return concat(
        other, make_index_sequence<Size1>(), make_index_sequence<Size2>(),
        make_index_sequence<OtherSize1>(), make_index_sequence<OtherSize2>());
  }
...
protected:
  ...
  template <typename T>
  static size_t len(const T *ptr) { // return length including null termination
    const T *it = ptr;
    while (*it++ != (T)0)
      ;
