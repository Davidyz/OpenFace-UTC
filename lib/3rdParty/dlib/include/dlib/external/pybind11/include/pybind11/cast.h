...
public:
  ...
  // lookup for a polymorphic type.  If the instance isn't derived, returns the
  // non-RTTI base version.
  ...
  src_and_type(const itype *src) {
    ...
    if (vsrc) {
      ...
      if (!same_type(cast_type, *instance_type)) {
        ...
        // pybind11-registered type, we can get the correct derived pointer
        // (which may be != base pointer) by a dynamic_cast to most derived
        // type:
  ...
  // Non-polymorphic type, so no dynamic casting; just call the generic version
  // directly
  template <typename T = itype,
            enable_if_t<!std::is_polymorphic<T>::value, int> = 0>
  static std::pair<const void *, const type_info *>
...
private:
  static handle decode_utfN(const char *buffer, ssize_t nbytes) {
    ...
    // versions require a non-const char * arguments, which is also a nuissance,
...
public:
  ...
  static PYBIND11_DESCR name() {
    return type_descr(_("Tuple[") + detail::concat(make_caster<Ts>::name()...) +
                      _("]"));
  }
...
// policy the function/method was declared with.  Some classes (most notably
// Eigen::Ref and related) need to avoid this, and so can do so by specializing
// this struct.
