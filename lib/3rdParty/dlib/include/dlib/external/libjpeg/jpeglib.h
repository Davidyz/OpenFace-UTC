...
typedef struct {
   ...
   */
  unsigned short
      quantval[DCTSIZE2]; /* quantization step for each coefficient */
...
struct jpeg_memory_mgr {
  ...
  JMETHOD(jvirt_sarray_ptr, request_virt_sarray,
          (j_common_ptr cinfo, int pool_id, int pre_zero,
  ...
  JMETHOD(JSAMPARRAY, access_virt_sarray,
          (j_common_ptr cinfo, jvirt_sarray_ptr ptr, JDIMENSION start_row,
           JDIMENSION num_rows, int writable));
...
EXTERN(void)
jpeg_set_quality JPP((j_compress_ptr cinfo, int quality, int force_baseline));
EXTERN(void)
jpeg_set_linear_quality
    JPP((j_compress_ptr cinfo, int scale_factor, int force_baseline));
EXTERN(void)
...
EXTERN(void) jpeg_suppress_tables JPP((j_compress_ptr cinfo, int suppress));
EXTERN(JQUANT_TBL *) jpeg_alloc_quant_table JPP((j_common_ptr cinfo));
EXTERN(JHUFF_TBL *) jpeg_alloc_huff_table JPP((j_common_ptr cinfo));

/* Main entry points for compression */
EXTERN(void)
jpeg_start_compress JPP((j_compress_ptr cinfo, int write_all_tables));
EXTERN(JDIMENSION)
...
EXTERN(int) jpeg_read_header JPP((j_decompress_ptr cinfo, int require_image));
...
EXTERN(int) jpeg_start_output JPP((j_decompress_ptr cinfo, int scan_number));
...
EXTERN(int) jpeg_resync_to_restart JPP((j_decompress_ptr cinfo, int desired));
