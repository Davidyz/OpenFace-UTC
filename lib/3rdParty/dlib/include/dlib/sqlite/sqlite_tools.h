...
public:
  transaction(database &db_) : db(db_), committed(false) {
...
template <typename T>
void query_object(database &db, const std::string &query, T &item) {
...
inline std::string query_text(database &db, const std::string &query) {
  statement st(db, query);
  st.exec();
  if (st.move_next() && st.get_num_columns() == 1) {
...
inline double query_double(database &db, const std::string &query) {
  statement st(db, query);
  st.exec();
  if (st.move_next() && st.get_num_columns() == 1) {
...
inline int query_int(database &db, const std::string &query) {
  statement st(db, query);
  st.exec();
  if (st.move_next() && st.get_num_columns() == 1) {
...
inline int64 query_int64(database &db, const std::string &query) {
  statement st(db, query);
  st.exec();
  if (st.move_next() && st.get_num_columns() == 1) {
...
inline const std::vector<char> query_blob(database &db,
