...
public:
  ...
  database(const std::string &file) { open(file); }

  bool is_open() const { return db.get() != 0; }
  ...
  inline void exec(const std::string &sql_statement);
...
public:
  statement(database &db_, const std::string sql_statement)
      : needs_reset(false), step_status(SQLITE_DONE), at_first_step(true),
        db(db_.db), stmt(0), sql_string(sql_statement) {
    // make sure requires clause is not broken
    DLIB_ASSERT(
        db_.is_open() == true,
        "\t statement::statement()"
    ...
    int status = sqlite3_prepare_v2(db.get(), sql_string.c_str(),
                                    sql_string.size() + 1, &stmt, NULL);
  ...
  const std::string &get_sql_string() const { return sql_string; }

  template <typename T>
  typename enable_if_c<std::numeric_limits<T>::is_integer>::type
  get_column(unsigned long idx, T &item) const {
    // unsigned ints won't fit into int all the time so put those into 64bit
    // ints.
    if (sizeof(T) < sizeof(int) ||
        (sizeof(T) == sizeof(int) && is_signed_type<T>::value))
      item = get_column_as_int(idx);
    else
      item = get_column_as_int64(idx);
  }

  void get_column(unsigned long idx, std::string &item) const {
    item = get_column_as_text(idx);
  }
  void get_column(unsigned long idx, float &item) const {
    item = get_column_as_double(idx);
  }
  void get_column(unsigned long idx, double &item) const {
    item = get_column_as_double(idx);
  }
  void get_column(unsigned long idx, long double &item) const {
    item = get_column_as_double(idx);
  }

  template <typename T>
  typename disable_if_c<std::numeric_limits<T>::is_integer>::type
  get_column(unsigned long idx, T &item) const {
  ...
  const std::vector<char> get_column_as_blob(unsigned long idx) const {
    ...
    const char *data =
        static_cast<const char *>(sqlite3_column_blob(stmt, idx));
  ...
  int get_column_as_int(unsigned long idx) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(idx < get_num_columns(), "\t int statement::get_column_as_int()"
                                             << "\n\t Invalid column index."
                                             << "\n\t idx:  " << idx
                                             << "\n\t this: " << this);
  ...
  template <typename T>
  typename enable_if_c<std::numeric_limits<T>::is_integer>::type
  bind(unsigned long idx, const T &item) {
    // unsigned ints won't fit into int all the time so put those into 64bit
    // ints.
    if (sizeof(T) < sizeof(int) ||
        (sizeof(T) == sizeof(int) && is_signed_type<T>::value))
  ...
  void bind(unsigned long idx, const std::string &item) {
    bind_text(idx, item);
  }
  ...
  void bind(unsigned long idx, const long double &item) {
    bind_double(idx, item);
  }
  ...
  void bind_blob(unsigned long parameter_id, const std::vector<char> &item) {
    ...
    reset();
    int status = sqlite3_bind_blob(stmt, parameter_id, &item[0], item.size(),
                                   SQLITE_TRANSIENT);
  ...
  void bind_object(unsigned long parameter_id, const T &item) {
    ...
    int status = sqlite3_bind_blob(stmt, parameter_id, str.data(), str.size(),
                                   SQLITE_TRANSIENT);
  ...
  void bind_text(unsigned long parameter_id, const std::string &item) {
    ...
    reset();
    int status = sqlite3_bind_text(stmt, parameter_id, item.c_str(), -1,
                                   SQLITE_TRANSIENT);
...
private:
  ...
  bool needs_reset; // true if sqlite3_step() has been called more recently than
                    // sqlite3_reset()
  int step_status;
  bool at_first_step;
...
void database::exec(const std::string &sql_statement) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
      is_open() == true,
      "\t void database::exec()"
