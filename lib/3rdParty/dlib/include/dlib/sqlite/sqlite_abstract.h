...
public:
  ...
  const std::string &get_database_filename() const;
  /*!
      requires
          - is_open() == true
      ensures
  ...
  int64 last_insert_rowid() const;
  /*!
      requires
          - is_open() == true
      ensures
          - Each element in a database table has a rowid which uniquely
  identifies it.  Therefore, this routine returns the rowid of the most recent
            successful INSERT into the database via this database instance.
          - If an INSERT has not been performed on the current database instance
  then the return value is 0.  This is true even if the database is not empty.
          - See the sqlite documentation for the full details on how this
  function behaves: http://www.sqlite.org/c3ref/last_insert_rowid.html
  !*/
};
...
class statement : noncopyable {
  /*!
      ...
      BINDABLE SQL PARAMETERS
          ...
          For example, you might need to execute many insert statements where
  each statement changes only the value of a field.  Since it is somewhat costly
  to construct a statement object for each SQL operation, SQLite supports
  defining bindable parameters for a statement object.  This allows you to reuse
  the same statement object.

          Therefore, in SQL statements used with SQLite, wherever it is valid to
          include a string literal, one can use a parameter in one of the
  following forms:
         ...
         In the examples above, NNN is an integer value and AAA is an
  identifier.  A parameter initially has a value of NULL.  You can use the
  bind_*() routines to attach values to the parameters.  Each call to a bind_*()
  routine overrides prior bindings on the same parameter.

         Each SQL parameter has a numeric ID which is used to reference it when
  invoking a bind_*() routine.  The leftmost SQL parameter in a statement has an
  index of 1, the next parameter has an index of 2, and so on, except when the
  following rules apply.  When the same named SQL parameter is used more than
  once, second and subsequent occurrences have the same index as the first
  occurrence.  The index for named parameters can be looked up using the
  get_parameter_id() method if desired. The index for "?NNN" parameters is the
  value of NNN. The NNN value must be between 1 and get_max_parameter_id().
  !*/

public:
  statement(database &db, const std::string sql_statement);
  /*!
      requires
          - db.is_open() == true
      ensures
  ...
  /*!
      ensures
          - returns a copy of the SQL statement used to create this statement
  object.
  !*/
  ...
  /*!
      ensures
          - if (there is a result row for this query) then
              ...
              - The get_column_as_*() routines can be used to access the
  elements of the row data.
              - returns true
          - else
              - returns false
              - #get_num_columns() == 0
  ...
  unsigned long get_num_columns() const;
  /*!
      ensures
          - returns the number of columns of data available via the
  get_column_as_*() routines.
  !*/

  template <typename T> void get_column(unsigned long idx, T &item) const;
  /*!
      requires
          - idx < get_num_columns()
      ensures
          - This function automatically selects how to extract the column data
  based on the type of item given. In particular:
  ...
  const std::vector<char> get_column_as_blob(unsigned long idx) const;
  /*!
      requires
          - idx < get_num_columns()
      ensures
  ...
  template <typename T>
  void get_column_as_object(unsigned long idx, T &item) const;
  /*!
      requires
          - idx < get_num_columns()
  ...
  const std::string get_column_as_text(unsigned long idx) const;
  /*!
      requires
          - idx < get_num_columns()
      ensures
  ...
  double get_column_as_double(unsigned long idx) const;
  /*!
      requires
          - idx < get_num_columns()
      ensures
  ...
  int get_column_as_int(unsigned long idx) const;
  /*!
      requires
          - idx < get_num_columns()
      ensures
  ...
  int64 get_column_as_int64(unsigned long idx) const;
  /*!
      requires
          - idx < get_num_columns()
      ensures
  ...
  const std::string get_column_name(unsigned long idx) const;
  /*!
      requires
          - idx < get_num_columns()
      ensures
  ...
  unsigned long get_max_parameter_id() const;
  /*!
      ensures
  ...
  unsigned long get_parameter_id(const std::string &name) const;
  /*!
      ensures
          - if (This SQL statement contains a SQL parameter with the given name)
  then
              - returns the parameter_id number which can be used in the
  bind_*() member functions defined below.
          - else
              - returns 0
  !*/
  ...
  template <typename T>
  void bind(unsigned long parameter_id, const T &item) const;
  /*!
      requires
          - 1 <= parameter_id <= get_max_parameter_id()
      ensures
          - This function automatically selects how to bind item to a statement
  based on the type of item given.  In particular:
              - if (T is a 32bit or smaller built in integer type) then
  ...
  void bind_blob(unsigned long parameter_id, const std::vector<char> &item);
  /*!
      requires
          - 1 <= parameter_id <= get_max_parameter_id()
      ensures
  ...
  template <typename T>
  void bind_object(unsigned long parameter_id, const T &item);
  /*!
      requires
          - 1 <= parameter_id <= get_max_parameter_id()
  ...
  void bind_double(unsigned long parameter_id, const double &item);
  /*!
      requires
          - 1 <= parameter_id <= get_max_parameter_id()
      ensures
  ...
  void bind_int(unsigned long parameter_id, const int &item);
  /*!
      requires
          - 1 <= parameter_id <= get_max_parameter_id()
      ensures
  ...
  void bind_int64(unsigned long parameter_id, const int64 &item);
  /*!
      requires
          - 1 <= parameter_id <= get_max_parameter_id()
      ensures
