// Copyright (C) 2005  Davis E. King (davis@dlib.net), and Nils Labugt, Keita
// Mochizuki License: Boost Software License   See LICENSE.txt for the full
// license.
...
public:
  struct point {
    ...
    point(signed char x_, signed char y_) : x(x_), y(y_) {}

    signed char x;
    signed char y;
  };

  letter() : points(0), w(0), count(0) {}

  letter(unsigned short width_, unsigned short point_count)
      : points(new point[point_count]), w(width_), count(point_count) {}
  ...
  unsigned short width() const { return w; }

  unsigned short num_of_points() const { return count; }

  point &operator[](unsigned short i) {
  ...
  const point &operator[](unsigned short i) const {
    DLIB_ASSERT(i < num_of_points(),
                "\tvoid letter::operator[]()"
                    << "\n\ti:               " << i
                    << "\n\tnum_of_points(): " << num_of_points());
  ...
  friend void serialize(const letter &item, std::ostream &out);

  friend void deserialize(letter &item, std::istream &in);

  void swap(letter &item) {
...
inline void swap(letter &a, letter &b) { a.swap(b); }
...
public:
  ...
  virtual bool has_character(unichar ch) const = 0;
  bool has_character(char ch) const {
    return this->has_character(zero_extend_cast<unichar>(ch));
  }
  bool has_character(wchar_t ch) const {
    return this->has_character(zero_extend_cast<unichar>(ch));
  }

  const letter &operator[](char ch) const {
    return (*this)[zero_extend_cast<unichar>(ch)];
  };
  const letter &operator[](wchar_t ch) const {
    return (*this)[zero_extend_cast<unichar>(ch)];
  };

  virtual const letter &operator[](unichar ch) const = 0;

  virtual unsigned long height() const = 0;

  virtual unsigned long ascender() const = 0;

  virtual unsigned long left_overflow() const = 0;

  virtual unsigned long right_overflow() const = 0;

  // ------------------------------------------------------------------------------------

  template <typename T, typename traits, typename alloc>
  void compute_size(
      const std::basic_string<T, traits, alloc> &str, unsigned long &width,
      unsigned long &height,
      typename std::basic_string<T, traits, alloc>::size_type first = 0,
      typename std::basic_string<T, traits, alloc>::size_type last =
          (std::basic_string<T, traits, alloc>::npos)) const {
  ...
  void draw_string(
      const canvas &c, const rectangle &rect,
      const std::basic_string<T, traits, alloc> &str, const pixel_type &color,
      typename std::basic_string<T, traits, alloc>::size_type first = 0,
      typename std::basic_string<T, traits, alloc>::size_type last =
          (std::basic_string<T, traits, alloc>::npos),
      const rectangle area_ = rectangle(
          std::numeric_limits<long>::min(), std::numeric_limits<long>::min(),
          std::numeric_limits<long>::max(),
          std::numeric_limits<long>::max())) const {
    typedef std::basic_string<T, traits, alloc> string;
    DLIB_ASSERT((last == string::npos) || (first <= last && last < str.size()),
    ...
    for (typename string::size_type i = first; i <= last; ++i) {
      ...
      // A combining character should be applied to the previous character, and
      // we therefore make one step back. If a combining comes right after a
      // newline, then there must be some kind of error in the string, and we
      // don't combine.
      ...
      if (area.bottom() + static_cast<long>(f.height()) < y_offset) {
        ...
        break;
                 ...
                 pos + static_cast<long>(f[str[i]].width() +
                                         f.right_overflow()) <
                     area.left()) {
        pos += f[str[i]].width();
        continue;
  ...
  template <typename T, typename traits, typename alloc>
  void draw_string(const canvas &c, const rectangle &rect,
                   const std::basic_string<T, traits, alloc> &str) const {
    draw_string(c, rect, str, 0, 0, (std::basic_string<T, traits, alloc>::npos),
                rectangle(std::numeric_limits<long>::min(),
  ...
  const rectangle compute_cursor_rect(
      const rectangle &rect, const std::basic_string<T, traits, alloc> &str,
      unsigned long index,
      typename std::basic_string<T, traits, alloc>::size_type first = 0,
      typename std::basic_string<T, traits, alloc>::size_type last =
          (std::basic_string<T, traits, alloc>::npos)) const {
    ...
    if (str.size() != 0) {
      for (typename string::size_type i = first; i <= last && i < index; ++i) {
        ...
        if (str[i] == '\n') {
          ...
          y += f.height();
          count = 0;
        } else if (is_combining_char(str[i]) == false && str[i] != '\r') {
          x += f[str[i]].width();
        }
      }
    }
  ...
  unsigned long compute_cursor_pos(
    ...
    while (pos <= last) {
      ...
      if (is_combining_char(str[pos]) == false && str[pos] != '\r') {
...
public:
  ...
  ~default_font() { delete[] l; }

  unsigned long height() const { return 16; }

  unsigned long ascender() const { return 12; }

  unsigned long left_overflow() const { return 1; }

  unsigned long right_overflow() const { return 2; }

  bool has_character(unichar ch) const {
  ...
  const letter &operator[](unichar ch) const {
    if (ch < 256)
      return l[ch];
...
public:
  ...
  unsigned long height() const { return fbb.height(); }
  unsigned long ascender() const { return std::max(0L, 1 - fbb.top()); }
  unsigned long left_overflow() const { return std::max(0L, -fbb.left()); }
  unsigned long right_overflow() const { return right_overflow_; }
...
private:
  bool bitmap_to_letter(array2d<char> &bitmap, unichar enc, unsigned long width,
                        int x_offset, int y_offset);

  array<letter> gl;
  unichar default_char; // if (is_intialized == true), then this MUST be an
                        // actual glyph
  bool is_initialized;
  rectangle fbb;
