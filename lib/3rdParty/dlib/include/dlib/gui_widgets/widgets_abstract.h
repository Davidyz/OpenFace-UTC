...
/*!
    GENERAL REMARKS
        This component is a collection of various windowing widgets such as
buttons, labels, text boxes, and so on.  This component also includes the
drawable interface, drawable_window, and font handling objects.  The file you
are currently viewing defines all the high level graphical widgets which are
        provided by this component that can appear in a drawable_window.  To
view the specifications for the other members of this component look at
...
/*!
    ...
    ensures
        - Displays a window titled "Open File" that will allow the user to
select a file.
        - The displayed window will start out showing the directory
get_current_dir() (i.e. it starts in the current working directory)
...
/*!
    ...
    ensures
        - Displays a window titled "Open File" that will allow the user to
select a file.  But only a file that already exists.
        - The displayed window will start out showing the directory
get_current_dir() (i.e. it starts in the current working directory)
        - The event_handler function is called on object if the user selects
          a file.  If the user closes the window without selecting a file
...
/*!
    requires
        - event_handler == a valid pointer to a member function of object T.
    ensures
        - Displays a window titled "Save File" that will allow the user to
select a file.
        - The displayed window will start out showing the directory
get_current_dir() (i.e. it starts in the current working directory)
        - The event_handler function is called on object if the user selects
...
/*!
    ensures
        - displays a message box with the given title and message.  It will have
a single button and when the user clicks it the message box will go away.
        - this function does not block but instead returns immediately.
!*/

void message_box_blocking(const std::string &title, const std::string &message);
/*!
    ensures
        - displays a message box with the given title and message.  It will have
a single button and when the user clicks it the message box will go away.
...
class toggle_button : public button_action {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          in the checked or unchecked state and when a user clicks on it it
  toggles its state.

          When this object is disabled it means it will not respond to user
  clicks.
  !*/
...
public:
  ...
  /*!
      ensures
          - #name() == name
          - this toggle_button has been resized such that it is big enough to
  contain the new name. throws
          - std::bad_alloc
  !*/
  ...
  /*!
      ensures
          - if (width and height are big enough to contain the name of this
  button) then
              - #width() == width_
              - #height() == height_
              - #top() == top()
              - #left() == left()
              - i.e. The location of the upper left corner of this button stays
  the same but its width and height are modified
  !*/
  ...
  /*!
      ensures
          - returns the text that is displayed in the tooltip for this
  toggle_button
  !*/

  template <typename style_type> void set_style(const style_type &style);
  /*!
      requires
  ...
  template <typename T>
  void set_click_handler(T &object, void (T::*event_handler)());
  /*!
      requires
          - event_handler is a valid pointer to a member function in T
      ensures
          - the event_handler function is called on object when the
  toggle_button is toggled by the user.
          - this event is NOT triggered by calling set_checked() or
  set_unchecked().
  ...
  void set_click_handler(const any_function<void()> &event_handler);
  /*!
      ensures
  ...
  /*!
      ...
      ensures
          - the event_handler function is called on object when the
  toggle_button is toggled by the user. self will be a reference to the
  toggle_button object that the user clicked.
          - this event is NOT triggered by calling set_checked() or
  set_unchecked().
          - any previous calls to this function are overridden by this new call.
            (i.e. you can only have one event handler associated with this
  ...
  /*!
      ensures
          - the event_handler function is called when the toggle_button is
            toggled by the user. self will be a reference to the toggle_button
  object that the user clicked.
          - this event is NOT triggered by calling set_checked() or
  set_unchecked().
          - any previous calls to this function are overridden by this new call.
            (i.e. you can only have one event handler associated with this
...
class text_field : public drawable {
  /*!
      INITIAL VALUE
          - text() == ""
          - width() == 10
          - height() == a height appropriate for the font used.  The text color
  will be black.
...
public:
  ...
  void set_width(unsigned long width_);
  /*!
      ensures
          - if (width >= 10) then
              - #width()  == width_
  ...
  /*!
      ensures
          - the event_handler function is called when the text in this
  text_field is modified by the user.
          - any previous calls to this function are overridden by this new call.
            (i.e. you can only have one event handler associated with this
            event at a time)
      throws
...
public:
  ...
  /*!
      ensures
          - if (width and height are big enough to contain the tabs) then
              ...
              - #top() == top()
              - #left() == left()
              - i.e. The location of the upper left corner of this widget stays
  the same but its width and height are modified
  !*/
  ...
  /*!
      ...
      ensures
          ...
          - else
              - whenever the tab with index idx is selected group.is_hidden() ==
  false
              - whenever the tab with index idx is deselected group.is_hidden()
  == true
          - whenever the position of *this changes the position of group will be
            updated so that it is still inside the tabbed_display.  The position
  of group will also be updated after this call to set_tab_group().
          - any previous calls to set_tab_group() with this index are overridden
  by this new call.  (i.e. you can only have one widget_group associated with a
  ...
  /*!
      ensures
          ...
          - does not change the position of this object.
            (i.e. the upper left corner of get_rect() remains at the same
  position)
  !*/

  template <typename T>
  void set_click_handler(T &object,
                         void (T::*event_handler)(unsigned long new_idx,
  ...
  /*!
      ...
      ensures
          - The event_handler function is called on object when the user clicks
            on a tab that isn't already selected.  new_idx will give the index
  of the newly selected tab and old_idx will give the index of the tab that was
  previously selected.
          - any previous calls to this function are overridden by this new call.
            (i.e. you can only have one event handler associated with this
            event at a time)
  ...
  void set_click_handler(const any_function<void(unsigned long new_idx,
                                                 unsigned long old_idx)> &eh);
  /*!
      ensures
...
public:
  ...
  /*!
      ...
      ensures
          - #list == a list of all the currently selected indices for this
  list_box.
  !*/
  ...
  /*!
      ...
      ensures
          - #size() == list.size()
          - Copies all the strings from list into *this in the order in which
  they are enumerated. (i.e. The first one goes into (*this)[0], the second into
  (*this)[1], and so on...)
  !*/

  const std::string &operator[](unsigned long index) const;
  /*!
      requires
          - index < size()
      ensures
  ...
  /*!
      ensures
          - if (this object will allow the user to select more than one item at
  a time) then
              - returns true
          - else
              - returns false
  ...
  template <typename T>
  void set_double_click_handler(T &object,
  ...
  /*!
      ...
      ensures
          ...
          - if (underline_ch is present in name) then
              - The menu with index idx will have the first underline_ch
  character in its name underlined and users will be able to activate the menu
...
class directed_graph_drawer : public zoomable_region {
  ...
  directed_graph/directed_graph_kernel_abstract.h
...
public:
  ...
  /*!
      ...
      ensures
          - returns a const reference to the graph that this widget has been
  drawing
  !*/
  ...
  template <typename T>
  void
  set_node_selected_handler(T &object,
  ...
  void set_node_deselected_handler(
      T &object, void (T::*event_handler)(unsigned long node_index));
  /*!
      requires
          - event_handler is a valid pointer to a member function in T
      ensures
  ...
  template <typename T>
  void set_node_deleted_handler(T &object, void (T::*event_handler)());
  /*!
      requires
          - event_handler is a valid pointer to a member function in T
      ensures
  ...
  void set_node_deleted_handler(const any_function<void()> &event_handler);
  /*!
      ensures
  ...
  template <typename T>
  void set_graph_modified_handler(T &object, void (T::*event_handler)());
  /*!
      requires
          - event_handler is a valid pointer to a member function in T
      ensures
  ...
  void set_graph_modified_handler(const any_function<void()> &event_handler);
  /*!
      ensures
...
private:
  ...
  operator=(directed_graph_drawer &); // assignment operator
};
...
public:
  ...
  void set_text_modified_handler(T &object,
                                 void (T::*event_handler)(unsigned long row,
                                                          unsigned long col));
  /*!
      requires
          - event_handler is a valid pointer to a member function in T
  ...
  void set_text_modified_handler(
      const any_function<void(unsigned long row, unsigned long col)>
          &event_handler);
  /*!
      ensures
...
class image_display : public scrollable_region {
  /*!
      INITIAL VALUE
          ...
          - get_default_overlay_rect_color() == rgb_alpha_pixel(255,0,0,255)
  (i.e. RED)
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          If you hold the Ctrl key you can zoom in and out using the mouse
  wheel. You can also add new overlay rectangles by holding shift, left
  clicking, and dragging the mouse.  Additionally, you can delete an overlay
  rectangle by double clicking on it and hitting delete or backspace.  Finally,
  you can also add part labels (if they have been defined by calling
  add_labelable_part_name()) by selecting an overlay rectangle with the mouse
  and then right clicking on the part.  If you want to move any rectangle or an
  object part then shift+right click and drag it.  Alternatively, if you haven't
  added any part labels via add_labelable_part_name() you can add parts to a
  rectangle by simply shift left clicking while it's selected.  This will add
  parts with integer names and the integer names begin with 0 and increase. This
          feature is only activated if the rectangle has no parts or all the
  parts are already integer names.
          ...
          The image is drawn such that:
              ...
              - the pixel img[img.nr()-1][0] is the lower left corner of the
  image.
              - the pixel img[0][img.nc()-1] is the upper right corner of the
  image.
              - the pixel img[img.nr()-1][img.nc()-1] is the lower right corner
  of the image.
  !*/
...
public:
  ...
  or a dlib::matrix or something convertible to a matrix via mat()
  ...
  /*!
      ensures
          - returns the label given to new overlay rectangles created by the
  user (i.e. when the user holds shift and adds them with the mouse)
  !*/
  ...
  /*!
      ensures
          - returns the color given to new overlay rectangles created by the
  user (i.e. when the user holds shift and adds them with the mouse)
  !*/
  ...
  /*!
      ensures
          - removes all use labelable parts.  Calling this function undoes
            ...
            user won't be able to label any parts after
  clear_labelable_part_names() is called.
  !*/
  ...
  /*!
      ensures
          - if this function returns true then it is possible for the user to
  add or remove overlay objects (e.g. rectangles) using the mouse and keyboard.
            If it returns false then the overlay is not user editable.
  !*/

  template <typename T>
  void set_overlay_rects_changed_handler(T &object, void (T::*event_handler)());
  /*
      requires
          - event_handler is a valid pointer to a member function in T
      ensures
  ...
  */
  ...
  void
  set_overlay_rects_changed_handler(const any_function<void()> &event_handler);
  /*
      ensures
  ...
  */
  ...
  void set_overlay_rect_selected_handler(
      T &object, void (T::*event_handler)(const overlay_rect &orect));
  /*
      requires
          - event_handler is a valid pointer to a member function in T
      ensures
          - The event_handler function is called on object when the user selects
            an overlay rectangle by double clicking on it.  The selected
     rectangle will be passed to event_handler().
          - any previous calls to this function are overridden by this new call.
            (i.e. you can only have one event handler associated with this
            event at a time)
      throws
  ...
  */
  ...
  /*
      ensures
          - The event_handler function is called when the user selects an
     overlay rectangle by double clicking on it.  The selected rectangle will be
  ...
  */
  ...
  void set_image_clicked_handler(T &object,
                                 void (T::*event_handler)(const point &p,
  ...
  /*
      requires
          - event_handler is a valid pointer to a member function in T
      ensures
          - The event_handler function is called on object when the user left
     clicks anywhere on the image.  When they do so this callback is called with
     the location of the image pixel which was clicked.  The is_double_click
     bool will also tell you if it was a double click or single click.
          - btn == the button that was released. (either base_window::LEFT,
     base_window::MIDDLE, or base_window::RIGHT)
          - any previous calls to this function are overridden by this new call.
            (i.e. you can only have one event handler associated with this
            event at a time)
      throws
  ...
  */
  ...
  void set_image_clicked_handler(
      const any_function<void(const point &p, bool is_double_click,
                              unsigned long btn)> &event_handler);
  /*
      ensures
          - The event_handler function is called when the user left clicks
     ...
     base_window::MIDDLE, or base_window::RIGHT)
          - Any previous calls to this function are overridden by this new call.
            (i.e. you can only have one event handler associated with this event
     at a time) throws
          - std::bad_alloc
  */
...
public:
  ...
  template <typename image_type> image_window(const image_type &img);
  /*!
      requires
          - image_type == an implementation of array2d/array2d_kernel_abstract.h
  or a dlib::matrix or something convertible to a matrix via mat()
  ...
  template <typename pixel_type>
  void add_overlay(const rectangle &r, pixel_type p = rgb_pixel(255, 0, 0));
  /*!
      ensures
  ...
  /*!
      ensures
          - returns true if and only if the get_next_double_click() and
            get_next_keypress() events are tied together.  If they are tied it
  means that you can use a loop of the following form to listen for both events
            simultaneously:
              while (mywindow.get_next_double_click(p) ||
  mywindow.get_next_keypress(key,printable))
              {
                  if (p.x() < 0)
  ...
  /*!
      ensures
          - This function blocks until the user double clicks on the image or
  the window is closed by the user.  It will also unblock for a keyboard key
  ...
  the window is closed by the user.  It will also unblock for a keyboard key
          ...
          - if (this function returns true) then
              - This means the user double clicked the mouse.
              - #p == the next image pixel the user clicked.
              - #mouse_button == the mouse button which was used to double
  click. This will be either dlib::base_window::LEFT, dlib::base_window::MIDDLE,
  or dlib::base_window::RIGHT
          - else
              - #p == point(-1,1)
  ...
  /*!
      ensures
          - This function blocks until the user presses a keyboard key or the
            window is closed by the user.  It will also unblock for a mouse
  double click if events_tied() == true.
          - if (this function returns true) then
              - This means the user pressed a keyboard key.
              - The keyboard button press is recorded into #key, #is_printable,
  and #state.  In particular, these variables are populated with the three
                identically named arguments to the
  base_window::on_keydown(key,is_printable,state) event.
  !*/
  ...
  /*!
      ensures
          - This function blocks until the user presses a keyboard key or the
            window is closed by the user.  It will also unblock for a mouse
  double click if events_tied() == true.
          - This function is the equivalent to calling
  get_next_keypress(key,is_printable,temp) and then discarding temp.
  !*/
...
class perspective_display : public drawable, noncopyable {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This object is a tool for displaying 3D point clouds on a screen.  You
  can navigate the display with the mouse.  Left click and drag rotates the
...
public:
  ...
  struct overlay_line {
    /*!
        WHAT THIS OBJECT REPRESENTS
            This object represents a line that is drawn on the screen.  Each
    line is represented by its two end points (p1 and p2) as well as a color.
    !*/
  ...
  struct overlay_dot {
    /*!
        WHAT THIS OBJECT REPRESENTS
            This object represents a dot that is drawn on the screen.  Each dot
    is represented by one point and a color.
    !*/
  ...
  void set_dot_double_clicked_handler(
      T &object, void (T::*event_handler)(const vector<double> &));
  /*
      requires
          - event_handler is a valid pointer to a member function in T
      ensures
          - The event_handler function is called on object when the user double
            clicks on one of the overlay dots.  The selected dot will be passed
     to event_handler().
          - Any previous calls to this function are overridden by this new call.
            (i.e. you can only have one event handler associated with this
            event at a time)
  */
  ...
  /*
      ensures
          - The event_handler function is called when the user double clicks on
     one of the overlay dots.  The selected dot will be passed to
     event_handler().
          - Any previous calls to this function are overridden by this new call.
            (i.e. you can only have one event handler associated with this
            event at a time)
...
class perspective_window : public drawable_window, noncopyable {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This is a simple window that is just a container for a
  perspective_display. It exists to make it easy to throw perspective_displays
  onto the screen without having to put together your own drawable_window
  objects.
  !*/
public:
  ...
  /*!
      ensures
          - The window is displayed on the screen and is 100x100 pixels in size.
          - This window will have point_cloud added to it via add_overlay() and
  the points will all be white.
  !*/
