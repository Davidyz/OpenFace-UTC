// Copyright (C) 2005  Davis E. King (davis@dlib.net), Nils Labugt, Keita
...
public:
  struct point {
    ...
    point(signed char x_, signed char y_);
    /*!
        ensures
            - #x == x_
            - #y == y_
  ...
  /*!
      ensures
          - returns the width reserved for this letter in pixels.  This is the
            ...
            letters.  It isn't necessarily the width of the actual letter
  itself. (for example, you can make a letter with a width less than how wide it
            actually is so that it overlaps with its neighbor letters.)
  !*/

  const unsigned short num_of_points() const;
  /*!
      ensures
...
void serialize(const letter &item, std::ostream &out);
/*!
    provides serialization support for letter objects
!*/

void deserialize(letter &item, std::istream &in);
/*!
...
class font {
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          for the font and functions to help you draw strings on a canvas
  object.
...
public:
  ...
  /* Cast char and wchar_t to unichar correctly when char or wchar_t is a signed
   * type */
  ...
  /*!
      ensures
          - if (has_character(ch) == true) then
              - returns a letter object that tells you how to draw this
  character.
          - else
              - returns some default glyph for characters that aren't in this
  font.
  !*/
  ...
  virtual const unsigned long height() const = 0;
  /*!
      ensures
          - returns the height in pixels of the tallest letter in the font
  !*/

  virtual const unsigned long ascender() const = 0;
  /*!
      ensures
          - returns the height() minus the number of pixels below the baseline
  used by the letter that hangs the lowest below the baseline.
  !*/

  virtual const unsigned long left_overflow() const = 0;
  /*!
      ensures
  ...
  virtual const unsigned long right_overflow() const = 0;
  /*!
      ensures
  ...
  void
  draw_string(const canvas &c, const rectangle &rect,
  ...
  /*!
      ...
      ensures
          ...
          - if (index < first) then
              - the returned cursor will be just before the character
  str[first].
          - if (last != std::basic_string<T,traits,alloc>::npos && index > last)
  then
              - the returned cursor will be just after the character str[last]
          - if (str.size() == 0) then
              - the returned cursor will be just at the start of the rectangle
  where str would be drawn if it wasn't empty.
          - if (index > str.size()-1) then
              - the returned cursor will be just after the character
  str[str.size()-1]
  !*/
...
public:
  ...
  /*!
      ...
      throws
          - std::bad_alloc
              This exception is thrown if there is a problem gathering the
  needed memory for the font object.
  !*/
...
class bdf_font : public font {
  ...
  /*!
      ...
      THREAD SAFETY
          If you only access this object via the functions in the parent class
  font then this object is thread safe.  But if you need to call any of the
...
public:
  bdf_font(long default_char = -1);
  /*!
      ensures
          - for all x:
              - #has_character(x) == false
                (i.e. this font starts out empty.  You have to call
  read_bdf_file() to load it with data)
