...
void draw_line(
    const canvas &c, const point &p1, const point &p2, const pixel_type &pixel,
    const rectangle &area = rectangle(std::numeric_limits<long>::min(),
    ...
    if (std::abs(rise) < std::abs(run)) {
      ...
      for (double i = first; i <= last; ++i) {
        ...
        if (y >= valid_area.top() && y <= valid_area.bottom()) {
          alpha_pixel.alpha =
              static_cast<unsigned char>((1.0 - (dy - y)) * max_alpha);
      ...
      for (double i = first; i <= last; ++i) {
        ...
        if (x >= valid_area.left() && x <= valid_area.right()) {
          alpha_pixel.alpha =
              static_cast<unsigned char>((1.0 - (dx - x)) * max_alpha);
          assign_pixel(c[y - c.top()][x - c.left()], alpha_pixel);
        }
...
inline void draw_line(const canvas &c, const point &p1, const point &p2) {
  draw_line(c, p1, p2, 0);
}

// ----------------------------------------------------------------------------------------

void draw_sunken_rectangle(const canvas &c, const rectangle &border,
                           unsigned char alpha = 255);

// ----------------------------------------------------------------------------------------

template <typename pixel_type>
inline void draw_pixel(const canvas &c, const point &p,
...
template <typename pixel_type>
void draw_checkered(const canvas &c, const rectangle &a,
                    const pixel_type &pixel1, const pixel_type &pixel2) {
...
void draw_button_down(const canvas &c, const rectangle &btn,
                      unsigned char alpha = 255);

// ----------------------------------------------------------------------------------------

void draw_button_up(const canvas &c, const rectangle &btn,
                    unsigned char alpha = 255);
...
template <typename pixel_type>
void draw_circle(
    const canvas &c, const point &center_point, double radius,
    const pixel_type &pixel,
    const rectangle &area = rectangle(std::numeric_limits<long>::min(),
                                      std::numeric_limits<long>::min(),
                                      std::numeric_limits<long>::max(),
  ...
  if (radius > 1) {
    ...
    top = static_cast<long>(
        sqrt(std::max(rs - (first_x - x - 0.5) * (first_x - x - 0.5), 0.0)) +
        0.5);
    top += y;
    long last = top;
    ...
    top = static_cast<long>(
        sqrt(std::max(rs - (last_x - x + 0.5) * (last_x - x + 0.5), 0.0)) +
        0.5);
    top += y;
    last = top;
  ...
  } else if (radius == 1 && x >= valid_area.left() && x <= valid_area.right() &&
             y >= valid_area.top() && y <= valid_area.bottom()) {
    assign_pixel(c[y - c.top()][x - c.left()], pixel);
  }
}
inline void draw_circle(const canvas &c, const point &center_point,
                        double radius) {
...
inline void draw_solid_circle(const canvas &c, const point &center_point,
                              double radius) {
...
void get_convex_polygon_shape(const std::vector<point> &points, const long top,
                              const long bottom,
...
/*!
    ...
    ensures
        - interprets points as the coordinates defining a convex polygon.  In
          particular, we interpret points as a list of the vertices of the
polygon and assume they are ordered in clockwise order.
        - #left_boundary.size() == bottom-top+1
        - #right_boundary.size() == bottom-top+1
        - for all top <= y <= bottom:
            - #left_boundary[y-top] == the x coordinate for the left most side
of the polygon at coordinate y.
            - #right_boundary[y-top] == the x coordinate for the right most side
of the polygon at coordinate y.
!*/
{
  using std::max;
  ...
  left_boundary.assign(bottom - top + 1,
                       std::numeric_limits<double>::infinity());
  right_boundary.assign(bottom - top + 1,
                        -std::numeric_limits<double>::infinity());
...
void draw_solid_convex_polygon(
    const canvas &c, const std::vector<point> &polygon, const pixel_type &pixel,
    const rectangle &area = rectangle(std::numeric_limits<long>::min(),
                                      std::numeric_limits<long>::min(),
                                      std::numeric_limits<long>::max(),
                                      std::numeric_limits<long>::max())) {
  ...
  // we will only want to loop over the part of left_boundary that is part of
  // the valid_area.
  long top = max(valid_area.top(), bounding_box.top());
  long bottom = min(valid_area.bottom(), bounding_box.bottom());

  // Since we look at the adjacent rows of boundary information when doing the
  // alpha blending, we want to make sure we always have some boundary
  // information unless we are at the absolute edge of the polygon.
  ...
  impl::get_convex_polygon_shape(polygon, top, bottom, left_boundary,
                                 right_boundary);
  ...
  for (unsigned long i = top_offset; i < left_boundary.size(); ++i) {
    ...
    if (std::abs(delta) <= 1) {
      if (std::floor(left_boundary[i]) != left_x) {
        ...
        temp.alpha = max_alpha - static_cast<unsigned char>(
                                     (left_boundary[i] - p.x()) * max_alpha);
        if (valid_area.contains(p))
          assign_pixel(c[p.y() - c.top()][p.x() - c.left()], temp);
      }
    } else if (delta < 0) // on the bottom side
    {
      for (long x = static_cast<long>(std::ceil(left_boundary[i - 1]));
           x < left_x; ++x) {
        const point p(x, i + top);
        rgb_alpha_pixel temp = alpha_pixel;
        temp.alpha = static_cast<unsigned char>((x - left_boundary[i - 1]) /
                                                std::abs(delta) * max_alpha);
        if (valid_area.contains(p))
          assign_pixel(c[p.y() - c.top()][p.x() - c.left()], temp);
      }
    } else // on the top side
    {
      const long old_left_x =
          static_cast<long>(std::ceil(left_boundary[i - 1]));
      for (long x = left_x; x < old_left_x; ++x) {
        const point p(x, i + top - 1);
        rgb_alpha_pixel temp = alpha_pixel;
        temp.alpha = static_cast<unsigned char>((x - left_boundary[i]) / delta *
                                                max_alpha);
        if (valid_area.contains(p))
          assign_pixel(c[p.y() - c.top()][p.x() - c.left()], temp);
      }
    ...
    if (std::abs(delta) <= 1) {
      if (std::ceil(right_boundary[i]) != right_x) {
        ...
        rgb_alpha_pixel temp = alpha_pixel;
        temp.alpha = max_alpha - static_cast<unsigned char>(
                                     (p.x() - right_boundary[i]) * max_alpha);
        if (valid_area.contains(p))
          assign_pixel(c[p.y() - c.top()][p.x() - c.left()], temp);
      }
    } else if (delta < 0) // on the top side
    {
      for (long x = static_cast<long>(std::floor(right_boundary[i - 1])) + 1;
           x <= right_x; ++x) {
        const point p(x, i + top - 1);
        rgb_alpha_pixel temp = alpha_pixel;
        temp.alpha = static_cast<unsigned char>((right_boundary[i] - x) /
                                                std::abs(delta) * max_alpha);
        if (valid_area.contains(p))
          assign_pixel(c[p.y() - c.top()][p.x() - c.left()], temp);
      }
    ...
    {
      const long old_right_x =
          static_cast<long>(std::floor(right_boundary[i - 1]));
      for (long x = right_x + 1; x <= old_right_x; ++x) {
        const point p(x, i + top);
        rgb_alpha_pixel temp = alpha_pixel;
        temp.alpha = static_cast<unsigned char>((right_boundary[i - 1] - x) /
                                                delta * max_alpha);
        if (valid_area.contains(p))
          assign_pixel(c[p.y() - c.top()][p.x() - c.left()], temp);
      }
...
inline void draw_solid_convex_polygon(const canvas &c,
...
void draw_image(
    const canvas &c, const point &p, const image_type &img,
    const rectangle &area_ = rectangle(std::numeric_limits<long>::min(),
                                       std::numeric_limits<long>::min(),
                                       std::numeric_limits<long>::max(),
                                       std::numeric_limits<long>::max())) {
  const long x = p.x();
  ...
  for (long row = area.top(); row <= area.bottom(); ++row) {
    for (long col = area.left(); col <= area.right(); ++col) {
      assign_pixel(c[row - c.top()][col - c.left()],
                   img[row - rect.top()][col - rect.left()]);
    }
  }
}
...
template <typename image_type>
void draw_image(
    const canvas &c, const rectangle &rect, const image_type &img,
    const rectangle &area_ = rectangle(std::numeric_limits<long>::min(),
                                       std::numeric_limits<long>::min(),
                                       std::numeric_limits<long>::max(),
                                       std::numeric_limits<long>::max())) {
  ...
  const matrix<long, 1> x =
      matrix_cast<long>(round(linspace(0, num_columns(img) - 1, rect.width())));
  const matrix<long, 1> y =
      matrix_cast<long>(round(linspace(0, num_rows(img) - 1, rect.height())));
...
void draw_rounded_rectangle(
    const canvas &c, const rectangle &rect, unsigned radius,
...
void fill_gradient_rounded(
    const canvas &c, const rectangle &rect, unsigned long radius,
    const pixel_type &top_color, const pixel_type &bottom_color,
    const rectangle &area = rectangle(std::numeric_limits<long>::min(),
                                      std::numeric_limits<long>::min(),
                                      std::numeric_limits<long>::max(),
                                      std::numeric_limits<long>::max())
  ...
  for (long y = c_top; y <= c_bottom; y++) {
    ...
    vector_to_pixel(color,
                    ((pixel_to_vector<unsigned long>(top_color) * c_t +
                      pixel_to_vector<unsigned long>(bottom_color) * c_s) /
                     c_div));
    ...
    if (s < radius) {
      m = radius -
          square_root((radius * radius - (radius - s) * (radius - s)) * 4) / 2;

      if (s == m && m + 1 < m_prev) // these are hacks to remove distracting
                                    // artefacts at small radii
        m++;
    } else if (t < radius) {
      m = radius -
          square_root((radius * radius - (radius - t) * (radius - t)) * 4) / 2;
...
template <typename pixel_type>
void draw_rectangle(
    const canvas &c, rectangle rect, const pixel_type &pixel,
    const rectangle &area = rectangle(std::numeric_limits<long>::min(),
                                      std::numeric_limits<long>::min(),
                                      std::numeric_limits<long>::max(),
                                      std::numeric_limits<long>::max())) {
  // top line
  draw_line(c, point(rect.left(), rect.top()), point(rect.right(), rect.top()),
            pixel, area);

  // bottom line
  draw_line(c, point(rect.left(), rect.bottom()),
            point(rect.right(), rect.bottom()), pixel, area);

  // left line
  draw_line(c, point(rect.left(), rect.top()),
            point(rect.left(), rect.bottom()), pixel, area);

  // right line
  draw_line(c, point(rect.right(), rect.top()),
            point(rect.right(), rect.bottom()), pixel, area);
}
inline void draw_rectangle(const canvas &c, rectangle rect) {
...
template <typename pixel_type>
void fill_rect(const canvas &c, const rectangle &rect,
               const pixel_type &pixel) {
...
void fill_rect_with_vertical_gradient(
    const canvas &c, const rectangle &rect, const pixel_type &pixel_top,
  ...
  for (long y = area.top(); y <= area.bottom(); ++y) {
    ...
    vector_to_pixel(pixel, ((pixel_to_vector<long>(pixel_top) * t +
                             pixel_to_vector<long>(pixel_bottom) * b) /
                            s));
