...
private:
  ...
  struct vals_internal {
    int width, height;
#ifdef WIN32
    COLORREF rgb2RGB(rgb_type &rgb) { return RGB(rgb.r, rgb.g, rgb.b); }
    ...
    template <typename T>
    void create(T *str, int height_want, bool italic, bool bold, bool fixed,
                rgb_type &background, rgb_type &foreground) {
      struct inner {
        inline static BOOL GetTextExtentPoint32(HDC hDC, LPCSTR str, int len,
                                                LPSIZE lpsize) {
          return ::GetTextExtentPoint32A(hDC, str, len, lpsize);
        }
        inline static BOOL GetTextExtentPoint32(HDC hDC, LPCWSTR str, int len,
                                                LPSIZE lpsize) {
          return ::GetTextExtentPoint32W(hDC, str, len, lpsize);
        }
        inline static BOOL TextOut(HDC hDC, int nxstart, int nystart,
                                   LPCSTR str, int cbstr) {
          return ::TextOutA(hDC, nxstart, nystart, str, cbstr);
        }
        inline static BOOL TextOut(HDC hDC, int nxstart, int nystart,
                                   LPCWSTR str, int cbstr) {
      ...
      if (!hFont || height_prev != height || attribute != attribute_prev) {
        ...
        hFont =
            CreateFont(height_want, 0, 0, 0, bold ? FW_BOLD : FW_DONTCARE,
                       italic ? TRUE : FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                       OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
                       fixed ? (FIXED_PITCH | FF_DONTCARE)
      ...
      if (pix_width_prev < width || pix_height_prev < height) {
        ...
        hBmp = CreateDIBSection(NULL, &bi, DIB_RGB_COLORS, (void **)&pixelint,
                                NULL, 0);
    ...
    template <typename T>
    vals_internal(T *str, int height_want, bool italic = false,
                  bool bold = false, bool fixed = false,
                  rgb_type background = rgb_type(),
                  rgb_type foreground = rgb_type()) {
      first = true;
      hFont = NULL;
      hDCBmp = 0;
      hBmpOld = 0;
    ...
    inline int get_ascender() { return ascender; }

    inline int get_descender() { return descender; }
    ...
    void create(T *str, int height_want, bool italic, bool bold, bool fixed,
      ...
      struct inner {
        ...
        inline static int XTextExtents(XFontSet fs, wchar_t *str, int len,
                                       XRectangle *ink, XRectangle *logical) {
          return XwcTextExtents(fs, str, len, ink, logical);
        }
        inline static void XDrawString(Display *d, Window w, XFontSet fs, GC gc,
      ...
      create_helper<sizeof(T)> ch(
          (typename size2inner_trait<sizeof(T)>::type_t *)str);
      setlocale(LC_CTYPE, "");
      if (d == NULL) {
        d = XOpenDisplay(NULL);
        if (d == 0) {
      ...
      char fontset[256];
      {
        ...
        p += sprintf(fontset, "-*-*-%s-%c-normal--%d-*-*-*-%c",
                     ...
                     fixed ? 'c' : 'p');
        if (fixed) {
          sprintf(p, ",-*-*-%s-%c-normal--%d-*-*-*-m", bold ? "bold" : "medium",
                  italic ? 'i' : 'r', height_want);
        }
      }
      bool equal_font;
      ...
      if (pix == None || pix_width_prev < width || pix_height_prev < height) {
        ...
        pix_height_prev = height * 2;
        pix =
            XCreatePixmap(d, DefaultRootWindow(d), pix_width_prev,
                          pix_height_prev, XDefaultDepth(d, DefaultScreen(d)));
    ...
    rgb_type color2rgb(unsigned long color, Display *d, Colormap &cmap) {
      if (col2rgb.count(color)) {
        ...
        rgb_type rgb_((byte)(xcol.red / 257), (byte)(xcol.green / 257),
                      (byte)(xcol.blue / 257));
        col2rgb[color] = rgb_;
        return rgb_;
      }
...
public:
  ...
  template <typename T>
  font_renderer(T *str, int height_want, bool italic = false, bool bold = false,
                bool fixed = false, rgb_type background = rgb_type(0, 0, 0),
                rgb_type foreground = rgb_type(255, 255, 255)) {
    render(str, height_want, italic, bold, fixed, background, foreground);
  }

  template <typename T>
  void render(T *str, int height_want, bool italic = false, bool bold = false,
              bool fixed = false, rgb_type background = rgb_type(0, 0, 0),
              rgb_type foreground = rgb_type(255, 255, 255)) {
    if (vi == NULL) {
      vi = new vals_internal(str, height_want, italic, bold, fixed, background,
                             foreground);
    } else {
  ...
  const byte *get_image() { return image; }
};
} // namespace font_renderer
...
public:
  ...
  virtual bool has_character(dlib::unichar ch) const {
  ...
  dlib::letter &get_letter(dlib::unichar ch) {
    if (letters.count(ch)) {
  ...
  virtual unsigned long height() const { return 12; }

  virtual unsigned long ascender() const { return ascender_; }

  virtual unsigned long left_overflow() const { return 1; }

  virtual unsigned long right_overflow() const { return 2; }
};
