...
/*!
    ...
    EVENT HANDLERS
        If you derive from any of the drawable objects and redefine any of the
on_*() event handlers then you should ensure that your version calls the same
event handler in the base object so that the base class part of your object will
also be able to process the event.

        Also note that all event handlers, including the user registered
callback functions, are executed in the event handling thread.   Additionally,
...
class draggable : public drawable {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents a drawable object that is draggable by the
  mouse. You use it by inheriting from it and defining the draw() method and any
...
public:
  ...
  void set_draggable_area(const rectangle &area);
  /*!
      ensures
          - #draggable_area() == area
  !*/
...
class mouse_over_event : public drawable {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents a drawable object with the addition of two
  events that will alert you when the mouse enters or leaves your drawable
  object.

          You use it by inheriting from it and defining the draw() method and
  any of the on_*() event handlers you need.
  !*/
...
class button_action : public mouse_over_event {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents the clicking action of a push button.  It
  provides simple callbacks that can be used to make various kinds of button
          widgets.
...
protected:
  ...
  /*!
      requires
          ...
          - is_enabled() == true
          - is_hidden() == false
          - the area in parent_window() defined by get_rect() has been
  invalidated. (This means you don't have to call invalidate_rectangle())
          - is called whenever this object transitions from the state where
  ...
  // does nothing by default
  virtual void on_button_up(bool mouse_over) {}
  /*!
      requires
          - enable_events() has been called
          - mutex drawable::m is locked
...
public:
  ...
  template <typename T>
  void set_click_handler(T &object, void (T::*event_handler)(button &self));
  /*!
      requires
          - event_handler is a valid pointer to a member function in T
      ensures
  ...
  template <typename T>
  void set_button_down_handler(T &object,
...
class scroll_bar : public drawable {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents a scroll bar.  The slider_pos() of the scroll
  bar ranges from 0 to max_slider_pos().  The 0 position of the scroll_bar is in
  the top or left side of the scroll_bar depending on its orientation.
...
public:
  ...
  scroll_bar(drawable_window &w, bar_orientation orientation);
  /*!
      ensures
          - #*this is properly initialized
          - #*this has been added to window w
  ...
  void set_slider_pos(unsigned long pos);
  /*!
      ensures
          - if (pos < 0) then
              - #slider_pos() == 0
  ...
  /*!
      ensures
          - returns the current position of the slider box within the scroll
  bar.
  !*/
  ...
  /*!
      ensures
          - returns the number of positions that the slider bar will jump when
  the user clicks on the empty gaps above or below the slider bar. (note that
  the slider will jump less than the jump size if it hits the end of the scroll
  bar)
  !*/
  ...
  template <typename T>
  void set_scroll_handler(T &object, void (T::*event_handler)());
  /*!
      requires
          - event_handler is a valid pointer to a member function in T
      ensures
          - The event_handler function is called whenever the user causes the
  slider box to move.
  ...
  void set_scroll_handler(const any_function<void()> &event_handler);
  /*!
      ensures
          - The event_handler function is called whenever the user causes the
  slider box to move.
...
class widget_group : public drawable {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents a grouping of drawable widgets.  It doesn't
  draw anything itself, rather it lets you manipulate the position, enabled
          status, and visibility of a set of widgets as a group.
  !*/

public:
  widget_group(drawable_window &w);
  /*!
      ensures
          - #*this is properly initialized
          - #*this has been added to window w
  ...
  /*!
      ensures
          ...
          - else
              - #size() == size()
          - The following conditions apply to this function as well as to all of
  the following functions so long as is_member(widget) == true: enable(),
  disable(), hide(), show(), set_z_order(), and set_pos().
              - #widget.left() == left()+x
  ...
  /*!
      ensures
          - returns true if widget is currently in this object, returns false
  otherwise.
  !*/
...
class image_widget : public draggable {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents a draggable image.  You give it an image to
  display by calling set_image().

          Also note that initially the draggable area is empty so it won't be
          draggable unless you call set_draggable_area() to some non-empty
  region.
...
public:
  image_widget(drawable_window &w);
  /*!
      ensures
          - #*this is properly initialized
          - #*this has been added to window w
...
public:
  ...
  virtual rectangle get_left_size() const {
    return rectangle();
  ...
  virtual rectangle get_right_size() const {
    return rectangle();
  } // return empty rect by default
  /*!
      ensures
  ...
  /*!
      ...
      ensures
          - draws the background of the menu_item on the canvas c at the
  location given by rect.
  !*/

  virtual void draw_left(const canvas &c, const rectangle &rect,
                         const bool enabled, const bool is_selected) const {}
  /*!
      requires
          - the mutex drawable::m is locked
      requires
  ...
  virtual void draw_middle(const canvas &c, const rectangle &rect,
                           const bool enabled,
                           const bool is_selected) const = 0;
  /*!
      requires
          - the mutex drawable::m is locked
      requires
          ...
          - is_selected == true if the menu_item is to be drawn selected
      ensures
          - draws the middle part of the menu_item on the canvas c at the
  location given by rect.
  !*/
  ...
  /*!
      ...
      requires
          ...
          - is_selected == true if the menu_item is to be drawn selected
      ensures
          - draws the right part of the menu_item on the canvas c at the
  location given by rect.
  !*/
};
...
public:
  template <typename T>
  menu_item_text(const std::string &str, T &object,
                 void (T::*on_click_handler)(), unichar hotkey = 0);
  /*!
      ensures
          - The text of this menu item will be str
          - the on_click_handler function is called on object when this
  menu_item clicked by the user.
          - #get_hot_key() == hotkey
  !*/
...
public:
  menu_item_submenu(const std::string &str, unichar hotkey = 0);
  /*!
      ensures
          - The text of this menu item will be str
          - #get_hot_key() == hotkey
  ...
  // overloads for wide character strings
  menu_item_submenu(const std::wstring &str, unichar hotkey = 0);

  menu_item_submenu(const dlib::ustring &str, unichar hotkey = 0);
};
...
class zoomable_region : public drawable {
  /*
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          protected member functions.  It provides functions for converting
     between pixel locations and the points in our 2D Cartesian graph so that
     when the user is scrolling/zooming the widget you can still determine where
          things are to be placed on the screen and what screen pixels
     correspond to in the Cartesian graph.
          ...
          the way out, and max_graph_point() which corresponds to the lower
     right corner when zoomed all the way out. The value of max_graph_point() is
  ...
  /*!
      ensures
          ...
          - When the user zooms out using the mouse wheel:
              - #zoom_scale() == zoom_scale() * zoom_increment()
          - So this function returns the number that determines how much the
  zoom changes when the mouse wheel is moved.
  !*/
  ...
  /*!
      ensures
          - returns the minimum allowed value of zoom_scale()
            (i.e. this is the number that determines how far out the user is
  allowed to zoom)
  !*/
  ...
  /*!
      ensures
          - returns the maximum allowed value of zoom_scale()
            (i.e. this is the number that determines how far in the user is
  allowed to zoom)
  !*/
...
protected:
  ...
  point graph_to_gui_space(const vector<double, 2> &graph_point) const;
  /*!
      requires
          - mutex drawable::m is locked
      ensures
  ...
  vector<double, 2> gui_to_graph_space(const point &pixel_point) const;
  /*!
      requires
          - mutex drawable::m is locked
      ensures
          - returns the point in Cartesian graph space that corresponds to the
  given pixel location
  !*/

  vector<double, 2> max_graph_point() const;
  /*!
      requires
          - mutex drawable::m is locked
      ensures
          - returns the pixel farthest from the graph point (0,0) that is still
            in the graph.  I.e. returns the point in graph space that
  corresponds to the lower right corner of the display_rect() when we are zoomed
            all the way out.
  !*/
  ...
  /*!
      ...
      ensures
          ...
          - if (min_zoom_scale() <= new_scale && new_scale <= max_zoom_scale())
  then
  ...
  void center_display_at_graph_point(const vector<double, 2> &graph_point);
  /*!
      requires
          - mutex drawable::m is locked
      ensures
  ...
  /*!
      ...
      ensures
          - on_view_changed() is called whenever the user causes the view of the
            zoomable_region to change.  That is, this function is called when
  the user scrolls or zooms around in the region.
  !*/
...
class scrollable_region : public drawable {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          scroll bar that allows the user to scroll around on a region that is
  much larger than the actual area occupied by this object on the screen.

          To use this object you inherit from it and make use of its public and
          protected member functions.  It provides a function, total_rect(),
  that tells you where the 2D region is on the screen.  You draw your stuff
          ...
          scroll bars the position of total_rect() is updated accordingly,
  causing the widget's content to scroll across the screen.
  !*/
...
public:
  ...
  /*!
      ensures
          - returns the current position of the vertical scroll bar.
            0 means it is at the top and bigger values represent scroll
  positions closer to the bottom.
  !*/
  ...
  /*!
      ensures
          ...
          - else
              - #horizontal_scroll_pos() == the valid scroll position closest to
  pos
  !*/
  ...
  /*!
      ensures
          ...
          - else
              - #vertical_scroll_pos() == the valid scroll position closest to
  pos
  !*/
...
protected:
  ...
  void scroll_to_rect(const rectangle &r);
  /*!
      requires
          - mutex drawable::m is locked
      ensures
  ...
  /*!
      ...
      ensures
          - on_view_changed() is called whenever the user causes the view of the
            scrollable_region to change.  That is, this function is called when
  the user scrolls around in the region.
  !*/
