...
/*!
    GENERAL REMARKS
        This file defines the drawable interface class and the drawable_window
which is just a window that is capable of displaying drawable objects (i.e.
objects that implement the drawable interface).
...
class drawable_window : public base_window {
  ...
  !*/
public:
  drawable_window(bool resizable = true, bool undecorated = false);
  /*!
      requires
  ...
  void set_background_color(unsigned long red, unsigned long green,
                            unsigned long blue);
  /*!
      ensures
  ...
  rgb_pixel background_color() const;
  /*!
      ensures
...
class drawable {
  /*!
      ...
      EVENT FORWARDING
          ...
          if a drawable registers to receive a certain kind of event then
  whenever its parent window receives that event the drawable object will get a
          ...
          draw() and on_user_event() events.  draw() is only called on a
  drawable object when that object is not hidden.  on_user_event() is only
  called for drawables that the on_user_event()'s first argument specifically
...
public:
  ...
  drawable(drawable_window &w, unsigned long events = 0)
      : m(w.wm), parent(w), hidden(false), enabled(true) {}
  /*!
      ensures
          ...
          - once events_are_enabled() == true this drawable will receive
            the on_user_event() event. (i.e. you always get this event, you
  don't have to enable it by setting something in the events bitset).
  ...
  /*!
      ...
      ensures
          ...
          - *this has been removed from its containing window parent_window()
  and its parent window will no longer try to dispatch events to it. Note that
  this does not trigger a redraw of the parent window.  If you want to do that
  ...
  virtual void set_z_order(long order);
  /*!
      ensures
          - #z_order() == order
  ...
  virtual void set_main_font(const shared_ptr_thread_safe<font> &f);
  /*!
      ensures
          - #main_font() == f
          - if (events_are_enabled() == true) then
  ...
  const shared_ptr_thread_safe<font> main_font() const;
  /*!
      ensures
  ...
  /*!
      ensures
          - returns true if this object is NOT currently displayed on
  parent_window() and false otherwise.
  !*/
  ...
  const drawable_window &parent_window() const;
  /*!
      ensures
  ...
  /*!
      ensures
          - returns the smallest number, i, that is the next user event number
  you can use in calls to parent.trigger_user_event((void*)this,i).
          - This function exists because of the following scenario.  Suppose
            ...
            solves that problem.  You would define
  derived1::next_free_user_event_number() so that it returned a number bigger
  than any user event numbers used by derived1 or its ancestors.  Then derived2
  could just call derived1::next_free_user_event_number() to find out what
  numbers it could use.
  !*/
...
protected:
  ...
  virtual void on_user_event(int i) {}
  /*!
      requires
          - events_are_enabled() == true
          - mutex m is locked
          - is called whenever the parent window receives an on_user_event(p,i)
  event where p == this.  (i.e. this is just a redirect of on_user_event for
  ...
  /*!
      requires
          ...
          - this is just the base_window::on_window_resized() event forwarded to
            this object.  See the gui_core specs for the details about this
  event. ensures
          - does not change the state of mutex m.
  !*/
