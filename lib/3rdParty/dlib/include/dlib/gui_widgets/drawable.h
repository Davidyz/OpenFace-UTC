...
class drawable_window : public base_window {
  /*!
      ...
      CONVENTION
          ...
          - widgets == this binary search tree contains every drawable that is
  in this window.  It is a mapping of each drawable's z-order to a pointer to
  said drawable.
          ...
          - mouse_wheel == this is a set of drawables that are in this window
  and want to receive the mouse wheel events.
          - mouse_click == this is a set of drawables that are in this window
  and want to receive the mouse click events.
          - window_resized == this is a set of drawables that are in this window
  and want to receive the window_resized event.
          ...
          - window_moved == this is a set of drawables that are in this window
  and want to receive window move events.
          ...
          - event_id == a number we use to tag events so we don't end up sending
            an event to a drawable more than once.  This could happen if one of
  the event handlers does something to reset the enumerator while we are
            dispatching events (e.g. creating a new widget).
  !*/
public:
  drawable_window(bool resizable = true, bool undecorated = false)
      : base_window(resizable, undecorated), bg_color(rgb_pixel(212, 208, 200)),
        lastx(-1), lasty(-1), event_id(1) {}
...
protected:
  ...
  void on_user_event(void *p, int i);
...
class drawable {
  ...
  /*!
      ...
      CONVENTION
          - events == a bitset specifying what events this drawable is to
  receive.
...
public:
  ...
  drawable(drawable_window &w, unsigned long events_ = 0)
      : m(w.wm), parent(w), hidden(false), enabled(true), lastx(w.lastx),
        lasty(w.lasty), mfont(default_font::get_font()), z_order_value(0),
        events(events_), enabled_events(false), event_id(0) {}
  ...
  virtual void set_main_font(const std::shared_ptr<font> &f) {
    auto_mutex M(m);
    mfont = f;
    parent.invalidate_rectangle(rect);
  }

  const std::shared_ptr<font> main_font() const {
    auto_mutex M(m);
    return mfont;
  }
  ...
  base_window &parent_window() { return parent; }
...
protected:
  ...
  virtual void on_user_event(int) {}
  ...
  virtual void on_mouse_up(unsigned long, unsigned long, long, long) {}
  ...
  virtual void on_wheel_up(unsigned long) {}

  virtual void on_wheel_down(unsigned long) {}
  ...
  virtual void on_string_put(const std::wstring &) {}
};
