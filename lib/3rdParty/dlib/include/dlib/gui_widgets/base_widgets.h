  ...
  /*!
      ...
      CONVENTION
          - if (the user is holding the left button down over this object) then
              - drag == true
              - x == the x position of the mouse relative to the upper left
  corner of this object.
              - y == the y position of the mouse relative to the upper left
  corner of this object.
          - else
              - drag == false
  !*/
...
public:
  draggable(drawable_window &w, unsigned long events = 0)
      : drawable(w, events | MOUSE_MOVE | MOUSE_CLICK), drag(false) {}

  virtual ~draggable() = 0;

  rectangle draggable_area() const {
    auto_mutex M(m);
    return area;
  }

  void set_draggable_area(const rectangle &area_) {
    auto_mutex M(m);
    area = area_;
  }

protected:
  bool is_being_dragged() const { return drag; }
...
public:
  mouse_over_event(drawable_window &w, unsigned long events = 0)
      : drawable(w, events | MOUSE_MOVE), is_mouse_over_(false) {}

  virtual ~mouse_over_event() = 0;
...
public:
  button_action(drawable_window &w, unsigned long events = 0)
      : mouse_over_event(w, events | MOUSE_MOVE | MOUSE_CLICK),
        is_depressed_(false), seen_click(false) {}

  virtual ~button_action() = 0;
...
protected:
  ...
  virtual void on_button_up(bool) {}
  ...
  void on_mouse_up(unsigned long btn, unsigned long, long x, long y);
...
class widget_group : public drawable {
  /*!
      ...
      CONVENTION
          - widgets contains all the drawable objects and their relative
...
public:
  widget_group(drawable_window &w) : drawable(w) {
    rect = rectangle(0, 0, -1, -1);
    enable_events();
  }

  virtual ~widget_group() { disable_events(); }

  void empty();

  void add(drawable &widget, unsigned long x, unsigned long y);

  void add(widget_group &widget, unsigned long x, unsigned long y);

  bool is_member(const drawable &widget) const;

  void remove(const drawable &widget);

  size_t size() const;

  void set_pos(long x, long y);

  void set_z_order(long order);
...
public:
  image_widget(drawable_window &w) : draggable(w) { enable_events(); }
...
public:
  tooltip(drawable_window &w) : mouse_over_event(w, MOUSE_CLICK) {}

  ~tooltip() { disable_events(); }
  ...
  const std::wstring wtext() const { return convert_utf32_to_wstring(utext()); }
...
private:
  ...
  public:
    tooltip_window(const std::shared_ptr<font> &f)
        : base_window(false, true), pad(3), mfont(f) {}
...
public:
  button(drawable_window &w) : button_action(w), btn_tooltip(w) {
  ...
  void set_name(const std::wstring &name_);

  void set_name(const dlib::ustring &name_);
  ...
  template <typename T>
  void set_click_handler(T &object, void (T::*event_handler_)(button &)) {
    auto_mutex M(m);
  ...
  template <typename T>
  void set_button_down_handler(T &object, void (T::*event_handler)()) {
    auto_mutex M(m);
    button_down_handler = make_mfp(object, event_handler);
  }

  void set_button_down_handler(const any_function<void()> &event_handler) {
    auto_mutex M(m);
    button_down_handler = event_handler;
  }

  template <typename T>
  void set_button_up_handler(T &object,
                             void (T::*event_handler)(bool mouse_over)) {
    auto_mutex M(m);
    button_up_handler = make_mfp(object, event_handler);
  }

  void set_button_up_handler(const any_function<void(bool)> &event_handler) {
    auto_mutex M(m);
    button_up_handler = event_handler;
  }

  template <typename T>
  void set_button_down_handler(T &object, void (T::*event_handler)(button &)) {
    auto_mutex M(m);
...
protected:
  void draw(const canvas &c) const {
    style->draw_button(c, rect, enabled, *mfont, lastx, lasty, name_,
                       is_depressed());
  }
...
public:
  enum bar_orientation { HORIZONTAL, VERTICAL };

  scroll_bar(drawable_window &w, bar_orientation orientation_);

  virtual ~scroll_bar();

  bar_orientation orientation() const;

  void set_length(unsigned long length);

  long max_slider_pos() const;

  void set_max_slider_pos(long mpos);

  void set_slider_pos(long pos);

  long slider_pos() const;

  template <typename T> void set_scroll_handler(T &object, void (T::*eh)()) {
    auto_mutex M(m);
    scroll_handler = make_mfp(object, eh);
  }

  void set_scroll_handler(const any_function<void()> &eh) {
    auto_mutex M(m);
    scroll_handler = eh;
  }
  ...
  void set_jump_size(long js);

  long jump_size() const;
...
private:
  ...
  void b1_up(bool mouse_over);

  void b2_down();

  void b2_up(bool mouse_over);

  void top_filler_down();

  void top_filler_up(bool mouse_over);

  void bottom_filler_down();

  void bottom_filler_up(bool mouse_over);

  void on_user_event(int i);

  void delayed_set_slider_pos(unsigned long dpos);
  ...
  public:
    filler(drawable_window &w, scroll_bar &object, void (scroll_bar::*down)(),
           void (scroll_bar::*up)(bool))
        : button_action(w), my_scroll_bar(object) {
  ...
  private:
    void draw(const canvas &c) const {
      my_scroll_bar.style->draw_scroll_bar_background(c, rect, enabled, lastx,
                                                      lasty, is_depressed());
    }

    void on_button_down() { bdown(); }

    void on_button_up(bool mouse_over) { bup(mouse_over); }
  ...
  public:
    slider_class(drawable_window &w, scroll_bar &object,
                 void (scroll_bar::*handler)())
        : draggable(w, MOUSE_MOVE), mouse_in_widget(false),
          my_scroll_bar(object) {
      callback = make_mfp(object, handler);
      enable_events();
    }

    ~slider_class() { disable_events(); }
  ...
  private:
    virtual void on_mouse_move(unsigned long state, long x, long y) {
    ...
    void on_mouse_leave() {
      if (mouse_in_widget &&
          my_scroll_bar.style->redraw_on_mouse_over_slider()) {
        mouse_in_widget = false;
        parent.invalidate_rectangle(rect);
      }
    }
    ...
    void on_drag() { callback(); }

    void draw(const canvas &c) const {
      my_scroll_bar.style->draw_scroll_bar_slider(c, rect, enabled, lastx,
                                                  lasty, is_being_dragged());
    }
  ...
  unsigned long get_slider_size() const;
  /*!
      ensures
          - returns the length in pixels the slider should have based on the
  current state of this scroll bar
  !*/
...
public:
  ...
  virtual rectangle get_left_size() const { return rectangle(); }
  virtual rectangle get_middle_size() const = 0;
  virtual rectangle get_right_size() const { return rectangle(); }

  virtual unichar get_hot_key() const { return 0; }

  virtual void draw_background(const canvas &, const rectangle &, const bool,
                               const bool) const {}

  virtual void draw_left(const canvas &, const rectangle &, const bool,
                         const bool) const {}

  virtual void draw_middle(const canvas &, const rectangle &, const bool,
                           const bool) const = 0;

  virtual void draw_right(const canvas &, const rectangle &, const bool,
                          const bool) const {}

  virtual void on_click() const {}

  virtual bool has_click_event() const { return false; }
};

// ----------------------------------------------------------------------------------------

class menu_item_submenu : public menu_item {
  void initialize(unichar hk) {
    ...
    if (hk != 0) {
      ...
      if (pos != std::string::npos) {
        ...
        rectangle r1 =
            f->compute_cursor_rect(rectangle(100000, 100000), str, pos);
        rectangle r2 =
            f->compute_cursor_rect(rectangle(100000, 100000), str, pos + 1);
...
public:
  menu_item_submenu(const std::string &str, unichar hk = 0)
      : text(convert_wstring_to_utf32(convert_mbstring_to_wstring(str))),
        f(default_font::get_font()), hotkey(hk) {
    initialize(hk);
  }

  menu_item_submenu(const std::wstring &str, unichar hk = 0)
      : text(convert_wstring_to_utf32(str)), f(default_font::get_font()),
        hotkey(hk) {
    initialize(hk);
  }

  menu_item_submenu(const dlib::ustring &str, unichar hk = 0)
      : text(str), f(default_font::get_font()), hotkey(hk) {
    initialize(hk);
  }

  virtual unichar get_hot_key() const { return hotkey; }

  virtual rectangle get_middle_size() const {
    unsigned long width, height;
    f->compute_size(text, width, height);
    return rectangle(width + 30, height);
  }

  virtual rectangle get_right_size() const { return rectangle(15, 5); }

  virtual void draw_background(const canvas &c, const rectangle &rect,
                               const bool enabled,
    ...
    if (enabled && is_selected) {
      fill_rect_with_vertical_gradient(c, rect, rgb_alpha_pixel(0, 200, 0, 100),
                                       rgb_alpha_pixel(0, 0, 0, 100));
      draw_rectangle(c, rect, rgb_alpha_pixel(0, 0, 0, 100));
    }
  }

  virtual void draw_right(const canvas &c, const rectangle &rect,
                          const bool enabled, const bool) const {
    if (c.intersect(rect).is_empty())
      return;
  ...
  virtual void draw_middle(const canvas &c, const rectangle &rect,
                           const bool enabled, const bool) const {
    if (c.intersect(rect).is_empty())
      return;
...
class menu_item_text : public menu_item {
  void initialize(const any_function<void()> &event_handler_, unichar hk) {
...
public:
  template <typename T>
  menu_item_text(const std::string &str, T &object, void (T::*event_handler_)(),
                 unichar hk = 0)
      : text(convert_wstring_to_utf32(convert_mbstring_to_wstring(str))),
        f(default_font::get_font()), hotkey(hk) {
  ...
  menu_item_text(const std::string &str,
                 const any_function<void()> &event_handler_, unichar hk = 0)
      : text(convert_wstring_to_utf32(convert_mbstring_to_wstring(str))),
        f(default_font::get_font()), hotkey(hk) {
  ...
  template <typename T>
  menu_item_text(const std::wstring &str, T &object,
                 void (T::*event_handler_)(), unichar hk = 0)
      : text(convert_wstring_to_utf32(str)), f(default_font::get_font()),
        hotkey(hk) {
    initialize(make_mfp(object, event_handler_), hk);
  }

  menu_item_text(const std::wstring &str,
                 const any_function<void()> &event_handler_, unichar hk = 0)
      : text(convert_wstring_to_utf32(str)), f(default_font::get_font()),
        hotkey(hk) {
    initialize(event_handler_, hk);
  ...
  template <typename T>
  menu_item_text(const dlib::ustring &str, T &object,
                 void (T::*event_handler_)(), unichar hk = 0)
      : text(str), f(default_font::get_font()), hotkey(hk) {
    initialize(make_mfp(object, event_handler_), hk);
  }

  menu_item_text(const dlib::ustring &str,
                 const any_function<void()> &event_handler_, unichar hk = 0)
      : text(str), f(default_font::get_font()), hotkey(hk) {
    initialize(event_handler_, hk);
  }
  ...
  virtual void on_click() const { action(); }

  virtual bool has_click_event() const { return true; }
...
public:
  virtual rectangle get_middle_size() const { return rectangle(10, 4); }
...
class popup_menu : public base_window {
  /*!
      ...
      CONVENTION
          ...
          - win_rect == a rectangle that is the exact size of this window and
  with its upper left corner at (0,0)
          ...
          - for all valid i:
              ...
              - item_enabled[i] == true if the ith menu_item is enabled, false
  otherwise
              ...
              - line_rects[i] == the rectangle for the entire line on which the
  ith menu item appears.
              - if (submenus[i] != 0) then
                  - the ith menu item has a submenu and it is pointed to by
  submenus[i]
...
public:
  ...
  template <typename menu_item_type>
  unsigned long add_menu_item(const menu_item_type &new_item) {
    auto_mutex M(wm);
    bool t = true;
    ...
    if (cur_rect.width() <
        left_width + middle_width + right.width() + 2 * item_pad) {
      cur_rect = resize_rect_width(cur_rect, left_width + middle_width +
                                                 right.width() + 2 * item_pad);
      recalc_rect_positions = true;
    }
    ...
    if (recalc_rect_positions) {
      ...
      for (unsigned long i = 0; i < left_rects.size(); ++i) {
        ...
        right_rects[i] = move_rect(
            right_rects[i],
            x + cur_rect.width() - right_rects[i].width() - item_pad, y);
    ...
    right =
        move_rect(right, x + cur_rect.width() - right.width() - item_pad, y);
    rectangle line(
        move_rect(rectangle(cur_rect.width(), all.height() + 2 * item_pad),
                  x - item_pad, y - item_pad));
  ...
  template <typename menu_item_type>
  unsigned long add_submenu(const menu_item_type &new_item,
                            popup_menu &submenu) {
    auto_mutex M(wm);
  ...
  void enable_menu_item(unsigned long idx);

  void disable_menu_item(unsigned long idx);
  ...
  template <typename T>
  void set_on_hide_handler(T &object, void (T::*event_handler)()) {
    auto_mutex M(wm);
  ...
  bool forwarded_on_keydown(unsigned long key, bool is_printable,
                            unsigned long state);
...
class zoomable_region : public drawable {
  /*!
      ...
      CONVENTION
          ...
          - zoom_scale() == scale
          - if (the user is currently dragging the graph around via the mouse)
  then
          ...
          - gui_to_graph_space(point(display_rect.left(),display_rect.top())) ==
  gr_orig
  !*/

public:
  zoomable_region(drawable_window &w, unsigned long events = 0);

  virtual ~zoomable_region() = 0;
  ...
  void set_zoom_increment(double zi);

  double zoom_increment() const;

  void set_max_zoom_scale(double ms);

  void set_min_zoom_scale(double ms);

  double min_zoom_scale() const;

  double max_zoom_scale() const;
...
protected:
  ...
  point graph_to_gui_space(const vector<double, 2> &p) const;

  vector<double, 2> gui_to_graph_space(const point &p) const;

  point max_graph_point() const;

  rectangle display_rect() const;

  double zoom_scale() const;

  void set_zoom_scale(double new_scale);

  void center_display_at_graph_point(const vector<double, 2> &p);
...
private:
  ...
  void adjust_origin(const point &gui_p, const vector<double, 2> &graph_p);
  /*!
      ensures
          - adjusts gr_orig so that we are as close to the following as
  possible:
              - graph_to_gui_space(graph_p) == gui_p
              - gui_to_graph_space(gui_p) == graph_p
  !*/

  vector<double, 2> gr_orig;  // point in graph space such that it's gui space
                              // point is the upper left of display_rect_
  vector<double, 2> lr_point; // point in graph space such that it is at the
                              // lower right corner of the screen at max zoom
  ...
  bool mouse_drag_screen; // true if the user is dragging the white background
                          // area
  vector<double, 2>
      drag_screen_point; // the starting point the mouse was at in graph space
                         // for the background area drag
...
class scrollable_region : public drawable {
  /*!
      ...
      CONVENTION
          ...
          - if (the user is currently dragging the total_rect around with a
  mouse drag) then
              - user_is_dragging_mouse == true
              - drag_origin == the point the mouse was at, with respect to
  total_rect, when the dragging started
          - else
              - user_is_dragging_mouse == false
  !*/

public:
  scrollable_region(drawable_window &w, unsigned long events = 0);

  virtual ~scrollable_region() = 0;
  ...
  unsigned long horizontal_mouse_wheel_scroll_increment() const;

  unsigned long vertical_mouse_wheel_scroll_increment() const;

  void set_horizontal_mouse_wheel_scroll_increment(unsigned long inc);

  void set_vertical_mouse_wheel_scroll_increment(unsigned long inc);

  unsigned long horizontal_scroll_increment() const;

  unsigned long vertical_scroll_increment() const;

  void set_horizontal_scroll_increment(unsigned long inc);

  void set_vertical_scroll_increment(unsigned long inc);

  long horizontal_scroll_pos() const;

  long vertical_scroll_pos() const;

  void set_horizontal_scroll_pos(long pos);

  void set_vertical_scroll_pos(long pos);
...
protected:
  ...
  const rectangle &display_rect() const;

  void set_total_rect_size(unsigned long width, unsigned long height);

  const rectangle &total_rect() const;

  void scroll_to_rect(const rectangle &r_);
...
private:
  bool need_h_scroll() const;

  bool need_v_scroll() const;
...
public:
  popup_menu_region(drawable_window &w);

  virtual ~popup_menu_region();

  void set_size(unsigned long width, unsigned long height);

  void set_rect(const rectangle &new_rect);
  ...
  bool popup_menu_visible() const {
    auto_mutex M(m);
    return popup_menu_shown;
  }
