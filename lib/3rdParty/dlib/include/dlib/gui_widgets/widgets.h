...
public:
  label(drawable_window &w) : drawable(w), text_color_(0, 0, 0) {
    enable_events();
  }
  ...
  void set_text_color(const rgb_pixel color);

  const rgb_pixel text_color() const;
...
public:
  toggle_button(drawable_window &w)
      : button_action(w), btn_tooltip(w), checked(false) {
  ...
  void set_size(unsigned long width_, unsigned long height_);

  void set_tooltip_text(const std::string &text);

  void set_tooltip_text(const std::wstring &text);

  void set_tooltip_text(const ustring &text);

  const std::string tooltip_text() const;
  ...
  template <typename style_type> void set_style(const style_type &style_) {
    auto_mutex M(m);
    style.reset(new style_type(style_));
    rect =
        move_rect(style->get_min_size(name_, *mfont), rect.left(), rect.top());
    parent.invalidate_rectangle(rect);
  }

  template <typename T>
  void set_click_handler(T &object, void (T::*event_handler_)()) {
    auto_mutex M(m);
    event_handler = make_mfp(object, event_handler_);
    event_handler_self.clear();
  }

  void set_click_handler(const any_function<void()> &event_handler_) {
    auto_mutex M(m);
  ...
  template <typename T>
  void set_click_handler(T &object,
...
protected:
  void draw(const canvas &c) const {
    style->draw_toggle_button(c, rect, enabled, *mfont, lastx, lasty, name_,
                              is_depressed(), checked);
  }

  void on_button_up(bool mouse_over);
  ...
  /*!
      ...
      CONVENTION
          ...
          - cursor_x == the x coordinate of the cursor relative to the left side
            of rect.  i.e. the number of pixels that separate the cursor from
  the left side of the text_field.
          ...
          - text_pos == the index of the first letter in text_ that appears in
            this text field.
          - text_width == the width of text_[text_pos] though
  text_[text.size()-1]
          ...
          - if (shift_pos != -1) then
              ...
              - the shift key is being held down or the left mouse button is
                being held down.
              - shift_pos == the position of the cursor when the shift or mouse
...
public:
  text_field(drawable_window &w)
      : drawable(w, MOUSE_CLICK | KEYBOARD_EVENTS | MOUSE_MOVE | STRING_PUT),
        text_color_(0, 0, 0), bg_color_(255, 255, 255), text_width(0),
        text_pos(0), recent_movement(false), has_focus(false),
        cursor_visible(false), cursor_pos(0), highlight_start(0),
        highlight_end(-1), shift_pos(-1), t(*this, &text_field::timer_action),
        right_click_menu(w) {
    ...
    right_click_menu.menu().add_menu_item(
        menu_item_text("Cut", *this, &text_field::on_cut, 't'));
    right_click_menu.menu().add_menu_item(
        menu_item_text("Copy", *this, &text_field::on_copy, 'C'));
    right_click_menu.menu().add_menu_item(
        menu_item_text("Paste", *this, &text_field::on_paste, 'P'));
    right_click_menu.menu().add_menu_item(
        menu_item_text("Delete", *this, &text_field::on_delete_selected, 'D'));
    right_click_menu.menu().add_menu_item(menu_item_separator());
    right_click_menu.menu().add_menu_item(
        menu_item_text("Select All", *this, &text_field::on_select_all, 'A'));
  ...
  void set_text(const dlib::ustring &text_);
  ...
  void set_background_color(const rgb_pixel color);

  const rgb_pixel background_color() const;
  ...
  template <typename T>
  void set_text_modified_handler(T &object, void (T::*event_handler)()) {
    auto_mutex M(m);
    text_modified_handler = make_mfp(object, event_handler);
  }

  template <typename T>
  void set_enter_key_handler(T &object, void (T::*event_handler)()) {
    auto_mutex M(m);
    enter_key_handler = make_mfp(object, event_handler);
  }

  void set_text_modified_handler(const any_function<void()> &event_handler) {
    auto_mutex M(m);
    text_modified_handler = event_handler;
  }

  void set_enter_key_handler(const any_function<void()> &event_handler) {
    auto_mutex M(m);
    enter_key_handler = event_handler;
  }

  template <typename T>
  void set_focus_lost_handler(T &object, void (T::*event_handler)()) {
    auto_mutex M(m);
    focus_lost_handler = make_mfp(object, event_handler);
  }

  void set_focus_lost_handler(const any_function<void()> &event_handler) {
    auto_mutex M(m);
...
private:
  ...
  void move_cursor(unsigned long pos);
  /*!
      requires
          - pos <= text_.size()
      ensures
...
protected:
  ...
  void on_mouse_down(unsigned long btn, unsigned long state, long x, long y,
  ...
  void on_keydown(unsigned long key, bool is_printable, unsigned long state);

  void on_string_put(const std::wstring &str);
};
...
public:
  text_box(drawable_window &w)
      : scrollable_region(w, MOUSE_CLICK | KEYBOARD_EVENTS | MOUSE_MOVE |
                                 STRING_PUT),
        text_color_(0, 0, 0), bg_color_(255, 255, 255), recent_movement(false),
        has_focus(false), cursor_visible(false), cursor_pos(0),
        highlight_start(0), highlight_end(-1), shift_pos(-1),
        t(*this, &text_box::timer_action), right_click_menu(w) {
    ...
    cursor_rect = mfont->compute_cursor_rect(
        rectangle(padding, padding, 1000000, 1000000), text_, 0);
    ...
    right_click_menu.menu().add_menu_item(
        menu_item_text("Cut", *this, &text_box::on_cut, 't'));
    right_click_menu.menu().add_menu_item(
        menu_item_text("Copy", *this, &text_box::on_copy, 'C'));
    right_click_menu.menu().add_menu_item(
        menu_item_text("Paste", *this, &text_box::on_paste, 'P'));
    right_click_menu.menu().add_menu_item(
        menu_item_text("Delete", *this, &text_box::on_delete_selected, 'D'));
    right_click_menu.menu().add_menu_item(menu_item_separator());
    right_click_menu.menu().add_menu_item(
        menu_item_text("Select All", *this, &text_box::on_select_all, 'A'));
...
private:
  ...
  // The reason for using user actions here rather than just having the timer
  // just call what it needs directly is to avoid a potential deadlock during
  // destruction of this widget.
  void timer_action() {
    parent.trigger_user_event(this,
                              scrollable_region::next_free_user_event_number());
  }
  /*!
      ensures
          - flips the state of cursor_visible
...
public:
  check_box(drawable_window &w) : toggle_button(w) {
...
public:
  radio_button(drawable_window &w) : toggle_button(w) {
...
class tabbed_display : public drawable {
  /*!
      ...
      CONVENTION
          ...
          - left_pad == the amount of padding in a tab to the left of the name
  string.
          - right_pad == the amount of padding in a tab to the right of the name
  string.
          - top_pad == the amount of padding in a tab to the top of the name
  string.
          - bottom_pad == the amount of padding in a tab to the bottom of the
  name string.
...
public:
  tabbed_display(drawable_window &w);

  virtual ~tabbed_display();

  void set_size(unsigned long width, unsigned long height);

  void set_number_of_tabs(unsigned long num);

  unsigned long selected_tab() const;

  unsigned long number_of_tabs() const;

  const std::string tab_name(unsigned long idx) const;

  const std::wstring tab_wname(unsigned long idx) const;

  const dlib::ustring &tab_uname(unsigned long idx) const;

  void set_tab_name(unsigned long idx, const std::string &new_name);
  ...
  template <typename T>
  void set_click_handler(T &object, void (T::*eh)(unsigned long new_idx,
  ...
  void set_tab_group(unsigned long idx, widget_group &group);
...
private:
  ...
  void draw_tab(const rectangle &tab, const canvas &c) const;
  /*!
      ensures
...
public:
  named_rectangle(drawable_window &w);
  ...
  void wrap_around(const rectangle &rect);
...
public:
  mouse_tracker(drawable_window &w);
...
public:
  box_win(const std::string &title_, const std::string &message_);

  box_win(const std::wstring &title_, const std::wstring &message_);

  box_win(const dlib::ustring &title_, const dlib::ustring &message_);
...
private:
  static void deleter_thread(void *param);
...
public:
  blocking_box_win(const std::string &title_, const std::string &message_);

  blocking_box_win(const std::wstring &title_, const std::wstring &message_);

  blocking_box_win(const dlib::ustring &title_, const dlib::ustring &message_);
...
inline void message_box(const std::string &title, const std::string &message,
...
inline void message_box_blocking(const std::string &title,
...
class list_box : public scrollable_region, public enumerable<const S> {
  /*!
      INITIAL VALUE
          - ms_enabled == false
          - items.size() == 0
      ...
      CONVENTION
          ...
          - items[i].width == the width of items[i].name as given by
  font::compute_size()
          - items[i].height == the height of items[i].name as given by
  font::compute_size()
...
public:
  list_box(drawable_window &w);

  ~list_box();

  bool is_selected(unsigned long index) const;
  ...
  template <typename T> void get_selected(T &list) const {
    auto_mutex M(m);
    list.clear();
  ...
  template <typename T> void load(const T &list) {
    auto_mutex M(m);
    items.clear();
    unsigned long i = 0;
  ...
  const S &operator[](unsigned long index) const;

  bool multiple_select_enabled() const;
  ...
  template <typename T>
  void set_double_click_handler(T &object, void (T::*eh)(unsigned long index)) {
    auto_mutex M(m);
    event_handler = make_mfp(object, eh);
  }

  void set_double_click_handler(const any_function<void(unsigned long)> &eh) {
    auto_mutex M(m);
    event_handler = eh;
  }

  template <typename T>
  void set_click_handler(T &object, void (T::*eh)(unsigned long index)) {
    auto_mutex M(m);
    single_click_event_handler = make_mfp(object, eh);
  }

  void set_click_handler(const any_function<void(unsigned long)> &eh) {
    auto_mutex M(m);
    single_click_event_handler = eh;
  }
  ...
  unsigned long get_selected() const;
...
private:
  ...
  template <typename SS> struct data {
    SS name;
    bool is_selected;
    unsigned long width;
...
class box_win : public drawable_window {
public:
  box_win(const std::string &title, bool has_text_field = false);
...
private:
  ...
  void enter_folder(const std::string &folder_name);

  void on_dirs_click(unsigned long idx);

  void on_files_click(unsigned long idx);

  void on_files_double_click(unsigned long);

  void on_cancel_click();

  void on_open_click();

  void on_path_button_click(toggle_button &btn);

  bool set_dir(const std::string &dir);
...
template <typename T>
void open_file_box(T &object, void (T::*event_handler)(const std::string &)) {
...
inline void
open_file_box(const any_function<void(const std::string &)> &event_handler) {
  using namespace open_file_box_helper;
  box_win *win = new box_win("Open File", true);
  win->set_click_handler(event_handler);
}
...
template <typename T>
void open_existing_file_box(T &object,
...
template <typename T>
void save_file_box(T &object, void (T::*event_handler)(const std::string &)) {
  using namespace open_file_box_helper;
...
inline void
save_file_box(const any_function<void(const std::string &)> &event_handler) {
  using namespace open_file_box_helper;
  box_win *win = new box_win("Save File", true);
  win->set_click_handler(event_handler);
}
...
public:
  ...
  void set_number_of_menus(unsigned long num);

  unsigned long number_of_menus() const;

  void set_menu_name(unsigned long idx, const std::string name,
                     char underline_ch = '\0');

  void set_menu_name(unsigned long idx, const std::wstring name,
                     char underline_ch = '\0');

  void set_menu_name(unsigned long idx, const dlib::ustring name,
                     char underline_ch = '\0');

  const std::string menu_name(unsigned long idx) const;

  const std::wstring menu_wname(unsigned long idx) const;

  const dlib::ustring menu_uname(unsigned long idx) const;

  popup_menu &menu(unsigned long idx);

  const popup_menu &menu(unsigned long idx) const;
...
protected:
  ...
  void on_mouse_down(unsigned long btn, unsigned long, long x, long y, bool);

  void on_mouse_move(unsigned long, long x, long y);

  void on_keydown(unsigned long key, bool is_printable, unsigned long state);

private:
  void show_menu(unsigned long i);
  ...
  /*!
      ...
      CONVENTION
          ...
          - for all valid i:
              ...
              - graph_.node(i).data.str_rect == a rectangle sized to contain
  graph_.node(i).data.name

          - if (the last mouse click in our parent window as in our
  display_rect_ ) then
...
public:
  directed_graph_drawer(drawable_window &w)
      : zoomable_region(w, MOUSE_CLICK | MOUSE_WHEEL | KEYBOARD_EVENTS),
        radius(25), edge_selected(false), last_mouse_click_in_display(false) {
    mouse_drag = false;
    selected_node = 0;

    // Whenever you make your own drawable (or inherit from draggable or
    // button_action) you have to remember to call this function to enable the
    // events.  The idea here is that you can perform whatever setup you need to
    // do to get your object into a valid state without needing to worry about
  ...
  const typename graph_type::node_type &graph_node(unsigned long i) const {
  ...
  typename graph_type::node_type &graph_node(unsigned long i) {
    DLIB_ASSERT(i < number_of_nodes(),
                "\tgraph_type::node_type& directed_graph_drawer::graph_node(i)"
                    << "\n\ti:                 " << i
                    << "\n\tnumber_of_nodes(): " << number_of_nodes());
  ...
  const graph_type &graph() const { return external_graph; }

  void save_graph(std::ostream &out) {
    auto_mutex M(m);
  ...
  void load_graph(std::istream &in) {
    auto_mutex M(m);
  ...
  void set_node_label(unsigned long i, const std::string &label) {
  ...
  void set_node_color(unsigned long i, rgb_pixel color) {
    auto_mutex M(m);
    DLIB_ASSERT(i < number_of_nodes(),
  ...
  rgb_pixel node_color(unsigned long i) const {
    auto_mutex M(m);
    DLIB_ASSERT(i < number_of_nodes(),
  ...
  const std::string node_label(unsigned long i) const {
    auto_mutex M(m);
    DLIB_ASSERT(i < number_of_nodes(),
  ...
  template <typename T>
  void set_node_selected_handler(T &object,
  ...
  template <typename T>
  void set_node_deselected_handler(T &object,
                                   void (T::*event_handler_)(unsigned long)) {
    auto_mutex M(m);
  ...
  template <typename T>
  void set_node_deleted_handler(T &object, void (T::*event_handler_)()) {
    auto_mutex M(m);
    node_deleted_handler = make_mfp(object, event_handler_);
  }

  void set_node_deleted_handler(const any_function<void()> &event_handler_) {
    auto_mutex M(m);
    node_deleted_handler = event_handler_;
  }

  template <typename T>
  void set_graph_modified_handler(T &object, void (T::*event_handler_)()) {
    auto_mutex M(m);
    graph_modified_handler = make_mfp(object, event_handler_);
  }

  void set_graph_modified_handler(const any_function<void()> &event_handler_) {
    auto_mutex M(m);
    graph_modified_handler = event_handler_;
  }

protected:
  void on_keydown(unsigned long key, bool, unsigned long) {
  ...
  void on_mouse_down(unsigned long btn, unsigned long state, long x, long y,
    ...
    if (display_rect().contains(x, y) &&
      ...
      for (unsigned long i = 0; i < graph_.number_of_nodes(); ++i) {
        ...
        if ((p - n).length() < radius) {
          ...
          if (selected_node != i) {
            ...
            if (selected_node != graph_.number_of_nodes() &&
                node_deselected_handler.is_set())
              node_deselected_handler(selected_node);
      ...
      if (click_hit_node == false) {
        for (unsigned long n = 0;
             n < graph_.number_of_nodes() && edge_selected == false; ++n) {
          const dlib::vector<double, 2> parent_center(
              graph_to_gui_space(graph_.node(n).data.p));
          for (unsigned long e = 0; e < graph_.node(n).number_of_children() &&
                                    edge_selected == false;
               ++e) {
            const dlib::vector<double, 2> child_center(
                graph_to_gui_space(graph_.node(n).child(e).data.p));

            rectangle area;
            area += parent_center;
            area += child_center;
            // if the point(x,y) is between the two nodes then lets consider it
            // further
            if (area.contains(point(x, y))) {
              p = point(x, y);
              const dlib::vector<double> z(0, 0, 1);
              // find the distance from the line between the two nodes
              const dlib::vector<double, 2> perpendicular(
                  z.cross(parent_center - child_center).normalize());
    ...
    if (display_rect().contains(x, y) && btn == base_window::LEFT &&
      ...
      for (unsigned long i = 0; i < graph_.number_of_nodes(); ++i) {
        ...
        if ((p - n).length() < radius) {
          ...
          // the same node
          if (graph_.has_edge(selected_node, i) == false &&
              selected_node != i &&
  ...
  void draw(const canvas &c) const {
    ...
    for (unsigned long i = 0; i < graph_.number_of_nodes(); ++i) {
      ...
      const rectangle circle_area(
          centered_rect(center, 2 * (rad + 8), 2 * (rad + 8)));
      ...
      for (unsigned long j = 0; j < graph_.node(i).number_of_parents(); ++j) {
        ...
        if (edge_selected && selected_edge_child == i &&
            selected_edge_parent == graph_.node(i).parent(j).index()) {
          color.red = 255;
          // we need to be careful when drawing this line to not draw it over
          // the node dots since it has a different color from them and would
          // look weird
        ...
        if (area.intersect(circle_area).is_empty() == false) {
          ...
          for (double i = 0; i < 8 * zoom_scale(); i += 0.1)
            draw_line(c, (r + v * i) + cross * i, (r + v * i) - cross * i,
                      color, area);
        }
      }
    }
    ...
    for (unsigned long i = 0; i < graph_.number_of_nodes(); ++i) {
      ...
      if (area.intersect(circle_area).is_empty() == false) {
        ...
        if (circle_area.contains(str_rect)) {
          mfont->draw_string(c, str_rect, sout.str(), color, 0,
          ...
          if (graph_.node(i).data.name.size() > 0) {
            ...
            str_rect =
                centered_rect(center.x(), center.y() - rad - mfont->height(),
                              str_rect.width(), str_rect.height());
            mfont->draw_string(c, str_rect, graph_.node(i).data.name, 0, 0,
                               std::string::npos, area);
          }
        }
        sout.str("");
...
private:
  ...
  typename graph_type::template rebind<
      data, char, typename graph_type::mem_manager_type>::other graph_;
...
public:
  text_grid(drawable_window &w);

  ~text_grid();

  void set_grid_size(unsigned long rows, unsigned long cols);

  unsigned long number_of_columns() const;

  unsigned long number_of_rows() const;

  int next_free_user_event_number() const;

  rgb_pixel border_color() const;

  void set_border_color(rgb_pixel color);

  const std::string text(unsigned long row, unsigned long col) const;
  ...
  void set_text(unsigned long row, unsigned long col, const std::string &str);
  ...
  const rgb_pixel text_color(unsigned long row, unsigned long col) const;

  void set_text_color(unsigned long row, unsigned long col,
                      const rgb_pixel color);

  const rgb_pixel background_color(unsigned long row, unsigned long col) const;

  void set_background_color(unsigned long row, unsigned long col,
                            const rgb_pixel color);

  bool is_editable(unsigned long row, unsigned long col) const;

  void set_editable(unsigned long row, unsigned long col, bool editable);

  void set_column_width(unsigned long col, unsigned long width);

  void set_row_height(unsigned long row, unsigned long height);
  ...
  template <typename T>
  void set_text_modified_handler(T &object,
...
private:
  ...
  rectangle get_text_rect(unsigned long row, unsigned long col) const;

  rectangle get_bg_rect(unsigned long row, unsigned long col) const;

  struct data_type {
    data_type()
        : text_color(0, 0, 0), bg_color(255, 255, 255), first(0),
          is_editable(true) {}
  ...
  void move_cursor(long row, long col, long new_cursor_pos);
      ...
      CONVENTION
          ...
          - if (rect_is_selected) then
              - selected_rect == the index in overlay_rects of the user selected
                rectangle.
              - last_right_click_pos == the last place we saw the user right
  click the mouse.
              - parts_menu.is_enabled() == true
              - if (it is actually a part of this rect that is selected) then
                  - selected_part_name == the name of the part in
  overlay_rects[selected_rect].parts that is selected.
              - else
          ...
          - if (moving_overlay) then
              - moving_rect == the index in overlay_rects that the move applies
  to.
              - if (moving_what == MOVING_PART) then
                  - moving_part_name == the name of the part in
                    overlay_rects[moving_rect] that is being moved around with
  the mouse.
              - else
...
public:
  image_display(drawable_window &w);

  ~image_display();

  template <typename image_type> void set_image(const image_type &new_img) {
    auto_mutex M(m);
    ...
    if (num_rows(new_img) != img.nr() || num_columns(new_img) != img.nc()) {
      if (zoom_in_scale != 1)
        set_total_rect_size(num_columns(new_img) * zoom_in_scale,
                            num_rows(new_img) * zoom_in_scale);
      else
        set_total_rect_size(num_columns(new_img) / zoom_out_scale,
                            num_rows(new_img) / zoom_out_scale);
    } else {
      parent.invalidate_rectangle(rect);
    }
  ...
  virtual void set_pos(long x, long y) {
    auto_mutex lock(m);
    scrollable_region::set_pos(x, y);
    parts_menu.set_rect(rect);
  }

  virtual void set_size(unsigned long width, unsigned long height) {
    auto_mutex lock(m);
  ...
  struct overlay_rect {
    ...
    overlay_rect(const rectangle &r, pixel_type p)
        : rect(r), crossed_out(false) {
      assign_pixel(color, p);
    }

    template <typename pixel_type>
    overlay_rect(const rectangle &r, pixel_type p, const std::string &l)
        : rect(r), label(l), crossed_out(false) {
      assign_pixel(color, p);
    }
    ...
    overlay_rect(const rectangle &r, pixel_type p, const std::string &l,
        ...
        : rect(r), label(l), parts(parts_), crossed_out(false) {
      assign_pixel(color, p);
    }
  ...
  struct overlay_circle {
    ...
    overlay_circle(const point &center_, const int radius_, pixel_type p,
                   const std::string &l)
        : center(center_), radius(radius_), label(l) {
      assign_pixel(color, p);
    }
  ...
  rectangle get_image_display_rect() const;

  std::vector<overlay_rect> get_overlay_rects() const;

  void set_default_overlay_rect_label(const std::string &label);

  std::string get_default_overlay_rect_label() const;

  void set_default_overlay_rect_color(const rgb_alpha_pixel &color);

  rgb_alpha_pixel get_default_overlay_rect_color() const;

  template <typename T>
  void set_overlay_rects_changed_handler(T &object,
  ...
  void set_overlay_rect_selected_handler(
      T &object, void (T::*event_handler_)(const overlay_rect &orect)) {
    auto_mutex M(m);
  ...
  template <typename T>
  void set_image_clicked_handler(T &object,
                                 void (T::*event_handler_)(const point &p,
                                                           bool is_double_click,
                                                           unsigned long btn)) {
    auto_mutex M(m);
  ...
  void set_image_clicked_handler(
      const any_function<void(const point &p, bool is_double_click,
                              unsigned long btn)> &event_handler_) {
    auto_mutex M(m);
    image_clicked_handler = event_handler_;
  }

  void add_labelable_part_name(const std::string &name);
  ...
  bool overlay_editing_is_enabled() const {
    auto_mutex M(m);
    return overlay_editing_enabled;
  }
...
private:
  ...
  void on_part_add(const std::string &part_name);

  rectangle get_rect_on_screen(unsigned long idx) const;

  rectangle get_rect_on_screen(rectangle orect) const;
  ...
  virtual int next_free_user_event_number() const {
    return scrollable_region::next_free_user_event_number() + 1;
  }
  // The reason for using user actions here rather than just having the timer
  // just call what it needs directly is to avoid a potential deadlock during
  ...
  any_function<void(const point &p, bool is_double_click, unsigned long btn)>
...
public:
  perspective_display(drawable_window &w);
  ...
  struct overlay_dot {
    ...
    overlay_dot(const vector<double> &p_) : p(p_) { assign_pixel(color, 255); }

    template <typename pixel_type>
    overlay_dot(const vector<double> &p_, pixel_type color_) : p(p_) {
      assign_pixel(color, color_);
    }
  ...
  void add_overlay(const std::vector<overlay_dot> &overlay);

  void clear_overlay();

  template <typename T>
  void set_dot_double_clicked_handler(
      T &object, void (T::*event_handler_)(const vector<double> &)) {
    auto_mutex M(m);
...
private:
  ...
  static bool compare_second(const std::pair<overlay_dot, float> &a,
...
public:
  ...
  perspective_window() : disp(*this) {
    set_size(100, 100);
    on_window_resized();
    show();
  }

  perspective_window(const std::vector<dlib::vector<double>> &point_cloud)
      : disp(*this) {
    set_size(100, 100);
    on_window_resized();
  ...
  void clear_overlay() { disp.clear_overlay(); }

  template <typename pixel_type>
  void add_overlay(const vector<double> &p1, const vector<double> &p2,
                   pixel_type p) {
...
public:
  ...
  image_window(const image_type &img)
      : gui_img(*this), window_has_closed(false), have_last_click(false),
        mouse_btn(0), clicked_signaler(this->wm), have_last_keypress(false),
  ...
  template <typename image_type>
  image_window(const image_type &img, const std::string &title)
      : gui_img(*this), window_has_closed(false), have_last_click(false),
        mouse_btn(0), clicked_signaler(this->wm), have_last_keypress(false),
        tie_input_events(false) {
    gui_img.set_image_clicked_handler(*this, &image_window::on_image_clicked);
  ...
  template <typename image_type> void set_image(const image_type &img) {
    const unsigned long padding =
        scrollable_region_style_default().get_border_size();
    auto_mutex M(wm);
    gui_img.set_image(img);

    // Only ever mess with the size of the window if the user is giving us an
    // image that is a different size.  Otherwise we assume that they will have
    // already sized the window to whatever they feel is reasonable for an image
    // of the current size.
    ...
    for (unsigned long i = 0; i < object.num_parts(); ++i) {
      if (object.part(i) != OBJECT_PART_NOT_PRESENT) {
        if (i < part_names.size())
          ...
          temp.push_back(
              overlay_circle(object.part(i), 7, rgb_pixel(0, 255, 0)));
      }
    }
  ...
  void add_overlay(const std::vector<full_object_detection> &objects,
    ...
    for (unsigned long i = 0; i < objects.size(); ++i) {
      rtemp.push_back(
          overlay_rect(objects[i].get_rect(), rgb_pixel(255, 0, 0)));
    }
      ...
      for (unsigned long j = 0; j < objects[i].num_parts(); ++j) {
        if (objects[i].part(j) != OBJECT_PART_NOT_PRESENT) {
          if (j < part_names.size())
            temp.push_back(overlay_circle(objects[i].part(j), 7,
                                          rgb_pixel(0, 255, 0), part_names[j]));
          else
            temp.push_back(
                overlay_circle(objects[i].part(j), 7, rgb_pixel(0, 255, 0)));
        }
      }
    }
  ...
  template <typename pixel_type> void add_overlay(const line &l, pixel_type p) {
    add_overlay(image_display::overlay_line(l.p1(), l.p2(), p));
  }

  void add_overlay(const line &l) { add_overlay(l, rgb_pixel(255, 0, 0)); }
  ...
  bool get_next_double_click(point &p, unsigned long &mouse_button);

  void tie_events();

  void untie_events();

  bool events_tied() const;
  ...
  bool get_next_keypress(unsigned long &key, bool &is_printable,
                         unsigned long &state);
...
private:
  ...
  void on_image_clicked(const point &p, bool is_double_click,
                        unsigned long btn);

  virtual void on_keydown(unsigned long key, bool is_printable,
                          unsigned long state);
