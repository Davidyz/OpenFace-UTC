...
namespace buc_impl {
inline void merge_sets(matrix<double> &dists, unsigned long dest,
                       unsigned long src) {
...
struct compare_dist {
  bool operator()(const sample_pair &a, const sample_pair &b) const {
...
unsigned long
bottom_up_cluster(const matrix_exp<EXP> &dists_,
  ...
  // push all the edges in the graph into a priority queue so the best edges to
  // merge come first.
...
struct snl_range {
  ...
  snl_range(double l, double u) : lower(l), upper(u) {
    DLIB_ASSERT(lower <= upper)
  }
...
inline std::vector<snl_range>
segment_number_line(const std::vector<double> &x,
                    const double max_range_width) {
  DLIB_CASSERT(max_range_width >= 0);

  // create initial ranges, one for each value in x.  So initially, all the
  // ranges have width of 0.
  ...
  // We will try two different clustering strategies.  One that does a simple
  // greedy left to right sweep and another that does a bottom up agglomerative
  // clustering.  This first loop runs the greedy left to right sweep.  Then at
  // the end of this routine we will return the results that produced the
  // tightest clustering.
  ...
  // Here we do the bottom up clustering.  So compute the edges connecting our
  // ranges. We will simply say there are edges between ranges if and only if
  // they are immediately adjacent on the number line.
  ...
  // Now create a list of the final ranges.  We will do this by keeping track of
  // which range we already added to final_ranges.
  ...
  // only use the greedy clusters if they found a clustering with fewer
  // clusters. Otherwise, the bottom up clustering probably produced a more
  // sensible clustering.
