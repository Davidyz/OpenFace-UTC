...
inline double newman_cluster_split(
    dlib::rand &rnd, const std::vector<ordered_sample_pair> &edges,
    ...
    matrix<double, 0, 1> &labels, const double eps,
...
!*/
{
  // Scale epsilon so that it is relative to the expected value of an element of
  // a unit vector of length node_degrees.size().
  ...
  while (eig < 0) {
    ...
    for (unsigned long i = 0;
         i < max_iterations && iteration_change > power_iter_eps; ++i) {
...
inline unsigned long newman_cluster_helper(
    ...
    const matrix<double, 0, 1> &Bdiag,        // diag(B) from the Newman paper
    const double &edge_sum,                   // m from the Newman paper
    std::vector<unsigned long> &labels, double modularity_threshold,
    const double eps, const unsigned long max_iterations)
/*!
    ensures
...
{
  matrix<double, 0, 1> l;
  const double modularity = newman_cluster_split(
      rnd, edges, node_degrees, Bdiag, edge_sum, l, eps, max_iterations);
  ...
  if (modularity > modularity_threshold && num_left_split > 0 &&
      num_right_split > 0) {
    ...
    for (unsigned long k = 0; k < edges.size(); ++k) {
      ...
      if (l(i) > 0 && l(j) > 0) {
        split_edges.push_back(
            ordered_sample_pair(left_idx_map[i], left_idx_map[j], d));
        modularity_threshold += d;
      }
    }
    modularity_threshold -=
        sum(left_node_degrees * sum(left_node_degrees)) / (2 * edge_sum);
    modularity_threshold /= 4 * edge_sum;

    unsigned long num_left_clusters;
    std::vector<unsigned long> left_labels;
    num_left_clusters = newman_cluster_helper(
        rnd, split_edges, left_node_degrees, left_Bdiag, edge_sum, left_labels,
        modularity_threshold, eps, max_iterations);
    ...
    for (unsigned long k = 0; k < edges.size(); ++k) {
      ...
      if (l(i) < 0 && l(j) < 0) {
        split_edges.push_back(
            ordered_sample_pair(right_idx_map[i], right_idx_map[j], d));
        modularity_threshold += d;
      }
    }
    modularity_threshold -=
        sum(right_node_degrees * sum(right_node_degrees)) / (2 * edge_sum);
    modularity_threshold /= 4 * edge_sum;

    unsigned long num_right_clusters;
    std::vector<unsigned long> right_labels;
    num_right_clusters = newman_cluster_helper(
        rnd, split_edges, right_node_degrees, right_Bdiag, edge_sum,
        right_labels, modularity_threshold, eps, max_iterations);
...
newman_cluster(const std::vector<ordered_sample_pair> &edges,
               std::vector<unsigned long> &labels, const double eps = 1e-4,
  ...
  dlib::rand rnd;
  return impl::newman_cluster_helper(rnd, edges, node_degrees, Bdiag, edge_sum,
                                     labels, 0, eps, max_iterations);
}

// ----------------------------------------------------------------------------------------

inline unsigned long newman_cluster(const std::vector<sample_pair> &edges,
...
remap_labels(const std::vector<unsigned long> &labels,
             unsigned long &num_labels)
/*!
    ensures
        - This function takes labels and produces a mapping which maps elements
of labels into the most compact range in [0, max] as possible.  In particular,
...
inline double modularity(const std::vector<sample_pair> &edges,
  ...
  const std::vector<unsigned long> &labels_ =
      dlib::impl::remap_labels(labels, num_labels);
...
inline double modularity(const std::vector<ordered_sample_pair> &edges,
                         const std::vector<unsigned long> &labels) {
  const unsigned long num_nodes = max_index_plus_one(edges);
  // make sure requires clause is not broken
  DLIB_ASSERT(labels.size() == num_nodes,
