...
public:
  ...
  virtual void remove_any(T &item) = 0;
  /*!
      requires
          - size() != 0
      ensures
...
template <typename T, typename compare> class asc_remover : public remover<T> {
  /*!
      REQUIREMENTS ON T
          T is swappable by a global swap() and
...
protected:
  // restricted functions
  asc_remover &operator=(const asc_remover &) {
    return *this;
  } // assignment operator
};
...
template <typename domain, typename range> class pair_remover {
...
public:
  ...
  virtual void remove_any(domain &d, range &r) = 0;
  /*!
      requires
...
protected:
  // restricted functions
  pair_remover &operator=(const pair_remover &) {
    return *this;
  } // assignment operator
};
...
protected:
  // restricted functions
  asc_pair_remover &operator=(const asc_pair_remover &) {
    return *this;
  } // assignment operator
};
...
template <typename T> remover<T>::~remover() {}
