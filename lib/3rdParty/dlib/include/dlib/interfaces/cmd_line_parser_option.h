...
template <typename charT> class cmd_line_parser_option {
  /*!
...
public:
  ...
  virtual ~cmd_line_parser_option() = 0;
  ...
  virtual const string_type &group_name() const = 0;
  /*!
      ensures
          - returns the name of the group this option is in.  If no group was
  set for this option then this function returns "".
  !*/
  ...
  virtual unsigned long number_of_arguments() const = 0;
  /*!
      ensures
  ...
  virtual const string_type &argument(unsigned long arg = 0,
                                      unsigned long N = 0) const = 0;
  /*!
      requires
...
protected:
  ...
  cmd_line_parser_option &operator=(const cmd_line_parser_option &) {
    return *this;
  }
};
