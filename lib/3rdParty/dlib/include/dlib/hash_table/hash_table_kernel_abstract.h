...
public:
  ...
  explicit hash_table(unsigned long expnum);
  /*!
      requires
          - expnum < 32
      ensures
...
inline void swap(hash_table<domain, range, mem_manager> &a,
...
template <typename domain, typename range, typename mem_manager>
void deserialize(hash_table<domain, range, mem_manager> &item,
                 std::istream &in);
/*!
