...
template <typename ht_base> class hash_table_kernel_c : public ht_base {
  typedef typename ht_base::domain_type domain;
  typedef typename ht_base::range_type range;

public:
  explicit hash_table_kernel_c(unsigned long expnum) : ht_base(expnum) {
    DLIB_CASSERT(expnum < 32, "\thash_table::hash_table(unsigned long)"
...
template <typename ht_base>
inline void swap(hash_table_kernel_c<ht_base> &a,
...
template <typename ht_base>
void hash_table_kernel_c<ht_base>::remove(const domain &d, domain &d_copy,
                                          range &r) {
  DLIB_CASSERT(
      this->operator[](d) != 0 &&
...
template <typename ht_base>
void hash_table_kernel_c<ht_base>::add(domain &d, range &r) {
...
template <typename ht_base>
void hash_table_kernel_c<ht_base>::destroy(const domain &d) {
  DLIB_CASSERT(
      (*this)[d] != 0,
...
template <typename ht_base>
void hash_table_kernel_c<ht_base>::remove_any(domain &d, range &r) {
  DLIB_CASSERT(
      this->size() != 0 &&
          (static_cast<const void *>(&d) != static_cast<void *>(&r)),
