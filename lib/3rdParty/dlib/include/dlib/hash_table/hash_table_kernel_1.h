  ...
  /*!
      ...
      CONVENTION
          ...
          if (current_element != 0) then
              table[current_bucket] == a pointer to the linked list that
  contains the node pointed to by current_element
          ...
          for all nodes:
              ...
              r == the range element stored in this node which is associated
  with d. next == pointer to the next node in the linked list or next == 0 if
...
public:
  ...
  explicit hash_table_kernel_1(unsigned long expnum);

  virtual ~hash_table_kernel_1();

  void clear();

  unsigned long count(const domain &item) const;
  ...
  const range *operator[](const domain &d) const;

  range *operator[](const domain &d);

  void swap(hash_table_kernel_1 &item);
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
inline void swap(hash_table_kernel_1<domain, range, mem_manager, compare> &a,
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void deserialize(hash_table_kernel_1<domain, range, mem_manager, compare> &item,
                 std::istream &in) {
  try {
    item.clear();
    unsigned long size;
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
hash_table_kernel_1<domain, range, mem_manager, compare>::hash_table_kernel_1(
    unsigned long expnum)
    : hash_size(0), current_element(0), at_start_(true) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
hash_table_kernel_1<domain, range, mem_manager,
                    compare>::~hash_table_kernel_1() {
  for (unsigned long i = 0; i < num_of_buckets; ++i) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void hash_table_kernel_1<domain, range, mem_manager, compare>::clear() {
  if (hash_size > 0) {
    for (unsigned long i = 0; i < num_of_buckets; ++i) {
      // delete this linked list
      node *temp = table[i];
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
size_t hash_table_kernel_1<domain, range, mem_manager, compare>::size() const {
  return hash_size;
}
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
unsigned long hash_table_kernel_1<domain, range, mem_manager, compare>::count(
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void hash_table_kernel_1<domain, range, mem_manager, compare>::add(domain &d,
                                                                   range &r) {
  ...
  // add this new node to the head of the linked list in bucket number
  // hash_value
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void hash_table_kernel_1<domain, range, mem_manager, compare>::destroy(
    const domain &d) {
  node *last;
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void hash_table_kernel_1<domain, range, mem_manager, compare>::remove(
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void hash_table_kernel_1<domain, range, mem_manager, compare>::remove_any(
    domain &d, range &r) {
  unsigned long i = 0;
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
const range *
hash_table_kernel_1<domain, range, mem_manager, compare>::operator[](
    const domain &d) const {
  node *temp = table[hash(d) & mask];
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
range *hash_table_kernel_1<domain, range, mem_manager, compare>::operator[](
    const domain &d) {
  node *temp = table[hash(d) & mask];
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void hash_table_kernel_1<domain, range, mem_manager, compare>::swap(
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
bool hash_table_kernel_1<domain, range, mem_manager, compare>::at_start()
    const {
  return at_start_;
}
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void hash_table_kernel_1<domain, range, mem_manager, compare>::reset() const {
  at_start_ = true;
  current_element = 0;
}
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
bool hash_table_kernel_1<domain, range, mem_manager,
...
hash_table_kernel_1<domain, range, mem_manager, compare>::element() const {
...
hash_table_kernel_1<domain, range, mem_manager, compare>::element() {
  p.d = &(current_element->d);
  p.r = &(current_element->r);
  return p;
}
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
bool hash_table_kernel_1<domain, range, mem_manager, compare>::move_next()
    const {
  if (at_start_) {
    at_start_ = false;
    ...
    if (current_element == 0) {
      ...
      if (current_element->next != 0) {
        ...
        for (current_bucket += 1; current_bucket < num_of_buckets;
             ++current_bucket) {
          if (table[current_bucket] != 0) {
