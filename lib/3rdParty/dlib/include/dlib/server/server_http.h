...
public:
  const Value &operator[](const Key &k) const {
    ...
    typename std::map<Key, Value, Comparer>::const_iterator ci =
        std::map<Key, Value, Comparer>::find(k);
...
struct incoming_things {
  incoming_things(const std::string &foreign_ip_, const std::string &local_ip_,
      ...
      : foreign_ip(foreign_ip_), foreign_port(foreign_port_),
        local_ip(local_ip_), local_port(local_port_) {}
...
private:
  ...
  virtual void on_connect(std::istream &in, std::ostream &out,
    ...
    try {
      ...
      dlog << LERROR << "Error processing request from: " << foreign_ip << " - "
           << e.what();
