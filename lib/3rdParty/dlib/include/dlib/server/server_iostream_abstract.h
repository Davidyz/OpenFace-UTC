...
class server_iostream : public server {
  ...
  /*!
      ...
      THREAD SAFETY
          Note that in on_connect() the input stream in is tied to the output
  stream out.  This means that when you read from in it will modify out and thus
...
protected:
  ...
  /*!
      ensures
          - if (there is a connection currently being serviced with the given
  id) then
              - the specified connection is shutdown. (i.e.
  connection::shutdown() is called on it so the iostreams operating on it will
  return EOF)
  !*/
...
private:
  virtual void on_connect(std::istream &in, std::ostream &out,
                          ...
                          unsigned short local_port, uint64 connection_id) = 0;
  /*!
      requires
          ...
          - in.tie() == &out (i.e. when you read from in it automatically calls
  out.flush())
          ...
          - connection_id == an integer that uniquely identifies this
  connection. It can be used by shutdown_connection() to terminate this
  connection. ensures
          - when the iostreams hit EOF on_connect() will terminate.
            (because this is how clear() signals you the server is shutting
  down)
