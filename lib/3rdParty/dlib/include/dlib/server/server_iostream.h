...
class server_iostream : public server {
  ...
  typedef map<uint64, connection *, memory_manager<char>::kernel_2a>::kernel_1b
      id_map;

public:
  server_iostream() : next_id(0) {}

  ~server_iostream() { server::clear(); }
...
private:
  ...
  void on_connect(connection &con) {
    bool my_fault = true;
    ...
    try {
      ...
      my_fault = false;
      on_connect(in, out, con.get_foreign_ip(), con.get_local_ip(),
                 con.get_foreign_port(), con.get_local_port(), this_con_id);
      ...
      // Seriously though, this way it is obvious to the user that something bad
      // happened since they probably won't have the dlib logger enabled.
      if (!my_fault)
        throw;
    }
  }
