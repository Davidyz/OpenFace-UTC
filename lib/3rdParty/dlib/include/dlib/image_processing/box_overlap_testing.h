...
public:
  test_box_overlap() : iou_thresh(0.5), percent_covered_thresh(1.0) {}

  explicit test_box_overlap(double iou_thresh_,
                            double percent_covered_thresh_ = 1.0)
      : iou_thresh(iou_thresh_),
        percent_covered_thresh(percent_covered_thresh_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(0 <= iou_thresh && iou_thresh <= 1 &&
                    0 <= percent_covered_thresh && percent_covered_thresh <= 1,
                "\t test_box_overlap::test_box_overlap(iou_thresh, "
  ...
  double get_percent_covered_thresh() const { return percent_covered_thresh; }

  double get_iou_thresh() const { return iou_thresh; }
...
inline void serialize(const test_box_overlap &item, std::ostream &out) {
  serialize(item.get_iou_thresh(), out);
  serialize(item.get_percent_covered_thresh(), out);
}

inline void deserialize(test_box_overlap &item, std::istream &in) {
...
find_tight_overlap_tester(const std::vector<std::vector<rectangle>> &rects) {
  ...
  // Relax these thresholds very slightly.  We do this because on some systems
  // the boxes that generated the max values erroneously trigger a box overlap
  // iou even though their percent covered and iou values are *equal* to the
  // thresholds but not greater.  That is, sometimes when double values get
  // moved around they change their values slightly, so this avoids the problems
  // that can create.
...
inline bool overlaps_any_box(const std::vector<rectangle> &rects,
                             const rectangle &rect) {
