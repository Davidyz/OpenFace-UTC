...
inline rectangle compute_box_dimensions(const double width_to_height_ratio,
                                        const double area) {
  ...
  */
  ...
  const int height =
      (int)std::floor(std::sqrt(area / width_to_height_ratio) + 0.5);
...
create_single_box_detection_template(const rectangle &object_box) {
...
inline std::vector<rectangle>
create_overlapped_2x2_detection_template(const rectangle &object_box) {
  std::vector<rectangle> result;

  const point c = center(object_box);

  result.push_back(rectangle() + c + object_box.tl_corner() +
                   object_box.tr_corner());
  result.push_back(rectangle() + c + object_box.bl_corner() +
                   object_box.br_corner());
  result.push_back(rectangle() + c + object_box.tl_corner() +
                   object_box.bl_corner());
  result.push_back(rectangle() + c + object_box.tr_corner() +
                   object_box.br_corner());
...
inline std::vector<rectangle>
create_grid_detection_template(const rectangle &object_box,
