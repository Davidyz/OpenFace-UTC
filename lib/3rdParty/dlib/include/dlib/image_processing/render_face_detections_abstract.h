...
/*!
    ...
    ensures
        - Interprets the given objects as face detections with parts annotated
using either the iBUG face landmark scheme or a 5 point face annotation.  We
then return a set of overlay lines that will draw the objects onto the screen in
a way that properly draws the outline of the face features defined by the part
          locations.
...
/*!
    ...
    ensures
        - This function is identical to the above render_face_detections()
routine except that it takes just a single full_object_detection instead of a
          std::vector of them.
!*/
