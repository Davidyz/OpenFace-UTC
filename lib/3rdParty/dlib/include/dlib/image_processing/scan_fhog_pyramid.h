...
public:
  inline rectangle image_to_feats(const rectangle &rect, int cell_size,
  ...
  inline rectangle feats_to_image(const rectangle &rect, int cell_size,
                                  int filter_rows_padding,
                                  int filter_cols_padding) const {
...
public:
  ...
  inline unsigned long get_detection_window_width() const {
    return window_width;
  }
  ...
  void detect(const feature_vector_type &w,
    ...
    // make sure requires clause is not broken
    DLIB_ASSERT(is_loaded_with_image() && w.size() >= get_num_dimensions(),
  ...
  build_fhog_filterbank(const feature_vector_type &weights) const {
    ...
    DLIB_ASSERT(weights.size() >= get_num_dimensions(),
                "\t fhog_filterbank scan_fhog_pyramid::build_fhog_filterbank()"
                    << "\n\t The number of weights isn't enough to fill out "
                       "the filterbank. "
    ...
    for (unsigned long i = 0; i < temp.filters.size(); ++i) {
      ...
      f = reshape(rowm(weights, range(i * size, (i + 1) * size - 1)), height,
                  width);
      ...
      for (long j = 0; j < w.size(); ++j) {
        if (w(j) != 0) {
          temp.col_filters[i].push_back(
              matrix_cast<float>(colm(u, j) * std::sqrt(w(j))));
          temp.row_filters[i].push_back(
              matrix_cast<float>(colm(v, j) * std::sqrt(w(j))));
        }
      }
    }
  ...
  double get_nuclear_norm_regularization_strength() const {
    return nuclear_norm_regularization_strength;
  }
...
private:
  inline void compute_fhog_window_size(unsigned long &width,
                                       unsigned long &height) const {
    const rectangle rect =
        centered_rect(point(0, 0), window_width, window_height);
    const rectangle temp =
        grow_rect(fe.image_to_feats(rect, cell_size, 1, 1), padding);
    width = temp.width();
    height = temp.height();
  }

  void get_mapped_rect_and_metadata(const unsigned long number_pyramid_levels,
                                    const rectangle &rect,
...
rectangle apply_filters_to_fhog(const fhog_filterbank &w,
  ...
  rectangle area;
  // use the separable filters if they would be faster than running the regular
  // filters.
  if (num_separable_filters >
      w.filters.size() * std::min(w.filters[0].nr(), w.filters[0].nc()) / 3.0) {
    ...
    for (unsigned long i = 1; i < w.filters.size(); ++i) {
      ...
      // overwriting it.
      spatially_filter_image(feats[i], saliency_image, w.filters[i], 1, false,
                             true);
    }
  } else {
    ...
    for (; i < w.row_filters.size(); ++i) {
      for (unsigned long j = 0; j < w.row_filters[i].size(); ++j) {
        if (saliency_image.size() == 0)
          area = float_spatially_filter_image_separable(
              feats[i], saliency_image, w.row_filters[i][j],
              w.col_filters[i][j], scratch, false);
        else
          area = float_spatially_filter_image_separable(
              feats[i], saliency_image, w.row_filters[i][j],
              w.col_filters[i][j], scratch, true);
      }
    }
    if (saliency_image.size() == 0) {
...
void deserialize(scan_fhog_pyramid<T, U> &item, std::istream &in) {
  ...
  if (version != 1)
    throw serialization_error("Unsupported version found when deserializing a "
                              "scan_fhog_pyramid object.");
  ...
  // When developing some feature extractor, it's easy to accidentally change
  // its number of dimensions and then try to deserialize data from an older
  // version of your extractor into the current code.  This check is here to
  // catch that kind of user error.
  long dims;
  deserialize(dims, in);
  if (item.get_num_dimensions() != dims)
    throw serialization_error(
        "Number of dimensions in serialized scan_fhog_pyramid doesn't match "
        "the expected number.");
}
...
void create_fhog_pyramid(const image_type &img,
                         const feature_extractor_type &fe,
                         array<array<array2d<float>>> &feats, int cell_size,
                         int filter_rows_padding, int filter_cols_padding,
                         unsigned long min_pyramid_layer_width,
                         unsigned long min_pyramid_layer_height,
                         unsigned long max_pyramid_levels) {
  ...
  // figure out how many pyramid levels we should be using based on the image
  // size
  pyramid_type pyr;
  do {
  ...
  DLIB_ASSERT(feats[0].size() == fe.get_num_planes(),
              "Invalid feature extractor used with dlib::scan_fhog_pyramid.  "
              "The output does not have the \n"
...
template <typename Pyramid_type, typename feature_extractor_type>
template <typename image_type>
void scan_fhog_pyramid<Pyramid_type, feature_extractor_type>::load(
    const image_type &img) {
  unsigned long width, height;
  compute_fhog_window_size(width, height);
  impl::create_fhog_pyramid<Pyramid_type>(
      img, fe, feats, cell_size, height, width, min_pyramid_layer_width,
...
bool scan_fhog_pyramid<Pyramid_type,
                       feature_extractor_type>::is_loaded_with_image() const {
  return feats.size() != 0;
}
...
void scan_fhog_pyramid<Pyramid_type, feature_extractor_type>::
    copy_configuration(const scan_fhog_pyramid &item) {
  ...
  nuclear_norm_regularization_strength =
      item.nuclear_norm_regularization_strength;
  fe = item.fe;
}
...
scan_fhog_pyramid<Pyramid_type,
                  feature_extractor_type>::get_num_detection_templates() const {
  return 1;
}
...
long scan_fhog_pyramid<Pyramid_type,
                       feature_extractor_type>::get_num_dimensions() const {
  unsigned long width, height;
  compute_fhog_window_size(width, height);
...
unsigned long
scan_fhog_pyramid<Pyramid_type,
                  feature_extractor_type>::get_max_pyramid_levels() const {
...
template <typename Pyramid_type, typename feature_extractor_type>
void scan_fhog_pyramid<Pyramid_type, feature_extractor_type>::
    set_max_pyramid_levels(unsigned long max_levels) {
  // make sure requires clause is not broken
...
namespace impl {
inline bool compare_pair_rect(const std::pair<double, rectangle> &a,
...
void detect_from_fhog_pyramid(const array<array<array2d<float>>> &feats,
                              const feature_extractor_type &fe,
                              const fhog_filterbank &w, const double thresh,
    ...
    for (long r = area.top(); r <= area.bottom(); ++r) {
      for (long c = area.left(); c <= area.right(); ++c) {
        ...
        if (saliency_image[r][c] >= thresh) {
          rectangle rect = fe.feats_to_image(
              centered_rect(point(c, r), det_box_width, det_box_height),
...
inline bool overlaps_any_box(const test_box_overlap &tester,
  ...
  for (unsigned long i = 0; i < rects.size(); ++i) {
    ...
    if (rects[i].weight_index == rect.weight_index &&
        tester(rects[i].rect, rect.rect))
      return true;
  }
  return false;
}
...
void scan_fhog_pyramid<Pyramid_type, feature_extractor_type>::detect(
    const fhog_filterbank &w, std::vector<std::pair<double, rectangle>> &dets,
    const double thresh) const {
  // make sure requires clause is not broken
  ...
  impl::detect_from_fhog_pyramid<pyramid_type>(
      feats, fe, w, thresh, height - 2 * padding, width - 2 * padding,
      cell_size, height, width, dets);
}
...
template <typename Pyramid_type, typename feature_extractor_type>
const rectangle
scan_fhog_pyramid<Pyramid_type, feature_extractor_type>::get_best_matching_rect(
    const rectangle &rect) const {
  rectangle mapped_rect, fhog_rect;
  unsigned long best_level;
  get_mapped_rect_and_metadata(max_pyramid_levels, rect, mapped_rect, fhog_rect,
                               best_level);
  return mapped_rect;
}
...
template <typename Pyramid_type, typename feature_extractor_type>
void scan_fhog_pyramid<Pyramid_type, feature_extractor_type>::
    get_mapped_rect_and_metadata(const unsigned long number_pyramid_levels,
                                 const rectangle &rect, rectangle &mapped_rect,
                                 rectangle &fhog_rect,
                                 unsigned long &best_level) const {
  pyramid_type pyr;
  best_level = 0;
  ...
  for (unsigned long l = 0; l < number_pyramid_levels; ++l) {
    const rectangle rect_fhog_space =
        fe.image_to_feats(pyr.rect_down(rect, l), cell_size, height, width);

    const rectangle win_image_space =
        pyr.rect_up(fe.feats_to_image(centered_rect(center(rect_fhog_space),
                                                    width - 2 * padding,
                                                    height - 2 * padding),
                                      cell_size, height, width),
                    l);
  ...
  mapped_rect = pyr.rect_up(fe.feats_to_image(shrink_rect(fhog_rect, padding),
                                              cell_size, height, width),
                            best_level);
}
...
template <typename Pyramid_type, typename feature_extractor_type>
void scan_fhog_pyramid<Pyramid_type, feature_extractor_type>::
    get_feature_vector(const full_object_detection &obj,
                       feature_vector_type &psi) const {
  // make sure requires clause is not broken
  DLIB_ASSERT(is_loaded_with_image() && psi.size() >= get_num_dimensions() &&
                  obj.num_parts() == 0,
  ...
  unsigned long best_level;
  rectangle fhog_rect;
  get_mapped_rect_and_metadata(feats.size(), obj.get_rect(), mapped_rect,
                               fhog_rect, best_level);
...
template <typename Pyramid_type, typename feature_extractor_type>
void scan_fhog_pyramid<Pyramid_type, feature_extractor_type>::
    set_min_pyramid_layer_size(unsigned long width, unsigned long height) {
  // make sure requires clause is not broken
  DLIB_ASSERT(width > 0 && height > 0,
...
unsigned long
scan_fhog_pyramid<Pyramid_type,
                  feature_extractor_type>::get_min_pyramid_layer_width() const {
...
template <typename Pyramid_type, typename feature_extractor_type>
unsigned long scan_fhog_pyramid<Pyramid_type, feature_extractor_type>::
    get_min_pyramid_layer_height() const {
...
matrix<unsigned char> draw_fhog(
  ...
  typename scan_fhog_pyramid<Pyramid_type,
                             feature_extractor_type>::fhog_filterbank fb =
      detector.get_scanner().build_fhog_filterbank(
          detector.get_w(weight_index));
...
unsigned long num_separable_filters(
  ...
  DLIB_ASSERT(weight_index < detector.num_detectors(),
              "\t unsigned long num_separable_filters()"
                  ...
                  << "\n\t detector.num_detectors(): "
                  << detector.num_detectors());
  DLIB_ASSERT(detector.get_w(weight_index).size() >=
                  detector.get_scanner().get_num_dimensions(),
              "\t unsigned long num_separable_filters()"
                  << "\n\t Invalid arguments were given to this function. "
                  << "\n\t detector.get_w(weight_index).size():         "
...
threshold_filter_singular_values(
  ...
  const long num_planes =
      detector.get_scanner().get_feature_extractor().get_num_planes();
  const long size = width * height;
  ...
  for (unsigned long j = 0; j < detector.num_detectors(); ++j) {
    ...
    if (j == weight_index) {
      ...
      for (long i = 0; i < num_planes; ++i) {
        ...
        set_rowm(weights, range(i * size, (i + 1) * size - 1)) =
            reshape_to_column_vector(f);
      }
    }
  ...
  return object_detector<
      ...
      detector.get_scanner(), detector.get_overlap_tester(), detector_weights);
}
...
void configure_nuclear_norm_regularizer(
  ...
  if (strength != 0) {
    ...
    for (long i = 0; i < num_planes; ++i) {
      prob.add_nuclear_norm_regularizer(i * width * height, height, width,
                                        strength);
    }
...
struct processed_weight_vector<
  ...
  typedef matrix<double, 0, 1> feature_vector_type;
  typedef typename scan_fhog_pyramid<
      Pyramid_type, feature_extractor_type>::fhog_filterbank fhog_filterbank;

  void
  init(const scan_fhog_pyramid<Pyramid_type, feature_extractor_type> &scanner) {
...
void evaluate_detectors(
  ...
  // Find the maximum sized filters and also most extreme pyramiding settings
  // used.
  ...
  for (unsigned long i = 0; i < detectors.size(); ++i) {
    ...
    max_filter_width =
        std::max(max_filter_width, scanner.get_fhog_window_width());
    max_filter_height =
        std::max(max_filter_height, scanner.get_fhog_window_height());
    max_pyramid_levels =
        std::max(max_pyramid_levels, scanner.get_max_pyramid_levels());
    min_pyramid_layer_width = std::min(min_pyramid_layer_width,
                                       scanner.get_min_pyramid_layer_width());
    min_pyramid_layer_height = std::min(min_pyramid_layer_height,
                                        scanner.get_min_pyramid_layer_height());
    if (cell_size != scanner.get_cell_size())
      all_cell_sizes_the_same = false;
  }

  std::vector<rect_detection> dets_accum;
  // Do to the HOG feature extraction to make the fhog pyramid.  Again, note
  // that we are making a pyramid that will work with any of the detectors.  But
  // only if all the cell sizes are the same.  If they aren't then we have to
  // calculate the pyramid for each detector individually.
  array<array<array2d<float>>> feats;
  if (all_cell_sizes_the_same) {
    impl::create_fhog_pyramid<pyramid_type>(
        img, detectors[0].get_scanner().get_feature_extractor(), feats,
        cell_size, max_filter_height, max_filter_width, min_pyramid_layer_width,
        min_pyramid_layer_height, max_pyramid_levels);
  }
  ...
  for (unsigned long i = 0; i < detectors.size(); ++i) {
    ...
    if (!all_cell_sizes_the_same) {
      impl::create_fhog_pyramid<pyramid_type>(
          img, scanner.get_feature_extractor(), feats, scanner.get_cell_size(),
          max_filter_height, max_filter_width, min_pyramid_layer_width,
          min_pyramid_layer_height, max_pyramid_levels);
    }

    const unsigned long det_box_width =
        scanner.get_fhog_window_width() - 2 * scanner.get_padding();
    const unsigned long det_box_height =
        scanner.get_fhog_window_height() - 2 * scanner.get_padding();
    // A single detector object might itself have multiple weight vectors in it.
    // So we need to evaluate all of them.
    for (unsigned d = 0; d < detectors[i].num_detectors(); ++d) {
      const double thresh =
          detectors[i].get_processed_w(d).w(scanner.get_num_dimensions());

      impl::detect_from_fhog_pyramid<pyramid_type>(
          feats, scanner.get_feature_extractor(),
          detectors[i].get_processed_w(d).get_detect_argument(),
          thresh + adjust_threshold, det_box_height, det_box_width, cell_size,
          max_filter_height, max_filter_width, temp_dets);
  ...
  for (unsigned long i = 0; i < dets_accum.size(); ++i) {
    const test_box_overlap tester =
        detectors[dets_accum[i].weight_index].get_overlap_tester();
