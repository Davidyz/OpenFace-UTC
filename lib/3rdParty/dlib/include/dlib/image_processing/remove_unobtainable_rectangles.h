...
namespace impl {
inline bool matches_rect(const std::vector<rectangle> &rects,
                         const rectangle &rect, const double eps) {
  for (unsigned long i = 0; i < rects.size(); ++i) {
    const double score =
        (rect.intersect(rects[i])).area() / (double)(rect + rects[i]).area();
    if (score > eps)
      return true;
  }
...
inline rectangle get_best_matching_rect(const std::vector<rectangle> &rects,
...
template <typename image_array_type, typename image_scanner_type>
std::vector<std::vector<rectangle>> pyramid_remove_unobtainable_rectangles(
  ...
  // If the trainer is setup to automatically fit the overlap tester to the data
  // then we should use the loosest possible overlap tester here.  Otherwise we
  // should use the tester the trainer will use.
  ...
  for (unsigned long k = 0; k < images.size(); ++k) {
    ...
    for (unsigned long j = 0; j < objs.size(); ++j) {
      const rectangle rect =
          trainer.get_scanner().get_best_matching_rect(objs[j]);
      const double score =
          (objs[j].intersect(rect)).area() / (double)(objs[j] + rect).area();
    ...
    // way when we compare them to each other in the following loop we will know
    // if any distinct truth rectangles get mapped to overlapping boxes.
...
template <typename image_array_type, typename Pyramid_type,
          typename Feature_extractor_type>
std::vector<std::vector<rectangle>> remove_unobtainable_rectangles(
    const structural_object_detection_trainer<
        scan_image_pyramid<Pyramid_type, Feature_extractor_type>> &trainer,
    const image_array_type &images,
    std::vector<std::vector<rectangle>> &object_locations) {
  return impl::pyramid_remove_unobtainable_rectangles(trainer, images,
                                                      object_locations);
}
...
std::vector<std::vector<rectangle>> remove_unobtainable_rectangles(
    const structural_object_detection_trainer<
        scan_fhog_pyramid<Pyramid_type, Feature_extractor_type>> &trainer,
    const image_array_type &images,
    std::vector<std::vector<rectangle>> &object_locations) {
  return impl::pyramid_remove_unobtainable_rectangles(trainer, images,
                                                      object_locations);
...
namespace impl {
template <typename image_array_type, typename scanner_type,
...
template <typename T> struct load_to_functor {
  load_to_functor(T &obj_) : obj(obj_) {}
  T &obj;

  template <typename U, typename V> void operator()(const U &u, V &v) {
    obj.load(u, v);
  }
};
} // namespace impl
...
template <typename image_array_type, typename feature_extractor,
          typename box_generator>
std::vector<std::vector<rectangle>> remove_unobtainable_rectangles(
    const structural_object_detection_trainer<
        scan_image_boxes<feature_extractor, box_generator>> &trainer,
    const image_array_type &images,
    std::vector<std::vector<rectangle>> &object_locations) {
  box_generator bg = trainer.get_scanner().get_box_generator();
  return impl::remove_unobtainable_rectangles(bg, trainer, images,
                                              object_locations);
}
...
std::vector<std::vector<rectangle>> remove_unobtainable_rectangles(
    const structural_object_detection_trainer<
        scan_image_custom<feature_extractor>> &trainer,
    const image_array_type &images,
    std::vector<std::vector<rectangle>> &object_locations) {
