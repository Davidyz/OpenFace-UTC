...
double sum_of_rects_in_images(
  ...
  for (unsigned long i = 0; i < rects.size(); ++i) {
    DLIB_ASSERT(rects[i].first < images.size(),
                "\t double sum_of_rects_in_images()"
                    << "\n\t rects[" << i
                    << "].first must refer to a valid image."
                    << "\n\t rects[" << i << "].first: " << rects[i].first
                    << "\n\t images.size(): " << images.size());
  }
#endif

  typedef typename image_traits<typename image_array_type::type>::pixel_type
      pixel_type;
  ...
  for (unsigned long i = 0; i < rects.size(); ++i) {
    ...
    const rectangle rect =
        get_rect(img).intersect(translate_rect(rects[i].second, position));
...
double sum_of_rects_in_images_movable_parts(
  ...
  for (unsigned long i = 0; i < movable_rects.size(); ++i) {
    DLIB_ASSERT(movable_rects[i].first < images.size(),
                "\t double sum_of_rects_in_images_movable_parts()"
                    ...
                    << "].first must refer to a valid image."
                    << "\n\t movable_rects[" << i
                    << "].first: " << movable_rects[i].first
                    << "\n\t images.size(): " << images.size());
    DLIB_ASSERT(center(movable_rects[i].second) == point(0, 0),
                "\t double sum_of_rects_in_images_movable_parts()"
                    << "\n\t movable_rects[" << i
                    << "].second: " << movable_rects[i].second);
  }
#endif
  typedef typename image_traits<typename image_array_type::type>::pixel_type
      pixel_type;
  ...
  for (unsigned long i = 0; i < fixed_rects.size(); ++i) {
    ...
    const rectangle rect = get_rect(img).intersect(
        translate_rect(fixed_rects[i].second, position));
    temp += sum(matrix_cast<ptype>(subm(mat(img), rect)));
  }
  ...
  if (images.size() > 0) {
    ...
    for (unsigned long i = 0; i < movable_rects.size(); ++i) {
      const typename image_array_type::type &img =
          images[movable_rects[i].first];

      sum_filter_assign(img, tempimg, movable_rects[i].second);

      const rectangle rect =
          get_rect(tempimg).intersect(translate_rect(window, position));
...
void find_points_above_thresh(std::vector<std::pair<double, point>> &dets,
                              const image_type &img_, const double thresh,
    ...
    for (long c = 0; c < img.nc(); ++c) {
      ...
      if (val >= thresh) {
        ...
        if (dets.size() < max_dets) {
          ...
          // locations throughout the image rather than just stopping the
          // detection procedure once we hit the max_dets limit. So this method
          // will result in a random subsample of all the detections >= thresh
          // being in dets at the end of scan_image().
          const unsigned long random_index =
              rnd.get_random_32bit_number() % count;
...
std::vector<point> find_peaks(
    const image_type &img_, const double non_max_suppression_radius,
  ...
  for (long r = 1; r + 1 < img.nr(); ++r) {
    for (long c = 1; c + 1 < img.nc(); ++c) {
      ...
      if (val <= img[r - 1][c] || val <= img[r + 1][c] ||
          val <= img[r][c + 1] || val <= img[r][c - 1] ||
          val <= img[r - 1][c - 1] || val <= img[r + 1][c + 1] ||
          val <= img[r - 1][c + 1] || val <= img[r + 1][c - 1]) {
        continue;
      }
  ...
  // First sort the peaks so the strongest peaks come first.  We will greedily
  // accept them and then do the normal peak sorting/non-max suppression thing.
  std::sort(peaks.rbegin(), peaks.rend(),
            [](const pt &a, const pt &b) { return a.first < b.first; });
  std::vector<point> final_peaks;
  const double radius_sqr =
      non_max_suppression_radius * non_max_suppression_radius;

  // If there are a lot of peaks then we will make a mask image and use that to
  // do the non-max suppression since this is fast when peaks.size() is large.
  // Otherwise we will do the simpler thing in the else block that doesn't
  // require us to allocate a temporary mask image.
  if (peaks.size() > 500 && radius_sqr != 0) {
    // hit will record which areas of the image have already been accounted for
    // by some peak.  So it is our mask image.
    ...
    // if peaks.size() is relatively small then this is a faster way to do the
    // non-max suppression.
    for (auto &p : peaks) {
      bool hits_any_existing_peak = false;
      // If the user set the radius to 0 then just copy the peaks to the output
...
void scan_image(std::vector<std::pair<double, point>> &dets,
  ...
  DLIB_ASSERT(
      images.size() > 0 && rects.size() > 0 && all_images_same_size(images),
      "\t void scan_image()"
          << "\n\t Invalid arguments given to this function."
          << "\n\t images.size(): " << images.size() << "\n\t rects.size():  "
          << rects.size() << "\n\t all_images_same_size(images): "
...
void scan_image_movable_parts(
  ...
  for (unsigned long i = 0; i < movable_rects.size(); ++i) {
    ...
    DLIB_ASSERT(center(movable_rects[i].second) == point(0, 0) &&
                ...
                "\t void scan_image_movable_parts()"
                    << "\n\t Invalid arguments given to this function."
                    << "\n\t movable_rects[" << i << "].second: "
                    << movable_rects[i].second << "\n\t movable_rects[" << i
                    << "].second.area(): " << movable_rects[i].second.area());
  }
#endif
