...
namespace impl {
inline bool compare_first(const std::pair<unsigned long, rectangle> &a,
...
determine_object_boxes(const image_scanner_type &scanner,
  ...
  for (unsigned long i = 0; i < rects.size(); ++i) {
    if (scanner.get_num_detection_templates() > 0) {
      ...
      const double match_score =
          (rects[i].intersect(temp).area()) / (double)(rects[i] + temp).area();
  ...
  while (sorted_rects.size() != 0) {
    ...
    object_boxes.push_back(
        centered_rect(point(0, 0), cur.width(), cur.height()));
    ...
    for (unsigned long itr = 0;
      ...
      while (i != sorted_rects.end()) {
        ...
        const double match_score =
            (cur.intersect(temp).area()) / (double)(cur + temp).area();
        if (match_score > min_match_score) {
...
void setup_grid_detection_templates(
  ...
  const std::vector<rectangle> &object_boxes =
      determine_object_boxes(scanner, rects, min_match_score);
  for (unsigned long i = 0; i < object_boxes.size(); ++i) {
    scanner.add_detection_template(
        object_boxes[i],
        create_grid_detection_template(object_boxes[i], cells_x, cells_y));
  }
}
...
void setup_grid_detection_templates_verbose(
  ...
  const std::vector<rectangle> &object_boxes =
      determine_object_boxes(scanner, rects, min_match_score);
  std::cout << "number of detection templates: " << object_boxes.size()
            << std::endl;
  for (unsigned long i = 0; i < object_boxes.size(); ++i) {
