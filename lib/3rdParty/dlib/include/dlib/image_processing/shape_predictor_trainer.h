...
class shape_predictor_trainer {
  /*!
      This thing really only works with unsigned char or rgb_pixel images (since
  we assume the threshold should be in the range [-128,128]).
  !*/
public:
  ...
  void set_padding_mode(padding_mode_t mode) { _padding_mode = mode; }

  padding_mode_t get_padding_mode() const { return _padding_mode; }
  ...
  unsigned long get_num_threads() const { return _num_threads; }
  void set_num_threads(unsigned long num) { _num_threads = num; }
  ...
  train(const image_array &images,
      ...
      for (unsigned long j = 0; j < objects[i].size(); ++j) {
        if (num_parts == 0) {
          ...
          DLIB_CASSERT(objects[i][j].num_parts() != 0,
                       "\t shape_predictor shape_predictor_trainer::train()"
                           << "\n\t You can't give objects that don't have any "
                              "parts to the trainer.");
          part_present.resize(num_parts);
        } else {
          DLIB_CASSERT(
              objects[i][j].num_parts() == num_parts,
              "\t shape_predictor shape_predictor_trainer::train()"
    ...
    DLIB_CASSERT(
        num_parts != 0,
        "\t shape_predictor shape_predictor_trainer::train()"
            << "\n\t You must give at least one full_object_detection if you "
               "want to train a shape model and it must have parts.");
    DLIB_CASSERT(sum(mat(part_present)) == (long)num_parts,
                 "\t shape_predictor shape_predictor_trainer::train()"
                     << "\n\t Each part must appear at least once in this "
                        "training data.  That is, "
                     << "\n\t you can't have a part that is always set to "
                        "OBJECT_PART_NOT_PRESENT.");

    // creating thread pool. if num_threads <= 1, trainer should work in caller
    // thread
    thread_pool tp(_num_threads > 1 ? _num_threads : 0);

    // determining the type of features used for this type of images
    typedef typename std::remove_const<
        typename std::remove_reference<decltype(images[0])>::type>::type
        image_type;
    ...
    const matrix<float, 0, 1> initial_shape =
        populate_training_sample_shapes(objects, samples);
    const std::vector<std::vector<dlib::vector<float, 2>>> pixel_coordinates =
        randomly_sample_pixel_coordinates(initial_shape);

    unsigned long trees_fit_so_far = 0;
    console_progress_indicator pbar(get_cascade_depth() *
    ...
    for (unsigned long cascade = 0; cascade < get_cascade_depth(); ++cascade) {
      ...
      // compute their representations relative to the initial shape first.
      ...
      parallel_for(
          ...
          [&](unsigned long i) {
            impl::extract_feature_pixel_values(
                images[samples[i].image_idx], samples[i].rect,
                samples[i].current_shape, initial_shape, anchor_idx, deltas,
                samples[i].feature_pixel_values);
          },
          1);
...
private:
  static void object_to_shape(
      const full_object_detection &obj, matrix<float, 0, 1> &shape,
      matrix<float, 0, 1>
          &present // a mask telling which elements of #shape are present.
  ) {
    shape.set_size(obj.num_parts() * 2);
    present.set_size(obj.num_parts() * 2);
    const point_transform_affine tform_from_img =
        impl::normalizing_tform(obj.get_rect());
    for (unsigned long i = 0; i < obj.num_parts(); ++i) {
      if (obj.part(i) != OBJECT_PART_NOT_PRESENT) {
        ...
        if (length(p) > 100) {
          std::cout << "Warning, one of your objects has parts that are way "
                       "outside its bounding box!  This is probably an error "
                       "in your annotation."
                    << std::endl;
        }
      } else {
  ...
  template <typename feature_type> struct training_sample {
    /*!
    ...
    CONVENTION
        ...
        - rect == the position of the object in the image_idx-th image.  All
    shape coordinates are coded relative to this rectangle.
  ...
  impl::regression_tree make_regression_tree(
      thread_pool &tp, std::vector<training_sample<feature_type>> &samples,
      const std::vector<dlib::vector<float, 2>> &pixel_coordinates) const {
    using namespace impl;
    ...
    const unsigned long num_split_nodes =
        static_cast<unsigned long>(std::pow(2.0, (double)get_tree_depth()) - 1);
    std::vector<matrix<float, 0, 1>> sums(num_split_nodes * 2 + 1);
    if (tp.num_threads_in_pool() > 1) {
      // Here we need to calculate shape differences and store sum of
      // differences into sums[0] to make it. I am splitting samples into
      // blocks, each block will be processed by separate thread, and the sum of
      // differences of each block is stored into separate place in block_sums
      ...
      const unsigned long block_size =
          std::max(1UL, (num + num_workers - 1) / num_workers);
      ...
      parallel_for(
          ...
          [&](unsigned long block) {
            ...
            const unsigned long block_end =
                std::min(num, block_begin + block_size);
            for (unsigned long i = block_begin; i < block_end; ++i) {
              samples[i].diff_shape =
    ...
    for (unsigned long i = 0; i < num_split_nodes; ++i) {
      ...
      const impl::split_feature split = generate_split(
          tp, samples, range.first, range.second, pixel_coordinates, sums[i],
          sums[left_child(i)], sums[right_child(i)]);
      tree.splits.push_back(split);
      const unsigned long mid =
          partition_samples(split, samples, range.first, range.second);
    ...
    for (unsigned long i = 0; i < parts.size(); ++i) {
      ...
      if (parts[i].second != parts[i].first)
        tree.leaf_values[i] = pointwise_multiply(
            present_counts, sums[num_split_nodes + i] * get_nu());
      else
        tree.leaf_values[i] = zeros_matrix(samples[0].target_shape);

      // now adjust the current shape based on these predictions
      parallel_for(
          tp, parts[i].first, parts[i].second,
          [&](unsigned long j) {
            samples[j].current_shape += tree.leaf_values[i];
            // For parts that aren't present in the training data, we just make
            // sure that the target shape always matches and therefore gives
            // zero error.  So this makes the algorithm simply ignore
            // non-present landmarks.
  ...
  impl::split_feature randomly_generate_split_feature(
    ...
    for (size_t i = 0; i < max_iters; ++i) {
      ...
      while (feat.idx1 == feat.idx2)
        feat.idx2 = rnd.get_integer(get_feature_pool_size());
      const double dist =
          length(pixel_coordinates[feat.idx1] - pixel_coordinates[feat.idx2]);
  ...
  generate_split(thread_pool &tp,
    ...
    const unsigned long num_workers = std::max(1UL, tp.num_threads_in_pool());
    const unsigned long block_size =
        std::max(1UL, (num_test_splits + num_workers - 1) / num_workers);
    ...
    parallel_for(
        ...
        [&](unsigned long block) {
          const unsigned long block_begin = block * block_size;
          const unsigned long block_end =
              std::min(block_begin + block_size, num_test_splits);

          for (unsigned long j = begin; j < end; ++j) {
            for (unsigned long i = block_begin; i < block_end; ++i) {
              if ((float)samples[j].feature_pixel_values[feats[i].idx1] -
                      (float)samples[j].feature_pixel_values[feats[i].idx2] >
                  feats[i].thresh) {
    ...
    for (unsigned long i = 0; i < num_test_splits; ++i) {
      ...
      if (left_cnt[i] != 0 && right_cnt != 0) {
        ...
        score = dot(left_sums[i], left_sums[i]) / left_cnt[i] +
                dot(temp, temp) / right_cnt;
        if (score > best_score) {
          best_score = score;
          best_feat = i;
        }
  ...
  template <typename feature_type>
  unsigned long
  partition_samples(const impl::split_feature &split,
                    std::vector<training_sample<feature_type>> &samples,
                    unsigned long begin, unsigned long end) const {
    // splits samples based on split (sorta like in quick sort) and returns the
    // mid point.  make sure you return the mid in a way compatible with how we
    // walk through the tree.

    unsigned long i = begin;
    for (unsigned long j = begin; j < end; ++j) {
      if ((float)samples[j].feature_pixel_values[split.idx1] -
              (float)samples[j].feature_pixel_values[split.idx2] >
          split.thresh) {
  ...
  matrix<float, 0, 1> populate_training_sample_shapes(
      ...
      std::vector<training_sample<feature_type>> &samples) const {
    samples.clear();
    ...
    for (unsigned long i = 0; i < samples.size(); ++i) {
      if ((i % get_oversampling_amount()) == 0) {
        ...
        while (min(hits) == 0 || iter < 2) {
          ++iter;
          const unsigned long rand_idx =
              rnd.get_random_32bit_number() % samples.size();
        ...
        samples[i].current_shape =
            pointwise_multiply(samples[i].current_shape, reciprocal(hits));
      }
    }
    for (unsigned long i = 0; i < samples.size(); ++i) {
  ...
  /*!
      ensures
          ...
          - for all valid i:
              - pixel_coordinates[i] == a point in the box defined by the
  min/max x/y arguments.
  !*/
  {
    pixel_coordinates.resize(get_feature_pool_size());
    for (unsigned long i = 0; i < get_feature_pool_size(); ++i) {
      pixel_coordinates[i].x() =
          rnd.get_random_double() * (max_x - min_x) + min_x;
      pixel_coordinates[i].y() =
          rnd.get_random_double() * (max_y - min_y) + min_y;
    }
  }

  std::vector<std::vector<dlib::vector<float, 2>>>
  randomly_sample_pixel_coordinates(
      const matrix<float, 0, 1> &initial_shape) const {
    ...
    for (unsigned long i = 0; i < get_cascade_depth(); ++i)
      randomly_sample_pixel_coordinates(pixel_coordinates[i], min_x, min_y,
                                        max_x, max_y);
...
image_dataset_metadata::dataset make_bounding_box_regression_training_data(
  ...
  for (size_t i = 0; i < truth.images.size(); ++i) {
    ...
    for (auto truth_box : truth.images[i].boxes) {
      ...
      auto det =
          max_scoring_element(detections[i], [&truth_box](const rectangle &r) {
            return box_intersection_over_union(r, truth_box.rect);
          });
      if (det.second > 0.5) {
        // Remove any existing parts and replace them with the truth_box
        // corners.
        ...
        // Now replace the bounding truth_box with the detector's bounding
        // truth_box.
