...
public:
  ...
  inline void copy_configuration(const feature_extractor_type &fe);

  inline void copy_configuration(const scan_image_pyramid &item);
  ...
  inline unsigned long
  get_num_stationary_components_per_detection_template() const;

  inline unsigned long get_num_components_per_detection_template() const;
  ...
  inline unsigned long get_max_detections_per_template() const;
  ...
  friend void serialize(const scan_image_pyramid<T, U> &item,
                        std::ostream &out);

  template <typename T, typename U>
  friend void deserialize(scan_image_pyramid<T, U> &item, std::istream &in);

private:
  static bool compare_pair_rect(const std::pair<double, rectangle> &a,
  ...
  friend void deserialize(detection_template &item, std::istream &in) {
    ...
    if (version != 1)
      throw serialization_error(
          "Unexpected version found while deserializing a "
          "dlib::scan_image_pyramid::detection_template object.");
  ...
  get_mapped_rect_and_metadata(const unsigned long number_pyramid_levels,
                               rectangle rect, rectangle &mapped_rect,
                               detection_template &best_template,
                               rectangle &object_box, unsigned long &best_level,
                               unsigned long &detection_template_idx) const;
    ...
    for (long x = -10; x <= 10; x += 10) {
      for (long y = -10; y <= 10; y += 10) {
        ...
        if (a.width() > 10000000 || a.height() > 10000000) {
          DLIB_CASSERT(false,
                       "The image_to_feat_space() routine is outputting "
                       "rectangles of an implausibly "
                           << "\nlarge size.  This means there is probably a "
                              "bug in your feature extractor.");
        }
        a = feats_config.feat_to_image_space(rect);
        if (a.width() > 10000000 || a.height() > 10000000) {
          DLIB_CASSERT(false,
                       "The feat_to_image_space() routine is outputting "
                       "rectangles of an implausibly "
                           << "\nlarge size.  This means there is probably a "
                              "bug in your feature extractor.");
        }
  ...
  feature_extractor_type feats_config; // just here to hold configuration.  use
                                       // it to populate the feats elements.
...
template <typename T, typename U>
void serialize(const scan_image_pyramid<T, U> &item, std::ostream &out) {
  int version = 3;
  serialize(version, out);
...
template <typename T, typename U>
void deserialize(scan_image_pyramid<T, U> &item, std::istream &in) {
  int version = 0;
  deserialize(version, in);
  if (version != 3)
  ...
  if (item.get_num_dimensions() != dims)
    throw serialization_error(
        "Number of dimensions in serialized scan_image_pyramid doesn't match "
        "the expected number.");
}
...
template <typename Pyramid_type, typename Feature_extractor_type>
scan_image_pyramid<Pyramid_type, Feature_extractor_type>::scan_image_pyramid()
    : max_dets_per_template(10000), max_pyramid_levels(1000),
      min_pyramid_layer_width(20), min_pyramid_layer_height(20) {}

// ----------------------------------------------------------------------------------------

template <typename Pyramid_type, typename Feature_extractor_type>
template <typename image_type>
void scan_image_pyramid<Pyramid_type, Feature_extractor_type>::load(
...
unsigned long scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
    get_max_detections_per_template() const {
...
void scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
    set_max_detections_per_template(unsigned long max_dets) {
...
template <typename Pyramid_type, typename Feature_extractor_type>
bool scan_image_pyramid<Pyramid_type,
...
template <typename Pyramid_type, typename Feature_extractor_type>
void scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
    copy_configuration(const feature_extractor_type &fe) {
...
template <typename Pyramid_type, typename Feature_extractor_type>
void scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
    copy_configuration(const scan_image_pyramid &item) {
...
void scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
  ...
  DLIB_ASSERT(
      ...
      "\t void scan_image_pyramid::add_detection_template()"
          << "\n\t The number of rects in this new detection template doesn't "
             "match "
  ...
  for (unsigned long i = 0; i < movable_feature_extraction_regions.size();
       ++i) {
    DLIB_ASSERT(center(movable_feature_extraction_regions[i]) == point(0, 0),
                "Invalid inputs were given to this function."
                    << "\n\t center(movable_feature_extraction_regions[" << i
                    << "]): " << center(movable_feature_extraction_regions[i])
                    << "\n\t this: " << this);
  }
#endif
...
template <typename Pyramid_type, typename Feature_extractor_type>
unsigned long scan_image_pyramid<
    Pyramid_type, Feature_extractor_type>::get_num_detection_templates() const {
...
unsigned long scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
  ...
  DLIB_ASSERT(get_num_detection_templates() > 0,
              "\t unsigned long "
              "scan_image_pyramid::get_num_stationary_components_per_detection_"
              "template()"
                  << "\n\t You need to give some detection templates before "
...
unsigned long scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
  ...
  DLIB_ASSERT(
      get_num_detection_templates() > 0,
      "\t unsigned long "
      "scan_image_pyramid::get_num_movable_components_per_detection_template()"
          << "\n\t You need to give some detection templates before calling "
             "this function. "
          << "\n\t get_num_detection_templates(): "
          << get_num_detection_templates() << "\n\t this: " << this);
...
template <typename Pyramid_type, typename Feature_extractor_type>
unsigned long scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
    get_num_components_per_detection_template() const {
  // make sure requires clause is not broken
  DLIB_ASSERT(get_num_detection_templates() > 0,
              "\t unsigned long "
              "scan_image_pyramid::get_num_components_per_detection_template()"
                  << "\n\t You need to give some detection templates before "
                     "calling this function. "
                  << "\n\t get_num_detection_templates(): "
                  << get_num_detection_templates() << "\n\t this: " << this);
...
template <typename Pyramid_type, typename Feature_extractor_type>
long scan_image_pyramid<Pyramid_type,
  ...
  return feats_config.get_num_dimensions() *
...
template <typename Pyramid_type, typename Feature_extractor_type>
unsigned long
scan_image_pyramid<Pyramid_type,
...
template <typename Pyramid_type, typename Feature_extractor_type>
void scan_image_pyramid<Pyramid_type, Feature_extractor_type>::detect(
    const feature_vector_type &w,
    std::vector<std::pair<double, rectangle>> &dets,
    const double thresh) const {
  // make sure requires clause is not broken
  DLIB_ASSERT(get_num_detection_templates() > 0 && is_loaded_with_image() &&
  ...
  std::vector<std::pair<unsigned int, rectangle>> stationary_region_rects(
      get_num_stationary_components_per_detection_template());
  std::vector<std::pair<unsigned int, rectangle>> movable_region_rects(
      get_num_movable_components_per_detection_template());
  pyramid_type pyr;
  ...
  for (unsigned long l = 0; l < feats.size(); ++l) {
    for (unsigned long i = 0; i < saliency_images.size(); ++i) {
      ...
      const unsigned long offset =
          get_num_detection_templates() + feats_config.get_num_dimensions() * i;

      // build saliency images for pyramid level l
      for (long r = 0; r < feats[l].nr(); ++r) {
        for (long c = 0; c < feats[l].nc(); ++c) {
          const typename feature_extractor_type::descriptor_type &descriptor =
              feats[l](r, c);
    ...
    for (unsigned long i = 0; i < det_templates.size(); ++i) {
      ...
      for (unsigned long j = 0; j < stationary_region_rects.size(); ++j) {
        stationary_region_rects[j] =
            std::make_pair(j, translate_rect(feats[l].image_to_feat_space(
                                                 det_templates[i].rects[j]),
                                             offset));
      }
      for (unsigned long j = 0; j < movable_region_rects.size(); ++j) {
        ...
        const rectangle temp =
            feats[l].image_to_feat_space(det_templates[i].movable_rects[j]);
        movable_region_rects[j] = std::make_pair(
            j + stationary_region_rects.size(),
      ...
      rectangle scaled_object_box =
          feats[l].image_to_feat_space(det_templates[i].object_box);
      scaled_object_box = centered_rect(point(0, 0), scaled_object_box.width(),
                                        scaled_object_box.height());
      ...
      scan_image_movable_parts(point_dets, saliency_images, scaled_object_box,
                               ...
                               thresh + template_specific_thresh,
                               max_dets_per_template);

      // convert all the point detections into rectangles at the original image
...
const rectangle scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
    get_best_matching_rect(const rectangle &rect) const {
  // make sure requires clause is not broken
  DLIB_ASSERT(get_num_detection_templates() > 0,
  ...
  get_mapped_rect_and_metadata(max_pyramid_levels, rect, mapped_rect,
                               best_template, object_box, best_level, junk);
  return mapped_rect;
}
...
void scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
  ...
  for (unsigned long l = 0; l < number_pyramid_levels; ++l) {
    ...
    for (unsigned long t = 0; t < det_templates.size(); ++t) {
      const double match_score =
          get_match_score(det_templates[t].object_box, temp);
  ...
  // Now we translate best_template into the right spot (it should be centered
  // at the location determined by rect) and convert it into the feature image
  // coordinate system.
  ...
  const rectangle scaled_object_box =
      feats_config.image_to_feat_space(best_template.object_box);
  object_box = centered_rect(origin - offset, scaled_object_box.width(),
                             scaled_object_box.height());

  // The input rectangle was mapped to one of the detection templates.  Reverse
  // the process to figure out what the mapped rectangle is in the original
  // input space.
  mapped_rect =
      translate_rect(best_template.object_box,
                     feats_config.feat_to_image_space(origin - offset));
...
full_object_detection scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
  ...
  unsigned long best_level, junk;
  rectangle object_box;
  get_mapped_rect_and_metadata(feats.size(), rect, mapped_rect, best_template,
                               object_box, best_level, junk);
  ...
  for (unsigned long i = 0;
       i < get_num_movable_components_per_detection_template(); ++i) {
    // make the saliency_image for the ith movable part.

    const rectangle part_rect = best_template.movable_rects[i];
    const rectangle area =
        grow_rect(object_box, part_rect.width() / 2, part_rect.height() / 2)
            .intersect(get_rect(feats[best_level]));

    saliency_image.set_size(area.height(), area.width());
    const unsigned long offset =
        get_num_detection_templates() +
        feats_config.get_num_dimensions() *
            (i + get_num_stationary_components_per_detection_template());

    // build saliency image for pyramid level best_level
    for (long r = area.top(); r <= area.bottom(); ++r) {
      for (long c = area.left(); c <= area.right(); ++c) {
        const typename feature_extractor_type::descriptor_type &descriptor =
            feats[best_level](r, c);
...
void scan_image_pyramid<Pyramid_type, Feature_extractor_type>::
  ...
  // make sure requires clause is not broken
  DLIB_ASSERT(
      get_num_detection_templates() > 0 && is_loaded_with_image() &&
  ...
  get_mapped_rect_and_metadata(feats.size(), obj.get_rect(), mapped_rect,
                               best_template, object_box, best_level,
  ...
  for (unsigned long i = 0; i < obj.num_parts(); ++i) {
    if (obj.part(i) != OBJECT_PART_NOT_PRESENT) {
      ...
      point loc = feats[best_level].image_to_feat_space(
          pyr.point_down(obj.part(i), best_level));
  ...
  for (unsigned long j = 0; j < rects.size(); ++j) {
    ...
    const unsigned long offset =
        get_num_detection_templates() +
        feats_config.get_num_dimensions() * template_region_id;
...
template <typename Pyramid_type, typename Feature_extractor_type>
unsigned long scan_image_pyramid<
    Pyramid_type, Feature_extractor_type>::get_min_pyramid_layer_width() const {
