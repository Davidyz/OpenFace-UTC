...
// a tree is just a std::vector<impl::split_feature>.  We use this function to
...
struct regression_tree {
  ...
  inline const matrix<float, 0, 1> &
  operator()(const std::vector<float> &feature_pixel_values,
  ...
  feature_pixel_values.size()
          ...
          - leaf_values.size() == splits.size()+1
            (i.e. there needs to be the right number of leaves given the number
  of splits in the tree) ensures
          - runs through the tree and returns the vector at the leaf we end up
  in.
  ...
  !*/
  {
    i = 0;
    while (i < splits.size()) {
      if ((float)feature_pixel_values[splits[i].idx1] -
              (float)feature_pixel_values[splits[i].idx2] >
          splits[i].thresh)
        i = left_child(i);
      else
        i = right_child(i);
...
inline vector<float, 2> location(const matrix<float, 0, 1> &shape,
                                 unsigned long idx)
/*!
    requires
        - idx < shape.size()/2
...
/*!
    ...
    ensures
        ...
        - for all valid i:
            - pixel_coordinates[i] == location(shape,#anchor_idx[i]) +
#deltas[i]
!*/
{
...
inline point_transform_affine
find_tform_between_shapes(const matrix<float, 0, 1> &from_shape,
                          const matrix<float, 0, 1> &to_shape) {
  DLIB_ASSERT(from_shape.size() == to_shape.size() &&
                  (from_shape.size() % 2) == 0 && from_shape.size() > 0,
              "");
...
inline point_transform_affine normalizing_tform(const rectangle &rect)
/*!
    ensures
        - returns a transform that maps rect.tl_corner() to (0,0) and
rect.br_corner() to (1,1).
!*/
{
  std::vector<vector<float, 2>> from_points, to_points;
...
inline point_transform_affine unnormalizing_tform(const rectangle &rect)
/*!
    ensures
...
void extract_feature_pixel_values(
    const image_type &img_, const rectangle &rect,
    ...
    ensures
        ...
        - for all valid i:
            - #feature_pixel_values[i] == the value of the pixel in img_ that
              corresponds to the pixel identified by
reference_pixel_anchor_idx[i] and reference_pixel_deltas[i] when the pixel is
located relative to current_shape rather than reference_shape.
!*/
{
  ...
  for (unsigned long i = 0; i < feature_pixel_values.size(); ++i) {
    // Compute the point in the current shape corresponding to the i-th pixel
    // and then map it from the normalized shape space into pixel space.
    point p =
        tform_to_img(tform * reference_pixel_deltas[i] +
                     location(current_shape, reference_pixel_anchor_idx[i]));
    if (area.contains(p))
...
public:
  ...
  /*!
      requires
          ...
          - for all valid i:
              - all the index values in forests[i] are less than
  pixel_coordinates[i].size()
          - for all valid i and j:
              - forests[i][j].leaf_values.size() is a power of 2.
                (i.e. we require a tree with all the levels fully filled out.
              - forests[i][j].leaf_values.size() ==
  forests[i][j].splits.size()+1 (i.e. there need to be the right number of
  leaves given the number of splits in the tree)
  !*/
  {
    anchor_idx.resize(pixel_coordinates.size());
    deltas.resize(pixel_coordinates.size());
    ...
    for (unsigned long i = 0; i < pixel_coordinates.size(); ++i)
      impl::create_shape_relative_encoding(initial_shape, pixel_coordinates[i],
                                           anchor_idx[i], deltas[i]);
  }

  unsigned long num_parts() const { return initial_shape.size() / 2; }
...
double test_shape_predictor(
  ...
  for (unsigned long i = 0; i < objects.size(); ++i) {
    for (unsigned long j = 0; j < objects[i].size(); ++j) {
      // Just use a scale of 1 (i.e. no scale at all) if the caller didn't
      // supply any scales.
