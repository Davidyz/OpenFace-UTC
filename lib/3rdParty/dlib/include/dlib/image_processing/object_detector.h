...
struct rect_detection {
  ...
  bool operator<(const rect_detection &item) const {
...
struct full_detection {
  ...
  bool operator<(const full_detection &item) const {
    return detection_confidence < item.detection_confidence;
  }
};
...
template <typename image_scanner_type> struct processed_weight_vector {
  ...
  /*!
      requires
          - w has already been assigned its value.  Note that the point of this
            ...
            instead of a feature_vector_type.  This avoids needing to construct
  the fhog_filterbank during each call to detect and therefore speeds up
            detection.
  !*/
  {}
...
public:
  ...
  object_detector(const image_scanner_type &scanner_,
  ...
  const processed_weight_vector<image_scanner_type> &
  get_processed_w(unsigned long idx = 0) const {
    return w[idx];
  }
...
private:
  bool overlaps_any_box(const std::vector<rect_detection> &rects,
...
void deserialize(object_detector<T> &item, std::istream &in) {
  ...
  if (version == 1) {
    ...
    throw serialization_error("Unexpected version encountered while "
                              "deserializing a dlib::object_detector object.");
  }
}
...
template <typename image_scanner_type>
object_detector<image_scanner_type>::object_detector(
    const image_scanner_type &scanner_, const test_box_overlap &overlap_tester,
    const feature_vector_type &w_)
...
object_detector<image_scanner_type>::object_detector(
  ...
  for (unsigned long i = 0; i < w_.size(); ++i) {
    DLIB_CASSERT(
        w_[i].size() == scanner_.get_num_dimensions() + 1,
        "\t object_detector::object_detector(scanner_,overlap_tester,w_)"
            << "\n\t Invalid inputs were given to this function "
            << "\n\t scanner_.get_num_detection_templates(): "
            << scanner_.get_num_detection_templates() << "\n\t w_[" << i
...
object_detector<image_scanner_type>::object_detector(
  ...
  *this = object_detector(detectors[0].get_scanner(),
                          detectors[0].get_overlap_tester(), weights);
}
...
object_detector<image_scanner_type>::operator=(const object_detector &item) {
  if (this == &item)
    return *this;
...
template <typename image_scanner_type>
template <typename image_type>
void object_detector<image_scanner_type>::operator()(
    const image_type &img, std::vector<rect_detection> &final_dets,
...
template <typename image_type>
void object_detector<image_scanner_type>::operator()(
    const image_type &img, std::vector<full_detection> &final_dets,
    double adjust_threshold) {
  ...
  for (unsigned long i = 0; i < dets.size(); ++i) {
    ...
    final_dets[i].rect = scanner.get_full_object_detection(
        dets[i].rect, w[dets[i].weight_index].w);
  }
}
...
object_detector<image_scanner_type>::operator()(const image_type &img,
                                                double adjust_threshold) {
  std::vector<rect_detection> dets;
  (*this)(img, dets, adjust_threshold);
...
void object_detector<image_scanner_type>::operator()(
  ...
  for (unsigned long i = 0; i < dets.size(); ++i) {
    final_dets.push_back(
        std::make_pair(dets[i].detection_confidence,
                       scanner.get_full_object_detection(
                           dets[i].rect, w[dets[i].weight_index].w)));
  }
}
...
template <typename image_type>
void object_detector<image_scanner_type>::operator()(
    const image_type &img, std::vector<full_object_detection> &final_dets,
    double adjust_threshold) {
  std::vector<rect_detection> dets;
  (*this)(img, dets, adjust_threshold);
  ...
  // convert all the rectangle detections into full_object_detections.
  for (unsigned long i = 0; i < dets.size(); ++i) {
    final_dets.push_back(scanner.get_full_object_detection(
        dets[i].rect, w[dets[i].weight_index].w));
  }
}
...
template <typename image_scanner_type>
const test_box_overlap &
object_detector<image_scanner_type>::get_overlap_tester() const {
...
object_detector<image_scanner_type>::get_scanner() const {
  return scanner;
}
