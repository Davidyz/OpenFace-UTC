...
public:
  image_hash_construction_failure(const std::string &a) : error(a) {}
};
...
template <typename image_array, typename pyramid, typename feature_extractor,
...
void setup_hashed_features(
  ...
  DLIB_ASSERT(0 < bits && bits <= 32 && num_samples > 1 && images.size() > 0,
              "\t void setup_hashed_features()"
  ...
  const random_subset_selector<typename feature_extractor::descriptor_type>
      &samps = randomly_sample_image_features(images, pyr, fe, num_samples);

  if (samps.size() <= 1)
    throw dlib::image_hash_construction_failure(
