...
template <typename T, size_t max_pool_size> class memory_manager_kernel_1 {
  /*!
      INITIAL VALUE
          allocations == 0
          next == 0
      ...
      REQUIREMENTS ON max_pool_size
          max_pool_size is the maximum number of nodes we will keep in our
  linked list at once. So you can put any value in for this argument.

      CONVENTION
          This memory manager implementation allocates T objects one at a time
  when there are allocation requests.  Then when there is a deallocate request
  the returning T object is place into a list of free blocks if that list has
  less than max_pool_size blocks in it.  subsequent allocation requests will be
  serviced by drawing from the free list whenever it isn't empty.
...
public:
  ...
  memory_manager_kernel_1() : allocations(0), next(0), pool_size(0) {}
  ...
  void swap(memory_manager_kernel_1 &item) {
...
private:
  ...
  operator=(memory_manager_kernel_1 &); // assignment operator
};

// ----------------------------------------------------------------------------------------

template <typename T> class memory_manager_kernel_1<T, 0> {
  /*!
      INITIAL VALUE
          allocations == 0
...
public:
  ...
  memory_manager_kernel_1() : allocations(0) {}
...
template <typename T, size_t max_pool_size>
inline void swap(memory_manager_kernel_1<T, max_pool_size> &a,
