...
template <typename T, size_t chunk_size> class memory_manager_kernel_2 {
  /*!
      INITIAL VALUE
          allocations == 0
          next == 0
...
public:
  ...
  memory_manager_kernel_2() : allocations(0), next(0), first_chunk(0) {
  ...
  void swap(memory_manager_kernel_2 &item) {
...
private:
  ...
  operator=(memory_manager_kernel_2 &); // assignment operator
};

template <typename T, size_t chunk_size>
inline void swap(memory_manager_kernel_2<T, chunk_size> &a,
