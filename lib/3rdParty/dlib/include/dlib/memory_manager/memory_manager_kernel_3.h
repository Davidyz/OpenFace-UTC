...
template <typename T, size_t chunk_size> class memory_manager_kernel_3 {
  /*!
      INITIAL VALUE
          allocations == 0
          next == 0
      ...
      REQUIREMENTS ON chunk_size
          chunk_size is the number of items of type T we will allocate at a
  time. so it must be > 0.

      CONVENTION
          This memory manager implementation allocates memory in blocks of
  chunk_size*sizeof(T) bytes.  All the sizeof(T) subblocks are kept in a linked
  list of free memory blocks and are given out whenever an allocation request
  occurs.  Also, memory is not freed until this object is destructed.
          ...
          - if (next != 0) then
              - next == the next pointer to return from allocate()
                and next == pointer to the first node in a linked list.  each
  node is one item in the memory pool.
              - the last node in the linked list has next set to 0
          - else
              - we need to call new to get the next pointer to return from
  allocate()
          ...
          - if (first_chunk != 0) then
              - first_chunk == the first node in a linked list that contains
  pointers to all the chunks we have ever allocated.  The last link in the list
  ...
  typedef binary_search_tree_kernel_2<size_t, char *,
...
public:
  ...
  memory_manager_kernel_3()
      : allocations(0), next(0), first_chunk(0), arrays(0) {
  ...
  size_t get_number_of_allocations() const { return allocations; }
  ...
  T *allocate() {
    ...
    if (next != 0) {
      ...
      node *block =
          static_cast<node *>(::operator new(sizeof(node) * chunk_size));
  ...
  void swap(memory_manager_kernel_3 &item) {
...
private:
  ...
  void initialize_array(T *array, size_t size) const {
    size_t i;
    try {
      for (i = 0; i < size; ++i) {
        // construct this new T object with placement new.
  ...
  void deinitialize_array(T *array, size_t size) const {
  ...
  memory_manager_kernel_3 &
  operator=(memory_manager_kernel_3 &); // assignment operator
};

template <typename T, size_t chunk_size>
inline void swap(memory_manager_kernel_3<T, chunk_size> &a,
