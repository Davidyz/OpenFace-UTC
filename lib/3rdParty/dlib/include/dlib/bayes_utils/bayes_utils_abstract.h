...
class assignment : public enumerable<map_pair<unsigned long, unsigned long>> {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object models an assignment of random variables to particular
  values. It is used with the joint_probability_table and
  conditional_probability_table objects to represent assignments of various
  random variables to actual values.

          So for example, if you had a joint_probability_table that represented
  the following table: P(A = 0, B = 0) = 0.2 P(A = 0, B = 1) = 0.3 P(A = 1, B =
  0) = 0.1 P(A = 1, B = 1) = 0.4

              Also lets define an enum so we have concrete index numbers for A
  and B enum { A = 0, B = 1};
          ...
          the key() refers to the index and the value() refers to the value at
...
public:
  ...
  /*!
      ensures
          - The exact functioning of this operator is undefined.  The only
  guarantee is that it establishes a total ordering on all possible assignment
  objects. In other words, this operator makes it so that you can use assignment
            objects in the associative containers but otherwise isn't of any
            particular use.
  !*/

  bool has_index(unsigned long idx) const;
  /*!
      ensures
...
void serialize(const assignment &item, std::ostream &out);
/*!
    provides serialization support
!*/

void deserialize(assignment &item, std::istream &in);
/*!
...
class joint_probability_table
    : public enumerable<map_pair<assignment, double>> {
  /*!
      INITIAL VALUE
          - size() == 0
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          the function p(X).  So this object models the probability of a
  particular set of variables (referred to as X).
  !*/
...
public:
  ...
  const double probability(const assignment &a) const;
  /*!
      ensures
  ...
  template <typename T>
  void marginalize(const T &vars, joint_probability_table &output_table) const;
  /*!
      requires
  ...
  void marginalize(const unsigned long var,
                   joint_probability_table &output_table) const;
  /*!
      ensures
          - is identical to calling the above marginalize() function with a set
            that contains only var.  Or in other words, performs a
  marginalization with just one variable var.  So that output_table will contain
  a table giving the marginal probability of var all by itself.
  !*/
  ...
  /*!
      ensures
          ...
          - after normalize() has finished it will be the case that the sum of
  all the entries in this table is 1.0.  This is accomplished by dividing all
            the entries by the sum described above.
  !*/

  void swap(joint_probability_table &item);
  /*!
      ensures
          - swaps *this and item
  !*/
...
void serialize(const joint_probability_table &item, std::ostream &out);
/*!
    provides serialization support
!*/

void deserialize(joint_probability_table &item, std::istream &in);
/*!
    provides deserialization support
!*/
...
class conditional_probability_table : noncopyable {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object models a conditional probability table.  That is, it
  models the function p( X | parents).  So this object models the conditional
          probability of a particular variable (referred to as X) given another
  set of variables (referred to as parents).
  !*/
...
public:
  ...
  /*!
      ensures
          - for all possible v and p:
            ...
            (i.e. this function clears out the table when you call it but
  doesn't change the value of num_values())
  !*/
  ...
  unsigned long num_values() const;
  /*!
      ensures
  ...
  /*!
      ensures
          - if (this conditional_probability_table has an entry for p(X = value,
  parents = ps)) then
              - returns true
          - else
              - returns false
...
void serialize(const conditional_probability_table &item, std::ostream &out);
/*!
    provides serialization support
!*/

void deserialize(conditional_probability_table &item, std::istream &in);
/*!
    provides deserialization support
!*/
...
public:
  ...
  /*!
      ensures
          - returns a reference to the conditional_probability_table associated
  with this node
  !*/

  const conditional_probability_table &table() const;
  /*!
      ensures
          - returns a const reference to the conditional_probability_table
...
void serialize(const bayes_node &item, std::ostream &out);
/*!
    provides serialization support
!*/

void deserialize(bayes_node &item, std::istream &in);
/*!
    provides deserialization support
!*/
...
template <typename T>
const double node_probability(const T &bn, unsigned long n, unsigned long value,
...
template <typename T>
const double set_node_probability(const T &bn, unsigned long n,
                                  unsigned long value,
...
class bayesian_network_gibbs_sampler : noncopyable {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          any nodes that have a probability of 1 in their conditional
  probability tables for any event.  So don't use this object for networks with
          deterministic nodes.
  !*/
public:
  ...
  directed_graph/directed_graph_kernel_abstract.h
          - T::type == bayes_node
      ensures
          - modifies randomly (via the Gibbs sampling technique) samples all the
  nodes in the network and updates their values with the newly sampled values
  !*/
};
...
class bayesian_network_join_tree : noncopyable {
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          for inference in bayesian networks.  It doesn't have any mutable
  state. To you use you just give it a directed_graph that contains a bayesian
          network and a graph object that contains that networks corresponding
          join tree.  Then you may query this object to determine the
  probabilities of any variables in the original bayesian network.
  !*/

public:
  template <typename bn_type, typename join_tree_type>
  bayesian_network_join_tree(const bn_type &bn,
                             const join_tree_type &join_tree);
  /*!
      requires
          - bn_type is an implementation of
  directed_graph/directed_graph_kernel_abstract.h
          - bn_type::type == bayes_node
          - join_tree_type is an implementation of graph/graph_kernel_abstract.h
          - join_tree_type::type is an implementation of
  set/set_compare_abstract.h and this set type contains unsigned long objects.
          - join_tree_type::edge_type is an implementation of
  set/set_compare_abstract.h and this set type contains unsigned long objects.
          - is_join_tree(bn, join_tree) == true
          - bn == a valid bayesian network with all its conditional probability
  tables filled out
          - for all valid n:
  ...
  /*!
      requires
          - idx < number_of_nodes()
      ensures
          - returns the probability distribution for the node with index idx
  that was in the bayesian network that *this was instantiated from.  Let D
  represent this distribution, then:
