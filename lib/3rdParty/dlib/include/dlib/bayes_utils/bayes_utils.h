...
public:
  ...
  bool has_index(unsigned long idx) const { return vals.is_in_domain(idx); }

  void add(unsigned long idx, unsigned long value = 0) {
    // make sure requires clause is not broken
    DLIB_ASSERT(has_index(idx) == false,
                "\tvoid assignment::add(idx)"
                    << "\n\tYou can't add the same index to an assignment "
  ...
  unsigned long &operator[](const long idx) {
    // make sure requires clause is not broken
    DLIB_ASSERT(has_index(idx) == true,
                "\tunsigned long assignment::operator[](idx)"
                    << "\n\tYou can't access an index value if it isn't "
  ...
  const unsigned long &operator[](const long idx) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(has_index(idx) == true,
                "\tunsigned long assignment::operator[](idx)"
                    << "\n\tYou can't access an index value if it isn't "
  ...
  void swap(assignment &item) { vals.swap(item.vals); }

  void remove(unsigned long idx) {
    // make sure requires clause is not broken
    DLIB_ASSERT(has_index(idx) == true,
                "\tunsigned long assignment::remove(idx)"
                    << "\n\tYou can't remove an index value if it isn't "
                       "already in the object"
                    << "\n\tidx:  " << idx << "\n\tthis: " << this);
  ...
  const map_pair<unsigned long, unsigned long> &element() const {
    ...
    DLIB_ASSERT(
        current_element_valid() == true,
        "\tconst map_pair<unsigned long,unsigned long>& assignment::element() "
        "const"
            << "\n\tyou can't access the current element if it doesn't exist"
            << "\n\tthis: " << this);
  ...
  friend inline void serialize(const assignment &item, std::ostream &out) {
    serialize(item.vals, out);
  }

  friend inline void deserialize(assignment &item, std::istream &in) {
...
inline std::ostream &operator<<(std::ostream &out, const assignment &a) {
  a.reset();
  out << "(";
  if (a.move_next())
...
inline void swap(assignment &a, assignment &b) { a.swap(b); }
...
class joint_probability_table {
  ...
  !*/
public:
  joint_probability_table(const joint_probability_table &t) {
    t.reset();
    while (t.move_next()) {
  ...
  joint_probability_table &operator=(const joint_probability_table &rhs) {
    if (this == &rhs)
      return *this;
  ...
  void set_probability(const assignment &a, double p) {
    // make sure requires clause is not broken
  ...
  bool has_entry_for(const assignment &a) const {
    return table.is_in_domain(a);
  }

  void add_probability(const assignment &a, double p) {
    // make sure requires clause is not broken
    DLIB_ASSERT(0.0 <= p && p <= 1.0,
  ...
  double probability(const assignment &a) const { return table[a]; }
  ...
  const map_pair<assignment, double> &element() const {
    ...
    DLIB_ASSERT(
        current_element_valid() == true,
        "\tconst map_pair<assignment,double>& "
        "joint_probability_table::element() const"
            << "\n\tyou can't access the current element if it doesn't exist"
            << "\n\tthis: " << this);
  ...
  template <typename T>
  void marginalize(const T &vars, joint_probability_table &out) const {
    out.clear();
    double p;
    reset();
    while (move_next()) {
  ...
  void marginalize(const unsigned long var,
                   joint_probability_table &out) const {
    out.clear();
    double p;
    reset();
    while (move_next()) {
  ...
  void swap(joint_probability_table &item) { table.swap(item.table); }

  friend inline void serialize(const joint_probability_table &item,
                               std::ostream &out) {
    serialize(item.table, out);
  }

  friend inline void deserialize(joint_probability_table &item,
                                 std::istream &in) {
...
inline void swap(joint_probability_table &a, joint_probability_table &b) {
  a.swap(b);
}
...
class conditional_probability_table : noncopyable {
  /*!
      ...
      CONVENTION
          - if (table.is_in_domain(ps) && value < num_vals && table[ps](value)
  >= 0) then
  ...
  !*/
public:
  conditional_probability_table() { clear(); }

  void set_num_values(unsigned long num) {
    num_vals = num;
    table.clear();
  }

  bool has_entry_for(unsigned long value, const assignment &ps) const {
  ...
  unsigned long num_values() const { return num_vals; }

  void set_probability(unsigned long value, const assignment &ps, double p) {
    // make sure requires clause is not broken
  ...
  double probability(unsigned long value, const assignment &ps) const {
    // make sure requires clause is not broken
  ...
  void empty_table() { table.clear(); }

  void swap(conditional_probability_table &item) {
    exchange(num_vals, item.num_vals);
    table.swap(item.table);
  }

  friend inline void serialize(const conditional_probability_table &item,
                               std::ostream &out) {
    serialize(item.table, out);
    serialize(item.num_vals, out);
  }

  friend inline void deserialize(conditional_probability_table &item,
                                 std::istream &in) {
    deserialize(item.table, in);
...
inline void swap(conditional_probability_table &a,
...
public:
  ...
  void set_value(unsigned long new_value) {
    // make sure requires clause is not broken
    DLIB_ASSERT(new_value < table().num_values(),
                "\tvoid bayes_node::set_value(new_value)"
                    << "\n\tnew_value must be less than the number of possible "
  ...
  conditional_probability_table &table() { return table_; }

  const conditional_probability_table &table() const { return table_; }

  bool is_evidence() const { return is_instantiated; }

  void set_as_nonevidence() { is_instantiated = false; }

  void set_as_evidence() { is_instantiated = true; }

  void swap(bayes_node &item) {
    exchange(value_, item.value_);
    exchange(is_instantiated, item.is_instantiated);
    table_.swap(item.table_);
  }

  friend inline void serialize(const bayes_node &item, std::ostream &out) {
    serialize(item.value_, out);
    serialize(item.is_instantiated, out);
    serialize(item.table_, out);
  }

  friend inline void deserialize(bayes_node &item, std::istream &in) {
...
inline void swap(bayes_node &a, bayes_node &b) { a.swap(b); }
...
template <typename T>
unsigned long node_num_values(const T &bn, unsigned long n) {
  // make sure requires clause is not broken
...
template <typename T>
void set_node_value(T &bn, unsigned long n, unsigned long val) {
  // make sure requires clause is not broken
...
// ----------------------------------------------------------------------------------------
template <typename T> unsigned long node_value(const T &bn, unsigned long n) {
  // make sure requires clause is not broken
  DLIB_ASSERT(n < bn.number_of_nodes(),
...
template <typename T> bool node_is_evidence(const T &bn, unsigned long n) {
  // make sure requires clause is not broken
  DLIB_ASSERT(n < bn.number_of_nodes(),
...
template <typename T> void set_node_as_evidence(T &bn, unsigned long n) {
  // make sure requires clause is not broken
  DLIB_ASSERT(n < bn.number_of_nodes(),
...
// ----------------------------------------------------------------------------------------
template <typename T> void set_node_as_nonevidence(T &bn, unsigned long n) {
  // make sure requires clause is not broken
  DLIB_ASSERT(n < bn.number_of_nodes(),
...
template <typename T>
void set_node_num_values(T &bn, unsigned long n, unsigned long num) {
  // make sure requires clause is not broken
  DLIB_ASSERT(n < bn.number_of_nodes(),
...
template <typename T>
double node_probability(const T &bn, unsigned long n, unsigned long value,
                        const assignment &parents) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
      n < bn.number_of_nodes() && value < node_num_values(bn, n),
  ...
  DLIB_ASSERT(
      ...
      "\tdouble bayes_node_utils::node_probability(bn, n, value, parents)"
          ...
          << "\n\tb.node(n).number_of_parents(): "
          << bn.node(n).number_of_parents());
...
template <typename T>
void set_node_probability(T &bn, unsigned long n, unsigned long value,
                          const assignment &parents, double p) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
      n < bn.number_of_nodes() && value < node_num_values(bn, n),
  ...
  while (parents.move_next()) {
    ...
    DLIB_ASSERT(bn.has_edge(x, n),
                "\tvoid bayes_node_utils::set_node_probability(bn, n, value, "
                "parents, p)"
                    << "\n\tInvalid arguments to this function"
                    << "\n\tn: " << n << "\n\tx: " << x);
...
template <typename T>
const assignment node_first_parent_assignment(const T &bn, unsigned long n) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
...
template <typename T>
bool node_next_parent_assignment(const T &bn, unsigned long n, assignment &a) {
  // make sure requires clause is not broken
  DLIB_ASSERT(n < bn.number_of_nodes(),
  ...
  // we got through the entire loop which means a carry propagated all the way
  // out so there must not be any more valid assignments left
  return false;
}

// ----------------------------------------------------------------------------------------

template <typename T> bool node_cpt_filled_out(const T &bn, unsigned long n) {
  // make sure requires clause is not broken
  DLIB_ASSERT(n < bn.number_of_nodes(),
  ...
  do {
    double sum = 0;
    // make sure that this assignment has an entry for all the values this node
    // can take one
...
public:
  ...
  template <typename T> void sample_graph(T &bn) {
...
private:
  template <typename T>
  double node_probability(const T &bn, unsigned long n)
  /*!
      requires
          - n < bn.number_of_nodes()
      ensures
...
public:
  ...
  virtual const matrix<double, 1> probability(unsigned long idx) const = 0;
};

template <typename T, typename U> class bnjt_impl : public bnjt {
  /*!
      This object is the implementation in the pimpl idiom
  !*/

public:
  bnjt_impl(const T &bn, const U &join_tree) {
    ...
    for (unsigned long i = 0; i < cliques.size(); ++i) {
      ...
      for (unsigned long n = 0; n < join_tree.number_of_nodes(); ++n) {
        if (join_tree.node(n).data.is_member(i) &&
            join_tree.node(n).data.size() < size) {
  ...
  virtual const matrix<double, 1> probability(unsigned long idx) const {
...
private:
  graph<joint_probability_table, joint_probability_table>::kernel_1a_c
      join_tree_values;
  ...
  bool set_contains_all_parents_of_node(const set_type &set,
  ...
  template <typename V>
  void pass_join_tree_message(const U &join_tree, V &bn_join_tree,
  ...
  template <typename V>
  void create_bayesian_network_join_tree(const T &bn, const U &join_tree,
                                         V &bn_join_tree)
  /*!
      requires
      ...
      ensures
          - bn_join_tree == the output of the join tree algorithm for bayesian
  network inference. So each node in this graph contains a
  joint_probability_table for the clique in the corresponding node in the
  join_tree graph.
  !*/
  {
    using namespace bayes_node_utils;
    ...
    // we need to keep track of which node is "in" each clique for the purposes
    // of initializing the tables in each clique.  So this vector will be used
    // to do that and a value of join_tree.number_of_nodes() means that the node
    // with that index is unassigned.
    std::vector<unsigned long> node_assigned_to(bn.number_of_nodes(),
                                                join_tree.number_of_nodes());
    ...
    for (unsigned long i = 0; i < join_tree.number_of_nodes(); ++i) {
      ...
      // we are making an assignment with all the values of the nodes it
      // represents set to 0
      ...
      // clique in the join tree can take on.  We do this by counting by one
      // through all legal values
      ...
      while (more_assignments) {
        ...
        // now check if any of the nodes in this cluster also have their parents
        // in this cluster
        join_tree.node(i).data.reset();
        while (join_tree.node(i).data.move_next()) {
          const unsigned long idx = join_tree.node(i).data.element();
          // if this clique contains all the parents of this node and also
          // hasn't been assigned to another clique
          if (set_contains_all_parents_of_node(join_tree.node(i).data,
                                               bn.node(idx)) &&
              (i == node_assigned_to[idx] ||
               node_assigned_to[idx] == join_tree.number_of_nodes())) {
            ...
            for (unsigned long j = 0; j < bn.node(idx).number_of_parents();
                 ++j) {
            ...
            bn_join_tree.node(i).data.set_probability(
                value,
                temp * node_probability(bn, idx, value[idx], parent_values));
          }
        }

        // now advance the value variable to its next possible state if there is
        // one
        ...
        while (value.move_next()) {
          ...
          if (value.element().value() ==
              node_num_values(bn, value.element().key())) {
    ...
    // the tree is now initialized.  Now all we need to do is perform the
    // propagation and we are done
    dlib::array<dlib::set<unsigned long>::compare_1b_c> remaining_msg_to_send;
    dlib::array<dlib::set<unsigned long>::compare_1b_c>
    ...
    for (unsigned long i = 0; i < remaining_msg_to_receive.size(); ++i) {
      for (unsigned long j = 0; j < join_tree.node(i).number_of_neighbors();
           ++j) {
        const unsigned long idx = join_tree.node(i).neighbor(j).index();
        unsigned long temp;
        temp = idx;
        remaining_msg_to_receive[i].add(temp);
        temp = idx;
        remaining_msg_to_send[i].add(temp);
      }
    }

    // now remaining_msg_to_receive[i] contains all the nodes that node i hasn't
    // yet received a message from. we will consider node 0 to be the root node.
    ...
    while (message_sent) {
      ...
      for (unsigned long i = 1; i < remaining_msg_to_send.size(); ++i) {
        // if node i hasn't sent any messages but has received all but one then
        // send a message to the one node who hasn't sent i a message
        if (remaining_msg_to_send[i].size() ==
                join_tree.node(i).number_of_neighbors() &&
            remaining_msg_to_receive[i].size() == 1) {
          unsigned long to;
          ...
          message_sent = true;
        } else if (remaining_msg_to_receive[i].size() == 0 &&
                   remaining_msg_to_send[i].size() > 0) {
          unsigned long to;
      ...
      if (remaining_msg_to_receive[0].size() == 0) {
        // send a message to all of the root nodes neighbors unless we have
        // already sent out he messages
...
class bayesian_network_join_tree : noncopyable {
  /*!
      use the pimpl idiom to push the template arguments from the class level to
  the constructor level
  !*/

public:
  template <typename T, typename U>
  bayesian_network_join_tree(const T &bn, const U &join_tree) {
    // make sure requires clause is not broken
    DLIB_ASSERT(
    ...
    DLIB_ASSERT(
        ...
        "\tbayesian_network_join_tree::bayesian_network_join_tree(bn,join_tree)"
            << "\n\tYou have given an invalid join tree for the supplied "
               "bayesian network"
            << "\n\tthis:              " << this);
    DLIB_ASSERT(
    ...
    for (unsigned long i = 0; i < bn.number_of_nodes(); ++i) {
      DLIB_ASSERT(bayes_node_utils::node_cpt_filled_out(bn, i) == true,
                  "\tbayesian_network_join_tree::bayesian_network_join_tree(bn,"
                  "join_tree)"
                      << "\n\tYou have given an invalid bayesian network. "
                      << "\n\tYou must finish filling out the "
                         "conditional_probability_table of node "
                      << i << "\n\tthis:              " << this);
    }
#endif

    impl.reset(
        new bayesian_network_join_tree_helpers::bnjt_impl<T, U>(bn, join_tree));
    num_nodes = bn.number_of_nodes();
  }

  const matrix<double, 1> probability(unsigned long idx) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(
        idx < number_of_nodes(),
  ...
  unsigned long number_of_nodes() const { return num_nodes; }

  void swap(bayesian_network_join_tree &item) {
...
inline void swap(bayesian_network_join_tree &a, bayesian_network_join_tree &b) {
  a.swap(b);
}
