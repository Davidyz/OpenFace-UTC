...
namespace dlib {
namespace impl {
inline bool get_bool_part(const bool &b) { return b; }

template <typename T> bool get_bool_part(const std::pair<T, bool> &item) {
  return item.second;
}
} // namespace impl
...
double average_precision(const std::vector<T, alloc> &items,
  ...
  for (std::vector<double>::reverse_iterator i = precision.rbegin();
       i != precision.rend(); ++i) {
