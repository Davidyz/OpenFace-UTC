...
void deserialize(frobmetric_training_sample<matrix_type> &item,
  ...
  if (version != 1)
    throw serialization_error("Unexpected version found while deserializing "
                              "dlib::frobmetric_training_sample.");
...
template <typename matrix_type> class vector_normalizer_frobmetric {
...
private:
  ...
  struct objective {
    objective(const std::vector<compact_frobmetric_training_sample> &samples_,
    ...
    double operator()(const matrix<double, 0, 1, mem_manager_type> &u) const {
      ...
      for (unsigned long i = 0; i < samples.size(); ++i) {
        ...
        for (unsigned long j = 0; j < unear.size(); ++j)
          A += unear[j] * samples[i].near_vects[j] *
               trans(samples[i].near_vects[j]);
        for (unsigned long j = 0; j < ufar.size(); ++j)
          A += ufar[j] * samples[i].far_vects[j] *
               trans(samples[i].far_vects[j]);
      }

      eigenvalue_decomposition<matrix<double, 0, 0, mem_manager_type>> ed(
          make_symmetric(A));
      Aminus = ed.get_pseudo_v() *
               diagm(upperbound(ed.get_real_eigenvalues(), 0)) *
               trans(ed.get_pseudo_v());
  ...
  struct derivative {
    derivative(unsigned long num_triples_,
    ...
    matrix<double, 0, 1, mem_manager_type>
    operator()(const matrix<double, 0, 1, mem_manager_type> &) const {
      // Note that Aminus is a function of u (the argument to this function),
      // but since Aminus will have been computed already by the most recent
      // call to the objective function we don't need to do anything with u.  We
      // can just use Aminus right away.
      ...
      for (unsigned long i = 0; i < samples.size(); ++i) {
        ...
        for (unsigned long j = 0; j < unear.size(); ++j)
          unear[j] = sum(
              pointwise_multiply(Aminus, samples[i].near_vects[j] *
                                             trans(samples[i].near_vects[j])));
        for (unsigned long j = 0; j < ufar.size(); ++j)
          ufar[j] = sum(
              pointwise_multiply(Aminus, samples[i].far_vects[j] *
                                             trans(samples[i].far_vects[j])));
  ...
  public:
    custom_stop_strategy(double C_, double eps_, bool be_verbose_,
    ...
    template <typename T>
    bool should_continue_search(const T &u, const double, const T &grad) {
      ++_cur_iter;
      ...
      if (_verbose) {
        std::cout << "iteration: " << _cur_iter
                  << "   max_gradient: " << max_gradient << std::endl;
      }

      // Only stop when the largest non-bound-constrained element of the
      // gradient is lower than the threshold.
...
public:
  ...
  bool uses_identity_matrix_prior() const { return _use_identity_matrix_prior; }
  ...
  void set_c(double C_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C_ > 0, "\t void vector_normalizer_frobmetric::set_c()"
  ...
  unsigned long get_max_iterations() const { return max_iter; }
  ...
  train(const std::vector<frobmetric_training_sample<matrix_type>> &samples) {
    ...
    DLIB_ASSERT(samples.size() > 0,
                "\tvoid vector_normalizer_frobmetric::train()"
                    << "\n\t you have to give a nonempty set of samples to "
                       "this function");
      ...
      for (unsigned long i = 0; i < samples.size(); ++i) {
        ...
        DLIB_ASSERT(samples[i].anchor_vect.size() == dims,
                    "\tvoid vector_normalizer_frobmetric::train()"
                        << "\n\t Invalid inputs were given to this function."
                        << "\n\t i:    " << i << "\n\t dims: " << dims
                        << "\n\t samples[i].anchor_vect.size(): "
                        << samples[i].anchor_vect.size());

        DLIB_ASSERT(samples[i].num_triples() != 0,
                    "\tvoid vector_normalizer_frobmetric::train()"
                        << "\n\t It is illegal for a training sample to have "
                           "no data in it"
                        << "\n\t i:    " << i);
        for (unsigned long j = 0; j < samples[i].near_vects.size(); ++j) {
    ...
    DLIB_ASSERT(
        is_finite(m),
        "Some of the input vectors to vector_normalizer_frobmetric::train() "
        "have infinite or NaN values");

    // Now we need to find tform.  So we setup the optimization problem and run
    // it over the next few lines of code.
    ...
    for (unsigned long i = 0; i < data.size(); ++i) {
      ...
      for (unsigned long j = 0; j < samples[i].far_vects.size(); ++j) {
        data[i].far_vects.push_back(samples[i].anchor_vect -
                                    samples[i].far_vects[j]);
        if (_use_identity_matrix_prior)
          far_norm.push_back(length_squared(data[i].far_vects.back()));
      }
      for (unsigned long j = 0; j < samples[i].near_vects.size(); ++j) {
        data[i].near_vects.push_back(samples[i].anchor_vect -
                                     samples[i].near_vects[j]);
        if (_use_identity_matrix_prior)
          near_norm.push_back(length_squared(data[i].near_vects.back()));
      }

      // Note that this loop only executes if _use_identity_matrix_prior ==
      // true.
    ...
    find_max_box_constrained(lbfgs_search_strategy(10),
                             ...
                             derivative(num_triples, data, Aminus, bias), u, 0,
                             C / num_triples);

    // What we need is the optimal Aminus which is a function of u.  So we
    // already have what we need and just need to put it into tform.
    eigenvalue_decomposition<matrix<double, 0, 0, mem_manager_type>> ed(
        make_symmetric(-Aminus));
    ...
    if (_use_identity_matrix_prior)
      tform = matrix_cast<scalar_type>(identity_matrix(Aminus) +
                                       diagm(sqrt(eigs)) *
                                           trans(ed.get_pseudo_v()));
    else
      tform = matrix_cast<scalar_type>(diagm(sqrt(eigs)) *
                                       trans(ed.get_pseudo_v()));
  ...
  friend void deserialize(vector_normalizer_frobmetric<mt> &item,
                          std::istream &in);

  template <typename mt>
  friend void serialize(const vector_normalizer_frobmetric<mt> &item,
                        std::ostream &out);
...
void deserialize(vector_normalizer_frobmetric<matrix_type> &item,
  ...
  if (version != 1 && version != 2)
    throw serialization_error("Unsupported version found while deserializing "
                              "dlib::vector_normalizer_frobmetric.");
