...
public:
  ...
  double current_n() const { return n; }
  ...
  void add(double y) {
    ...
    // R should always be symmetric.  This line improves numeric stability of
    // this algorithm.
    R = 0.5 * (R + trans(R));
    w = w + R * x * (y - trans(x) * w);

    // Also, recursively keep track of the residual error between the given
    // value and what our linear predictor outputs.
    residual_squared =
        residual_squared + std::pow((y - trans(x) * w), 2.0) * temp;
  ...
  double gradient() const {
    ...
    DLIB_ASSERT(current_n() > 1,
                "\t double running_gradient::gradient()"
                    << "\n\t You must add more values into this object before "
  ...
  double probability_gradient_greater_than(double thresh) const {
    ...
    DLIB_ASSERT(
        ...
        "\t double running_gradient::probability_gradient_greater_than()"
            << "\n\t You must add more values into this object before calling "
               "this function."
            << "\n\t this: " << this);
...
double probability_gradient_less_than(const T &container, double thresh) {
  ...
  DLIB_ASSERT(g.current_n() > 2,
              "\t double probability_gradient_less_than()"
                  << "\n\t You need more than 2 elements in the given "
                     "container to call this function.");
...
template <typename T>
double find_upper_quantile(const T &container_, double quantile) {
  ...
  std::nth_element(container.begin(), container.begin() + idx_upper,
                   container.end());
...
size_t count_steps_without_decrease(const T &container,
  ...
  for (auto i = container.rbegin(); i != container.rend(); ++i) {
    ...
    if (g.current_n() > 2) {
      ...
      // container backwards.  So here we are really checking if the gradient
      // isn't decreasing.
