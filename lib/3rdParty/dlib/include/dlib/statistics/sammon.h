...
public:
  ...
  operator()(const std::vector<matrix_type> &data, const long num_dims) {
    ...
    DLIB_ASSERT(
        0 < num_dims && num_dims <= data[0].size(),
        "\t std::vector<matrix<double,0,1> > sammon_projection::operator()"
            << "\n\t Invalid inputs were given to this function."
            << "\n\t data.size():    " << data.size() << "\n\t num_dims:       "
    ...
    for (unsigned long i = 0; i < data.size(); ++i) {
      DLIB_ASSERT(
          is_col_vector(data[i]) && data[i].size() == data[0].size(),
          "\t std::vector<matrix<double,0,1> > sammon_projection::operator()"
              << "\n\t Invalid inputs were given to this function."
              << "\n\t data[" << i << "].size():    " << data[i].size()
              << "\n\t data[0].size(): " << data[0].size()
              << "\n\t is_col_vector(data[" << i
              << "]): " << is_col_vector(data[i]));
    }
#endif
  ...
  void operator()(const std::vector<matrix_type> &data, const long num_dims,
    ...
    // make sure requires clause is not broken
    DLIB_ASSERT(
        num_dims > 0 && num_iters > 0 && err_delta > 0.0,
        "\t std::vector<matrix<double,0,1> > sammon_projection::operator()"
            << "\n\t Invalid inputs were given to this function."
            << "\n\t data.size(): " << data.size() << "\n\t num_dims:    "
            << num_dims << "\n\t num_iters:   " << num_iters
...
private:
  void compute_relative_distances(
      ...
      matrix<double, 0, 0>
          &data, // input data (matrix whose columns are the input vectors)
  ...
  !*/
  {
    ...
    const long nd = N * (N - 1) /
                    2; // num of pairs of points = size of the distances vectors

    matrix<double, 0, 1> dsij, inv_dsij; // d*_ij: pair-wise distances in the
                                         // input space (and inverses)
    dsij.set_size(nd, 1);
    ...
    matrix<double, 0, 0> dE, dE2,
        dtemp; // matrices representing error partial derivatives
    dE.set_size(d, N);
    dE2.set_size(d, N);
    dtemp.set_size(d, N);

    matrix<double, 0, 1> inv_dij,
        alpha; // utility vectors used to compute the partial derivatives
    inv_dij.set_size(N,
                     1);  // inv_dij is 1.0/dij, but we only need it column-wise
    ...
    while (num_iters--) {
      ...
      compute_relative_distances(dij, projs);
      double err_new =
          ic * sum(pointwise_multiply(squared(dij - dsij), inv_dsij));
