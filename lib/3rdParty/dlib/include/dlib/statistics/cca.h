...
matrix<T, 0, 1>
impl_cca(const matrix_type &L, const matrix_type &R, matrix<T> &Ltrans,
         matrix<T> &Rtrans, unsigned long num_correlations,
         unsigned long extra_rank, unsigned long q,
         unsigned long num_output_correlations, double regularization) {
  matrix<T> Ul, Vl;
  matrix<T> Ur, Vr;
  matrix<T> U, V;
  matrix<T, 0, 1> Dr, Dl, D;

  // Note that we add a few more singular vectors in because it helps improve
  // the final results if we run this part with a little higher rank than the
  // final SVD.
  svd_fast(L, Ul, Dl, Vl, num_correlations + extra_rank, q);
  svd_fast(R, Ur, Dr, Vr, num_correlations + extra_rank, q);

  // Zero out singular values that are essentially zero so they don't cause
  // numerical difficulties in the code below.
  const double eps =
      std::numeric_limits<T>::epsilon() * std::max(max(Dr), max(Dl)) * 100;
  Dl = round_zeros(Dl + regularization, eps);
  Dr = round_zeros(Dr + regularization, eps);

  // This matrix is really small so we can do a normal full SVD on it.  Note
  // that we also throw away the columns of Ul and Ur corresponding to zero
  // singular values.
...
matrix<T, 0, 1> cca(const matrix<T> &L, const matrix<T> &R, matrix<T> &Ltrans,
  ...
  DLIB_ASSERT(num_correlations > 0 && L.size() > 0 && R.size() > 0 &&
                  L.nr() == R.nr() && regularization >= 0,
              "\t matrix cca()"
                  << "\n\t Invalid inputs were given to this function."
  ...
  const unsigned long n =
      min(num_correlations, (unsigned long)min(R.nr(), min(L.nc(), R.nc())));
...
matrix<T, 0, 1> cca(const std::vector<sparse_vector_type> &L,
  ...
  const unsigned long num_output_correlations =
      min(num_correlations, std::min<unsigned long>(R.size(), n));
  return impl_cca(L, R, Ltrans, Rtrans, num_correlations, extra_rank, q,
                  num_output_correlations, regularization);
}
...
cca(const random_subset_selector<sparse_vector_type, Rand_type> &L,
    ...
    unsigned long extra_rank = 5, unsigned long q = 2) {
  return cca(L.to_std_vector(), R.to_std_vector(), Ltrans, Rtrans,
             num_correlations, extra_rank, q);
}
