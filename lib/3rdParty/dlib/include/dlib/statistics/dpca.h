...
template <typename matrix_type> class discriminant_pca {
  /*!
      ...
      CONVENTION
          ...
          - total_count == the number of times add_to_total_variance() has been
  called.
          - within_count == the number of times add_to_within_class_variance()
  has been called.
          - between_count == the number of times add_to_between_class_variance()
  has been called.
          - between_weight == between_class_weight()
          - within_weight == within_class_weight()

          - if (total_count != 0)
              - total_sum == the sum of all vectors given to
  add_to_total_variance()
              - the covariance of all the elements given to
  add_to_total_variance() is given by:
                  - let avg == total_sum/total_count
                  - covariance == total_cov/total_count - avg*trans(avg)
          - if (within_count != 0)
              - within_cov/within_count == the normalized within class scatter
  matrix
          - if (between_count != 0)
              - between_cov/between_count == the normalized between class
  scatter matrix
  !*/
...
public:
  ...
  scalar_type within_class_weight() const { return within_weight; }
  ...
  scalar_type between_class_weight() const { return between_weight; }

  template <typename EXP1, typename EXP2>
  void add_to_within_class_variance(const matrix_exp<EXP1> &x,
                                    const matrix_exp<EXP2> &y) {
    // make sure requires clause is not broken
    DLIB_ASSERT(is_col_vector(x) && is_col_vector(y) && x.size() == y.size() &&
  ...
  template <typename EXP1, typename EXP2>
  void add_to_between_class_variance(const matrix_exp<EXP1> &x,
                                     const matrix_exp<EXP2> &y) {
    // make sure requires clause is not broken
    DLIB_ASSERT(is_col_vector(x) && is_col_vector(y) && x.size() == y.size() &&
                    (in_vector_size() == 0 || x.size() == in_vector_size()),
  ...
  template <typename EXP> void add_to_total_variance(const matrix_exp<EXP> &x) {
    // make sure requires clause is not broken
  ...
  friend void deserialize(discriminant_pca &item, std::istream &in) {
  ...
  friend void serialize(const discriminant_pca &item, std::ostream &out) {
  ...
  discriminant_pca operator+(const discriminant_pca &item) const {
    ...
    DLIB_ASSERT(
        ...
        "\t discriminant_pca discriminant_pca::operator+()"
            << "\n\t The two discriminant_pca objects being added must have "
               "compatible parameters"
            << "\n\t in_vector_size():            " << in_vector_size()
            << "\n\t item.in_vector_size():       " << item.in_vector_size()
            << "\n\t between_class_weight():      " << between_class_weight()
            << "\n\t item.between_class_weight(): "
            << item.between_class_weight()
            << "\n\t within_class_weight():       " << within_class_weight()
            << "\n\t item.within_class_weight():  "
            << item.within_class_weight()
            << "\n\t this:                        " << this);

    discriminant_pca temp(item);

    // We need to make sure to ignore empty matrices.  That's what these if
    // statements are for.
...
private:
  void compute_dpca_matrix(general_matrix &dpca_mat,
                           general_matrix &eigenvalues, const double eps,
                           long num_rows) const {
    general_matrix cov;

    // now combine the three measures of variance into a single matrix by using
    // the within_weight and between_weight weights.
    ...
    // sort the eigenvalues and eigenvectors so that the biggest eigenvalues
    // come first
    ...
    if (num_rows == 0) {
      ...
      for (long r = 0; r < eigenvalues.size() && total < thresh; ++r) {
        // Don't even think about looking at eigenvalues that are 0.  If we go
        // this far then we have all we need.
        if (eigenvalues(r) == 0)
          break;
      ...
      if (num_vectors == 0)
        throw discriminant_pca_error("While performing discriminant_pca, all "
                                     "eigenvalues were negative or 0");
    } else {
  ...
  general_matrix get_total_covariance_matrix() const
  /*!
      ensures
          - returns the covariance matrix of all the data given to the
  add_to_total_variance()
  !*/
  {
    ...
    // In this case we actually have something to make a total covariance matrix
    // out of. So do that.
