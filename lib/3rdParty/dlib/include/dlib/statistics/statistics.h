...
public:
  running_stats() {
    ...
    COMPILE_TIME_ASSERT((is_same_type<float, T>::value ||
  ...
  T skewness() const {
    ...
    temp = temp1 * temp *
           (sum_cub - 3 * sum_sqr * sum * temp + 2 * cubed(sum) * temp * temp) /
  ...
  T ex_kurtosis() const {
    ...
    T temp = 1 / n;
    T m4 = temp * (sum_four - 4 * sum_cub * sum * temp +
                   6 * sum_sqr * sum * sum * temp * temp -
                   3 * quaded(sum) * cubed(temp));
    T m2 = temp * (sum_sqr - sum * sum * temp);
    temp = (n - 1) * ((n + 1) * m4 / (m2 * m2) - 3 * (n - 1)) /
           ((n - 2) * (n - 3));
  ...
  template <typename U>
  friend void serialize(const running_stats<U> &item, std::ostream &out);

  template <typename U>
  friend void deserialize(running_stats<U> &item, std::istream &in);
...
void deserialize(running_stats<T> &item, std::istream &in) {
  ...
  if (version != 2)
    throw dlib::serialization_error(
        "Unexpected version number found while deserializing "
        "dlib::running_stats object.");
...
public:
  ...
  running_scalar_covariance
  operator+(const running_scalar_covariance &rhs) const {
...
public:
  ...
  T forget_factor() const { return forget; }
...
public:
  explicit running_stats_decayed(T decay_halflife = 1000) {
    DLIB_ASSERT(decay_halflife > 0);
  ...
  template <typename U>
  friend void serialize(const running_stats_decayed<U> &item,
                        std::ostream &out);

  template <typename U>
  friend void deserialize(running_stats_decayed<U> &item, std::istream &in);
...
void deserialize(running_stats_decayed<T> &item, std::istream &in) {
  ...
  if (version != 1)
    throw dlib::serialization_error(
        "Unexpected version number found while deserializing "
        "dlib::running_stats_decayed object.");
...
double correlation(const std::vector<T, alloc> &a,
  ...
  DLIB_ASSERT(a.size() == b.size() && a.size() > 1,
              "\t double correlation(a,b)"
                  << "\n\t a and b must be the same length and have more than "
                     "one element."
                  << "\n\t a.size(): " << a.size()
                  << "\n\t b.size(): " << b.size());
...
double mean_squared_error(const std::vector<T, alloc> &a,
  ...
  return mean(
      squared(matrix_cast<double>(mat(a)) - matrix_cast<double>(mat(b))));
}
...
public:
  ...
  long in_vector_size() const { return vect_size; }

  long current_n() const { return static_cast<long>(total_count); }
  ...
  template <typename T>
  typename disable_if<is_matrix<T>>::type add(const T &val) {
    // make sure requires clause is not broken
    DLIB_ASSERT(((long)max_index_plus_one(val) <= in_vector_size() &&
                 in_vector_size() > 0),
                "\t void running_covariance::add()"
                    << "\n\t Invalid inputs were given to this function"
                    << "\n\t max_index_plus_one(val): "
                    << max_index_plus_one(val)
  ...
  template <typename T>
  typename enable_if<is_matrix<T>>::type add(const T &val) {
    // make sure requires clause is not broken
    DLIB_ASSERT(is_col_vector(val) &&
                    (in_vector_size() == 0 || val.size() == in_vector_size()),
                "\t void running_covariance::add()"
                    << "\n\t Invalid inputs were given to this function"
  ...
  const column_matrix mean() const {
    ...
    DLIB_ASSERT(in_vector_size() != 0,
                "\t running_covariance::mean()"
                    << "\n\t This object can not execute this function in its "
                       "current state."
                    << "\n\t in_vector_size(): " << in_vector_size()
                    << "\n\t current_n():      " << current_n()
                    << "\n\t this:             " << this);
  ...
  const general_matrix covariance() const {
    ...
    return (total_cov - total_sum * trans(total_sum) / total_count) /
           (total_count - 1);
  }

  const running_covariance operator+(const running_covariance &item) const {
    // make sure requires clause is not broken
    DLIB_ASSERT((in_vector_size() == 0 || item.in_vector_size() == 0 ||
                 in_vector_size() == item.in_vector_size()),
                "\t running_covariance running_covariance::operator+()"
                    << "\n\t The two running_covariance objects being added "
                       "must have compatible parameters"
                    << "\n\t in_vector_size():            " << in_vector_size()
                    << "\n\t item.in_vector_size():       "
                    << item.in_vector_size()
                    << "\n\t this:                        " << this);
...
public:
  ...
  running_cross_covariance() { clear(); }
  ...
  long x_vector_size() const { return x_vect_size; }

  long y_vector_size() const { return y_vect_size; }
  ...
  typename enable_if_c<!is_matrix<T>::value && !is_matrix<U>::value>::type
  add(const T &x, const U &y) {
    // make sure requires clause is not broken
    DLIB_ASSERT(((long)max_index_plus_one(x) <= x_vector_size() &&
                 x_vector_size() > 0) &&
                    ((long)max_index_plus_one(y) <= y_vector_size() &&
                     y_vector_size() > 0),
  ...
  add(const T &x, const U &y) {
    // make sure requires clause is not broken
    DLIB_ASSERT((is_col_vector(x) && x.size() == x_vector_size() &&
                 x_vector_size() > 0) &&
                    ((long)max_index_plus_one(y) <= y_vector_size() &&
                     y_vector_size() > 0),
                "\t void running_cross_covariance::add()"
  ...
  add(const T &x, const U &y) {
    ...
    DLIB_ASSERT(((long)max_index_plus_one(x) <= x_vector_size() &&
                 x_vector_size() > 0) &&
                    (is_col_vector(y) && y.size() == (long)y_vector_size() &&
                     y_vector_size() > 0),
                "\t void running_cross_covariance::add()"
                    << "\n\t Invalid inputs were given to this function"
                    << "\n\t max_index_plus_one(x): " << max_index_plus_one(x)
  ...
  operator+(const running_cross_covariance &item) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(
        (x_vector_size() == 0 || item.x_vector_size() == 0 ||
         x_vector_size() == item.x_vector_size()) &&
            (y_vector_size() == 0 || item.y_vector_size() == 0 ||
             y_vector_size() == item.y_vector_size()),
        "\t running_cross_covariance running_cross_covariance::operator+()"
            << "\n\t The two running_cross_covariance objects being added must "
...
public:
  ...
  template <typename vector_type> void train(const vector_type &samples) {
    ...
    DLIB_ASSERT(is_finite(m),
                "Some of the input vectors to vector_normalizer::train() have "
                "infinite or NaN values");
  }

  long in_vector_size() const { return m.nr(); }

  long out_vector_size() const { return m.nr(); }

  const matrix_type &means() const { return m; }

  const matrix_type &std_devs() const { return sd; }
  ...
  friend void deserialize(vector_normalizer<mt> &item, std::istream &in);

  template <typename mt>
  friend void serialize(const vector_normalizer<mt> &item, std::ostream &out);
...
void deserialize(vector_normalizer<matrix_type> &item, std::istream &in) {
  ...
  if (pca.size() != 0)
    throw serialization_error(
        "Error deserializing object of type vector_normalizer\n"
        "It looks like a serialized vector_normalizer_pca was accidentally "
        "deserialized into \n"
...
public:
  ...
  void train(const vector_type &samples, const double eps = 0.99) {
    ...
    // dimensionality reduction so you can't use a vector with a fixed
    // dimension.
    ...
    DLIB_ASSERT(is_finite(m),
                "Some of the input vectors to vector_normalizer_pca::train() "
                "have infinite or NaN values");
  }

  long in_vector_size() const { return m.nr(); }

  long out_vector_size() const { return pca.nr(); }

  const matrix<scalar_type, 0, 1, mem_manager_type> &means() const { return m; }
  ...
  template <typename T>
  friend void deserialize(vector_normalizer_pca<T> &item, std::istream &in);

  template <typename T>
  friend void serialize(const vector_normalizer_pca<T> &item,
                        std::ostream &out);

private:
  template <typename mat_type>
  void train_pca_impl(const mat_type &samples, const double eps) {
    m = mean(samples);
...
void deserialize(vector_normalizer_pca<matrix_type> &item, std::istream &in) {
  ...
  if (item.pca.nc() != item.m.nr())
    throw serialization_error(
        ...
        "It looks like a serialized vector_normalizer was accidentally "
        "deserialized into \n"
...
inline double binomial_random_vars_are_different(uint64_t k1, uint64_t n1,
                                                 uint64_t k2, uint64_t n2) {
  ...
  // The basic statistic only tells you if the random variables are different.
  // But it's nice to know which way they are different, i.e., which one is
  // bigger.  So stuff that information into the sign bit of the return value.
  if (p1 >= p2)
    return logll;
  else
    return -logll;
...
inline double event_correlation(uint64_t A_count, uint64_t B_count,
  ...
  // How likely is it that the odds of A happening is different when conditioned
  // on whether or not B happened?
