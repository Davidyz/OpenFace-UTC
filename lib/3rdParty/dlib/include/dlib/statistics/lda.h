...
inline std::map<unsigned long, unsigned long>
make_class_labels(const std::vector<unsigned long> &row_labels) {
...
template <typename T> matrix<T, 0, 1> center_matrix(matrix<T> &X) {
  matrix<T, 1> mean;
...
template <typename T>
void compute_lda_transform(matrix<T> &X, matrix<T, 0, 1> &mean,
                           const std::vector<unsigned long> &row_labels,
                           unsigned long lda_dims = 500,
                           unsigned long extra_pca_dims = 200) {
  std::map<unsigned long, unsigned long> class_labels =
      impl::make_class_labels(row_labels);
  // LDA can only give out at most class_labels.size()-1 dimensions so don't try
  // to compute more than that.
  ...
  DLIB_CASSERT(class_labels.size() > 1,
               "\t void compute_lda_transform()"
                   << "\n\t You can't call this function if the number of "
                      "distinct class labels is less than 2.");
  DLIB_CASSERT(X.size() != 0 && (long)row_labels.size() == X.nr() &&
                   lda_dims != 0,
               "\t void compute_lda_transform()"
  ...
  // pick out only the number of dimensions we are supposed to for the output,
  // unless we should just keep them all, then don't do anything.
