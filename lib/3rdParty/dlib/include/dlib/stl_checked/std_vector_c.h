...
public:
  ...
  explicit std_vector_c(const Allocator &alloc = Allocator())
      : base_type(alloc) {}
  ...
  reference operator[](size_type n) {
    DLIB_CASSERT(n < size(), "\treference std_vector_c::operator[](n)"
  ...
  const_reference operator[](size_type n) const {
    DLIB_CASSERT(n < size(), "\tconst_reference std_vector_c::operator[](n)"
                                 << "\n\tYou have supplied an invalid index"
                                 << "\n\tthis:   " << this << "\n\tn:      "
                                 << n << "\n\tsize(): " << size());
...
// Add these swaps just to make absolutely sure the specialized swap always gets
// called even if the compiler is crappy and would otherwise mess it up.
template <typename T, typename Allocator>
void swap(std_vector_c<T, Allocator> &x, std_vector_c<T, Allocator> &y) {
  x.swap(y);
}

template <typename T, typename Allocator>
void swap(std::vector<T, Allocator> &x, std_vector_c<T, Allocator> &y) {
  x.swap(y);
}

template <typename T, typename Allocator>
void swap(std_vector_c<T, Allocator> &x, std::vector<T, Allocator> &y) {
  y.swap(x);
}
...
void serialize(const std_vector_c<T, alloc> &item, std::ostream &out) {
  try {
...
template <typename T, typename alloc>
void deserialize(std_vector_c<T, alloc> &item, std::istream &in) {
  try {
    unsigned long size;
    deserialize(size, in);
    item.resize(size);
...
template <typename T, typename alloc>
struct is_std_vector<std_vector_c<T, alloc>> {
