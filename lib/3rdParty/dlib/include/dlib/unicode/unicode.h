...
struct unichar_traits {
  ...
  static char_type *move(char_type *s1, const char_type *s2, size_t n) {
    return static_cast<char_type *>(
        std::memmove(s1, s2, sizeof(char_type) * n));
  }
  ...
  static int_type to_int_type(const char_type &c) {
    return zero_extend_cast<int_type>(c);
  }
...
int u8_to_u32(charT &result, std::istream &in)
/*!
    ensures
...
!*/
{
  ...
  if ((ch[0] & ~0x07) == 0xF0) {
    ...
    if ((ch[0] ^ 0xF6) < 4)
      return -1;
    ch[0] &= 0x07;
    result = static_cast<charT>((ch[0] << 18) | (ch[1] << 12) | (ch[2] << 6) |
                                ch[3]);
    if (result < 0x10000)
      return -1; // overlong form
...
public:
  basic_utf8_streambuf(std::ifstream &fin_) : fin(fin_) {
    this->setg(in_buffer + max_putback, in_buffer + max_putback,
...
protected:
  ...
  int_type underflow() {
    ...
    std::memmove(in_buffer + (max_putback - num_put_back),
                 this->gptr() - num_put_back, num_put_back);
    ...
    this->setg(in_buffer + (max_putback - num_put_back),
               in_buffer + max_putback, in_buffer + max_putback + i);
...
#endif
template <typename T> bool is_combining_char(const T ch_) {
...
inline const ustring convert_utf8_to_utf32(const std::string &str) {
...
void unichar_to_surrogate_pair(unichar unicode, unichar &first,
                               unichar &second);

const ustring convert_wstring_to_utf32(const std::wstring &wstr);
...
public:
  basic_utf8_ifstream() : std::basic_istream<charT>(&buf), buf(fin) {}

  basic_utf8_ifstream(const char *file_name,
  ...
  basic_utf8_ifstream(const std::string &file_name,
                      std::ios_base::openmode mode = std::ios::in)
      : std::basic_istream<charT>(&buf), buf(fin) {
