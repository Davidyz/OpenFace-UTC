...
template <typename T> class pipe {
  /*!
      INITIAL VALUE
          ...
          - enabled == true
          - data == a pointer to an array of ((pipe_max_size>0)?pipe_max_size:1)
  T objects.
          - dequeue_waiters == 0
          - enqueue_waiters == 0
          - first == 1
      ...
      CONVENTION
          ...
          - dequeue_waiters == the number of threads blocked on calls to
  dequeue()
          - enqueue_waiters == the number of threads blocked on calls to
  enqueue() and wait_until_empty()
          - unblock_sig_waiters == the number of threads blocked on calls to
            wait_for_num_blocked_dequeues() and the destructor.  (i.e. the
  number of blocking calls to unblock_sig.wait())

          - dequeue_sig == the signaler that threads blocked on calls to
  dequeue() wait on
          - enqueue_sig == the signaler that threads blocked on calls to
  enqueue() or wait_until_empty() wait on.
          - unblock_sig == the signaler that is signaled when a thread stops
  blocking on a call to enqueue() or dequeue().  It is also signaled when a
  dequeue that will probably block is called.  The destructor and
  wait_for_num_blocked_dequeues are the only things that will wait on this
  signaler.

          - if (pipe_size > 0) then
              - data[first] == the next item to dequeue
              - data[last] == the item most recently added via enqueue, so the
  last to dequeue.
...
public:
  ...
  explicit pipe(size_t maximum_size);
  ...
  void wait_for_num_blocked_dequeues(unsigned long num) const;

  void enable();

  void disable();

  bool is_enqueue_enabled() const;

  void disable_enqueue();

  void enable_enqueue();

  bool is_dequeue_enabled() const;
  ...
  bool enqueue_or_timeout(T &item, unsigned long timeout);

  bool enqueue_or_timeout(T &&item, unsigned long timeout) {
    return enqueue_or_timeout(item, timeout);
  }

  bool dequeue_or_timeout(T &item, unsigned long timeout);
...
template <typename T>
pipe<T>::pipe(size_t maximum_size)
    : pipe_size(0), pipe_max_size(maximum_size), enabled(true),
      data(new T[(maximum_size > 0) ? maximum_size : 1]), first(1), last(1),
      dequeue_sig(m), enqueue_sig(m), unblock_sig(m), dequeue_waiters(0),
      enqueue_waiters(0), unblock_sig_waiters(0), enqueue_enabled(true),
      dequeue_enabled(true) {}

// ----------------------------------------------------------------------------------------

template <typename T> pipe<T>::~pipe() {
  auto_mutex M(m);
  ++unblock_sig_waiters;
...
template <typename T> void pipe<T>::empty() {
  auto_mutex M(m);
  pipe_size = 0;
...
template <typename T> void pipe<T>::wait_until_empty() const {
  auto_mutex M(m);
...
template <typename T> void pipe<T>::enable() {
  auto_mutex M(m);
  enabled = true;
}
...
template <typename T> void pipe<T>::disable() {
  auto_mutex M(m);
  enabled = false;
  dequeue_sig.broadcast();
  enqueue_sig.broadcast();
...
template <typename T> bool pipe<T>::is_enabled() const {
  auto_mutex M(m);
  return enabled;
}
...
template <typename T> size_t pipe<T>::max_size() const {
  auto_mutex M(m);
...
template <typename T> size_t pipe<T>::size() const {
  auto_mutex M(m);
  return pipe_size;
}
...
template <typename T> bool pipe<T>::enqueue(T &item) {
  auto_mutex M(m);
  ++enqueue_waiters;
...
template <typename T> bool pipe<T>::dequeue(T &item) {
  auto_mutex M(m);
  ++dequeue_waiters;
...
template <typename T>
bool pipe<T>::enqueue_or_timeout(T &item, unsigned long timeout) {
  auto_mutex M(m);
  ++enqueue_waiters;
...
template <typename T>
bool pipe<T>::dequeue_or_timeout(T &item, unsigned long timeout) {
  auto_mutex M(m);
  ++dequeue_waiters;
...
template <typename T>
void pipe<T>::wait_for_num_blocked_dequeues(unsigned long num) const {
  auto_mutex M(m);
  ++unblock_sig_waiters;
...
template <typename T> bool pipe<T>::is_enqueue_enabled() const {
  auto_mutex M(m);
...
template <typename T> void pipe<T>::disable_enqueue() {
  auto_mutex M(m);
...
template <typename T> void pipe<T>::enable_enqueue() {
  auto_mutex M(m);
...
template <typename T> bool pipe<T>::is_dequeue_enabled() const {
  auto_mutex M(m);
...
template <typename T> void pipe<T>::disable_dequeue() {
  auto_mutex M(m);
...
template <typename T> void pipe<T>::enable_dequeue() {
  auto_mutex M(m);
  dequeue_enabled = true;
}
