...
template <typename T> class pipe {
  ...
  containing items of type T.  It is suitable for passing objects between
  threads.
...
public:
  ...
  /*!
      ensures
          - returns true if the enqueue() and enqueue_or_timeout() functions are
            ...
            if is_enqueue_enabled() returns true.  But if is_enqueue_enabled()
  == false then enqueue functions are always disabled no matter the state of
            is_enabled())
  !*/
  ...
  /*!
      ensures
          - returns true if the dequeue() and dequeue_or_timeout() functions are
            ...
            if is_dequeue_enabled() returns true.  But if is_dequeue_enabled()
  == false then dequeue functions are always disabled no matter the state of
            is_enabled())
  !*/
  ...
  /*!
      ensures
          - if (size() == max_size()) then
              - this call to enqueue() blocks until one of the following is the
  case:
                  - there is room in the pipe for another item
                  - max_size() == 0 and another thread is trying to dequeue from
  this pipe and we can pass our item object directly to that thread.
  ...
  /*!
      ensures
          - if (size() == max_size() && timeout > 0) then
              - this call to enqueue_or_timeout() blocks until one of the
  ...
  /*!
      ensures
          - if (size() == 0) then
              - this call to dequeue() blocks until one of the following is the
  case:
                  - there is something in the pipe we can dequeue
                  - max_size() == 0 and another thread is trying to enqueue an
  item onto this pipe and we can receive our item directly from that thread.
                  - someone calls disable()
  ...
  /*!
      ensures
          - if (size() == 0 && timeout > 0) then
              - this call to dequeue_or_timeout() blocks until one of the
  following is the case:
                  - there is something in the pipe we can dequeue
                  - max_size() == 0 and another thread is trying to enqueue an
  item onto this pipe and we can receive our item directly from that thread.
