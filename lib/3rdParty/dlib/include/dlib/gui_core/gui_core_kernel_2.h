...
void get_from_clipboard(std::wstring &str);

void get_from_clipboard(dlib::ustring &str);
...
public:
  ...
  inline pixel *operator[](unsigned long row) const {
    DLIB_ASSERT(
        row < height(),
  ...
  void fill(unsigned char red_, unsigned char green_,
            unsigned char blue_) const;

private:
  friend class gui_core_kernel_2_globals::event_handler_thread;

  canvas(unsigned char *bits_, unsigned long left_, unsigned long top_,
         unsigned long right_, unsigned long bottom_)
      : rectangle(left_, top_, right_, bottom_), bits(bits_), width_(width()),
        height_(height()), row_width(width_ * 4) {}
...
template <> struct pixel_traits<canvas::pixel> {
...
private:
  ...
  bool has_been_resized; // true if someone called set_size() and the
                         // on_window_resized() event hasn't yet occurred.
  bool has_been_moved; // true if someone called set_pos() and the
                       // on_window_moved() event hasn't yet occurred.

  // The following 3 variables (and x11_stuff.last_click_time) are only accessed
  // from the event handling loop (except for being initialized below). They
  // record the last mouse click event details.
...
public:
  base_window(bool resizable_ = true, bool undecorated = false);
  ...
  void set_title(const dlib::ustring &title_);
  ...
  void set_pos(long x_, long y_);

  void get_pos(long &x_, long &y_);

  void get_size(unsigned long &width_, unsigned long &height_) const;
...
protected:
  ...
  virtual void on_user_event(void *, int) {}
