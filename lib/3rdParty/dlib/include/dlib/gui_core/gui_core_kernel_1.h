...
private:
  friend LRESULT CALLBACK gui_core_kernel_1_globals::WndProc(HWND, UINT, WPARAM,
                                                             LPARAM);

  canvas(unsigned char *bits_, unsigned long padding_, unsigned long left_,
         unsigned long top_, unsigned long right_, unsigned long bottom_)
      : rectangle(left_, top_, right_, bottom_), bits(bits_), width_(width()),
        height_(height()), row_width(width_ * 3 + padding_) {}
...
class base_window {
  friend LRESULT CALLBACK gui_core_kernel_1_globals::WndProc(HWND, UINT, WPARAM,
                                                             LPARAM);
  dlib::shared_ptr_thread_safe<gui_core_kernel_1_globals::event_handler_thread>
      globals;
  ...
  // also note that this variable is only accessed from the event handling
  // thread (except for being initialized below in the constructor, but that is
  // inside the window_table mutex so it doesn't matter).
  bool mouse_in;
...
private:
  virtual void paint(const canvas &) = 0;
