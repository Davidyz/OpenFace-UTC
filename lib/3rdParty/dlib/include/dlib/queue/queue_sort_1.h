...
template <typename queue_base> class queue_sort_1 : public queue_base {
...
public:
  ...
  template <typename compare_type> void sort(const compare_type &compare) {
    if (this->size() > 1) {
...
private:
  ...
  void sort_this_queue(queue_base &queue, long depth,
  ...
  !*/
  {
    if (queue.size() <= 1) {
      ...
      if (left.size() > right.size())
        ratio = left.size() /
                (right.size() + 1); // add 1 so we can't divide by zero
      else
...
inline void swap(queue_sort_1<queue_base> &a, queue_sort_1<queue_base> &b) {
  a.swap(b);
}
...
template <typename queue_base> void queue_sort_1<queue_base>::sort() {
  if (this->size() > 1) {
