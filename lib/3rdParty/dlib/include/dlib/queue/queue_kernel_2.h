...
template <typename T, unsigned long block_size,
          typename mem_manager = default_memory_manager>
class queue_kernel_2 : public enumerable<T>, public remover<T> {
  ...
  /*!
      ...
      CONVENTION
          ...
          if (queue_size > 0)
          {
              ...
              when enqueuing elements inside each node item[0] is filled first,
  then item[1], then item[2], etc.
...
public:
  ...
  queue_kernel_2()
      : in(0), out(0), queue_size(0), current_element(0), at_start_(true) {}
  ...
  void cat(queue_kernel_2 &item);

  T &current();

  const T &current() const;

  void swap(queue_kernel_2 &item);
...
private:
  void delete_nodes(node *start, node *end);
  /*!
      requires
          ...
          - by following the next pointers you eventually hit the node pointed
            to by end
      ensures
          - calls delete on the start node, the end node, and all nodes in
  between
  !*/
...
template <typename T, unsigned long block_size, typename mem_manager>
inline void swap(queue_kernel_2<T, block_size, mem_manager> &a,
                 queue_kernel_2<T, block_size, mem_manager> &b) {
  a.swap(b);
}

template <typename T, unsigned long block_size, typename mem_manager>
void deserialize(queue_kernel_2<T, block_size, mem_manager> &item,
                 std::istream &in) {
  try {
    item.clear();
    unsigned long size;
...
queue_kernel_2<T, block_size, mem_manager>::~queue_kernel_2() {
  COMPILE_TIME_ASSERT(0 < block_size &&
                      block_size < (unsigned long)(2000000000));
...
template <typename T, unsigned long block_size, typename mem_manager>
void queue_kernel_2<T, block_size, mem_manager>::clear() {
  if (queue_size > 0) {
    delete_nodes(out, in);
    queue_size = 0;
  }
...
template <typename T, unsigned long block_size, typename mem_manager>
void queue_kernel_2<T, block_size, mem_manager>::enqueue(T &item) {
  if (queue_size == 0) {
...
template <typename T, unsigned long block_size, typename mem_manager>
void queue_kernel_2<T, block_size, mem_manager>::dequeue(T &item) {
  // swap out into item
...
template <typename T, unsigned long block_size, typename mem_manager>
void queue_kernel_2<T, block_size, mem_manager>::cat(
...
template <typename T, unsigned long block_size, typename mem_manager>
T &queue_kernel_2<T, block_size, mem_manager>::current() {
...
template <typename T, unsigned long block_size, typename mem_manager>
const T &queue_kernel_2<T, block_size, mem_manager>::current() const {
  return out->item[out_pos];
}
...
template <typename T, unsigned long block_size, typename mem_manager>
void queue_kernel_2<T, block_size, mem_manager>::swap(
    queue_kernel_2<T, block_size, mem_manager> &item) {
  exchange(in, item.in);
...
template <typename T, unsigned long block_size, typename mem_manager>
size_t queue_kernel_2<T, block_size, mem_manager>::size() const {
  return queue_size;
}
...
template <typename T, unsigned long block_size, typename mem_manager>
bool queue_kernel_2<T, block_size, mem_manager>::at_start() const {
  return at_start_;
}
...
template <typename T, unsigned long block_size, typename mem_manager>
void queue_kernel_2<T, block_size, mem_manager>::reset() const {
  at_start_ = true;
  current_element = 0;
}
...
template <typename T, unsigned long block_size, typename mem_manager>
bool queue_kernel_2<T, block_size, mem_manager>::current_element_valid() const {
...
template <typename T, unsigned long block_size, typename mem_manager>
const T &queue_kernel_2<T, block_size, mem_manager>::element() const {
...
template <typename T, unsigned long block_size, typename mem_manager>
T &queue_kernel_2<T, block_size, mem_manager>::element() {
  return current_element->item[current_element_pos];
}
...
template <typename T, unsigned long block_size, typename mem_manager>
bool queue_kernel_2<T, block_size, mem_manager>::move_next() const {
  if (at_start_) {
    at_start_ = false;
...
template <typename T, unsigned long block_size, typename mem_manager>
void queue_kernel_2<T, block_size, mem_manager>::remove_any(T &item) {
  dequeue(item);
}
...
template <typename T, unsigned long block_size, typename mem_manager>
void queue_kernel_2<T, block_size, mem_manager>::delete_nodes(node *start,
                                                              node *end) {
  node *temp;
  while (start != end) {
    temp = start;
