...
class queue_kernel_1 : public enumerable<T>, public remover<T> {
...
public:
  ...
  void cat(queue_kernel_1 &item);

  T &current();

  const T &current() const;

  void swap(queue_kernel_1 &item);
...
private:
  void delete_nodes(node *start, unsigned long length);
  /*!
      requires
...
inline void swap(queue_kernel_1<T, mem_manager> &a,
                 queue_kernel_1<T, mem_manager> &b) {
  a.swap(b);
}

template <typename T, typename mem_manager>
void deserialize(queue_kernel_1<T, mem_manager> &item, std::istream &in) {
  try {
    item.clear();
    unsigned long size;
    deserialize(size, in);
...
template <typename T, typename mem_manager>
void queue_kernel_1<T, mem_manager>::clear() {
  delete_nodes(out, queue_size);
  queue_size = 0;
...
template <typename T, typename mem_manager>
void queue_kernel_1<T, mem_manager>::enqueue(T &item) {
  // make new node
  node *temp = new node;
...
template <typename T, typename mem_manager>
void queue_kernel_1<T, mem_manager>::dequeue(T &item) {
  // swap out into item
...
template <typename T, typename mem_manager>
void queue_kernel_1<T, mem_manager>::cat(queue_kernel_1<T, mem_manager> &item) {
...
template <typename T, typename mem_manager>
T &queue_kernel_1<T, mem_manager>::current() {
  return out->item;
}
...
template <typename T, typename mem_manager>
const T &queue_kernel_1<T, mem_manager>::current() const {
  return out->item;
}
...
template <typename T, typename mem_manager>
void queue_kernel_1<T, mem_manager>::swap(
...
template <typename T, typename mem_manager>
bool queue_kernel_1<T, mem_manager>::at_start() const {
  return at_start_;
}
...
template <typename T, typename mem_manager>
size_t queue_kernel_1<T, mem_manager>::size() const {
  return queue_size;
}
...
template <typename T, typename mem_manager>
void queue_kernel_1<T, mem_manager>::reset() const {
  at_start_ = true;
  current_element = 0;
}
...
template <typename T, typename mem_manager>
bool queue_kernel_1<T, mem_manager>::current_element_valid() const {
...
template <typename T, typename mem_manager>
const T &queue_kernel_1<T, mem_manager>::element() const {
...
template <typename T, typename mem_manager>
T &queue_kernel_1<T, mem_manager>::element() {
  return current_element->item;
}
...
template <typename T, typename mem_manager>
bool queue_kernel_1<T, mem_manager>::move_next() const {
  if (at_start_) {
    at_start_ = false;
...
template <typename T, typename mem_manager>
void queue_kernel_1<T, mem_manager>::remove_any(T &item) {
  dequeue(item);
}
...
template <typename T, typename mem_manager>
void queue_kernel_1<T, mem_manager>::delete_nodes(node *start,
