...
class queue : public enumerable<T>, public remover<T> {
...
inline void swap(queue<T, mem_manager> &a, queue<T, mem_manager> &b) {
  a.swap(b);
}
/*!
...
template <typename T, typename mem_manager>
void deserialize(queue<T, mem_manager> &item, std::istream &in);
/*!
