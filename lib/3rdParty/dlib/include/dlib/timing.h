...
/*!A timing
    ...
    example, if we wanted to find out how much time we spent in a loop a
convenient way to do this would be as follows:
...
inline void start(int i) { time_buf()[i] -= ts(); }
...
inline void stop(int i) { time_buf()[i] += ts(); }
...
inline void print() {
  ...
  for (int i = 0; i < TIME_SLOTS; ++i) {
    if (time_buf()[i] != 0) {
      ...
      string name;
      // Check if the name buffer is empty.  Use the name it contains if it
      // isn't.
      if (name_buf(i, "")[0] != '\0')
        name = cast_to_string(i) + ": " + name_buf(i, "");
      else
      ...
      // make sure the name is always the same length.  Do so by padding with
      // spaces
      ...
      if (time < 1000)
        ...
        cout << "  " << name << ": " << time / 1000.0 / 60.0 << " minutes"
             << endl;
      else
        cout << "  " << name << ": " << time / 1000.0 / 60.0 / 60.0 << " hours"
             << endl;
    }
  }
}
