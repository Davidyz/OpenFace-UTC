...
template <typename T> struct unordered_pair {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object is very similar to the std::pair struct except
  unordered_pair is only capable of representing an unordered set of two items
  rather than an ordered list of two items like std::pair.
  ...
  template <typename U>
  unordered_pair(const unordered_pair<U> &p)
  ...
  unordered_pair &operator=(const unordered_pair &item)
  /*!
      ensures
          - #*this == item
  !*/
...
template <typename T>
void serialize(const unordered_pair<T> &item, std::ostream &out) {
  try {
...
template <typename T>
void deserialize(unordered_pair<T> &item, std::istream &in) {
  try {
    T a, b;
    deserialize(a, in);
    deserialize(b, in);
