...
template <typename T> reference_wrapper<T> ref(reference_wrapper<T> obj) {
  return obj;
}
/*!
    ensures
...
template <typename T> reference_wrapper<const T> cref(const T &obj) {
...
template <typename T>
reference_wrapper<const T> cref(reference_wrapper<T> obj) {
  return cref(obj.get());
}
/*!
    ensures
        - converts the given reference_wrapper into a reference_wrapper that
contains a constant reference.
!*/
