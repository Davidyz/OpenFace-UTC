...
struct float_details {
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          particular, a float_details object represents a floating point number
  with a 64 bit mantissa and 16 bit exponent.  These are stored in the public
          fields of the same names.

          The main use of this object is to convert floating point values into a
          known uniform representation so they can be serialized to an output
  stream. This allows dlib serialization code to work on any system, regardless
  of the floating point representation used by the hardware.  It also means
          that, for example, a double can be serialized and then deserialized
  into a float and it will perform the appropriate conversion.
          ...
          following special values:
              ...
              - is_ninf
              - is_nan
          These values are used to indicate that the floating point value should
  be either infinity, negative infinity, or not-a-number respectively.
  !*/

  float_details(int64 man, int16 exp) : mantissa(man), exponent(exp) {}
  /*!
      ensures
          - #mantissa == man
          - #exponent == exp
  ...
  float_details() : mantissa(0), exponent(0) {}
  /*!
      ensures
  ...
  /*!
      ensures
          - converts the given value into a float_details representation.  This
            means that converting #*this back into a floating point number
  should recover the input val.
  !*/
  ...
  /*!
      ensures
          - converts the contents of this float_details object into a floating
  point number.
  !*/
...
private:
  ...
  template <typename T> void convert_from_T(const T &val) {
    mantissa = 0;
    ...
    if (val == std::numeric_limits<T>::infinity()) {
      ...
      int exp;
      mantissa =
          static_cast<int64>(std::frexp(val, &exp) * (((uint64)1) << digits));
      exponent = exp - digits;
      ...
      // exponent generally smaller which can make serialization and other
      // things more efficient in some cases.
  ...
  template <typename T> T convert_to_T() const {
    if (exponent < is_inf)
