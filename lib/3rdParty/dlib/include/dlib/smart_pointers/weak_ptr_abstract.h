...
template <typename T> class weak_ptr {
  ...
  is already managed by a shared_ptr. To access the object, a weak_ptr can be
  converted to a shared_ptr using the member function lock().
...
public:
  ...
  template <typename Y> weak_ptr(const weak_ptr<Y> &r);
  /*!
      requires
          - Y* must be convertible to T*
      ensures
...
template <typename T> void swap(weak_ptr<T> &a, weak_ptr<T> &b) { a.swap(b); }
/*!
