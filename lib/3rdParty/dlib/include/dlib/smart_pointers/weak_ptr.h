  ...
  /*!
      CONVENTION
          - if (weak_node != 0) then
              ...
              - weak_node->ref_count == the number of weak_ptrs that reference
  this->data
          - else
              - data == 0
...
public:
  ...
  template <typename Y> bool _private_less(const weak_ptr<Y> &rhs) const {
    if (expired()) {
      if (rhs.expired()) {
        return false;
      } else {
