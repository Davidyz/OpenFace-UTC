...
class shared_ptr {
  ...
  shared_ptr contains a pointer to some object and when the last shared_ptr that
  points to the object is destructed or reset() then the object is guaranteed to
  be deleted.
  ...
  this shared_ptr is declared inside the dlib namespace rather than std::tr1.
...
public:
  ...
  template <typename Y, typename D> shared_ptr(Y *p, const D &d);
  /*!
      requires
          - p is convertible to a T* type pointer
  ...
  template <typename Y> explicit shared_ptr(const weak_ptr<Y> &r);
  /*!
      requires
          - Y* is convertible to T*
      ensures
          ...
          - #use_count() == #r.use_count()
          - If r is empty, constructs an empty shared_ptr object; otherwise,
  constructs a shared_ptr object that shares ownership with r. throws
          - bad_weak_ptr
...
/*!
    ensures
        - Defines an operator< on shared_ptr types appropriate for use in the
...
/*!
    ...
    - else
        - returns a shared_ptr<T> object that stores static_cast<T*>(r.get())
...
/*!
    - if (r.get() == 0) then
        - returns shared_ptr<T>()
    - else
        - returns a shared_ptr<T> object that stores const_cast<T*>(r.get()) and
...
template <typename D, typename T> D *get_deleter(const shared_ptr<T> &p);
/*!
    ensures
