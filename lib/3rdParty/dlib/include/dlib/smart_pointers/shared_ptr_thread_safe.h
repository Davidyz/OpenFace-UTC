...
struct shared_ptr_thread_safe_node {
  shared_ptr_thread_safe_node() : ref_count(1), del(0) {}
...
public:
  ...
  shared_ptr_thread_safe() : data(0), shared_node(0) {}

  template <typename Y> explicit shared_ptr_thread_safe(Y *p) : data(p) {
    DLIB_ASSERT(p != 0, "\tshared_ptr::shared_ptr_thread_safe(p)"
                            << "\n\tp can't be null"
                            << "\n\tthis: " << this);
    try {
  ...
  shared_ptr_thread_safe(Y *p, const D &d) : data(p) {
    DLIB_ASSERT(p != 0, "\tshared_ptr::shared_ptr_thread_safe(p,d)"
                            << "\n\tp can't be null"
                            << "\n\tthis: " << this);
    try {
      shared_node = 0;
  ...
  void reset() { shared_ptr_thread_safe().swap(*this); }

  template <typename Y> void reset(Y *p) {
    DLIB_ASSERT(p != 0, "\tshared_ptr::reset(p)"
                            << "\n\tp can't be null"
                            << "\n\tthis: " << this);
  ...
  template <typename Y, typename D> void reset(Y *p, const D &d) {
    DLIB_ASSERT(p != 0, "\tshared_ptr::reset(p,d)"
                            << "\n\tp can't be null"
                            << "\n\tthis: " << this);
  ...
  T *operator->() const {
    DLIB_ASSERT(get() != 0, "\tshared_ptr::operator->()"
                                << "\n\tget() can't be null"
                                << "\n\tthis: " << this);
  ...
  template <typename D> D *_get_deleter() const {
    if (shared_node) {
      auto_mutex M(shared_node->m);
      if (shared_node->del)
  ...
  template <typename Y>
  bool _private_less(const shared_ptr_thread_safe<Y> &rhs) const {
...
template <typename T, typename U>
shared_ptr_thread_safe<T>
const_pointer_cast(shared_ptr_thread_safe<U> const &r) {
...
std::basic_ostream<E, T> &operator<<(std::basic_ostream<E, T> &os,
                                     shared_ptr_thread_safe<Y> const &p) {
  os << p.get();
  return os;
}
