...
struct shared_ptr_node {
  shared_ptr_node() : ref_count(1), del(0), weak_node(0) {}
...
template <typename T> class shared_ptr {
  /*!
      CONVENTION
          ...
          - if (there are any weak_ptrs that reference this->data) then
              - shared_node->weak_node->ref_count == the number of referencing
  weak_ptrs
          - else
              - shared_node->weak_node == 0
  !*/

  template <typename D> struct deleter_template : public shared_ptr_deleter {
  ...
  struct default_deleter : public shared_ptr_deleter {
    ...
    void *get_deleter_void(const std::type_info &) const { return 0; }
  };

public:
  typedef T element_type;

  shared_ptr() : data(0), shared_node(0) {}

  template <typename Y> explicit shared_ptr(Y *p) : data(p) {
    DLIB_ASSERT(p != 0, "\tshared_ptr::shared_ptr(p)"
                            << "\n\tp can't be null"
                            << "\n\tthis: " << this);
    try {
  ...
  template <typename Y, typename D> shared_ptr(Y *p, const D &d) : data(p) {
    DLIB_ASSERT(p != 0, "\tshared_ptr::shared_ptr(p,d)"
                            << "\n\tp can't be null"
                            << "\n\tthis: " << this);
    try {
      shared_node = 0;
  ...
  template <typename Y>
  shared_ptr(const shared_ptr<Y> &r, const shared_ptr_static_cast &) {
  ...
  template <typename Y>
  shared_ptr(const shared_ptr<Y> &r, const shared_ptr_const_cast &) {
  ...
  template <typename Y>
  shared_ptr(const shared_ptr<Y> &r, const shared_ptr_dynamic_cast &) {
  ...
  template <typename Y> bool _private_less(const shared_ptr<Y> &rhs) const {
