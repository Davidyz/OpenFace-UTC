...
public:
  ...
  inline crc32(const std::string &item);

  inline crc32(const std::vector<char> &item);

  inline virtual ~crc32();

  inline void clear();

  inline void add(unsigned char item);

  inline void add(const std::string &item);

  inline void add(const std::vector<char> &item);

  inline operator unsigned long() const { return get_checksum(); }

  inline unsigned long get_checksum() const;

  inline void swap(crc32 &item);

  inline crc32 &operator=(const crc32 &);

private:
  unsigned long checksum;

  inline unsigned long table(unsigned int idx) const {
    /*
    ...
    */
    ...
    const static unsigned long crc_table[256] = {
        0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x76dc419,  0x706af48f,
...
inline void swap(crc32 &a, crc32 &b) { a.swap(b); }
...
crc32::crc32() { checksum = 0xFFFFFFFF; }
...
void crc32::clear() { checksum = 0xFFFFFFFF; }
...
unsigned long crc32::get_checksum() const { return checksum ^ 0xFFFFFFFF; }

// ----------------------------------------------------------------------------------------

void crc32::swap(crc32 &item) { exchange(checksum, item.checksum); }

// ----------------------------------------------------------------------------------------

crc32 &crc32::operator=(const crc32 &item) {
