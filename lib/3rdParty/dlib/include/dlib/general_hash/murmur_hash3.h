...
// code. The code in this particular file was modified by Davis E. King.  In
...
DLIB_FORCE_INLINE uint32 murmur_getblock(const uint32 *p, int i) {
  // The reason we do a memcpy() here instead of simply returning p[i] is
  // because doing it this way avoids violations of the strict aliasing rule
  // when all these functions are inlined into the user's code.
  uint32 temp;
  memcpy(&temp, p + i, 4);
  return temp;
}
...
inline uint32 murmur_hash3(const void *key, const int len,
                           const uint32 seed = 0) {
...
inline uint32 murmur_hash3_3(const uint32 v1, const uint32 v2,
                             const uint32 v3) {
...
inline std::pair<uint64, uint64>
murmur_hash3_128bit(const void *key, const int len, const uint32 seed = 0) {
  const uint8 *data = (const uint8 *)key;
  ...
  if (is_little_endian) {
    for (int i = 0; i < nblocks; i++) {
      ...
      k1 *= c1;
      k1 = DLIB_ROTL64(k1, 31);
      k1 *= c2;
      h1 ^= k1;

      h1 = DLIB_ROTL64(h1, 27);
      h1 += h2;
      h1 = h1 * 5 + 0x52dce729;

      k2 *= c2;
      k2 = DLIB_ROTL64(k2, 33);
      k2 *= c1;
      h2 ^= k2;

      h2 = DLIB_ROTL64(h2, 31);
      h2 += h1;
      h2 = h2 * 5 + 0x38495ab5;
    }
  } else {
...
inline std::pair<uint64, uint64> murmur_hash3_128bit(const uint32 &v1,
                                                     const uint32 &v2,
                                                     const uint32 &v3,
                                                     const uint32 &v4) {
  uint64 h1 = 0;
...
inline std::pair<uint64, uint64> murmur_hash3_128bit_3(uint64 k1, uint64 k2,
                                                       uint64 k3) {
  uint64 h1 = k3;
  uint64 h2 = k3;
