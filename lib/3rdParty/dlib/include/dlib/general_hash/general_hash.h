...
template <typename T> class general_hash {
public:
  inline unsigned long operator()(const T &item) const;
};
/*!
...
template <typename T>
unsigned long general_hash<T>::operator()(const T &item) const {
...
general_hash<std::string>::operator()(const std::string &item) const {
  return hash(item);
}
