...
template <typename T>
T hamming_distance(const T &a, const T &b)
/*!
    requires
        - T is an unsigned integral type
    ensures
...
template <typename T>
T hamming_distance(const std::pair<T, T> &a, const std::pair<T, T> &b)
/*!
    requires
        - T is an unsigned integral type or a std::pair that, recursively,
eventually contains unsigned integral types. ensures
        - returns the number of bits which differ between a and b.
!*/
{
