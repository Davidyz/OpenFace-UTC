...
class tokenizer {
  /*!
      INITIAL VALUE
          stream_is_set() == false
          get_identifier_head() == "_" + lowercase_letters() +
  uppercase_letters() get_identifier_body() == "_" + lowercase_letters() +
  uppercase_letters() + numbers()
      ...
      TOKENS
          ...
          This means that if you concatenated all returned tokens it would
  exactly reproduce the contents of the input stream.
...
public:
  enum { END_OF_LINE, END_OF_FILE, IDENTIFIER, CHAR, NUMBER, WHITE_SPACE };
  ...
  void set_identifier_token(const std::string &head, const std::string &body);
  /*!
      requires
  ...
  const std::string get_identifier_head() const;
  /*!
      ensures
  ...
  const std::string get_identifier_body() const;
  /*!
      ensures
  ...
  const std::string lowercase_letters() const;
  /*!
      ensures
  ...
  const std::string uppercase_letters() const;
  /*!
      ensures
          - returns "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
      throws
...
inline void swap(tokenizer &a, tokenizer &b) { a.swap(b); }
/*!
