...
template <typename tokenizer> class tokenizer_kernel_c : public tokenizer {
...
inline void swap(tokenizer_kernel_c<tokenizer> &a,
...
template <typename tokenizer>
void tokenizer_kernel_c<tokenizer>::set_identifier_token(
...
template <typename tokenizer>
std::istream &tokenizer_kernel_c<tokenizer>::get_stream() const {
  // make sure requires clause is not broken
  DLIB_CASSERT(
...
template <typename tokenizer>
int tokenizer_kernel_c<tokenizer>::peek_type() const {
  // make sure requires clause is not broken
  DLIB_CASSERT(this->stream_is_set() == true,
               "\tint tokenizer::peek_type()"
                   << "\n\tyou must set a stream for this object before you "
                      "peek at what it contains"
                   << "\n\tthis: " << this);
...
template <typename tokenizer>
const std::string &tokenizer_kernel_c<tokenizer>::peek_token() const {
  // make sure requires clause is not broken
  DLIB_CASSERT(this->stream_is_set() == true,
...
template <typename tokenizer>
void tokenizer_kernel_c<tokenizer>::get_token(int &type, std::string &token) {
  // make sure requires clause is not broken
  DLIB_CASSERT(this->stream_is_set() == true,
               "\tvoid tokenizer::get_token()"
                   << "\n\tyou must set a stream for this object before you "
                      "can get tokens from it."
                   << "\n\tthis: " << this);
