...
public:
  // The name of this enum is irrelevant but on some compilers (gcc on MAC OS X)
  // not having it named causes an error for whatever reason
  ...
  void swap(tokenizer_kernel_1 &item);
...
private:
  ...
  tokenizer_kernel_1 &
  operator=(const tokenizer_kernel_1 &); // assignment operator
...
inline void swap(tokenizer_kernel_1 &a, tokenizer_kernel_1 &b) { a.swap(b); }
