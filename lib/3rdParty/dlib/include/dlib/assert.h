...
#if defined(__APPLE__) && defined(__GNUC_LIBSTD__) &&                          \
    ((__GNUC_LIBSTD__ - 0) * 100 + __GNUC_LIBSTD_MINOR__ - 0 <= 402)
// Apple has not updated libstdc++ in some time and anything under 4.02 does not
// have <initializer_list> for sure.
...
template <typename T> struct assert_are_same_type<T, T> {
  enum { value = 1 };
};
template <typename T, typename U> struct assert_are_not_same_type {
  enum { value = 1 };
};
template <typename T> struct assert_are_not_same_type<T, T> {};

template <typename T, typename U> struct assert_types_match {
  enum { value = 0 };
};
template <typename T> struct assert_types_match<T, T> {
  enum { value = 1 };
};
} // namespace dlib

// gcc 4.8 will warn about unused typedefs.  But we use typedefs in some of the
// compile time assert macros so we need to make it not complain about them "not
// being used".
#ifdef __GNUC__
...
// Use the newer static_assert if it's available since it produces much more
// readable error messages.
#ifdef DLIB_HAS_STATIC_ASSERT
#define COMPILE_TIME_ASSERT(expression)                                        \
  static_assert(expression, "Failed assertion")
#define ASSERT_ARE_SAME_TYPE(type1, type2)                                     \
  static_assert(::dlib::assert_types_match<type1, type2>::value,               \
                "These types should be the same but aren't.")
#define ASSERT_ARE_NOT_SAME_TYPE(type1, type2)                                 \
  static_assert(!::dlib::assert_types_match<type1, type2>::value,              \
                "These types should NOT be the same.")
#else
#define COMPILE_TIME_ASSERT(expression)                                        \
  DLIB_NO_WARN_UNUSED typedef char DLIB_BOOST_JOIN(                            \
      DLIB_CTA,                                                                \
      __LINE__)[::dlib::compile_time_assert<(bool)(expression)>::value]

#define ASSERT_ARE_SAME_TYPE(type1, type2)                                     \
  DLIB_NO_WARN_UNUSED typedef char DLIB_BOOST_JOIN(                            \
      DLIB_AAST, __LINE__)[::dlib::assert_are_same_type<type1, type2>::value]

#define ASSERT_ARE_NOT_SAME_TYPE(type1, type2)                                 \
  DLIB_NO_WARN_UNUSED typedef char DLIB_BOOST_JOIN(                            \
      DLIB_AANST,                                                              \
      __LINE__)[::dlib::assert_are_not_same_type<type1, type2>::value]
#endif
...
// This macro is not needed if you have a real C++ compiler.  It's here to work
// around bugs in Visual Studio's preprocessor.
#define DLIB_WORKAROUND_VISUAL_STUDIO_BUGS(x) x
// Make it so the 2nd argument of DLIB_CASSERT is optional.  That is, you can
// call it like DLIB_CASSERT(exp) or DLIB_CASSERT(exp,message).
...
#define DLIBM_CASSERT_CHOOSER(...)                                             \
  DLIB_WORKAROUND_VISUAL_STUDIO_BUGS(DLIBM_GET_3TH_ARG(                        \
      __VA_ARGS__, DLIBM_CASSERT_2_ARGS, DLIBM_CASSERT_1_ARGS))
#define DLIB_CASSERT(...)                                                      \
...
/*!A DLIB_ASSERT_HAS_STANDARD_LAYOUT
    ...
    This macro is meant to cause a compiler error if a type doesn't have a
simple memory layout (like a C struct). In particular, types with simple layouts
are ones which can be copied via memcpy().


    This was called a POD type in C++03 and in C++0x we are looking to check if
    it is a "standard layout type".  Once we can use C++0x we can change this
macro to something that uses the std::is_standard_layout type_traits class. See:
...
#define DLIB_ASSERT_HAS_STANDARD_LAYOUT(type)                                  \
  union DLIB_BOOST_JOIN(DAHSL_, __LINE__) {                                    \
    type TYPE_NOT_STANDARD_LAYOUT;                                             \
  };                                                                           \
  DLIB_NO_WARN_UNUSED typedef char DLIB_BOOST_JOIN(                            \
      DAHSL2_, __LINE__)[sizeof(DLIB_BOOST_JOIN(DAHSL_, __LINE__))];
...
/*!
    ensures
        - this function does nothing
          ...
          It is called only when an DLIB_ASSERT or DLIB_CASSERT fails and is
