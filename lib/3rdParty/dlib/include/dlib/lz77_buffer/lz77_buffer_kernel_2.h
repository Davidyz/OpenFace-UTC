...
template <typename sliding_buffer> class lz77_buffer_kernel_2 {
  /*!
      REQUIREMENTS ON sliding_buffer
          sliding_buffer must be an implementation of
  sliding_buffer/sliding_buffer_kernel_abstract.h and must be instantiated to
  contain unsigned char data
      ...
      CONVENTION
          ...
          hash_table[i] == 0 if there is no linked list for this element of the
  hash table.
...
public:
  lz77_buffer_kernel_2(unsigned long total_limit_,
  ...
  void find_match(unsigned long &index, unsigned long &length,
                  unsigned long min_match_length);

  inline unsigned long get_history_buffer_limit() const {
    return history_limit;
  }

  inline unsigned long get_lookahead_buffer_limit() const {
    return lookahead_limit;
  }

  inline unsigned long get_history_buffer_size() const { return history_size; }

  inline unsigned long get_lookahead_buffer_size() const {
    return lookahead_size;
  }

  inline unsigned char lookahead_buffer(unsigned long index) const {
    return buffer[lookahead_limit - 1 - index];
  }

  inline unsigned char history_buffer(unsigned long index) const {
    return buffer[lookahead_limit + index];
  }

  inline void shift_buffers(unsigned long N) { shift_buffer(N); }

private:
  inline unsigned long hash(unsigned char a, unsigned char b, unsigned char c,
  ...
  void shift_buffer(unsigned long N);
  /*!
      requires
          - N <= lookahead_size
      ensuers
  ...
  lz77_buffer_kernel_2(
      lz77_buffer_kernel_2<sliding_buffer> &); // copy constructor
  lz77_buffer_kernel_2<sliding_buffer> &
  operator=(lz77_buffer_kernel_2<sliding_buffer> &); // assignment operator
};
...
lz77_buffer_kernel_2<sliding_buffer>::lz77_buffer_kernel_2(
  ...
  try {
    hash_table = new node *[buffer.size()];
  } catch (...) {
    delete[] id_table;
    delete[] nodes;
    throw;
...
template <typename sliding_buffer>
void lz77_buffer_kernel_2<sliding_buffer>::clear() {
  lookahead_size = 0;
  history_size = 0;
  next_free_node = 0;
...
template <typename sliding_buffer>
void lz77_buffer_kernel_2<sliding_buffer>::shift_buffer(unsigned long N) {
  ...
  if (new_nodes != 0) {
    ...
    for (; i > stop; --i) {
      ...
      unsigned long new_hash =
          hash(buffer[i], buffer[i - 1], buffer[i - 2], buffer[i - 3]);
  ...
  for (; i > stop; --i) {
    ...
    if (id_table[old_id] == 0) {
      unsigned long old_hash =
          hash(buffer[old], buffer[old - 1], buffer[old - 2], buffer[old - 3]);
...
template <typename sliding_buffer>
void lz77_buffer_kernel_2<sliding_buffer>::add(unsigned char symbol) {
...
template <typename sliding_buffer>
void lz77_buffer_kernel_2<sliding_buffer>::find_match(
    unsigned long &index, unsigned long &length,
    unsigned long min_match_length) {
  ...
  const unsigned long hash_value =
      hash(lookahead_buffer(0), lookahead_buffer(1), lookahead_buffer(2),
           lookahead_buffer(3));
