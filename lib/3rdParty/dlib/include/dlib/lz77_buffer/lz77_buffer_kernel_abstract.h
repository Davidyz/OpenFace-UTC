...
class lz77_buffer {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents a pair of buffers (history and lookahead
  buffers) used during lz77 style compression.

          It's main function is to search the history buffer for long strings
  which match the contents (or a part of the contents) of the lookahead buffer.
      ...
      HISTORY AND LOOKAHEAD BUFFERS
          ...
          What shift_buffers() does in english:
              This function just means that the buffers have their contents
  shifted left by N elements and that elements shifted out of the lookahead
  buffer go into the history buffer.   An example will make it clearer.

              Suppose that we have the following buffers before we apply
  shift_buffers() history_buffer() == "hey" and lookahead_buffer() == "lookahead
  buffer" And in the same format as the above diagram it would be | hey |
  lookahead buffer |  <-- contents of buffers | 210 | 0123456789...    |  <--
  index numbers
              ...
              such a nondeterministic way.  It is just to allow a lot of freedom
  in the implementations of this object.
  !*/

public:
  lz77_buffer(unsigned long total_limit, unsigned long lookahead_limit);
  /*!
      requires
          - 6 < total_limit < 32
  ...
  void shift_buffers(unsigned long N);
  /*!
      requires
  ...
  /*!
      ensures
          - if (get_lookahead_buffer_size() == get_lookahead_buffer_limit())
  then
  ...
  unsigned long get_history_buffer_limit() const;
  /*!
      ensures
          - returns the max number of symbols that can fit in the history buffer
  !*/

  unsigned long get_lookahead_buffer_limit() const;
  /*!
      ensures
          - returns the max number of symbols that can fit in the lookahead
  buffer
  !*/

  unsigned long get_history_buffer_size() const;
  /*!
      ensures
          - returns the number of symbols currently in the history buffer
  !*/

  unsigned long get_lookahead_buffer_size() const;
  /*!
      ensures
          - returns the number of symbols currently in the lookahead buffer
  !*/

  unsigned char lookahead_buffer(unsigned long index) const;
  /*!
      requires
  ...
  unsigned char history_buffer(unsigned long index) const;
  /*!
      requires
