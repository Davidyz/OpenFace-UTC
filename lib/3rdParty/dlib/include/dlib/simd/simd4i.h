...
public:
  ...
  inline simd4i(int32 r0, int32 r1, int32 r2, int32 r3) : x{r0, r1, r2, r3} {}

  inline simd4i &operator=(const simd4i &v) {
    x = v.x;
    return *this;
  }
  inline simd4i &operator=(const int32 &v) {
    *this = simd4i(v);
    return *this;
  }
...
#else
  return simd4i(lhs[0] + rhs[0], lhs[1] + rhs[1], lhs[2] + rhs[2],
                lhs[3] + rhs[3]);
#endif
}
...
inline simd4i operator*(const simd4i &lhs, const simd4i &rhs) {
  ...
  int32 _rhs[4];
  rhs.store(_rhs);
  return simd4i(_lhs[0] * _rhs[0], _lhs[1] * _rhs[1], _lhs[2] * _rhs[2],
                _lhs[3] * _rhs[3]);
#elif defined(DLIB_HAVE_VSX)
...
#else
  return simd4i(~lhs[0], ~lhs[1], ~lhs[2], ~lhs[3]);
#endif
}
...
#else
  return simd4i(lhs[0] << rhs, lhs[1] << rhs, lhs[2] << rhs, lhs[3] << rhs);
#endif
}
...
inline simd4i operator>>(const simd4i &lhs, const int &rhs) {
  ...
  int32 _lhs[4];
  lhs.store(_lhs);
  return simd4i(_lhs[0] >> rhs, _lhs[1] >> rhs, _lhs[2] >> rhs, _lhs[3] >> rhs);
#else
  return simd4i(lhs[0] >> rhs, lhs[1] >> rhs, lhs[2] >> rhs, lhs[3] >> rhs);
#endif
}
...
#else
  return simd4i(
      lhs[0] == rhs[0] ? 0xFFFFFFFF : 0, lhs[1] == rhs[1] ? 0xFFFFFFFF : 0,
      lhs[2] == rhs[2] ? 0xFFFFFFFF : 0, lhs[3] == rhs[3] ? 0xFFFFFFFF : 0);
#endif
}
...
inline simd4i min(const simd4i &lhs, const simd4i &rhs) {
  ...
  int32 _rhs[4];
  rhs.store(_rhs);
  return simd4i(std::min(_lhs[0], _rhs[0]), std::min(_lhs[1], _rhs[1]),
                std::min(_lhs[2], _rhs[2]), std::min(_lhs[3], _rhs[3]));
#elif defined(DLIB_HAVE_VSX)
  return vec_min(lhs(), rhs());
#elif defined(DLIB_HAVE_NEON)
  return (int32x4_t)vminq_s32(lhs, rhs);
#else
  return simd4i(std::min(lhs[0], rhs[0]), std::min(lhs[1], rhs[1]),
...
inline simd4i max(const simd4i &lhs, const simd4i &rhs) {
  ...
  int32 _rhs[4];
  rhs.store(_rhs);
  return simd4i(std::max(_lhs[0], _rhs[0]), std::max(_lhs[1], _rhs[1]),
                std::max(_lhs[2], _rhs[2]), std::max(_lhs[3], _rhs[3]));
#elif defined(DLIB_HAVE_VSX)
  return vec_max(lhs(), rhs());
#elif defined(DLIB_HAVE_NEON)
  return vmaxq_s32(lhs, rhs);
#else
  return simd4i(std::max(lhs[0], rhs[0]), std::max(lhs[1], rhs[1]),
