...
#if defined(_MSC_VER) && (defined(_M_I86) || defined(_M_IX86) ||               \
...
inline std::array<unsigned int, 4> cpuid(int function_id) {
  ...
  return info;
    ...
    (defined(__i386__) || defined(__i686__) || defined(__amd64__) ||           \
     defined(__x86_64__))
#include <cpuid.h>
...
inline bool cpu_has_sse2_instructions() {
  return 0 != (cpuid(1)[3] & (1 << 26));
}
inline bool cpu_has_sse3_instructions() {
  return 0 != (cpuid(1)[2] & (1 << 0));
}
inline bool cpu_has_sse41_instructions() {
  return 0 != (cpuid(1)[2] & (1 << 19));
}
inline bool cpu_has_sse42_instructions() {
  return 0 != (cpuid(1)[2] & (1 << 20));
}
inline bool cpu_has_avx_instructions() {
  return 0 != (cpuid(1)[2] & (1 << 28));
}
inline bool cpu_has_avx2_instructions() {
  return 0 != (cpuid(7)[1] & (1 << 5));
}
inline bool cpu_has_avx512_instructions() {
  return 0 != (cpuid(7)[1] & (1 << 16));
}

inline void warn_about_unavailable_but_used_cpu_instructions() {
#if defined(DLIB_HAVE_AVX2)
  if (!cpu_has_avx2_instructions())
    std::cerr << "Dlib was compiled to use AVX2 instructions, but these aren't "
                 "available on your machine."
              << std::endl;
#elif defined(DLIB_HAVE_AVX)
  if (!cpu_has_avx_instructions())
    std::cerr << "Dlib was compiled to use AVX instructions, but these aren't "
                 "available on your machine."
              << std::endl;
#elif defined(DLIB_HAVE_SSE41)
  if (!cpu_has_sse41_instructions())
    std::cerr << "Dlib was compiled to use SSE41 instructions, but these "
                 "aren't available on your machine."
              << std::endl;
#elif defined(DLIB_HAVE_SSE3)
  if (!cpu_has_sse3_instructions())
    std::cerr << "Dlib was compiled to use SSE3 instructions, but these aren't "
                 "available on your machine."
              << std::endl;
#elif defined(DLIB_HAVE_SSE2)
  if (!cpu_has_sse2_instructions())
    std::cerr << "Dlib was compiled to use SSE2 instructions, but these aren't "
                 "available on your machine."
              << std::endl;
#endif
}
