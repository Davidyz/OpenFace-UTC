...
public:
  ...
  inline simd8f(float r0, float r1, float r2, float r3, float r4, float r5,
                float r6, float r7) {
  ...
  inline simd8f &operator=(const simd8i &rhs) {
    *this = simd8f(rhs);
    return *this;
  }
...
public:
  ...
  inline simd8f() {}
  inline simd8f(const simd4f &low_, const simd4f &high_)
...
public:
  ...
  inline simd8f_bool() {}
  inline simd8f_bool(const simd4f_bool &low_, const simd4f_bool &high_)
      : _low(low_), _high(high_) {}
...
#else
  return simd8f(lhs.low() + rhs.low(), lhs.high() + rhs.high());
#endif
}
...
#else
  return simd8f_bool(lhs.low() == rhs.low(), lhs.high() == rhs.high());
#endif
}
...
#else
  return simd8f(min(lhs.low(), rhs.low()), min(lhs.high(), rhs.high()));
#endif
}
...
#else
  return simd8f(max(lhs.low(), rhs.low()), max(lhs.high(), rhs.high()));
#endif
}
...
#else
  return simd8f(reciprocal(item.low()), reciprocal(item.high()));
#endif
}
...
#else
  return simd8f(reciprocal_sqrt(item.low()), reciprocal_sqrt(item.high()));
#endif
}
...
inline float sum(const simd8f &item) {
  ...
  return _mm_cvtss_f32(_mm_add_ss(_mm256_castps256_ps128(temp2),
                                  _mm256_extractf128_ps(temp2, 1)));
#else
...
#else
  return simd8f(sqrt(item.low()), sqrt(item.high()));
#endif
}
...
#else
  return simd8f(ceil(item.low()), ceil(item.high()));
#endif
}
...
#else
  return simd8f(floor(item.low()), floor(item.high()));
#endif
}
