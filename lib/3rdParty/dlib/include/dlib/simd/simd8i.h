...
public:
  ...
  inline simd8i(int32 r0, int32 r1, int32 r2, int32 r3, int32 r4, int32 r5,
                int32 r6, int32 r7) {
...
public:
  ...
  inline simd8i() {}
  inline simd8i(const simd4i &low_, const simd4i &high_)
...
#else
  return simd8i(lhs.low() + rhs.low(), lhs.high() + rhs.high());
#endif
}
...
#else
  return simd8i(lhs.low() << rhs, lhs.high() << rhs);
#endif
}
...
#else
  return simd8i(min(lhs.low(), rhs.low()), min(lhs.high(), rhs.high()));
#endif
}
...
#else
  return simd8i(max(lhs.low(), rhs.low()), max(lhs.high(), rhs.high()));
#endif
}

// ----------------------------------------------------------------------------------------

inline int32 sum(const simd8i &item) { return sum(item.low() + item.high()); }
