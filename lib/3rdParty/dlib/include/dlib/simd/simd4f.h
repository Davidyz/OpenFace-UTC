...
public:
  ...
  inline simd4f(float r0, float r1, float r2, float r3) : x{r0, r1, r2, r3} {}

  inline simd4f &operator=(const simd4f &v) {
    x = v.x;
    return *this;
  }
  inline simd4f &operator=(const float &v) {
    *this = simd4f(v);
    return *this;
  }
...
public:
  ...
  inline simd4f(const simd4i &val) {
    x[0] = val[0];
    x[1] = val[1];
    x[2] = val[2];
    x[3] = val[3];
...
public:
  ...
  inline simd4f_bool() {}
  inline simd4f_bool(bool r0, bool r1, bool r2, bool r3) {
    x[0] = r0;
    x[1] = r1;
    x[2] = r2;
    x[3] = r3;
...
#else
  return simd4f(lhs[0] + rhs[0], lhs[1] + rhs[1], lhs[2] + rhs[2],
                lhs[3] + rhs[3]);
#endif
}
...
#else
  return simd4f_bool(lhs[0] == rhs[0], lhs[1] == rhs[1], lhs[2] == rhs[2],
                     lhs[3] == rhs[3]);
#endif
}
...
#else
  return simd4f(std::min(lhs[0], rhs[0]), std::min(lhs[1], rhs[1]),
...
#else
  return simd4f(std::max(lhs[0], rhs[0]), std::max(lhs[1], rhs[1]),
...
inline simd4f reciprocal(const simd4f &item) {
  ...
  return estimate;
#else
  return simd4f(1.0f / item[0], 1.0f / item[1], 1.0f / item[2], 1.0f / item[3]);
#endif
}
...
inline simd4f reciprocal_sqrt(const simd4f &item) {
  ...
  return estimate;
#else
  return simd4f(1.0f / std::sqrt(item[0]), 1.0f / std::sqrt(item[1]),
                1.0f / std::sqrt(item[2]), 1.0f / std::sqrt(item[3]));
#endif
}
...
inline simd4f sqrt(const simd4f &item) {
  ...
  return rcorr;
#else
  return simd4f(std::sqrt(item[0]), std::sqrt(item[1]), std::sqrt(item[2]),
                std::sqrt(item[3]));
#endif
}
...
#else
  return simd4f(std::ceil(item[0]), std::ceil(item[1]), std::ceil(item[2]),
                std::ceil(item[3]));
#endif
}
...
#else
  return simd4f(std::floor(item[0]), std::floor(item[1]), std::floor(item[2]),
                std::floor(item[3]));
#endif
}
...
#else
  return simd4f(cmp[0] ? a[0] : b[0], cmp[1] ? a[1] : b[1],
                cmp[2] ? a[2] : b[2], cmp[3] ? a[3] : b[3]);
#endif
}
