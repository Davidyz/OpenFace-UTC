...
template <typename sbuf> class lzp_buffer_kernel_1 {
  /*!
...
public:
  explicit lzp_buffer_kernel_1(unsigned long buffer_size);
...
private:
  ...
  operator=(const lzp_buffer_kernel_1<sbuf> &); // assignment operator
};
...
lzp_buffer_kernel_1<sbuf>::lzp_buffer_kernel_1(unsigned long buffer_size)
    : table_size(65536), table(new unsigned long[table_size]) {
...
template <typename sbuf> lzp_buffer_kernel_1<sbuf>::~lzp_buffer_kernel_1() {
  delete[] table;
}

// ----------------------------------------------------------------------------------------

template <typename sbuf> void lzp_buffer_kernel_1<sbuf>::clear() {
  for (unsigned long i = 0; i < buffer.size(); ++i)
    buffer[i] = 0;
...
template <typename sbuf>
void lzp_buffer_kernel_1<sbuf>::add(unsigned char symbol) {
...
template <typename sbuf>
unsigned long lzp_buffer_kernel_1<sbuf>::predict_match(unsigned long &index) {
...
template <typename sbuf> size_t lzp_buffer_kernel_1<sbuf>::size() const {
  return buffer.size();
}
...
template <typename sbuf>
unsigned char lzp_buffer_kernel_1<sbuf>::operator[](unsigned long index) const {
  return buffer[index];
}
