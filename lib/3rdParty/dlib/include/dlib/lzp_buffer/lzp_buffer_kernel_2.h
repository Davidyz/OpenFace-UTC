...
template <typename sbuf> class lzp_buffer_kernel_2 {
  /*!
      REQUIREMENTS ON sbuf
          sbuf is an implementation of
  sliding_buffer/sliding_buffer_kernel_abstract.h T == unsigned char
...
public:
  explicit lzp_buffer_kernel_2(unsigned long buffer_size);

  virtual ~lzp_buffer_kernel_2();

  void clear();

  inline void add(unsigned char symbol);

  inline unsigned long predict_match(unsigned long &index);

  inline size_t size() const;

  inline unsigned char operator[](unsigned long index) const;

private:
  inline bool verify(unsigned long index) const
  /*!
      ensures
  ...
  operator=(const lzp_buffer_kernel_2<sbuf> &); // assignment operator
};
...
lzp_buffer_kernel_2<sbuf>::lzp_buffer_kernel_2(unsigned long buffer_size)
    : table3(0), table4(0), table_size(65536) {
...
template <typename sbuf> lzp_buffer_kernel_2<sbuf>::~lzp_buffer_kernel_2() {
  delete[] table3;
  delete[] table4;
}
...
template <typename sbuf> void lzp_buffer_kernel_2<sbuf>::clear() {
  for (unsigned long i = 0; i < buffer.size(); ++i)
    buffer[i] = 0;
...
template <typename sbuf>
void lzp_buffer_kernel_2<sbuf>::add(unsigned char symbol) {
...
template <typename sbuf>
unsigned long lzp_buffer_kernel_2<sbuf>::predict_match(unsigned long &index) {
...
template <typename sbuf> size_t lzp_buffer_kernel_2<sbuf>::size() const {
  return buffer.size();
}
...
template <typename sbuf>
unsigned char lzp_buffer_kernel_2<sbuf>::operator[](unsigned long index) const {
  return buffer[index];
}
