...
public:
  explicit lzp_buffer(unsigned long buffer_size);
  /*!
      requires
          - 10 < buffer_size < 32
      ensures
  ...
  unsigned long predict_match(unsigned long &index);
  /*!
      ensures
          ...
          - if (*this can generate a prediction) then
              - #index == the predicted location of a match in the history
  buffer. (i.e. (*this)[#index] is the first symbol of the predicted match)
