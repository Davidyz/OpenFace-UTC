...
template <typename T> struct is_graph : public default_is_kind_value {
  /*!
...
template <typename T> struct is_directed_graph : public default_is_kind_value {
  /*!
      - if (T is an implementation of
  directed_graph/directed_graph_kernel_abstract.h) then
...
struct is_matrix : public default_is_kind_value {
  /*!
      - if (T is some kind of matrix expression from the
  matrix/matrix_exp_abstract.h component) then
...
template <typename T> struct is_rand : public default_is_kind_value {
  /*!
...
template <typename T> struct is_config_reader : public default_is_kind_value {
  /*!
      - if (T is an implementation of
  config_reader/config_reader_kernel_abstract.h) then
...
template <typename T> struct is_std_vector<T &> {
  const static bool value = is_std_vector<T>::value;
};
template <typename T> struct is_std_vector<const T &> {
  const static bool value = is_std_vector<T>::value;
};
template <typename T> struct is_std_vector<const T> {
  const static bool value = is_std_vector<T>::value;
