...
void load_libsvm_formatted_data(const std::string &file_name,
  ...
  while (fin.peek() != EOF) {
    ...
    if (!sin)
      throw sample_data_io_error("On line: " + cast_to_string(line_num) +
...
typename enable_if<
    is_const_type<typename sample_type::value_type::first_type>>::type
fix_nonzero_indexing(std::vector<sample_type, alloc> &samples) {
  typedef typename sample_type::value_type pair_type;
  typedef typename basic_type<typename pair_type::first_type>::type key_type;
...
// If the "first" values in the std::pair objects are not const then we can
// modify them directly and that is what this version of fix_nonzero_indexing()
// does.
template <typename sample_type, typename alloc>
typename disable_if<
    is_const_type<typename sample_type::value_type::first_type>>::type
...
template <typename sample_type, typename label_type, typename alloc1,
          typename alloc2>
typename disable_if<is_matrix<sample_type>, void>::type
save_libsvm_formatted_data(const std::string &file_name,
  ...
  for (unsigned long i = 0; i < samples.size(); ++i) {
    ...
    for (typename sample_type::const_iterator j = samples[i].begin();
         j != samples[i].end(); ++j) {
      if (j->second != 0)
...
template <typename sample_type, typename label_type, typename alloc1,
          typename alloc2>
typename enable_if<is_matrix<sample_type>, void>::type
save_libsvm_formatted_data(const std::string &file_name,
                           const std::vector<sample_type, alloc1> &samples,
                           const std::vector<label_type, alloc2> &labels) {
  // make sure requires clause is not broken
