    ...
    for (unsigned long j = 0; j < data.images[i].boxes.size(); ++j) {
      if (source.should_load_box(data.images[i].boxes[j])) {
        if (data.images[i].boxes[j].ignore) {
          ...
          min_rect_size =
              std::min<double>(min_rect_size, rects.back().rect.area());
        }
        rects.back().label = data.images[i].boxes[j].label;
      }
    }

    if (!source.should_skip_empty_images() ||
        impl::num_non_ignored_boxes(rects) != 0) {
      load_image(img, data.images[i].filename);
      if (rects.size() != 0) {
        // if shrinking the image would still result in the smallest box being
        // bigger than the box area threshold then shrink the image.
...
// ******* THIS FUNCTION IS DEPRECATED, you should use another version of
// load_image_dataset() *******
template <typename image_type, typename MM>
std::vector<std::vector<rectangle>>
load_image_dataset(array<image_type, MM> &images,
                   std::vector<std::vector<rectangle>> &object_locations,
...
load_image_dataset(array_type &images,
  ...
  return load_image_dataset(images, object_locations,
                            image_dataset_file(filename));
}
...
std::vector<std::vector<rectangle>> load_image_dataset(
  ...
  // find out what parts are being used in the dataset.  Store results in
  // all_parts.
  for (unsigned long i = 0; i < data.images.size(); ++i) {
    for (unsigned long j = 0; j < data.images[i].boxes.size(); ++j) {
      if (source.should_load_box(data.images[i].boxes[j])) {
        const std::map<std::string, point> &parts =
            data.images[i].boxes[j].parts;
    ...
    for (unsigned long j = 0; j < data.images[i].boxes.size(); ++j) {
      if (source.should_load_box(data.images[i].boxes[j])) {
        if (data.images[i].boxes[j].ignore) {
          ...
          std::vector<point> partlist(parts_idx.size(),
...
std::vector<std::vector<rectangle>> load_image_dataset(
  ...
  std::vector<std::string> parts_list;
  return load_image_dataset(images, object_locations,
                            image_dataset_file(filename), parts_list);
}
