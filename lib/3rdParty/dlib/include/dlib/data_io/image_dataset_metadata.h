...
struct box {
  ...
  box()
      : difficult(false), truncated(false), occluded(false), ignore(false),
        pose(0), detection_score(0), angle(0), gender(UNKNOWN), age(0) {}

  box(const rectangle &rect_)
      : rect(rect_), difficult(false), truncated(false), occluded(false),
        ignore(false), pose(0), detection_score(0), angle(0), gender(UNKNOWN),
        age(0) {}
  ...
  // value of 0 would indicate that the object is in its "standard" upright
  // pose. Therefore, to make the object appear upright we would have to rotate
  // the image counter-clockwise by angle radians.
  double angle;
...
void save_image_dataset_metadata(const dataset &meta,
...
void load_image_dataset_metadata(dataset &meta, const std::string &filename);
/*!
    ensures
