...
template <typename T> struct range_set {
  int top_min;
  int top_max;
  int bottom_min;
  int bottom_max;
...
template <typename T> bool is_terminal_set(const range_set<T> &item) {
  return (item.top_min >= item.top_max && item.bottom_min >= item.bottom_max);
}

// ------------------------------------------------------------------------------------

template <typename T>
void split(const range_set<T> &rset, range_set<T> &a, range_set<T> &b) {
...
void find_best_column_range(const matrix_exp<EXP> &sum_pos,
                            const matrix_exp<EXP> &sum_neg,
                            const range_set<T> &row_range, T &weight, int &left,
                            int &right) {
  left = 0;
  right = -1;
  weight = 0;
  ...
  for (long c = 0; c < sum_pos.nc(); ++c) {
    ...
    T temp =
        sum_pos(row_range.bottom_max + 1, c) - sum_pos(row_range.top_min, c);
    if (row_range.top_max <= row_range.bottom_min)
      temp +=
          sum_neg(row_range.bottom_min + 1, c) - sum_neg(row_range.top_max, c);
...
std::vector<rectangle> max_sum_submatrix(const matrix_exp<EXP> &mat,
  ...
  /*
      This function is basically an implementation of the efficient subwindow
     search (I-ESS) algorithm presented in the following paper: Efficient
     Algorithms for Subwindow Search in Object Detection and Localization by
     Senjian An, Patrick Peursum, Wanquan Liu and Svetha Venkatesh In CVPR 2009
  ...
  */
  ...
  while (q.size() > 0) {
    if (is_terminal_set(q.top())) {
      ...
      rectangle rect(left, q.top().top_min, right, q.top().bottom_min);
      ...
      // We are going to blank out the weights we just used.  So adjust the sum
