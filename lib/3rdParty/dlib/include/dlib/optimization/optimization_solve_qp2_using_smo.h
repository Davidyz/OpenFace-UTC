...
template <typename T> typename T::type maximum_nu_impl(const T &y) {
  ...
  return static_cast<scalar_type>(
      2.0 * (scalar_type)std::min(pos_count, neg_count) / (scalar_type)y.nr());
}
...
template <typename T> typename T::value_type maximum_nu(const T &y) {
  return maximum_nu_impl(mat(y));
}
...
public:
  ...
  operator()(const matrix_exp<EXP1> &Q, const matrix_exp<EXP2> &y,
             ...
             scalar_type eps) {
    DLIB_ASSERT(
        Q.nr() == Q.nc() && y.size() == Q.nr() && y.size() > 1 &&
            is_col_vector(y) && sum((y == +1) + (y == -1)) == y.size() &&
            0 < nu && nu <= 1 && eps > 0,
  ...
  const column_matrix &get_gradient() const { return df; }

private:
  // -------------------------------------------------------------------------------------

  template <typename scalar_type, typename scalar_vector_type,
            typename scalar_vector_type2>
  inline void set_initial_alpha(const scalar_vector_type &y,
                                const scalar_type nu,
                                scalar_vector_type2 &alpha) const {
    ...
    if (count != num_total || has_slack == false) {
      ...
      sout << "Invalid nu of " << nu << ".  It is required that: 0 < nu < "
           << 2 * (scalar_type)count / y.nr();
  ...
  template <typename scalar_vector_type, typename scalar_type, typename EXP,
            typename U, typename V>
  inline bool
  find_working_group(const V &y, const U &alpha, const matrix_exp<EXP> &Q,
                     const scalar_vector_type &df, const scalar_type tau,
                     const scalar_type eps, long &i_out, long &j_out) const {
    using namespace std;
    long ip = 0;
    long jp = 0;
    long in = 0;
  ...
  template <typename EXP, typename T, typename U>
  inline void optimize_working_pair(T &alpha, const matrix_exp<EXP> &Q,
                                    const U &df, const scalar_type tau,
