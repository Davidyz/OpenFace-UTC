...
public:
  ...
  template <typename T> typename T::matrix_type operator()(const T &x) const {
...
const central_differences<funct> derivative(const funct &f) {
  return central_differences<funct>(f);
}
template <typename funct>
const central_differences<funct> derivative(const funct &f, double eps) {
  DLIB_ASSERT(eps > 0, "\tcentral_differences derivative(f,eps)"
...
struct clamped_function_object {
  clamped_function_object(const funct &f_, const matrix_exp<EXP1> &x_lower_,
                          const matrix_exp<EXP2> &x_upper_)
      : f(f_), x_lower(x_lower_), x_upper(x_upper_) {}

  template <typename T> double operator()(const T &x) const {
...
double find_min(search_strategy_type search_strategy,
  ...
  while (stop_strategy.should_continue_search(x, f_value, g) &&
    ...
    double alpha = line_search(
        make_line_search_function(f, x, s, f_value), f_value,
        ...
        min_f, search_strategy.get_max_line_search_iterations());
...
double find_max(search_strategy_type search_strategy,
  ...
  // This function is basically just a copy of find_min() but with - put in the
  // right places to flip things around so that it ends up looking for the max
  // rather than the min.
  ...
  while (stop_strategy.should_continue_search(x, f_value, g) &&
    ...
    double alpha = line_search(
        negate_function(make_line_search_function(f, x, s, f_value)), f_value,
        negate_function(make_line_search_function(der, x, s, g)),
        dot(g, s), // compute initial gradient for the line search
        search_strategy.get_wolfe_rho(), search_strategy.get_wolfe_sigma(),
        -max_f, search_strategy.get_max_line_search_iterations());
...
double find_min_using_approximate_derivatives(
  ...
  DLIB_CASSERT(is_col_vector(x) && derivative_eps > 0,
               "\tdouble find_min_using_approximate_derivatives()"
                   << "\n\tYou have to supply column vectors to this function"
                   << "\n\tx.nc():         " << x.nc()
  ...
  while (stop_strategy.should_continue_search(x, f_value, g) &&
    ...
    double alpha = line_search(
        ...
        dot(g, s), // Sometimes the following line is a better way of
                   // determining the initial gradient.
...
double find_max_using_approximate_derivatives(
  ...
  // Just negate the necessary things and call the find_min version of this
  // function.
  return -find_min_using_approximate_derivatives(search_strategy, stop_strategy,
                                                 negate_function(f), x, -max_f,
                                                 derivative_eps);
}
...
T zero_bounded_variables(const double eps, T vect, const T &x,
                         const T &gradient, const U &x_lower,
                         const V &x_upper) {
...
template <typename T, typename U, typename V>
T gap_step_assign_bounded_variables(const double eps, T vect, const T &x,
                                    const T &gradient, const U &x_lower,
                                    const V &x_upper) {
  for (long i = 0; i < gradient.size(); ++i) {
    const double tol = eps * std::abs(x(i));
    // If x(i) is an active bound constraint then we should set its search
    // direction such that a single step along the direction either does nothing
    // or closes the gap of size tol before hitting the bound exactly.
    if (x_lower(i) + tol >= x(i) && gradient(i) > 0)
...
double find_min_box_constrained(search_strategy_type search_strategy,
                                ...
                                const matrix_exp<EXP2> &x_upper) {
  /*
      The implementation of this function is more or less based on the
     discussion in the paper Projected Newton-type Methods in Machine Learning
  ...
  */
  ...
  DLIB_CASSERT(is_col_vector(x) && is_col_vector(x_lower) &&
               ...
               "\tdouble find_min_box_constrained()"
                   << "\n\t The inputs to this function must be equal length "
                      "column vectors."
  ...
  double last_alpha = 1;
  while (stop_strategy.should_continue_search(x, f_value, g)) {
    s = search_strategy.get_next_direction(
        x, f_value, zero_bounded_variables(gap_eps, g, x, g, x_lower, x_upper));
    s = gap_step_assign_bounded_variables(gap_eps, s, x, g, x_lower, x_upper);

    double alpha = backtracking_line_search(
        make_line_search_function(clamp_function(f, x_lower, x_upper), x, s,
                                  f_value),
        f_value, dot(g, s), // compute gradient for the line search
        last_alpha, search_strategy.get_wolfe_rho(),
        search_strategy.get_max_line_search_iterations());

    // Do a trust region style thing for alpha.  The idea is that if we take a
    // small step then we are likely to take another small step.  So we reuse
    // the alpha from the last iteration unless the line search didn't shrink
    // alpha at all, in that case, we start with a bigger alpha next time.
    if (alpha == last_alpha)
...
double find_min_box_constrained(search_strategy_type search_strategy,
  ...
  return find_min_box_constrained(
      search_strategy, stop_strategy, f, der, x,
...
double find_max_box_constrained(search_strategy_type search_strategy,
  ...
  // This function is basically just a copy of find_min_box_constrained() but
  // with - put in the right places to flip things around so that it ends up
  // looking for the max rather than the min.
  ...
  while (stop_strategy.should_continue_search(x, f_value, g)) {
    ...
    double alpha = backtracking_line_search(
        negate_function(make_line_search_function(
            clamp_function(f, x_lower, x_upper), x, s, f_value)),
        f_value, dot(g, s), // compute gradient for the line search
        last_alpha, search_strategy.get_wolfe_rho(),
        search_strategy.get_max_line_search_iterations());
    ...
    // Don't forget to negate the output from the line search since it is  from
    // the unnegated version of f()
    f_value *= -1;
...
double find_max_box_constrained(search_strategy_type search_strategy,
  ...
  typedef typename T::type scalar_type;
  return find_max_box_constrained(
      search_strategy, stop_strategy, f, der, x,
      uniform_matrix<scalar_type>(x.size(), 1, x_lower),
      uniform_matrix<scalar_type>(x.size(), 1, x_upper));
}
