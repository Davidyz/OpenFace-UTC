...
private:
  ...
  template <typename U> double get_value(const U &r) const {
...
make_line_search_function(const funct &f, const T &start, const T &direction) {
  ...
  DLIB_ASSERT(is_col_vector(start) && is_col_vector(direction) &&
...
inline double poly_min_extrap(double f0, double d0, double x1, double f_x1,
  ...
  DLIB_ASSERT(0 < x1 && x1 < x2, "Invalid inputs were given to this function.\n"
                                     << "x1: " << x1 << "    x2: " << x2);
  // The contents of this function follow the equations described on page 58 of
  // the book Numerical Optimization by Nocedal and Wright, second edition.
  matrix<double, 2, 2> m;
  matrix<double, 2, 1> v;
...
inline double lagrange_poly_min_extrap(double p1, double p2, double p3,
  ...
  DLIB_ASSERT(p1 < p2 && p2 < p3 && f1 >= f2 && f2 <= f3,
              "   p1: " << p1 << "   p2: " << p2 << "   p3: " << p3 << "   f1: "
                        << f1 << "   f2: " << f2 << "   f3: " << f3);

  // This formula is out of the book Nonlinear Optimization by Andrzej
  // Ruszczynski.  See section 5.2.
  double temp1 = f1 * (p3 * p3 - p2 * p2) + f2 * (p1 * p1 - p3 * p3) +
                 f3 * (p2 * p2 - p1 * p1);
...
double line_search(const funct &f, const double f0, const funct_der &der,
  ...
  DLIB_ASSERT(0 < rho && rho < sigma && sigma < 1 && max_iter > 0,
              "\tdouble line_search()"
  ...
  // block 2.6.2 from the book Practical Methods of Optimization by R. Fletcher.
  // The sectioning phase is an implementation of 2.6.4 from the same book.
  ...
  while (true) {
    ...
    if (last_alpha < alpha)
      ...
      alpha =
          alpha + (last_alpha - alpha) * poly_min_extrap(val, val_der, last_val,
                                                         last_val_der, 1e10);
  ...
  while (true) {
    ...
    if (std::abs(max_possible_alpha * d0) <=
        std::abs(f0) * std::numeric_limits<double>::epsilon())
      return alpha;
...
double backtracking_line_search(const funct &f, double f0, double d0,
                                ...
                                unsigned long max_iter) {
  DLIB_ASSERT(0 < rho && rho < 1 && max_iter > 0,
              "\tdouble backtracking_line_search()"
                  << "\n\tYou have given invalid arguments to this function"
                  << "\n\t rho:      " << rho << "\n\t max_iter: " << max_iter);

  // make sure alpha is going in the right direction.  That is, it should be
...
public:
  optimize_single_variable_failure(const std::string &s) : error(s) {}
};
...
find_min_single_variable(const funct &f, double &starting_point,
  ...
  DLIB_CASSERT(
      eps > 0 && max_iter > 1 && begin <= starting_point &&
          starting_point <= end && initial_search_radius > 0,
      "eps: " << eps << "\n max_iter: " << max_iter << "\n begin: " << begin
  ...
  // find three bracketing points such that f1 > f2 < f3.   Do this by
  // generating a sequence of points expanding away from 0.   Also note that, in
  // the following code, it is always the case that p1 < p2 < p3.

  // The first thing we do is get a starting set of 3 points that are inside the
  // [begin,end] bounds
  ...
  // Now we have 3 points on the function.  Start looking for a bracketing set
  // such that f1 > f2 < f3 is the case.
  while (!(f1 > f2 && f2 < f3)) {
    // check for hitting max_iter or if the interval is now too small
    if (f_evals >= max_iter) {
      throw optimize_single_variable_failure(
          "The max number of iterations of single variable optimization have "
          "been reached\n"
          "without converging.");
    }
    if (p3 - p1 < eps) {
    ...
    // If the left most points are identical in function value then expand out
    // the left a bit, unless it's already at bound or we would drop that left
    // most point anyway because it's bad.
    ...
    if (f1 <= f3) {
      // check if the minimum is butting up against the bounds and if so then
      // pick a point between p1 and p2 in the hopes that shrinking the interval
      // will be a good thing to do.  Or if p1 and p2 aren't differentiated then
    ...
    // otherwise f3 is small and we should take a step to the right
      ...
      // check if the minimum is butting up against the bounds and if so then
      // pick a point between p2 and p3 in the hopes that shrinking the interval
      // will be a good thing to do.  Or if p2 and p3 aren't differentiated then
      // try and get them to obtain different values.
  ...
  // Within this loop we maintain the invariant that: f1 > f2 < f3 and p1 < p2 <
  // p3
  ...
  while (f_evals < max_iter && p3 - p1 > eps) {
    ...
    // Remove one of the endpoints of our bracket depending on where the new
    // point falls.
    if (p_min < p2) {
...
template <typename funct> class negate_function_object {
public:
  negate_function_object(const funct &f_) : f(f_) {}

  template <typename T> double operator()(const T &x) const { return -f(x); }
...
const negate_function_object<funct> negate_function(const funct &f) {
  return negate_function_object<funct>(f);
}
...
template <typename funct>
double
find_max_single_variable(const funct &f, double &starting_point,
                         const double begin = -1e200, const double end = 1e200,
                         const double eps = 1e-3, const long max_iter = 100,
                         const double initial_search_radius = 1) {
  return -find_min_single_variable(negate_function(f), starting_point, begin,
                                   end, eps, max_iter, initial_search_radius);
}
