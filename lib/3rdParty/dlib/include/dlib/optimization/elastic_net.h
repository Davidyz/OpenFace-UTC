...
public:
  ...
  explicit elastic_net(const matrix_exp<EXP> &XX)
      : eps(1e-5), max_iterations(50000), verbose(false) {
    // make sure requires clause is not broken
    DLIB_ASSERT(XX.size() > 0 && XX.nr() == XX.nc(),
                "\t elastic_net::elastic_net(XX)"
                    << " \n\t XX must be a non-empty square matrix."
                    << " \n\t XX.nr():   " << XX.nr() << " \n\t XX.nc():   "
                    << XX.nc() << " \n\t this: " << this);

    // If the number of columns in X is big and in particular bigger than the
    // number of rows then we can get rid of them by doing some SVD magic. Doing
    // this doesn't make the final results of anything change but makes all the
    // matrices have dimensions that are X.nr() in size, which can be much
    // smaller.
    matrix<double, 0, 1> s;
  ...
  elastic_net(const matrix_exp<EXP1> &XX, const matrix_exp<EXP2> &XY)
      : elastic_net(XX) {
    // make sure requires clause is not broken
    DLIB_ASSERT(XX.size() > 0 && XX.nr() == XX.nc() && is_col_vector(XY) &&
                    XX.nc() == XY.size(),
  ...
  long size() const { return u.nr(); }

  template <typename EXP> void set_xy(const matrix_exp<EXP> &XY) {
    // make sure requires clause is not broken
    DLIB_ASSERT(is_col_vector(XY) && XY.size() == size(),
    ...
    // appears in the algorithm is in terms of dot products with w and x
    // vectors. But those vectors are always in the span of X and therefore we
    // only see the part of the norm of Y that is in the span of X (and hence u
    // since u and X have the same span by construction)
  ...
  bool have_target_values() const { return Y.size() != 0; }

  void set_epsilon(double eps_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(eps_ > 0, "\t void elastic_net::set_epsilon()"
  ...
  operator()(double ridge_lambda,
    ...
    // make sure requires clause is not broken
    DLIB_ASSERT(have_target_values() && ridge_lambda > 0 && lasso_budget > 0,
    ...
    // First check if lasso_budget is so big that it isn't even active.  We do
    // this by doing just ridge regression and checking the result.
    matrix<double, 0, 1> betas =
        eig_vects * tmp(inv(diagm(eig_vals + ridge_lambda)) * eig_vects_xdoty);
    if (sum(abs(betas)) <= lasso_budget)
      return betas;

    // Set w back to 0.  We will compute the w corresponding to what is
    // currently in alpha layer on.  This way w and alpha are always in sync.
    w = 0;
    wy_mult = 0;
    wdoty = 0;
    ...
    auto dot = [&](const matrix<double, 0, 1> &w, const en_sample2 &x) {
      ...
      // Do the base dot product but don't forget to add in the -(1/t)*y part
      // from the svm reduction paper
      double val = rowm(X, x.r) * w + xmul * wdoty + wy_mult * xdoty(x.r) +
                   xmul * wy_mult * ynorm;
    ...
    // perform w += scale*x;
    auto add_to = [&](matrix<double, 0, 1> &w, double scale,
                      const en_sample2 &x) {
      const double xmul = -x.label * (1 / lasso_budget);
      wy_mult += scale * xmul;
    ...
    for (size_t i = 0; i < samples.size(); ++i) {
      ...
      for (long c = 0; c < X.nc(); ++c)
        x.xdotx +=
            std::pow(X(x.r, c) + xmul * Y(c), 2.0) - std::pow(xmul * Y(c), 2.0);
...
private:
  ...
  double wdoty;
  double
      wy_mult; // logically, the real w is what is in the w vector + wy_mult*Y
  matrix<double, 0, 1> w;
