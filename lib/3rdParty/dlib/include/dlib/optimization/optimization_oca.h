...
namespace dlib {
template <typename matrix_type> class oca_problem {
public:
  typedef typename matrix_type::type scalar_type;

  virtual ~oca_problem() {}

  virtual bool risk_has_lower_bound(scalar_type &) const { return false; }

  virtual bool optimization_status(scalar_type, scalar_type, scalar_type,
                                   scalar_type, unsigned long,
                                   unsigned long) const = 0;

  virtual scalar_type get_c() const = 0;

  virtual long get_num_dimensions() const = 0;

  virtual void get_risk(matrix_type &current_solution, scalar_type &risk_value,
                        matrix_type &risk_subgradient) const = 0;
};
...
public:
  ...
  void set_subproblem_epsilon(double eps_) { sub_eps = eps_; }

  double get_subproblem_epsilon() const { return sub_eps; }

  void set_subproblem_max_iterations(unsigned long sub_max_iter_) {
  ...
  unsigned long get_subproblem_max_iterations() const { return sub_max_iter; }

  void set_inactive_plane_threshold(unsigned long inactive_thresh_) {
    // make sure requires clause is not broken
  ...
  unsigned long get_inactive_plane_threshold() const { return inactive_thresh; }

  template <typename matrix_type>
  typename matrix_type::type
  operator()(const oca_problem<matrix_type> &problem, matrix_type &w,
             unsigned long num_nonnegative = 0,
             unsigned long force_weight_to_1 =
                 std::numeric_limits<unsigned long>::max()) const {
    matrix_type empty_prior;
    return oca_impl(problem, w, empty_prior, false, num_nonnegative,
                    force_weight_to_1, 0);
  }

  template <typename matrix_type>
  typename matrix_type::type
  solve_with_elastic_net(const oca_problem<matrix_type> &problem,
                         matrix_type &w, double lasso_lambda,
                         unsigned long force_weight_to_1 =
                             std::numeric_limits<unsigned long>::max()) const {
    matrix_type empty_prior;
    return oca_impl(problem, w, empty_prior, false, 0, force_weight_to_1,
                    lasso_lambda);
  }

  template <typename matrix_type>
  typename matrix_type::type operator()(const oca_problem<matrix_type> &problem,
                                        matrix_type &w,
                                        const matrix_type &prior) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(
        is_col_vector(prior) && prior.size() == problem.get_num_dimensions(),
...
private:
  template <typename matrix_type>
  typename matrix_type::type
  oca_impl(const oca_problem<matrix_type> &problem, matrix_type &w,
           const matrix_type &prior, bool have_prior,
           unsigned long num_nonnegative, unsigned long force_weight_to_1,
           const double lasso_lambda) const {
    const unsigned long num_dims = problem.get_num_dimensions();

    // make sure requires clause is not broken
    DLIB_ASSERT(problem.get_c() > 0 && problem.get_num_dimensions() > 0 &&
                ...
                "\t scalar_type oca::operator()"
                    ...
                    << "\n\t this: " << this);
    if (have_prior) {
      DLIB_ASSERT(lasso_lambda == 0,
                  "Solver doesn't support using a prior with lasso.");
      DLIB_ASSERT(num_nonnegative == 0, "Solver doesn't support using a prior "
                                        "with non-negative constraints.");
    } else if (lasso_lambda != 0) {
      DLIB_ASSERT(
          num_nonnegative == 0,
          "Solver doesn't support using lasso with non-negative constraints.");
    }
    ...
    while (true) {
      ...
      // If alpha is empty then initialize it (we must always have sum(alpha) ==
      // C). But otherwise, just append a zero.
      if (alpha.size() == 0)
        alpha = uniform_matrix<scalar_type>(1, 1, C);
      else
        alpha = join_cols(alpha, zeros_matrix<scalar_type>(1, 1));

      const scalar_type wnorm =
          0.5 * ridge_lambda * trans(w) * w + lasso_lambda * sum(abs(w));
      const double prior_part = have_prior ? dot(w, prior) : 0;
      cur_obj = wnorm + C * cur_risk + prior_norm - prior_part;

      // report current status
      const scalar_type risk_gap =
          cur_risk - (cp_obj - wnorm + prior_part - prior_norm) / C;
      if (counter > 0 &&
          problem.optimization_status(cur_obj, cur_obj - cp_obj, cur_risk,
                                      risk_gap, planes.size(), counter)) {
        break;
      }
      ...
      // accuracy that is related to how big the error gap is.  Also, we
      // multiply by ridge_lambda because the objective function for the QP we
      // solve was implicitly scaled by ridge_lambda.  That is, we want to ask
      // the QP solver to solve the problem until the duality gap is 0.1 times
      // smaller than what it is now.  So the factor of ridge_lambda is
      // necessary to make this happen.
      scalar_type eps = std::min<scalar_type>(sub_eps, 0.1 * ridge_lambda *
                                                           (cur_obj - cp_obj));
      // just a sanity check
      if (eps < 1e-16)
        eps = 1e-16;
      // Note that we warm start this optimization by using the alpha from the
      // last iteration as the starting point.
      if (lasso_lambda != 0) {
        // copy planes into a matrix so we can call solve_qp4_using_smo()
        matrix<scalar_type, 0, 0, mem_manager_type, layout_type> planes_mat(
            num_dims, planes.size());
        for (unsigned long i = 0; i < planes.size(); ++i)
          set_colm(planes_mat, i) = planes[i];

        btemp = ridge_lambda * mat(bs) - trans(planes_mat) * d;
        solve_qp4_using_smo(planes_mat, K, btemp, d, alpha, lambda, eps,
                            sub_max_iter, (scalar_type)(2 * lasso_lambda));
      } else if (num_nonnegative != 0) {
        // copy planes into a matrix so we can call solve_qp4_using_smo()
        matrix<scalar_type, 0, 0, mem_manager_type, layout_type> planes_mat(
            num_nonnegative, planes.size());
        for (unsigned long i = 0; i < planes.size(); ++i)
          set_colm(planes_mat, i) = colm(planes[i], 0, num_nonnegative);

        solve_qp4_using_smo(planes_mat, K, mat(bs), d, alpha, lambda, eps,
                            sub_max_iter);
      } else {
      ...
      else if (num_nonnegative != 0) // threshold the first num_nonnegative w
                                     // elements if necessary.
        set_rowm(w, range(0, num_nonnegative - 1)) =
            lowerbound(rowm(w, range(0, num_nonnegative - 1)), 0);
      ...
      // Compute the lower bound on the true objective given to us by the
      // cutting plane subproblem.
      cp_obj = -0.5 * ridge_lambda * trans(w) * w + trans(alpha) * mat(bs);
      if (have_prior)
        w += prior;

      // If it has been a while since a cutting plane was an active constraint
