...
struct viterbi_data {
  viterbi_data()
      : val(-std::numeric_limits<double>::infinity()), back_index(0) {}
  double val;
  unsigned long back_index;
};

template <long NC>
inline bool advance_state(matrix<unsigned long, 1, NC> &node_states,
...
void find_max_factor_graph_viterbi(const map_problem &prob,
  ...
  DLIB_ASSERT(prob.num_states() > 0,
              "\t void find_max_factor_graph_viterbi()"
                  << "\n\t The nodes in a factor graph have to be able to take "
                     "on more than 0 states.");
  DLIB_ASSERT(
      std::pow(num_states, (double)order) <
  ...
  const unsigned long trellis_size =
      static_cast<unsigned long>(std::pow(num_states, (double)order));
    ...
    if (node < order) {
      ...
      /*
          WHAT'S THE DEAL WITH THIS PREPROCESSOR MACRO?
              ...
              time constant then this function runs significantly faster.  So
         this macro is here to let us do that.  It just lets us avoid
         replicating this code block in the following if statements for
...
#define DLIB_FMFGV_WORK                                                        \
  ...
  for (unsigned long i = 0; i < trellis_size; ++i) {                           \
    ...
    for (unsigned long s = 0; s < num_states; ++s) {                           \
      const double temp = prob.factor_value(node, node_states) +               \
                          trellis[node - 1][count % trellis_size].val;         \
      if (temp > best_score) {                                                 \
        best_score = temp;                                                     \
    ...
    trellis[node][i].back_index = back_index;                                  \
      ...
      // The general case, here we don't define the size of node_states at
      // compile time.
