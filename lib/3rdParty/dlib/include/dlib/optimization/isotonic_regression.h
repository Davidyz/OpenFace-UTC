...
public:
  ...
  void fit_with_linear_output_interpolation(const_iterator begin,
    ...
    // output we linearly interpolate.  Note that this actually fits the data
    // less than the step-function, but in many applications might be closer to
    // what you really when when using isotonic_regression than the step
    // function.
...
private:
  ...
  void do_isotonic_regression(const_iterator begin, const_iterator end) {
    blocks.clear();
  ...
  template <typename T> static double get_val(const T &v) { return v; }

  template <typename T, typename U>
  static double get_val(const std::pair<T, U> &v) {
    return v.second;
  }

  template <typename T> static void set_val(T &v, double val) { v = val; }

  template <typename T, typename U>
  static void set_val(std::pair<T, U> &v, double val) {
    v.second = val;
  }
