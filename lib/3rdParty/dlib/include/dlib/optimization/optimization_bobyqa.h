...
public:
  bobyqa_failure(const std::string &s) : error(s) {}
};
...
public:
  template <typename funct, typename T, typename U>
  double find_min(const funct &f, T &x, long npt, const U &xl_, const U &xu_,
                  const double rhobeg, const double rhoend,
                  const long max_f_evals) const {
    ...
    return bobyqa_(f, x.size(), npt, &x(0), &xl(0), &xu(0), rhobeg, rhoend,
                   max_f_evals, w.get());
  }

private:
  template <typename funct>
  doublereal bobyqa_(const funct &calfun, const integer n, const integer npt,
                     doublereal *x, const doublereal *xl, const doublereal *xu,
                     const doublereal rhobeg, const doublereal rhoend,
                     const integer maxfun, doublereal *w) const {
    ...
    /* Local variables */
    integer j, id_, np, iw, igo, ihq, ixb, ixa, ifv, isl, jsl, ipq, ivl, ixn,
     ...
     */
    /*       error return occurs if any of the differences XU(I)-XL(I),
     * I=1,...,N, */
    ...
    /*     partitions of W, in order to provide useful and exact information
     * about */
    ...
    return bobyqb_(calfun, n, npt, &x[1], &xl[1], &xu[1], rhobeg, rhoend,
                   maxfun, &w[ixb], &w[ixp], &w[ifv], &w[ixo], &w[igo], &w[ihq],
                   &w[ipq], &w[ibmat], &w[izmat], ndim, &w[isl], &w[isu],
                   &w[ixn], &w[ixa], &w[id_], &w[ivl], &w[iw]);
    // L40:
    ;
  } /* bobyqa_ */
  ...
  template <typename funct>
  doublereal bobyqb_(const funct &calfun, const integer n, const integer npt,
                     doublereal *x, const doublereal *xl, const doublereal *xu,
                     const doublereal rhobeg, const doublereal rhoend,
                     const integer maxfun, doublereal *xbase, doublereal *xpt,
                     doublereal *fval, doublereal *xopt, doublereal *gopt,
                     doublereal *hq, doublereal *pq, doublereal *bmat,
                     doublereal *zmat, const integer ndim, doublereal *sl,
                     doublereal *su, doublereal *xnew, doublereal *xalt,
                     doublereal *d__, doublereal *vlag, doublereal *w) const {
    /* System generated locals */
    ...
    doublereal dx;
    integer np;
    doublereal den = 0, one = 0, ten = 0, dsq = 0, rho = 0, sum = 0, two = 0,
               diff = 0, half = 0, beta = 0, gisq = 0;
    integer knew = 0;
    doublereal temp, suma, sumb, bsum, fopt;
    integer kopt = 0, nptm;
    doublereal zero, curv;
    integer ksav;
    doublereal gqsq = 0, dist = 0, sumw = 0, sumz = 0, diffa = 0, diffb = 0,
               diffc = 0, hdiag = 0;
    integer kbase;
    doublereal alpha = 0, delta = 0, adelt = 0, denom = 0, fsave = 0, bdtol = 0,
               delsq = 0;
    integer nresc, nfsav;
    ...
    /*     XOPT is set to the displacement from XBASE of the trust region
     * centre. */
     ...
     * the */
    /*       vector of variables for the next call of CALFUN. XNEW also
     * satisfies */
    /*       the SL and SU constraints in the way that has just been mentioned.
     */
    /*     XALT is an alternative to XNEW, chosen by ALTMOV, that may replace
     * XNEW */
     ...
     */
    /*     W is a one-dimensional array that is used for working space. Its
     * length */
    ...
    prelim_(calfun, n, npt, &x[1], &xl[1], &xu[1], rhobeg, maxfun, &xbase[1],
            &xpt[xpt_offset], &fval[1], &gopt[1], &hq[1], &pq[1],
    ...
    fsave = fval[1];
    if (nf < npt) {
      throw bobyqa_failure("Return from BOBYQA because the objective function "
                           "has been called max_f_evals times.");
      // goto L720;
    }
    kbase = 1;
  ...
  L20:
     ...
     */
    /*     The integer NTRITS is set to the number "trust region" iterations
     * that */
  ...
  L60:
    trsbox_(n, npt, &xpt[xpt_offset], &xopt[1], &gopt[1], &hq[1], &pq[1],
            &sl[1], &su[1], delta, &xnew[1], &d__[1], &w[1], &w[np], &w[np + n],
    ...
    if (dnorm < half * rho) {
       ...
       * or */
      /*     not our work with the current RHO seems to be complete. Either RHO
       * is */
      /*     decreased or termination occurs if the errors in the quadratic
       * model at */
      ...
      for (k = 1; k <= i__1; ++k) {
        ...
        for (i__ = 1; i__ <= i__2; ++i__) {
          ...
          for (j = 1; j <= i__3; ++j) {
            /* L110: */
            bmat[ip + j * bmat_dim1] =
                bmat[ip + j * bmat_dim1] + w[i__] * vlag[j] + vlag[i__] * w[j];
          }
        }
      }
  ...
  L190:
    nfsav = nf;
    kbase = kopt;
    rescue_(calfun, n, npt, &xl[1], &xu[1], maxfun, &xbase[1], &xpt[xpt_offset],
  ...
  L210:
    altmov_(n, npt, &xpt[xpt_offset], &xopt[1], &bmat[bmat_offset],
            &zmat[zmat_offset], ndim, &sl[1], &su[1], kopt, knew, adelt,
            &xnew[1], &xalt[1], alpha, cauchy, &w[1], &w[np], &w[ndim + 1]);
    i__1 = n;
    for (i__ = 1; i__ <= i__1; ++i__) {
      /* L220: */
  ...
  L230:
    ...
    if (ntrits == 0) {
      ...
      if (denom <= half * (d__1 * d__1)) {
        ...
        throw bobyqa_failure("Return from BOBYQA because of much cancellation "
                             "in a denominator.");
        // goto L720;
      }
  ...
  L360:
    ...
    if (ntrits > 0) {
      if (vquad >= zero) {
        throw bobyqa_failure("Return from BOBYQA because a trust region step "
                             "has failed to reduce Q.");
        // goto L720;
      }
      ratio = (f - fopt) / vquad;
      if (ratio <= tenth) {
    ...
    if (ntrits > 0) {
      ...
      for (i__ = 1; i__ <= i__1; ++i__) {
        ...
        for (k = 1; k <= i__2; ++k) {
          /* L620: */
          sum = sum + bmat[k + i__ * bmat_dim1] * vlag[k] +
                xpt[k + i__ * xpt_dim1] * w[k];
        }
        if (xopt[i__] == sl[i__]) {
          /* Computing MIN */
  ...
  void altmov_(const integer n, const integer npt, const doublereal *xpt,
               const doublereal *xopt, const doublereal *bmat,
               const doublereal *zmat, const integer ndim, const doublereal *sl,
               const doublereal *su, const integer kopt, const integer knew,
               const doublereal adelt, doublereal *xnew, doublereal *xalt,
               doublereal &alpha, doublereal &cauchy, doublereal *glag,
               doublereal *hcol, doublereal *w) const {
    /* System generated locals */
    integer xpt_dim1, xpt_offset, bmat_dim1, bmat_offset, zmat_dim1,
    ...
    integer iflag;
    doublereal scale = 0, csave = 0, tempa = 0, tempb = 0, tempd = 0,
               const__ = 0, sumin = 0, ggfree = 0;
    integer ibdsav = 0;
    doublereal dderiv = 0, bigstp = 0, predsq = 0, presav = 0, distsq = 0,
    ...
    /*     XNEW will be set to a suitable new position for the interpolation
     * point */
    ...
    /*     XALT also provides a large value of the modulus of the KNEW-th
     * Lagrange */
    ...
    /*     HCOL is a working space vector of length NPT for the second
     * derivative */
    /*       coefficients of the KNEW-th Lagrange function. */
    /*     W is a working space vector of length 2N that is going to hold the */
    /*       constrained Cauchy step from XOPT of the Lagrange function,
     * followed */
  ...
  template <typename funct>
  void prelim_(const funct &calfun, const integer n, const integer npt,
               doublereal *x, const doublereal *xl, const doublereal *xu,
               const doublereal rhobeg, const integer maxfun, doublereal *xbase,
               doublereal *xpt, doublereal *fval, doublereal *gopt,
               doublereal *hq, doublereal *pq, doublereal *bmat,
               doublereal *zmat, const integer ndim, const doublereal *sl,
               const doublereal *su, integer &nf, integer &kopt) const {
    /* System generated locals */
    integer xpt_dim1, xpt_offset, bmat_dim1, bmat_offset, zmat_dim1,
        zmat_offset, i__1, i__2;
    doublereal d__1, d__2, d__3, d__4;
    ...
    doublereal two = 0, fbeg = 0, diff = 0, half = 0, temp = 0, zero = 0,
               recip = 0, stepa = 0, stepb = 0;
    integer itemp;
    doublereal rhosq;
    ...
    /*     GOPT is usually the gradient of the quadratic model at XOPT+XBASE,
     * but */
    /*       it is set by PRELIM to the gradient of the quadratic model at
     * XBASE. */
    ...
    /*     SUBROUTINE PRELIM sets the elements of XBASE, XPT, FVAL, GOPT, HQ,
     * PQ, */
  ...
  template <typename funct>
  void rescue_(const funct &calfun, const integer n, const integer npt,
               const doublereal *xl, const doublereal *xu, const integer maxfun,
               doublereal *xbase, doublereal *xpt, doublereal *fval,
               doublereal *xopt, doublereal *gopt, doublereal *hq,
               doublereal *pq, doublereal *bmat, doublereal *zmat,
               const integer ndim, doublereal *sl, doublereal *su, integer &nf,
               const doublereal delta, integer &kopt, doublereal *vlag,
               doublereal *ptsaux, doublereal *ptsid, doublereal *w) const {
    /* System generated locals */
    integer xpt_dim1, xpt_offset, bmat_dim1, bmat_offset, zmat_dim1,
        zmat_offset, i__1, i__2, i__3;
    doublereal d__1, d__2, d__3, d__4;
    ...
    integer nptm;
    doublereal zero = 0, hdiag = 0, fbase = 0, sfrac = 0, denom = 0, vquad = 0,
               sumpq = 0;
    ...
    for (j = 1; j <= i__1; ++j) {
      ...
      if ((d__2 = ptsaux[(j << 1) + 2], std::abs(d__2)) <
          half * (d__1 = ptsaux[(j << 1) + 1], std::abs(d__1))) {
    ...
    for (j = 1; j <= i__2; ++j) {
      ...
      if (jpn <= npt) {
        ...
        bmat[j * bmat_dim1 + 1] =
            -bmat[jp + j * bmat_dim1] - bmat[jpn + j * bmat_dim1];
        zmat[j * zmat_dim1 + 1] =
            std::sqrt(2.) / (d__1 = ptsaux[(j << 1) + 1] * ptsaux[(j << 1) + 2],
                             std::abs(d__1));
        zmat[jp + j * zmat_dim1] =
            zmat[j * zmat_dim1 + 1] * ptsaux[(j << 1) + 2] * temp;
        zmat[jpn + j * zmat_dim1] =
            -zmat[j * zmat_dim1 + 1] * ptsaux[(j << 1) + 1] * temp;
      } else {
    ...
    if (npt >= n + np) {
      ...
      for (k = np << 1; k <= i__2; ++k) {
        ...
        ptsid[k] = (doublereal)ip + (doublereal)iq / (doublereal)np + sfrac;
  ...
  L80:
    ...
    if (knew != kopt) {
       ...
       * reinstated. */
      /*     The nonnegative values of W(NDIM+K) are required in the search
       * below. */
  ...
  L120:
    ...
    for (k = 1; k <= i__2; ++k) {
      ...
      if (k == kopt) {
        ...
        iq = (integer)((doublereal)np * ptsid[k] - (doublereal)(ip * np));
        if (iq > 0) {
          iw = 1;
          if (ip == 0) {
            iw = 2;
     ...
     * yet */
    /*     in XPT. Also the final BMAT and ZMAT matrices are complete, but,
     * apart */
    /*     from the shift of XBASE, the updating of the quadratic model remains
     * to */
    /*     be done. The following cycle through the new interpolation points
     * begins */
    /*     by putting the new point in XPT(KPT,.) and by setting PQ(KPT) to
     * zero, */
  ...
  void trsbox_(const integer n, const integer npt, const doublereal *xpt,
               const doublereal *xopt, const doublereal *gopt,
               const doublereal *hq, const doublereal *pq, const doublereal *sl,
               const doublereal *su, const doublereal delta, doublereal *xnew,
               doublereal *d__, doublereal *gnew, doublereal *xbdi,
               doublereal *s, doublereal *hs, doublereal *hred, doublereal *dsq,
               doublereal *crvmin) const {
    /* System generated locals */
    ...
    integer isav;
    doublereal temp = 0, zero = 0, xsav = 0, xsum = 0, angbd = 0, dredg = 0,
               sredg = 0;
    integer iterc;
    doublereal resid = 0, delsq = 0, ggsav = 0, tempa = 0, tempb = 0,
               redmax = 0, dredsq = 0, redsav = 0, onemin = 0, gredsq = 0,
               rednew = 0;
    integer itcsav = 0;
    doublereal rdprev = 0, rdnext = 0, stplen = 0, stepsq = 0;
    integer itermax = 0;

    /*     The arguments N, NPT, XPT, XOPT, GOPT, HQ, PQ, SL and SU have the
     * same */
     ...
     * an */
    /*       initial value of zero. Thus XNEW is XOPT+D after the final
     * iteration. */
    /*     GNEW holds the gradient of the quadratic model at XOPT+D. It is
     * updated */
    /*       when D is updated. */
    /*     XBDI is a working space vector. For I=1,2,...,N, the element XBDI(I)
     * is */
    /*       set to -1.0, 0.0, or 1.0, the value being nonzero if and only if
     * the */
     ...
     * the */
    /*       current search direction, and the changes in the gradient of Q
     * along S */
    /*       and the reduced D, respectively, where the reduced D is the same as
     * D, */
    /*       except that the components of the fixed variables are zero. */
    /*     DSQ will be set to the square of the length of XNEW-XOPT. */
    /*     CRVMIN is set to zero if D reaches the trust region boundary.
     * Otherwise */
     ...
     */
    /*     search is restricted by a constraint, then the procedure is
     * restarted, */
    /*     the values of the variables that are at their bounds being fixed. If
     */
    /*     the trust region boundary is reached, then further changes may be
     * made */
    /*     to D, each one being in the two dimensional space that is spanned */
    /*     by the current D and the gradient of Q at XOPT+D, staying on the
     * trust */
  ...
  void update_(const integer n, const integer npt, doublereal *bmat,
               doublereal *zmat, const integer ndim, doublereal *vlag,
               const doublereal beta, const doublereal denom,
               const integer knew, doublereal *w) const {
    /* System generated locals */
     ...
     */
    /*     at most ZTEST. The first NDIM elements of W are used for working
     * space. */
    ...
    for (j = 2; j <= i__2; ++j) {
      if ((d__1 = zmat[knew + j * zmat_dim1], std::abs(d__1)) > ztest) {
        ...
        for (i__ = 1; i__ <= i__1; ++i__) {
          temp =
              tempa * zmat[i__ + zmat_dim1] + tempb * zmat[i__ + j * zmat_dim1];
          zmat[i__ + j * zmat_dim1] =
              tempa * zmat[i__ + j * zmat_dim1] - tempb * zmat[i__ + zmat_dim1];
          /* L20: */
...
template <typename funct, typename T, typename U>
double find_min_bobyqa(const funct &f, T &x, long npt, const U &x_lower,
                       const U &x_upper, const double rho_begin,
                       const double rho_end, const long max_f_evals) {
  // The starting point (i.e. x) must be a column vector.
  COMPILE_TIME_ASSERT(T::NC <= 1);

  // check the requirements.  Also split the assert up so that the error message
  // isn't huge.
  DLIB_CASSERT(is_col_vector(x) && is_col_vector(x_lower) &&
                   is_col_vector(x_upper) && x.size() == x_lower.size() &&
                   x_lower.size() == x_upper.size() && x.size() > 1 &&
                   max_f_evals > 1,
  ...
  DLIB_CASSERT(
      x.size() + 2 <= npt && npt <= (x.size() + 1) * (x.size() + 2) / 2 &&
          0 < rho_end && rho_end < rho_begin &&
          min(x_upper - x_lower) > 2 * rho_begin && min(x - x_lower) >= 0 &&
          min(x_upper - x) >= 0,
      "\tdouble find_min_bobyqa()"
          << "\n\t Invalid arguments have been given to this function"
          << "\n\t ntp in valid range: "
  ...
  return impl.find_min(f, x, npt, x_lower, x_upper, rho_begin, rho_end,
                       max_f_evals);
}
...
template <typename funct, typename T, typename U>
double find_max_bobyqa(const funct &f, T &x, long npt, const U &x_lower,
                       const U &x_upper, const double rho_begin,
                       const double rho_end, const long max_f_evals) {
  // The starting point (i.e. x) must be a column vector.
  COMPILE_TIME_ASSERT(T::NC <= 1);

  return -find_min_bobyqa(negate_function(f), x, npt, x_lower, x_upper,
                          rho_begin, rho_end, max_f_evals);
}
