...
template <typename matrix_type> class oca_problem {
  ...
  Optionally, there can also be non-negativity constraints on some or all of the
              elements of w.
          ...
          Or it can use the elastic net regularizer:
              Minimize: f(w) == 0.5*(1-lasso_lambda)*length_squared(w) +
  lasso_lambda*sum(abs(w)) + C*R(w)

              Where lasso_lambda is a number in the range [0, 1) and controls
              trade-off between doing L1 and L2 regularization.  R(w) has the
...
public:
  ...
  virtual bool optimization_status(scalar_type current_objective_value,
                                   ...
                                   unsigned long num_iterations) const = 0;
  /*!
      requires
          - This function is called by the OCA optimizer each iteration.
          - current_objective_value == the current value of the objective
  function f(w)
          - current_error_gap == The bound on how much lower the objective
  function can drop before we reach the optimal point.  At the optimal solution
  the error gap is equal to 0.
          - current_risk_value == the current value of the R(w) term of the
  objective function.
          - current_risk_gap == the bound on how much lower the risk term can
  go.  At the optimal solution the risk gap is zero.
          - num_cutting_planes == the number of cutting planes the algorithm is
  currently using.
          - num_iterations == A count of the total number of iterations that
  have executed since we started running the optimization. ensures
          - If it is appropriate to terminate the optimization then this
  ...
  /*!
      ...
      ensures
          ...
          - Note that #risk_value and #risk_subgradient are NOT multiplied by
  get_c()
  !*/
};
...
class oca {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object is a tool for solving the optimization problem defined
  above by the oca_problem abstract class.

          For reference, OCA solves optimization problems with the following
  form: Minimize: f(w) == 0.5*length_squared(w) + C*R(w)

              Where R(w) is a user-supplied convex function and C > 0.
  Optionally, this object can also add non-negativity constraints to some or all
  ...
  same interpretation as above.
          ...
          from the Journal of Machine Learning Research:
              Optimized Cutting Plane Algorithm for Large-Scale Risk
  Minimization Vojtech Franc, Soren Sonnenburg; 10(Oct):2157--2192, 2009.
...
public:
  ...
  /*!
      ...
      ensures
          - solves the given oca problem and stores the solution in #w.  In
  particular, this function solves: Minimize: f(w) == 0.5*length_squared(w) +
  C*R(w)
          ...
          - if (num_nonnegative != 0) then
              - Adds the constraint that #w(i) >= 0 for all i < num_nonnegative.
                ...
                non-negative.  This includes the copies of w passed to
  get_risk() in the form of the current_solution vector as well as the final
          ...
          - if (force_weight_to_1 < problem.get_num_dimensions()) then
              - The optimizer enforces the following constraints:
                  ...
                  - for all i > force_weight_to_1:
                      - #w(i) == 0
                  - That is, the element in the weight vector at the index
  indicated by force_weight_to_1 will have a value of 1 upon completion of this
  function, while all subsequent elements of w will have values of 0.
  !*/
  ...
  solve_with_elastic_net(const oca_problem<matrix_type> &problem,
                         matrix_type &w, scalar_type lasso_lambda,
                         unsigned long force_weight_to_1 =
                             std::numeric_limits<unsigned long>::max()) const;
  /*!
      requires
          ...
          - 0 <= lasso_lambda < 1
      ensures
          - Solves the given oca problem and stores the solution in #w, but uses
  an elastic net regularizer instead of the normal L2 regularizer.  In
  ...
  void set_subproblem_epsilon(double eps);
  /*!
      requires
          - eps > 0
      ensures
  ...
  double get_subproblem_epsilon() const;
  /*!
      ensures
  ...
  void set_subproblem_max_iterations(unsigned long sub_max_iter);
  /*!
      requires
          - sub_max_iter > 0
      ensures
  ...
  unsigned long get_subproblem_max_iterations() const;
  /*!
      ensures
  ...
  void set_inactive_plane_threshold(unsigned long inactive_thresh);
  /*!
      requires
          - inactive_thresh > 0
      ensures
  ...
  unsigned long get_inactive_plane_threshold() const;
  /*!
      ensures
