...
template <typename T>
unsigned long
fill_parse_tree(std::vector<parse_tree_element<T>> &parse_tree, const T &tag,
                const array2d<std::map<T, parse_tree_element<T>>> &back, long r,
                long c)
/*!
    requires
...
!*/
{
  ...
  const unsigned long idx_left =
      fill_parse_tree(parse_tree, item.c.left_tag, back, r, k - 1);
  const unsigned long idx_right =
      fill_parse_tree(parse_tree, item.c.right_tag, back, k, c);
...
void find_max_parse_cky(const std::vector<T> &sequence,
  ...
  array2d<std::map<T, parse_tree_element<T>>> back(sequence.size(),
                                                   sequence.size());
...
typename enable_if_c<enabled>::type conditional_print(const T &item,
                                                      std::ostream &out) {
  out << item << " ";
}

template <bool enabled, typename T>
typename disable_if_c<enabled>::type conditional_print(const T &,
                                                       std::ostream &) {}

template <bool print_tag, bool skip_tag, typename T, typename U>
void print_parse_tree_helper(const std::vector<parse_tree_element<T>> &tree,
                             const std::vector<U> &words, unsigned long i,
                             const T &tag_to_skip, std::ostream &out) {
  ...
  if (tree[i].left < tree.size()) {
    left_recurse = true;
    print_parse_tree_helper<print_tag, skip_tag>(tree, words, tree[i].left,
                                                 tag_to_skip, out);
  } else {
...
std::string parse_tree_to_string(const std::vector<parse_tree_element<T>> &tree,
  ...
  std::ostringstream sout;
  impl::print_parse_tree_helper<false, false>(tree, words, root_idx,
                                              tree[root_idx].tag, sout);
  return sout.str();
}
...
parse_tree_to_string_tagged(const std::vector<parse_tree_element<T>> &tree,
  ...
  std::ostringstream sout;
  impl::print_parse_tree_helper<true, false>(tree, words, root_idx,
                                             tree[root_idx].tag, sout);
  return sout.str();
}
...
void helper_find_trees_without_tag(
    ...
    std::vector<unsigned long> &tree_roots, unsigned long idx) {
  if (idx < tree.size()) {
    if (tree[idx].tag != tag) {
