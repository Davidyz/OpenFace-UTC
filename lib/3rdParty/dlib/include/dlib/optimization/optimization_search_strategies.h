...
public:
  ...
  double get_wolfe_rho() const { return 0.001; }

  double get_wolfe_sigma() const { return 0.01; }

  unsigned long get_max_line_search_iterations() const { return 100; }

  template <typename T>
  const matrix<double, 0, 1> &get_next_direction(const T &, const double,
                                                 const T &funct_derivative) {
    if (been_used == false) {
      been_used = true;
      prev_direction = -funct_derivative;
    } else {
      // Use the Polak-Ribiere (4.1.12) conjugate gradient described by Fletcher
      // on page 83
      const double temp = trans(prev_derivative) * prev_derivative;
      // If this value hits zero then just use the direction of steepest
      // descent.
      ...
      double b =
          trans(funct_derivative - prev_derivative) * funct_derivative / (temp);
...
public:
  ...
  template <typename T>
  const matrix<double, 0, 1> &get_next_direction(const T &x, const double,
                                                 const T &funct_derivative) {
    if (been_used == false) {
      been_used = true;
      ...
      // Try to set the initial value of the H matrix to something reasonable if
      // we are still in the early stages of figuring out what it is.  This
      // formula below is what is suggested in the book Numerical Optimization
      // by Nocedal and Wright in the chapter on Quasi-Newton methods.
      ...
      if (gHg < std::numeric_limits<double>::infinity() &&
        ...
        H += (1 + gHg / dg) * delta * trans(delta) / (dg) -
             (delta * trans(gH) + Hg * trans(delta)) / (dg);
      } else {
...
public:
  explicit lbfgs_search_strategy(unsigned long max_size_)
      : max_size(max_size_), been_used(false) {
    DLIB_ASSERT(max_size > 0, "\t lbfgs_search_strategy(max_size)"
  ...
  const matrix<double, 0, 1> &get_next_direction(const T &x, const double,
    ...
    if (been_used == false) {
      ...
      if (data.size() > 0) {
        ...
        // Take a guess at what the first H matrix should be.  This formula
        // below is what is suggested in the book Numerical Optimization by
        // Nocedal and Wright in the chapter on Large Scale Unconstrained
        // Optimization (in the L-BFGS section).
        double H_0 = 1.0 / data[data.size() - 1].rho /
                     dot(data[data.size() - 1].y, data[data.size() - 1].y);
...
template <typename hessian_funct> class newton_search_strategy_obj {
public:
  explicit newton_search_strategy_obj(const hessian_funct &hess)
      : hessian(hess) {}
...
newton_search_strategy_obj<hessian_funct>
newton_search_strategy(hessian_funct hessian) {
