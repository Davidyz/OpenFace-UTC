...
unsigned long solve_trust_region_subproblem(const matrix_exp<EXP1> &B,
  ...
  */
  ...
  const T BB_min_eigenvalue =
      min(diag(BB) - (sum_cols(abs(BB)) - abs(diag(BB))));

  const T g_norm = length(gg);

  T lambda_min = 0;
  T lambda_max = put_in_range(0, std::numeric_limits<T>::max(),
                              g_norm / radius - BB_min_eigenvalue);

  // If we can tell that the minimum is at 0 then don't do anything.  Just
  // return the answer.
  ...
  for (unsigned long i = 0; i < max_iter; ++i) {
    ...
    if (R.nr() == 2) {
      ...
      triangular_solver(CblasLeft, CblasLower, CblasNoTrans, CblasNonUnit, R,
                        p);
    }
    ...
    // make sure the chosen lambda is within our bracket (but not exactly at
    // either end).
    const T gap = (lambda_max - lambda_min) * 0.01;
    lambda = put_in_range(lambda_min + gap, lambda_max - gap, lambda);

    // Keep track of how much lambda is thrashing around inside the search
    // bracket.  If it keeps moving around a whole lot then cut the search
    // bracket in half.
  ...
  // We are probably in the "hard case".   Use an eigenvalue decomposition to
  // sort things out. Either that or the eps was just set too tight and really
  // we are already done.
  ...
  ev = pointwise_multiply(ev, ev > max(abs(ev)) *
  ...
  // If we really are in the hard case then this if will be true.  Otherwise,
  // the p we found in the "easy case" loop up top is the best answer.
  if (length(p_hard) < radius && length(p_hard) >= length(p)) {
    // adjust the length of p_hard by adding a component along the eigenvector
    // associated with the smallest eigenvalue.  We want to make it the case
    // that length(p) == radius.
    const T tau = std::sqrt(radius * radius - length_squared(p_hard));
    p = p_hard + tau * colm(ed.get_pseudo_v(), min_eig_idx);

    // if we have to do an eigenvalue decomposition then say we did all the
    // iterations
    return max_iter;
  }
...
bool bounds_violated(const matrix_exp<EXP1> &v, const matrix_exp<EXP2> &l,
...
template <typename EXP1, typename EXP2, typename T, long NR, long NC,
          typename MM, typename L, typename EXP3>
void solve_trust_region_subproblem_bounded(const matrix_exp<EXP1> &B_,
                                           const matrix_exp<EXP2> &g_,
                                           const typename EXP1::type radius_,
                                           matrix<T, NR, NC, MM, L> &p_,
                                           double eps, unsigned long max_iter,
  ...
  // make sure the problem is feasible.  That is, there should be a point inside
  // the lower and upper bounds that has a norm <= radius_
  DLIB_ASSERT(
      length(clamp(zeros_matrix(lower_), lower_, upper_)) <= radius_,
      "The lower and upper bounds are incompatible with the radius since there "
      "is no point within the bounds with a norm less than the radius.");

  // We are going to solve this by greedily finding the most violated bound
  // constraint, locking that variable to its constrained value, removing it
  // from the problem, and then resolving.  We do that until no more constraint
  ...
  // keep a table that tells us how to map any reduced QP back to the original
  // QP
  ...
  while (impl::bounds_violated(p, lower, upper)) {
    // Find the most violated variable and fix its value to a constant (the
    // bound value).
    ...
    // Removing a variable changes the radius, so we have to subtract the
    // bounded value from the radius so as to not change the effective radius
...
template <typename stop_strategy_type, typename funct_model>
double find_min_trust_region(stop_strategy_type stop_strategy,
  ...
  */
  ...
  DLIB_ASSERT(is_finite(x),
              "The objective function generated non-finite outputs");
  DLIB_ASSERT(is_finite(g),
              "The objective function generated non-finite outputs");
  DLIB_ASSERT(is_finite(h),
              "The objective function generated non-finite outputs");

  // Sometimes the loop below won't modify x because the trust region step
  // failed. This bool tells us when we are in that case.
  bool stale_x = false;

  while (stale_x || stop_strategy.should_continue_search(x, f_value, g)) {
    const unsigned long iter =
        solve_trust_region_subproblem(h, g, radius, p, 0.1, 20);
    ...
    // If the sub-problem can't find a way to improve then stop.  This only
    // happens when p is essentially 0.
    if (std::abs(predicted_improvement) <=
        std::abs(measured_improvement) * std::numeric_limits<type>::epsilon())
      break;

    // predicted_improvement shouldn't be negative but it might be if something
    // went wrong in the trust region solver.  So put abs() here to guard
    // against that.  This way the sign of rho is determined only by the sign of
    ...
    if (rho < 0.25) {
      ...
      // something has gone horribly wrong if the radius has shrunk to zero.  So
...
template <typename funct_model> struct negate_tr_model {
  ...
  template <typename T> typename T::type operator()(const T &x) const {
    return -model(x);
  }

  template <typename T, typename U>
  void get_derivative_and_hessian(const T &x, T &d, U &h) const {
...
template <typename stop_strategy_type, typename funct_model>
double find_max_trust_region(stop_strategy_type stop_strategy,
                             const funct_model &model,
                             typename funct_model::column_vector &x,
                             double radius = 1) {
  // make sure requires clause is not broken
  ...
  return -find_min_trust_region(stop_strategy,
                                negate_tr_model<funct_model>(model), x, radius);
}
