...
public:
  simple_hash_map() : scan_dist(6) { data.resize(5000); }

  void insert(const unsigned long a, const unsigned long b,
  ...
  !*/
  {
    ...
    const unsigned long empty_bucket =
    ...
    // if we get this far it means the hash table is filling up.  So double its
    // size.
  ...
  unsigned long operator()(const unsigned long a, const unsigned long b) const
  /*!
      requires
  ...
  !*/
  {
    DLIB_ASSERT(
        a != b,
        "An invalid map_problem was given to find_max_factor_graph_nmplp()."
            << "\nNode " << a
            << " is listed as being a neighbor with itself, which is illegal.");
    ...
    // this should never happen (since this function requires (a,b) to be in the
    // hash table
    DLIB_ASSERT(
        false,
        "An invalid map_problem was given to find_max_factor_graph_nmplp()."
            << "\nThe nodes in the map_problem are inconsistent because node "
            << a << " is in the neighbor list"
            << "\nof node " << b << " but node " << b
            << " isn't in the neighbor list of node " << a
...
void find_max_factor_graph_nmplp(const map_problem &prob,
  ...
  DLIB_ASSERT(eps > 0, "\t void find_max_factor_graph_nmplp()"
                           << "\n\t eps must be greater than zero"
                           << "\n\t eps:  " << eps);

  /*
      This function is an implementation of the NMPLP algorithm introduced in
     the following papers: Fixing Max-Product: Convergent Message Passing
     Algorithms for MAP LP-Relaxations (2008) by Amir Globerson and Tommi
     Jaakkola

          Introduction to dual decomposition for inference (2011)
          by David Sontag, Amir Globerson, and Tommi Jaakkola

      In particular, this function implements the star MPLP update equations
     shown as equation 1.20 from the paper Introduction to dual decomposition
     for inference (the method was called NMPLP in the first paper).  It should
     also be noted that the original description of the NMPLP in the first paper
     had an error in the equations and the second paper contains corrected
     equations, which is what this function uses.
  */
  ...
  delta_elements.reserve(prob.number_of_nodes() *
                         prob.num_states(prob.begin()) * 3);

  impl::simple_hash_map delta_idx;

  // Initialize delta to zero and fill up the hash table with the appropriate
  // values so we can index into delta later on.
  ...
  // These arrays will end up with a length equal to the maximum number of
  // neighbors of any node in the graph.  So reserve a bigish number of slots so
  // that we are very unlikely to need to preform an expensive reallocation
  ...
  for (iter = 0; iter < max_iter && max_change > eps; ++iter) {
    ...
    for (node_iterator i = prob.begin(); i != prob.end(); ++i) {
      ...
      for (neighbor_iterator j = prob.begin(i); j != prob.end(i); ++j) {
        ...
        gamma_ji[jcnt].assign(num_states_xi,
        ...
        for (unsigned long xi = 0; xi < num_states_xi; ++xi) {
          for (unsigned long xj = 0; xj < num_states_xj; ++xj) {
            gamma_ji[jcnt][xi] =
                std::max(gamma_ji[jcnt][xi], prob.factor_value(i, j, xi, xj) +
                                                 delta_to_j_no_i[jcnt][xj]);
          }
      ...
      for (neighbor_iterator j = prob.begin(i); j != prob.end(i); ++j) {
        ...
        for (unsigned long xi = 0; xi < num_states_xi; ++xi) {
          double new_val =
              -1 / (num_neighbors + 1) * gamma_i[xi] + gamma_ji[jcnt][xi];
