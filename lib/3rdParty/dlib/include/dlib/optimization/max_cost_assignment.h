...
typename EXP::type assignment_cost(const matrix_exp<EXP> &cost,
                                   const std::vector<long> &assignment) {
  DLIB_ASSERT(cost.nr() == cost.nc(), "\t type assignment_cost(cost,assignment)"
                                          << "\n\t cost.nr(): " << cost.nr()
                                          << "\n\t cost.nc(): " << cost.nc());
...
namespace impl {
template <typename EXP>
inline void compute_slack(const long x, std::vector<typename EXP::type> &slack,
...
template <typename EXP>
std::vector<long> max_cost_assignment(const matrix_exp<EXP> &cost_) {
  const_temp_matrix<EXP> cost(cost_);
  typedef typename EXP::type type;
  // This algorithm only works if the elements of the cost matrix can be
  // reliably compared using operator==. However, comparing for equality with
  // floating point numbers is not a stable operation. So you need to use an
  // integer cost matrix.
  ...
  */
  ...
  for (long match_size = 0; match_size < cost.nc(); ++match_size) {
    ...
    */
    ...
    // Flip the edges along the augmenting path.  This means we will add one
    // more item to our matching.
    for (long cx = x_start, cy = y_start, ty; cx != -1;
