...
/*
    ...
    derived by using an important theorem from the theory of constrained
   optimization. This theorem tells us that any optimal point of a constrained
   function must satisfy what are called the KKT conditions (also sometimes
   called just the KT conditions, especially in older literature).  A very good
   book to consult regarding this topic is Practical Methods of Optimization
   (second edition) by R. Fletcher.  Below I will try to explain the general
   idea of how this is applied.
    ...
    problem must satisfy the following 5 conditions:
        ...
        3. Let L(alpha, x, y) == f(alpha) - trans(x)*alpha - y*(trans(e)*alpha -
   C) Where x is a vector of length alpha.size() and y is a single scalar. Then
   the derivative of L with respect to alpha must == 0 So we get the following
   as our 3rd condition: f'(alpha) - x - y*e == 0
...
template <typename EXP1, typename EXP2, typename T, long NR, long NC,
          typename MM, typename L>
unsigned long solve_qp_using_smo(const matrix_exp<EXP1> &Q,
                                 const matrix_exp<EXP2> &b,
                                 matrix<T, NR, NC, MM, L> &alpha, T eps,
                                 unsigned long max_iter) {
  // make sure requires clause is not broken
  DLIB_ASSERT(Q.nr() == Q.nc() && is_col_vector(b) && is_col_vector(alpha) &&
                  b.size() == alpha.size() && b.size() == Q.nr() &&
                  alpha.size() > 0 && min(alpha) >= 0 && eps > 0 &&
                  max_iter > 0,
  ...
  for (; iter < max_iter; ++iter) {
    ...
    //    - big_idx   == the index of the largest element in df such that
    //    alpha(big_idx) > 0
    // These two indices will tell us which two alpha values are most in
    // violation of the KKT optimality conditions.
    ...
    T quad_coef = Q(big_idx, big_idx) + Q(little_idx, little_idx) -
                  2 * Q(big_idx, little_idx);
    if (quad_coef <= tau)
      quad_coef = tau;
    ...
    // Make sure alpha stays feasible.  That is, make sure the updated alpha
    // doesn't violate the non-negativity constraint.
    if (alpha(big_idx) < 0) {
      // Since an alpha can't be negative we will just set it to 0 and shift all
      // the weight to the other alpha.
      alpha(big_idx) = 0;
    ...
    if ((iter % 300) == 299) {
      // Perform this form of the update every so often because doing so can
      // help avoid the buildup of numerical errors you get with the alternate
      // update below.
      df = Q * alpha - b;
    } else {
      // Now update the gradient. We will perform the equivalent of: df =
      // Q*alpha - b;
...
template <typename EXP1, typename EXP2, typename EXP3, typename EXP4,
          typename T, long NR, long NC, typename MM, typename L, long NR2,
          long NC2>
unsigned long solve_qp4_using_smo(
    const matrix_exp<EXP1> &A, const matrix_exp<EXP2> &Q,
    const matrix_exp<EXP3> &b, const matrix_exp<EXP4> &d,
    matrix<T, NR, NC, MM, L> &alpha, matrix<T, NR2, NC2, MM, L> &lambda, T eps,
    unsigned long max_iter, T max_lambda = std::numeric_limits<T>::infinity()) {
  // make sure requires clause is not broken
  DLIB_ASSERT(A.nc() == alpha.size() && Q.nr() == Q.nc() && is_col_vector(b) &&
                  is_col_vector(alpha) && b.size() == alpha.size() &&
                  b.size() == Q.nr() && alpha.size() > 0 && min(alpha) >= 0 &&
                  eps > 0 && max_iter > 0,
              "\t void solve_qp4_using_smo()"
                  ...
                  << "\n\t eps:                  " << eps
                  << "\n\t max_iter:             " << max_iter);
  DLIB_ASSERT(is_col_vector(d) == true && max_lambda >= 0 && d.size() == A.nr(),
              "\t void solve_qp4_using_smo()"
                  << "\n\t Invalid arguments were given to this function"
                  << "\n\t A.nr():     " << A.nr() << "\n\t d.size():   "
  ...
  */
  ...
  // Compute f'(alpha) (i.e. the gradient of f(alpha) with respect to alpha) for
  // the current alpha.
...
template <typename EXP1, typename EXP2, typename T, long NR, long NC,
          typename MM, typename L>
unsigned long
solve_qp_box_constrained(const matrix_exp<EXP1> &Q, const matrix_exp<EXP2> &b,
                         matrix<T, NR, NC, MM, L> &alpha,
                         const matrix<T, NR, NC, MM, L> &lower,
                         const matrix<T, NR, NC, MM, L> &upper, T eps = 1e-10,
                         unsigned long max_iter = 30000) {
  // make sure requires clause is not broken
  DLIB_ASSERT(Q.nr() == Q.nc() && alpha.size() == lower.size() &&
                  alpha.size() == upper.size() && is_col_vector(b) &&
                  is_col_vector(alpha) && is_col_vector(lower) &&
                  is_col_vector(upper) && b.size() == alpha.size() &&
                  b.size() == Q.nr() && alpha.size() > 0 &&
                  0 <= min(alpha - lower) && 0 <= max(upper - alpha) &&
                  eps > 0 && max_iter > 0,
  ...
  // cout << "objective value: " << 0.5*trans(alpha)*Q*alpha + trans(b)*alpha <<
  // endl;

  // Now do the main iteration block of this solver.  The coordinate descent
  // method we used above can improve the objective rapidly in the beginning.
  // However, Nesterov's method has more rapid convergence once it gets going so
  // this is what we use for the main iteration.
  matrix<T, NR, NC, MM, L> v, v_old;
  v = alpha;
  // We need to get an upper bound on the Lipschitz constant for this QP. Since
  // that is just the max eigenvalue of Q we can do it using Gershgorin disks.
...
bool has_uniform_offdiag_vectors(
    const std::map<unordered_pair<size_t>, matrix<T, NR, NC, MM, L>>
        &Q_offdiag) {
...
template <typename T, long NR, long NC, typename MM, typename L>
matrix<T, 0, 0, MM, L>
compact_offdiag(const size_t &num_blocks,
                const std::map<unordered_pair<size_t>, matrix<T, NR, NC, MM, L>>
                    &Q_offdiag) {
...
unsigned long solve_qp_box_constrained_blockdiag(
    ...
    const std::vector<matrix<T, NR, NC, MM, L>> &uppers, T eps = 1e-10,
    unsigned long max_iter = 30000) {
  // make sure requires clause is not broken
  DLIB_CASSERT(Q_blocks.size() > 0);
  DLIB_CASSERT(
      Q_blocks.size() == bs.size() && Q_blocks.size() == alphas.size() &&
          Q_blocks.size() == lowers.size() && Q_blocks.size() == uppers.size(),
  ...
  for (auto &Q : Q_blocks) {
    DLIB_CASSERT(Q.nr() == Q.nc(),
                 "All the matrices in Q_blocks have the same dimensions.");
    DLIB_CASSERT(Q.size() > 0, "All the matrices in Q_blocks must be non-empty "
                               "and have the same dimensions.");
    DLIB_CASSERT(Q.nr() == Q_blocks[0].nr() && Q.nc() == Q_blocks[0].nc(),
                 "All the matrices in Q_blocks have the same dimensions.");
  }
  ...
  for (size_t i = 0; i < alphas.size(); ++i) {
    ...
    for (auto &Qoffdiag : Q_offdiag) {
      ...
      DLIB_CASSERT(
          is_col_vector(Q_offdiag_element) &&
              Q_offdiag_element.size() == Q_blocks[0].nr(),
          "is_col_vector(Q_offdiag["
              << r << "," << c << "]): " << is_col_vector(Q_offdiag_element)
              << "\n"
              << "Q_offdiag[" << r << "," << c
              << "].size():         " << Q_offdiag_element.size() << "\n"
              << "Q_blocks[0].nr():                  " << Q_blocks[0].nr());
    }

    DLIB_CASSERT(is_col_vector(alphas[i]) &&
                     alphas[i].size() == Q_blocks[0].nr(),
                 "is_col_vector(alphas["
                     << i << "]): " << is_col_vector(alphas[i]) << "\n"
                     << "alphas[" << i
                     << "].size():         " << alphas[i].size() << "\n"
                     << "Q_blocks[0].nr():               " << Q_blocks[0].nr());

    DLIB_CASSERT(is_col_vector(lowers[i]) &&
                     lowers[i].size() == Q_blocks[0].nr(),
                 "is_col_vector(lowers["
                     << i << "]): " << is_col_vector(lowers[i]) << "\n"
                     << "lowers[" << i
                     << "].size():         " << lowers[i].size() << "\n"
                     << "Q_blocks[0].nr():               " << Q_blocks[0].nr());

    DLIB_CASSERT(is_col_vector(uppers[i]) &&
                     uppers[i].size() == Q_blocks[0].nr(),
                 "is_col_vector(uppers["
                     << i << "]): " << is_col_vector(uppers[i]) << "\n"
                     << "uppers[" << i
                     << "].size():         " << uppers[i].size() << "\n"
                     << "Q_blocks[0].nr():               " << Q_blocks[0].nr());

    DLIB_CASSERT(0 <= min(alphas[i] - lowers[i]),
                 "min(alphas[" << i << "]-lowers[" << i
                               << "]): " << min(alphas[i] - lowers[i]));
    DLIB_CASSERT(0 <= max(uppers[i] - alphas[i]),
                 "max(uppers[" << i << "]-alphas[" << i
                               << "]): " << max(uppers[i] - alphas[i]));
  }
  DLIB_CASSERT(eps > 0 && max_iter > 0,
               "eps: " << eps << "\nmax_iter: " << max_iter);
#endif // ENABLE_ASSERTS

  const auto offdiag_compact =
      impl::compact_offdiag(Q_blocks.size(), Q_offdiag);
  ...
  auto compute_df = [&]() {
    ...
    // Don't forget to include the Q_offdiag terms in the computation.  Note
    // that we have two options for how we can compute this part.  If Q_offdiag
    // is uniform and can be compacted into a simple matrix and there are a lot
    // of off diagonal entries then it's faster to do it as a matrix multiply.
    // Otherwise we do the more general computation.
  ...
  for (size_t i = 0; i < QQ.size(); ++i) {
    ...
    Q_ggd[i] =
        Q_diag[i] + (sum_cols(abs(Q_blocks[i])) - abs(diag(Q_blocks[i])));
  }
  for (auto &p : Q_offdiag) {
    long r = p.first.first;
    long c = p.first.second;
  ...
  for (long iter = 0; iter < num_variables * 2; ++iter) {
    ...
    // Now update the gradient. We will perform the equivalent of: df = Q*alpha
    // + b; except we only need to compute one column of the matrix multiply
    // because only one element of alpha changed.
  ...
  for (iter = 0; iter < max_iter; ++iter) {
    ...
    for (size_t j = 0; j < alphas.size(); ++j) {
      v[j] = clamp(alphas[j] - 1.0 / lipschitz_bound * df[j], lowers[j],
                   uppers[j]);
      alphas[j] =
          clamp((1 - gamma) * v[j] + gamma * v_old[j], lowers[j], uppers[j]);
    }
...
template <typename EXP1, typename EXP2, typename T, long NRa, long NRb>
unsigned long
find_gap_between_convex_hulls(const matrix_exp<EXP1> &A,
                              const matrix_exp<EXP2> &B, matrix<T, NRa, 1> &cA,
                              matrix<T, NRb, 1> &cB, const double eps,
