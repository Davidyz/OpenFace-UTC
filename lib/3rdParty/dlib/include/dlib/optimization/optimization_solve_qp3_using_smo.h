...
public:
  ...
  operator()(const matrix_exp<EXP1> &Q, const matrix_exp<EXP2> &p,
             ...
             scalar_type eps) {
    DLIB_ASSERT(
        Q.nr() == Q.nc() && y.size() == Q.nr() && p.size() == y.size() &&
            y.size() > 0 && is_col_vector(y) && is_col_vector(p) &&
            sum((y == +1) + (y == -1)) == y.size() && Cp > 0 && Cn > 0 &&
            eps > 0,
...
private:
  ...
  inline void set_initial_alpha(const scalar_vector_type &y,
                                const scalar_type B, const scalar_type Cp,
    ...
    if (count != num_total) {
      ...
      sout << "Invalid QP3 constraint parameters of B: " << B << ", Cp: " << Cp
           << ", Cn: " << Cn;
  ...
  find_working_group(const V &y, const U &alpha, const matrix_exp<EXP> &Q,
                     const scalar_vector_type &df, const scalar_type Cp,
                     const scalar_type Cn, const scalar_type tau,
  ...
  template <typename EXP, typename EXP2, typename T, typename U>
  inline void optimize_working_pair(T &alpha, const matrix_exp<EXP> &Q,
                                    const matrix_exp<EXP2> &y, const U &df,
                                    const scalar_type &tau, const long i,
                                    const long j, const scalar_type &Cp,
                                    const scalar_type &Cn) const {
