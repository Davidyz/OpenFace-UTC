...
double
find_optimal_parameters(double initial_search_radius, double eps,
                        const unsigned int max_f_evals, matrix<double, 0, 1> &x,
                        const matrix<double, 0, 1> &x_lower,
                        const matrix<double, 0, 1> &x_upper, const funct &f) {
  DLIB_CASSERT(x.size() == x_lower.size() && x_lower.size() == x_upper.size() &&
                   x.size() > 0,
  ...
  DLIB_CASSERT(min(x_upper - x_lower) > 0 && min(x - x_lower) >= 0 &&
                   min(x_upper - x) >= 0,
               "\t double find_optimal_parameters()"
                   << "\n\t The bounds constraints have to make sense and also "
  ...
  // if the search radius is too big then shrink it so it fits inside the
  // bounds.
  ...
  if (x.size() == 1) {
    // BOBYQA requires x to have at least 2 variables in it.  So we can't call
    // it in this case.  Instead we call find_min_single_variable().
    ...
    try {
      double dx = x(0);
      find_min_single_variable(ff, dx, x_lower(0), x_upper(0), eps, max_f_evals,
    ...
    try {
      ...
      find_min_bobyqa(ff, start_x, 2 * x.size() + 1, x_lower, x_upper,
                      initial_search_radius, eps, max_f_evals);
