...
class array2d : public enumerable<T> {
  ...
  memory_manager_stateless/memory_manager_stateless_kernel_abstract.h
      ...
      ENUMERATION ORDER
          ...
          in a row will be enumerated beginning with the 0th column, then the
  1st column and so on.
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          row major order.  Moreover, there is no padding at the end of each
  row. This means that its width_step() value is always equal to
  sizeof(type)*nc().
  !*/
...
public:
  ...
  /*!
      ...
      ensures
          ...
          - #nr() == rows
          - #at_start() == true
          - if (the call to set_size() doesn't change the dimensions of this
  array) then
              - all elements in this array retain their values from before this
  function was called
          - else
              - all elements in this array have initial values for their type
      throws
  ...
  const row operator[](long row_index) const;
  /*!
      requires
          - 0 <= row_index < nr()
      ensures
  ...
  this object.
          - The iterator will iterate over the elements of the object in row
  major order.
  !*/
...
inline void swap(array2d<T, mem_manager> &a, array2d<T, mem_manager> &b) {
  a.swap(b);
}
/*!
...
template <typename T, typename mem_manager>
void serialize(const array2d<T, mem_manager> &item, std::ostream &out);
/*!
...
template <typename T, typename mem_manager>
void deserialize(array2d<T, mem_manager> &item, std::istream &in);
/*!
