...
void serialize(const array2d<rgb_pixel, mem_manager> &item, std::ostream &out) {
  try {
...
template <typename mem_manager>
void deserialize(array2d<rgb_pixel, mem_manager> &item, std::istream &in) {
  try {
    COMPILE_TIME_ASSERT(sizeof(rgb_pixel) == 3);
...
template <typename mem_manager>
void serialize(const array2d<bgr_pixel, mem_manager> &item, std::ostream &out) {
  try {
    // The reason the serialization is a little funny is because we are trying
    // to maintain backwards compatibility with an older serialization format
    // used by dlib while also encoding things in a way that lets the array2d
...
template <typename mem_manager>
void deserialize(array2d<bgr_pixel, mem_manager> &item, std::istream &in) {
  try {
    COMPILE_TIME_ASSERT(sizeof(bgr_pixel) == 3);
...
template <typename mem_manager>
void serialize(const array2d<hsi_pixel, mem_manager> &item, std::ostream &out) {
  try {
    // The reason the serialization is a little funny is because we are trying
    // to maintain backwards compatibility with an older serialization format
    // used by dlib while also encoding things in a way that lets the array2d
...
template <typename mem_manager>
void deserialize(array2d<hsi_pixel, mem_manager> &item, std::istream &in) {
  try {
    COMPILE_TIME_ASSERT(sizeof(hsi_pixel) == 3);
...
template <typename mem_manager>
void serialize(const array2d<rgb_alpha_pixel, mem_manager> &item,
               std::ostream &out) {
  try {
    // The reason the serialization is a little funny is because we are trying
    // to maintain backwards compatibility with an older serialization format
...
template <typename mem_manager>
void deserialize(array2d<rgb_alpha_pixel, mem_manager> &item,
                 std::istream &in) {
  try {
    COMPILE_TIME_ASSERT(sizeof(rgb_alpha_pixel) == 4);
...
template <typename mem_manager>
void serialize(const array2d<unsigned char, mem_manager> &item,
               std::ostream &out) {
  try {
    // The reason the serialization is a little funny is because we are trying
    // to maintain backwards compatibility with an older serialization format
...
template <typename mem_manager>
void deserialize(array2d<unsigned char, mem_manager> &item, std::istream &in) {
  try {
    long nr, nc;
    deserialize(nr, in);
    deserialize(nc, in);
