...
public:
  // These typedefs are here for backwards compatibility with older versions of
  // dlib.
    ...
    const T &operator[](long column) const {
      ...
      DLIB_ASSERT(column < nc() && column >= 0,
                  "\tconst T& array2d::operator[](long column) const"
                      << "\n\tThe column index given must be less than the "
                         "number of columns."
                      << "\n\tthis:    " << this << "\n\tcolumn:  " << column
                      << "\n\tnc(): " << nc());
  ...
  array2d(array2d &&item) : array2d() { swap(item); }
  ...
  bool current_element_valid() const { return (cur != 0); }
  ...
  size_t size() const {
    return static_cast<size_t>(nc_) * static_cast<size_t>(nr_);
  }

  long width_step() const { return nc_ * sizeof(T); }
...
template <typename T, typename mem_manager>
void array2d<T, mem_manager>::set_size(long rows, long cols) {
  // make sure requires clause is not broken
  DLIB_ASSERT((cols >= 0 && rows >= 0),
...
template <typename T, typename MM> struct is_array2d<array2d<T, MM>> {
