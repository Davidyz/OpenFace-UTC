...
template <typename domain, typename range, typename mem_manager,
          typename compare = std::less<domain>>
class binary_search_tree_kernel_1
    : public enumerable<map_pair<domain, range>>,
  ...
  /*!
      ...
      CONVENTION
          ...
          if (current_element != 0 && current_element != tree_root) then
              stack[stack_pos-1] == the parent of the node pointed to by
  current_element
  ...
  and right points to the right subtree or 0 if there is no right subtree and
              ...
              for all trees:
                  the height of the left and right subtrees differ by at most
  one
          }
  ...
  public:
    ...
    const domain &key() const { return *d; }

    const range &value() const { return *r; }

    range &value() { return *r; }
  };
...
public:
  ...
  binary_search_tree_kernel_1()
      : tree_size(0), tree_root(0), current_element(0), tree_height(0),
        at_start_(true), stack_pos(0), stack(ppool.allocate_array(50)) {}
  ...
  inline unsigned long count(const domain &item) const;

  inline void add(domain &d, range &r);

  void remove(const domain &d, domain &d_copy, range &r);

  void destroy(const domain &item);

  inline const range *operator[](const domain &item) const;

  inline range *operator[](const domain &item);

  inline void swap(binary_search_tree_kernel_1 &item);

  // function from the asc_pair_remover interface
  void remove_any(domain &d, range &r);
  ...
  const map_pair<domain, range> &element() const;

  map_pair<domain, range> &element();

  bool move_next() const;

  void remove_last_in_order(domain &d, range &r);

  void remove_current_element(domain &d, range &r);

  void position_enumerator(const domain &d) const;
...
private:
  ...
  inline void double_rotate_right(node *&t);
  /*!
      requires
          - t->balance == -2
          - t->left->balance == 1
  ...
  inline void double_rotate_left(node *&t);
  /*!
      requires
          - t->balance == 2
          - t->right->balance == -1
  ...
  bool remove_biggest_element_in_tree(node *&t, domain &d, range &r);
  /*!
      requires
  ...
  bool remove_least_element_in_tree(node *&t, domain &d, range &r);
  /*!
      requires
          - t != 0  (i.e. there must be something in the tree to remove)
          - t == reference to the pointer in t's parent node that points to t
  ...
  bool add_to_tree(node *&t, domain &d, range &r);
  /*!
      requires
          - t == reference to the pointer in t's parent node that points to t
      ensures
  ...
  bool remove_from_tree(node *&t, const domain &d, domain &d_copy, range &r);
  /*!
      requires
  ...
  bool remove_from_tree(node *&t, const domain &item);
  /*!
      requires
  ...
  const range *return_reference(const node *t, const domain &d) const;
  /*!
      ensures
  ...
  range *return_reference(node *t, const domain &d);
  /*!
      ensures
          - if (there is a domain element equivalent to d in t) then
              - returns a pointer to the element in the range equivalent to d
  ...
  inline bool keep_node_balanced(node *&t);
  /*!
      requires
          - t != 0
          - t == reference to the pointer in t's parent node that points to t
  ...
  unsigned long get_count(const domain &item, node *tree_root) const;
  /*!
      requires
  ...
  /*!
      requires
          - t != 0
      ensures
          - deallocates the node pointed to by t and all of t's left and right
  children
  !*/

  void push(node *n) const {
    stack[stack_pos] = n;
    ++stack_pos;
  }
  /*!
  ...
  bool fix_stack(node *t, unsigned char depth = 0);
  /*!
      requires
          - current_element != 0
          - depth == 0
  ...
  bool remove_current_element_from_tree(node *&t, domain &d, range &r,
  ...
  /*!
      requires
          ...
          - current_element != 0
      ensures
          - removes the data in the node given by current_element and swaps it
  into #d and #r.
          - #t is still a binary search tree
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
inline void
swap(binary_search_tree_kernel_1<domain, range, mem_manager, compare> &a,
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
binary_search_tree_kernel_1<domain, range, mem_manager,
                            compare>::~binary_search_tree_kernel_1() {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager, compare>::clear() {
  if (tree_size > 0) {
    delete_tree(tree_root);
    tree_root = 0;
    tree_size = 0;
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
size_t
binary_search_tree_kernel_1<domain, range, mem_manager, compare>::size() const {
  return tree_size;
}
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
short binary_search_tree_kernel_1<domain, range, mem_manager, compare>::height()
    const {
  return tree_height;
}
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
unsigned long
binary_search_tree_kernel_1<domain, range, mem_manager, compare>::count(
    const domain &item) const {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager, compare>::add(
    domain &d, range &r) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager, compare>::remove(
    const domain &d, domain &d_copy, range &r) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager, compare>::destroy(
    const domain &item) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager,
                                 compare>::remove_any(domain &d, range &r) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
range *
binary_search_tree_kernel_1<domain, range, mem_manager, compare>::operator[](
    const domain &item) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager, compare>::swap(
    binary_search_tree_kernel_1<domain, range, mem_manager, compare> &item) {
  pool.swap(item.pool);
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager,
                                 compare>::remove_last_in_order(domain &d,
                                                                range &r) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager,
                                 compare>::remove_current_element(domain &d,
                                                                  range &r) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager,
                                 compare>::position_enumerator(const domain &d)
    const {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
bool binary_search_tree_kernel_1<domain, range, mem_manager,
                                 compare>::at_start() const {
  return at_start_;
}
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager, compare>::reset()
    const {
  at_start_ = true;
  current_element = 0;
  stack_pos = 0;
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
bool binary_search_tree_kernel_1<domain, range, mem_manager,
                                 compare>::current_element_valid() const {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
const map_pair<domain, range> &
binary_search_tree_kernel_1<domain, range, mem_manager, compare>::element()
    const {
...
bool binary_search_tree_kernel_1<domain, range, mem_manager,
  ...
  if (at_start_) {
    ...
    if (current_element == 0) {
      ...
      bool from_left =
          false; // true if we went up and were coming from a left child node
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager,
                                 compare>::double_rotate_right(node *&t) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_1<domain, range, mem_manager,
                                 compare>::double_rotate_left(node *&t) {
  node *temp = t;
  t = t->right->left;
...
bool binary_search_tree_kernel_1<domain, range, mem_manager, compare>::
    remove_biggest_element_in_tree(node *&t, domain &d, range &r) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
bool binary_search_tree_kernel_1<domain, range, mem_manager, compare>::
    remove_least_element_in_tree(node *&t, domain &d, range &r) {
  // make a reference to the current node so we don't have to dereference a
  // pointer a bunch of times
  node &tree = *t;
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
bool binary_search_tree_kernel_1<domain, range, mem_manager,
                                 compare>::add_to_tree(node *&t, domain &d,
                                                       range &r) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
bool binary_search_tree_kernel_1<domain, range, mem_manager, compare>::
    remove_current_element_from_tree(node *&t, domain &d, range &r,
  ...
  if (t == current_element) {
    ...
    if (tree.balance == -1) {
      ...
      balance +=
          remove_current_element_from_tree(tree.left, d, r, cur_stack_pos + 1);
  ...
  } else if ((cur_stack_pos < stack_pos &&
              stack[cur_stack_pos] == tree.right) ||
             tree.right == current_element) {
    // go right
    if (tree.balance == 1) {
      int balance = tree.balance;
      balance -=
          remove_current_element_from_tree(tree.right, d, r, cur_stack_pos + 1);
      tree.balance = balance;
      return !tree.balance;
    } else {
      int balance = tree.balance;
  ...
  // this return should never happen but do it anyway to suppress compiler
  // warnings
  return false;
}
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
bool binary_search_tree_kernel_1<domain, range, mem_manager,
                                 compare>::remove_from_tree(node *&t,
                                                            const domain &d,
                                                            domain &d_copy,
                                                            range &r) {
  // make a reference to the current node so we don't have to dereference
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
range *binary_search_tree_kernel_1<domain, range, mem_manager,
                                   compare>::return_reference(node *t,
                                                              const domain &d) {
  while (t != 0) {
...
const range *
binary_search_tree_kernel_1<domain, range, mem_manager,
                            compare>::return_reference(const node *t,
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
bool binary_search_tree_kernel_1<domain, range, mem_manager,
                                 compare>::keep_node_balanced(node *&t) {
  // make a reference to the current node so we don't have to dereference
  // a pointer a bunch of times
  node &tree = *t;
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
unsigned long
binary_search_tree_kernel_1<domain, range, mem_manager, compare>::get_count(
    const domain &d, node *tree_root) const {
  if (tree_root != 0) {
    if (comp(d, tree_root->d)) {
      ...
      return get_count(d, tree_root->left) + get_count(d, tree_root->right) + 1;
    }
  }
  return 0;
}
