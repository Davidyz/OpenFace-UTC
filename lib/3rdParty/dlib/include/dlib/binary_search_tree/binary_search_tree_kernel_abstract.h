...
class binary_search_tree : public enumerable<map_pair<domain, range>>,
  ...
  /*!
      REQUIREMENTS ON domain
          domain must be comparable by compare where compare is a functor
  compatible with std::less and domain is swappable by a global swap() and
  ...
  memory_manager_stateless/memory_manager_stateless_kernel_abstract.h
      ...
      ENUMERATION ORDER
          ...
          range element) elements in ascending order according to the compare
  functor. (i.e. the elements are enumerated in sorted order)
...
public:
  ...
  unsigned long count(const domain &d) const;
  /*!
      ensures
  ...
  /*!
      requires
          - size() > 0
      ensures
          - the last/biggest (according to the compare functor) element in the
  domain of *this has been removed and swapped into #d.  The element in the
  range of *this associated with #d has also been removed and swapped into #r.
          - #count(#d) == count(#d) - 1
          - #size() == size() - 1
          - #at_start() == true
  !*/
  ...
  /*!
      ...
      ensures
          - the current element given by element() has been removed and swapped
  into d and r.
          - #d == element().key()
          - #r == element().value()
          - #count(#d) == count(#d) - 1
  ...
  /*!
      ensures
          ...
          - if (count(d) > 0) then
              - #element().key() == d
          - else if (there are any items in the domain of *this that are bigger
  than d according to the compare functor) then
              - #element().key() == the smallest item in the domain of *this
  that is bigger than d according to the compare functor.
          - else
  ...
  void swap(binary_search_tree &item);
  /*!
      ensures
          - swaps *this and item
  !*/
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
inline void swap(binary_search_tree<domain, range, mem_manager, compare> &a,
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void deserialize(binary_search_tree<domain, range, mem_manager, compare> &item,
                 std::istream &in);
/*!
