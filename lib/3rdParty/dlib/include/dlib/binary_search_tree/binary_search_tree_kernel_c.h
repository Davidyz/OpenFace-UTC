...
template <typename bst_base>
inline void swap(binary_search_tree_kernel_c<bst_base> &a,
...
template <typename bst_base>
void binary_search_tree_kernel_c<bst_base>::add(domain &d, range &r) {
  DLIB_CASSERT(static_cast<const void *>(&d) != static_cast<void *>(&r),
               "\tvoid binary_search_tree::add"
                   << "\n\tyou can't call add() and give the same object to "
                      "both parameters."
                   << "\n\tthis:       " << this << "\n\t&d:         " << &d
                   << "\n\t&r:         " << &r
                   << "\n\tsize():     " << this->size());
...
template <typename bst_base>
void binary_search_tree_kernel_c<bst_base>::destroy(const domain &d) {
...
template <typename bst_base>
void binary_search_tree_kernel_c<bst_base>::remove(const domain &d,
                                                   domain &d_copy, range &r) {
  DLIB_CASSERT(
      this->operator[](d) != 0 &&
...
template <typename bst_base>
void binary_search_tree_kernel_c<bst_base>::remove_any(domain &d, range &r) {
  DLIB_CASSERT(
      this->size() != 0 &&
          (static_cast<const void *>(&d) != static_cast<void *>(&r)),
...
template <typename bst_base>
void binary_search_tree_kernel_c<bst_base>::remove_last_in_order(domain &d,
                                                                 range &r) {
...
template <typename bst_base>
void binary_search_tree_kernel_c<bst_base>::remove_current_element(domain &d,
                                                                   range &r) {
  DLIB_CASSERT(
      this->current_element_valid() == true,
