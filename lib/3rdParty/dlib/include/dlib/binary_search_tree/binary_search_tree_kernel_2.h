...
class binary_search_tree_kernel_2
  ...
  /*!
      ...
      CONVENTION
          ...
          for all nodes:
          {
              - left points to the left subtree or NIL if there is no left
  subtree
              ...
              - ordering of nodes is determined by comparing each node's d
  member
...
public:
  ...
  binary_search_tree_kernel_2()
      : NIL(pool.allocate()), tree_size(0), tree_root(NIL), current_element(0),
        at_start_(true) {
    NIL->color = black;
    NIL->left = 0;
    NIL->right = 0;
  ...
  inline void swap(binary_search_tree_kernel_2 &item);
...
private:
  ...
  void remove_biggest_element_in_tree(node *t, domain &d, range &r);
  /*!
      requires
  ...
  /*!
      ...
      ensures
          ...
          - #t is still a binary search tree
          - if (the node that was removed was the one pointed to by
  ...
  void add_to_tree(node *t, domain &d, range &r);
  /*!
      requires
          - t != NIL
      ensures
  ...
  void remove_from_tree(node *t, const domain &d, domain &d_copy, range &r);
  /*!
      requires
          - return_reference(t,d) != 0
      ensures
          - #d_copy is equivalent to d
          - the first element in t equivalent to d that is encountered when
  searching down the tree from t has been removed and swapped into #d_copy.
  Also, the associated range element has been removed and swapped into #r.
          - if (the node that got removed wasn't current_element) then
              - adjusts the current_element pointer if the data in the node that
  ...
  void remove_from_tree(node *t, const domain &d);
  /*!
      requires
          - return_reference(t,d) != 0
      ensures
  ...
  short tree_height(node *t) const;
  /*!
      ensures
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
inline void
swap(binary_search_tree_kernel_2<domain, range, mem_manager, compare> &a,
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
binary_search_tree_kernel_2<domain, range, mem_manager,
                            compare>::~binary_search_tree_kernel_2() {
  if (tree_root != NIL)
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_2<domain, range, mem_manager, compare>::clear() {
  if (tree_size > 0) {
    delete_tree(tree_root);
    tree_root = NIL;
    tree_size = 0;
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
size_t
binary_search_tree_kernel_2<domain, range, mem_manager, compare>::size() const {
  return tree_size;
}
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
short binary_search_tree_kernel_2<domain, range, mem_manager, compare>::height()
    const {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
unsigned long
binary_search_tree_kernel_2<domain, range, mem_manager, compare>::count(
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_2<domain, range, mem_manager, compare>::add(
    domain &d, range &r) {
  if (tree_size == 0) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_2<domain, range, mem_manager, compare>::remove(
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_2<domain, range, mem_manager, compare>::destroy(
    const domain &item) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_2<domain, range, mem_manager,
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
range *
binary_search_tree_kernel_2<domain, range, mem_manager, compare>::operator[](
    const domain &d) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_2<domain, range, mem_manager, compare>::swap(
    binary_search_tree_kernel_2<domain, range, mem_manager, compare> &item) {
  pool.swap(item.pool);
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
bool binary_search_tree_kernel_2<domain, range, mem_manager,
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_2<domain, range, mem_manager, compare>::reset()
    const {
  at_start_ = true;
  current_element = 0;
}
...
binary_search_tree_kernel_2<domain, range, mem_manager, compare>::element()
    const {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
range *binary_search_tree_kernel_2<domain, range, mem_manager,
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
void binary_search_tree_kernel_2<domain, range, mem_manager,
                                 compare>::fix_after_remove(node *t) {
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
short binary_search_tree_kernel_2<domain, range, mem_manager,
                                  compare>::tree_height(node *t) const {
  if (t == NIL)
    return 0;
...
template <typename domain, typename range, typename mem_manager,
          typename compare>
unsigned long
binary_search_tree_kernel_2<domain, range, mem_manager, compare>::get_count(
