...
class directed_graph : noncopyable {
  ...
  memory_manager_stateless/memory_manager_stateless_kernel_abstract.h
      ...
      POINTERS AND REFERENCES TO INTERNAL DATA
          The only time pointers or references to nodes or edges become invalid
  is when they reference nodes or edges that have been removed from a graph.

      INITIAL VALUE
          number_of_nodes() == 0

      WHAT THIS OBJECT REPRESENTS
          This object represents a directed graph which is a set of nodes with
  directed edges connecting various nodes.

          In this object if there is a directed edge from a node A to a node B
  then I say that A is the parent of B and B is the child of A.
...
public:
  ...
  struct node_type {
    ...
    unsigned long number_of_parents() const;
    /*!
        ensures
            - returns the number of parents of this node
    !*/

    unsigned long number_of_children() const;
    /*!
        ensures
    ...
    /*!
        requires
            - edge_index < number_of_parents()
        ensures
            - returns a non-const reference to the edge_index'th edge data for
    the edge connecting to node this->parent(edge_index)
    !*/
    ...
    /*!
        ...
        ensures
            - returns a non-const reference to the edge_index'th edge data for
    the edge connecting to node this->child(edge_index)
    !*/
  };
...
inline void swap(directed_graph<T, mem_manager> &a,
...
template <typename T, typename mem_manager>
void serialize(const directed_graph<T, mem_manager> &item, std::ostream &out);
/*!
    provides deserialization support
!*/

template <typename T, typename mem_manager>
void deserialize(directed_graph<T, mem_manager> &item, std::istream &in);
/*!
    provides deserialization support
!*/
