...
class sockstreambuf : public std::streambuf {
  /*!
      ...
      CONVENTION
          ...
          - max_putback == the maximum number of chars to have in the put back
  buffer.
  !*/
...
public:
  ...
  sockstreambuf(connection *con_)
      : con(*con_), out_buffer(0), in_buffer(0), autoflush(false) {
    init();
  }

  sockstreambuf(const std::unique_ptr<connection> &con_)
      : con(*con_), out_buffer(0), in_buffer(0), autoflush(false) {
    init();
  }
  ...
  void flush_output_on_read() { autoflush = true; }

  bool flushes_output_on_read() const { return autoflush; }

  void do_not_flush_output_on_read() { autoflush = false; }
...
protected:
  void init() {
    ...
    setg(in_buffer + max_putback, in_buffer + max_putback,
