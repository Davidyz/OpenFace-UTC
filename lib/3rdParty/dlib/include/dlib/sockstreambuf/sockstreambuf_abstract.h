...
class sockstreambuf : public std::streambuf {
  /*!
      ...
      NOTE:
          ...
          internally.  So if you need to ensure that data is actually sent then
  you should flush the stream.
      ...
      THREADING
          ...
          - You may call shutdown() on the connection object and this will
            ...
            will appear the same as hitting EOF.  (note that EOF for a
  sockstreambuf means that the connection has closed)
          - It is safe to read from and write to the sockstreambuf at the same
  time from different threads so long as flushes_output_on_read()==false.
          - It is not safe to try to putback a char and read from the stream
  from different threads
  !*/
public:
  sockstreambuf(connection *con);
  /*!
      requires
  ...
  sockstreambuf(const std::unique_ptr<connection> &con);
  /*!
      requires
          - con == a valid connection object
      ensures
  ...
  /*!
      ensures
          - This function returns true if this object will flush its output
  buffer to the network socket before performing any network read.
          - if (flushes_output_on_read() == true)
              - It is not safe to make concurrent read and write calls to this
  object.
  !*/
