...
public:
  random_color_transform(dlib::rand &rnd, const double gamma_magnitude = 0.5,
                         const double color_magnitude = 0.2) {
    // pick a random gamma correction factor.
    double gamma =
        std::max(0.0, 1 + gamma_magnitude * (rnd.get_random_double() - 0.5));
    ...
    const double m =
        255 * std::max(std::max(red_scale, green_scale), blue_scale);
    red_scale /= m;
    green_scale /= m;
    blue_scale /= m;

    // Now compute a lookup table for all the color channels.  The table tells
    // us what the transform does.
    table.resize(256 * 3);
    unsigned long i = 0;
    for (int k = 0; k < 256; ++k) {
...
void disturb_colors(image_type &img_, dlib::rand &rnd,
...
template <typename image_type>
void apply_random_color_offset(image_type &img_, dlib::rand &rnd) {
  // Make a random color offset.  This tform matrix came from looking at the
  // covariance matrix of RGB values in a bunch of images.  In particular, if
  // you multiply Gaussian random vectors by tform it will result in vectors
  // with the same covariance matrix as the original RGB data.  Also, this color
  // transform is what is suggested by the paper:
  //  Krizhevsky, Alex, Ilya Sutskever, and Geoffrey E. Hinton. "Imagenet
  //  classification with deep convolutional neural networks." Advances in
  //  neural information processing systems. 2012.
  // Except that we used the square root of the eigenvalues (which I'm pretty
  // sure is what the authors intended).
  matrix<double, 3, 3> tform;
  tform = -66.379, 25.094, 6.79698, -68.0492, -0.302309, -13.9539, -68.4907,
  -24.0199, 7.27653;
  matrix<double, 3, 1> v;
  v = rnd.get_random_gaussian(), rnd.get_random_gaussian(),
  ...
  // Make up lookup tables that apply the color mapping so we don't have to put
  // a bunch of complicated conditional branches in the loop below.
