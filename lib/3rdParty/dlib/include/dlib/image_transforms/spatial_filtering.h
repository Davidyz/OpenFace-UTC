...
template <typename in_image_type, typename out_image_type, typename EXP,
          typename T>
rectangle grayscale_spatially_filter_image(const in_image_type &in_img_,
                                           out_image_type &out_img_,
                                           const matrix_exp<EXP> &filter_,
                                           T scale, bool use_abs, bool add_to) {
  const_temp_matrix<EXP> filter(filter_);
  COMPILE_TIME_ASSERT(
      pixel_traits<
          typename image_traits<in_image_type>::pixel_type>::has_alpha ==
      false);
  COMPILE_TIME_ASSERT(
      pixel_traits<
          typename image_traits<out_image_type>::pixel_type>::has_alpha ==
      false);
...
template <typename in_image_type, typename out_image_type, typename EXP>
rectangle float_spatially_filter_image(const in_image_type &in_img_,
                                       out_image_type &out_img_,
                                       const matrix_exp<EXP> &filter_,
                                       bool add_to) {

  const_temp_matrix<EXP> filter(filter_);
  DLIB_ASSERT(filter.size() != 0, "\trectangle spatially_filter_image()"
...
struct is_float_filtering2 {
  const static bool value =
      is_same_type<typename image_traits<in_image_type>::pixel_type,
                   float>::value &&
      is_same_type<typename image_traits<out_image_type>::pixel_type,
                   float>::value &&
...
typename enable_if_c<
    pixel_traits<
        typename image_traits<out_image_type>::pixel_type>::grayscale &&
        is_float_filtering2<in_image_type, out_image_type, EXP>::value,
    rectangle>::type
spatially_filter_image(const in_image_type &in_img, out_image_type &out_img,
                       const matrix_exp<EXP> &filter, T scale,
                       bool use_abs = false, bool add_to = false) {
  if (use_abs == false) {
    if (scale == 1)
      return impl::float_spatially_filter_image(in_img, out_img, filter,
                                                add_to);
    else
      return impl::float_spatially_filter_image(in_img, out_img, filter / scale,
                                                add_to);
  } else {
    return impl::grayscale_spatially_filter_image(in_img, out_img, filter,
                                                  scale, true, add_to);
  }
}
...
spatially_filter_image(const in_image_type &in_img, out_image_type &out_img,
                       const matrix_exp<EXP> &filter, T scale,
                       bool use_abs = false, bool add_to = false) {
  return impl::grayscale_spatially_filter_image(in_img, out_img, filter, scale,
                                                use_abs, add_to);
}
...
typename disable_if_c<
    pixel_traits<typename image_traits<out_image_type>::pixel_type>::grayscale,
    rectangle>::type
spatially_filter_image(const in_image_type &in_img_, out_image_type &out_img_,
                       const matrix_exp<EXP> &filter_, T scale) {
  const_temp_matrix<EXP> filter(filter_);
  COMPILE_TIME_ASSERT(
      pixel_traits<
          typename image_traits<in_image_type>::pixel_type>::has_alpha ==
  ...
  for (long r = first_row; r < last_row; ++r) {
    for (long c = first_col; c < last_col; ++c) {
      ...
      typedef matrix<typename EXP::type, pixel_traits<pixel_type>::num, 1>
          ptype;
      ptype p;
      ptype temp;
      temp = 0;
      for (long m = 0; m < filter.nr(); ++m) {
        for (long n = 0; n < filter.nc(); ++n) {
          // pull out the current pixel and put it into p
          p = pixel_to_vector<typename EXP::type>(
              in_img[r - first_row + m][c - first_col + n]);
          temp += p * filter(m, n);
        }
      }
...
rectangle grayscale_spatially_filter_image_separable(
    const in_image_type &in_img_, out_image_type &out_img_,
    const matrix_exp<EXP1> &_row_filter, const matrix_exp<EXP2> &_col_filter,
    T scale, bool use_abs, bool add_to) {
  ...
  DLIB_ASSERT(scale != 0 && row_filter.size() != 0 && col_filter.size() != 0 &&
                  is_vector(row_filter) && is_vector(col_filter),
...
// This overload is optimized to use SIMD instructions when filtering float
// images with float filters.
template <typename in_image_type, typename out_image_type, typename EXP1,
          typename EXP2>
rectangle float_spatially_filter_image_separable(
  ...
  // variables.
  COMPILE_TIME_ASSERT(
      (is_float_filtering<in_image_type, out_image_type, EXP1, EXP2>::value ==
       true));
...
typename enable_if_c<
    pixel_traits<
        ...
        is_float_filtering<in_image_type, out_image_type, EXP1, EXP2>::value,
    rectangle>::type
spatially_filter_image_separable(const in_image_type &in_img,
                                 out_image_type &out_img,
                                 const matrix_exp<EXP1> &row_filter,
                                 const matrix_exp<EXP2> &col_filter, T scale,
                                 bool use_abs = false, bool add_to = false) {
  if (use_abs == false) {
    out_image_type scratch;
    if (scale == 1)
      return float_spatially_filter_image_separable(
          in_img, out_img, row_filter, col_filter, scratch, add_to);
    else
      return float_spatially_filter_image_separable(
          in_img, out_img, row_filter / scale, col_filter, scratch, add_to);
  } else {
    return impl::grayscale_spatially_filter_image_separable(
        in_img, out_img, row_filter, col_filter, scale, true, add_to);
  }
}
...
spatially_filter_image_separable(const in_image_type &in_img,
                                 ...
                                 bool use_abs = false, bool add_to = false) {
  return impl::grayscale_spatially_filter_image_separable(
      in_img, out_img, row_filter, col_filter, scale, use_abs, add_to);
}
...
typename disable_if_c<
    pixel_traits<typename image_traits<out_image_type>::pixel_type>::grayscale,
    rectangle>::type
spatially_filter_image_separable(const in_image_type &in_img_,
                                 out_image_type &out_img_,
                                 const matrix_exp<EXP1> &_row_filter,
                                 const matrix_exp<EXP2> &_col_filter, T scale) {
  const_temp_matrix<EXP1> row_filter(_row_filter);
  const_temp_matrix<EXP2> col_filter(_col_filter);
  COMPILE_TIME_ASSERT(
      pixel_traits<
...
rectangle spatially_filter_image_separable(const in_image_type &in_img,
  ...
  return spatially_filter_image_separable(in_img, out_img, row_filter,
                                          col_filter, 1);
}
...
rectangle spatially_filter_image_separable_down(
    const unsigned long downsample, const in_image_type &in_img_,
    out_image_type &out_img_, const matrix_exp<EXP1> &row_filter,
    const matrix_exp<EXP2> &col_filter, T scale, bool use_abs = false,
    bool add_to = false) {
  COMPILE_TIME_ASSERT(
      pixel_traits<
          typename image_traits<in_image_type>::pixel_type>::has_alpha ==
  ...
  DLIB_ASSERT(downsample > 0 && scale != 0 && row_filter.size() % 2 == 1 &&
                  col_filter.size() % 2 == 1 && is_vector(row_filter) &&
  ...
  const long last_row =
      (long)std::ceil((in_img.nr() - col_border) / downsample) - 1;
  const long last_col =
      (long)std::ceil((in_img.nc() - row_border) / downsample) - 1;

  // zero border pixels
  const rectangle non_border =
      rectangle(first_col, first_row, last_col, last_row);
  zero_border_pixels(out_img, non_border);
  ...
  for (long r = 0; r < temp_img.nr(); ++r) {
    for (long c = non_border.left(); c <= non_border.right(); ++c) {
      ...
      for (long n = 0; n < row_filter.size(); ++n) {
        // pull out the current pixel and put it into p
        p = get_pixel_intensity(
            in_img[r][c * downsample - row_filter.size() / 2 + n]);
        temp += p * row_filter(n);
      }
      temp_img[r][c] = temp;
    }
  ...
  for (long r = non_border.top(); r <= non_border.bottom(); ++r) {
    for (long c = non_border.left(); c <= non_border.right(); ++c) {
      ptype temp = 0;
      for (long m = 0; m < col_filter.size(); ++m) {
        temp += temp_img[r * downsample - col_filter.size() / 2 + m][c] *
                col_filter(m);
      }
...
rectangle spatially_filter_image_separable_down(
    ...
    const matrix_exp<EXP2> &col_filter) {
  return spatially_filter_image_separable_down(downsample, in_img, out_img,
                                               row_filter, col_filter, 1);
}
...
inline void separable_3x3_filter_block_grayscale(
    T (&block)[NR][NC], const in_image_type &img_, const long &r, const long &c,
  ...
  for (long rr = 0; rr < NR + 2; ++rr) {
    for (long cc = 0; cc < NC; ++cc) {
      row_filt[rr][cc] =
          get_pixel_intensity(img[r + rr - 1][c + cc - 1]) * fe1 +
  ...
  for (long rr = 0; rr < NR; ++rr) {
    for (long cc = 0; cc < NC; ++cc) {
      block[rr][cc] = (row_filt[rr][cc] * fe1 + row_filt[rr + 1][cc] * fm +
...
template <long NR, long NC, typename T, typename U, typename in_image_type>
inline void
separable_3x3_filter_block_rgb(T (&block)[NR][NC], const in_image_type &img_,
  ...
  for (long rr = 0; rr < NR + 2; ++rr) {
    for (long cc = 0; cc < NC; ++cc) {
      row_filt[rr][cc].red = img[r + rr - 1][c + cc - 1].red * fe1 +
                             img[r + rr - 1][c + cc].red * fm +
                             img[r + rr - 1][c + cc + 1].red * fe2;
      row_filt[rr][cc].green = img[r + rr - 1][c + cc - 1].green * fe1 +
                               img[r + rr - 1][c + cc].green * fm +
                               img[r + rr - 1][c + cc + 1].green * fe2;
      row_filt[rr][cc].blue = img[r + rr - 1][c + cc - 1].blue * fe1 +
                              img[r + rr - 1][c + cc].blue * fm +
                              img[r + rr - 1][c + cc + 1].blue * fe2;
    }
  }

  for (long rr = 0; rr < NR; ++rr) {
    for (long cc = 0; cc < NC; ++cc) {
      block[rr][cc].red = row_filt[rr][cc].red * fe1 +
                          row_filt[rr + 1][cc].red * fm +
                          row_filt[rr + 2][cc].red * fe2;
      block[rr][cc].green = row_filt[rr][cc].green * fe1 +
                            row_filt[rr + 1][cc].green * fm +
                            row_filt[rr + 2][cc].green * fe2;
      block[rr][cc].blue = row_filt[rr][cc].blue * fe1 +
                           row_filt[rr + 1][cc].blue * fm +
                           row_filt[rr + 2][cc].blue * fe2;
    }
  }
}
...
template <typename T>
matrix<T, 0, 1> create_gaussian_filter(double sigma, int max_size) {
...
rectangle gaussian_blur(const in_image_type &in_img, out_image_type &out_img,
  ...
  if (sigma < 18) {
    typedef typename pixel_traits<
        typename image_traits<out_image_type>::pixel_type>::basic_pixel_type
        type;
    typedef typename promote<type>::type ptype;
    const matrix<ptype, 0, 1> &filt =
        create_gaussian_filter<ptype>(sigma, max_size);
...
namespace impl {
template <bool add_to, typename image_type1, typename image_type2>
void sum_filter(const image_type1 &img_, image_type2 &out_,
                const rectangle &rect) {
  const_image_view<image_type1> img(img_);
  image_view<image_type2> out(out_);
  DLIB_ASSERT(
      img.nr() == out.nr() && img.nc() == out.nc() &&
  ...
  for (long r = 0; r < img.nr(); ++r) {
    // set to sum at point(-1,r). i.e. should be equal to sum(mat(img),
    // translate_rect(rect, point(-1,r))) We compute it's value in the next
    // loop.
    ptype cur_sum = 0;

    // Update the first part of column_sum since we only work on the c+width
    // part of column_sum in the main loop.
    const long top = r + rect.top() - 1;
    const long bottom = r + rect.bottom();
    for (long k = 0; k < width; ++k) {
      const long right = k - width + rect.right();

      const ptype br_corner =
          area.contains(right, bottom) ? img[bottom][right] : 0;
    ...
    for (long c = 0; c < img.nc(); ++c) {
      ...
      if (add_to)
        out[r][c] +=
            static_cast<typename image_traits<image_type2>::pixel_type>(
                cur_sum);
      else
        out[r][c] = static_cast<typename image_traits<image_type2>::pixel_type>(
            cur_sum);
    }
  }
}
...
public:
  ...
  const T &front() const { return data[first]; }

  const T &back() const { return data[last]; }
...
template <typename image_type1, typename image_type2>
void max_filter(image_type1 &img_, image_type2 &out_, const long width,
                const long height,
                const typename image_traits<image_type1>::pixel_type &thresh) {
  image_view<image_type1> img(img_);
  image_view<image_type2> out(out_);
  DLIB_ASSERT(
      width > 0 && height > 0 && out.nr() == img.nr() && out.nc() == img.nc() &&
          is_same_object(img_, out_) == false,
      "\t void max_filter()"
          << "\n\t Invalid arguments given to this function."
          << "\n\t img.nr(): " << img.nr() << "\n\t img.nc(): " << img.nc()
  ...
  dlib::impl::fast_deque<std::pair<long, pixel_type>> Q(
