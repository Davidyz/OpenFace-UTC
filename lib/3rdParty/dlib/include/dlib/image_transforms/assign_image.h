...
void impl_assign_image(image_view<dest_image_type> &dest,
...
template <typename dest_image_type, typename src_image_type>
void impl_assign_image_scaled(image_view<dest_image_type> &dest,
                              const src_image_type &src, const double thresh) {
  DLIB_ASSERT(thresh > 0, "\tvoid assign_image_scaled()"
  ...
  if (pixel_traits<dest_pixel>::max() >=
          pixel_traits<typename src_image_type::type>::max() &&
      pixel_traits<dest_pixel>::min() <=
          pixel_traits<typename src_image_type::type>::min()) {
    impl_assign_image(dest, src);
    return;
  }
  ...
  typedef typename pixel_traits<typename src_image_type::type>::basic_pixel_type
      spix_type;
  ...
  // Figure out the range of pixel values based on image statistics.  There
  // might be some huge outliers so don't just pick the min and max values.
  ...
  const double scale =
      (upper != lower) ? ((dest_max - dest_min) / (upper - lower)) : 0;
...
template <typename dest_image_type, typename src_image_type>
void impl_assign_image_scaled(dest_image_type &dest_, const src_image_type &src,
                              const double thresh) {
  image_view<dest_image_type> dest(dest_);
...
template <typename dest_image_type, typename src_image_type>
void assign_image_scaled(dest_image_type &dest, const src_image_type &src,
...
void assign_all_pixels(image_view<dest_image_type> &dest_img,
...
template <typename dest_image_type, typename src_pixel_type>
void assign_all_pixels(dest_image_type &dest_img_,
                       const src_pixel_type &src_pixel) {
...
void assign_border_pixels(
    image_view<image_type> &img, long x_border_size, long y_border_size,
...
template <typename image_type>
void assign_border_pixels(
    image_type &img_, long x_border_size, long y_border_size,
    const typename image_traits<image_type>::pixel_type &p) {
...
template <typename image_type>
void zero_border_pixels(image_view<image_type> &img, long x_border_size,
                        long y_border_size) {
  DLIB_ASSERT(x_border_size >= 0 && y_border_size >= 0,
              "\tvoid zero_border_pixels(img, p, border_size)"
                  << "\n\tYou have given an invalid border_size"
