...
namespace impl {
template <typename T> inline T edge_diff_uint(const T &a, const T &b) {
  if (a > b)
    return a - b;
  else
    return b - a;
...
template <typename T, typename enabled = void> struct edge_diff_funct {
  typedef double diff_type;

  template <typename pixel_type>
  double operator()(const pixel_type &a, const pixel_type &b) const {
    return length(pixel_to_vector<double>(a) - pixel_to_vector<double>(b));
  }
};

template <> struct edge_diff_funct<uint8, void> {
  typedef uint8 diff_type;
  uint8 operator()(const uint8 &a, const uint8 &b) const {
    return edge_diff_uint(a, b);
  }
};

template <> struct edge_diff_funct<uint16, void> {
  typedef uint16 diff_type;
  uint16 operator()(const uint16 &a, const uint16 &b) const {
    return edge_diff_uint(a, b);
  }
};

template <> struct edge_diff_funct<uint32, void> {
  typedef uint32 diff_type;
  uint32 operator()(const uint32 &a, const uint32 &b) const {
    return edge_diff_uint(a, b);
  }
};

template <> struct edge_diff_funct<double, void> {
  typedef double diff_type;
...
struct edge_diff_funct<T, typename enable_if<is_matrix<T>>::type> {
  typedef double diff_type;
  double operator()(const T &a, const T &b) const { return length(a - b); }
};

// ------------------------------------------------------------------------------------

template <typename T> struct graph_image_segmentation_data_T {
...
template <typename T> struct segment_image_edge_data_T {
  segment_image_edge_data_T() {}

  segment_image_edge_data_T(const rectangle &rect, const point &p1,
                            const point &p2, const T &diff_)
      : idx1(p1.y() * rect.width() + p1.x()),
        idx2(p2.y() * rect.width() + p2.x()), diff(diff_) {}
...
template <typename image_view_type> struct uint8_or_uint16_pixels {
...
get_pixel_edges(const in_image_type &in_img,
  ...
  const unsigned long num_edges = shrink_rect(area, 1).area() * 4 +
                                  in_img.nr() * 2 * 3 - 4 +
                                  (in_img.nc() - 2) * 2 * 3;
  typedef segment_image_edge_data_T<T> segment_image_edge_data;
  sorted_edges.resize(num_edges);

  // integrate counts.  The idea is to have sorted_edges[counts[i]] be the
  // location that edges with an edge_diff of i go.  So counts[0] == 0,
  // counts[1] == number of 0 edge diff edges, etc.
  ...
  while (be.move_next()) {
    ...
    if (area.contains(c - 1, r)) {
      ...
      sorted_edges[counts[diff]++] =
          segment_image_edge_data(area, p, point(c - 1, r), diff);
    }
    ...
    if (area.contains(c, r - 1)) {
      ...
      sorted_edges[counts[diff]++] =
          segment_image_edge_data(area, p, point(c, r - 1), diff);
    }
  ...
  // same thing as the above loop but now we do it on the interior of the image
  // and therefore don't have to include the boundary checking if statements
  // used above.
  for (long r = 1; r + 1 < in_img.nr(); ++r) {
    for (long c = 1; c + 1 < in_img.nc(); ++c) {
      const point p(c, r);
      ...
      sorted_edges[counts[diff]++] =
          segment_image_edge_data(area, p, point(c + 1, r - 1), diff);
      diff = edge_diff(pix, in_img[r + 1][c + 1]);
      sorted_edges[counts[diff]++] =
          segment_image_edge_data(area, p, point(c + 1, r + 1), diff);
...
// This is the general purpose version of get_pixel_edges().  It handles all
// pixel types.
template <typename in_image_type, typename T>
...
void segment_image(const in_image_type &in_img_, out_image_type &out_img_,
  ...
  COMPILE_TIME_ASSERT(
      is_unsigned_type<
          typename image_traits<out_image_type>::pixel_type>::value);
  ...
  for (unsigned long i = 0; i < sorted_edges.size(); ++i) {
    ...
    if (set1 != set2) {
      ...
      const diff_type tau1 =
          static_cast<diff_type>(k / data[set1].component_size);
      const diff_type tau2 =
          static_cast<diff_type>(k / data[set2].component_size);
      ...
      if (diff <= mint) {
        ...
        data[new_set].component_size =
            data[set1].component_size + data[set2].component_size;
  ...
  if (min_size != 0) {
    for (unsigned long i = 0; i < sorted_edges.size(); ++i) {
      ...
      unsigned long set1 = sets.find_set(idx1);
      unsigned long set2 = sets.find_set(idx2);
      if (set1 != set2 && (data[set1].component_size < min_size ||
                           data[set2].component_size < min_size)) {
        const unsigned long new_set = sets.merge_sets(set1, set2);
        data[new_set].component_size =
            data[set1].component_size + data[set2].component_size;
...
template <typename in_image_type, typename diff_type>
void find_basic_candidate_object_locations(
    const in_image_type &in_img,
    const std::vector<dlib::impl::segment_image_edge_data_T<diff_type>>
        &sorted_edges,
    std::vector<rectangle> &out_rects, std::vector<edge_data> &edges,
    const double k, const unsigned long min_size) {
  using namespace dlib::impl;
  ...
  std::pair<unsigned long, unsigned long> last_blob_edge(
  ...
  for (std::map<unsigned long, rectangle>::iterator i = boxes.begin();
       i != boxes.end(); ++i) {
  ...
  dlib::set<std::pair<unsigned long, unsigned long>, mm_type>::kernel_1a
      neighbors_final;
  for (unsigned long i = 0; i < rejected_edges.size(); ++i) {
    const unsigned long idx1 = rejected_edges[i].idx1;
    const unsigned long idx2 = rejected_edges[i].idx2;
    ...
    if (set1 != set2) {
      ...
      if (!neighbors_final.is_member(p)) {
        ...
        edge_data temp;
        const diff_type mint =
            std::min(data[set1].internal_diff, data[set2].internal_diff);
...
void find_candidate_object_locations(
    const in_image_type &in_img_, std::vector<rectangle> &rects,
    const matrix_exp<EXP> &kvals, const unsigned long min_size = 20,
  ...
  for (long j = 0; j < kvals.size(); ++j) {
    ...
    find_basic_candidate_object_locations(in_img, sorted_edges, working_rects,
                                          edges, k, min_size);
    ...
    //    Segmentation as Selective Search for Object Recognition" by Koen E. A.
    //    van de Sande, et al.
    ...
    bool did_merge = true;
    for (unsigned long iter = 0; did_merge && iter < max_merging_iterations;
         ++iter) {
      did_merge = false;
      sets.clear();
      ...
      for (unsigned long i = 0; i < edges.size(); ++i) {
        ...
        if (temp.set1 != temp.set2) {
          rectangle merged_rect =
              working_rects[temp.set1] + working_rects[temp.set2];
...
template <typename in_image_type>
void find_candidate_object_locations(const in_image_type &in_img,
...
namespace impl {
template <typename T>
void mbd_raster_scan(const T &img, array2d<rgb_pixel> &dist,
                     array2d<rgb_pixel> &lower, array2d<rgb_pixel> &upper,
...
void mbd_raster_scan(const T &img, U &dist, array2d<pixel_type> &lower,
                     array2d<pixel_type> &upper, bool do_left_right_scans) {
  auto area = shrink_rect(get_rect(img), 1);

  auto check_neighbor = [&](long r, long c, long neighbor_r, long neighbor_c) {
    auto l =
        std::min(lower[neighbor_r][neighbor_c], get_pixel_intensity(img[r][c]));
    auto u =
        std::max(upper[neighbor_r][neighbor_c], get_pixel_intensity(img[r][c]));
    auto d = u - l;
    if (d < dist[r][c]) {
      lower[r][c] = l;
      upper[r][c] = u;
...
template <typename in_image_type, typename out_image_type>
typename disable_if_c<is_rgb_image<in_image_type>::value>::type
min_barrier_distance(const in_image_type &img_, out_image_type &dist_,
                     size_t iterations = 10, bool do_left_right_scans = true) {
...
template <typename in_image_type, typename out_image_type>
typename enable_if_c<is_rgb_image<in_image_type>::value>::type
min_barrier_distance(const in_image_type &img_, out_image_type &dist_,
                     size_t iterations = 10, bool do_left_right_scans = true) {
  DLIB_CASSERT(iterations > 0);
