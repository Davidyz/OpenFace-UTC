...
public:
  explicit hough_transform(unsigned long size_) : _size(size_) {
  ...
  long nr() const { return _size; }

  long nc() const { return _size; }

  std::pair<point, point> get_line(const point &p) const {
    DLIB_ASSERT(rectangle(0, 0, size() - 1, size() - 1).contains(p) == true,
                "\t pair<point,point> hough_transform::get_line(point)"
                    << "\n\t Invalid arguments given to this function."
                    << "\n\t p:      " << p << "\n\t size(): " << size());

    // First we compute the radius measured in pixels from the center and the
  ...
  double get_line_angle_in_degrees(const point &p) const {
    double angle, radius;
    get_line_properties(p, angle, radius);
    return angle;
  }

  void get_line_properties(const point &p, double &angle_in_degrees,
                           double &radius) const {
  ...
  point get_best_hough_point(const point &p, const image_type &himg_) {
  ...
  void operator()(const in_image_type &img_, const rectangle &box,
                  out_image_type &himg_) const {
    ...
    auto record_hit = [&](const point &hough_point, const point & /*img_point*/,
                          const in_pixel_type &val) {
  ...
  template <typename in_image_type, typename out_image_type>
  void operator()(const in_image_type &img_, out_image_type &himg_) const {
  ...
  std::vector<std::vector<point>> find_pixels_voting_for_lines(
      const in_image_type &img, const rectangle &box,
      const std::vector<point> &hough_points,
      const unsigned long angle_window_size = 1,
      const unsigned long radius_window_size = 1) const {
    ...
    for (auto &p : hough_points)
      DLIB_CASSERT(get_rect(*this).contains(p),
                   "You gave a hough_points that isn't actually in the Hough "
    ...
    // make a map that lets us look up in constant time if a hough point is in
    // the constituent_points output and if so where.
    matrix<uint32> hmap(size(), size());
    hmap = hough_points.size();
    for (size_t i = 0; i < hough_points.size(); ++i) {
      rectangle area =
          centered_rect(hough_points[i], angle_window_size, radius_window_size)
              .intersect(get_rect(hmap));
  ...
  template <typename in_image_type>
  std::vector<std::vector<point>> find_pixels_voting_for_lines(
      const in_image_type &img, const std::vector<point> &hough_points,
      const unsigned long angle_window_size = 1,
      const unsigned long radius_window_size = 1) const {
    rectangle box(0, 0, num_columns(img) - 1, num_rows(img) - 1);
    return find_pixels_voting_for_lines(img, box, hough_points,
                                        angle_window_size, radius_window_size);
  }

  template <typename image_type, typename thresh_type>
  std::vector<point> find_strong_hough_points(
      const image_type &himg_, const thresh_type hough_count_threshold,
      const double angle_nms_thresh, const double radius_nms_thresh) {
    ...
    // Now do non-max suppression.  First, sort the initial_lines so the best
    // lines come first.
    std::sort(
        initial_lines.rbegin(), initial_lines.rend(),
        [](const std::pair<double, point> &a,
           const std::pair<double, point> &b) { return a.first < b.first; });
    for (auto &r : initial_lines) {
      ...
      for (auto &ref : final_angle_and_radius) {
        ...
        if ((std::abs(ref_angle - angle) < angle_nms_thresh &&
             std::abs(ref_radius - radius) < radius_nms_thresh) ||
            (180 - std::abs(ref_angle - angle) < angle_nms_thresh &&
             std::abs(ref_radius + radius) < radius_nms_thresh)) {
          too_close = true;
          break;
        }
  ...
  template <typename in_image_type, typename record_hit_function_type>
  void
  perform_generic_hough_transform(const in_image_type &img_,
                                  const rectangle &box,
                                  record_hit_function_type record_hit) const {
    ...
    for (long r = area.top(); r <= area.bottom(); ++r) {
      ...
      for (long c = area.left(); c <= area.right(); ++c) {
        ...
        if (val != 0) {
          ...
          {
              double theta = t*pi/even_size;
              double radius = (x*std::cos(theta) + y*std::sin(theta))/sqrt_2 +
          even_size/2 + 0.5; long rr = static_cast<long>(radius);
...
private:
  unsigned long _size;
  unsigned long
      even_size; // equal to _size if _size is even, otherwise equal to _size-1.
