...
template <typename T> class integral_image_generic : noncopyable {
  /*!
      REQUIREMENTS ON T
...
public:
  ...
  value_type get_sum_of_area(const rectangle &rect) const;
  /*!
      requires
          - rect.is_empty() == false
...
template <typename T>
void swap(integral_image_generic<T> &a, integral_image_generic<T> &b) {
  a.swap(b);
}
/*!
...
typename integral_image_type::value_type haar_x(const integral_image_type &img,
                                                const point &p, long width)
    /*!
        requires
            - get_rect(img).contains(centered_rect(p,width,width)) == true
            - integral_image_type == a type that implements the
    integral_image_generic interface defined above ensures
            - returns the response of a Haar wavelet centered at the point p
              ...
              and has the following shape:
                ...
                ----++++
                ----++++
              That is, the wavelet is square and computes the sum of pixels on
    the right minus the sum of pixels on the left.
    !*/

    // ----------------------------------------------------------------------------------------

    template <typename integral_image_type>
    typename integral_image_type::value_type
    haar_y(const integral_image_type &img, const point &p, long width)
/*!
    requires
        - get_rect(img).contains(centered_rect(p,width,width)) == true
        - integral_image_type == a type that implements the
