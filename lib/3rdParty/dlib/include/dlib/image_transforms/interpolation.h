...
template <typename T> struct sub_image_proxy {
  sub_image_proxy() = default;

  sub_image_proxy(T &img, rectangle rect) {
    ...
    _data = (char *)image_data(img) + sizeof(pixel_type) * rect.left() +
            rect.top() * _width_step;
  }
...
template <typename T> struct const_sub_image_proxy {
  const_sub_image_proxy() = default;

  const_sub_image_proxy(const T &img, rectangle rect) {
    rect = rect.intersect(get_rect(img));
    typedef typename image_traits<T>::pixel_type pixel_type;
    ...
    _nc = rect.width();
    _width_step = width_step(img);
    _data = (const char *)image_data(img) + sizeof(pixel_type) * rect.left() +
            rect.top() * _width_step;
  }
...
template <typename T> struct image_traits<sub_image_proxy<T>> {
  typedef typename image_traits<T>::pixel_type pixel_type;
};
template <typename T> struct image_traits<const sub_image_proxy<T>> {
  typedef typename image_traits<T>::pixel_type pixel_type;
};
template <typename T> struct image_traits<const_sub_image_proxy<T>> {
  typedef typename image_traits<T>::pixel_type pixel_type;
};
template <typename T> struct image_traits<const const_sub_image_proxy<T>> {
  typedef typename image_traits<T>::pixel_type pixel_type;
};

template <typename T> inline long num_rows(const sub_image_proxy<T> &img) {
  return img._nr;
}
template <typename T> inline long num_columns(const sub_image_proxy<T> &img) {
  return img._nc;
}

template <typename T>
inline long num_rows(const const_sub_image_proxy<T> &img) {
  return img._nr;
}
template <typename T>
inline long num_columns(const const_sub_image_proxy<T> &img) {
  return img._nc;
}

template <typename T> inline void *image_data(sub_image_proxy<T> &img) {
  return img._data;
}
template <typename T>
...
template <typename T> inline long width_step(const sub_image_proxy<T> &img) {
  return img._width_step;
}

template <typename T>
inline long width_step(const const_sub_image_proxy<T> &img) {
  return img._width_step;
}

template <typename T>
void set_image_size(sub_image_proxy<T> &img, long rows, long cols) {
  DLIB_CASSERT(img._nr == rows && img._nc == cols,
               "A sub_image can't be resized."
                   << "\n\t img._nr: " << img._nr << "\n\t img._nc: " << img._nc
                   << "\n\t rows:    " << rows << "\n\t cols:    " << cols);
}
...
template <typename T>
inline sub_image_proxy<matrix<T>> sub_image(T *img, long nr, long nc,
                                            long row_stride) {
...
template <typename T>
inline const const_sub_image_proxy<matrix<T>>
...
class interpolate_bilinear {
  template <typename T> struct is_rgb_image {
...
public:
  ...
  typename disable_if<is_rgb_image<image_view_type>, bool>::type
  operator()(const image_view_type &img, const dlib::vector<T, 2> &p,
             pixel_type &result) const {
    COMPILE_TIME_ASSERT(
        pixel_traits<typename image_view_type::pixel_type>::has_alpha == false);
  ...
  template <typename T, typename image_view_type, typename pixel_type>
  typename enable_if<is_rgb_image<image_view_type>, bool>::type
  operator()(const image_view_type &img, const dlib::vector<T, 2> &p,
             pixel_type &result) const {
    COMPILE_TIME_ASSERT(
        pixel_traits<typename image_view_type::pixel_type>::has_alpha == false);
...
public:
  ...
  operator()(const image_view_type &img, const dlib::vector<T, 2> &p,
    ...
    const double temp =
        interpolate(p - pp, img[r - 1][c - 1], img[r - 1][c], img[r - 1][c + 1],
                    img[r][c - 1], img[r][c], img[r][c + 1], img[r + 1][c - 1],
  ...
  operator()(const image_view_type &img, const dlib::vector<T, 2> &p,
    ...
    const double red = interpolate(
        p - pp, img[r - 1][c - 1].red, img[r - 1][c].red, img[r - 1][c + 1].red,
        img[r][c - 1].red, img[r][c].red, img[r][c + 1].red,
        img[r + 1][c - 1].red, img[r + 1][c].red, img[r + 1][c + 1].red);
    const double green = interpolate(
        p - pp, img[r - 1][c - 1].green, img[r - 1][c].green,
        img[r - 1][c + 1].green, img[r][c - 1].green, img[r][c].green,
        img[r][c + 1].green, img[r + 1][c - 1].green, img[r + 1][c].green,
        img[r + 1][c + 1].green);
    const double blue =
        interpolate(p - pp, img[r - 1][c - 1].blue, img[r - 1][c].blue,
                    img[r - 1][c + 1].blue, img[r][c - 1].blue, img[r][c].blue,
                    img[r][c + 1].blue, img[r + 1][c - 1].blue,
                    img[r + 1][c].blue, img[r + 1][c + 1].blue);
...
private:
  /*  tl tm tr
      ...
      bl bm br
  */
  // The above is the pixel layout in our little 3x3 neighborhood. interpolate()
  // will fit a quadratic to these 9 pixels and then use that quadratic to find
  // the interpolated value at point p.
  inline double interpolate(const dlib::vector<double, 2> &p, double tl,
                            double tm, double tr, double ml, double mm,
                            double mr, double bl, double bm, double br) const {
    matrix<double, 6, 1> w;
    // x
    ...
    // x^2
    w(2) = (tl + tr + ml + mr + bl + br) * 0.16666666666 -
           (tm + mm + bm) * 0.333333333;
    // x*y
    w(3) = (tl - tr - bl + br) * 0.25;
    // y^2
    w(4) = (tl + tm + tr + bl + bm + br) * 0.16666666666 -
           (ml + mm + mr) * 0.333333333;
    // 1 (constant term)
    w(5) = (tm + ml + mr + bm) * 0.222222222 -
           (tl + tr + bl + br) * 0.11111111 + (mm)*0.55555556;
...
void transform_image(const image_type1 &in_img, image_type2 &out_img,
  ...
  DLIB_ASSERT(get_rect(out_img).contains(area) == true &&
              ...
              "\t void transform_image()"
                  ...
                  << "\n\t get_rect(out_img).contains(area): "
                  << get_rect(out_img).contains(area)
  ...
  for (long r = area.top(); r <= area.bottom(); ++r) {
    for (long c = area.left(); c <= area.right(); ++c) {
      if (!interp(imgv, map_point(dlib::vector<double, 2>(c, r)),
                  out_imgv[r][c]))
...
point_transform_affine rotate_image(const image_type1 &in_img,
  ...
  point_transform_affine trans = point_transform_affine(
      R, -R * dcenter(get_rect(out_img)) + dcenter(rimg));
...
public:
  helper_resize_image(double x_scale_, double y_scale_)
      : x_scale(x_scale_), y_scale(y_scale_) {}

  dlib::vector<double, 2> operator()(const dlib::vector<double, 2> &p) const {
...
void resize_image(const image_type1 &in_img, image_type2 &out_img,
  ...
  const double x_scale = (num_columns(in_img) - 1) /
                         (double)std::max<long>((num_columns(out_img) - 1), 1);
  const double y_scale = (num_rows(in_img) - 1) /
                         (double)std::max<long>((num_rows(out_img) - 1), 1);
...
template <typename image_type1, typename image_type2>
typename disable_if_c<(is_rgb_image<image_type1>::value &&
                       is_rgb_image<image_type2>::value) ||
                      (is_grayscale_image<image_type1>::value &&
                       is_grayscale_image<image_type2>::value)>::type
resize_image(const image_type1 &in_img_, image_type2 &out_img_,
             interpolate_bilinear) {
  // make sure requires clause is not broken
  DLIB_ASSERT(is_same_object(in_img_, out_img_) == false,
              "\t void resize_image()"
                  << "\n\t Invalid inputs were given to this function."
                  << "\n\t is_same_object(in_img_, out_img_):  "
                  << is_same_object(in_img_, out_img_));
  ...
  const double x_scale =
      (in_img.nc() - 1) / (double)std::max<long>((out_img.nc() - 1), 1);
  const double y_scale =
      (in_img.nr() - 1) / (double)std::max<long>((out_img.nr() - 1), 1);
  double y = -y_scale;
  for (long r = 0; r < out_img.nr(); ++r) {
    ...
    if (pixel_traits<U>::grayscale) {
      ...
      for (long c = 0; c < out_img.nc(); ++c) {
        ...
        assign_pixel(temp, 0);
        vector_to_pixel(
            temp, (1 - tb_frac) * ((1 - lr_frac) * pixel_to_vector<double>(tl) +
                                   lr_frac * pixel_to_vector<double>(tr)) +
                      tb_frac * ((1 - lr_frac) * pixel_to_vector<double>(bl) +
                                 lr_frac * pixel_to_vector<double>(br)));
        assign_pixel(out_img[r][c], temp);
      }
    }
  }
...
template <typename image_type, typename image_type2>
typename enable_if_c<
    is_grayscale_image<image_type>::value &&
    is_grayscale_image<image_type2>::value &&
    images_have_same_pixel_types<image_type, image_type2>::value>::type
resize_image(const image_type &in_img_, image_type2 &out_img_,
             interpolate_bilinear) {
  // make sure requires clause is not broken
  DLIB_ASSERT(is_same_object(in_img_, out_img_) == false,
              "\t void resize_image()"
  ...
  for (long r = 0; r < out_img.nr(); ++r) {
    ...
    for (;; c += 4) {
      ...
      if (fright[3] >= in_img.nc())
        break;
      simd4f tl(in_img[top][fleft[0]], in_img[top][fleft[1]],
                in_img[top][fleft[2]], in_img[top][fleft[3]]);
      simd4f tr(in_img[top][fright[0]], in_img[top][fright[1]],
                in_img[top][fright[2]], in_img[top][fright[3]]);
      simd4f bl(in_img[bottom][fleft[0]], in_img[bottom][fleft[1]],
                in_img[bottom][fleft[2]], in_img[bottom][fleft[3]]);
      simd4f br(in_img[bottom][fright[0]], in_img[bottom][fright[1]],
                in_img[bottom][fright[2]], in_img[bottom][fright[3]]);
...
template <typename image_type>
typename enable_if<is_rgb_image<image_type>>::type
resize_image(const image_type &in_img_, image_type &out_img_,
             interpolate_bilinear) {
  // make sure requires clause is not broken
  DLIB_ASSERT(is_same_object(in_img_, out_img_) == false,
              "\t void resize_image()"
  ...
  for (long r = 0; r < out_img.nr(); ++r) {
    ...
    for (;; c += 4) {
      ...
      if (fright[3] >= in_img.nc())
        break;
      simd4f tl(in_img[top][fleft[0]].red, in_img[top][fleft[1]].red,
                in_img[top][fleft[2]].red, in_img[top][fleft[3]].red);
      simd4f tr(in_img[top][fright[0]].red, in_img[top][fright[1]].red,
                in_img[top][fright[2]].red, in_img[top][fright[3]].red);
      simd4f bl(in_img[bottom][fleft[0]].red, in_img[bottom][fleft[1]].red,
                in_img[bottom][fleft[2]].red, in_img[bottom][fleft[3]].red);
      simd4f br(in_img[bottom][fright[0]].red, in_img[bottom][fright[1]].red,
                in_img[bottom][fright[2]].red, in_img[bottom][fright[3]].red);
      ...
      tl = simd4f(in_img[top][fleft[0]].green, in_img[top][fleft[1]].green,
                  in_img[top][fleft[2]].green, in_img[top][fleft[3]].green);
      tr = simd4f(in_img[top][fright[0]].green, in_img[top][fright[1]].green,
                  in_img[top][fright[2]].green, in_img[top][fright[3]].green);
      bl = simd4f(
          in_img[bottom][fleft[0]].green, in_img[bottom][fleft[1]].green,
          in_img[bottom][fleft[2]].green, in_img[bottom][fleft[3]].green);
      br = simd4f(
          in_img[bottom][fright[0]].green, in_img[bottom][fright[1]].green,
          in_img[bottom][fright[2]].green, in_img[bottom][fright[3]].green);
      ...
      tl = simd4f(in_img[top][fleft[0]].blue, in_img[top][fleft[1]].blue,
                  in_img[top][fleft[2]].blue, in_img[top][fleft[3]].blue);
      tr = simd4f(in_img[top][fright[0]].blue, in_img[top][fright[1]].blue,
                  in_img[top][fright[2]].blue, in_img[top][fright[3]].blue);
      bl = simd4f(in_img[bottom][fleft[0]].blue, in_img[bottom][fleft[1]].blue,
                  in_img[bottom][fleft[2]].blue, in_img[bottom][fleft[3]].blue);
      br = simd4f(
          in_img[bottom][fright[0]].blue, in_img[bottom][fright[1]].blue,
          in_img[bottom][fright[2]].blue, in_img[bottom][fright[3]].blue);
      out = simd4i(tlf * tl + trf * tr + blf * bl + brf * br);
      out.store(fout);
...
void resize_image(double size_scale, image_type &img) {
  ...
  image_type temp;
  set_image_size(temp, std::round(size_scale * num_rows(img)),
                 std::round(size_scale * num_columns(img)));
...
point_transform_affine flip_image_left_right(const image_type1 &in_img,
  ...
  from.push_back(r.tl_corner());
  to.push_back(r.tr_corner());
  from.push_back(r.bl_corner());
  to.push_back(r.br_corner());
  from.push_back(r.tr_corner());
  to.push_back(r.tl_corner());
  from.push_back(r.br_corner());
  to.push_back(r.bl_corner());
...
namespace impl {
inline rectangle flip_rect_left_right(const rectangle &rect,
...
inline rectangle tform_object(const point_transform_affine &tran,
                              const rectangle &rect) {
  return centered_rect(tran(center(rect)), rect.width(), rect.height());
}

inline mmod_rect tform_object(const point_transform_affine &tran,
                              mmod_rect rect) {
  rect.rect = tform_object(tran, rect.rect);
  return rect;
}

inline full_object_detection tform_object(const point_transform_affine &tran,
...
void flip_image_dataset_left_right(
  ...
  for (unsigned long i = 0; i < images.size(); ++i) {
    ...
    for (unsigned long j = 0; j < objects[i].size(); ++j) {
      objects[i][j] =
          impl::flip_rect_left_right(objects[i][j], get_rect(images[i]));
    }
  }
}
...
void flip_image_dataset_left_right(
  ...
  for (unsigned long i = 0; i < images.size(); ++i) {
    ...
    for (unsigned long j = 0; j < objects2[i].size(); ++j) {
      objects2[i][j] =
          impl::flip_rect_left_right(objects2[i][j], get_rect(images[i]));
    }
  }
}
...
void rotate_image_dataset(double angle, image_array_type &images,
  ...
  for (unsigned long i = 0; i < images.size(); ++i) {
    ...
    for (unsigned long j = 0; j < objects[i].size(); ++j) {
      ...
      objects[i][j] =
          centered_rect(tran(center(rect)), rect.width(), rect.height());
    }
  }
}
  ...
  dlib::parallel_for(0, images.size(), [&](long j) {
    ...
    for (long i = 0; i < angles.size(); ++i) {
      ...
      const point_transform_affine tran =
          rotate_image(images[j], temp, angles(i));
...
struct chip_details {
  ...
  chip_details(const rectangle &rect_)
      : rect(rect_), angle(0), rows(rect_.height()), cols(rect_.width()) {}
  chip_details(const drectangle &rect_)
      : rect(rect_), angle(0), rows((unsigned long)(rect_.height() + 0.5)),
        cols((unsigned long)(rect_.width() + 0.5)) {}
  chip_details(const drectangle &rect_, unsigned long size)
      : rect(rect_), angle(0) {
    compute_dims_from_size(size);
  }
  chip_details(const drectangle &rect_, unsigned long size, double angle_)
      : rect(rect_), angle(angle_) {
    compute_dims_from_size(size);
  }
  ...
  chip_details(const std::vector<dlib::vector<T, 2>> &chip_points,
    ...
    DLIB_CASSERT(chip_points.size() == img_points.size() &&
                     chip_points.size() >= 2,
    ...
    const point_transform_affine tform =
        find_similarity_transform(chip_points, img_points);
    dlib::vector<double, 2> p(1, 0);
    p = tform.get_m() * p;

    // There are only 3 things happening in a similarity transform.  There is a
    // rescaling, a rotation, and a translation.  So here we pick out the scale
    // and rotation parameters.
    angle = std::atan2(p.y(), p.x());
    // Note that the translation and scale part are represented by the
    // extraction rectangle.  So here we build the appropriate rectangle.
    const double scale = length(p);
    rect = centered_drect(tform(point(dims.cols, dims.rows) / 2.0),
                          dims.cols * scale, dims.rows * scale);
  }
...
private:
  void compute_dims_from_size(unsigned long size) {
...
inline point_transform_affine get_mapping_to_chip(const chip_details &details) {
  std::vector<dlib::vector<double, 2>> from, to;
  point p1(0, 0);
  point p2(details.cols - 1, 0);
  point p3(details.cols - 1, details.rows - 1);
  to.push_back(p1);
  from.push_back(rotate_point<double>(center(details.rect),
                                      details.rect.tl_corner(), details.angle));
  to.push_back(p2);
  from.push_back(rotate_point<double>(center(details.rect),
                                      details.rect.tr_corner(), details.angle));
  to.push_back(p3);
  from.push_back(rotate_point<double>(center(details.rect),
                                      details.rect.br_corner(), details.angle));
  return find_affine_transform(from, to);
}

// ----------------------------------------------------------------------------------------

inline full_object_detection map_det_to_chip(const full_object_detection &det,
...
namespace impl {
template <typename image_type1, typename image_type2>
void basic_extract_image_chip(const image_type1 &img, const rectangle &location,
                              image_type2 &chip)
/*!
    ensures
        - This function doesn't do any scaling or rotating. It just pulls out
the chip in the given rectangle.  This also means the output image has the same
dimensions as the location rectangle.
!*/
{
  ...
  for (long r = chip_area.top(), rr = area.top(); r <= chip_area.bottom();
       ++r, ++rr) {
    for (long c = chip_area.left(), cc = area.left(); c <= chip_area.right();
         ++c, ++cc) {
...
void extract_image_chips(const image_type1 &img,
  ...
  for (unsigned long i = 0; i < chip_locations.size(); ++i) {
    DLIB_CASSERT(chip_locations[i].size() != 0 &&
                 ...
                 "\t void extract_image_chips()"
                     << "\n\t Invalid inputs were given to this function."
                     << "\n\t chip_locations[" << i
                     << "].size():            " << chip_locations[i].size()
                     << "\n\t chip_locations[" << i << "].rect.is_empty(): "
                     << chip_locations[i].rect.is_empty());
  }
#endif
  ...
  pyramid_down<2> pyr;
  long max_depth = 0;
  // If the chip is supposed to be much smaller than the source subwindow then
  // you can't just extract it using bilinear interpolation since at a high
  // enough downsampling amount it would effectively turn into nearest neighbor
  // interpolation.  So we use an image pyramid to make sure the interpolation
  // is fast but also high quality.  The first thing we do is figure out how
  // deep the image pyramid needs to be.
  ...
  for (unsigned long i = 0; i < chip_locations.size(); ++i) {
    ...
    while (rect.area() > chip_locations[i].size()) {
      ...
      ++depth;
      // We drop the image size by a factor of 2 each iteration and then assume
      // a border of 2 pixels is needed to avoid any border effects of the crop.
      grow = grow * 2 + 2;
    }
    drectangle rot_rect;
    const vector<double, 2> cent = center(chip_locations[i].rect);
    rot_rect += rotate_point<double>(cent, chip_locations[i].rect.tl_corner(),
                                     chip_locations[i].angle);
    rot_rect += rotate_point<double>(cent, chip_locations[i].rect.tr_corner(),
                                     chip_locations[i].angle);
    rot_rect += rotate_point<double>(cent, chip_locations[i].rect.bl_corner(),
                                     chip_locations[i].angle);
    rot_rect += rotate_point<double>(cent, chip_locations[i].rect.br_corner(),
                                     chip_locations[i].angle);
    bounding_box += grow_rect(rot_rect, grow).intersect(get_rect(img));
    max_depth = std::max(depth, max_depth);
  }
  // std::cout << "max_depth: " << max_depth << std::endl;
  // std::cout << "crop amount: " <<
  // bounding_box.area()/(double)get_rect(img).area() << std::endl;

  // now make an image pyramid
  dlib::array<array2d<typename image_traits<image_type1>::pixel_type>> levels(
      max_depth);
  if (levels.size() != 0)
  ...
  for (unsigned long i = 0; i < chips.size(); ++i) {
    // If the chip doesn't have any rotation or scaling then use the basic
    // version of chip extraction that just does a fast copy.
    if (chip_locations[i].angle == 0 &&
      ...
      int level = -1;
      drectangle rect =
          translate_rect(chip_locations[i].rect, -bounding_box.tl_corner());
      while (pyr.rect_down(rect).area() > chip_locations[i].size()) {
        ++level;
        rect = pyr.rect_down(rect);
      }

      // find the appropriate transformation that maps from the chip to the
      // input image
      from.clear();
      to.clear();
      from.push_back(get_rect(chips[i]).tl_corner());
      to.push_back(rotate_point<double>(center(rect), rect.tl_corner(),
                                        chip_locations[i].angle));
      from.push_back(get_rect(chips[i]).tr_corner());
      to.push_back(rotate_point<double>(center(rect), rect.tr_corner(),
                                        chip_locations[i].angle));
      from.push_back(get_rect(chips[i]).bl_corner());
      to.push_back(rotate_point<double>(center(rect), rect.bl_corner(),
                                        chip_locations[i].angle));
...
template <typename image_type1, typename image_type2,
          typename interpolation_type>
void extract_image_chip(const image_type1 &img, const chip_details &location,
                        image_type2 &chip, const interpolation_type &interp) {
  // If the chip doesn't have any rotation or scaling then use the basic version
  // of chip extraction that just does a fast copy.
  if (location.angle == 0 && location.rows == location.rect.height() &&
...
inline chip_details get_face_chip_details(const full_object_detection &det,
...
template <typename image_type>
void extract_image_4points(const image_type &img_, image_type &out_,
  ...
  const std::array<dpoint, 4> corners = {
      bounding_box.tl_corner(), bounding_box.tr_corner(),
  ...
  matrix<long long> idists = matrix_cast<long long>(
      -round(std::numeric_limits<long long>::max() * (dists / max(dists))));
...
image_type jitter_image(const image_type &img, dlib::rand &rnd) {
  ...
  const point rand_translate =
      dpoint(rnd.get_double_in_range(-translate_amount, translate_amount) *
                 rect.width(),
             rnd.get_double_in_range(-translate_amount, translate_amount) *
                 rect.height());

  // perturb the scale of the crop by a fraction of the object's size
  const double rand_scale_perturb =
      rnd.get_double_in_range(min_object_height, max_object_height);

  const long box_size = rect.height() / rand_scale_perturb;
  const auto crop_rect =
      centered_rect(center(rect) + rand_translate, box_size, box_size);
  const double angle =
      rnd.get_double_in_range(-max_rotation_degrees, max_rotation_degrees) *
      pi / 180;
  image_type crop;
  extract_image_chip(img,
                     chip_details(crop_rect,
                                  chip_dims(num_rows(img), num_columns(img)),
                                  angle),
                     crop);
