...
struct connected_if_both_not_zero {
  ...
  bool operator()(const image_type &img, const point &a, const point &b) const {
...
struct zero_pixels_are_background {
  template <typename image_type>
  bool operator()(const image_type &img, const point &p) const {
...
struct nothing_is_background {
  template <typename image_type>
  bool operator()(const image_type &, const point &) const {
    return false;
  }
};
...
unsigned long label_connected_blobs(
    const image_type &img_, const background_functor_type &is_background,
    const neighbors_functor_type &get_neighbors,
    const connected_functor_type &is_connected, label_image_type &label_img_) {
  // make sure requires clause is not broken
  DLIB_ASSERT(is_same_object(img_, label_img_) == false,
              "\t unsigned long label_connected_blobs()"
                  << "\n\t The input image and output label image can't be the "
                     "same object.");
      ...
      while (neighbors.size() > 0) {
        ...
        for (unsigned long i = 0; i < window.size(); ++i) {
          if (area.contains(window[i]) &&       // point in image.
              ...
              label_img[window[i].y()][window[i].x()] ==
                  0 &&                         // haven't already labeled it.
...
unsigned long label_connected_blobs_watershed(
    const in_image_type &img_, out_image_type &labels_,
  ...
  struct watershed_points {
    ...
    watershed_points(const point &p_, float score_, unsigned int label_)
        : p(p_), score(score_), label(label_) {}
    ...
    bool is_seed() const {
      return label == std::numeric_limits<unsigned int>::max();
    }

    bool operator<(const watershed_points &rhs) const {
      // If two pixels have the same score then we take the one with the
      // smallest label out of the priority queue first.  We do this so that
      // seed points that are downhill from some larger blob will be consumed by
      // it if they haven't grown before the larger blob's flooding reaches
      // them.  Doing this helps a lot to avoid spuriously splitting blobs.
      if (score == rhs.score) {
  ...
  // background_thresh.  We do however blur, if smoothing!=0, the pixel values
  // used to do the watershed.
  in_image_type img2_;
  if (smoothing != 0)
  ...
  for (long r = 1; r + 1 < img.nr(); ++r) {
    for (long c = 1; c + 1 < img.nc(); ++c) {
      ...
      if (val < img2[r + 1][c] || val < img2[r - 1][c] ||
          val < img2[r][c + 1] || val < img2[r][c - 1]) {
        continue;
      }

      next.push(watershed_points(point(c, r), val,
  ...
  while (next.size() > 0) {
    ...
    // background pixel (i.e. it hasn't been flooded over by a neighboring blob
    // and consumed by it) then we create a new label for this new blob.
    ...
    for (auto &n : neighbors) {
      if (!area.contains(n) || labels[n.y()][n.x()] != 0 ||
          img[n.y()][n.x()] < background_thresh)
        continue;
