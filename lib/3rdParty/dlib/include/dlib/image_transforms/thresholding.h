...
namespace impl {
template <typename U, typename V, typename basic_pixel_type>
void partition_pixels_float_work(unsigned long begin, unsigned long end,
                                 U &&cumsum, V &&sorted,
                                 basic_pixel_type &pix_thresh,
                                 unsigned long &int_thresh) {

  auto histsum = [&](long begin, long end) { return end - begin; };
  ...
  // those >= thresh (the right group), what would the sum of absolute
  // deviations of each pixel from the mean of its group be?  total_abs(thresh)
  // computes that value.
  ...
  auto total_abs = [&](unsigned long thresh) {
    ...
    score +=
        right_avg * histsum(thresh, right_idx) - histsumi(thresh, right_idx);
...
template <typename U, typename V, typename basic_pixel_type>
void recursive_partition_pixels_float(unsigned long begin, unsigned long end,
                                      U &&cumsum, V &&sorted,
                                      basic_pixel_type &pix_thresh) {
  unsigned long int_thresh;
  partition_pixels_float_work(begin, end, cumsum, sorted, pix_thresh,
                              int_thresh);
}

template <typename U, typename V, typename basic_pixel_type, typename... T>
void recursive_partition_pixels_float(unsigned long begin, unsigned long end,
                                      U &&cumsum, V &&sorted,
                                      basic_pixel_type &pix_thresh,
                                      T &&...more_thresholds) {
  ...
  partition_pixels_float_work(begin, end, cumsum, sorted, pix_thresh,
                              int_thresh);
  recursive_partition_pixels_float(int_thresh, end, cumsum, sorted,
                                   more_thresholds...);
}
...
void partition_pixels_float(
    ...
    typename pixel_traits<typename image_traits<image_type>::pixel_type>::
        basic_pixel_type &pix_thresh,
    T &&...more_thresholds) {
  /*
  ...
  */
  ...
  typedef typename pixel_traits<typename image_traits<image_type>::pixel_type>::
      basic_pixel_type basic_pixel_type;
  ...
  recursive_partition_pixels_float(0, img.size(), cumsum, sorted, pix_thresh,
                                   more_thresholds...);
}

template <typename image_type> struct is_u16img_or_less {
  typedef typename image_traits<image_type>::pixel_type pixel_type;
  typedef typename pixel_traits<pixel_type>::basic_pixel_type basic_pixel_type;

  const static bool value =
      sizeof(basic_pixel_type) <= 2 && pixel_traits<pixel_type>::is_unsigned;
};
} // namespace impl

template <typename image_type, typename... T>
typename disable_if<impl::is_u16img_or_less<image_type>>::type partition_pixels(
...
namespace impl {
template <typename U, typename basic_pixel_type>
void partition_pixels_work(unsigned long begin, unsigned long end,
                           U &&total_abs, basic_pixel_type &pix_thresh,
                           unsigned long &int_thresh) {
  int_thresh = begin;
  double min_sad = std::numeric_limits<double>::infinity();
  for (unsigned long i = begin; i < end; ++i) {
...
template <typename U, typename basic_pixel_type>
void recursive_partition_pixels(unsigned long begin, unsigned long end,
                                U &&total_abs, basic_pixel_type &pix_thresh) {
  unsigned long int_thresh;
  partition_pixels_work(begin, end, total_abs, pix_thresh, int_thresh);
}

template <typename U, typename basic_pixel_type, typename... T>
void recursive_partition_pixels(unsigned long begin, unsigned long end,
                                U &&total_abs, basic_pixel_type &pix_thresh,
                                T &&...more_thresholds) {
  unsigned long int_thresh;
...
template <typename image_type, typename... T>
typename enable_if<impl::is_u16img_or_less<image_type>>::type partition_pixels(
    const image_type &img,
    typename pixel_traits<typename image_traits<image_type>::pixel_type>::
        basic_pixel_type &pix_thresh,
    T &&...more_thresholds) {
  ...
  COMPILE_TIME_ASSERT(
      pixel_traits<
          typename image_traits<image_type>::pixel_type>::is_unsigned == true);
  ...
  auto total_abs = [&](unsigned long begin, unsigned long thresh) {
    ...
    score +=
        right_avg * histsum(thresh, right_idx) - histsumi(thresh, right_idx);
    score -= right_avg * histsum(right_idx, hist.size()) -
             histsumi(right_idx, hist.size());
    return score;
  };

  impl::recursive_partition_pixels(0, hist.size(), total_abs, pix_thresh,
                                   more_thresholds...);
}
...
typename pixel_traits<
    typename image_traits<image_type>::pixel_type>::basic_pixel_type
partition_pixels(const image_type &img) {
  typedef typename pixel_traits<typename image_traits<image_type>::pixel_type>::
      basic_pixel_type basic_pixel_type;
...
void threshold_image(const in_image_type &in_img_, out_image_type &out_img_,
                     typename pixel_traits<typename image_traits<
                         in_image_type>::pixel_type>::basic_pixel_type thresh) {
  COMPILE_TIME_ASSERT(
      pixel_traits<
...
template <typename in_image_type, typename out_image_type>
void threshold_image(const in_image_type &in_img, out_image_type &out_img) {
  threshold_image(in_img, out_img, partition_pixels(in_img));
}

template <typename image_type>
void threshold_image(image_type &img,
                     typename pixel_traits<typename image_traits<
                         image_type>::pixel_type>::basic_pixel_type thresh) {
  threshold_image(img, img, thresh);
}

// ----------------------------------------------------------------------------------------

template <typename image_type> void threshold_image(image_type &img) {
...
template <typename in_image_type, typename out_image_type>
void auto_threshold_image(const in_image_type &in_img_,
  ...
  COMPILE_TIME_ASSERT(
      pixel_traits<
          typename image_traits<in_image_type>::pixel_type>::is_unsigned ==
      true);
  COMPILE_TIME_ASSERT(
      pixel_traits<
          typename image_traits<out_image_type>::pixel_type>::is_unsigned ==
      true);
...
template <typename image_type> void auto_threshold_image(image_type &img) {
...
void hysteresis_threshold(
    ...
    typename pixel_traits<typename image_traits<in_image_type>::pixel_type>::
        basic_pixel_type lower_thresh,
    typename pixel_traits<typename image_traits<in_image_type>::pixel_type>::
        basic_pixel_type upper_thresh) {
  COMPILE_TIME_ASSERT(
      pixel_traits<
          typename image_traits<in_image_type>::pixel_type>::has_alpha ==
      false);
  ...
  DLIB_ASSERT(is_same_object(in_img_, out_img_) == false,
              "\tvoid hysteresis_threshold(in_img_, out_img_, lower_thresh, "
              "upper_thresh)"
  ...
  for (long r = 0; r < in_img.nr(); ++r) {
    for (long c = 0; c < in_img.nc(); ++c) {
      typename pixel_traits<
          typename image_traits<in_image_type>::pixel_type>::basic_pixel_type p;
      ...
      if (p >= upper_thresh) {
        ...
        while (pos > 0) {
          ...
          // This is the base case of our recursion.  We want to stop if we hit
          // a pixel we have already visited.
          if (out_img[r][c] == on_pixel)
            continue;

          out_img[r][c] = on_pixel;

          // put the neighbors of this pixel on the stack if they are bright
          // enough
          if (r - 1 >= 0) {
            if (pos < size &&
                get_pixel_intensity(in_img[r - 1][c]) >= lower_thresh) {
              rstack[pos] = r - 1;
              cstack[pos] = c;
              ++pos;
            }
            if (pos < size && c - 1 >= 0 &&
                get_pixel_intensity(in_img[r - 1][c - 1]) >= lower_thresh) {
              rstack[pos] = r - 1;
              cstack[pos] = c - 1;
              ++pos;
            }
            if (pos < size && c + 1 < in_img.nc() &&
                get_pixel_intensity(in_img[r - 1][c + 1]) >= lower_thresh) {
              rstack[pos] = r - 1;
              cstack[pos] = c + 1;
              ++pos;
          ...
          if (pos < size && c - 1 >= 0 &&
              get_pixel_intensity(in_img[r][c - 1]) >= lower_thresh) {
            rstack[pos] = r;
            cstack[pos] = c - 1;
            ++pos;
          }
...
template <typename in_image_type, typename out_image_type>
void hysteresis_threshold(const in_image_type &in_img,
                          out_image_type &out_img) {
  using basic_pixel_type = typename pixel_traits<
