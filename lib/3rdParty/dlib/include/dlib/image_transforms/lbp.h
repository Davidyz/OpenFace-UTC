...
void make_uniform_lbp_image(const image_type &img_, image_type2 &lbp_) {
  ...
  for (long r = 1; r + 1 < img.nr(); ++r) {
    for (long c = 1; c + 1 < img.nc(); ++c) {
      ...
      unsigned char x = 0;
      if (get_pixel_intensity(img[r - 1][c - 1]) > pix)
        b1 = 0x80;
      if (get_pixel_intensity(img[r - 1][c]) > pix)
        b2 = 0x40;
      if (get_pixel_intensity(img[r - 1][c + 1]) > pix)
        b3 = 0x20;
      x |= b1;
      if (get_pixel_intensity(img[r][c - 1]) > pix)
        b4 = 0x10;
      x |= b2;
      if (get_pixel_intensity(img[r][c + 1]) > pix)
        b5 = 0x08;
...
void extract_histogram_descriptors(const image_type &img_, const point &loc,
  ...
  const rectangle window =
      centered_rect(loc, block_size * cell_size, block_size * cell_size);
  ...
  for (unsigned int br = 0; br < block_size; ++br) {
    ...
    for (unsigned int bc = 0; bc < block_size; ++bc) {
      ...
      rectangle cell(cell_left, cell_top, cell_left + cell_size - 1,
  ...
  for (long r = 0; r < lbp.nr(); r += cell_size) {
    for (long c = 0; c < lbp.nc(); c += cell_size) {
      const rectangle cell =
          rectangle(c, r, c + cell_size - 1, r + cell_size - 1)
              .intersect(get_rect(lbp));
      // make the actual histogram for this cell
...
void extract_highdim_face_lbp_descriptors(const image_type &img,
  ...
  // We only do feature extraction from these face parts.  These are things like
  // the corners of the eyes and mouth and stuff like that.
    ...
    for (unsigned long iter = 1; iter < num_scales; ++iter) {
      ...
      for (unsigned long i = 0; i < parts.size(); ++i)
        extract_histogram_descriptors(
            lbp, pyr.point_down(parts[i], num_pyr_calls), feats);
