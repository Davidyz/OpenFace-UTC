...
class random_cropper {
  ...
  long min_object_length_long_dim = 75;  // cropped object will be at least this
                                         // many pixels along its longest edge.
  long min_object_length_short_dim = 30; // cropped object will be at least this
                                         // many pixels along its shortest edge.
  double max_object_size = 0.7; // cropped object will be at most this fraction
...
public:
  ...
  double get_background_crops_fraction() const {
    return background_crops_fraction;
  }
  ...
  const chip_dims &get_chip_dims() const { return dims; }

  void set_chip_dims(const chip_dims &dims_) { dims = dims_; }

  void set_chip_dims(unsigned long rows, unsigned long cols) {
    set_chip_dims(chip_dims(rows, cols));
  }

  bool get_randomly_flip() const { return randomly_flip; }

  void set_randomly_flip(bool value) { randomly_flip = value; }
  ...
  long get_min_object_length_long_dim() const {
    return min_object_length_long_dim;
  }
  long get_min_object_length_short_dim() const {
    return min_object_length_short_dim;
  }
  ...
  void operator()(const image_type1 &img, const std::vector<mmod_rect> &rects,
    ...
    // copy rects into crop_rects and set ones that are outside the crop to
    // ignore or drop entirely as appropriate.
    crop_rects.clear();
    for (auto rect : rects) {
      // map to crop
      ...
      if (get_rect(crop).intersect(rect.rect).area() != 0) {
        ...
        if (!get_rect(crop).contains(rect.rect) ||
            ((long)rect.rect.height() < min_object_length_long_dim &&
             (long)rect.rect.width() < min_object_length_long_dim) ||
            ((long)rect.rect.height() < min_object_length_short_dim ||
             (long)rect.rect.width() < min_object_length_short_dim)) {
          rect.ignore = true;
        }
...
private:
  template <typename image_type1>
  void make_crop_plan(const image_type1 &img,
                      const std::vector<mmod_rect> &rects,
                      chip_details &crop_plan, bool &should_flip_crop) {
    std::lock_guard<std::mutex> lock(rnd_mutex);
    rectangle crop_rect;
    if (has_non_ignored_box(rects) &&
        rnd.get_random_double() >= background_crops_fraction) {
      auto rect = rects[randomly_pick_rect(rects)].rect;

      // perturb the location of the crop by a small fraction of the object's
      // size.
      const point rand_translate =
          dpoint(rnd.get_double_in_range(-translate_amount, translate_amount) *
                     std::max(rect.height(), rect.width()),
      ...
      drectangle drect = centered_drect(center(rect) + rand_translate,
                                        rect.width() / max_object_size,
                                        rect.height() / max_object_size);
      ...
      if (drect.width() / drect.height() < target_aspect)
        drect = centered_drect(drect, target_aspect * drect.height(),
                               drect.height());
      else
        drect =
            centered_drect(drect, drect.width(), drect.width() / target_aspect);
      ...
      double min_scale1 =
          std::max(min_object_length_short_dim / current_width,
                   min_object_length_short_dim / current_height);
      // at least one dimension needs to be longer than the long dim.
      double min_scale2 = std::min(min_object_length_long_dim / current_width,
                                   min_object_length_long_dim / current_height);
      double min_scale = std::max(min_scale1, min_scale2);

      const double rand_scale_perturb =
          1.0 / rnd.get_double_in_range(min_scale, 1);
      crop_rect = centered_drect(drect, drect.width() * rand_scale_perturb,
                                 drect.height() * rand_scale_perturb);
  ...
  bool has_non_ignored_box(const std::vector<mmod_rect> &rects) const {
    for (auto &&b : rects) {
      if (!b.ignore)
        return true;
    }
  ...
  size_t randomly_pick_rect(const std::vector<mmod_rect> &rects) {
  ...
  rectangle make_random_cropping_rect(const image_type &img_) {
    const_image_view<image_type> img(img_);
    // Figure out what rectangle we want to crop from the image.  We are going
    // to crop out an image of size this->dims, so we pick a random scale factor
    // that lets this random box be either as big as it can be while still
    // fitting in the image or as small as a 3x zoomed in box randomly somewhere
    // in the image.
    double mins = 1.0 / 3.0, maxs = std::min(img.nr() / (double)dims.rows,
                                             img.nc() / (double)dims.cols);
...
inline std::ostream &operator<<(std::ostream &out, const random_cropper &item) {
  using std::endl;
  ...
  out << "  randomly_flip:               " << std::boolalpha
      << item.get_randomly_flip() << endl;
  out << "  max_rotation_degrees:        " << item.get_max_rotation_degrees()
      << endl;
  out << "  min_object_length_long_dim:  "
      << item.get_min_object_length_long_dim() << endl;
  out << "  min_object_length_short_dim: "
      << item.get_min_object_length_short_dim() << endl;
  out << "  max_object_size:             " << item.get_max_object_size()
      << endl;
  out << "  background_crops_fraction:   "
      << item.get_background_crops_fraction() << endl;
  out << "  translate_amount:            " << item.get_translate_amount()
      << endl;
  return out;
}
