...
public:
  ...
  template <typename image_type> void load(const image_type &img_) {
  ...
  value_type get_sum_of_area(const rectangle &rect) const {
    DLIB_ASSERT(
        get_rect(*this).contains(rect) == true && rect.is_empty() == false,
  ...
  void swap(integral_image_generic &item) { int_img.swap(item.int_img); }
...
template <typename integral_image_type>
typename integral_image_type::value_type haar_y(const integral_image_type &img,
                                                const point &p, long width) {
  DLIB_ASSERT(
      get_rect(img).contains(centered_rect(p, width, width)) == true,
