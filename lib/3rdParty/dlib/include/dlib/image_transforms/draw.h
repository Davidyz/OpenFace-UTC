...
void draw_line(long x1, long y1, long x2, long y2, image_type &c_,
  ...
  if (x1 == x2) {
    ...
    // This part is a little more complicated because we are going to perform
    // alpha blending so the diagonal lines look nice.
...
template <typename image_type, typename pixel_type>
void draw_line(image_type &c, const point &p1, const point &p2,
               const pixel_type &val) {
...
template <typename image_type, typename pixel_type>
void draw_rectangle(image_type &c, const rectangle &rect,
                    const pixel_type &val) {
...
template <typename image_type, typename pixel_type>
void draw_rectangle(image_type &c, const rectangle &rect, const pixel_type &val,
...
template <typename image_type, typename pixel_type>
void fill_rect(image_type &img_, const rectangle &rect,
...
matrix<typename image_traits<typename image_array_type::value_type>::pixel_type>
tile_images(const image_array_type &images) {
  typedef
      typename image_traits<typename image_array_type::value_type>::pixel_type
          T;
  ...
  const unsigned long size_nr =
      (size_nc * (size_nc - 1) >= images.size()) ? size_nc - 1 : size_nc;
  // Figure out the size we have to use for each chip in the big main image.  We
  // will use the largest dimensions seen across all the chips.
  long nr = 0;
  long nc = 0;
...
template <typename image_type, typename pixel_type>
void draw_solid_circle(image_type &img_, const dpoint &center_point,
                       double radius, const pixel_type &pixel) {
  image_view<image_type> img(img_);
  using std::sqrt;
