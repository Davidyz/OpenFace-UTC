...
bool is_binary_image(const image_type &img_)
/*!
    ensures
...
bool is_binary_image(const unsigned char (&structuring_element)[M][N])
/*!
    ensures
        - returns true if structuring_element contains only on_pixel and
off_pixel values.
        - returns false otherwise
!*/
{
...
void binary_dilation(const in_image_type &in_img_, out_image_type &out_img_,
...
template <typename in_image_type, typename out_image_type, long M, long N>
void binary_erosion(const in_image_type &in_img_, out_image_type &out_img_,
                    const unsigned char (&structuring_element)[M][N]) {
  typedef typename image_traits<in_image_type>::pixel_type in_pixel_type;
  typedef typename image_traits<out_image_type>::pixel_type out_pixel_type;
  COMPILE_TIME_ASSERT(pixel_traits<in_pixel_type>::has_alpha == false);
...
void binary_intersection(const in_image_type1 &in_img1_,
...
template <typename in_image_type1, typename in_image_type2,
          typename out_image_type>
void binary_union(const in_image_type1 &in_img1_,
                  const in_image_type2 &in_img2_, out_image_type &out_img_) {
  typedef typename image_traits<in_image_type1>::pixel_type in_pixel_type1;
  typedef typename image_traits<in_image_type2>::pixel_type in_pixel_type2;
  typedef typename image_traits<out_image_type>::pixel_type out_pixel_type;
  COMPILE_TIME_ASSERT(pixel_traits<in_pixel_type1>::has_alpha == false);
...
template <typename in_image_type1, typename in_image_type2,
          typename out_image_type>
void binary_difference(const in_image_type1 &in_img1_,
                       const in_image_type2 &in_img2_,
                       out_image_type &out_img_) {
  typedef typename image_traits<in_image_type1>::pixel_type in_pixel_type1;
  typedef typename image_traits<in_image_type2>::pixel_type in_pixel_type2;
...
void binary_complement(const in_image_type &in_img_, out_image_type &out_img_) {
  typedef typename image_traits<in_image_type>::pixel_type in_pixel_type;
  typedef typename image_traits<out_image_type>::pixel_type out_pixel_type;
  COMPILE_TIME_ASSERT(pixel_traits<in_pixel_type>::has_alpha == false);
  COMPILE_TIME_ASSERT(pixel_traits<out_pixel_type>::has_alpha == false);
...
namespace impl {
template <typename image_type>
inline bool should_remove_pixel(const image_type &img, long r, long c,
                                int iter) {
...
template <typename image_type>
inline void add_to_remove(std::vector<point> &to_remove,
                          array2d<unsigned char> &marker, const image_type &img,
                          long r, long c, int iter) {
...
template <typename image_type>
inline bool is_bw_border_pixel(const image_type &img, long r, long c) {
  unsigned int p2 = img[r - 1][c];
  unsigned int p3 = img[r - 1][c + 1];
  unsigned int p4 = img[r][c + 1];
  unsigned int p5 = img[r + 1][c + 1];
...
inline void add_if(std::vector<point> &to_check2,
                   const array2d<unsigned char> &marker, long c, long r) {
  if (marker[r][c])
...
template <typename image_type> void skeleton(image_type &img_) {
  /*
      The implementation of this function is based on the paper
      "A fast parallel algorithm for thinning digital patterns” by T.Y. Zhang
     and C.Y. Suen. and also the excellent discussion of it at:
  ...
  */
  ...
  // thinning to being at the border of the image.  But more importantly, it
  // causes the mask to have a border of 0 pixels as well which we use later to
  // avoid indexing outside the image inside add_to_remove().
...
template <typename image_type>
std::vector<point> find_line_endpoints(const image_type &img_) {
  const_image_view<image_type> img(img_);
  ...
  for (long r = 0; r < img.nr(); ++r) {
    for (long c = 0; c < img.nc(); ++c) {
      if (img[r][c] &&
          line_ending_patterns[encode_8_pixel_neighbors(img, point(c, r))]) {
