...
class mlp : noncopyable {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          trained using the back propagation algorithm.  The training algorithm
  also incorporates the momentum method.  That is, each round of back
  propagation training also adds a fraction of the previous update.  This
  fraction is controlled by the momentum term set in the constructor.

          The activation function used at each node is the sigmoid function.
  I.e. sigmoid(x) = 1/(1 + pow(e,-x)).  Thus the output of the network is always
  in the range [0,1]
  !*/

public:
  mlp(long nodes_in_input_layer, long nodes_in_first_hidden_layer,
      long nodes_in_second_hidden_layer = 0, long nodes_in_output_layer = 1,
      double alpha = 0.1, double momentum = 0.8);
  /*!
      requires
  ...
  long input_layer_nodes() const;
  /*!
      ensures
          - returns the number of nodes in the input layer
  !*/

  long first_hidden_layer_nodes() const;
  /*!
      ensures
  ...
  long second_hidden_layer_nodes() const;
  /*!
      ensures
  ...
  /*!
      ensures
          - returns the momentum term used by this object during back
  propagation training.  The momentum is is the fraction of a previous update to
...
inline void swap(mlp &a, mlp &b) { a.swap(b); }
/*!
    provides a global swap function
!*/

void serialize(const mlp &item, std::ostream &out);
/*!
    provides serialization support
!*/

void deserialize(mlp &item, std::istream &in);
/*!
