  ...
  /*!
      ...
      CONVENTION
          ...
          - if (second_hidden_nodes == 0) then
              - for all i and j:
                  - w1(i,j) == the weight on the link from node i in the first
  hidden layer to input node j
                  - w3(i,j) == the weight on the link from node i in the output
  layer to first hidden layer node j
              - for all i and j:
          ...
          - else
              - for all i and j:
                  - w1(i,j) == the weight on the link from node i in the first
  hidden layer to input node j
                  - w2(i,j) == the weight on the link from node i in the second
  hidden layer to first hidden layer node j
                  - w3(i,j) == the weight on the link from node i in the output
  layer to second hidden layer node j
              - for all i and j:
                  - w1m == the momentum terms for w1 from the previous update
...
public:
  mlp_kernel_1(long nodes_in_input_layer, long nodes_in_first_hidden_layer,
               long nodes_in_second_hidden_layer = 0,
               long nodes_in_output_layer = 1, double alpha_ = 0.1,
      ...
      : input_nodes(nodes_in_input_layer),
        ...
        output_nodes(nodes_in_output_layer), alpha(alpha_),
        momentum(momentum_) {
  ...
  long input_layer_nodes() const { return input_nodes; }

  long first_hidden_layer_nodes() const { return first_hidden_nodes; }

  long second_hidden_layer_nodes() const { return second_hidden_nodes; }

  long output_layer_nodes() const { return output_nodes; }
  ...
  const matrix<double> operator()(const matrix_exp<EXP> &in) const {
  ...
  void train(const matrix_exp<EXP1> &example_in,
    ...
    if (second_hidden_nodes == 0) {
      ...
      e3 = pointwise_multiply(
          example_out - o, uniform_matrix<double>(output_nodes, 1, 1.0) - o, o);
      e1 = pointwise_multiply(
          tmp1, uniform_matrix<double>(first_hidden_nodes + 1, 1, 1.0) - tmp1,
          trans(w3) * e3);
      ...
      e3 = pointwise_multiply(
          example_out - o, uniform_matrix<double>(output_nodes, 1, 1.0) - o, o);
      e2 = pointwise_multiply(
          tmp2, uniform_matrix<double>(second_hidden_nodes + 1, 1, 1.0) - tmp2,
          trans(w3) * e3);
      e1 = pointwise_multiply(
          tmp1, uniform_matrix<double>(first_hidden_nodes + 1, 1, 1.0) - tmp1,
          trans(w2) * e2);
  ...
  template <typename EXP>
  void train(const matrix_exp<EXP> &example_in, double example_out) {
    matrix<double, 1, 1> e_out;
    e_out(0) = example_out;
    train(example_in, e_out);
  }

  double get_average_change() const {
    // sum up all the weight changes
    double delta = sum(abs(w1m)) + sum(abs(w2m)) + sum(abs(w3m));

    // divide by the number of weights
    delta /= w1m.nr() * w1m.nc() + w2m.nr() * w2m.nc() + w3m.nr() * w3m.nc();

    return delta;
  }

  void swap(mlp_kernel_1 &item) {
  ...
  friend void serialize(const mlp_kernel_1 &item, std::ostream &out);

  friend void deserialize(mlp_kernel_1 &item, std::istream &in);
...
inline void swap(mlp_kernel_1 &a, mlp_kernel_1 &b) { a.swap(b); }

// ----------------------------------------------------------------------------------------

inline void serialize(const mlp_kernel_1 &item, std::ostream &out) {
  try {
...
inline void deserialize(mlp_kernel_1 &item, std::istream &in) {
  try {
