...
typedef config_reader_kernel_1<map<std::string, std::string>::kernel_1b,
                               map<std::string, void *>::kernel_1b,
                               tokenizer::kernel_1a>
    config_reader;

template <> struct is_config_reader<config_reader> {
...
template <> struct is_config_reader<config_reader_thread_safe> {
  const static bool value = true;
};
