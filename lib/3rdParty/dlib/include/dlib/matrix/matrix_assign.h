...
namespace dlib {
/*
    This file contains some templates that are used inside the
   matrix_blas_bindings.h file to bind various matrix expressions to optimized
...
template <typename T> void zero_matrix(T &m) {
...
// This template struct is used to tell us if a matrix expression contains a
// matrix multiply.
template <typename T> struct has_matrix_multiply {
...
struct has_matrix_multiply<matrix_add_exp<T, U>> {
  const static bool value =
      has_matrix_multiply<T>::value || has_matrix_multiply<U>::value;
};
...
template <typename T> struct has_matrix_multiply<matrix_div_scal_exp<T>> {
  const static bool value = has_matrix_multiply<T>::value;
};

template <typename T> struct has_matrix_multiply<matrix_op<T>> {
  const static bool value = has_matrix_multiply<T>::value;
};

template <typename T> struct has_matrix_multiply<op_trans<T>> {
  const static bool value = has_matrix_multiply<T>::value;
};

template <typename T> struct has_matrix_multiply<op_conj_trans<T>> {
  const static bool value = has_matrix_multiply<T>::value;
};

template <typename T> struct has_matrix_multiply<op_conj<T>> {
  const static bool value = has_matrix_multiply<T>::value;
};
...
template <typename T> struct matrix_type_id {
...
struct matrix_type_id<
    matrix_op<op_std_vect_to_mat<std::vector<value_type, alloc>>>> {
  const static int value = column_matrix;
};

template <typename value_type, typename alloc>
struct matrix_type_id<
    matrix_op<op_std_vect_to_mat<std_vector_c<value_type, alloc>>>> {
  const static int value = column_matrix;
};
...
template <typename T> struct matrix_type_id<matrix_op<op_pointer_to_mat<T>>> {
  const static int value = general_matrix;
};

// ------------------------------------------------------------------------------------

template <typename T, typename U> struct same_matrix {
  const static int T_id = matrix_type_id<T>::value;
  const static int U_id = matrix_type_id<U>::value;
  // The check for unknown_matrix is here so that we can be sure that matrix
  // types other than the ones specifically enumerated above never get pushed
  // into any of the BLAS bindings.  So saying they are never the same as
  // anything else prevents them from matching any of the BLAS bindings.
...
// This template struct is used to tell us if two matrix expressions both
// contain the same sequence of operators, expressions.    It also only has a
// value of true if the T expression contains only matrices with the given
// layout.
template <typename T, typename U, typename layout> struct same_exp {
  const static bool value =
      (is_same_type<typename T::exp_type, typename U::exp_type>::value ||
       same_matrix<typename T::exp_type, typename U::exp_type>::value) &&
      is_same_type<typename T::layout_type, layout>::value;
};

// Used only below.  They help strip off the const and & qualifiers that can
// show up in the LHS_ref_type and RHS_ref_type typedefs.
...
template <typename Tlhs, typename Ulhs, typename Trhs, typename Urhs,
          typename layout>
struct same_exp<matrix_multiply_exp<Tlhs, Trhs>,
                matrix_multiply_exp<Ulhs, Urhs>, layout> {
  // The reason this case is more complex than the others is because the
  // matrix_multiply_exp will use a temporary matrix instead of Tlhs or Trhs in
  // the event that one of these types corresponds to an expensive expression.
  // So we have to use the type that really gets used.  The following typedefs
  // are here to pick out that true type.
...
struct same_exp<matrix_add_exp<Tlhs, Trhs>, matrix_add_exp<Ulhs, Urhs>,
                layout> {
  const static bool value = same_exp<Tlhs, Ulhs, layout>::value &&
                            same_exp<Trhs, Urhs, layout>::value;
};

template <typename Tlhs, typename Ulhs, typename Trhs, typename Urhs,
          typename layout>
struct same_exp<matrix_subtract_exp<Tlhs, Trhs>,
                matrix_subtract_exp<Ulhs, Urhs>, layout> {
  const static bool value = same_exp<Tlhs, Ulhs, layout>::value &&
                            same_exp<Trhs, Urhs, layout>::value;
};

template <typename T, typename U, bool Tb, bool Ub, typename layout>
struct same_exp<matrix_mul_scal_exp<T, Tb>, matrix_mul_scal_exp<U, Ub>,
                layout> {
...
template <typename T, typename U, typename layout>
struct same_exp<matrix_op<op_conj_trans<T>>, matrix_op<op_conj_trans<U>>,
                layout> {
  const static bool value = same_exp<T, U, layout>::value;
};
...
// This is a helper that is used below to apply the same_exp template to matrix
// expressions.
...
template <typename dest_exp, typename src_exp, typename enabled = void>
struct matrix_assign_blas_helper {
  ...
  static void assign(dest_exp &dest, const EXP &src,
                     typename src_exp::type alpha, bool add_to,
                     bool transpose) {
    if (transpose == false)
  ...
  static void assign(dest_exp &dest, const matrix_multiply_exp<EXP1, EXP2> &src,
                     typename src_exp::type alpha, bool add_to,
                     bool transpose) {
...
// This is a macro to help us add overloads for the matrix_assign_blas_helper
// template. Using this macro it is easy to add overloads for arbitrary matrix
// expressions.
#define DLIB_ADD_BLAS_BINDING(src_expression)                                  \
  template <typename T, typename L> struct DLIB_BOOST_JOIN(blas, __LINE__) {   \
    const static bool value =                                                  \
        sizeof(yes_type) == sizeof(test<T, L>(src_expression));                \
  };                                                                           \
                                                                               \
  template <typename dest_exp, typename src_exp>                               \
      struct matrix_assign_blas_helper<                                        \
          dest_exp, src_exp,                                                   \
          typename enable_if<DLIB_BOOST_JOIN(blas, __LINE__) < src_exp,        \
                             typename dest_exp::layout_type>>::type > {        \
    static void assign(dest_exp &dest, const src_exp &src,                     \
                       typename src_exp::type alpha, bool add_to,              \
...
template <typename dest_exp, typename src_exp>
void matrix_assign_blas_proxy(dest_exp &dest, const src_exp &src,
                              typename src_exp::type alpha, bool add_to,
                              bool transpose);
/*!
    requires
...
template <typename dest_exp, typename src_exp, typename src_exp2>
void matrix_assign_blas_proxy(dest_exp &dest,
...
template <typename dest_exp, typename src_exp, bool Sb>
void matrix_assign_blas_proxy(dest_exp &dest,
...
template <typename T, long NR, long NC, typename MM, typename L,
          typename src_exp>
void matrix_assign_blas(matrix<T, NR, NC, MM, L> &dest, const src_exp &src);
...
void matrix_assign_blas_proxy(dest_exp &dest,
                              ...
                              typename src_exp::type alpha, bool add_to,
                              bool transpose) {
  if (has_matrix_multiply<src_exp>::value ||
      has_matrix_multiply<src_exp2>::value) {
...
// Once we get into this function it means that we are dealing with a matrix of
// float, double, complex<float>, or complex<double> and the src_exp contains at
// least one matrix multiply.

template <typename T, long NR, long NC, typename MM, typename L, long NR2,
          long NC2, bool Sb>
void matrix_assign_blas(
    matrix<T, NR, NC, MM, L> &dest,
...
template <typename T, long NR, long NC, typename MM, typename L,
          typename src_exp>
void matrix_assign_blas(assignable_sub_matrix<T, NR, NC, MM, L> &dest,
                        const src_exp &src) {
...
template <typename T, typename src_exp>
void matrix_assign_blas(assignable_ptr_matrix<T> &dest, const src_exp &src) {
...
template <typename T, long NR, long NC, typename MM, typename L,
          typename src_exp>
void matrix_assign_blas(assignable_row_matrix<T, NR, NC, MM, L> &dest,
                        const src_exp &src) {
  if (src.aliases(dest.m)) {
    matrix<T, NR, NC, MM, L> temp(dest.nr(), dest.nc());
    matrix_assign_blas_proxy(temp, src, 1, false, false);
...
template <typename T, long NR, long NC, typename MM, typename L,
          typename src_exp>
void matrix_assign_blas(assignable_col_matrix<T, NR, NC, MM, L> &dest,
                        const src_exp &src) {
  if (src.aliases(dest.m)) {
    matrix<T, NR, NC, MM, L> temp(dest.nr(), dest.nc());
    matrix_assign_blas_proxy(temp, src, 1, false, false);
...
inline typename enable_if_c<
    (is_same_type<T, float>::value || is_same_type<T, double>::value ||
     is_same_type<T, std::complex<float>>::value ||
     is_same_type<T, std::complex<double>>::value) &&
    blas_bindings::has_matrix_multiply<src_exp>::value>::type
matrix_assign_big(matrix<T, NR, NC, MM, L> &dest, const src_exp &src) {
...
inline typename enable_if_c<
    (is_same_type<T, float>::value || is_same_type<T, double>::value ||
     ...
     is_same_type<T, std::complex<double>>::value) &&
    blas_bindings::has_matrix_multiply<src_exp>::value>::type
matrix_assign_big(assignable_sub_matrix<T, NR, NC, MM, L> &dest,
                  const src_exp &src) {
  blas_bindings::matrix_assign_blas(dest, src);
}
...
inline typename enable_if_c<
    (is_same_type<T, float>::value || is_same_type<T, double>::value ||
     ...
     is_same_type<T, std::complex<double>>::value) &&
    blas_bindings::has_matrix_multiply<src_exp>::value>::type
matrix_assign_big(assignable_ptr_matrix<T> &dest, const src_exp &src) {
  blas_bindings::matrix_assign_blas(dest, src);
}
...
inline typename enable_if_c<
    (is_same_type<T, float>::value || is_same_type<T, double>::value ||
     ...
     is_same_type<T, std::complex<double>>::value) &&
    blas_bindings::has_matrix_multiply<src_exp>::value>::type
matrix_assign_big(assignable_row_matrix<T, NR, NC, MM, L> &dest,
                  const src_exp &src) {
  blas_bindings::matrix_assign_blas(dest, src);
}
...
inline typename enable_if_c<
    (is_same_type<T, float>::value || is_same_type<T, double>::value ||
     ...
     is_same_type<T, std::complex<double>>::value) &&
    blas_bindings::has_matrix_multiply<src_exp>::value>::type
matrix_assign_big(assignable_col_matrix<T, NR, NC, MM, L> &dest,
                  const src_exp &src) {
  blas_bindings::matrix_assign_blas(dest, src);
}
