...
public:
  ...
  void get_q(matrix<T, R, C, MM, L> &Q) const;
...
private:
  ...
  template <typename EXP>
  const matrix_type solve_mat(const matrix_exp<EXP> &B) const;

  template <typename EXP>
  const matrix_type solve_vect(const matrix_exp<EXP> &B) const;
#endif
  ...
  */
  typedef matrix<type, 0, 1, mem_manager_type, column_major_layout>
      column_vector_type;
...
template <typename EXP>
qr_decomposition<matrix_exp_type>::qr_decomposition(const matrix_exp<EXP> &A) {
...
template <typename matrix_exp_type>
long qr_decomposition<matrix_exp_type>::nr() const {
  return m;
}
...
template <typename matrix_exp_type>
long qr_decomposition<matrix_exp_type>::nc() const {
  return n;
}
...
template <typename matrix_exp_type>
bool qr_decomposition<matrix_exp_type>::is_full_rank() const {
...
template <typename matrix_exp_type>
const typename qr_decomposition<matrix_exp_type>::matrix_type
qr_decomposition<matrix_exp_type>::get_r() const {
  matrix_type R(n, n);
  for (long i = 0; i < n; i++) {
    for (long j = 0; j < n; j++) {
      if (i < j) {
...
template <typename matrix_exp_type>
const typename qr_decomposition<matrix_exp_type>::matrix_type
qr_decomposition<matrix_exp_type>::get_q() const {
  matrix_type Q;
  get_q(Q);
  return Q;
}
...
template <typename matrix_exp_type>
template <typename T, long R, long C, typename MM, typename L>
void qr_decomposition<matrix_exp_type>::get_q(matrix<T, R, C, MM, L> &X) const {
#ifdef DLIB_USE_LAPACK
...
template <typename EXP>
const typename qr_decomposition<matrix_exp_type>::matrix_type
qr_decomposition<matrix_exp_type>::solve(const matrix_exp<EXP> &B) const {
  COMPILE_TIME_ASSERT((is_same_type<type, typename EXP::type>::value));
  ...
  // Solve R*X = Y;
  triangular_solver(CblasLeft, CblasUpper, CblasNoTrans, CblasNonUnit, QR_, X,
                    n);
...
template <typename EXP>
const typename qr_decomposition<matrix_exp_type>::matrix_type
qr_decomposition<matrix_exp_type>::solve_vect(const matrix_exp<EXP> &B) const {
...
template <typename EXP>
const typename qr_decomposition<matrix_exp_type>::matrix_type
qr_decomposition<matrix_exp_type>::solve_mat(const matrix_exp<EXP> &B) const {
  const long nx = B.nc();
  matrix_type X(B);
  long i = 0, j = 0, k = 0;
