...
specialization of the std::complex template class.
...
template <typename T> struct is_complex<const std::complex<T> &> {
  static const bool value = true;
};
template <typename T> struct is_complex<const std::complex<T>> {
  static const bool value = true;
...
template <typename EXP> inline bool is_row_vector(const matrix_exp<EXP> &m) {
  return m.nr() == 1;
}

template <typename EXP> inline bool is_col_vector(const matrix_exp<EXP> &m) {
  return m.nc() == 1;
}

template <typename EXP> inline bool is_vector(const matrix_exp<EXP> &m) {
  return is_row_vector(m) || is_col_vector(m);
}

// ----------------------------------------------------------------------------------------

template <typename EXP> inline bool is_finite(const matrix_exp<EXP> &m) {
...
namespace impl {
template <typename T> const T &magnitude(const T &item) { return item; }
template <typename T> T magnitude(const std::complex<T> &item) {
  return std::norm(item);
}
} // namespace impl

template <typename EXP>
void find_min_and_max(const matrix_exp<EXP> &m, typename EXP::type &min_val,
...
template <typename EXP> point max_point(const matrix_exp<EXP> &m) {
  DLIB_ASSERT(m.size() > 0, "\tpoint max_point(const matrix_exp& m)"
                                << "\n\tm can't be empty"
                                << "\n\tm.size():   " << m.size()
                                << "\n\tm.nr():     " << m.nr()
                                << "\n\tm.nc():     " << m.nc());
...
template <typename EXP> point min_point(const matrix_exp<EXP> &m) {
  DLIB_ASSERT(m.size() > 0, "\tpoint min_point(const matrix_exp& m)"
                                << "\n\tm can't be empty"
                                << "\n\tm.size():   " << m.size()
                                << "\n\tm.nr():     " << m.nr()
                                << "\n\tm.nc():     " << m.nc());
...
template <typename EXP> long index_of_max(const matrix_exp<EXP> &m) {
...
template <typename EXP> long index_of_min(const matrix_exp<EXP> &m) {
...
template <typename EXP>
const typename matrix_exp<EXP>::type max(const matrix_exp<EXP> &m) {
  DLIB_ASSERT(m.size() > 0,
...
template <typename EXP>
const typename matrix_exp<EXP>::type min(const matrix_exp<EXP> &m) {
  DLIB_ASSERT(m.size() > 0,
...
template <typename M1, typename M2> struct op_binary_min : basic_op_mm<M1, M2> {
...
inline const matrix_op<op_binary_min<EXP1, EXP2>>
min_pointwise(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP1::type, typename EXP2::type>::value == true));
  ...
  DLIB_ASSERT(a.nr() == b.nr() && a.nc() == b.nc(),
              "\t const matrix_exp min_pointwise(const matrix_exp& a, const "
              "matrix_exp& b)"
                  << "\n\ta.nr(): " << a.nr() << "\n\ta.nc(): " << a.nc()
                  << "\n\tb.nr(): " << b.nr() << "\n\tb.nc(): " << b.nc());
...
min_pointwise(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b,
  ...
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP1::type, typename EXP2::type>::value == true));
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP2::type, typename EXP3::type>::value == true));
  COMPILE_TIME_ASSERT(EXP1::NR == EXP2::NR || EXP1::NR == 0 || EXP2::NR == 0);
  ...
  DLIB_ASSERT(a.nr() == b.nr() && a.nc() == b.nc() && b.nr() == c.nr() &&
                  b.nc() == c.nc(),
              "\tconst matrix_exp min_pointwise(a,b,c)"
                  << "\n\tYou can only make a do a pointwise min between "
...
template <typename M1, typename M2> struct op_binary_max : basic_op_mm<M1, M2> {
...
template <typename EXP1, typename EXP2>
inline const matrix_op<op_binary_max<EXP1, EXP2>>
max_pointwise(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP1::type, typename EXP2::type>::value == true));
  COMPILE_TIME_ASSERT(EXP1::NR == EXP2::NR || EXP1::NR == 0 || EXP2::NR == 0);
  COMPILE_TIME_ASSERT(EXP1::NC == EXP2::NC || EXP1::NC == 0 || EXP2::NC == 0);
  DLIB_ASSERT(a.nr() == b.nr() && a.nc() == b.nc(),
              "\t const matrix_exp max_pointwise(const matrix_exp& a, const "
              "matrix_exp& b)"
                  << "\n\ta.nr(): " << a.nr() << "\n\ta.nc(): " << a.nc()
                  << "\n\tb.nr(): " << b.nr() << "\n\tb.nc(): " << b.nc());
...
max_pointwise(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b,
  ...
  DLIB_ASSERT(a.nr() == b.nr() && a.nc() == b.nc() && b.nr() == c.nr() &&
              ...
              "\tconst matrix_exp max_pointwise(a,b,c)"
                  << "\n\tYou can only make a do a pointwise max between "
                     "equally sized matrices"
                  << "\n\ta.nr(): " << a.nr() << "\n\ta.nc(): " << a.nc()
                  << "\n\tb.nr(): " << b.nr() << "\n\tb.nc(): " << b.nc()
                  << "\n\tc.nr(): " << c.nr() << "\n\tc.nc(): " << c.nc());
...
template <typename EXP>
typename enable_if_c<std::numeric_limits<typename EXP::type>::is_integer,
                     double>::type
length(const matrix_exp<EXP> &m) {
...
template <typename EXP>
typename disable_if_c<std::numeric_limits<typename EXP::type>::is_integer,
                      const typename EXP::type>::type
length(const matrix_exp<EXP> &m) {
  DLIB_ASSERT(is_vector(m) == true,
              "\ttype length(const matrix_exp& m)"
                  << "\n\tm must be a row or column vector"
...
template <typename M> struct op_trans {
  ...
  template <typename U>
  bool destructively_aliases(const matrix_exp<U> &item) const {
    return m.aliases(item);
  }
};

template <typename M>
const matrix_op<op_trans<M>> trans(const matrix_exp<M> &m) {
...
template <typename M>
const matrix_diag_exp<M> &trans(const matrix_diag_exp<M> &m) {
  return m;
}
...
template <typename EXP> struct is_not_ct_vector {
  static const bool value = (EXP::NR != 1 && EXP::NC != 1);
};

template <typename EXP1, typename EXP2>
typename enable_if_c<(is_not_ct_vector<EXP1>::value) ||
                         (is_not_ct_vector<EXP2>::value),
...
dot(const matrix_exp<EXP1> &m1, const matrix_exp<EXP2> &m2) {
  ...
  COMPILE_TIME_ASSERT(EXP1::NR * EXP1::NC == 0 || EXP2::NR * EXP2::NC == 0);

  DLIB_ASSERT(
      is_vector(m1) && is_vector(m2) && m1.size() == m2.size() && m1.size() > 0,
      "\t type dot(const matrix_exp& m1, const matrix_exp& m2)"
          << "\n\t You can only compute the dot product between non-empty "
             "vectors of equal length."
          << "\n\t is_vector(m1): " << is_vector(m1) << "\n\t is_vector(m2): "
          << is_vector(m2) << "\n\t m1.size():     " << m1.size()
          << "\n\t m2.size():     " << m2.size());

  if (is_col_vector(m1) && is_col_vector(m2))
    return (trans(m1) * m2)(0);
  if (is_col_vector(m1) && is_row_vector(m2))
    return (m2 * m1)(0);
  if (is_row_vector(m1) && is_col_vector(m2))
    return (m1 * m2)(0);
...
template <typename EXP1, typename EXP2>
typename enable_if_c<EXP1::NR == 1 && EXP2::NR == 1 && EXP1::NC != 1 &&
                         EXP2::NC != 1,
                     typename EXP1::type>::type
dot(const matrix_exp<EXP1> &m1, const matrix_exp<EXP2> &m2) {
...
template <typename EXP1, typename EXP2>
typename enable_if_c<EXP1::NR == 1 && EXP2::NC == 1 && EXP1::NC != 1 &&
                         EXP2::NR != 1,
                     typename EXP1::type>::type
dot(const matrix_exp<EXP1> &m1, const matrix_exp<EXP2> &m2) {
  DLIB_ASSERT(m1.size() == m2.size(),
...
template <typename EXP1, typename EXP2>
typename enable_if_c<EXP1::NC == 1 && EXP2::NR == 1 && EXP1::NR != 1 &&
                         EXP2::NC != 1,
                     typename EXP1::type>::type
dot(const matrix_exp<EXP1> &m1, const matrix_exp<EXP2> &m2) {
  DLIB_ASSERT(m1.size() == m2.size(),
...
template <typename EXP1, typename EXP2>
typename enable_if_c<EXP1::NC == 1 && EXP2::NC == 1 && EXP1::NR != 1 &&
                         EXP2::NR != 1,
                     typename EXP1::type>::type
dot(const matrix_exp<EXP1> &m1, const matrix_exp<EXP2> &m2) {
  DLIB_ASSERT(m1.size() == m2.size(),
...
template <typename EXP1, typename EXP2>
typename enable_if_c<(EXP1::NC * EXP1::NR == 1) || (EXP2::NC * EXP2::NR == 1),
                     typename EXP1::type>::type
dot(const matrix_exp<EXP1> &m1, const matrix_exp<EXP2> &m2) {
  DLIB_ASSERT(m1.size() == m2.size(),
              "\t type dot(const matrix_exp& m1, const matrix_exp& m2)"
...
template <typename M, long R, long C> struct op_removerc {
...
template <typename M> struct op_removerc2 {
  op_removerc2(const M &m_, const long R_, const long C_)
      : m(m_), R(R_), C(C_) {}
  const M &m;
  const long R;
  const long C;
...
const matrix_op<op_removerc<EXP, R, C>> removerc(const matrix_exp<EXP> &m) {
  ...
  DLIB_ASSERT(m.nr() > R && R >= 0 && m.nc() > C && C >= 0,
              "\tconst matrix_exp removerc<R,C>(const matrix_exp& m)"
                  << "\n\tYou can't remove a row/column from a matrix if it "
                     "doesn't have that row/column"
                  << "\n\tm.nr(): " << m.nr() << "\n\tm.nc(): " << m.nc()
                  << "\n\tR:      " << R << "\n\tC:      " << C);
...
template <typename EXP>
const matrix_op<op_removerc2<EXP>> removerc(const matrix_exp<EXP> &m, long R,
                                            long C) {
  DLIB_ASSERT(m.nr() > R && R >= 0 && m.nc() > C && C >= 0,
...
template <typename M, long C> struct op_remove_col {
...
template <typename M> struct op_remove_col2 {
...
template <long C, typename EXP>
const matrix_op<op_remove_col<EXP, C>> remove_col(const matrix_exp<EXP> &m) {
  // You can't remove the given column from the matrix because the matrix
  // doesn't have a column with that index.
...
template <typename EXP>
const matrix_op<op_remove_col2<EXP>> remove_col(const matrix_exp<EXP> &m,
                                                long C) {
  DLIB_ASSERT(
      m.nc() > C && C >= 0,
...
template <typename M, long R> struct op_remove_row {
...
template <typename M> struct op_remove_row2 {
...
template <long R, typename EXP>
const matrix_op<op_remove_row<EXP, R>> remove_row(const matrix_exp<EXP> &m) {
...
template <typename EXP>
const matrix_op<op_remove_row2<EXP>> remove_row(const matrix_exp<EXP> &m,
                                                long R) {
  DLIB_ASSERT(
      m.nr() > R && R >= 0,
...
template <typename M> struct op_diagm {
...
template <typename EXP>
const matrix_diag_op<op_diagm<EXP>> diagm(const matrix_exp<EXP> &m) {
  // You can only make a diagonal matrix out of a row or column vector
  COMPILE_TIME_ASSERT(EXP::NR == 0 || EXP::NR == 1 || EXP::NC == 1 ||
                      EXP::NC == 0);
...
template <typename M1, typename M2> struct op_diagm_mult : basic_op_mm<M1, M2> {
...
template <typename EXP1, typename EXP2>
inline const matrix_diag_op<op_diagm_mult<EXP1, EXP2>>
operator*(const matrix_diag_exp<EXP1> &a, const matrix_diag_exp<EXP2> &b) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP1::type, typename EXP2::type>::value));
  COMPILE_TIME_ASSERT(EXP1::NR == EXP2::NR || EXP1::NR == 0 || EXP2::NR == 0);
  COMPILE_TIME_ASSERT(EXP1::NC == EXP2::NC || EXP1::NC == 0 || EXP2::NC == 0);
  DLIB_ASSERT(a.nr() == b.nr() && a.nc() == b.nc(),
              "\tconst matrix_exp operator(const matrix_diag_exp& a, const "
              "matrix_diag_exp& b)"
                  << "\n\tYou can only multiply diagonal matrices together if "
...
template <typename M> struct op_diag {
...
template <typename EXP>
const matrix_op<op_diag<EXP>> diag(const matrix_exp<EXP> &m) {
  typedef op_diag<EXP> op;
  return matrix_op<op>(op(m.ref()));
}

template <typename EXP> struct diag_exp {
  typedef matrix_op<op_diag<EXP>> type;
};

// ----------------------------------------------------------------------------------------

template <typename M, typename target_type> struct op_cast {
...
template <typename target_type, typename EXP>
const matrix_op<op_cast<EXP, target_type>>
matrix_cast(const matrix_exp<EXP> &m) {
...
template <typename EXP, typename S>
const typename enable_if<is_built_in_scalar_type<S>,
                         matrix_op<op_lessthan<EXP, S>>>::type
operator<(const matrix_exp<EXP> &m, const S &s) {
  // you can only use this relational operator with the built in scalar types
  // like long, float, etc.
...
const typename enable_if<is_built_in_scalar_type<S>,
                         matrix_op<op_lessthan<EXP, S>>>::type
operator>(const S &s, const matrix_exp<EXP> &m) {
  // you can only use this relational operator with the built in scalar types
  // like long, float, etc.
  COMPILE_TIME_ASSERT(is_built_in_scalar_type<typename EXP::type>::value);
...
template <typename EXP, typename S>
const typename enable_if<is_built_in_scalar_type<S>,
                         matrix_op<op_lessthan_eq<EXP, S>>>::type
operator<=(const matrix_exp<EXP> &m, const S &s) {
  // you can only use this relational operator with the built in scalar types
  // like long, float, etc.
  COMPILE_TIME_ASSERT(is_built_in_scalar_type<typename EXP::type>::value);
...
template <typename EXP, typename S>
const typename enable_if<is_built_in_scalar_type<S>,
                         matrix_op<op_greaterthan<EXP, S>>>::type
operator>(const matrix_exp<EXP> &m, const S &s) {
  // you can only use this relational operator with the built in scalar types
  // like long, float, etc.
  COMPILE_TIME_ASSERT(is_built_in_scalar_type<typename EXP::type>::value);
...
template <typename EXP, typename S>
const typename enable_if<is_built_in_scalar_type<S>,
                         matrix_op<op_greaterthan_eq<EXP, S>>>::type
operator>=(const matrix_exp<EXP> &m, const S &s) {
  // you can only use this relational operator with the built in scalar types
  // like long, float, etc.
  COMPILE_TIME_ASSERT(is_built_in_scalar_type<typename EXP::type>::value);
...
template <typename EXP, typename S>
const typename enable_if<is_built_in_scalar_type<S>,
                         matrix_op<op_equal_to<EXP, S>>>::type
operator==(const matrix_exp<EXP> &m, const S &s) {
  // you can only use this relational operator with the built in scalar types
  // like long, float, etc.
  COMPILE_TIME_ASSERT(is_built_in_scalar_type<typename EXP::type>::value);
...
template <typename EXP, typename S>
const typename enable_if<is_built_in_scalar_type<S>,
                         matrix_op<op_not_equal_to<EXP, S>>>::type
operator!=(const matrix_exp<EXP> &m, const S &s) {
  // you can only use this relational operator with the built in scalar types
  // like long, float, etc.
  COMPILE_TIME_ASSERT(is_built_in_scalar_type<typename EXP::type>::value);
...
template <typename T, long NR, long NC, typename MM, typename U, typename L>
typename disable_if<is_matrix<U>, void>::type
set_all_elements(matrix<T, NR, NC, MM, L> &m, const U &value) {
...
template <typename T, long NR, long NC, typename MM, typename U, typename L>
typename enable_if<is_matrix<U>, void>::type
set_all_elements(matrix<T, NR, NC, MM, L> &m, const U &value) {
  for (long r = 0; r < m.nr(); ++r) {
    for (long c = 0; c < m.nc(); ++c) {
      m(r, c) = value;
...
template <typename EXP>
inline const typename matrix_exp<EXP>::matrix_type
tmp(const matrix_exp<EXP> &m) {
  return typename matrix_exp<EXP>::matrix_type(m);
}

// ----------------------------------------------------------------------------------------

template <typename EXP> constexpr bool is_row_major(const matrix_exp<EXP> &) {
...
template <typename EXP>
const typename lazy_disable_if<is_matrix<typename EXP::type>, EXP>::type
sum(const matrix_exp<EXP> &m) {
  typedef typename matrix_exp<EXP>::type type;
...
template <typename EXP>
const typename lazy_enable_if<is_matrix<typename EXP::type>, EXP>::type
sum(const matrix_exp<EXP> &m) {
  typedef typename matrix_exp<EXP>::type type;
...
template <typename M> struct op_sumr {
...
template <typename EXP>
const matrix_op<op_sumr<EXP>> sum_rows(const matrix_exp<EXP> &m) {
  DLIB_ASSERT(m.size() > 0, "\tconst matrix_exp sum_rows(m)"
...
template <typename M> struct op_sumc {
...
template <typename EXP>
const matrix_op<op_sumc<EXP>> sum_cols(const matrix_exp<EXP> &m) {
  DLIB_ASSERT(m.size() > 0, "\tconst matrix_exp sum_cols(m)"
                                << "\n\t The matrix can't be empty"
                                << "\n\t m.size(): " << m.size());
...
template <typename EXP>
inline const typename disable_if<is_complex<typename EXP::type>,
                                 typename matrix_exp<EXP>::type>::type
mean(const matrix_exp<EXP> &m) {
...
template <typename EXP>
inline const typename enable_if<is_complex<typename EXP::type>,
                                typename matrix_exp<EXP>::type>::type
mean(const matrix_exp<EXP> &m) {
...
template <typename EXP>
const typename matrix_exp<EXP>::type variance(const matrix_exp<EXP> &m) {
  using dlib::pow;
  using std::pow;
...
template <typename EXP>
const typename matrix_exp<EXP>::type stddev(const matrix_exp<EXP> &m) {
  using dlib::sqrt;
  using std::sqrt;
...
template <typename EXP> struct visual_studio_sucks_cov_helper {
  typedef typename EXP::type inner_type;
  typedef matrix<typename inner_type::type, inner_type::NR, inner_type::NR,
                 typename EXP::mem_manager_type>
      type;
};

template <typename EXP>
const typename visual_studio_sucks_cov_helper<EXP>::type
covariance(const matrix_exp<EXP> &m) {
  ...
  // perform static checks to make sure the matrices contained in m are column
  // vectors
  ...
  for (long i = 0; i < m.nr(); ++i) {
    DLIB_ASSERT(
        m(0).size() == m(i).size() && m(i).size() > 0 && is_col_vector(m(i)),
        "\tconst matrix covariance(const matrix_exp& m)"
            << "\n\tYou can only apply covariance() to a column matrix of "
               "column matrices"
            << "\n\tm(0).size(): " << m(0).size()
            << "\n\tm(i).size(): " << m(i).size() << "\n\tis_col_vector(m(i)): "
            << (is_col_vector(m(i)) ? "true" : "false")
            << "\n\ti:         " << i);
  }
#endif
...
template <typename EXP>
const typename matrix_exp<EXP>::type prod(const matrix_exp<EXP> &m) {
  typedef typename matrix_exp<EXP>::type type;
...
template <typename T> struct op_uniform_matrix_3 : does_not_alias {
...
template <typename T>
const matrix_op<op_uniform_matrix_3<T>> uniform_matrix(long nr, long nc,
                                                       const T &val) {
...
template <typename T>
const matrix_op<op_uniform_matrix_3<T>> zeros_matrix(long nr, long nc) {
  DLIB_ASSERT(nr >= 0 && nc >= 0, "\tconst matrix_exp zeros_matrix<T>(nr, nc)"
...
template <typename EXP>
const matrix_op<op_uniform_matrix_3<typename EXP::type>>
zeros_matrix(const matrix_exp<EXP> &mat) {
...
template <typename T>
const matrix_op<op_uniform_matrix_3<T>> ones_matrix(long nr, long nc) {
  DLIB_ASSERT(nr >= 0 && nc >= 0, "\tconst matrix_exp ones_matrix<T>(nr, nc)"
                                      << "\n\tnr and nc have to be >= 0"
                                      << "\n\tnr: " << nr << "\n\tnc: " << nc);
  typedef op_uniform_matrix_3<T> op;
...
template <typename EXP>
const matrix_op<op_uniform_matrix_3<typename EXP::type>>
ones_matrix(const matrix_exp<EXP> &mat) {
  DLIB_ASSERT(mat.nr() >= 0 && mat.nc() >= 0,
...
template <typename T, long NR_, long NC_>
...
const matrix_op<op_uniform_matrix_2<T, NR, NC>> uniform_matrix(const T &val) {
...
template <typename T, long NR_, long NC_, T val>
...
template <typename T, long NR, long NC, T val>
...
struct op_gaussian_randm : does_not_alias {
  op_gaussian_randm(long nr_, long nc_, unsigned long seed_)
  ...
  const_ret_type apply(long r, long c) const {
    return gaussian_random_hash(r, c, seed);
  }
...
inline const matrix_op<op_gaussian_randm>
gaussian_randm(long nr, long nc, unsigned long seed = 0) {
  DLIB_ASSERT(nr >= 0 && nc >= 0, "\tmatrix_exp gaussian_randm(nr, nc, seed)"
...
template <typename M> struct op_add_diag {
  op_add_diag(const M &m_, const typename M::type &value_)
      : m(m_), value(value_) {}
  const M &m;
...
template <typename T> struct op_identity_matrix_2 : does_not_alias {
...
const matrix_diag_op<op_identity_matrix_2<T>> identity_matrix(const U &size) {
  // the size argument must be some scalar value, not a matrix!
  COMPILE_TIME_ASSERT(is_matrix<U>::value == false);

  DLIB_ASSERT(size > 0, "\tconst matrix_exp identity_matrix<T>(size)"
...
template <typename EXP>
const matrix_diag_op<op_identity_matrix_2<typename EXP::type>>
identity_matrix(const matrix_exp<EXP> &mat) {
  DLIB_ASSERT(mat.nr() == mat.nc(), "\tconst matrix_exp identity_matrix(mat)"
...
const matrix_op<op_add_diag<EXP>>
operator+(const matrix_exp<EXP> &lhs,
  ...
  // You can only add matrices together if they both have the same number of
  // rows and columns.
  DLIB_ASSERT(
      lhs.nc() == rhs.nc() && lhs.nr() == rhs.nr(),
      "\tconst matrix_exp operator+(const matrix_exp& lhs, const matrix_exp& "
      "rhs)"
...
template <typename EXP, typename T>
const matrix_op<op_add_diag<EXP>>
operator+(const matrix_exp<matrix_diag_op<op_identity_matrix_2<T>>>
              &DLIB_IF_ASSERT(lhs),
...
template <typename T, long N> struct op_const_diag_matrix : does_not_alias {
  op_const_diag_matrix(const long &size_, const T &value_)
...
template <typename T, typename U>
const typename disable_if<is_matrix<U>,
                          matrix_diag_op<op_const_diag_matrix<T, 0>>>::type
operator*(const matrix_exp<matrix_diag_op<op_identity_matrix_2<T>>> &m,
          const U &value) {
...
template <typename EXP, typename T, long N>
const matrix_op<op_add_diag<EXP>>
operator+(const matrix_exp<EXP> &lhs,
...
template <typename EXP, typename T, long N>
const matrix_op<op_add_diag<EXP>>
operator+(const matrix_exp<matrix_diag_op<op_const_diag_matrix<T, N>>> &lhs,
          const matrix_exp<EXP> &rhs) {
  // both matrices must contain the same type of element
  COMPILE_TIME_ASSERT((is_same_type<T, typename EXP::type>::value == true));
...
template <typename T, long N> struct op_identity_matrix : does_not_alias {
  const static long cost = 1;
  const static long NR = N;
  const static long NC = N;
  typedef default_memory_manager mem_manager_type;
...
const typename disable_if<is_matrix<U>,
                          matrix_diag_op<op_const_diag_matrix<T, N>>>::type
operator*(const matrix_exp<matrix_diag_op<op_identity_matrix<T, N>>> &m,
          const U &value) {
...
template <typename EXP, typename T, long N>
const matrix_op<op_add_diag<EXP>>
operator+(const matrix_exp<matrix_diag_op<op_identity_matrix<T, N>>>
              &DLIB_IF_ASSERT(lhs),
          const matrix_exp<EXP> &rhs) {
  // both matrices must contain the same type of element
  COMPILE_TIME_ASSERT((is_same_type<T, typename EXP::type>::value == true));
...
template <typename M, long R, long C> struct op_rotate {
...
template <long R, long C, typename EXP>
const matrix_op<op_rotate<EXP, R, C>> rotate(const matrix_exp<EXP> &m) {
...
namespace impl {
// A template to tell me if two types can be multiplied together in a sensible
// way.  Here I'm saying it is ok if they are both the same type or one is the
// complex version of the other.
template <typename T, typename U> struct compatible {
  static const bool value = false;
  typedef T type;
};
template <typename T> struct compatible<T, T> {
  static const bool value = true;
  typedef T type;
};
template <typename T> struct compatible<std::complex<T>, T> {
  static const bool value = true;
  typedef std::complex<T> type;
};
template <typename T> struct compatible<T, std::complex<T>> {
  static const bool value = true;
  typedef std::complex<T> type;
};
} // namespace impl
...
struct op_pointwise_multiply : basic_op_mm<M1, M2> {
  op_pointwise_multiply(const M1 &m1_, const M2 &m2_)
      : basic_op_mm<M1, M2>(m1_, m2_) {}

  typedef typename impl::compatible<typename M1::type, typename M2::type>::type
      type;
  typedef const type const_ret_type;
  const static long cost = M1::cost + M2::cost + 1;
...
template <typename EXP1, typename EXP2>
inline const matrix_op<op_pointwise_multiply<EXP1, EXP2>>
pointwise_multiply(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b) {
  COMPILE_TIME_ASSERT(
      (impl::compatible<typename EXP1::type, typename EXP2::type>::value ==
       true));
  COMPILE_TIME_ASSERT(EXP1::NR == EXP2::NR || EXP1::NR == 0 || EXP2::NR == 0);
  COMPILE_TIME_ASSERT(EXP1::NC == EXP2::NC || EXP1::NC == 0 || EXP2::NC == 0);
  DLIB_ASSERT(a.nr() == b.nr() && a.nc() == b.nc(),
              "\tconst matrix_exp pointwise_multiply(const matrix_exp& a, "
              "const matrix_exp& b)"
                  << "\n\tYou can only make a do a pointwise multiply with two "
                     "equally sized matrices"
                  << "\n\ta.nr(): " << a.nr() << "\n\ta.nc(): " << a.nc()
                  << "\n\tb.nr(): " << b.nr() << "\n\tb.nc(): " << b.nc());
...
pointwise_multiply(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b,
  ...
  DLIB_ASSERT(a.nr() == b.nr() && a.nc() == b.nc() && b.nr() == c.nr() &&
              ...
              "\tconst matrix_exp pointwise_multiply(a,b,c)"
                  << "\n\tYou can only make a do a pointwise multiply between "
                     "equally sized matrices"
                  << "\n\ta.nr(): " << a.nr() << "\n\ta.nc(): " << a.nc()
                  << "\n\tb.nr(): " << b.nr() << "\n\tb.nc(): " << b.nc()
                  << "\n\tc.nr(): " << c.nr() << "\n\tc.nc(): " << c.nc());
...
struct op_pointwise_multiply4 : basic_op_mmmm<M1, M2, M3, M4> {
  op_pointwise_multiply4(const M1 &m1_, const M2 &m2_, const M3 &m3_,
                         const M4 &m4_)
...
inline const matrix_op<op_pointwise_multiply4<EXP1, EXP2, EXP3, EXP4>>
pointwise_multiply(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b,
  ...
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP2::type, typename EXP3::type>::value == true));
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP3::type, typename EXP4::type>::value == true));
  COMPILE_TIME_ASSERT(EXP1::NR == EXP2::NR || EXP1::NR == 0 || EXP2::NR == 0);
  COMPILE_TIME_ASSERT(EXP1::NC == EXP2::NC || EXP1::NC == 0 || EXP2::NC == 0);
  COMPILE_TIME_ASSERT(EXP2::NR == EXP3::NR || EXP2::NR == 0 || EXP3::NR == 0);
  COMPILE_TIME_ASSERT(EXP2::NC == EXP3::NC || EXP2::NC == 0 || EXP3::NC == 0);
...
template <typename P,
          int type =
              static_switch<pixel_traits<P>::grayscale, pixel_traits<P>::rgb,
                            pixel_traits<P>::hsi, pixel_traits<P>::rgb_alpha,
                            pixel_traits<P>::lab>::value>
struct pixel_to_vector_helper;

template <typename P> struct pixel_to_vector_helper<P, 1> {
  template <typename M> static void assign(M &m, const P &pixel) {
    m(0) = static_cast<typename M::type>(pixel);
  }
};

template <typename P> struct pixel_to_vector_helper<P, 2> {
  template <typename M> static void assign(M &m, const P &pixel) {
...
template <typename P> struct pixel_to_vector_helper<P, 3> {
  template <typename M> static void assign(M &m, const P &pixel) {
...
template <typename P> struct pixel_to_vector_helper<P, 4> {
  template <typename M> static void assign(M &m, const P &pixel) {
    m(0) = static_cast<typename M::type>(pixel.red);
    m(1) = static_cast<typename M::type>(pixel.green);
    m(2) = static_cast<typename M::type>(pixel.blue);
...
template <typename P> struct pixel_to_vector_helper<P, 5> {
  template <typename M> static void assign(M &m, const P &pixel) {
...
inline const matrix<T, pixel_traits<P>::num, 1>
pixel_to_vector(const P &pixel) {
...
template <typename P> struct vector_to_pixel_helper<P, 1> {
  template <typename M> static void assign(P &pixel, const M &m) {
    pixel = static_cast<unsigned char>(m(0));
  }
};

template <typename P> struct vector_to_pixel_helper<P, 2> {
  template <typename M> static void assign(P &pixel, const M &m) {
...
template <typename P> struct vector_to_pixel_helper<P, 3> {
  template <typename M> static void assign(P &pixel, const M &m) {
...
template <typename P> struct vector_to_pixel_helper<P, 4> {
  template <typename M> static void assign(P &pixel, const M &m) {
    pixel.red = static_cast<unsigned char>(m(0));
    pixel.green = static_cast<unsigned char>(m(1));
    pixel.blue = static_cast<unsigned char>(m(2));
...
template <typename P> struct vector_to_pixel_helper<P, 5> {
  template <typename M> static void assign(P &pixel, const M &m) {
...
template <typename P, typename EXP>
inline void vector_to_pixel(P &pixel, const matrix_exp<EXP> &vector) {
...
template <typename M, long lower, long upper> struct op_clamp : basic_op_m<M> {
...
template <long l, long u, typename EXP>
const matrix_op<op_clamp<EXP, l, u>> clamp(const matrix_exp<EXP> &m) {
...
template <typename M> struct op_clamp2 : basic_op_m<M> {
  typedef typename M::type type;
...
template <typename EXP>
const matrix_op<op_clamp2<EXP>> clamp(const matrix_exp<EXP> &m,
...
clamp(const matrix_exp<EXP1> &m, const matrix_exp<EXP2> &lower,
  ...
  COMPILE_TIME_ASSERT(EXP2::NR == EXP3::NR || EXP2::NR == 0 || EXP3::NR == 0);
  COMPILE_TIME_ASSERT(EXP2::NC == EXP3::NC || EXP2::NC == 0 || EXP3::NC == 0);
  DLIB_ASSERT(m.nr() == lower.nr() && m.nc() == lower.nc() &&
                  m.nr() == upper.nr() && m.nc() == upper.nc(),
...
template <typename M> struct op_lowerbound : basic_op_m<M> {
  typedef typename M::type type;
...
template <typename EXP>
const matrix_op<op_lowerbound<EXP>>
lowerbound(const matrix_exp<EXP> &m, const typename EXP::type &thresh) {
...
template <typename M> struct op_upperbound : basic_op_m<M> {
  typedef typename M::type type;
...
template <typename EXP>
const matrix_op<op_upperbound<EXP>>
upperbound(const matrix_exp<EXP> &m, const typename EXP::type &thresh) {
...
template <typename M> struct op_reshape {
  op_reshape(const M &m_, const long &rows_, const long &cols_)
...
template <typename EXP>
const matrix_op<op_reshape<EXP>> reshape(const matrix_exp<EXP> &m,
                                         const long &rows, const long &cols) {
  DLIB_ASSERT(m.size() == rows * cols && rows > 0 && cols > 0,
              "\tconst matrix_exp reshape(m, rows, cols)"
                  << "\n\t The size of m must match the dimensions you want to "
                     "reshape it into."
                  << "\n\t m.size():  " << m.size()
                  << "\n\t rows*cols: " << rows * cols
                  << "\n\t rows:      " << rows << "\n\t cols:      " << cols);
...
template <typename EXP1, typename EXP2>
typename disable_if<is_complex<typename EXP1::type>, bool>::type
equal(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b,
      const typename EXP1::type eps =
          100 * std::numeric_limits<typename EXP1::type>::epsilon()) {
...
template <typename EXP1, typename EXP2>
typename enable_if<is_complex<typename EXP1::type>, bool>::type
equal(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b,
      const typename EXP1::type::value_type eps =
          100 *
          std::numeric_limits<typename EXP1::type::value_type>::epsilon()) {
  // check if the dimensions don't match
  if (a.nr() != b.nr() || a.nc() != b.nc())
    return false;
...
template <typename M1, typename M2> struct op_scale_columns {
...
template <typename EXP1, typename EXP2>
const matrix_op<op_scale_columns<EXP1, EXP2>>
scale_columns(const matrix_exp<EXP1> &m, const matrix_exp<EXP2> &v) {
  ...
  const long v_len =
      ((EXP2::NR) * (EXP2::NC) == 0) ? 0 : (tmax<EXP2::NR, EXP2::NC>::value);
  ...
  DLIB_ASSERT(is_vector(v) == true && v.size() == m.nc(),
              "\tconst matrix_exp scale_columns(m, v)"
                  << "\n\tv must be a row or column vector and its length must "
                     "match the number of columns in m"
                  << "\n\tm.nr(): " << m.nr() << "\n\tm.nc(): " << m.nc()
                  << "\n\tv.nr(): " << v.nr() << "\n\tv.nc(): " << v.nc());
...
template <typename M1, typename M2> struct op_scale_columns_diag {
...
template <typename EXP1, typename EXP2>
const matrix_op<op_scale_columns_diag<EXP1, EXP2>>
operator*(const matrix_exp<EXP1> &m, const matrix_diag_exp<EXP2> &d) {
  // Both arguments to this function must contain the same type of element
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP1::type, typename EXP2::type>::value == true));
...
template <typename M1, typename M2> struct op_scale_rows {
...
template <typename EXP1, typename EXP2>
const matrix_op<op_scale_rows<EXP1, EXP2>>
scale_rows(const matrix_exp<EXP1> &m, const matrix_exp<EXP2> &v) {
  // Both arguments to this function must contain the same type of element
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP1::type, typename EXP2::type>::value == true));
  // The v argument must be a row or column vector.
...
template <typename M1, typename M2> struct op_scale_rows_diag {
...
template <typename EXP1, typename EXP2>
const matrix_op<op_scale_rows_diag<EXP1, EXP2>>
operator*(const matrix_diag_exp<EXP2> &d, const matrix_exp<EXP1> &m) {
  // Both arguments to this function must contain the same type of element
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP1::type, typename EXP2::type>::value == true));
...
/*
    The idea here is to catch expressions of the form d*M*d where d is diagonal
   and M is some square matrix and turn them into something equivalent to
    pointwise_multiply(diag(d)*trans(diag(d)), M).

    The reason for this is that doing it this way is more numerically stable. In
   particular, doing 2 matrix multiplies as suggested by d*M*d could result in
   an asymmetric matrix even if M is symmetric to begin with.
*/

template <typename M1, typename M2, typename M3> struct op_diag_m_diag {
  // This operator represents M1*M2*M3 where M1 and M3 are diagonal

  op_diag_m_diag(const M1 &m1_, const M2 &m2_, const M3 &m3_)
  ...
  template <typename U>
  bool destructively_aliases(const matrix_exp<U> &item) const {
    return m2.destructively_aliases(item) || m1.aliases(item) ||
           m3.aliases(item);
  }
};
...
template <typename EXP1, typename EXP2, typename EXP3>
const matrix_op<op_diag_m_diag<EXP1, EXP2, EXP3>>
operator*(const matrix_diag_exp<EXP1> &d,
  ...
  // figure out the compile time known length of d
  const long v_len =
      ((EXP1::NR) * (EXP1::NC) == 0) ? 0 : (tmax<EXP1::NR, EXP1::NC>::value);
...
template <typename EXP1, typename EXP2, typename EXP3>
const matrix_op<op_diag_m_diag<EXP1, EXP2, EXP3>>
operator*(const matrix_exp<matrix_op<op_scale_rows_diag<EXP2, EXP1>>> &m,
          const matrix_diag_exp<EXP3> &d) {
  // Both arguments to this function must contain the same type of element
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP3::type, typename EXP2::type>::value == true));

  // figure out the compile time known length of d
  const long v_len =
      ((EXP3::NR) * (EXP3::NC) == 0) ? 0 : (tmax<EXP3::NR, EXP3::NC>::value);
...
struct sort_columns_sort_helper {
  template <typename T> bool operator()(const T &item1, const T &item2) const {
    return item1.first < item2.first;
  }
};

template <typename T, long NR, long NC, typename mm, typename l1, long NR2,
          long NC2, typename mm2, typename l2>
void sort_columns(matrix<T, NR, NC, mm, l1> &m,
  ...
  DLIB_ASSERT(is_col_vector(v) == true && v.size() == m.nc(),
              "\tconst matrix_exp sort_columns(m, v)"
                  << "\n\tv must be a column vector and its length must match "
...
template <typename T, long NR, long NC, typename mm, typename l1, long NR2,
          long NC2, typename mm2, typename l2>
void rsort_columns(matrix<T, NR, NC, mm, l1> &m,
                   matrix<T, NR2, NC2, mm2, l2> &v) {
  COMPILE_TIME_ASSERT(NC2 == 1 || NC2 == 0);
  COMPILE_TIME_ASSERT(NC == NR2 || NC == 0 || NR2 == 0);
...
template <typename M1, typename M2> struct op_tensor_product {
...
template <typename EXP1, typename EXP2>
inline const matrix_op<op_tensor_product<EXP1, EXP2>>
tensor_product(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP1::type, typename EXP2::type>::value == true));
...
template <typename M> struct op_make_symmetric : basic_op_m<M> {
...
template <typename EXP>
const matrix_op<op_make_symmetric<EXP>>
make_symmetric(const matrix_exp<EXP> &m) {
  DLIB_ASSERT(m.nr() == m.nc(), "\tconst matrix make_symmetric(m)"
...
template <typename M> struct op_lowerm : basic_op_m<M> {
...
template <typename M> struct op_lowerm_s : basic_op_m<M> {
  typedef typename M::type type;
...
template <typename EXP>
const matrix_op<op_lowerm<EXP>> lowerm(const matrix_exp<EXP> &m) {
  typedef op_lowerm<EXP> op;
  return matrix_op<op>(op(m.ref()));
}

template <typename EXP>
const matrix_op<op_lowerm_s<EXP>> lowerm(const matrix_exp<EXP> &m,
                                         typename EXP::type s) {
...
template <typename M> struct op_upperm : basic_op_m<M> {
...
template <typename M> struct op_upperm_s : basic_op_m<M> {
  typedef typename M::type type;
...
template <typename EXP>
const matrix_op<op_upperm<EXP>> upperm(const matrix_exp<EXP> &m) {
  typedef op_upperm<EXP> op;
  return matrix_op<op>(op(m.ref()));
}

template <typename EXP>
const matrix_op<op_upperm_s<EXP>> upperm(const matrix_exp<EXP> &m,
                                         typename EXP::type s) {
...
inline const matrix<double> randm(long nr, long nc, rand_gen &rnd) {
  DLIB_ASSERT(nr >= 0 && nc >= 0, "\tconst matrix randm(nr, nc, rnd)"
                                      << "\n\tInvalid inputs to this function"
                                      << "\n\tnr: " << nr << "\n\tnc: " << nc);
...
inline const matrix<double> randm(long nr, long nc) {
  DLIB_ASSERT(nr >= 0 && nc >= 0, "\tconst matrix randm(nr, nc)"
                                      << "\n\tInvalid inputs to this function"
                                      << "\n\tnr: " << nr << "\n\tnc: " << nc);

  matrix<double> m(nr, nc);
  // make a double that contains RAND_MAX + the smallest number that still
  // makes the resulting double slightly bigger than
...
inline const matrix_range_exp<double> linspace(double start, double end,
                                               long num) {
  DLIB_ASSERT(num >= 0, "\tconst matrix_exp linspace(start, end, num)"
                            << "\n\tInvalid inputs to this function"
                            << "\n\tstart: " << start << "\n\tend:   " << end
                            << "\n\tnum:   " << num);
...
template <typename M> struct op_linpiece {
  op_linpiece(const double val_, const M &joints_)
      : joints(joints_), val(val_) {}
...
template <typename EXP>
const matrix_op<op_linpiece<EXP>> linpiece(const double val,
...
inline const matrix_log_range_exp<double> logspace(double start, double end,
                                                   long num) {
  DLIB_ASSERT(num >= 0, "\tconst matrix_exp logspace(start, end, num)"
                            << "\n\tInvalid inputs to this function"
                            << "\n\tstart: " << start << "\n\tend:   " << end
                            << "\n\tnum:   " << num);
...
template <typename M1, typename M2> struct op_cart_prod {
...
template <typename EXP1, typename EXP2>
const matrix_op<op_cart_prod<EXP1, EXP2>>
cartesian_product(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP1::type, typename EXP2::type>::value == true));
...
template <typename M> struct op_mat_to_vect {
...
template <typename EXP>
const matrix_op<op_mat_to_vect<EXP>>
reshape_to_column_vector(const matrix_exp<EXP> &m) {
...
template <typename T, long NR_, long NC_, typename MM> struct op_mat_to_vect2 {
...
const matrix_op<op_mat_to_vect2<T, NR, NC, MM>>
reshape_to_column_vector(const matrix<T, NR, NC, MM, row_major_layout> &m) {
...
template <typename M1, typename M2> struct op_join_rows {
  op_join_rows(const M1 &m1_, const M2 &m2_)
      : m1(m1_), m2(m2_), _nr(std::max(m1.nr(), m2.nr())) {}
  const M1 &m1;
  const M2 &m2;
  const long _nr;

  template <typename T, typename U, bool selection> struct type_selector;
  ...
  // If both const_ret_types are references then we should use them as the
  // const_ret_type type but otherwise we should use the normal type.
  typedef typename M1::const_ret_type T1;
  typedef typename M1::type T2;
  typedef typename M2::const_ret_type T3;
  typedef typename type_selector<T1, T2,
                                 is_reference_type<T1>::value &&
                                     is_reference_type<T3>::value>::type
      const_ret_type;
...
template <typename EXP1, typename EXP2>
inline const matrix_op<op_join_rows<EXP1, EXP2>>
join_rows(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP1::type, typename EXP2::type>::value == true));
  // You are getting an error on this line because you are trying to join two
  // matrices that don't have the same number of rows
  ...
  DLIB_ASSERT(
      ...
      "\tconst matrix_exp join_rows(const matrix_exp& a, const matrix_exp& b)"
          << "\n\tYou can only use join_rows() if both matrices have the same "
             "number of rows"
          << "\n\ta.nr(): " << a.nr() << "\n\tb.nr(): " << b.nr()
          << "\n\ta.nc(): " << a.nc() << "\n\tb.nc(): " << b.nc());
...
template <typename M1, typename M2> struct op_join_cols {
  op_join_cols(const M1 &m1_, const M2 &m2_)
      : m1(m1_), m2(m2_), _nc(std::max(m1.nc(), m2.nc())) {}
  const M1 &m1;
  const M2 &m2;
  const long _nc;
...
template <typename EXP1, typename EXP2>
inline const matrix_op<op_join_cols<EXP1, EXP2>>
join_cols(const matrix_exp<EXP1> &a, const matrix_exp<EXP2> &b) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename EXP1::type, typename EXP2::type>::value == true));
  // You are getting an error on this line because you are trying to join two
  // matrices that don't have the same number of columns
  ...
  DLIB_ASSERT(
      ...
      "\tconst matrix_exp join_cols(const matrix_exp& a, const matrix_exp& b)"
          << "\n\tYou can only use join_cols() if both matrices have the same "
             "number of columns"
          << "\n\ta.nr(): " << a.nr() << "\n\tb.nr(): " << b.nr()
          << "\n\ta.nc(): " << a.nc() << "\n\tb.nc(): " << b.nc());
...
template <typename M> struct op_fliplr {
...
template <typename M>
const matrix_op<op_fliplr<M>> fliplr(const matrix_exp<M> &m) {
...
template <typename M> struct op_flipud {
...
template <typename M>
const matrix_op<op_flipud<M>> flipud(const matrix_exp<M> &m) {
...
template <typename M> struct op_flip {
...
template <typename M> const matrix_op<op_flip<M>> flip(const matrix_exp<M> &m) {
...
uint32 hash(const matrix<T, NR, NC, MM, L> &item, uint32 seed = 0) {
