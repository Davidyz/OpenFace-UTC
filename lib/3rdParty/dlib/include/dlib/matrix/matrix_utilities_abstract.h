...
/*!
    ensures
        - returns true if and only if the given matrix expression uses the
row_major_layout.
!*/
...
const matrix_exp trans(const matrix_exp &m);
/*!
    ensures
...
const matrix_type::type dot(const matrix_exp &m1, const matrix_exp &m2);
/*!
    requires
        - is_vector(m1) == true
        - is_vector(m2) == true
...
const matrix_exp lowerm(const matrix_exp &m);
/*!
    ensures
...
const matrix_exp upperm(const matrix_exp &m);
/*!
    ensures
        - returns a matrix M such that:
            - M::type == the same type that was in m
...
const matrix_exp make_symmetric(const matrix_exp &m);
/*!
    requires
        - m.nr() == m.nc()
    ...
    ensures
        - returns a matrix M such that:
            ...
            - M is a symmetric matrix, that is, M == trans(M) and
              it is constructed from the lower triangular part of m.
...
const matrix_exp uniform_matrix(const T &val);
/*!
    requires
        - NR > 0 && NC > 0
    ensures
...
template <typename T>
const matrix_exp uniform_matrix(long nr, long nc, const T &val);
/*!
    requires
        - nr >= 0 && nc >= 0
    ensures
...
const matrix_exp ones_matrix(const matrix_exp &mat);
/*!
    requires
        - mat.nr() >= 0 && mat.nc() >= 0
    ensures
...
template <typename T> const matrix_exp ones_matrix(long nr, long nc);
/*!
    requires
        - nr >= 0 && nc >= 0
    ensures
...
const matrix_exp zeros_matrix(const matrix_exp &mat);
/*!
    requires
        - mat.nr() >= 0 && mat.nc() >= 0
    ensures
...
template <typename T> const matrix_exp zeros_matrix(long nr, long nc);
/*!
    requires
        - nr >= 0 && nc >= 0
    ensures
...
const matrix_exp identity_matrix(const matrix_exp &mat);
/*!
    requires
        - mat.nr() == mat.nc()
    ensures
...
template <typename T> const matrix_exp identity_matrix(long N);
/*!
    requires
        - N > 0
    ensures
...
template <typename T, long N> const matrix_exp identity_matrix();
/*!
    requires
        - N > 0
    ensures
...
const matrix_exp linspace(double start, double end, long num);
/*!
    requires
        - num >= 0
    ensures
        - returns a matrix M such that:
            ...
            - M.size() == num
            - M == a row vector with num linearly spaced values beginning with
start and stopping with end.
            - M(num-1) == end
            - if (num > 1) then
                - M(0) == start
!*/
...
const matrix_exp logspace(double start, double end, long num);
/*!
    requires
        - num >= 0
    ensures
        - returns a matrix M such that:
            ...
            - is_row_vector(M) == true
            - M.size() == num
            - M == a row vector with num logarithmically spaced values beginning
with 10^start and stopping with 10^end. (i.e. M == pow(10, linspace(start, end,
num)))
            - M(num-1) == 10^end
!*/
...
const matrix_exp linpiece(const double val, const matrix_exp &joints);
/*!
    requires
        ...
        - for all valid i < j:
            - joints(i) < joints(j)
    ensures
        - linpiece() is useful for creating piecewise linear functions of val.
For example, if w is a parameter vector then you can represent a piecewise
linear function of val as: f(val) = dot(w, linpiece(val, linspace(0,100,5))). In
          this case, f(val) is piecewise linear on the intervals [0,25],
[25,50], [50,75], [75,100].  Moreover, w(i) defines the derivative of f(val) in
the i-th interval.  Finally, outside the interval [0,100] f(val) has a
derivative of zero and f(0) == 0.
...
template <long R, long C> const matrix_exp rotate(const matrix_exp &m);
/*!
    ensures
...
const matrix_exp fliplr(const matrix_exp &m);
/*!
    ensures
...
const matrix_exp flipud(const matrix_exp &m);
/*!
    ensures
...
const matrix_exp flip(const matrix_exp &m);
/*!
    ensures
...
const matrix_exp reshape(const matrix_exp &m, long rows, long cols);
/*!
    requires
        - m.size() == rows*cols
        - rows > 0
    ...
    ensures
        ...
        - i.e. The matrix m is reshaped into a new matrix of rows by cols
          dimension.  Additionally, the elements of m are laid into M in row
major order.
!*/

// ----------------------------------------------------------------------------------------

const matrix_exp reshape_to_column_vector(const matrix_exp &m);
/*!
    ensures
        - returns a matrix M such that:
...
template <long R, long C> const matrix_exp removerc(const matrix_exp &m);
/*!
    requires
        - m.nr() > R >= 0
        - m.nc() > C >= 0
...
const matrix_exp removerc(const matrix_exp &m, long R, long C);
/*!
    requires
        - m.nr() > R >= 0
        - m.nc() > C >= 0
...
template <long R> const matrix_exp remove_row(const matrix_exp &m);
/*!
    requires
        - m.nr() > R >= 0
    ensures
...
template <long C> const matrix_exp remove_col(const matrix_exp &m);
/*!
    requires
        - m.nc() > C >= 0
    ensures
...
const matrix_exp remove_col(const matrix_exp &m, long C);
/*!
    requires
        - m.nc() > C >= 0
    ensures
...
const matrix_exp matrix_cast(const matrix_exp &m);
/*!
    ensures
...
void set_all_elements(matrix<T, NR, NC, MM, L> &m, U value);
/*!
    ensures
        - for all valid r and c:
          m(r,c) == value
...
const matrix_exp::matrix_type tmp(const matrix_exp &m);
/*!
    ensures
...
/*!
    ...
    ensures
        ...
        - Note that if the memory layout of the elements in item change between
          hardware platforms then hash() will give different outputs.  If you
want hash() to always give the same output for the same input then you must
          ensure that elements of item always have the same layout in memory.
          Typically this means using fixed width types and performing byte
swapping to account for endianness before passing item to hash().
!*/

// ----------------------------------------------------------------------------------------

// if matrix_exp contains non-complex types (e.g. float, double)
bool equal(const matrix_exp &a, const matrix_exp &b,
           const matrix_exp::type epsilon =
               100 * std::numeric_limits<matrix_exp::type>::epsilon());
/*!
    ensures
        - if (a and b don't have the same dimensions) then
            - returns false
        - else if (there exists an r and c such that abs(a(r,c)-b(r,c)) >
epsilon) then
            - returns false
        - else
            - returns true
...
bool equal(const matrix_exp &a, const matrix_exp &b,
           const matrix_exp::type::value_type epsilon =
               100 *
               std::numeric_limits<matrix_exp::type::value_type>::epsilon());
/*!
    ensures
        - if (a and b don't have the same dimensions) then
            - returns false
        - else if (there exists an r and c such that abs(real(a(r,c)-b(r,c))) >
epsilon or abs(imag(a(r,c)-b(r,c))) > epsilon) then
            - returns false
        - else
            - returns true
!*/
...
const matrix_exp pointwise_multiply(const matrix_exp &a, const matrix_exp &b,
                                    const matrix_exp &c, const matrix_exp &d);
/*!
    performs
pointwise_multiply(pointwise_multiply(a,b),pointwise_multiply(c,d));
!*/
...
/*!
    requires
        ...
        - a and b both contain the same type of element
    ensures
        - This function joins two matrices together by concatenating their
columns.
        - returns a matrix R such that:
            - R::type == the same type that was in a and b.
            - R.nr() == a.nr() + b.nr()
...
const matrix_exp tensor_product(const matrix_exp &a, const matrix_exp &b);
/*!
    requires
        - a and b both contain the same type of element
    ensures
...
const matrix_exp cartesian_product(const matrix_exp &A, const matrix_exp &B);
/*!
    requires
        - A and B both contain the same type of element
    ensures
...
const matrix_exp scale_columns(const matrix_exp &m, const matrix_exp &v);
/*!
    requires
        - is_vector(v) == true
        - v.size() == m.nc()
...
const matrix_exp scale_rows(const matrix_exp &m, const matrix_exp &v);
/*!
    requires
        - is_vector(v) == true
        - v.size() == m.nr()
...
template <typename T> void sort_columns(matrix<T> &m, matrix<T> &v);
/*!
    requires
    ...
    ensures
        ...
        - sorts the columns of m according to the values in v.
          i.e.
            ...
            - Let #v(new(i)) == v(i) (i.e. new(i) is the index element i moved
to)
...
template <typename T> void rsort_columns(matrix<T> &m, matrix<T> &v);
/*!
    requires
        - is_col_vector(v) == true
        - v.size() == m.nc()
...
const matrix_exp::type length_squared(const matrix_exp &m);
/*!
    requires
        - is_vector(m) == true
    ensures
...
const matrix_exp::type length(const matrix_exp &m);
/*!
    requires
        - is_vector(m) == true
    ensures
        ...
        - if (m contains integer valued elements) then
            - The return type is a double that represents the length. Therefore,
the return value of length() is always represented using a floating point type.
!*/

// ----------------------------------------------------------------------------------------

bool is_row_vector(const matrix_exp &m);
/*!
    ensures
        - if (m.nr() == 1) then
            - return true
...
bool is_col_vector(const matrix_exp &m);
/*!
    ensures
        - if (m.nc() == 1) then
            - return true
...
bool is_vector(const matrix_exp &m);
/*!
    ensures
...
bool is_finite(const matrix_exp &m);
/*!
    ensures
        - returns true if all the values in m are finite values and also not any
kind of NaN value.
!*/
...
template <typename S>
const matrix_exp operator<(const matrix_exp &m, const S &s);
/*!
    requires
...
template <typename S>
const matrix_exp operator<(const S &s, const matrix_exp &m);
/*!
    requires
        - is_built_in_scalar_type<S>::value == true
        - is_built_in_scalar_type<matrix_exp::type>::value == true
...
const matrix_exp::type min(const matrix_exp &m);
/*!
    requires
        - m.size() > 0
    ensures
...
const matrix_exp::type max(const matrix_exp &m);
/*!
    requires
        - m.size() > 0
    ensures
...
void find_min_and_max(const matrix_exp &m, matrix_exp::type &min_val,
...
/*!
    requires
        - m.size() > 0
    ensures
        - returns the location of the maximum element of the array, that is, if
the returned point is P then it will be the case that: m(P.y(),P.x()) == max(m).
!*/

// ----------------------------------------------------------------------------------------

dlib::vector<double, 2> max_point_interpolated(const matrix_exp &m);
/*!
    requires
        - m.size() > 0
    ensures
        - Like max_point(), this function finds the location in m with the
largest value.  However, we additionally use some quadratic interpolation to
find the location of the maximum point with sub-pixel accuracy.  Therefore, the
...
/*!
    requires
        - m.size() > 0
    ensures
        - returns the location of the minimum element of the array, that is, if
the returned point is P then it will be the case that: m(P.y(),P.x()) == min(m).
!*/

// ----------------------------------------------------------------------------------------

const matrix_exp::type sum(const matrix_exp &m);
/*!
    ensures
...
const matrix_exp sum_rows(const matrix_exp &m);
/*!
    requires
        - m.size() > 0
    ensures
...
const matrix_exp sum_cols(const matrix_exp &m);
/*!
    requires
        - m.size() > 0
    ensures
...
const matrix_exp::type prod(const matrix_exp &m);
/*!
    ensures
...
const matrix_exp::type mean(const matrix_exp &m);
/*!
    ensures
...
const matrix_exp::type variance(const matrix_exp &m);
/*!
    ensures
...
const matrix_exp::type stddev(const matrix_exp &m);
/*!
    ensures
...
/*!
    ...
    ensures
        - returns the unbiased sample covariance matrix for the set of samples
          in m.
          (i.e. 1.0/(m.nr()-1)*(sum of all (m(i) - mean(m))*trans(m(i) -
mean(m))))
        - the returned matrix will contain elements of type
matrix_exp::type::type.
...
const matrix<double> randm(long nr, long nc, rand_gen &rnd);
/*!
    requires
        - nr >= 0
        - nc >= 0
        - rand_gen == an object that implements the rand/rand_float_abstract.h
interface ensures
...
inline const matrix_exp gaussian_randm(long nr, long nc,
...
/*!
    requires
        ...
        - nc >= 0
    ensures
        - returns a matrix with its values filled with 0 mean unit variance
Gaussian random numbers.
        ...
        - The returned matrix is lazily evaluated using the expression templates
          technique.  This means that the returned matrix doesn't take up any
memory and is only an expression template.  The values themselves are computed
on demand using the gaussian_random_hash() routine.
        - returns a matrix M such that
            - M::type == double
            - M.nr() == nr
            - M.nc() == nc
...
const matrix<T, pixel_traits<P>::num, 1> pixel_to_vector(const P &pixel);
/*!
    requires
...
template <typename P> void vector_to_pixel(P &pixel, const matrix_exp &vector);
/*!
    requires
...
template <long lower, long upper> const matrix_exp clamp(const matrix_exp &m);
/*!
    ensures
        - returns a matrix R such that:
            - R::type == the same type that was in m
...
const matrix_exp clamp(const matrix_exp &m, const matrix_exp::type &lower,
...
const matrix_exp clamp(const matrix_exp &m, const matrix_exp &lower,
...
const matrix_exp lowerbound(const matrix_exp &m,
...
const matrix_exp upperbound(const matrix_exp &m,
                            const matrix_exp::type &thresh);
/*!
    ensures
        - returns a matrix R such that:
