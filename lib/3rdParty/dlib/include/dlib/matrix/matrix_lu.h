...
template <typename EXP>
lu_decomposition<matrix_exp_type>::lu_decomposition(const matrix_exp<EXP> &A)
    : LU(A), m(A.nr()), n(A.nc()) {
  using namespace std;
  using std::abs;
  ...
  // Turn the piv_temp vector into a more useful form.  This way we will have
  // the identity rowm(A,piv) == L*U.  The permutation vector that comes out of
  // LAPACK is somewhat different.
...
template <typename matrix_exp_type>
bool lu_decomposition<matrix_exp_type>::is_square() const {
  return m == n;
}
...
template <typename matrix_exp_type>
long lu_decomposition<matrix_exp_type>::nr() const {
  return m;
}
...
template <typename matrix_exp_type>
long lu_decomposition<matrix_exp_type>::nc() const {
  return n;
}
...
template <typename matrix_exp_type>
bool lu_decomposition<matrix_exp_type>::is_singular() const {
...
template <typename matrix_exp_type>
const typename lu_decomposition<matrix_exp_type>::matrix_type
lu_decomposition<matrix_exp_type>::get_l() const {
  if (LU.nr() >= LU.nc())
...
template <typename matrix_exp_type>
const typename lu_decomposition<matrix_exp_type>::matrix_type
lu_decomposition<matrix_exp_type>::get_u() const {
  if (LU.nr() >= LU.nc())
...
template <typename matrix_exp_type>
const typename lu_decomposition<matrix_exp_type>::pivot_column_vector_type &
lu_decomposition<matrix_exp_type>::get_pivot() const {
  return piv;
}
...
template <typename matrix_exp_type>
typename lu_decomposition<matrix_exp_type>::type
lu_decomposition<matrix_exp_type>::det() const {
  // make sure requires clause is not broken
  DLIB_ASSERT(is_square() == true,
...
template <typename EXP>
const typename lu_decomposition<matrix_exp_type>::matrix_type
lu_decomposition<matrix_exp_type>::solve(const matrix_exp<EXP> &B) const {
  COMPILE_TIME_ASSERT((is_same_type<type, typename EXP::type>::value));
