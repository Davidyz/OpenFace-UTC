...
namespace impl {
inline bool next_is_whitespace(std::istream &in) {
  return in.peek() == '\n' || in.peek() == ' ' || in.peek() == ',' ||
         in.peek() == '\t' || in.peek() == '\r';
}
} // namespace impl
...
std::istream &operator>>(std::istream &in, matrix<T, NR, NC, MM, L> &m) {
  ...
  while (!stop && in.peek() != EOF) {
    ...
    // Eat next block of whitespace but also note if we hit the start of the
    // next line.
    while (next_is_whitespace(in)) {
  ...
  if (num_rows * num_cols != (long)buf.size() || (NR != 0 && NR != num_rows) ||
      (NC != 0 && NC != num_cols)) {
    in.clear(in.rdstate() | std::ios::failbit);
    return in;
  }
