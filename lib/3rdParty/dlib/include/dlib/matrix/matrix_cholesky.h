...
public:
  ...
  const typename EXP::matrix_type solve(const matrix_exp<EXP> &B) const;
...
bool cholesky_decomposition<matrix_exp_type>::is_spd() const {
  return isspd;
}
...
template <typename matrix_exp_type>
const typename cholesky_decomposition<matrix_exp_type>::matrix_type &
cholesky_decomposition<matrix_exp_type>::get_l() const {
  return L_;
}
...
template <typename matrix_exp_type>
template <typename EXP>
cholesky_decomposition<matrix_exp_type>::cholesky_decomposition(
...
#ifdef DLIB_USE_LAPACK
  L_ = A_;
  const type eps = max(abs(diag(L_))) *
                   std::sqrt(std::numeric_limits<type>::epsilon()) / 100;

  // check if the matrix is actually symmetric
  bool is_symmetric = true;
  for (long r = 0; r < L_.nr() && is_symmetric; ++r) {
    for (long c = r + 1; c < L_.nc() && is_symmetric; ++c) {
      // this is approximately doing: is_symmetric = is_symmetric && ( L_(k,j)
      // == L_(j,k))
  ...
  const type eps =
      max(abs(diag(A))) * std::sqrt(std::numeric_limits<type>::epsilon()) / 100;
...
template <typename matrix_exp_type>
template <typename EXP>
const typename EXP::matrix_type
cholesky_decomposition<matrix_exp_type>::solve(const matrix_exp<EXP> &B) const {
  COMPILE_TIME_ASSERT((is_same_type<type, typename EXP::type>::value));
