...
inline const matrix_range_exp<long> range(long start, long end) {
  return matrix_range_exp<long>(start, end);
}

inline const matrix_range_exp<long> range(long start, long inc, long end) {
  DLIB_ASSERT(inc > 0, "\tconst matrix_exp range(start, inc, end)"
...
template <typename M> struct op_subm {
  op_subm(const M &m_x, const long &r_x, const long &c_x, const long &nr_x,
          const long &nc_x)
...
const matrix_op<op_subm<EXP>> subm(const matrix_exp<EXP> &m, long r, long c,
                                   long nr, long nc) {
  DLIB_ASSERT(r >= 0 && c >= 0 && nr >= 0 && nc >= 0 && r + nr <= m.nr() &&
                  c + nc <= m.nc(),
...
template <typename EXP>
const matrix_op<op_subm<EXP>> subm_clipped(const matrix_exp<EXP> &m, long r,
                                           long c, long nr, long nc) {
  rectangle box(c, r, c + nc - 1, r + nr - 1);
  box = box.intersect(get_rect(m));
  typedef op_subm<EXP> op;
  return matrix_op<op>(
      op(m.ref(), box.top(), box.left(), box.height(), box.width()));
}

// ----------------------------------------------------------------------------------------

template <typename EXP>
const matrix_op<op_subm<EXP>> subm(const matrix_exp<EXP> &m,
  ...
  typedef op_subm<EXP> op;
  return matrix_op<op>(
      op(m.ref(), rect.top(), rect.left(), rect.height(), rect.width()));
}

// ----------------------------------------------------------------------------------------

template <typename EXP>
const matrix_op<op_subm<EXP>> subm_clipped(const matrix_exp<EXP> &m,
                                           rectangle rect) {
...
template <typename M1, typename M2, typename M3> struct op_subm_range {
...
template <typename EXP, typename EXPr, typename EXPc>
const matrix_op<op_subm_range<EXP, EXPr, EXPc>>
subm(const matrix_exp<EXP> &m, const matrix_exp<EXPr> &rows,
  ...
  DLIB_ASSERT(
      0 <= min(rows) && max(rows) < m.nr() && 0 <= min(cols) &&
          max(cols) < m.nc() && (rows.nr() == 1 || rows.nc() == 1) &&
          (cols.nr() == 1 || cols.nc() == 1),
      "\tconst matrix_exp subm(const matrix_exp& m, const matrix_exp& rows, "
...
template <typename M> struct op_rowm {
...
template <typename EXP>
const matrix_op<op_rowm<EXP>> rowm(const matrix_exp<EXP> &m, long row) {
...
template <typename EXP> struct rowm_exp {
  typedef matrix_op<op_rowm<EXP>> type;
};

// ----------------------------------------------------------------------------------------

template <typename M> struct op_rowm2 {
  op_rowm2(const M &m_, const long &row_, const long &len)
      : m(m_), row(row_), length(len) {}
  const M &m;
  const long row;
  const long length;
...
template <typename EXP>
const matrix_op<op_rowm2<EXP>> rowm(const matrix_exp<EXP> &m, long row,
                                    long length) {
  DLIB_ASSERT(row >= 0 && row < m.nr() && length >= 0 && length <= m.nc(),
...
template <typename M1, typename M2> struct op_rowm_range {
...
const matrix_op<op_rowm_range<EXP1, EXP2>> rowm(const matrix_exp<EXP1> &m,
...
template <typename M> struct op_colm {
...
template <typename EXP>
const matrix_op<op_colm<EXP>> colm(const matrix_exp<EXP> &m, long col) {
...
template <typename EXP> struct colm_exp {
  typedef matrix_op<op_colm<EXP>> type;
};

// ----------------------------------------------------------------------------------------

template <typename M> struct op_colm2 {
  op_colm2(const M &m_, const long &col_, const long &len)
      : m(m_), col(col_), length(len) {}
  const M &m;
  const long col;
  const long length;
...
template <typename EXP>
const matrix_op<op_colm2<EXP>> colm(const matrix_exp<EXP> &m, long col,
                                    long length) {
  DLIB_ASSERT(col >= 0 && col < m.nc() && length >= 0 && length <= m.nr(),
...
template <typename M1, typename M2> struct op_colm_range {
...
template <typename EXP1, typename EXP2>
const matrix_op<op_colm_range<EXP1, EXP2>> colm(const matrix_exp<EXP1> &m,
...
template <typename T> class assignable_ptr_matrix {
public:
  typedef T type;
  typedef row_major_layout layout_type;
  typedef matrix<T, 0, 0, default_memory_manager, layout_type> matrix_type;

  assignable_ptr_matrix(T *ptr_, long nr_, long nc_)
      : ptr(ptr_), height(nr_), width(nc_) {}

  T &operator()(long r, long c) { return ptr[r * width + c]; }

  const T &operator()(long r, long c) const { return ptr[r * width + c]; }

  long nr() const { return height; }
  long nc() const { return width; }

  template <typename EXP>
  assignable_ptr_matrix &operator=(const matrix_exp<EXP> &exp) {
    // You can only assign to a set_ptrm() expression with a source matrix that
    // contains the same type of elements as the target (i.e. you can't mix
    ...
    DLIB_ASSERT(exp.nr() == height && exp.nc() == width,
                "\tassignable_matrix_expression set_ptrm()"
                    << "\n\tYou have tried to assign to this object using a "
                       "matrix that isn't the right size"
  ...
  assignable_ptr_matrix &operator=(const T &value) {
...
template <typename T>
assignable_ptr_matrix<T> set_ptrm(T *ptr, long nr, long nc = 1) {
  DLIB_ASSERT(
      nr >= 0 && nc >= 0,
...
public:
  ...
  assignable_sub_matrix(matrix<T, NR, NC, mm, l> &m_, long top_, long left_,
                        long height_, long width_)
      : m(m_), left(left_), top(top_), width(width_), height(height_) {}

  T &operator()(long r, long c) { return m(r + top, c + left); }

  const T &operator()(long r, long c) const { return m(r + top, c + left); }

  long nr() const { return height; }
  long nc() const { return width; }

  template <typename EXP>
  assignable_sub_matrix &operator=(const matrix_exp<EXP> &exp) {
    DLIB_ASSERT(exp.nr() == height && exp.nc() == width,
  ...
  assignable_sub_matrix &operator=(const T &value) {
...
template <typename T, long NR, long NC, typename mm, typename l>
assignable_sub_matrix<T, NR, NC, mm, l> set_subm(matrix<T, NR, NC, mm, l> &m,
                                                 const rectangle &rect) {
  DLIB_ASSERT(get_rect(m).contains(rect) == true,
              "\tassignable_matrix_expression set_subm(matrix& m, const "
              "rectangle& rect)"
                  << "\n\tYou have specified invalid sub matrix dimensions"
                  << "\n\tm.nr(): " << m.nr() << "\n\tm.nc(): " << m.nc()
                  << "\n\trect.left():   " << rect.left()
  ...
  return assignable_sub_matrix<T, NR, NC, mm, l>(m, rect.top(), rect.left(),
                                                 rect.height(), rect.width());
}

template <typename T, long NR, long NC, typename mm, typename l>
assignable_sub_matrix<T, NR, NC, mm, l>
set_subm(matrix<T, NR, NC, mm, l> &m, long r, long c, long nr, long nc) {
  DLIB_ASSERT(r >= 0 && c >= 0 && nr >= 0 && nc >= 0 && r + nr <= m.nr() &&
                  c + nc <= m.nc(),
...
public:
  ...
  assignable_sub_range_matrix(matrix<T, NR, NC, mm, l> &m_, const EXPr &rows_,
                              const EXPc &cols_)
      : m(m_), rows(rows_), cols(cols_) {}

  T &operator()(long r, long c) { return m(rows(r), cols(c)); }

  long nr() const { return rows.size(); }
  long nc() const { return cols.size(); }

  template <typename EXP>
  assignable_sub_range_matrix &operator=(const matrix_exp<EXP> &exp) {
    DLIB_ASSERT(exp.nr() == rows.size() && exp.nc() == cols.size(),
                "\tassignable_matrix_expression set_subm(matrix& m, const "
                "matrix_exp rows, const matrix_exp cols)"
                    << "\n\tYou have tried to assign to this object using a "
                       "matrix that isn't the right size"
                    << "\n\texp.nr() (source matrix): " << exp.nr()
                    << "\n\texp.nc() (source matrix): " << exp.nc()
  ...
  assignable_sub_range_matrix &operator=(const T &value) {
...
template <typename T, long NR, long NC, typename mm, typename l, typename EXPr,
          typename EXPc>
assignable_sub_range_matrix<T, NR, NC, mm, l, EXPr, EXPc>
set_subm(matrix<T, NR, NC, mm, l> &m, const matrix_exp<EXPr> &rows,
         const matrix_exp<EXPc> &cols) {
  DLIB_ASSERT(
      0 <= min(rows) && max(rows) < m.nr() && 0 <= min(cols) &&
          max(cols) < m.nc() && (rows.nr() == 1 || rows.nc() == 1) &&
          (cols.nr() == 1 || cols.nc() == 1),
      "\tassignable_matrix_expression set_subm(matrix& m, const matrix_exp& "
      "rows, const matrix_exp& cols)"
          << "\n\tYou have specified invalid sub matrix dimensions"
          << "\n\tm.nr():     " << m.nr() << "\n\tm.nc():     " << m.nc()
          << "\n\tmin(rows):  " << min(rows) << "\n\tmax(rows):  " << max(rows)
          << "\n\tmin(cols):  " << min(cols) << "\n\tmax(cols):  " << max(cols)
  ...
  return assignable_sub_range_matrix<T, NR, NC, mm, l, EXPr, EXPc>(
      m, rows.ref(), cols.ref());
}
...
template <typename T, long NR, long NC, typename mm, typename l, typename EXPr>
assignable_sub_range_matrix<T, NR, NC, mm, l, EXPr, matrix_range_exp<long>>
set_rowm(matrix<T, NR, NC, mm, l> &m, const matrix_exp<EXPr> &rows) {
  DLIB_ASSERT(0 <= min(rows) && max(rows) < m.nr() &&
                  (rows.nr() == 1 || rows.nc() == 1),
              "\tassignable_matrix_expression set_rowm(matrix& m, const "
              "matrix_exp& rows)"
                  << "\n\tYou have specified invalid sub matrix dimensions"
                  << "\n\tm.nr():     " << m.nr() << "\n\tm.nc():     "
                  << m.nc() << "\n\tmin(rows):  " << min(rows)
  ...
  return assignable_sub_range_matrix<T, NR, NC, mm, l, EXPr,
                                     matrix_range_exp<long>>(
      m, rows.ref(), range(0, m.nc() - 1));
}

// ----------------------------------------------------------------------------------------

template <typename T, long NR, long NC, typename mm, typename l, typename EXPc>
assignable_sub_range_matrix<T, NR, NC, mm, l, matrix_range_exp<long>, EXPc>
set_colm(matrix<T, NR, NC, mm, l> &m, const matrix_exp<EXPc> &cols) {
  DLIB_ASSERT(0 <= min(cols) && max(cols) < m.nc() &&
                  (cols.nr() == 1 || cols.nc() == 1),
              "\tassignable_matrix_expression set_colm(matrix& m, const "
              "matrix_exp& cols)"
                  << "\n\tYou have specified invalid sub matrix dimensions"
                  << "\n\tm.nr():     " << m.nr() << "\n\tm.nc():     "
                  << m.nc() << "\n\tmin(cols):  " << min(cols)
  ...
  return assignable_sub_range_matrix<T, NR, NC, mm, l, matrix_range_exp<long>,
                                     EXPc>(m, range(0, m.nr() - 1), cols.ref());
}
...
public:
  ...
  assignable_col_matrix(matrix<T, NR, NC, mm, l> &m_, const long col_)
      : m(m_), col(col_) {}

  T &operator()(long r, long) { return m(r, col); }

  const T &operator()(long r, long) const { return m(r, col); }

  long nr() const { return m.nr(); }
  long nc() const { return 1; }

  template <typename EXP>
  assignable_col_matrix &operator=(const matrix_exp<EXP> &exp) {
  ...
  assignable_col_matrix &operator=(const T &value) {
...
template <typename T, long NR, long NC, typename mm, typename l>
assignable_col_matrix<T, NR, NC, mm, l> set_colm(matrix<T, NR, NC, mm, l> &m,
                                                 const long col) {
  DLIB_ASSERT(col >= 0 && col < m.nc(),
...
public:
  ...
  assignable_row_matrix(matrix<T, NR, NC, mm, l> &m_, const long row_)
      : m(m_), row(row_) {}

  T &operator()(long, long c) { return m(row, c); }

  const T &operator()(long, long c) const { return m(row, c); }

  long nr() const { return 1; }
  long nc() const { return m.nc(); }

  template <typename EXP>
  assignable_row_matrix &operator=(const matrix_exp<EXP> &exp) {
  ...
  assignable_row_matrix &operator=(const T &value) {
...
template <typename T, long NR, long NC, typename mm, typename l>
assignable_row_matrix<T, NR, NC, mm, l> set_rowm(matrix<T, NR, NC, mm, l> &m,
                                                 const long row) {
  DLIB_ASSERT(row >= 0 && row < m.nr(),
