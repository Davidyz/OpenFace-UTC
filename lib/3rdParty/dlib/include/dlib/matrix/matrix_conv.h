...
namespace impl {
template <typename T> const T &conj(const T &item) { return item; }
template <typename T> std::complex<T> conj(const std::complex<T> &item) {
...
template <typename M1, typename M2, bool flip_m2 = false> struct op_conv {
  op_conv(const M1 &m1_, const M2 &m2_)
      : m1(m1_), m2(m2_), nr_(m1.nr() + m2.nr() - 1),
        nc_(m1.nc() + m2.nc() - 1) {
  ...
  const_ret_type apply(long r, long c) const {
    ...
    for (long rr = min_rr; rr <= max_rr; ++rr) {
      for (long cc = min_cc; cc <= max_cc; ++cc) {
        if (flip_m2)
          temp += m1(rr, cc) * dlib::impl::conj(m2(m2.nr() - r + rr - 1,
                                                   m2.nc() - c + cc - 1));
        else
...
const matrix_op<op_conv<M1, M2>> conv(const matrix_exp<M1> &m1,
                                      const matrix_exp<M2> &m2) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename M1::type, typename M2::type>::value == true));
...
template <typename M1, typename M2>
const matrix_op<op_conv<M1, M2, true>> xcorr(const matrix_exp<M1> &m1,
                                             const matrix_exp<M2> &m2) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename M1::type, typename M2::type>::value == true));
...
namespace impl {
inline size_t bounding_power_of_two(size_t n) {
  size_t s = 1;
...
typename EXP1::matrix_type xcorr_fft(const matrix_exp<EXP1> &u,
  ...
  return subm(real(ifft(pointwise_multiply(U, conj(V)))),
              U.nr() - u.nr() - v.nr() + 1, U.nc() - u.nc() - v.nc() + 1,
              u.nr() + v.nr() - 1, u.nc() + v.nc() - 1);
}

// ----------------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------

template <typename M1, typename M2, bool flip_m2 = false> struct op_conv_same {
  op_conv_same(const M1 &m1_, const M2 &m2_)
      : m1(m1_), m2(m2_), nr_(m1.nr()), nc_(m1.nc()) {
...
template <typename M1, typename M2>
const matrix_op<op_conv_same<M1, M2>> conv_same(const matrix_exp<M1> &m1,
                                                const matrix_exp<M2> &m2) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename M1::type, typename M2::type>::value == true));
...
template <typename M1, typename M2>
const matrix_op<op_conv_same<M1, M2, true>>
xcorr_same(const matrix_exp<M1> &m1, const matrix_exp<M2> &m2) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename M1::type, typename M2::type>::value == true));
...
template <typename M1, typename M2, bool flip_m2 = false> struct op_conv_valid {
...
template <typename M1, typename M2>
const matrix_op<op_conv_valid<M1, M2>> conv_valid(const matrix_exp<M1> &m1,
                                                  const matrix_exp<M2> &m2) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename M1::type, typename M2::type>::value == true));
...
template <typename M1, typename M2>
const matrix_op<op_conv_valid<M1, M2, true>>
xcorr_valid(const matrix_exp<M1> &m1, const matrix_exp<M2> &m2) {
  COMPILE_TIME_ASSERT(
      (is_same_type<typename M1::type, typename M2::type>::value == true));
