...
template <typename OP> class matrix_op;

template <typename OP> struct matrix_traits<matrix_op<OP>> {
...
template <typename OP> class matrix_op : public matrix_exp<matrix_op<OP>> {
  /*!
...
private:
  ...
  // kind of object.
  template <typename T1> matrix_op(T1);

public:
  matrix_op(const OP &op_) : op(op_) {}

  const_ret_type operator()(long r, long c) const { return op.apply(r, c); }
  ...
  template <typename U> bool aliases(const matrix_exp<U> &item) const {
    return op.aliases(item);
  }

  template <typename U>
  bool destructively_aliases(const matrix_exp<U> &item) const {
    return op.destructively_aliases(item);
  }
...
template <typename OP> class matrix_diag_op;

template <typename OP> struct matrix_traits<matrix_diag_op<OP>> {
  typedef typename OP::type type;
  typedef typename OP::const_ret_type const_ret_type;
  typedef typename OP::mem_manager_type mem_manager_type;
  typedef typename OP::layout_type layout_type;
...
class matrix_diag_op : public matrix_diag_exp<matrix_diag_op<OP>> {
  /*!
      WHAT THIS OBJECT REPRESENTS
          The matrix_diag_op is simply a tool for reducing the amount of
  boilerplate you need to write when creating matrix expressions.
  !*/
...
public:
  ...
  typedef
      typename matrix_traits<matrix_diag_op>::mem_manager_type mem_manager_type;
...
private:
  ...
  // error if someone tries to create an instance of this object with the wrong
  // kind of object.
  template <typename T1> matrix_diag_op(T1);

public:
  matrix_diag_op(const OP &op_) : op(op_) {}
...
struct does_not_alias {
  ...
  template <typename U> bool destructively_aliases(const U &) const {
    return false;
  }
};
...
template <typename M> struct basic_op_m {
  /*!
...
private:
  ...
  // error if someone tries to create an instance of this object with the wrong
  // kind of object.
  template <typename T1> basic_op_m(T1);

public:
  basic_op_m(const M &m_) : m(m_) {}
...
template <typename M1, typename M2> struct basic_op_mm {
  /*!
      This is a partial implementation of a matrix operator that preserves
...
private:
  ...
  // error if someone tries to create an instance of this object with the wrong
  // kind of object.
  template <typename T1, typename T2> basic_op_mm(T1, T2);

public:
  basic_op_mm(const M1 &m1_, const M2 &m2_) : m1(m1_), m2(m2_) {}
...
template <typename M1, typename M2, typename M3> struct basic_op_mmm {
  /*!
      This is a partial implementation of a matrix operator that preserves
      the dimensions of its arguments and doesn't have destructive aliasing.
  !*/
...
private:
  ...
  // error if someone tries to create an instance of this object with the wrong
  // kind of object.
  template <typename T1, typename T2, typename T3> basic_op_mmm(T1, T2, T3);

public:
  basic_op_mmm(const M1 &m1_, const M2 &m2_, const M3 &m3_)
...
public:
  basic_op_mmmm(const M1 &m1_, const M2 &m2_, const M3 &m3_, const M4 &m4_)
...
#define DLIB_DEFINE_OP_M(op_name, function, extra_cost)                        \
  template <typename M> struct op_name {                                       \
    op_name(const M &m_) : m(m_) {}                                            \
                                                                               \
    const M &m;                                                                \
                                                                               \
...
#define DLIB_DEFINE_OP_MS(op_name, function, extra_cost)                       \
  template <typename M, typename S> struct op_name {                           \
    op_name(const M &m_, const S &s_) : m(m_), s(s_) {}                        \
                                                                               \
    const M &m;                                                                \
    const S s;                                                                 \
                                                                               \
...
#define DLIB_DEFINE_OP_SM(op_name, function, extra_cost)                       \
  template <typename S, typename M> struct op_name {                           \
    op_name(const S &s_, const M &m_) : m(m_), s(s_) {}                        \
                                                                               \
    const M &m;                                                                \
    const S s;                                                                 \
                                                                               \
