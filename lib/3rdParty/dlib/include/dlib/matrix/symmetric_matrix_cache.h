...
struct op_symm_cache : basic_op_m<M> {
  inline op_symm_cache(const M &m_, long max_size_megabytes_)
      : basic_op_m<M>(m_), max_size_megabytes(max_size_megabytes_),
  ...
  op_symm_cache(const op_symm_cache &item)
      : basic_op_m<M>(item.m), diag_cache(item.diag_cache),
        max_size_megabytes(item.max_size_megabytes), is_initialized(false) {
    lookup.assign(this->m.nr(), -1);
  }
  ...
  const type *diag() const {
    init();
    return &diag_cache(0);
  }

  long *diag_ref_count() const { return &diag_reference_count; }

private:
  inline bool is_cached(long r) const { return (lookup[r] != -1); }
  ...
  inline void init() const {
    if (is_initialized == false) {
      ...
      long max_size =
          (max_size_megabytes * 1024 * 1024) / (this->m.nr() * sizeof(type));
  ...
  void make_sure_next_is_unreferenced() const {
  ...
  inline void add_col_to_cache(long c) const {
    init();
    ...
    // make the lookup table so that it says c is now cached at the spot
    // indicated by next
    lookup[c] = next;
    rlookup[next] = c;
  ...
  INITIAL VALUE
      ...
      - is_initialized == false
      - max_size_megabytes == the max_size_megabytes from
  ...
  CONVENTION
      ...
      - if (is_initialized) then
          ...
          - next == the next element in the cache table to use to cache
  something
          - references[i] == the number of outstanding references to cache
  element cache[i]

          - diag_reference_count == the number of outstanding references to
  diag_cache. (this isn't really needed.  It's just here so that we can reuse
  the matrix expression from colm() to implement diag())
  !*/
...
template <typename cache_element_type, typename EXP>
const matrix_op<op_symm_cache<EXP, cache_element_type>>
symmetric_matrix_cache(const matrix_exp<EXP> &m, long max_size_megabytes) {
  // Don't check that m is symmetric since doing so would be extremely onerous
  // for the kinds of matrices intended for use with the symmetric_matrix_cache.
  // Check everything else though.
  DLIB_ASSERT(m.size() > 0 && m.nr() == m.nc() && max_size_megabytes >= 0,
              "\tconst matrix_exp symmetric_matrix_cache(const matrix_exp& m, "
              "max_size_megabytes)"
...
template <typename M, typename cache_element_type> struct op_colm_symm_cache {
  typedef cache_element_type type;

  op_colm_symm_cache(const M &m_, const type *data_, long *ref_count_)
      : m(m_), data(data_), ref_count(ref_count_) {
    *ref_count += 1;
  }

  op_colm_symm_cache(const op_colm_symm_cache &item)
      : m(item.m), data(item.data), ref_count(item.ref_count) {
    *ref_count += 1;
  }

  ~op_colm_symm_cache() { *ref_count -= 1; }
...
inline const matrix_op<op_colm_symm_cache<EXP, cache_element_type>>
colm(const matrix_exp<matrix_op<op_symm_cache<EXP, cache_element_type>>> &m,
     long col) {
  ...
  return matrix_op<op>(op(m.ref().op.m, p.first, p.second));
}
...
template <typename EXP, typename cache_element_type>
inline const matrix_op<op_colm_symm_cache<EXP, cache_element_type>>
diag(const matrix_exp<matrix_op<op_symm_cache<EXP, cache_element_type>>> &m) {
  typedef op_colm_symm_cache<EXP, cache_element_type> op;
  return matrix_op<op>(
      op(m.ref().op.m, m.ref().op.diag(), m.ref().op.diag_ref_count()));
}

// ----------------------------------------------------------------------------------------

template <typename M, typename cache_element_type> struct op_rowm_symm_cache {
  typedef cache_element_type type;

  op_rowm_symm_cache(const M &m_, const type *data_, long *ref_count_)
      : m(m_), data(data_), ref_count(ref_count_) {
    *ref_count += 1;
  }

  op_rowm_symm_cache(const op_rowm_symm_cache &item)
      : m(item.m), data(item.data), ref_count(item.ref_count) {
    *ref_count += 1;
  }

  ~op_rowm_symm_cache() { *ref_count -= 1; }
...
template <typename EXP, typename cache_element_type>
inline const matrix_op<op_rowm_symm_cache<EXP, cache_element_type>>
rowm(const matrix_exp<matrix_op<op_symm_cache<EXP, cache_element_type>>> &m,
     long row) {
