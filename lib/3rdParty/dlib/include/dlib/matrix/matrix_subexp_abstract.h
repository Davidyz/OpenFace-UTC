...
template <long start, long inc, long end> const matrix_exp range();
/*!
    requires
        - inc > 0
    ensures
...
template <long start, long end> const matrix_exp range() {
  return range<start, 1, end>();
}

const matrix_exp range(long start, long inc, long end);
/*!
    requires
        - inc > 0
    ensures
...
const matrix_exp range(long start, long end) { return range(start, 1, end); }
...
const matrix_exp subm(const matrix_exp &m, long row, long col, long nr,
                      long nc);
/*!
    requires
        - row >= 0
...
const matrix_exp subm_clipped(const matrix_exp &m, long row, long col, long nr,
                              long nc);
/*!
    ensures
        - This function is just like subm() except that it will automatically
clip the indicated sub matrix window so that it does not extend outside m. In
particular:
...
const matrix_exp subm_clipped(const matrix_exp &m, const rectangle &rect);
/*!
    ensures
...
const matrix_exp rowm(const matrix_exp &m, long row);
/*!
    requires
        - 0 <= row < m.nr()
    ensures
...
const matrix_exp rowm(const matrix_exp &m, long row, long length);
/*!
    requires
        - 0 <= row < m.nr()
        - 0 <= length <= m.nc()
...
const matrix_exp colm(const matrix_exp &m, long col);
/*!
    requires
        - 0 <= col < m.nc()
    ensures
...
const matrix_exp colm(const matrix_exp &m, long col, long length);
/*!
    requires
        - 0 <= col < m.nc()
        - 0 <= length <= m.nr()
...
template <typename T>
assignable_matrix_expression set_ptrm(T *ptr, long nr, long nc = 1);
/*!
    requires
    ...
    ensures
        ...
        - statements of the following form:
          ...
          result in it being the case that:
            - mat(ptr,nr,nc) ==
uniform_matrix<matrix::type>(nr,nc,scalar_value).

        - In addition to the normal assignment statements using the = symbol,
you may also use the usual += and -= versions of the assignment operator.  In
these cases, they have their usual effect.
!*/

// ----------------------------------------------------------------------------------------

assignable_matrix_expression set_subm(matrix &m, long row, long col, long nr,
                                      long nc);
/*!
    requires
        - row >= 0
...
assignable_matrix_expression set_subm(matrix &m, const matrix_exp &rows,
                                      const matrix_exp &cols);
/*!
    requires
        - rows and cols contain integral elements (e.g. int, long)
...
assignable_matrix_expression set_rowm(matrix &m, long row);
/*!
    requires
        - 0 <= row < m.nr()
    ensures
...
assignable_matrix_expression set_colm(matrix &m, long col);
/*!
    requires
        - 0 <= col < m.nr()
    ensures
