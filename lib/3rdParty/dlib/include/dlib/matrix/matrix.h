...
// matrix<> was templated in a way making NR ending up 0, but division by 0 at
// runtime is not possible because the division operation is inside "if (NR!=0)"
// block.
...
// This template will perform the needed loop for element multiplication using
// whichever dimension is provided as a compile time constant (if one is at
// all).
template <typename LHS, typename RHS, long lhs_nc = LHS::NC,
          long rhs_nr = RHS::NR>
struct matrix_multiply_helper {
  typedef typename LHS::type type;
  template <typename RHS_, typename LHS_>
  inline const static type eval(const RHS_ &rhs, const LHS_ &lhs, const long r,
                                const long c) {
...
template <typename LHS, typename RHS, long lhs_nc>
...
template <typename LHS, typename RHS> class matrix_multiply_exp;
...
struct matrix_traits<matrix_multiply_exp<LHS, RHS>> {
  ...
  // always evaluate any complex expressions so that the BLAS bindings can
  // happen.
  const static bool lhs_is_costly =
      (LHS::cost > 2) && (RHS::NC != 1 || LHS::cost >= 10000);
  const static bool rhs_is_costly =
      (RHS::cost > 2) && (LHS::NR != 1 || RHS::cost >= 10000);
#else
  const static bool lhs_is_costly = (LHS::cost > 4) && (RHS::NC != 1);
  const static bool rhs_is_costly = (RHS::cost > 4) && (LHS::NR != 1);
#endif

  // Note that if we decide that one of the matrices is too costly we will
  // evaluate it into a temporary.  Doing this resets its cost back to 1.
  const static long lhs_cost = ((lhs_is_costly == true) ? 1 : (LHS::cost));
  const static long rhs_cost = ((rhs_is_costly == true) ? 1 : (RHS::cost));

  // The cost of evaluating an element of a matrix multiply is the cost of
  // evaluating elements from RHS and LHS times the number of rows/columns in
  // the RHS/LHS matrix.  If we don't know the matrix dimensions then just
  // assume it is really large.
  const static long cost =
      ((tmax<LHS::NC, RHS::NR>::value != 0)
           ? ((lhs_cost + rhs_cost) * tmax<LHS::NC, RHS::NR>::value)
           : (10000));
};

template <typename T, bool is_ref> struct conditional_matrix_temp {
  typedef typename T::matrix_type type;
};
template <typename T> struct conditional_matrix_temp<T, true> {
  typedef T &type;
};
...
public:
  ...
  typedef typename matrix_traits<matrix_multiply_exp>::const_ret_type
      const_ret_type;
  typedef typename matrix_traits<matrix_multiply_exp>::mem_manager_type
      mem_manager_type;
  ...
  const static bool lhs_is_costly =
      matrix_traits<matrix_multiply_exp>::lhs_is_costly;
  const static bool rhs_is_costly =
      matrix_traits<matrix_multiply_exp>::rhs_is_costly;
  ...
  typedef
      typename conditional_matrix_temp<const LHS, lhs_is_costly == false>::type
          LHS_ref_type;
  typedef
      typename conditional_matrix_temp<const RHS, rhs_is_costly == false>::type
          RHS_ref_type;

  // This constructor exists simply for the purpose of causing a compile time
  // error if someone tries to create an instance of this object with the wrong
  // kind of objects.
  template <typename T1, typename T2> matrix_multiply_exp(T1, T2);

  inline matrix_multiply_exp(const LHS &lhs_, const RHS &rhs_)
      : lhs(lhs_), rhs(rhs_) {
    // You are trying to multiply two incompatible matrices together.  The
    // number of columns in the matrix on the left must match the number of rows
    ...
    DLIB_ASSERT(lhs.nc() == rhs.nr() && lhs.size() > 0 && rhs.size() > 0,
                "\tconst matrix_exp operator*(const matrix_exp& lhs, const "
                "matrix_exp& rhs)"
                    << "\n\tYou are trying to multiply two incompatible "
                       "matrices together"
                    << "\n\tlhs.nr(): " << lhs.nr() << "\n\tlhs.nc(): "
                    << lhs.nc() << "\n\trhs.nr(): " << rhs.nr()
                    << "\n\trhs.nc(): " << rhs.nc() << "\n\t&lhs: " << &lhs
    ...
    // You can't multiply matrices together if they don't both contain the same
    // type of elements.
    COMPILE_TIME_ASSERT(
        (is_same_type<typename LHS::type, typename RHS::type>::value == true));
  }

  inline const type operator()(const long r, const long c) const {
  ...
  long nr() const { return lhs.nr(); }

  long nc() const { return rhs.nc(); }

  template <typename U> bool aliases(const matrix_exp<U> &item) const {
    return lhs.aliases(item) || rhs.aliases(item);
  }
...
inline const matrix_multiply_exp<EXP1, EXP2>
operator*(const matrix_exp<EXP1> &m1, const matrix_exp<EXP2> &m2) {
  return matrix_multiply_exp<EXP1, EXP2>(m1.ref(), m2.ref());
}

template <typename M, bool use_reference = true> class matrix_mul_scal_exp;

// -------------------------

// Now we declare some overloads that cause any scalar multiplications to
// percolate up and outside of any matrix multiplies.  Note that we are using
// the non-reference containing mode of the matrix_mul_scal_exp object since we
// are passing in locally constructed matrix_multiply_exp objects.  So the
// matrix_mul_scal_exp object will contain copies of matrix_multiply_exp objects
// rather than references to them.  This could result in extra matrix copies if
// the matrix_multiply_exp decided it should evaluate any of its arguments.  So
// we also try to not apply this percolating operation if the
// matrix_multiply_exp would contain a fully evaluated copy of the original
// matrix_mul_scal_exp expression.
//
// Also, the reason we want to apply this transformation in the first place is
// because it (1) makes the expressions going into matrix multiply expressions
// simpler and (2) it makes it a lot more straightforward to bind BLAS calls to
// matrix expressions involving scalar multiplies.
template <typename EXP1, typename EXP2>
inline const typename disable_if_c<
    matrix_multiply_exp<matrix_mul_scal_exp<EXP1>,
                        matrix_mul_scal_exp<EXP2>>::both_are_costly,
    matrix_mul_scal_exp<matrix_multiply_exp<EXP1, EXP2>, false>>::type
operator*(const matrix_mul_scal_exp<EXP1> &m1,
...
template <typename EXP1, typename EXP2>
inline const typename disable_if_c<
    matrix_multiply_exp<matrix_mul_scal_exp<EXP1>, EXP2>::lhs_is_costly,
    matrix_mul_scal_exp<matrix_multiply_exp<EXP1, EXP2>, false>>::type
operator*(const matrix_mul_scal_exp<EXP1> &m1, const matrix_exp<EXP2> &m2) {
  typedef matrix_multiply_exp<EXP1, EXP2> exp1;
  typedef matrix_mul_scal_exp<exp1, false> exp2;
...
template <typename EXP1, typename EXP2>
inline const typename disable_if_c<
    matrix_multiply_exp<EXP1, matrix_mul_scal_exp<EXP2>>::rhs_is_costly,
    matrix_mul_scal_exp<matrix_multiply_exp<EXP1, EXP2>, false>>::type
operator*(const matrix_exp<EXP1> &m1, const matrix_mul_scal_exp<EXP2> &m2) {
  typedef matrix_multiply_exp<EXP1, EXP2> exp1;
  typedef matrix_mul_scal_exp<exp1, false> exp2;
...
template <typename LHS, typename RHS> class matrix_add_exp;
...
public:
  ...
  typedef
      typename matrix_traits<matrix_add_exp>::mem_manager_type mem_manager_type;
  ...
  // error if someone tries to create an instance of this object with the wrong
  // kind of objects.
  template <typename T1, typename T2> matrix_add_exp(T1, T2);

  matrix_add_exp(const LHS &lhs_, const RHS &rhs_) : lhs(lhs_), rhs(rhs_) {
    ...
    // You can only add matrices together if they both contain the same types of
    // elements.
    COMPILE_TIME_ASSERT(
        (is_same_type<typename LHS::type, typename RHS::type>::value == true));
  }

  const type operator()(long r, long c) const { return lhs(r, c) + rhs(r, c); }
  ...
  template <typename U>
  bool destructively_aliases(const matrix_exp<U> &item) const {
    return lhs.destructively_aliases(item) || rhs.destructively_aliases(item);
  }

  long nr() const { return lhs.nr(); }

  long nc() const { return lhs.nc(); }
...
template <typename EXP1, typename EXP2>
inline const matrix_add_exp<EXP1, EXP2> operator+(const matrix_exp<EXP1> &m1,
...
template <typename LHS, typename RHS> class matrix_subtract_exp;
...
public:
  ...
  typedef typename matrix_traits<matrix_subtract_exp>::const_ret_type
      const_ret_type;
  typedef typename matrix_traits<matrix_subtract_exp>::mem_manager_type
      mem_manager_type;
  ...
  // error if someone tries to create an instance of this object with the wrong
  // kind of objects.
  template <typename T1, typename T2> matrix_subtract_exp(T1, T2);

  matrix_subtract_exp(const LHS &lhs_, const RHS &rhs_) : lhs(lhs_), rhs(rhs_) {
    // You can only subtract one matrix from another if they both have the same
    ...
    // You can only subtract one matrix from another if they both contain
...
template <typename EXP1, typename EXP2>
inline const matrix_subtract_exp<EXP1, EXP2>
operator-(const matrix_exp<EXP1> &m1, const matrix_exp<EXP2> &m2) {
...
template <typename M> class matrix_div_scal_exp;

template <typename M> struct matrix_traits<matrix_div_scal_exp<M>> {
...
public:
  ...
  typedef typename matrix_traits<matrix_div_scal_exp>::const_ret_type
      const_ret_type;
  typedef typename matrix_traits<matrix_div_scal_exp>::mem_manager_type
      mem_manager_type;
  ...
  // error if someone tries to create an instance of this object with the wrong
  // kind of objects.
  template <typename T1> matrix_div_scal_exp(T1, const type &);

  matrix_div_scal_exp(const M &m_, const type &s_) : m(m_), s(s_) {}

  const type operator()(long r, long c) const { return m(r, c) / s; }
...
inline const typename enable_if_c<
    std::numeric_limits<typename EXP::type>::is_integer,
    matrix_div_scal_exp<EXP>>::type
...
template <typename T, bool is_ref> struct conditional_reference {
  typedef T type;
};
template <typename T> struct conditional_reference<T, true> {
  typedef T &type;
};

template <typename M, bool use_reference>
class matrix_mul_scal_exp
    : public matrix_exp<matrix_mul_scal_exp<M, use_reference>> {
  /*!
      REQUIREMENTS ON M
          - must be a matrix_exp object.
...
public:
  ...
  typedef typename matrix_traits<matrix_mul_scal_exp>::const_ret_type
      const_ret_type;
  typedef typename matrix_traits<matrix_mul_scal_exp>::mem_manager_type
      mem_manager_type;
  ...
  // error if someone tries to create an instance of this object with the wrong
  // kind of objects.
  template <typename T1> matrix_mul_scal_exp(T1, const type &);

  matrix_mul_scal_exp(const M &m_, const type &s_) : m(m_), s(s_) {}
  ...
  typedef
      typename conditional_reference<const M, use_reference>::type M_ref_type;
...
template <typename EXP, typename S>
inline typename disable_if<is_matrix<S>, const matrix_mul_scal_exp<EXP>>::type
operator*(const matrix_exp<EXP> &m, const S &s) {
  typedef typename EXP::type type;
  return matrix_mul_scal_exp<EXP>(m.ref(), static_cast<type>(s));
}

template <typename EXP, typename S, bool B>
inline typename disable_if<is_matrix<S>, const matrix_mul_scal_exp<EXP>>::type
operator*(const matrix_mul_scal_exp<EXP, B> &m, const S &s) {
  typedef typename EXP::type type;
...
template <typename EXP, typename S, bool B>
inline typename disable_if<is_matrix<S>, const matrix_mul_scal_exp<EXP>>::type
operator*(const S &s, const matrix_mul_scal_exp<EXP, B> &m) {
  typedef typename EXP::type type;
  return matrix_mul_scal_exp<EXP>(m.m, static_cast<type>(s) * m.s);
}
...
template <typename EXP, bool B, typename S>
inline const typename disable_if_c<
    std::numeric_limits<typename EXP::type>::is_integer,
    matrix_mul_scal_exp<EXP>>::type
operator/(const matrix_mul_scal_exp<EXP, B> &m, const S &s) {
...
template <typename M> struct op_s_div_m : basic_op_m<M> {
  typedef typename M::type type;
  ...
  const_ret_type apply(long r, long c) const { return s / this->m(r, c); }
};

template <typename EXP, typename S>
const typename disable_if<is_matrix<S>, matrix_op<op_s_div_m<EXP>>>::type
operator/(const S &val, const matrix_exp<EXP> &m) {
  typedef typename EXP::type type;
...
inline const matrix_mul_scal_exp<EXP> operator-(const matrix_exp<EXP> &m) {
  return matrix_mul_scal_exp<EXP>(m.ref(), -1);
}

template <typename EXP, bool B>
inline const matrix_mul_scal_exp<EXP>
operator-(const matrix_mul_scal_exp<EXP, B> &m) {
  return matrix_mul_scal_exp<EXP>(m.m, -1 * m.s);
}

// ----------------------------------------------------------------------------------------

template <typename M> struct op_add_scalar : basic_op_m<M> {
  typedef typename M::type type;
  ...
  const static long cost = M::cost + 1;
  typedef const typename M::type const_ret_type;
  const_ret_type apply(long r, long c) const { return this->m(r, c) + s; }
};

template <typename EXP, typename T>
const typename disable_if<is_matrix<T>, matrix_op<op_add_scalar<EXP>>>::type
operator+(const matrix_exp<EXP> &m, const T &val) {
  typedef typename EXP::type type;
...
template <typename EXP, typename T>
const typename disable_if<is_matrix<T>, matrix_op<op_add_scalar<EXP>>>::type
operator+(const T &val, const matrix_exp<EXP> &m) {
  typedef typename EXP::type type;
...
template <typename M> struct op_subl_scalar : basic_op_m<M> {
  typedef typename M::type type;
...
template <typename EXP, typename T>
const typename disable_if<is_matrix<T>, matrix_op<op_subl_scalar<EXP>>>::type
operator-(const T &val, const matrix_exp<EXP> &m) {
  typedef typename EXP::type type;
...
template <typename M> struct op_subr_scalar : basic_op_m<M> {
  typedef typename M::type type;
...
template <typename EXP, typename T>
const typename disable_if<is_matrix<T>, matrix_op<op_subr_scalar<EXP>>>::type
operator-(const matrix_exp<EXP> &m, const T &val) {
  typedef typename EXP::type type;
...
template <typename EXP1, typename EXP2>
bool operator==(const matrix_exp<EXP1> &m1, const matrix_exp<EXP2> &m2) {
...
template <typename EXP1, typename EXP2>
inline bool operator!=(const matrix_exp<EXP1> &m1, const matrix_exp<EXP2> &m2) {
  return !(m1 == m2);
}
...
template <typename T> struct op_pointer_to_mat;
template <typename T> struct op_pointer_to_col_vect;

template <typename T, long num_rows, long num_cols, typename mem_manager,
          typename layout>
...
template <typename T, long num_rows, long num_cols, typename mem_manager,
          typename layout>
class matrix
    : public matrix_exp<matrix<T, num_rows, num_cols, mem_manager, layout>> {
...
public:
  ...
  explicit matrix(long length) {
    ...
    DLIB_ASSERT(length >= 0, "\tmatrix::matrix(length)"
    ...
    if (NR == 1) {
      DLIB_ASSERT(NC == 0 || NC == length,
                  "\tmatrix::matrix(length)"
                      << "\n\tSince this is a statically sized matrix length "
                         "must equal NC"
                      << "\n\tlength: " << length << "\n\tNR:     " << NR
                      << "\n\tNC:     " << NC << "\n\tthis:   " << this);
  ...
  template <typename EXP> matrix(const matrix_exp<EXP> &m) {
    ...
    COMPILE_TIME_ASSERT(
        (is_same_type<typename EXP::type, type>::value == true) ||
    ...
    DLIB_ASSERT((NR == 0 || NR == m.nr()) && (NC == 0 || NC == m.nc()),
                "\tmatrix& matrix::matrix(const matrix_exp& m)"
                    << "\n\tYou are trying to assign a dynamically sized "
                       "matrix to a statically sized matrix with the wrong size"
                    << "\n\tNR:     " << NR << "\n\tNC:     " << NC
                    << "\n\tm.nr(): " << m.nr() << "\n\tm.nc(): " << m.nc()
                    << "\n\tthis:   " << this);
    ...
    if (NR * NC != 0) {
      DLIB_ASSERT(l.size() == NR * NC,
                  "\t matrix::matrix(const std::initializer_list& l)"
                      << "\n\t You are trying to initialize a statically sized "
                         "matrix with a list that doesn't have a matching size."
                      << "\n\t l.size(): " << l.size()
                      << "\n\t NR*NC:    " << NR * NC);
      ...
      DLIB_ASSERT(
          l.size() % NR == 0,
          "\t matrix::matrix(const std::initializer_list& l)"
              << "\n\t You are trying to initialize a statically sized matrix "
                 "with a list that doesn't have a compatible size."
              << "\n\t l.size(): " << l.size() << "\n\t NR:       " << NR);
  ...
  std::unique_ptr<T[]> steal_memory() { return data.steal_memory(); }
  ...
  matrix(matrix &&item) {
    ...
    if (data._private_is_owned_by_matlab() ==
        item.data._private_is_owned_by_matlab()) {
      swap(item);
    } else {
  ...
  matrix &operator=(matrix &&rhs) {
    ...
    // different locations have different ownership settings.
    if (data._private_is_owned_by_matlab() ==
        rhs.data._private_is_owned_by_matlab()) {
      swap(rhs);
    } else {
  ...
  template <typename U, size_t len> explicit matrix(U (&array)[len]) {
  ...
  inline operator const type() const {
    ...
    DLIB_ASSERT(nr() == 1 && nc() == 1,
                "\tmatrix::operator const type"
                    << "\n\tYou can only attempt to implicit convert a matrix "
                       "to a scalar if"
...
#ifdef MATLAB_MEX_FILE
  void _private_set_mxArray(mxArray *mem) { data._private_set_mxArray(mem); }

  mxArray *_private_release_mxArray() {
    return data._private_release_mxArray();
  }

  void _private_mark_owned_by_matlab() { data._private_mark_owned_by_matlab(); }
  ...
  void set_size(long length) {
    ...
    // row vector.
    COMPILE_TIME_ASSERT(NR == 1 || NC == 1);
    DLIB_ASSERT(length >= 0, "\tvoid matrix::set_size(length)"
                                 << "\n\tlength must be at least 0"
                                 << "\n\tlength: " << length
                                 << "\n\tNR:     " << NR << "\n\tNC:     " << NC
                                 << "\n\tthis:   " << this);
  ...
  template <typename U, size_t len> matrix &operator=(U (&array)[len]) {
    COMPILE_TIME_ASSERT(NR * NC == len && len > 0);
    size_t idx = 0;
    for (long r = 0; r < NR; ++r) {
      for (long c = 0; c < NC; ++c) {
  ...
  template <typename EXP> matrix &operator=(const matrix_exp<EXP> &m) {
    ...
    if (m.destructively_aliases(*this) == false) {
      // This if statement is seemingly unnecessary since set_size() contains
      // this exact same if statement.  However, structuring the code this way
      // causes gcc to handle the way it inlines this function in a much more
      // favorable way.
  ...
  template <typename EXP> matrix &operator+=(const matrix_exp<EXP> &m) {
    ...
    if (nr() == m.nr() && nc() == m.nc()) {
      ...
      DLIB_ASSERT(size() == 0, "\t const matrix::operator+=(m)"
                                   << "\n\t You are trying to add two matrices "
    ...
    if (nr() == m.nr() && nc() == m.nc()) {
      ...
      DLIB_ASSERT(size() == 0,
                  "\t const matrix::operator-=(m)"
                      << "\n\t You are trying to subtract two matrices that "
  ...
  matrix &operator+=(const matrix &m) {
    ...
    if (nr() == m.nr() && nc() == m.nc()) {
      ...
      DLIB_ASSERT(this->size() == 0,
                  "\t const matrix::operator+=(m)"
                      << "\n\t You are trying to add two matrices that have "
  ...
  void swap(matrix &item) { data.swap(item.data); }

  template <typename U> bool aliases(const matrix_exp<U> &) const {
    return false;
  }

  bool
  aliases(const matrix_exp<matrix<T, num_rows, num_cols, mem_manager, layout>>
              &item) const {
    return (this == &item);
  }

  template <typename U>
  bool destructively_aliases(const matrix_exp<U> &) const {
    return false;
  }

  // These two aliases() routines are defined in matrix_mat.h
  bool aliases(const matrix_exp<matrix_op<op_pointer_to_mat<T>>> &item) const;
  bool
  aliases(const matrix_exp<matrix_op<op_pointer_to_col_vect<T>>> &item) const;
...
private:
  struct literal_assign_helper {
    /*
        This struct is a helper struct returned by the operator<<() function
       below.  It is used primarily to enable us to put DLIB_CASSERT statements
       on the usage of the operator<< form of matrix assignment.
    */

    literal_assign_helper(const literal_assign_helper &item)
        : m(item.m), r(item.r), c(item.c), has_been_used(false) {}
    explicit literal_assign_helper(matrix *m_)
        : m(m_), r(0), c(0), has_been_used(false) {
      next();
    }
    ~literal_assign_helper() noexcept(false) {
      DLIB_CASSERT(!has_been_used || r == m->nr(),
                   "You have used the matrix comma based assignment "
                   "incorrectly by failing to\n"
                   "supply a full set of values for every element of a matrix "
                   "object.\n");
    }

    const literal_assign_helper &operator,(const T &val) const {
      DLIB_CASSERT(r < m->nr() && c < m->nc(),
                   "You have used the matrix comma based assignment "
                   "incorrectly by attempting to\n"
                       << "supply more values than there are elements in the "
                          "matrix object being assigned to.\n\n"
...
void swap(matrix<T, NR, NC, mm, l> &a, matrix<T, NR, NC, mm, l> &b) {
  a.swap(b);
}
...
void serialize(const matrix<T, NR, NC, mm, l> &item, std::ostream &out) {
  try {
    // The reason the serialization is a little funny is because we are trying
    // to maintain backwards compatibility with an older serialization format
    // used by dlib while also encoding things in a way that lets the array2d
    // and matrix objects have compatible serialization formats.
  ...
  try {
    ...
    if (NR != 0 && nr != NR)
      throw serialization_error(
          "Error while deserializing a dlib::matrix.  Invalid rows");
    if (NC != 0 && nc != NC)
      throw serialization_error(
          "Error while deserializing a dlib::matrix.  Invalid columns");
...
template <typename T, long NR, long NC, typename mm, typename l>
void serialize(const ramdump_t<matrix<T, NR, NC, mm, l>> &item_,
               std::ostream &out) {
...
template <typename T, long NR, long NC, typename mm, typename l>
void deserialize(ramdump_t<matrix<T, NR, NC, mm, l>> &&item_,
                 std::istream &in) {
  auto &item = item_.item;
  long nr, nc;
  deserialize(nr, in);
...
public:
  ...
  template <typename EXP> std::ostream &operator<<(const matrix_exp<EXP> &m) {
    for (long r = 0; r < m.nr(); ++r) {
      for (long c = 0; c < m.nc(); ++c) {
        if (c + 1 == m.nc())
          out << m(r, c) << "\n";
...
inline print_matrix_as_csv_helper operator<<(std::ostream &out,
...
template <typename EXP> class const_temp_matrix;

template <typename EXP> struct matrix_traits<const_temp_matrix<EXP>> {
  typedef typename EXP::type type;
...
template <typename EXP>
class const_temp_matrix : public matrix_exp<const_temp_matrix<EXP>>,
                          noncopyable {
public:
  typedef typename matrix_traits<const_temp_matrix>::type type;
  typedef
      typename matrix_traits<const_temp_matrix>::const_ret_type const_ret_type;
  typedef typename matrix_traits<const_temp_matrix>::mem_manager_type
      mem_manager_type;
  ...
  template <typename U> bool aliases(const matrix_exp<U> &item) const {
    return ref_.aliases(item);
  }

  template <typename U>
  bool destructively_aliases(const matrix_exp<U> &item) const {
    return ref_.destructively_aliases(item);
  }
...
typedef matrix<double, 0, 0, default_memory_manager, column_major_layout>
    matrix_colmajor;
typedef matrix<float, 0, 0, default_memory_manager, column_major_layout>
    fmatrix_colmajor;
