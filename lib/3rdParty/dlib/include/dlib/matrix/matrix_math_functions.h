...
namespace impl {
template <typename type> inline type sigmoid(const type &val) {
...
inline type round_zeros_eps(const type &val, const S &eps) {
  // you can only round matrices that contain built in scalar types like double,
  // long, float, etc.
...
template <typename type> inline type round_zeros(const type &val) {
  // you can only round matrices that contain built in scalar types like double,
  // long, float, etc.
  COMPILE_TIME_ASSERT(is_built_in_scalar_type<type>::value);
...
template <typename type> inline type squared(const type &val) {
  return val * val;
}

template <typename type> inline type sign(const type &val) {
  if (val >= 0)
    return +1;
  else
    return -1;
...
template <typename type> type cubed(const type &val) { return val * val * val; }

template <typename type, typename S>
inline type pow1(const type &val, const S &s) {
  // you can only call pow() on matrices that contain floats, doubles or long
  // doubles.
...
template <typename type> inline type reciprocal(const type &val) {
  // you can only compute reciprocal matrices that contain floats, doubles or
  // long doubles.
  COMPILE_TIME_ASSERT(
...
template <typename type> inline type reciprocal_max(const type &val) {
  // you can only compute reciprocal_max matrices that contain floats, doubles
  // or long doubles.
  COMPILE_TIME_ASSERT((is_same_type<type, float>::value == true ||
                       is_same_type<type, double>::value == true ||
                       is_same_type<type, long double>::value == true));
...
DLIB_DEFINE_FUNCTION_M(op_reciprocal_max, reciprocal_max, impl::reciprocal_max,
                       6);

// ----------------------------------------------------------------------------------------

template <typename M, typename enabled = void> struct op_round : basic_op_m<M> {
...
template <typename M>
struct op_round<M, typename enable_if_c<
                       std::numeric_limits<typename M::type>::is_integer>::type>
    : basic_op_m<M> {
  op_round(const M &m_) : basic_op_m<M>(m_) {}
  ...
  const_ret_type apply(long r, long c) const { return this->m(r, c); }
};

template <typename EXP>
const matrix_op<op_round<EXP>> round(const matrix_exp<EXP> &m) {
  // you can only round matrices that contain built in scalar types like double,
  // long, float, etc.
...
template <typename M> struct op_normalize : basic_op_m<M> {
  typedef typename M::type type;
...
template <typename EXP>
const matrix_op<op_normalize<EXP>> normalize(const matrix_exp<EXP> &m) {
  // you can only compute normalized matrices that contain floats, doubles or
  // long doubles.
  COMPILE_TIME_ASSERT(
...
template <typename EXP>
const matrix_op<op_abs<EXP>> abs(const matrix_exp<EXP> &m) {
...
template <typename M> struct op_complex_matrix : basic_op_m<M> {
  ...
  const_ret_type apply(long r, long c) const { return type(this->m(r, c)); }
};

template <typename EXP>
const matrix_op<op_complex_matrix<EXP>>
complex_matrix(const matrix_exp<EXP> &m) {
...
struct op_complex_matrix2 : basic_op_mm<M1, M2> {
  op_complex_matrix2(const M1 &m1_, const M2 &m2_)
...
const matrix_op<op_complex_matrix2<EXP1, EXP2>>
complex_matrix(const matrix_exp<EXP1> &real_part,
  ...
  DLIB_ASSERT(
      real_part.nr() == imag_part.nr() && real_part.nc() == imag_part.nc(),
      "\tconst matrix_exp::type complex_matrix(real_part, imag_part)"
          << "\n\tYou can only make a complex matrix from two equally sized "
             "matrices"
...
template <typename M> struct op_norm : basic_op_m<M> {
...
template <typename EXP>
const matrix_op<op_norm<EXP>> norm(const matrix_exp<EXP> &m) {
...
template <typename M> struct op_real : basic_op_m<M> {
...
template <typename EXP>
const matrix_op<op_real<EXP>> real(const matrix_exp<EXP> &m) {
...
template <typename M> struct op_imag : basic_op_m<M> {
...
template <typename EXP>
const matrix_op<op_imag<EXP>> imag(const matrix_exp<EXP> &m) {
