...
template <typename T> class matrix_range_exp;

template <typename T> struct matrix_traits<matrix_range_exp<T>> {
  typedef T type;
...
public:
  ...
  typedef
      typename matrix_traits<matrix_range_exp>::const_ret_type const_ret_type;
  typedef typename matrix_traits<matrix_range_exp>::mem_manager_type
      mem_manager_type;
  ...
  matrix_range_exp(T start_, T end_) {
    start = start_;
    if (start_ <= end_)
      inc = 1;
    else
  ...
  matrix_range_exp(T start_, T inc_, T end_) {
    start = start_;
  ...
  matrix_range_exp(T start_, T end_, long num, bool) {
    start = start_;
    nc_ = num;
    if (num > 1) {
  ...
  const_ret_type operator()(long, long c) const { return start + c * inc; }

  const_ret_type operator()(long c) const { return start + c * inc; }
...
template <typename T> class matrix_log_range_exp;

template <typename T> struct matrix_traits<matrix_log_range_exp<T>> {
  typedef T type;
  typedef const T const_ret_type;
  typedef default_memory_manager mem_manager_type;
  typedef row_major_layout layout_type;
...
public:
  ...
  typedef typename matrix_traits<matrix_log_range_exp>::const_ret_type
      const_ret_type;
  typedef typename matrix_traits<matrix_log_range_exp>::mem_manager_type
      mem_manager_type;
  ...
  matrix_log_range_exp(T start_, T end_, long num) {
    start = start_;
    nc_ = num;
    if (num > 1) {
      inc = (end_ - start_) / (num - 1);
  ...
  const_ret_type operator()(long, long c) const {
    return std::pow((T)10, start + c * inc);
  }

  const_ret_type operator()(long c) const {
    return std::pow((T)10, start + c * inc);
  }
...
template <long start, long inc_, long end> class matrix_range_static_exp;
...
class matrix_range_static_exp
    : public matrix_exp<matrix_range_static_exp<start, inc_, end_>> {
public:
  typedef typename matrix_traits<matrix_range_static_exp>::type type;
  typedef typename matrix_traits<matrix_range_static_exp>::const_ret_type
      const_ret_type;
  typedef typename matrix_traits<matrix_range_static_exp>::mem_manager_type
      mem_manager_type;
  ...
  typedef
      typename matrix_traits<matrix_range_static_exp>::layout_type layout_type;
