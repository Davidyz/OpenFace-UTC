...
template <typename set_base> class set_compare : public set_base {
...
public:
  bool operator<(const set_compare &rhs) const;
  /*!
      ensures
          - #at_start() == true
          ...
          - else if (size() > rhs.size()) then
              - returns false
          - else
              - returns true if there exists an integer j such that 0 <= j <
  size() and for all integers i such that 0 <= i < j where it is true that
                (*this)[i] == rhs[i] and (*this)[j] < rhs[j]
              - returns false if there is no j that will satisfy the above
  conditions.
  !*/
...
inline void swap(set_compare<set_base> &a, set_compare<set_base> &b) {
  a.swap(b);
}
/*!
