...
class auto_unlock {
  ...
  !*/
public:
  explicit auto_unlock(const mutex &m);
  /*!
      ensures
  ...
  explicit auto_unlock(const rmutex &m);
  /*!
      ensures
          - #*this is properly initialized
          - does not modify m in any way
  ...
  explicit auto_unlock(const read_write_mutex &m);
  /*!
      ensures
          - #*this is properly initialized
          - does not modify m in any way
...
class auto_unlock_readonly {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents a mechanism for automatically unlocking
          a read_write_mutex object.  It is useful when you already have a
  locked mutex and want to make sure it gets unlocked even if an exception is
  thrown or you quit the function at a weird spot.  Note that the mutex is
  unlocked by calling unlock_readonly() on it.
  !*/
public:
  explicit auto_unlock_readonly(const read_write_mutex &m);
  /*!
      ensures
          - #*this is properly initialized
          - does not modify m in any way
...
private:
  ...
  auto_unlock_readonly &
  operator=(auto_unlock_readonly &); // assignment operator
};
