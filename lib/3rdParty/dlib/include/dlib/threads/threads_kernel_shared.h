...
// this file should be included at the bottom of one of the thread kernel
// headers for a specific platform.
// #include "../threads.h"
...
class threader {
  /*!
      ...
      CONVENTION
          ...
          - total_count == the number of threads that are executing anywhere.
  i.e. pool_count + the ones that are currently running some user function.
          ...
          - thread_ids == a set that contains the thread id for each thread
...
public:
  ...
  /*!
      ensures
          - if (there are no threads currently running and we haven't set
  do_not_ever_destruct) then
              - calls delete this
          - else
              - does nothing
  !*/
  ...
  void unregister_thread_end_handler(T &obj, void (T::*handler)()) {
    ...
    while (reg.reg.move_next()) {
      while (reg.reg.current_element_valid() &&
             reg.reg.element().value() == mfp) {
...
private:
  ...
  struct registry_type {
    mutex m;
    binary_search_tree<thread_id_type, member_function_pointer<>,
                       memory_manager<char>::kernel_2a>::kernel_2a_c reg;
  };
...
inline bool create_new_thread(void (*funct)(void *), void *param) {
  try {
...
template <typename T>
inline void register_thread_end_handler(T &obj, void (T::*handler)()) {
  DLIB_ASSERT(is_dlib_thread(),
              "\tvoid register_thread_end_handler"
                  << "\n\tYou can't register a thread end handler for a thread "
                     "dlib didn't spawn.");

  threads_kernel_shared::thread_pool().register_thread_end_handler(obj,
                                                                   handler);
}
...
template <typename T>
inline void unregister_thread_end_handler(T &obj, void (T::*handler)()) {
  // Check if the thread pool has been destroyed and if it has then don't do
  // anything. This bool here is always true except when the program has started
  // to terminate and the thread pool object has been destroyed.  This if is
  // here to catch other global objects that have destructors that try to call
  // unregister_thread_end_handler(). Without this check we get into trouble if
  // the thread pool is destroyed before these objects.
  if (threads_kernel_shared::thread_pool_has_been_destroyed == false)
    threads_kernel_shared::thread_pool().unregister_thread_end_handler(obj,
                                                                       handler);
}
