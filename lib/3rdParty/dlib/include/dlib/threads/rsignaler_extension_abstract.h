...
class rsignaler {
  ...
  gives a thread the ability to wake up other threads that are waiting for a
          ...
          Each rsignaler object is associated with one and only one rmutex
  object. More than one rsignaler object may be associated with a single rmutex
...
public:
  rsignaler(const rmutex &associated_mutex);
  /*!
      ensures
