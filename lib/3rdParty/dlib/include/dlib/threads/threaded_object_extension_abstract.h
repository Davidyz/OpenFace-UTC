...
class threaded_object {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object represents a simple threaded object.  To use it you
  inherit from it and define the thread() function.  Then when you call start()
...
public:
  ...
  /*!
      ...
      ensures
          - if (is_alive() && this->thread() is currently supposed to be
  executing) then
              - returns true
          - else
              - returns false
  ...
  /*!
      requires
          - is not called from this->thread()
      ensures
          - if (this->thread() has been called by some thread and has yet to
  terminate) then
              - returns true
          - else
              - returns false
  ...
  /*!
      requires
          - is not called from this->thread()
      ensures
          - returns true if the thread will automatically restart upon
  termination and false otherwise.  Note that every time a thread starts it sets
  should_respawn() back to false.  Therefore, a single call to set_respawn() can
  cause at most one respawn to occur.
  !*/
  ...
  /*!
      requires
          - is not called from this->thread()
      ensures
          - This function atomically executes set_respawn() and start().  The
  precise meaning of this is defined below.
          - if (is_alive()) then
              - #should_respawn() == true
          - else
              - #should_respawn() == false
...
protected:
  ...
  /*!
      ...
      ensures
          - calls to this function block until (#is_running() == true ||
  #should_stop() == true)
