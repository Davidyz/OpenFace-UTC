...
inline thread_id_type get_thread_id() { return GetCurrentThreadId(); }
...
public:
  ...
  void lock() const { cs.lock(); }

  void unlock() const { cs.unlock(); }
...
public:
  signaler(const mutex &associated_mutex) : m(associated_mutex) {}
  ...
  bool wait_or_timeout(unsigned long milliseconds) const {
    std::unique_lock<std::mutex> cs(m.cs, std::defer_lock);
    auto status =
        cv.wait_until(cs, std::chrono::system_clock::now() +
                              std::chrono::milliseconds(milliseconds));
    return status == std::cv_status::no_timeout;
  }

  void signal() const { cv.notify_one(); }

  void broadcast() const { cv.notify_all(); }
