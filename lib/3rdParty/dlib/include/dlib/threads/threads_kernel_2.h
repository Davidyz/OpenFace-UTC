...
inline thread_id_type get_thread_id() { return pthread_self(); }
...
public:
  ...
  void lock() const { pthread_mutex_lock(&myMutex); }

  void unlock() const { pthread_mutex_unlock(&myMutex); }
...
public:
  signaler(const mutex &assoc_mutex)
      : associated_mutex(&assoc_mutex.myMutex), m(assoc_mutex) {
    if (pthread_cond_init(&cond, 0)) {
      throw dlib::thread_error(ECREATE_SIGNALER,
                               "in function signaler::signaler() an error "
                               "occurred making the signaler");
    }
  }

  ~signaler() { pthread_cond_destroy(&cond); }

  void wait() const { pthread_cond_wait(&cond, associated_mutex); }
  ...
  bool wait_or_timeout(unsigned long milliseconds) const {
    ...
    if (pthread_cond_timedwait(&cond, associated_mutex, &time_to_wait) ==
        ETIMEDOUT) {
      return false;
    } else {
      return true;
  ...
  void signal() const { pthread_cond_signal(&cond); }

  void broadcast() const { pthread_cond_broadcast(&cond); }

  const mutex &get_mutex() const { return m; }
...
bool spawn_thread(void (*funct)(void *), void *param);
/*!
