...
template <typename T> class thread_specific_data {
  /*!
      CONVENTION
          - for all valid ID:
  ...
  !*/
public:
  thread_specific_data() { thread_end_handler_calls_left = 0; }

  ~thread_specific_data() {
    // We should only call the unregister_thread_end_handler function if there
    // are some outstanding callbacks we expect to get.  Otherwise lets avoid
    // calling it since the dlib state that maintains the registered thread end
    // handlers may have been destructed already (since the program might be in
    ...
    if (call_unregister)
      unregister_thread_end_handler(const_cast<thread_specific_data &>(*this),
                                    &thread_specific_data::thread_end_handler);
  ...
  inline T &data() { return get_data(); }

  inline const T &data() const { return get_data(); }
...
private:
  T &get_data() const {
    ...
    if (item) {
      ...
      // register an end handler for this thread so long as it is a dlib created
      // thread.
      T *new_item = new T;
      ...
      try {
        ...
        if (is_dlib_thread(id)) {
          register_thread_end_handler(
              const_cast<thread_specific_data &>(*this),
              &thread_specific_data::thread_end_handler);
  ...
  thread_specific_data &
  operator=(thread_specific_data &); // assignment operator
};
