...
class auto_mutex {
  ...
  !*/
public:
  explicit auto_mutex(const mutex &m_) : m(&m_), r(0), rw(0) { m->lock(); }

  explicit auto_mutex(const rmutex &r_) : m(0), r(&r_), rw(0) { r->lock(); }

  explicit auto_mutex(const read_write_mutex &rw_) : m(0), r(0), rw(&rw_) {
    rw->lock();
  }
...
public:
  explicit auto_mutex_readonly(const read_write_mutex &rw_)
      : rw(rw_), _has_write_lock(false), _has_read_lock(true) {
    rw.lock_readonly();
  }

  ~auto_mutex_readonly() { unlock(); }
  ...
  bool has_read_lock() { return _has_read_lock; }
