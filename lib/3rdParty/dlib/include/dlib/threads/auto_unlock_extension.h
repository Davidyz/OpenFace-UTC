...
class auto_unlock {
  ...
  !*/
public:
  explicit auto_unlock(const mutex &m_) : m(&m_), r(0), rw(0) {}

  explicit auto_unlock(const rmutex &r_) : m(0), r(&r_), rw(0) {}

  explicit auto_unlock(const read_write_mutex &rw_) : m(0), r(0), rw(&rw_) {}
...
public:
  explicit auto_unlock_readonly(const read_write_mutex &rw_) : rw(rw_) {}

  ~auto_unlock_readonly() { rw.unlock_readonly(); }
