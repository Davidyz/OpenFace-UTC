...
class multithreaded_object {
  /*!
      ...
      CONVENTION
          - number_of_threads_registered() == thread_ids.size() +
  dead_threads.size()
          ...
          - threads_started == the number of threads that have been spawned to
  run thread_helper but haven't ended yet.

          - dead_threads == a queue that contains all the member function
  pointers for threads that are currently registered but not running
...
public:
  ...
  virtual ~multithreaded_object() = 0;

  void clear();

  bool is_running() const;

  unsigned long number_of_threads_alive() const;

  unsigned long number_of_threads_registered() const;
...
protected:
  ...
  template <typename T> void register_thread(T &object, void (T::*thread)()) {
    auto_mutex M(m_);
    try {
      mfp mf;
...
private:
  ...
  rmutex m_;
  rsignaler s;
  map<thread_id_type, mfp, memory_manager<char>::kernel_2a>::kernel_1a
      thread_ids;
  ...
  multithreaded_object &
  operator=(multithreaded_object &); // assignment operator
};
