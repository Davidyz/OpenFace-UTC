...
template <typename T> class future {
  /*!
      ...
      CONVENTION
          ...
          - if (tp.get() != 0)
              - tp == a pointer to the thread_pool_implementation that is using
  this future object
              - task_id == the task id of the task in the thread pool tp that is
  using this future object.
  !*/
public:
  future() : task_id(0) {}

  future(const T &item) : task_id(0), var(item) {}

  future(const future &item) : task_id(0), var(item.get()) {}
  ...
  bool is_ready() const { return tp.get() == 0; }
...
class thread_pool_implementation {
  ...
  friend class thread_pool;
  explicit thread_pool_implementation(unsigned long num_threads);
...
public:
  ...
  uint64 add_task_internal(const bfp_type &bfp,
  ...
  uint64 add_task_internal(const bfp_type &bfp) {
    std::shared_ptr<function_object_copy> temp;
...
private:
  bool is_worker_thread(const thread_id_type id) const;
  /*!
      requires
          - m is locked
      ensures
          - if (thread with given id is one of the thread pool's worker threads
  or num_threads_in_pool() == 0) then
              - returns true
          - else
              - returns false
  !*/
  ...
  long find_empty_task_slot() const;
  /*!
      requires
          - m is locked
      ensures
  ...
  uint64 make_next_task_id(long idx);
  /*!
      requires
          - m is locked
          - 0 <= idx < tasks.size()
  ...
  unsigned long task_id_to_index(uint64 id) const;
  /*!
      requires
          - m is locked
  ...
  struct task_state_type {
    task_state_type()
        : is_being_processed(false), task_id(0), next_task_id(2), arg1(0),
          arg2(0), eptr(nullptr) {}

    bool is_ready() const
    /*!
        ensures
            - if (is_empty() == false && no thread is currently processing this
    task) then
                - returns true
            - else
                - returns false
    ...
    /*!
        ensures
            - if (this task state is empty.  i.e. it doesn't contain a task to
    be processed) then
                - returns true
            - else
                - returns false
  ...
  thread_pool_implementation &
  operator=(thread_pool_implementation &); // assignment operator
};
...
public:
  ...
  ~thread_pool() {
    try {
      ...
      std::cerr << "An unhandled exception was inside a dlib::thread_pool when "
                   "it was destructed."
                << std::endl;
  ...
  void wait_for_task(uint64 task_id) const { impl->wait_for_task(task_id); }

  unsigned long num_threads_in_pool() const {
    return impl->num_threads_in_pool();
  }

  void wait_for_all_tasks() const { impl->wait_for_all_tasks(); }

  bool is_task_thread() const { return impl->is_task_thread(); }
  ...
  template <typename F> uint64 add_task_by_value(const F &function_object) {
    ...
    ptr = new thread_pool_implementation::function_object_copy_instance<F>(
        function_object);
    std::shared_ptr<thread_pool_implementation::function_object_copy>
        function_copy(ptr);
  ...
  uint64 add_task_by_value(const T &obj, void (T::*funct)() const) {
    ...
    ptr =
        new thread_pool_implementation::function_object_copy_instance<const T>(
            obj);
    std::shared_ptr<thread_pool_implementation::function_object_copy>
        function_copy(ptr);
