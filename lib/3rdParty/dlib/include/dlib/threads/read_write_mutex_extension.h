...
class read_write_mutex {
  /*!
      ...
      CONVENTION
          - Each time someone gets a read only lock they take one of the
  "available locks" and each write lock takes all possible locks (i.e.
  max_locks).  The number of available locks is recorded in available_locks. Any
  time you try to lock this object and there aren't available locks you have to
  wait.
...
public:
  read_write_mutex()
      : s(m), max_locks(0xFFFFFFFF), available_locks(max_locks),
        write_lock_in_progress(false), write_lock_active(false) {}

  explicit read_write_mutex(unsigned long max_locks_)
      : s(m), max_locks(max_locks_), available_locks(max_locks_),
        write_lock_in_progress(false), write_lock_active(false) {
  ...
  void unlock_readonly() const {
    ...
    if (available_locks != max_locks && !write_lock_active) {
      ...
      // only perform broadcast when there is another thread that might be
      // listening
  ...
  unsigned long max_readonly_locks() const { return max_locks; }
