...
const T string_cast(const std::basic_string<charT, traits, alloc> &str);
/*!
    requires
  ...
  /*!
      WHAT THIS OBJECT REPRESENTS
          ...
          the following example:
              ...
              dval = sa = "3.141";  // executes: val =
  string_cast<double>("3.141");

          After executing, val will be equal to 1234 and dval will be 3.141.
          Note that you can use string_assign to assign to any type which you
  could use with string_cast(), except for std::basic_string, assigning to this
          type is ambiguous for boring technical reasons.  But there isn't much
          point in using this tool to assign from one string to another so it
  doesn't matter.

          Additionally, note that there is a global instance of this object,
  dlib::sa. So you never have to create a string_assign object yourself.
  Finally, this object is totally stateless and threadsafe.
  !*/
};
...
std::string pad_int_with_zeros(int i, unsigned long width = 6);
/*!
    ensures
...
const std::string narrow(const std::basic_string<charT, traits, alloc> &str);
/*!
    ensures
        - returns str as a std::string by converting every character in it to a
char. Note that any characters that do not have a mapping to type char will be
...
/*!
    ...
    ensures
        - returns a copy of str S such that:
            ...
            - The second and all subsequent lines start with rest_pad space
characters.
            - The first line is no longer than max_per_line -
(rest_pad-first_pad) characters.
            - The second and all subsequent lines are no longer than
max_per_line characters.
!*/
    ...
    ensures
        ...
        - else
            - let P be a string defined as follows:
                ...
                - P == (pad_string + pad_string + ... +
pad_string).substr(0,pad_length - str.size()) (i.e. P == a string with the above
specified size that contains just repitions of the pad_string)
...
/*!
    ensures
        ...
        - returns rpad( lpad(str, (pad_length-str_size)/2 + str_size,
pad_string), pad_length, pad_string);
!*/
...
/*!
    requires
        - delim == a valid null-terminated C string
    ensures
        - returns right_substr(str,
std::basic_string<charT,traits,alloc>(delim))
!*/
...
/*!
    ensures
        - This function splits string into two parts, the split is based on the
first occurrence of any character from delim.
        - let delim_pos = str.find_first_of(delim)
        - if (delim_pos == std::string::npos) then
            - returns make_pair(str,"")
        - else
            - return make_pair(str.substr(0, delim_pos),
...
/*!
    ensures
        - This function splits string into two parts, the split is based on the
last occurrence of any character from delim.
        - let delim_pos = str.find_last_of(delim)
        - if (delim_pos == std::string::npos) then
            - returns make_pair(str,"")
        - else
...
/*!
    ensures
        ...
        - returns a vector V such that:
            ...
            - for all i: V[i] == The ith substring.  Note that it will not
contain any delimiter characters (i.e. characters in delim).  It will also never
be an empty string.
            - V contains the substrings in the order in which they appear in
str. That is, V[0] contains the first substring, V[1] the second, and so on.
!*/
