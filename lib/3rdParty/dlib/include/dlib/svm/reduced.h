...
template <typename trainer_type> class reduced_decision_function_trainer {
public:
  ...
  reduced_decision_function_trainer() : num_bv(0) {}

  reduced_decision_function_trainer(const trainer_type &trainer_,
                                    const unsigned long num_sb_)
      : trainer(trainer_), num_bv(num_sb_) {
  ...
  train(const in_sample_vector_type &x, const in_scalar_vector_type &y) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(num_bv > 0, "\t reduced_decision_function_trainer::train(x,y)"
                                << "\n\t You have tried to use an "
                                   "uninitialized version of this object"
                                << "\n\t num_bv: " << num_bv);
...
private:
  ...
  template <typename in_sample_vector_type, typename in_scalar_vector_type>
  const decision_function<kernel_type>
  do_train(const in_sample_vector_type &x,
           const in_scalar_vector_type &y) const {
    ...
    // now find a linearly independent subset of the training points of num_bv
    // points.
    linearly_independent_subset_finder<kernel_type> lisf(
        dec_funct.kernel_function, num_bv);
    fill_lisf(lisf, x);

    // The next few statements just find the best weights with which to
    // approximate the dec_funct object with the smaller set of vectors in the
    // lisf dictionary.  This is really just a simple application of some linear
    // algebra.  For the details see page 554 of Learning with kernels by
    // Scholkopf and Smola where they talk about "Optimal Expansion
    // Coefficients."
    ...
    alpha =
        lisf.get_inv_kernel_marix() *
        (kernel_matrix(kern, lisf, dec_funct.basis_vectors) * dec_funct.alpha);

    decision_function<kernel_type> new_df(alpha, 0, kern,
                                          lisf.get_dictionary());

    // now we have to figure out what the new bias should be.  It might be a
    // little different since we just messed with all the weights and vectors.
    double bias = 0;
...
const reduced_decision_function_trainer<trainer_type>
reduced(const trainer_type &trainer, const unsigned long num_bv) {
  // make sure requires clause is not broken
  DLIB_ASSERT(num_bv > 0,
...
template <typename kernel_type> class objective {
  /*
...
public:
  objective(const distance_function<kernel_type> &dist_funct_,
            matrix<scalar_type, 0, 1, mem_manager_type> &b_,
            matrix<sample_type, 0, 1, mem_manager_type> &out_vectors_)
      : dist_funct(dist_funct_), b(b_), out_vectors(out_vectors_) {}

  const matrix<scalar_type, 0, 1, mem_manager_type> state_to_vector() const
  /*!
      ensures
  ...
  !*/
  {
    matrix<scalar_type, 0, 1, mem_manager_type> z(
        b.nr() + out_vectors.size() * out_vectors(0).nr());
    long i = 0;
  ...
  void
  vector_to_state(const matrix<scalar_type, 0, 1, mem_manager_type> &z) const
  /*!
      requires
          - z came from the state_to_vector() function or has a compatible
  format ensures
  ...
  double operator()(const matrix<scalar_type, 0, 1, mem_manager_type> &z) const
  /*!
      ensures
  ...
  !*/
  {
    ...
    for (long i = 0; i < out_vectors.size(); ++i) {
      for (long j = 0; j < dist_funct.get_basis_vectors().nr(); ++j) {
        temp -= b(i) * dist_funct.get_alpha()(j) *
                k(out_vectors(i), dist_funct.get_basis_vectors()(j));
      }
    }
...
template <typename kernel_type> class objective_derivative {
  /*!
...
public:
  ...
  operator()(const matrix<scalar_type, 0, 1, mem_manager_type> &z) const
  /*!
      ensures
          - loads the current approximate distance function with z
  ...
  !*/
  {
    ...
    for (long i = 0; i < out_vectors.size(); ++i) {
      for (long j = 0; j < dist_funct.get_basis_vectors().size(); ++j) {
        res(i) -= dist_funct.get_alpha()(j) *
                  k(out_vectors(i), dist_funct.get_basis_vectors()(j));
      }
    }

    // now compute the gradient of the actual vectors that go into the kernel
    // functions
    ...
    for (long i = 0; i < out_vectors.size(); ++i) {
      ...
      for (long j = 0; j < dist_funct.get_basis_vectors().nr(); ++j) {
        temp -= dist_funct.get_alpha()(j) *
                K_der(dist_funct.get_basis_vectors()(j), out_vectors(i));
      }
...
template <typename K, typename stop_strategy_type, typename T>
distance_function<K>
approximate_distance_function(stop_strategy_type stop_strategy,
  ...
  // The next few statements just find the best weights with which to
  // approximate the target object with the set of basis vectors in
  // starting_basis.  This is really just a simple application of some linear
  // algebra.  For the details see page 554 of Learning with kernels by
  // Scholkopf and Smola where they talk about "Optimal Expansion Coefficients."
  ...
  beta = pinv(kernel_matrix(kern, starting_basis)) *
         (kernel_matrix(kern, starting_basis, target.get_basis_vectors()) *
          target.get_alpha());
  matrix<sample_type, 0, 1, mem_manager_type> out_vectors(mat(starting_basis));

  // Now setup to do a global optimization of all the parameters in the
  ...
  matrix<scalar_type, 0, 1, mem_manager_type> opt_starting_point(
      obj.state_to_vector());

  // perform a full optimization of all the parameters (i.e. both beta and the
  // basis vectors together)
  find_min(lbfgs_search_strategy(20), stop_strategy, obj, obj_der,
  ...
  // Do a final reoptimization of beta just to make sure it is optimal given the
  // new set of basis vectors.
  beta = pinv(kernel_matrix(kern, out_vectors)) *
         (kernel_matrix(kern, out_vectors, target.get_basis_vectors()) *
          target.get_alpha());

  // It is possible that some of the beta weights will be very close to zero.
  // Lets remove the basis vectors with these essentially zero weights.
  const scalar_type eps =
      max(abs(beta)) * std::numeric_limits<scalar_type>::epsilon();
...
template <typename trainer_type> class reduced_decision_function_trainer2 {
public:
  typedef typename trainer_type::kernel_type kernel_type;
  typedef typename trainer_type::scalar_type scalar_type;
  typedef typename trainer_type::sample_type sample_type;
  ...
  reduced_decision_function_trainer2(const trainer_type &trainer_,
                                     const long num_sb_,
                                     const double eps_ = 1e-3)
      : trainer(trainer_), num_bv(num_sb_), eps(eps_) {
  ...
  train(const in_sample_vector_type &x, const in_scalar_vector_type &y) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(num_bv > 0, "\t reduced_decision_function_trainer2::train(x,y)"
                                << "\n\t You have tried to use an "
                                   "uninitialized version of this object"
                                << "\n\t num_bv: " << num_bv);
    return do_train(mat(x), mat(y));
...
private:
  ...
  do_train(const in_sample_vector_type &x,
    ...
    target = dec_funct;
    approx = approximate_distance_function(objective_delta_stop_strategy(eps),
                                           target, lisf);

    decision_function<kernel_type> new_df(approx.get_alpha(), 0, kern,
...
template <typename trainer_type>
const reduced_decision_function_trainer2<trainer_type>
reduced2(const trainer_type &trainer, const long num_bv, double eps = 1e-3) {
