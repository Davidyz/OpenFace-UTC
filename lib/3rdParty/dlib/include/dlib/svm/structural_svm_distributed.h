...
template <typename matrix_type> struct oracle_response {
...
template <typename matrix_type> struct oracle_request {
  typedef typename matrix_type::type scalar_type;
...
private:
  ...
  template <typename matrix_type, typename feature_vector_type>
  ...
  public:
    ...
    node_type(
        ...
        unsigned short port, unsigned long num_threads)
  ...
  private:
    void thread() {
      ...
      while (in.dequeue(msg)) {
        ...
        if (msg.template contains<bridge_status>() &&
          ...
          const oracle_request<matrix_type> &req =
              msg.template get<oracle_request<matrix_type>>();

          oracle_response<matrix_type> &data =
              temp.template get<oracle_response<matrix_type>>();
          ...
          bool buffer_subgradients_locally =
              with_buffer_time.mean() < without_buffer_time.mean();

          // every 50 iterations we should try to flip the buffering scheme to
          // see if doing it the other way might be better.
    ...
    struct binder {
      ...
      void call_oracle(long begin, long end) {
        ...
        if (end - begin <= 1 || !buffer_subgradients_locally) {
          ...
          for (long i = begin; i < end; ++i) {
            self.cache[i].separation_oracle_cached(
                req.converged, req.skip_cache, req.saved_current_risk_gap,
                req.current_solution, loss, ftemp);
          ...
          for (long i = begin; i < end; ++i) {
            ...
            self.cache[i].separation_oracle_cached(
                req.converged, req.skip_cache, req.saved_current_risk_gap,
                req.current_solution, loss_temp, ftemp);
            loss += loss_temp;
            add_to(faccum, ftemp);
          }
    ...
    typedef type_safe_union<impl::oracle_request<matrix_type>, bridge_status>
        tsu_in;
    ...
    mutable std::vector<cache_element_structural_svm<
        structural_svm_problem<matrix_type, feature_vector_type>>>
        cache;
...
public:
  svm_struct_controller_node()
      : eps(0.001), max_iterations(10000),
        cache_based_eps(std::numeric_limits<double>::infinity()),
        verbose(false), C(1) {}

  double get_cache_based_epsilon() const { return cache_based_eps; }

  void set_cache_based_epsilon(double eps_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(eps_ > 0,
  ...
  void set_epsilon(double eps_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(eps_ > 0, "\t void svm_struct_controller_node::set_epsilon()"
                              << "\n\t eps_ must be greater than 0"
                              << "\n\t eps_: " << eps_
                              << "\n\t this: " << this);
  ...
  void add_nuclear_norm_regularizer(long first_dimension, long rows, long cols,
    ...
    // make sure requires clause is not broken
    DLIB_ASSERT(
        0 <= first_dimension && 0 <= rows && 0 <= cols &&
  ...
  void set_c(double C_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C_ > 0, "\t void svm_struct_controller_node::set_c()"
  ...
  unsigned long get_num_processing_nodes() const { return nodes.size(); }

  void remove_processing_nodes() { nodes.clear(); }
  ...
  double operator()(const oca &solver, matrix_type &w) const {
    ...
    DLIB_ASSERT(get_num_processing_nodes() != 0,
                "\t double svm_struct_controller_node::operator()"
                    << "\n\t You must add some processing nodes before calling "
                       "this function."
                    << "\n\t this: " << this);
    ...
    for (unsigned long i = 0; i < nuclear_norm_regularizers.size(); ++i) {
      problem.add_nuclear_norm_regularizer(
          ...
          nuclear_norm_regularizers[i].nr, nuclear_norm_regularizers[i].nc,
  ...
  public:
    invalid_problem(const std::string &a) : error(a) {}
  };
...
private:
  ...
  public:
    ...
    problem_type(const std::vector<network_address> &nodes_)
        : nodes(nodes_), in(3), num_dims(0) {
      ...
      for (unsigned long i = 0; i < bridges.size(); ++i) {
        bridges[i].reset(new bridge(connect_to(nodes[i]), receive(in),
                                    transmit(*out_pipes[i])));
      }

      // The remote processing nodes are supposed to all send the problem
      // dimensionality upon connection. So get that and make sure everyone
      // agrees on what it's supposed to be.
      tsu_in temp;
    ...
    // them, but since we are overloading get_risk() they are never called so
    // they don't matter.
    virtual long get_num_samples() const { return 0; }
    virtual void get_truth_joint_feature_vector(long, matrix_type &) const {}
    virtual void separation_oracle(const long, const matrix_type &,
                                   scalar_type &, matrix_type &) const {}

    virtual long get_num_dimensions() const { return num_dims; }
    ...
    virtual void get_risk(matrix_type &w, scalar_type &risk,
      ...
      tsu_out temp_out;
      for (unsigned long i = 0; i < out_pipes.size(); ++i) {
        temp_out.template get<oracle_request<matrix_type>>().current_solution =
            w;
        temp_out.template get<oracle_request<matrix_type>>()
            .saved_current_risk_gap = this->saved_current_risk_gap;
        temp_out.template get<oracle_request<matrix_type>>().skip_cache =
            this->skip_cache;
        temp_out.template get<oracle_request<matrix_type>>().converged =
            this->converged;
      ...
      while (responses < out_pipes.size()) {
        ...
        if (temp_in.template contains<oracle_response<matrix_type>>()) {
          ++responses;
          const oracle_response<matrix_type> &data =
              temp_in.template get<oracle_response<matrix_type>>();
