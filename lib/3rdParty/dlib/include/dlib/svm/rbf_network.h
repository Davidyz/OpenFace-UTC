...
template <typename Kern> class rbf_network_trainer {
  /*!
...
public:
  ...
  rbf_network_trainer() : num_centers(10) {}

  void set_kernel(const kernel_type &k) { kernel = k; }

  const kernel_type &get_kernel() const { return kernel; }

  void set_num_centers(const unsigned long num) { num_centers = num; }

  unsigned long get_num_centers() const { return num_centers; }
  ...
  void swap(rbf_network_trainer &item) {
...
private:
  ...
  do_train(const in_sample_vector_type &x,
    ...
    typedef typename decision_function<kernel_type>::scalar_vector_type
        scalar_vector_type;
    ...
    // use the linearly_independent_subset_finder object to select the centers.
    // So here we show it all the data samples so it can find the best centers.
    linearly_independent_subset_finder<kernel_type> lisf(kernel, num_centers);
    fill_lisf(lisf, x);

    const long num_centers = lisf.size();

    // fill the K matrix with the output of the kernel for all the center and
    // sample point pairs
...
void swap(rbf_network_trainer<sample_type> &a,
