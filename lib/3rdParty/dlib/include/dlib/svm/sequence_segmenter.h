...
template <typename ss_feature_extractor> class feature_extractor {
  /*!
...
public:
  ...
  unsigned long num_features() const {
    ...
    if (ss_feature_extractor::use_high_order_features)
      return NL + NL * NL +
             (NL * NL + NL) * fe.num_features() * fe.window_size();
    else
      return NL + NL * NL + NL * fe.num_features() * fe.window_size();
  }

  unsigned long order() const { return 1; }
...
private:
  template <typename feature_setter> struct dot_functor {
    /*!
        WHAT THIS OBJECT REPRESENTS
    ...
    for (int i = 0; i < window_size; ++i) {
      ...
      if (0 <= pos && pos < (long)x.size()) {
        ...
        if (ss_feature_extractor::use_high_order_features && y.size() > 1) {
          const unsigned long off2 = num_labels() * base_dims +
                                     (y(0) * num_labels() + y(1)) * base_dims;
      ...
      if (ss_feature_extractor::use_high_order_features)
        offset +=
            num_labels() * base_dims + num_labels() * num_labels() * base_dims;
      else
...
public:
  ...
  const feature_extractor &get_feature_extractor() const {
    return labeler.get_feature_extractor().fe;
  }

  const matrix<double, 0, 1> &get_weights() const {
    return labeler.get_weights();
  }
  ...
  friend void deserialize(sequence_segmenter &item, std::istream &in) {
    ...
    if (dims != total_feature_vector_size(item.get_feature_extractor())) {
      throw serialization_error(
          "Incompatible feature extractor found while deserializing "
          "dlib::sequence_segmenter. Wrong value of "
