...
public:
  ...
  void set_c(double C_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C_ > 0, "\t void structural_graph_labeling_trainer::set_c()"
  ...
  double get_loss_on_negative_class() const { return loss_neg; }

  double get_loss_on_positive_class() const { return loss_pos; }
  ...
  train(const dlib::array<graph_type> &samples,
    ...
    structural_svm_graph_labeling_problem<graph_type> prob(samples, labels,
                                                           losses, num_threads);
    ...
    populate_weights(w, edge_weights, node_weights,
                     prob.get_num_edge_weights());
...
private:
  template <typename T>
  typename enable_if<is_matrix<T>>::type
  populate_weights(const matrix<double, 0, 1> &w, T &edge_weights,
                   T &node_weights, long split_idx) const {
    edge_weights = rowm(w, range(0, split_idx - 1));
    node_weights = rowm(w, range(split_idx, w.size() - 1));
  }

  template <typename T>
  typename disable_if<is_matrix<T>>::type
  populate_weights(const matrix<double, 0, 1> &w, T &edge_weights,
                   T &node_weights, long split_idx) const {
    edge_weights.clear();
    node_weights.clear();
    ...
    for (long i = split_idx; i < w.size(); ++i) {
      if (w(i) != 0)
        node_weights.insert(node_weights.end(),
                            std::make_pair(i - split_idx, w(i)));
    }
  }
