...
inline bool is_learning_problem_impl(const T &x, const U &x_labels) {
  return is_col_vector(x) && is_col_vector(x_labels) &&
         x.size() == x_labels.size() && x.size() > 0;
}

template <typename T, typename U>
inline bool is_learning_problem(const T &x, const U &x_labels) {
...
template <typename T, typename U>
bool is_binary_classification_problem_impl(const T &x, const U &x_labels) {
...
test_binary_decision_function_impl(const dec_funct_type &dec_funct,
  ...
  for (long i = 0; i < x_test.nr(); ++i) {
    ...
    if (y_test(i) == +1.0) {
      ...
      throw dlib::error("invalid input labels to the "
                        "test_binary_decision_function() function");
    }
  }
...
test_binary_decision_function(const dec_funct_type &dec_funct,
                              const in_sample_vector_type &x_test,
                              const in_scalar_vector_type &y_test) {
  return test_binary_decision_function_impl(dec_funct, mat(x_test),
                                            mat(y_test));
}
...
bool is_sequence_segmentation_problem(
  ...
  if (is_learning_problem(samples, segments)) {
    for (unsigned long i = 0; i < samples.size(); ++i) {
      // Make sure the segments are inside samples[i] and don't overlap with
      // each other.
  ...
  if (is_learning_problem(samples, labels)) {
    for (unsigned long i = 0; i < samples.size(); ++i) {
      ...
      for (unsigned long j = 0; j < labels[i].size(); ++j) {
        if (!(-1 <= labels[i][j] &&
              labels[i][j] < (long)samples[i].second.size()))
          return false;
...
inline void
serialize(const labeled_detection<detection_type_, label_type_> &item,
          std::ostream &out) {
...
template <typename detection_type_, typename label_type_>
inline void deserialize(labeled_detection<detection_type_, label_type_> &item,
                        std::istream &in) {
...
cross_validate_trainer_impl(const trainer_type &trainer,
  ...
  // make sure requires clause is not broken
  DLIB_ASSERT(
      is_binary_classification_problem(x, y) == true && 1 < folds &&
          folds <= std::min(sum(y > 0), sum(y < 0)),
  ...
  // figure out how many positive and negative examples we will have in each
  // fold
  ...
  for (long i = 0; i < folds; ++i) {
    ...
    try {
      ...
      res += test_binary_decision_function(
          trainer.train(rowm(x, x_train), y_train), rowm(x, x_test), y_test);
...
cross_validate_trainer(const trainer_type &trainer,
                       const in_sample_vector_type &x,
                       const in_scalar_vector_type &y, const long folds) {
  return cross_validate_trainer_impl(trainer, mat(x), mat(y), folds);
}

// ----------------------------------------------------------------------------------------

namespace prob_impl {
template <typename vect_type> struct objective {
  objective(const vect_type &f_, const vect_type &t_) : f(f_), t(t_) {}

  double operator()(const matrix<double, 2, 1> &x) const {
    const double A = x(0);
    const double B = x(1);
    ...
    for (unsigned long i = 0; i < f.size(); ++i) {
      ...
      // See the paper "A Note on Platt's Probabilistic Outputs for Support
      // Vector Machines" for an explanation of why this code looks the way it
      // does (rather than being the obvious formula).
      if (val < 0)
...
template <typename vect_type> struct der {
  der(const vect_type &f_, const vect_type &t_) : f(f_), t(t_) {}

  matrix<double, 2, 1> operator()(const matrix<double, 2, 1> &x) const {
    const double A = x(0);
    const double B = x(1);
...
template <typename vect_type> struct hessian {
  hessian(const vect_type &f_, const vect_type &t_) : f(f_), t(t_) {}

  matrix<double, 2, 2> operator()(const matrix<double, 2, 1> &x) const {
    const double A = x(0);
    const double B = x(1);
    ...
    for (unsigned long i = 0; i < f.size(); ++i) {
      ...
      // compute pn = 1 - pp
      // but do so in a way that avoids numerical overflow and catastrophic
      // cancellation.
      double pp, pn;
      if (val < 0) {
...
inline double platt_scale(const std::pair<double, double> &params,
                          const double score) {
...
learn_platt_scaling(const std::vector<T, alloc> &scores,
  ...
  for (unsigned long i = 0; i < labels.size(); ++i) {
    ...
    if (labels[i] == +1.0) {
      ...
      throw dlib::error(
          "invalid input labels to the learn_platt_scaling() function.");
    }
  }
  ...
  val = 0;
  find_min(newton_search_strategy(obj_hessian), objective_delta_stop_strategy(),
           obj, obj_der, val, 0);
...
train_probabilistic_decision_function(const trainer_type &trainer,
  ...
  */
  ...
  // make sure requires clause is not broken
  DLIB_ASSERT(is_binary_classification_problem(x, y) == true && 1 < folds &&
                  folds <= (long)x.size(),
  ...
  return probabilistic_function<typename trainer_type::trained_function_type>(
      A, B, trainer.train(x, y));
}

// ----------------------------------------------------------------------------------------

template <typename trainer_type> struct trainer_adapter_probabilistic {
  typedef probabilistic_function<typename trainer_type::trained_function_type>
      trained_function_type;

  const trainer_type trainer;
  const long folds;

  trainer_adapter_probabilistic(const trainer_type &trainer_, const long folds_)
      : trainer(trainer_), folds(folds_) {}

  template <typename T, typename U>
  const trained_function_type train(const T &samples, const U &labels) const {
    return train_probabilistic_decision_function(trainer, samples, labels,
                                                 folds);
  }
};
...
typename enable_if<is_matrix<T>, void>::type randomize_samples(T &t, U &u, V &v,
                                                               rand_type &r) {
  // make sure requires clause is not broken
  DLIB_ASSERT(is_vector(t) && is_vector(u) && is_vector(v) &&
                  u.size() == t.size() && u.size() == v.size(),
...
template <typename T, typename U, typename V, typename rand_type>
typename disable_if<is_matrix<T>, void>::type
randomize_samples(T &t, U &u, V &v, rand_type &r) {
  // make sure requires clause is not broken
...
typename disable_if<is_rand<V>, void>::type randomize_samples(T &t, U &u,
                                                              V &v) {
  rand r;
...
typename enable_if_c<is_matrix<T>::value && is_rand<rand_type>::value,
                     void>::type
randomize_samples(T &t, U &u, rand_type &r) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
      is_vector(t) && is_vector(u) && u.size() == t.size(),
...
template <typename T, typename U, typename rand_type>
typename disable_if_c<is_matrix<T>::value || !is_rand<rand_type>::value,
                      void>::type
randomize_samples(T &t, U &u, rand_type &r) {
  // make sure requires clause is not broken
...
template <typename T, typename U>
typename disable_if<is_rand<U>, void>::type randomize_samples(T &t, U &u) {
  rand r;
...
typename enable_if_c<is_matrix<T>::value && is_rand<rand_type>::value,
                     void>::type
randomize_samples(T &t, rand_type &r) {
  // make sure requires clause is not broken
  DLIB_ASSERT(is_vector(t),
              "\t randomize_samples(t)"
                  << "\n\t invalid inputs were given to this function"
                  << "\n\t is_vector(t): "
                  << (is_vector(t) ? "true" : "false"));
...
template <typename T, typename rand_type>
typename disable_if_c<(is_matrix<T>::value == true) ||
                          (is_rand<rand_type>::value == false),
                      void>::type
randomize_samples(T &t, rand_type &r) {
  long n = t.size() - 1;
  while (n > 0) {
...
template <typename T> void randomize_samples(T &t) {
  rand r;
