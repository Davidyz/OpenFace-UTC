...
public:
  ...
  one_vs_all_trainer() : verbose(false), num_threads(4) {}
  ...
  trained_function_type train(const std::vector<sample_type> &all_samples,
    ...
    DLIB_ASSERT(is_learning_problem(all_samples, all_labels),
                "\t trained_function_type "
                "one_vs_all_trainer::train(all_samples,all_labels)"
    ...
    for (unsigned long i = 0; i < distinct_labels.size(); ++i) {
      ...
      if (itr == trainers.end() && default_trainer.is_empty()) {
        ...
        sout << "In one_vs_all_trainer, no trainer registered for the " << l
             << " label.";
    ...
    parallel_for_helper helper(all_samples, all_labels, default_trainer,
                               trainers, verbose, distinct_labels);
    parallel_for(num_threads, 0, distinct_labels.size(), helper, 500);

    if (helper.error_message.size() != 0) {
      throw dlib::error("binary trainer threw while training one vs. all "
  ...
  struct parallel_for_helper {
    parallel_for_helper(const std::vector<sample_type> &all_samples_,
        ...
        : all_samples(all_samples_), all_labels(all_labels_),
          ...
          verbose(verbose_), distinct_labels(distinct_labels_) {}
    ...
    void operator()(long i) const {
      try {
        ...
        if (verbose) {
          ...
          std::cout << "Training classifier for " << l << " vs. all"
                    << std::endl;
        }
        ...
        any_decision_function<sample_type, scalar_type> binary_df =
            trainer.train(all_samples, labels);
