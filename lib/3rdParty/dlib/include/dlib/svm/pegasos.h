...
template <typename K> class svm_pegasos {
...
public:
  ...
  template <typename K_> struct rebind {
    typedef svm_pegasos<K_> other;
  };

  svm_pegasos()
      : max_sv(40), lambda_c1(0.0001), lambda_c2(0.0001), tau(0.01),
        tolerance(0.01), train_count(0),
        w(offset_kernel<kernel_type>(kernel, tau), tolerance, max_sv, false) {}

  svm_pegasos(const kernel_type &kernel_, const scalar_type &lambda_,
              const scalar_type &tolerance_, unsigned long max_num_sv)
      : max_sv(max_num_sv), kernel(kernel_), lambda_c1(lambda_),
        lambda_c2(lambda_), tau(0.01), tolerance(tolerance_), train_count(0),
        w(offset_kernel<kernel_type>(kernel, tau), tolerance, max_sv, false) {
  ...
  void clear() {
    ...
    w = kc_type(offset_kernel<kernel_type>(kernel, tau), tolerance, max_sv,
                false);
    train_count = 0;
  }

  void set_kernel(kernel_type k) {
    kernel = k;
    clear();
  }

  void set_max_num_sv(unsigned long max_num_sv) {
    // make sure requires clause is not broken
  ...
  unsigned long get_max_num_sv() const { return max_sv; }
  ...
  void set_lambda(scalar_type lambda_) {
    // make sure requires clause is not broken
  ...
  void set_lambda_class1(scalar_type lambda_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(0 < lambda_,
  ...
  void set_lambda_class2(scalar_type lambda_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(0 < lambda_,
  ...
  const scalar_type get_lambda_class1() const { return lambda_c1; }

  const scalar_type get_lambda_class2() const { return lambda_c2; }

  const scalar_type get_tolerance() const { return tolerance; }

  const kernel_type get_kernel() const { return kernel; }

  unsigned long get_train_count() const {
    return static_cast<unsigned long>(train_count);
  }

  scalar_type train(const sample_type &x, const scalar_type &y) {
    // make sure requires clause is not broken
    DLIB_ASSERT(y == -1 || y == 1,
  ...
  const decision_function<kernel_type> get_decision_function() const {
    distance_function<offset_kernel<kernel_type>> df =
        w.get_distance_function();
    return decision_function<kernel_type>(df.get_alpha(),
                                          -tau * sum(df.get_alpha()), kernel,
                                          df.get_basis_vectors());
  }

  void swap(svm_pegasos &item) {
...
template <typename K> void swap(svm_pegasos<K> &a, svm_pegasos<K> &b) {
  a.swap(b);
}
...
void replicate_settings(const svm_pegasos<T> &source, svm_pegasos<U> &dest) {
...
template <typename trainer_type> class batch_trainer {

  // ------------------------------------------------------------------------------------

  template <typename K, typename sample_vector_type> class caching_kernel {
  public:
    ...
    caching_kernel(const K &kern, const sample_vector_type &samps,
                   long cache_size_)
    ...
    bool operator==(const caching_kernel &item) const {
      return item.real_kernel == real_kernel && item.samples == samples;
    }
  ...
  private:
    ...
    void build_cache() const {
      std::sort(cache->frequency_of_use.rbegin(),
                cache->frequency_of_use.rend());
      counter = 0;
    ...
    struct cache_type {
      ...
      std::vector<long> sample_location; // where in the cache a sample is.  -1
                                         // means not in cache
...
public:
  ...
  batch_trainer() : min_learning_rate(0.1), use_cache(false), cache_size(100) {}

  batch_trainer(const trainer_type &trainer_,
                const scalar_type min_learning_rate_, bool verbose_,
                bool use_cache_, long cache_size_ = 100)
      : trainer(trainer_), min_learning_rate(min_learning_rate_),
        verbose(verbose_), use_cache(use_cache_), cache_size(cache_size_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(0 < min_learning_rate_ && cache_size_ > 0,
  ...
  const scalar_type get_min_learning_rate() const { return min_learning_rate; }
...
private:
  ...
  do_train(const in_sample_vector_type &x,
    ...
    while (cur_learning_rate > min_learning_rate) {
      ...
      // keep feeding the trainer data until its learning rate goes below our
      // threshold
      ...
      if (verbose) {
        if ((count & 0x7FF) == 0) {
          std::cout << "\rbatch_trainer(): Percent complete: "
                    << 100 * min_learning_rate / cur_learning_rate
                    << "             " << std::flush;
        }
        ++count;
      }
    ...
    if (verbose) {
      ...
      std::cout << "\rbatch_trainer(): Percent complete: 100           "
                << std::endl;
  ...
  template <typename in_sample_vector_type, typename in_scalar_vector_type>
  const decision_function<kernel_type>
  do_train_cached(const in_sample_vector_type &x,
                  const in_scalar_vector_type &y) const {
    ...
    typedef typename trainer_type::template rebind<ckernel_type>::other
        rebound_trainer_type;
    ...
    if (verbose) {
      ...
      std::cout << "\rbatch_trainer(): Percent complete: 100           "
                << std::endl;
      std::cout << "    Num sv: " << cached_df.basis_vectors.size()
                << std::endl;
      std::cout << "    bias:   " << cached_df.b << std::endl;

      return decision_function<kernel_type>(cached_df.alpha, cached_df.b,
                                            trainer.get_kernel(),
...
const batch_trainer<trainer_type>
batch(const trainer_type &trainer,
...
template <typename trainer_type>
const batch_trainer<trainer_type>
batch_cached(const trainer_type &trainer,
             const typename trainer_type::scalar_type min_learning_rate = 0.1,
             long cache_size = 100) {
  return batch_trainer<trainer_type>(trainer, min_learning_rate, false, true,
                                     cache_size);
}
...
const batch_trainer<trainer_type> verbose_batch_cached(
    ...
    const typename trainer_type::scalar_type min_learning_rate = 0.1,
    long cache_size = 100) {
  return batch_trainer<trainer_type>(trainer, min_learning_rate, true, true,
                                     cache_size);
}
