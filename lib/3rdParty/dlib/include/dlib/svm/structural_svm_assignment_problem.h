...
namespace dlib {
template <long n, typename T> struct column_matrix_static_resize {
  typedef T type;
};
...
template <typename T> struct add_one_to_static_feat_size {
  typedef typename column_matrix_static_resize<
      1, typename T::feature_vector_type>::type type;
};

// ----------------------------------------------------------------------------------------

template <typename feature_extractor>
class structural_svm_assignment_problem
    : noncopyable,
      public structural_svm_problem_threaded<
          matrix<double, 0, 1>,
          typename add_one_to_static_feat_size<feature_extractor>::type> {
public:
  typedef matrix<double, 0, 1> matrix_type;
  typedef typename add_one_to_static_feat_size<feature_extractor>::type
      feature_vector_type;
  ...
  structural_svm_assignment_problem(const std::vector<sample_type> &samples_,
      ...
      : structural_svm_problem_threaded<matrix_type, feature_vector_type>(
            num_threads),
        samples(samples_), labels(labels_), fe(fe_),
    ...
    DLIB_ASSERT(
        loss_per_false_association > 0 && loss_per_missed_association > 0,
        "\t "
    ...
    if (force_assignment) {
      DLIB_ASSERT(is_forced_assignment_problem(samples, labels),
                  "\t "
                  "structural_svm_assignment_problem::structural_svm_"
                  "assignment_problem()"
                      << "\n\t invalid inputs were given to this function"
...
private:
  ...
  template <typename T>
  void append_to_sparse_vect(T &psi, const T &vect) const {
  ...
  virtual void separation_oracle(const long idx,
    ...
    if (force_assignment) {
      ...
      unsigned long rhs_size =
          samples[idx].second.size() + samples[idx].first.size();
      size = rhs_size;
    }
      ...
      for (long c = 0; c < cost.nc(); ++c) {
        if (r < (long)samples[idx].first.size()) {
          if (c < (long)samples[idx].second.size()) {
            fe.get_features(samples[idx].first[r], samples[idx].second[c],
                            feats);
            const double bias = current_solution(current_solution.size() - 1);
            cost(r, c) =
                dot(colm(current_solution, 0, current_solution.size() - 1),
                    feats) +
                bias;

            // add in the loss since this corresponds to an incorrect
            // prediction.
            if (c != labels[idx][r]) {
    ...
    if (cost.size() != 0) {
      ...
      // double to integer.
      const double scale =
          (std::numeric_limits<dlib::int64>::max() / 1000) / max(abs(cost));
      matrix<dlib::int64> int_cost =
          matrix_cast<dlib::int64>(round(cost * scale));
      assignment = max_cost_assignment(int_cost);
      assignment.resize(samples[idx].first.size());
    }

    loss = 0;
    // adjust assignment so that non-assignments have a value of -1. Also
    // compute loss.
