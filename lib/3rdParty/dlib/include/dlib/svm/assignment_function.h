...
public:
  ...
  explicit assignment_function(const matrix<double, 0, 1> &weights_,
                               double bias_)
      : weights(weights_), bias(bias_), force_assignment(false) {
  ...
  assignment_function(const matrix<double, 0, 1> &weights_, double bias_,
                      const feature_extractor &fe_)
      : fe(fe_), weights(weights_), bias(bias_), force_assignment(false) {
    // make sure requires clause is not broken
  ...
  assignment_function(const matrix<double, 0, 1> &weights_, double bias_,
                      const feature_extractor &fe_, bool force_assignment_)
      : fe(fe_), weights(weights_), bias(bias_),
        force_assignment(force_assignment_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(fe_.num_features() ==
                    static_cast<unsigned long>(weights_.size()),
                "\t "
                "assignment_function::assignment_function(weights_,fe_,force_"
                "assignment_)"
                    << "\n\t These sizes should match"
                    << "\n\t fe_.num_features(): " << fe_.num_features()
                    << "\n\t weights_.size():    " << weights_.size()
  ...
  result_type operator()(const sample_type &item) const {
...
void deserialize(assignment_function<feature_extractor> &item,
  ...
  item = assignment_function<feature_extractor>(weights, bias, fe,
                                                force_assignment);
}
