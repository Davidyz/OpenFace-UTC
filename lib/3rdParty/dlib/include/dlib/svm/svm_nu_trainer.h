...
template <typename K> class svm_nu_trainer {
public:
  ...
  svm_nu_trainer() : nu(0.1), cache_size(200), eps(0.001) {}

  svm_nu_trainer(const kernel_type &kernel_, const scalar_type &nu_)
      : kernel_function(kernel_), nu(nu_), cache_size(200), eps(0.001) {
  ...
  const scalar_type get_nu() const { return nu; }
...
private:
  ...
  void calculate_rho_and_b(const scalar_vector_type2 &y,
                           const scalar_vector_type &alpha,
                           const scalar_vector_type &df, scalar_type &rho,
                           scalar_type &b) const {
    using namespace std;
    long num_p_free = 0;
    long num_n_free = 0;
...
template <typename K> void swap(svm_nu_trainer<K> &a, svm_nu_trainer<K> &b) {
  a.swap(b);
}
