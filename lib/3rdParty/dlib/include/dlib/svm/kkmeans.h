...
public:
  ...
  kkmeans(const kcentroid<kernel_type> &kc_) : kc(kc_), min_change(0.01) {
    set_number_of_centers(1);
  }

  ~kkmeans() {}

  const kernel_type &get_kernel() const { return kc.get_kernel(); }
  ...
  unsigned long number_of_centers() const { return centers.size(); }

  template <typename T, typename U>
  void train(const T &samples, const U &initial_centers, long max_iter = 1000) {
  ...
  void set_min_change(scalar_type min_change_) {
    // make sure requires clause is not broken
  ...
  const scalar_type get_min_change() const { return min_change; }
  ...
  friend void deserialize(kkmeans &item, std::istream &in) {
    ...
    for (unsigned long i = 0; i < item.centers.size(); ++i) {
      std::unique_ptr<kcentroid<kernel_type>> temp(
          new kcentroid<kernel_type>(kernel_type()));
      deserialize(*temp, in);
...
private:
  ...
  void do_train(const matrix_type &samples, const matrix_type2 &initial_centers,
                long max_iter = 1000) {
    COMPILE_TIME_ASSERT(
        (is_same_type<typename matrix_type::type, sample_type>::value));
    COMPILE_TIME_ASSERT(
        (is_same_type<typename matrix_type2::type, sample_type>::value));

    // make sure requires clause is not broken
    DLIB_ASSERT(
        samples.nc() == 1 && initial_centers.nc() == 1 &&
    ...
    long count = 0;
    const unsigned long min_num_change =
        static_cast<unsigned long>(min_change * samples.size());
    unsigned long num_changed = min_num_change;
    while (assignment_changed && count < max_iter &&
           num_changed >= min_num_change) {
      ++count;
...
struct dlib_pick_initial_centers_data {
  dlib_pick_initial_centers_data()
      : idx(0), dist(std::numeric_limits<double>::infinity()) {}
  long idx;
  double dist;
  bool operator<(const dlib_pick_initial_centers_data &d) const {
    return dist < d.dist;
  }
};
...
void pick_initial_centers(long num_centers, vector_type1 &centers,
  ...
  /*
      This function is basically just a non-randomized version of the kmeans++
     algorithm described in the paper: kmeans++: The Advantages of Careful
     Seeding by Arthur and Vassilvitskii

  */

  // make sure requires clause is not broken
  DLIB_ASSERT(num_centers > 1 && 0 <= percentile && percentile < 1 &&
                  samples.size() > 1,
  ...
  const long best_idx = static_cast<long>(
      std::max(0.0, samples.size() - samples.size() * percentile - 1));
  ...
  for (long i = 0; i < num_centers - 1; ++i) {
    ...
    for (unsigned long s = 0; s < samples.size(); ++s) {
      ...
      const double dist =
          k_cc + k(samples[s], samples[s]) - 2 * k(samples[s], centers[i]);
...
void find_clusters_using_kmeans(const array_type &samples,
...
#ifdef ENABLE_ASSERTS
  {
    ...
    for (unsigned long i = 0; i < samples.size(); ++i) {
      DLIB_ASSERT(is_vector(samples[i]) && samples[i].nr() == nr &&
                      samples[i].nc() == nc,
                  "\tvoid find_clusters_using_kmeans()"
                      << "\n\t You passed invalid arguments to this function"
...
void find_clusters_using_angular_kmeans(
  ...
  for (unsigned long i = 0; i < samples.size(); ++i) {
    ...
    // If there are zero vectors in samples then just say their length is 1 so
    // we can avoid a division by zero check later on.  Also, this doesn't
    // matter since zero vectors can be assigned to any cluster randomly as
    // there is no basis for picking one based on angle.
    if (lengths.back() == 0)
      lengths.back() = 1;
  }

  // We will keep the centers as unit vectors at all times throughout the
  // processing.
  for (unsigned long i = 0; i < centers.size(); ++i) {
    double len = length(centers[i]);
    // Avoid having length 0 centers.  If that is the case then pick another
    // center at random.
    while (len == 0) {
      centers[i] = matrix_cast<scalar_type>(
          gaussian_randm(centers[i].nr(), centers[i].nc(), seed++));
