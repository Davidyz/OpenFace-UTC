...
template <typename trainer_type> class roc_trainer_type {
  /*!
  ...
  svm_nu_trainer)
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          To do this you would use an instance of this object declared as
  follows: roc_trainer_type<trainer_type>(your_trainer, 0.99, +1);
  !*/
...
public:
  ...
  roc_trainer_type(const trainer_type &trainer_,
  ...
  const trained_function_type train(const in_sample_vector_type &samples,
                                    const in_scalar_vector_type &labels) const
  /*!
      requires
      ...
      ensures
          - performs training using the trainer object given to this object's
            constructor, then modifies the bias term in the returned decision
  function as discussed above, and finally returns the decision function.
  !*/
};
...
const roc_trainer_type<trainer_type>
roc_c1_trainer(const trainer_type &trainer,
...
decision_function objects (e.g. svm_nu_trainer) ensures
        - returns a roc_trainer_type object that has been instantiated with the
given arguments.  The returned roc trainer will select the decision function
...
template <typename trainer_type>
const roc_trainer_type<trainer_type>
roc_c2_trainer(const trainer_type &trainer,
               const typename trainer_type::scalar_type &desired_accuracy) {
  return roc_trainer_type<trainer_type>(trainer, desired_accuracy, -1);
}
/*!
