...
template <typename image_scanner_type, typename svm_struct_prob_type>
void configure_nuclear_norm_regularizer(const image_scanner_type &,
                                        svm_struct_prob_type &) {
  // does nothing by default.  Specific scanner types overload this function to
...
public:
  ...
  explicit structural_object_detection_trainer(
    ...
    DLIB_ASSERT(scanner_.get_num_detection_templates() > 0,
                "\t "
                "structural_object_detection_trainer::structural_object_"
                "detection_trainer(scanner_)"
  ...
  const image_scanner_type &get_scanner() const { return scanner; }

  bool auto_set_overlap_tester() const { return auto_overlap_tester; }
  ...
  test_box_overlap get_overlap_tester() const {
    ...
    DLIB_ASSERT(auto_set_overlap_tester() == false,
                "\t test_box_overlap "
                "structural_object_detection_trainer::get_overlap_tester()"
                    << "\n\t You can't call this function if the overlap "
                       "tester is generated dynamically."
                    << "\n\t this: " << this);
  ...
  scalar_type get_epsilon() const { return eps; }
  ...
  void set_c(scalar_type C_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C_ > 0, "\t void structural_object_detection_trainer::set_c()"
  ...
  void set_loss_per_missed_target(double loss) {
    ...
    DLIB_ASSERT(
        loss > 0,
        "\t void "
        "structural_object_detection_trainer::set_loss_per_missed_target(loss)"
            << "\n\t Invalid inputs were given to this function "
            << "\n\t loss: " << loss << "\n\t this: " << this);
  ...
  void set_loss_per_false_alarm(double loss) {
    ...
    DLIB_ASSERT(
        loss > 0,
        "\t void "
        "structural_object_detection_trainer::set_loss_per_false_alarm(loss)"
            << "\n\t Invalid inputs were given to this function "
            << "\n\t loss: " << loss << "\n\t this: " << this);
  ...
  train(const image_array_type &images,
    ...
    return train_impl(images, truth_object_detections, empty_ignore,
                      test_box_overlap());
  }
  ...
  train(const image_array_type &images,
        ...
        const test_box_overlap &ignore_overlap_tester =
            test_box_overlap()) const {
    return train_impl(images, truth_object_detections, ignore,
                      ignore_overlap_tester);
  }

  template <typename image_array_type>
  const trained_function_type train(const image_array_type &images,
                                    const std::vector<std::vector<rectangle>>
                                        &truth_object_detections) const {
    std::vector<std::vector<rectangle>> empty_ignore(images.size());
    return train(images, truth_object_detections, empty_ignore,
                 test_box_overlap());
  }
  ...
  train(const image_array_type &images,
        ...
        const test_box_overlap &ignore_overlap_tester =
            test_box_overlap()) const {
    std::vector<std::vector<full_object_detection>> truth_dets(
    ...
    for (unsigned long i = 0; i < truth_object_detections.size(); ++i) {
      for (unsigned long j = 0; j < truth_object_detections[i].size(); ++j) {
        truth_dets[i].push_back(
            full_object_detection(truth_object_detections[i][j]));
      }
    }
...
private:
  template <typename image_array_type>
  const trained_function_type
  train_impl(const image_array_type &images,
             const std::vector<std::vector<full_object_detection>>
                 &truth_object_detections,
             const std::vector<std::vector<rectangle>> &ignore,
      ...
      for (unsigned long j = 0; j < truth_object_detections[i].size(); ++j) {
        DLIB_ASSERT(
            ...
            "structural_object_detection_trainer::train()"
                ...
                << truth_object_detections[i][j].num_parts()
                << "\n\t "
                   "get_scanner().get_num_movable_components_per_detection_"
                   "template(): "
                << get_scanner()
                       .get_num_movable_components_per_detection_template()
    ...
    structural_svm_object_detection_problem<image_scanner_type,
                                            image_array_type>
        svm_prob(scanner, overlap_tester, auto_overlap_tester, images,
                 truth_object_detections, ignore, ignore_overlap_tester,
                 num_threads);
    ...
    return object_detector<image_scanner_type>(
        scanner, svm_prob.get_overlap_tester(), w);
  }
