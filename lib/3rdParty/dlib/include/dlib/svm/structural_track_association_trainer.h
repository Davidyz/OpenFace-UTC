...
public:
  structural_track_association_trainer() { set_defaults(); }
  ...
  void set_loss_per_false_association(double loss) {
    ...
    DLIB_ASSERT(loss > 0,
                "\t void "
                "structural_track_association_trainer::set_loss_per_false_"
                "association(loss)"
  ...
  void set_loss_per_track_break(double loss) {
    ...
    DLIB_ASSERT(
        loss > 0,
        "\t void "
        "structural_track_association_trainer::set_loss_per_track_break(loss)"
            << "\n\t Invalid inputs were given to this function "
            << "\n\t loss: " << loss << "\n\t this: " << this);
  ...
  double get_loss_per_track_break() const { return loss_per_track_break; }
  ...
  void set_c(double C_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C_ > 0, "\t void structural_track_association_trainer::set_c()"
  ...
  train(const std::vector<
    ...
    DLIB_ASSERT(is_track_association_problem(samples),
                ...
                "structural_track_association_trainer::train()"
                    << "\n\t invalid inputs were given to this function"
    ...
    feature_extractor_track_association<detection_type> fe(
        num_dims, learn_nonnegative_weights ? num_dims : 0);
    ...
    std::vector<std::pair<std::vector<detection_type>, std::vector<track_type>>>
        assignment_samples;
    std::vector<std::vector<long>> labels;
    for (unsigned long i = 0; i < samples.size(); ++i)
      convert_dets_to_association_sets(samples[i], assignment_samples, labels);

    return track_association_function<detection_type>(
        trainer.train(assignment_samples, labels));
  }
...
private:
  ...
  find_num_dims(const std::vector<std::vector<std::vector<
    ...
    DLIB_CASSERT(false, "No detection objects were given in the call to "
                        "dlib::structural_track_association_trainer::train()");
  }

  template <typename detections_at_single_time_step, typename detection_type,
            typename track_type>
  static void convert_dets_to_association_sets(
      const std::vector<detections_at_single_time_step> &det_history,
      std::vector<std::pair<std::vector<detection_type>,
                            std::vector<track_type>>> &data,
      std::vector<std::vector<long>> &labels) {
    if (det_history.size() < 1)
      return;

    typedef typename detections_at_single_time_step::value_type::label_type
        label_type;
  ...
  template <typename track_type, typename label_type,
            typename labeled_detection>
  static void
  add_dets_to_tracks(std::vector<track_type> &tracks,
                     std::map<label_type, unsigned long> &track_labels,
                     const std::vector<labeled_detection> &dets) {
