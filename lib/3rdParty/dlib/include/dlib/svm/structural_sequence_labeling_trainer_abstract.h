...
public:
  ...
  explicit structural_sequence_labeling_trainer(const feature_extractor &fe);
  /*!
      ensures
          - #get_c() == 100
          - this object isn't verbose
  ...
  /*!
      ensures
          - During training, this object basically runs the sequence_labeler on
            each training sample, over and over.  To speed this up, it is
  possible to cache the results of these labeler invocations.  This function
  ...
  /*!
      ensures
          - returns the SVM regularization parameter.  It is the parameter
            ...
            improving the generalization of the resulting sequence labeler.
  Larger values encourage exact fitting while smaller values of C may encourage
            better generalization.
  !*/

  double get_loss(unsigned long label) const;
  /*!
      requires
          - label < num_labels()
      ensures
          - returns the loss incurred when a sequence element with the given
            ...
            correctly classifying this type of label.  Larger loss values
  indicate that we care more strongly than smaller values.
  !*/

  void set_loss(unsigned long label, double value);
  /*!
      requires
          - label < num_labels()
          - value >= 0
  ...
  const sequence_labeler<feature_extractor>
  train(const std::vector<sample_sequence_type> &x,
        const std::vector<labeled_sequence_type> &y) const;
  /*!
      requires
      ...
      ensures
          ...
          - returns a function F with the following properties:
              - F(new_x) == A sequence of predicted labels for the elements of
  new_x.
