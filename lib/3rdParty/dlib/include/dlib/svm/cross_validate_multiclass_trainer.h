...
const matrix<double> cross_validate_multiclass_trainer(
  ...
  // make sure requires clause is not broken
  DLIB_ASSERT(is_learning_problem(x, y) == true && 1 < folds &&
                  folds <= static_cast<long>(x.size()),
  ...
  // figure out how many samples from each class will be in the test and train
  // splits
  std::map<label_type, long> num_in_test, num_in_train;
  for (typename std::map<label_type, long>::iterator i = label_counts.begin();
       i != label_counts.end(); ++i) {
    const long in_test = i->second / folds;
    if (in_test == 0) {
      std::ostringstream sout;
      sout << "In dlib::cross_validate_multiclass_trainer(), the number of "
              "folds was larger"
           << std::endl;
      sout << "than the number of elements of one of the training classes."
           << std::endl;
  ...
  for (long i = 0; i < folds; ++i) {
    ...
    try {
      ...
      res += test_multiclass_decision_function(trainer.train(x_train, y_train),
                                               x_test, y_test);
