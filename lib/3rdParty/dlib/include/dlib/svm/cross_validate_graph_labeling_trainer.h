...
matrix<double, 1, 2> cross_validate_graph_labeling_trainer(
  ...
  for (long i = 0; i < folds; ++i) {
    ...
    const typename trainer_type::trained_function_type &labeler =
        trainer.train(samples_train, labels_train, losses_train);
...
matrix<double, 1, 2> cross_validate_graph_labeling_trainer(
  ...
  std::vector<std::vector<double>> losses;
  return cross_validate_graph_labeling_trainer(trainer, samples, labels, losses,
                                               folds);
}
