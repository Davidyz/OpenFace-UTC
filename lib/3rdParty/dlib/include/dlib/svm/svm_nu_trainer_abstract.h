...
template <typename K> class svm_nu_trainer {
  ...
  SMO algorithm.
          ...
          The implementation of the nu-svm training algorithm used by this
  object is based on the following excellent papers:
...
public:
  ...
  svm_nu_trainer(const kernel_type &kernel, const scalar_type &nu);
  /*!
      requires
          - 0 < nu <= 1
      ensures
  ...
  const scalar_type get_nu() const;
  /*!
      ensures
  ...
  /*!
      ...
      ensures
          - trains a nu support vector classifier given the training samples in
  x and labels in y.  Training is done when the error is less than
  get_epsilon().
