...
template <typename K> class rbf_network_trainer {
  /*!
      REQUIREMENTS ON K
...
public:
  ...
  void set_num_centers(const unsigned long num_centers);
  /*!
      ensures
          - #get_num_centers() == num_centers
  !*/

  const unsigned long get_num_centers() const;
  /*!
      ensures
          - returns the maximum number of centers (a.k.a. basis_vectors in the
            trained decision_function) you will get when you train this object
  on data.
  !*/
...
template <typename K>
void swap(rbf_network_trainer<K> &a, rbf_network_trainer<K> &b) {
  a.swap(b);
}
/*!
