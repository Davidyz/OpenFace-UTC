...
template <typename dec_funct_type> class null_trainer_type {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          function and do some kind of post processing to it.  The
  null_trainer_type object is useful because you can use it to run an already
...
public:
  ...
  null_trainer_type(const dec_funct_type &dec_funct);
  /*!
      ensures
  ...
  const dec_funct_type &train(const in_sample_vector_type &x,
...
template <typename dec_funct_type>
const null_trainer_type<dec_funct_type>
null_trainer(const dec_funct_type &dec_funct) {
  return null_trainer_type<dec_funct_type>(dec_funct);
}
/*!
    ensures
        - returns a null_trainer_type object that has been instantiated with
          the given arguments.  That is, this function returns a
null_trainer_type trainer that will return a copy of the given dec_funct object
every time someone calls its train() function.
!*/
