...
template <typename kern_type> class svm_pegasos {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          The implementation of the Pegasos algorithm used by this object is
  based on the following excellent paper: Pegasos: Primal estimated sub-gradient
  solver for SVM (2007) by Shai Shalev-Shwartz, Yoram Singer, Nathan Srebro In
  ICML

          This SVM training algorithm has two interesting properties.  First,
  the pegasos algorithm itself converges to the solution in an amount of time
          unrelated to the size of the training set (in addition to being quite
  fast to begin with).  This makes it an appropriate algorithm for learning from
          very large datasets.  Second, this object uses the dlib::kcentroid
  object to maintain a sparse approximation of the learned decision function.
          This means that the number of support vectors in the resulting
  decision function is also unrelated to the size of the dataset (in normal SVM
...
public:
  ...
  const scalar_type get_lambda_class1() const;
  /*!
      ensures
  ...
  const scalar_type get_lambda_class2() const;
  /*!
      ensures
  ...
  const scalar_type get_tolerance() const;
  /*!
      ensures
  ...
  unsigned long get_max_num_sv() const;
  /*!
      ensures
  ...
  /*!
      ...
      ensures
          ...
          - returns the current learning rate
            (i.e.
  1/(get_train_count()*min(get_lambda_class1(),get_lambda_class2())) )
  !*/
...
void swap(svm_pegasos<kern_type> &a, svm_pegasos<kern_type> &b) {
  a.swap(b);
}
/*!
...
template <typename kern_type>
void serialize(const svm_pegasos<kern_type> &item, std::ostream &out);
/*!
    provides serialization support for svm_pegasos objects
!*/

template <typename kern_type>
void deserialize(svm_pegasos<kern_type> &item, std::istream &in);
/*!
    provides serialization support for svm_pegasos objects
!*/
...
template <typename trainer_type> class batch_trainer {
  /*!
      REQUIREMENTS ON trainer_type
          - trainer_type == some kind of online trainer object (e.g.
  svm_pegasos) replicate_settings() must also be defined for the type.
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          use objects like svm_pegasos with functions (e.g.
  cross_validate_trainer) that expect batch mode training objects.
  !*/
...
public:
  ...
  batch_trainer(const trainer_type &online_trainer,
  ...
  /*!
      requires
          ...
          - cache_size_ > 0
      ensures
          - returns a batch trainer object that uses the given online_trainer
  object to train a decision function.
  ...
  const scalar_type get_min_learning_rate() const;
  /*!
      ensures
...
/*!
    requires
        - min_learning_rate > 0
        - trainer_type == some kind of online trainer object that creates
decision_function objects (e.g. svm_pegasos).  replicate_settings() must also be
defined for the type. ensures
