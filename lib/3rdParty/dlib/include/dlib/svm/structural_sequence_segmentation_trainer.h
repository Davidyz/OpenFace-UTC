...
public:
  ...
  const feature_extractor &get_feature_extractor() const {
    return trainer.get_feature_extractor().fe;
  }

  void set_num_threads(unsigned long num) { trainer.set_num_threads(num); }

  unsigned long get_num_threads() const { return trainer.get_num_threads(); }
  ...
  double get_epsilon() const { return trainer.get_epsilon(); }

  unsigned long get_max_iterations() const {
    return trainer.get_max_iterations();
  }
  ...
  void be_verbose() { trainer.be_verbose(); }

  void be_quiet() { trainer.be_quiet(); }

  void set_oca(const oca &item) { trainer.set_oca(item); }

  const oca get_oca() const { return trainer.get_oca(); }
  ...
  double get_c() const { return trainer.get_c(); }

  void set_loss_per_missed_segment(double loss) {
    // make sure requires clause is not broken
    DLIB_ASSERT(loss >= 0,
                "\t void "
                "structural_sequence_segmentation_trainer::set_loss_per_missed_"
                "segment(loss)"
  ...
  double get_loss_per_missed_segment() const { return loss_per_missed_segment; }

  void set_loss_per_false_alarm(double loss) {
    // make sure requires clause is not broken
    DLIB_ASSERT(loss >= 0,
                "\t void "
                "structural_sequence_segmentation_trainer::set_loss_per_false_"
                "alarm(loss)"
                    << "\n\t invalid inputs were given to this function"
                    << "\n\t loss: " << loss << "\n\t this: " << this);
  ...
  train(const std::vector<sample_sequence_type> &x,
    ...
    DLIB_ASSERT(is_sequence_segmentation_problem(x, y) == true,
                "\t sequence_segmenter "
                "structural_sequence_segmentation_trainer::train(x,y)"
                    << "\n\t invalid inputs were given to this function"
                    << "\n\t x.size(): " << x.size()
    ...
    return sequence_segmenter<feature_extractor>(
        temp.get_weights(), trainer.get_feature_extractor().fe);
  }

private:
  structural_sequence_labeling_trainer<
      impl_ss::feature_extractor<feature_extractor>>
      trainer;
