...
template <typename K> class svr_trainer {
public:
  ...
  svr_trainer() : C(1), eps_insensitivity(0.1), cache_size(200), eps(0.001) {}
  ...
  void set_c(scalar_type C_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C_ > 0, "\t void svr_trainer::set_c()"
...
private:
  ...
  template <typename M> struct op_quad {
    explicit op_quad(const M &m_) : m(m_) {}
  ...
  const matrix_op<op_quad<EXP>> make_quad(const matrix_exp<EXP> &m) const
  /*!
      ensures
  ...
  do_train(const in_sample_vector_type &x,
    ...
    solver(symmetric_matrix_cache<float>(
               make_quad(kernel_matrix(kernel_function, x)), cache_size),
           uniform_matrix<scalar_type>(2 * x.size(), 1, eps_insensitivity) +
               join_cols(y, -y),
           join_cols(uniform_matrix<scalar_type>(x.size(), 1, 1),
                     uniform_matrix<scalar_type>(x.size(), 1, -1)),
           0, C, C, alpha, eps);

    scalar_type b;
    calculate_b(alpha, solver.get_gradient(), C, b);

    alpha = -rowm(alpha, range(0, x.size() - 1)) +
            rowm(alpha, range(x.size(), alpha.size() - 1));
  ...
  template <typename scalar_vector_type>
  void calculate_b(const scalar_vector_type &alpha,
                   const scalar_vector_type &df, const scalar_type &C,
                   scalar_type &b) const {
    using namespace std;
    long num_free = 0;
...
template <typename K> void swap(svr_trainer<K> &a, svr_trainer<K> &b) {
  a.swap(b);
}
