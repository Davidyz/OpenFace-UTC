...
public:
  ...
  roc_trainer_type() : desired_accuracy(0), class_selection(0) {}

  roc_trainer_type(const trainer_type &trainer_,
                   const scalar_type &desired_accuracy_,
                   const scalar_type &class_selection_)
      : trainer(trainer_), desired_accuracy(desired_accuracy_),
        class_selection(class_selection_) {
...
private:
  ...
  do_train(const in_sample_vector_type &samples,
           const in_scalar_vector_type &labels) const {
    ...
    // obtain all the scores from the df using all the class_selection labeled
    // samples
    ...
    // now pick out the index that gives us the desired accuracy with regards to
    // selected class
    unsigned long idx =
        static_cast<unsigned long>(desired_accuracy * scores.size() + 0.5);
    if (idx >= scores.size())
      idx = scores.size() - 1;

    df.b = scores[idx];

    // In this case add a very small extra amount to the bias so that all the
    // samples with the class_selection label are classified correctly.
