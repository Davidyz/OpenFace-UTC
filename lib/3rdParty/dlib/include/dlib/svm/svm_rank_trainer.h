...
template <typename matrix_type, typename sample_type>
...
public:
  ...
  */
  ...
  oca_problem_ranking_svm(
      ...
      : samples(samples_), C(C_), be_verbose(be_verbose_), eps(eps_),
        max_iterations(max_iter), dims(dims_) {}

  virtual scalar_type get_c() const { return C; }

  virtual long get_num_dimensions() const { return dims; }
  ...
  virtual void get_risk(matrix_type &w, scalar_type &risk,
    ...
    // Note that we want the risk value to be in terms of the fraction of
    // overall rank flips.  So a risk of 0.1 would mean that rank flips happen <
    // 10% of the time.
...
template <typename matrix_type, typename sample_type, typename scalar_type>
oca_problem_ranking_svm<matrix_type, sample_type> make_oca_problem_ranking_svm(
    const scalar_type C, const std::vector<ranking_pair<sample_type>> &samples,
    const bool be_verbose, const scalar_type eps,
    const unsigned long max_iterations, const unsigned long dims) {
...
public:
  ...
  // non-linear kernel to the svm_rank_trainer object.  You have to use one of
  // the linear kernels with this trainer.
  COMPILE_TIME_ASSERT(
  ...
  explicit svm_rank_trainer(const scalar_type &C_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C_ > 0, "\t svm_rank_trainer::svm_rank_trainer()"
  ...
  void set_epsilon(scalar_type eps_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(eps_ > 0, "\t void svm_rank_trainer::set_epsilon()"
  ...
  void set_c(scalar_type C_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C_ > 0, "\t void svm_rank_trainer::set_c()"
                            << "\n\t C_ must be greater than 0"
                            << "\n\t C_:    " << C_ << "\n\t this: " << this);
  ...
  const scalar_type get_c() const { return C; }
  ...
  train(const std::vector<ranking_pair<sample_type>> &samples) const {
    ...
    if (has_prior()) {
      ...
      if ((unsigned long)prior.size() < dims) {
        matrix<scalar_type, 0, 1> prior_temp =
            join_cols(prior, zeros_matrix<scalar_type>(dims - prior.size(), 1));
        solver(make_oca_problem_ranking_svm<w_type>(C, samples, verbose, eps,
                                                    max_iterations, dims),
               w, prior_temp);
      } else {
        solver(make_oca_problem_ranking_svm<w_type>(C, samples, verbose, eps,
    ...
    // Copy the results into the output basis vector.  The output vector might
    // be a sparse vector container so we need to use this special kind of copy
    // to handle that case.
