...
template <typename K> class svm_one_class_trainer {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          solving one-class classification problems.  It is implemented using
  the SMO algorithm.

          The implementation of the training algorithm used by this object is
  based on the following excellent paper:
...
public:
  ...
  svm_one_class_trainer(const kernel_type &kernel, const scalar_type &nu);
  /*!
      requires
          - 0 < nu <= 1
      ensures
  ...
  /*!
      ...
      ensures
          - trains a one-class support vector classifier given the training
  samples in x. Training is done when the error is less than get_epsilon().
