...
void test_track_association_function(
    ...
    unsigned long &total_dets, unsigned long &correctly_associated_dets) {
  const typename track_association_function::association_function_type &f =
      assoc.get_assignment_function();
  ...
  std::map<label_type, long>
      track_idx; // tracks[track_idx[id]] == track with ID id.

  for (unsigned long j = 0; j < samples.size(); ++j) {
    std::vector<labeled_detection<detection_type, label_type>> dets =
        samples[j];
    ...
    for (unsigned long k = 0; k < assignments.size(); ++k) {
      ...
      if (assignments[k] != -1) {
        ...
        if (track_idx.count(dets[k].label) &&
...
double test_track_association_function(
  ...
  for (unsigned long i = 0; i < samples.size(); ++i) {
    impl::test_track_association_function(assoc, samples[i], total_dets,
                                          correctly_associated_dets);
  }
...
double cross_validate_track_association_trainer(
  ...
  for (long i = 0; i < folds; ++i) {
    ...
    const track_association_function<detection_type> &df =
        trainer.train(samples_train);
    for (long cnt = 0; cnt < num_in_test; ++cnt) {
      impl::test_track_association_function(
          df, samples[next_test_idx], total_dets, correctly_associated_dets);
