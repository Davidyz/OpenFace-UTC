...
enum active_learning_mode { max_min_margin, ratio_margin };

// ----------------------------------------------------------------------------------------

template <typename kernel_type, typename in_sample_vector_type,
          typename in_scalar_vector_type, typename in_sample_vector_type2>
std::vector<unsigned long> impl_rank_unlabeled_training_samples(
    const svm_c_linear_dcd_trainer<kernel_type> &trainer,
    const in_sample_vector_type &samples, const in_scalar_vector_type &labels,
  ...
  // If there aren't any training samples then all unlabeled_samples are equally
  // good. So just report an arbitrary ordering.
  ...
  // We are going to score each unlabeled sample and put the score and index
  // into results.  Then at the end of this function we just sort it and return
  // the indices.
  std::vector<std::pair<double, unsigned long>> results;
  results.resize(unlabeled_samples.size());

  // make sure we use this trainer's ability to warm start itself since that
  // will make this whole function run a lot faster.  But first, we need to find
  // out what the state we will be warm starting from is.
  typedef typename svm_c_linear_dcd_trainer<kernel_type>::optimizer_state
      optimizer_state;
  ...
  for (long i = 0; i < unlabeled_samples.size(); ++i) {
    ...
    if (mode == max_min_margin) {
      // The score for this sample is its min possible margin over possible
      // labels. Therefore, this score measures how much flexibility we have to
      // label this sample however we want.  The intuition being that the most
      // useful points to label are the ones that are still free to obtain
      // either label.
      results[i] = std::make_pair(std::min(margin_p, margin_n), i);
    } else {
      // In this case, the score for the sample is a ratio that tells how close
      // the two margin values are to each other.  The closer they are the
      // better.  So in this case we are saying we are looking for samples that
      // have the same preference for either class label.
  ...
  // transfer results into a vector with just sample indices so we can return
  // it.
...
std::vector<unsigned long> rank_unlabeled_training_samples(
  ...
  return impl_rank_unlabeled_training_samples(
      trainer, mat(samples), mat(labels), mat(unlabeled_samples), mode);
}
