...
public:
  ...
  explicit feature_extractor_track_association(unsigned long num_dims_,
                                               unsigned long num_nonnegative_)
      : num_dims(num_dims_), num_nonnegative(num_nonnegative_) {}

  unsigned long num_features() const { return num_dims; }

  unsigned long num_nonnegative_weights() const { return num_nonnegative; }
  ...
  friend void serialize(const feature_extractor_track_association &item,
                        std::ostream &out) {
    serialize(item.num_dims, out);
    serialize(item.num_nonnegative, out);
  }

  friend void deserialize(feature_extractor_track_association &item,
                          std::istream &in) {
...
public:
  ...
  track_association_function(const association_function_type &assoc_)
      : assoc(assoc_) {}
  ...
  friend void serialize(const track_association_function &item,
                        std::ostream &out) {
    int version = 1;
  ...
  friend void deserialize(track_association_function &item, std::istream &in) {
    ...
    if (version != 1)
      throw serialization_error("Unexpected version found while deserializing "
                                "dlib::track_association_function.");

    deserialize(item.assoc, in);
  }

private:
  assignment_function<feature_extractor_track_association<detection_type>>
      assoc;
};
