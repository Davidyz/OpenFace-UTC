...
namespace dlib {
/*
    This file contains optimized overloads of the kcentroid object for the
   following linear cases: kcentroid<linear_kernel<T>>
...
template <typename T> class kcentroid<linear_kernel<T>> {
...
public:
  ...
  void scale_by(scalar_type cscale) { alpha *= cscale; }
  ...
  distance_function<kernel_type> get_distance_function() const {
    if (samples_seen > 0) {
      typename distance_function<kernel_type>::sample_vector_type
          temp_basis_vectors;
      ...
      return distance_function<kernel_type>(temp_alpha, squared_norm(), kernel,
                                            temp_basis_vectors);
    } else {
...
private:
  void do_train(const sample_type &x, scalar_type cscale, scalar_type xscale) {
    set_size_of_w(x);
  ...
  void set_size_of_w(const sample_type &x) {
    if (x.size() != w.size()) {
...
template <typename T> class kcentroid<offset_kernel<linear_kernel<T>>> {
...
public:
  ...
  distance_function<kernel_type> get_distance_function() const {
    ...
    if (samples_seen > 0) {
      ...
      // has an implicit extra dimension tacked on to it with the value of
      // w_extra. The kernel we are using takes normal vectors and implicitly
      // tacks the value x_extra onto their end.  So what we are doing here is
      // scaling w so that the value it should have tacked onto it is x_scale.
      // Note that we also adjust alpha so that the combination of alpha*w stays
      // the same.
      scalar_type scale;
...
template <typename T> class kcentroid<sparse_linear_kernel<T>> {
...
public:
  ...
  scalar_type squared_norm() const { return alpha * alpha * length_squared(w); }
...
private:
  void do_train(const sample_type &x, scalar_type cscale, scalar_type xscale) {
    ...
    if (temp != 0) {
      ...
      for (typename std::map<unsigned long, scalar_type>::iterator i =
               w.begin();
           i != w.end(); ++i) {
      ...
      for (typename sample_type::const_iterator i = x.begin(); i != x.end();
           ++i) {
...
// ----------------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------
//               Overloads for when kernel_type ==
//               offset_kernel<sparse_linear_kernel>
// ----------------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------

template <typename T> class kcentroid<offset_kernel<sparse_linear_kernel<T>>> {
