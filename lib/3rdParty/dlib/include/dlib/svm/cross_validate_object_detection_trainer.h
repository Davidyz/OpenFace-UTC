...
number_of_truth_hits(const std::vector<full_object_detection> &truth_boxes,
...
/*!
    ensures
        - returns the number of elements in truth_boxes which are overlapped by
an element of boxes.  In this context, two boxes, A and B, overlap if and only
if overlap_tester(A,B) == true.
        - No element of boxes is allowed to account for more than one element of
truth_boxes.
        - The returned number is in the range [0,truth_boxes.size()]
        - Adds the score for each box from boxes into all_dets and labels each
with a bool indicating if it hit a truth box.  Note that we skip boxes that
...
!*/
{
  ...
  for (unsigned long i = 0; i < boxes.size(); ++i) {
    ...
    if (used[i] ||
        !overlaps_any_box(overlaps_ignore_tester, ignore, boxes[i].second)) {
...
number_of_truth_hits(const std::vector<full_object_detection> &truth_boxes,
                     ...
                     std::vector<std::pair<double, bool>> &all_dets,
                     unsigned long &missing_detections) {
  return number_of_truth_hits(truth_boxes, ignore, boxes, overlap_tester,
                              all_dets, missing_detections, overlap_tester);
}
...
const matrix<double, 1, 3> test_object_detection_function(
  ...
  for (unsigned long i = 0; i < images.size(); ++i) {
    ...
    correct_hits += impl::number_of_truth_hits(truth_dets[i], ignore[i], hits,
                                               overlap_tester, all_dets,
                                               missing_detections);
...
const matrix<double, 1, 3> test_object_detection_function(
  ...
  return test_object_detection_function(detector, images, rects, ignore,
                                        overlap_tester, adjust_threshold);
}
...
const matrix<double, 1, 3> test_object_detection_function(
  ...
  return test_object_detection_function(detector, images, truth_dets, ignore,
                                        overlap_tester, adjust_threshold);
}
...
namespace impl {
template <typename array_type> struct array_subset_helper {
  typedef typename array_type::mem_manager_type mem_manager_type;

  array_subset_helper(const array_type &array_,
                      const std::vector<unsigned long> &idx_set_)
      : array(array_), idx_set(idx_set_) {}

  unsigned long size() const { return idx_set.size(); }

  typedef typename array_type::type type;
  const type &operator[](unsigned long idx) const {
    return array[idx_set[idx]];
  }
...
template <typename T>
const matrix_op<op_array_to_mat<array_subset_helper<T>>>
mat(const array_subset_helper<T> &m) {
...
const matrix<double, 1, 3> cross_validate_object_detection_trainer(
  ...
  // make sure requires clause is not broken
  DLIB_CASSERT(is_learning_problem(images, truth_dets) == true &&
                   ignore.size() == images.size() && 1 < folds &&
                   folds <= static_cast<long>(images.size()),
  ...
  for (long iter = 0; iter < folds; ++iter) {
    ...
    impl::array_subset_helper<image_array_type> array_subset(images,
                                                             train_idx_set);
    typename trainer_type::trained_function_type detector = trainer.train(
        array_subset, training_rects, training_ignores, overlap_tester);
    for (unsigned long i = 0; i < test_idx_set.size(); ++i) {
      std::vector<std::pair<double, rectangle>> hits;
      detector(images[test_idx_set[i]], hits, adjust_threshold);

      correct_hits += impl::number_of_truth_hits(
          truth_dets[test_idx_set[i]], ignore[i], hits, overlap_tester,
...
const matrix<double, 1, 3> cross_validate_object_detection_trainer(
  ...
  return cross_validate_object_detection_trainer(
      trainer, images, dets, ignore, folds, overlap_tester, adjust_threshold);
}
...
const matrix<double, 1, 3> cross_validate_object_detection_trainer(
  ...
  return cross_validate_object_detection_trainer(trainer, images, truth_dets,
                                                 ignore, folds, overlap_tester,
                                                 adjust_threshold);
}
