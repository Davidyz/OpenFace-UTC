...
typename enable_if<is_matrix<T>>::type assign(T &dest,
...
template <typename T, typename EXP>
typename disable_if<is_matrix<T>>::type assign(T &dest,
                                               const matrix_exp<EXP> &src) {
  // make sure requires clause is not broken
  DLIB_ASSERT(is_vector(src),
              "\t void assign(dest,src)"
...
template <typename T, typename U>
typename disable_if_c<is_matrix<T>::value || is_matrix<U>::value>::type
assign(T &dest,     // sparse
       const U &src // sparse
) {
...
typename disable_if<is_matrix<S>>::type
assign(std::map<T, U, Comp, Alloc> &dest, // sparse
       const S &src                       // sparse
) {
  dest.clear();
...
namespace impl {
template <typename T, typename U>
typename T::value_type::second_type general_dot(const T &a, const U &b) {
  typedef typename T::value_type::second_type scalar_type;
...
template <typename T, typename U>
inline typename T::value_type::second_type dot(const T &a, const U &b) {
  return general_dot(a, b);
}

template <typename T, typename U, typename alloc>
U dot(const std::vector<std::pair<T, U>, alloc> &a,
      const std::vector<std::pair<T, U>, alloc> &b) {
  // You are getting this error because you are attempting to use sparse sample
  // vectors but you aren't using an unsigned integer as your key type in the
  // sparse vectors.
...
template <typename T>
inline typename T::value_type::second_type dot(const T &a, const T &b) {
  return impl::dot(a, b);
}

template <typename T1, typename T2, typename T3, typename T4, typename T5,
          typename T6>
inline T4 dot(const std::vector<T1, T2> &a, const std::map<T3, T4, T5, T6> &b) {
  return impl::dot(a, b);
}

template <typename T1, typename T2, typename T3, typename T4, typename T5,
          typename T6>
inline T4 dot(const std::map<T3, T4, T5, T6> &a, const std::vector<T1, T2> &b) {
  return impl::dot(a, b);
}
...
template <typename T, typename EXP>
typename T::value_type::second_type dot(const matrix_exp<EXP> &b, const T &a) {
  return dot(a, b);
}
...
template <typename T, typename U>
typename disable_if<is_matrix<T>, void>::type scale_by(T &a, const U &value) {
...
template <typename T, typename U>
typename enable_if<is_matrix<T>, void>::type scale_by(T &a, const U &value) {
  a *= value;
}
...
template <typename T>
typename disable_if<is_matrix<T>, T>::type add(const T &a, const T &b) {
  T temp;
...
template <typename T, typename U>
typename enable_if_c<is_matrix<T>::value && is_matrix<U>::value,
                     matrix_add_exp<T, U>>::type
...
template <typename T>
typename disable_if<is_matrix<T>, T>::type subtract(const T &a, const T &b) {
  T temp;
...
template <typename T, typename U>
typename enable_if_c<is_matrix<T>::value && is_matrix<U>::value,
                     matrix_subtract_exp<T, U>>::type
subtract(const T &a, const U &b) {
...
namespace impl {
template <typename T>
typename enable_if<is_matrix<typename T::type>, unsigned long>::type
max_index_plus_one(const T &samples) {
  if (samples.size() > 0)
...
template <typename T>
typename enable_if<is_built_in_scalar_type<typename T::type>,
                   unsigned long>::type
max_index_plus_one(const T &sample) {
  return sample.size();
}

// This !is_built_in_scalar_type<typename T::type>::value is here to avoid an
// inexplicable bug in Vistual Studio 2005
template <typename T>
typename enable_if_c<(!is_built_in_scalar_type<typename T::type>::value) &&
                         (is_pair<typename T::type::value_type>::value),
                     unsigned long>::type
max_index_plus_one(const T &samples) {
  ...
  for (long i = 0; i < samples.size(); ++i) {
    if (samples(i).size() > 0)
      max_dim =
          std::max<unsigned long>(max_dim, (--samples(i).end())->first + 1);
  }
...
template <typename T>
typename enable_if<is_pair<typename T::value_type>, unsigned long>::type
max_index_plus_one(const T &sample) {
  if (sample.size() > 0)
...
typename disable_if_c<
    is_pair<typename T::value_type>::value ||
        is_same_type<typename T::value_type, sample_pair>::value ||
        is_same_type<typename T::value_type, ordered_sample_pair>::value,
    unsigned long>::type
max_index_plus_one(const T &samples) {
...
inline void add_to(matrix<T, NR, NC, MM, L> &dest, const matrix_exp<EXP> &src) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
      is_vector(dest) &&
          max_index_plus_one(src) <= static_cast<unsigned long>(dest.size()),
...
template <typename T, long NR, long NC, typename MM, typename L, typename EXP>
inline typename disable_if<is_matrix<EXP>>::type
add_to(matrix<T, NR, NC, MM, L> &dest, const EXP &src) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
      is_vector(dest) &&
          max_index_plus_one(src) <= static_cast<unsigned long>(dest.size()),
...
template <typename T, long NR, long NC, typename MM, typename L, typename EXP,
          typename U>
inline typename disable_if<is_matrix<EXP>>::type
add_to(matrix<T, NR, NC, MM, L> &dest, const EXP &src, const U &C) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
      is_vector(dest) &&
          max_index_plus_one(src) <= static_cast<unsigned long>(dest.size()),
...
template <typename T, long NR, long NC, typename MM, typename L, typename EXP>
inline void subtract_from(matrix<T, NR, NC, MM, L> &dest,
                          const matrix_exp<EXP> &src) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
      is_vector(dest) &&
...
template <typename T, long NR, long NC, typename MM, typename L, typename EXP>
inline typename disable_if<is_matrix<EXP>>::type
subtract_from(matrix<T, NR, NC, MM, L> &dest, const EXP &src) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
      is_vector(dest) &&
          max_index_plus_one(src) <= static_cast<unsigned long>(dest.size()),
...
template <typename T, long NR, long NC, typename MM, typename L, typename EXP,
          typename U>
inline void subtract_from(matrix<T, NR, NC, MM, L> &dest,
                          const matrix_exp<EXP> &src, const U &C) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
      is_vector(dest) &&
          max_index_plus_one(src) <= static_cast<unsigned long>(dest.size()),
...
template <typename T, long NR, long NC, typename MM, typename L, typename EXP,
          typename U>
inline typename disable_if<is_matrix<EXP>>::type
subtract_from(matrix<T, NR, NC, MM, L> &dest, const EXP &src, const U &C) {
  // make sure requires clause is not broken
  DLIB_ASSERT(
      is_vector(dest) &&
          max_index_plus_one(src) <= static_cast<unsigned long>(dest.size()),
...
template <typename T> typename T::value_type::second_type min(const T &a) {
...
template <typename T> typename T::value_type::second_type max(const T &a) {
  typedef typename T::value_type::second_type type;
...
inline matrix<typename sparse_vector_type::value_type::second_type, 0, 1>
sparse_to_dense(const sparse_vector_type &vect, unsigned long num_dimensions) {
  ...
  for (typename sparse_vector_type::const_iterator j = vect.begin();
       j != vect.end(); ++j) {
...
matrix<value_type, 0, 1>
sparse_to_dense(const std::vector<std::pair<idx_type, value_type>, alloc> &vect,
...
matrix<T2, 0, 1> sparse_to_dense(const std::map<T1, T2, T3, T4> &vect,
                                 unsigned long num_dimensions) {
  return impl::sparse_to_dense(vect, num_dimensions);
}
...
template <typename T>
typename enable_if<is_matrix<T>, T &>::type sparse_to_dense(T &item) {
  return item;
}

template <typename EXP>
matrix<typename EXP::type, 0, 1> sparse_to_dense(const matrix_exp<EXP> &item,
                                                 unsigned long num) {
...
template <typename T, typename U, typename alloc>
void make_sparse_vector_inplace(std::vector<std::pair<T, U>, alloc> &vect) {
  if (vect.size() > 0) {
...
void sparse_matrix_vector_multiply(const std::vector<sample_pair> &edges,
  ...
  // make sure requires clause is not broken
  DLIB_ASSERT(
      max_index_plus_one(edges) <= (unsigned long)v.size() && is_col_vector(v),
...
void sparse_matrix_vector_multiply(const matrix_exp<EXP> &m,
  ...
  for (typename sparse_vector_type::const_iterator i = v.begin(); i != v.end();
       ++i) {
