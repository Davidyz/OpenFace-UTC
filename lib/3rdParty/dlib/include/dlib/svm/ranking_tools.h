...
template <typename T>
void deserialize(ranking_pair<T> &item, std::istream &in) {
  int version = 0;
  deserialize(version, in);
  if (version != 1)
    throw dlib::serialization_error(
        "Wrong version found while deserializing dlib::ranking_pair");
...
template <typename T>
typename disable_if<is_matrix<T>, bool>::type
is_ranking_problem(const std::vector<ranking_pair<T>> &samples) {
  if (samples.size() == 0)
    return false;
...
template <typename T>
typename enable_if<is_matrix<T>, bool>::type
is_ranking_problem(const std::vector<ranking_pair<T>> &samples) {
  if (samples.size() == 0)
    return false;
...
unsigned long max_index_plus_one(const ranking_pair<T> &item) {
  return std::max(max_index_plus_one(item.relevant),
                  max_index_plus_one(item.nonrelevant));
}
...
void count_ranking_inversions(const std::vector<T> &x, const std::vector<T> &y,
  ...
  for (i = 0, j = 0; i < x_count.size(); ++i) {
    // Skip past y values that are in the correct order with respect to
    // xsort[i].
  ...
  for (i = 0, j = 0; j < y_count.size(); ++j) {
    // Skip past x values that are in the incorrect order with respect to
    // ysort[j].
...
namespace impl {
inline bool compare_first_reverse_second(const std::pair<double, bool> &a,
...
test_ranking_function(const ranking_function &funct,
  ...
  for (unsigned long i = 0; i < samples.size(); ++i) {
    ...
    // that, if you get a block of ranking values that are all equal, the
    // elements marked as true will come last.  This prevents a ranking from
    // outputting a constant value for everything and still getting a good MAP
    // score.
    std::sort(total_scores.rbegin(), total_scores.rend(),
              impl::compare_first_reverse_second);
    total_ranking.clear();
    ...
    // Note that we don't need to look at nonrel_counts since it is redundant
    // with the information in rel_counts in this case.
    total_wrong += sum(mat(rel_counts));
  }

  const double rank_swaps =
      static_cast<double>(total_pairs - total_wrong) / total_pairs;
...
cross_validate_ranking_trainer(const trainer_type &trainer,
                               ...
                               const long folds) {
  // make sure requires clause is not broken
  DLIB_ASSERT(is_ranking_problem(samples) && 1 < folds &&
                  folds <= static_cast<long>(samples.size()),
  ...
  for (long i = 0; i < folds; ++i) {
    ...
    const typename trainer_type::trained_function_type &df =
        trainer.train(samples_train);
    ...
    for (unsigned long i = 0; i < samples_test.size(); ++i) {
      ...
      // Now compute the average precision for this sample.  We need to sort the
      // results and the back them into total_ranking.  Note that we sort them
      // so that, if you get a block of ranking values that are all equal, the
      // elements marked as true will come last.  This prevents a ranking from
      // outputting a constant value for everything and still getting a good MAP
      // score.
      std::sort(total_scores.rbegin(), total_scores.rend(),
