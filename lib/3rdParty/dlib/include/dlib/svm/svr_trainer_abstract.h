...
template <typename K> class svr_trainer {
  ...
  support vector regression.  It is implemented using the SMO algorithm.

          The implementation of the eps-SVR training algorithm used by this
  object is based on the following paper:
...
public:
  ...
  void set_cache_size(long cache_size);
  /*!
      requires
          - cache_size > 0
      ensures
  ...
  const long get_cache_size() const;
  /*!
      ensures
  ...
  /*!
      ensures
          - returns the SVR regularization parameter.  It is the parameter that
            ...
            fitting while smaller values of C may encourage better
  generalization.
  !*/
  ...
  /*!
      ...
      ensures
          - performs support vector regression given the training samples in x
  and target values in y.
