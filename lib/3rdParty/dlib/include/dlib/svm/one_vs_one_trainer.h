...
public:
  ...
  one_vs_one_trainer() : verbose(false), num_threads(4) {}
  ...
  struct invalid_label : public dlib::error {
    invalid_label(const std::string &msg, const label_type &l1_,
                  const label_type &l2_)
        : dlib::error(msg), l1(l1_), l2(l2_){};

    virtual ~invalid_label() throw() {}
  ...
  trained_function_type train(const std::vector<sample_type> &all_samples,
    ...
    for (unsigned long i = 0; i < distinct_labels.size(); ++i) {
      for (unsigned long j = i + 1; j < distinct_labels.size(); ++j) {
        pairs.push_back(
            unordered_pair<label_type>(distinct_labels[i], distinct_labels[j]));

        // make sure we have a trainer for this pair
        const typename binary_function_table::const_iterator itr =
            trainers.find(pairs.back());
        if (itr == trainers.end() && default_trainer.is_empty()) {
          ...
          sout << "In one_vs_one_trainer, no trainer registered for the ("
               ...
               << ") label pair.";
          throw invalid_label(sout.str(), pairs.back().first,
                              pairs.back().second);
        }
      }
    }
    ...
    parallel_for_helper helper(all_samples, all_labels, default_trainer,
                               trainers, verbose, pairs);
    parallel_for(num_threads, 0, pairs.size(), helper, 500);

    if (helper.error_message.size() != 0) {
      throw dlib::error("binary trainer threw while training one vs. one "
                        "classifier.  Error was: " +
...
private:
  typedef std::map<unordered_pair<label_type>, any_trainer>
      binary_function_table;

  struct parallel_for_helper {
    parallel_for_helper(const std::vector<sample_type> &all_samples_,
        ...
        : all_samples(all_samples_), all_labels(all_labels_),
          default_trainer(default_trainer_), trainers(trainers_),
          verbose(verbose_), pairs(pairs_) {}
    ...
    void operator()(long i) const {
      try {
        ...
        if (verbose) {
          ...
          std::cout << "Training classifier for " << p.first << " vs. "
                    << p.second << std::endl;
        }
        ...
        any_decision_function<sample_type, scalar_type> binary_df =
