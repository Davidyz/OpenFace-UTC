...
public:
  ...
  krr_trainer() : verbose(false), max_basis_size(400), ekm_stale(true) {}
  ...
  void set_kernel(const kernel_type &k) { kern = k; }
  ...
  const scalar_type get_lambda() const { return trainer.get_lambda(); }
...
private:
  ...
  do_train(const in_sample_vector_type &x, const in_scalar_vector_type &y,
    ...
    // The first thing we do is make sure we have an appropriate ekm ready for
    // use below.
    if (basis_loaded()) {
      if (ekm_stale) {
        ekm.load(kern, basis);
      ...
      linearly_independent_subset_finder<kernel_type> lisf(kern,
                                                           max_basis_size);
      fill_lisf(lisf, x);
      ekm.load(lisf);
    }
    ...
    if (verbose) {
      std::cout << "\nNumber of basis vectors used: " << ekm.out_vector_size()
                << std::endl;
    }
    ...
    if (verbose) {
      std::cout << "Mean EKM projection error:                  " << rs.mean()
                << std::endl;
    ...
    decision_function<
        linear_kernel<matrix<scalar_type, 0, 0, mem_manager_type>>>
        lin_df;
  ...
  /*!
      CONVENTION
          ...
          - will_use_regression_loss_for_loo_cv() ==
  trainer.will_use_regression_loss_for_loo_cv()
          - get_search_lambdas() == trainer.get_search_lambdas()

          - basis_loaded() == (basis.size() != 0)
  !*/

  rr_trainer<linear_kernel<matrix<scalar_type, 0, 0, mem_manager_type>>>
      trainer;
