...
long find_next_best_alpha_to_update(
    const scalar_vector_type &S, const scalar_vector_type &Q,
    const scalar_vector_type &alpha,
    const matrix<long, 0, 1, mem_manager_type> &active_bases,
    const bool search_all_alphas, typename scalar_vector_type::type eps)
/*!
    ensures
...
!*/
{
  ...
  for (long i = 0; i < S.nr(); ++i) {
    ...
    if (active_bases(i) >= 0) {
      ...
      // we only pick an alpha to remove if this is a wide search and it wasn't
      // one of the recently added ones
        ...
        value =
            Q(i) * Q(i) / (S(i) - alpha(idx)) - std::log(1 - S(i) / alpha(idx));
      }
...
template <typename kern_type> class rvm_trainer {
  /*!
  ...
  maximisation for sparse Bayesian models. In C. M. Bishop and B. J. Frey
  (Eds.), Proceedings of the Ninth International Workshop on Artificial
  Intelligence and Statistics, Key West, FL, Jan 3-6.

      This code mostly does what is described in the above paper with the
  exception that here we use a different stopping condition as well as a
  modified alpha selection rule.  See the code for the exact details.
  !*/
...
public:
  ...
  rvm_trainer() : eps(0.001), max_iterations(2000) {}

  void set_max_iterations(unsigned long max_iterations_) {
  ...
  void set_kernel(const kernel_type &k) { kernel = k; }
...
private:
  ...
  do_train(const in_sample_vector_type &x,
    ...
    /*! This is the convention for the active_bases variable in the function:
        - if (active_bases(i) >= 0) then
            - alpha(active_bases(i)) == the alpha value associated with sample
    x(i)
            - weights(active_bases(i)) == the weight value associated with
    sample x(i)
            - colm(phi, active_bases(i)) == the column of phi associated with
    sample x(i)
            - colm(phi, active_bases(i)) == kernel column i (from
    get_kernel_colum())
        - else
            - the i'th sample isn't in the model and notionally has an alpha of
    infinity and a weight of 0.
    !*/
    ...
    while (iterations != max_iterations) {
      iterations++;
      if (recompute_beta) {
        // calculate the current t_estimate. (this is the predicted t value for
        // each sample according to the current state of the classifier)
        t_estimate = phi * weights;

        // calculate the current beta
        beta = sigmoid(t_estimate);
        beta = pointwise_multiply(
            beta,
            (uniform_matrix<scalar_type>(beta.nr(), beta.nc(), 1) - beta));
        recompute_beta = false;
      }

      // Compute optimal weights and sigma for current alpha using IRLS.  This
      // is the same technique documented in the paper by equations 12-14.
      ...
      while (weight_delta > 0.0001) {
        ...
        t_hat = t_estimate + trans(scale_columns(trans(t - sigmoid(t_estimate)),
                                                 reciprocal(beta)));
        ...
        beta = sigmoid(t_estimate);
        beta = pointwise_multiply(
            beta, uniform_matrix<scalar_type>(beta.nr(), beta.nc(), 1) - beta);
      }
      ...
      // compute S and Q using equations 24 and 25 (tempv =
      // phi*sigma*trans(phi)*B*t_hat)
      tempv = phi * tmp(sigma * tmp(trans(phi) *
                                    trans(scale_columns(trans(t_hat), beta))));
      for (long i = 0; i < S.size(); ++i) {
        ...
        // something in the active set then just get it right out of phi,
      ...
      const long selected_idx = rvm_helpers::find_next_best_alpha_to_update(
          S, Q, alpha, active_bases, search_all_alphas, eps);

      // if find_next_best_alpha_to_update didn't find any good alpha to update
      if (selected_idx == -1) {
        if (search_all_alphas == false) {
          // jump us to where search_all_alphas will be set to true and try
          // again
          ticker = rounds_of_narrow_search;
          continue;
        } else {
      ...
      if (active_bases(selected_idx) >= 0) {
        ...
        const scalar_type s =
            alpha(idx) * S(selected_idx) / (alpha(idx) - S(selected_idx));
        const scalar_type q =
            alpha(idx) * Q(selected_idx) / (alpha(idx) - S(selected_idx));
        ...
        if (q * q - s > 0) {
          ...
          // the new alpha value is infinite so remove the selected alpha from
          // our model
        ...
        if (q * q - s > 0) {
          ...
          // update phi by adding the new sample's kernel matrix column in as
          // one of phi's columns
    ...
    return decision_function<kernel_type>(mat(final_weights),
                                          ...
                                          kernel, mat(dictionary));
  }

  // ------------------------------------------------------------------------------------

  template <typename M1, typename M2>
  long pick_initial_vector(const M1 &x, const M2 &t) const {
    scalar_vector_type K_col;
    double max_projection = -std::numeric_limits<scalar_type>::infinity();
    long max_idx = 0;
    // find the row in the kernel matrix that has the biggest normalized
    // projection onto the t vector
  ...
  template <typename T>
  void get_kernel_colum(long idx, const T &x, scalar_vector_type &col) const {
    col.set_size(x.nr());
  ...
  template <typename M1, typename M2>
  scalar_type compute_initial_alpha(const M1 &phi, const M2 &t) const {
...
const typename kernel_type::scalar_type rvm_trainer<kernel_type>::tau =
    static_cast<typename kernel_type::scalar_type>(0.001);

// ----------------------------------------------------------------------------------------

template <typename K> void swap(rvm_trainer<K> &a, rvm_trainer<K> &b) {
  a.swap(b);
}
...
template <typename kern_type> class rvm_regression_trainer {
  /*!
...
public:
  ...
  rvm_regression_trainer() : eps(0.001) {}
  ...
  template <typename in_sample_vector_type, typename in_scalar_vector_type>
  const decision_function<kernel_type>
  train(const in_sample_vector_type &x, const in_scalar_vector_type &t) const {
    return do_train(mat(x), mat(t));
  }

  void swap(rvm_regression_trainer &item) {
    exchange(kernel, item.kernel);
    exchange(eps, item.eps);
  }
...
private:
  ...
  const decision_function<kernel_type>
  do_train(const in_sample_vector_type &x,
           const in_scalar_vector_type &t) const {
    ...
    while (true) {
      ...
      var = length_squared(t - phi * weights) /
            (x.nr() - weights.size() + trans(alpha) * diag(sigma));
  ...
  template <typename M1, typename M2>
  scalar_type compute_initial_alpha(const M1 &phi, const M2 &t,
                                    const scalar_type &var) const {
    const double temp = length_squared(phi);
    const double temp2 = trans(phi) * t;
...
template <typename kernel_type>
const typename kernel_type::scalar_type
    rvm_regression_trainer<kernel_type>::tau =
        static_cast<typename kernel_type::scalar_type>(0.001);

// ----------------------------------------------------------------------------------------

template <typename K>
void swap(rvm_regression_trainer<K> &a, rvm_regression_trainer<K> &b) {
  a.swap(b);
}
