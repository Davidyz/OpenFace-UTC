...
struct get_feats_functor {
  get_feats_functor(std::vector<std::pair<unsigned long, double>> &feats_)
      : feats(feats_) {}
...
void get_feature_vector(std::vector<std::pair<unsigned long, double>> &feats,
...
class structural_svm_sequence_labeling_problem
    : noncopyable,
      public structural_svm_problem_threaded<
          matrix<double, 0, 1>, std::vector<std::pair<unsigned long, double>>> {
public:
  ...
  structural_svm_sequence_labeling_problem(
      ...
      const feature_extractor &fe_, unsigned long num_threads = 2)
    ...
    DLIB_ASSERT(is_sequence_labeling_problem(samples, labels) == true &&
                ...
                "\t "
                "structural_svm_sequence_labeling_problem::structural_svm_"
                "sequence_labeling_problem()"
...
private:
  virtual long get_num_dimensions() const { return fe.num_features(); }

  virtual long get_num_samples() const { return samples.size(); }

  void get_joint_feature_vector(const sequence_type &sample,
    ...
    for (unsigned long i = 0; i < sample.size(); ++i) {
      candidate_labeling =
          rowm(mat(label), range(i, std::max((int)i - order, 0)));
  ...
  public:
    ...
    map_prob(const sequence_type &sequence_,
        ...
        : sequence(sequence_), label(label_), fe(fe_), weights(weights_),
  ...
  virtual void separation_oracle(const long idx,
    ...
    find_max_factor_graph_viterbi(
        map_prob(samples[idx], labels[idx], fe, current_solution, loss_values),
        y);
