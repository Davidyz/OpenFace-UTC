...
public:
  ...
  explicit krls(const kernel_type &kernel_, scalar_type tolerance_ = 0.001,
      ...
      : kernel(kernel_), my_tolerance(tolerance_),
  ...
  decision_function<kernel_type> get_decision_function() const {
    return decision_function<kernel_type>(mat(alpha), -sum(mat(alpha)) * tau,
...
const typename kernel_type::scalar_type krls<kernel_type>::tau =
    static_cast<typename kernel_type::scalar_type>(0.01);
