...
public:
  ...
  // non-linear kernel to the svm_c_linear_dcd_trainer object.  You have to use
  // one of the linear kernels with this trainer.
  COMPILE_TIME_ASSERT(
  ...
  svm_c_linear_dcd_trainer()
      : Cpos(1), Cneg(1), eps(0.1), max_iterations(10000), verbose(false),
        have_bias(true), last_weight_1(false), do_shrinking(true),
        do_svm_l2(false) {}

  explicit svm_c_linear_dcd_trainer(const scalar_type &C_)
      : Cpos(C_), Cneg(C_), eps(0.1), max_iterations(10000), verbose(false),
        have_bias(true), last_weight_1(false), do_shrinking(true),
        do_svm_l2(false) {
    // make sure requires clause is not broken
    DLIB_ASSERT(0 < C_, "\tsvm_c_trainer::svm_c_linear_dcd_trainer(kernel,C)"
                            << "\n\t invalid inputs were given to this function"
                            << "\n\t C_: " << C_);
  }

  bool includes_bias() const { return have_bias; }
  ...
  bool shrinking_enabled() const { return do_shrinking; }

  void enable_shrinking(bool enabled) { do_shrinking = enabled; }

  bool solving_svm_l2_problem() const { return do_svm_l2; }

  void solve_svm_l2_problem(bool enabled) { do_svm_l2 = enabled; }
  ...
  void set_c(scalar_type C) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C > 0, "\t void svm_c_linear_dcd_trainer::set_c()"
  ...
  void set_c_class1(scalar_type C) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C > 0, "\t void svm_c_linear_dcd_trainer::set_c_class1()"
                           << "\n\t C must be greater than 0"
                           << "\n\t C:    " << C << "\n\t this: " << this);
  ...
  void set_c_class2(scalar_type C) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C > 0, "\t void svm_c_linear_dcd_trainer::set_c_class2()"
                           << "\n\t C must be greater than 0"
                           << "\n\t C:    " << C << "\n\t this: " << this);
  ...
  private:
    ...
    void init(const in_sample_vector_type &x, const in_scalar_vector_type &y,
              bool have_bias_, bool last_weight_1_, bool do_svm_l2_,
              scalar_type Cpos, scalar_type Cneg) {
      const long new_dims = max_index_plus_one(x);
      long new_idx = 0;

      if (did_init) {
        DLIB_CASSERT(
            have_bias_ == have_bias && last_weight_1_ == last_weight_1,
            "\t decision_function svm_c_linear_dcd_trainer::train(x,y,state)"
                << "\n\t The given state object is invalid because the "
                   "previous trainer was configured differently."
        ...
        DLIB_CASSERT(
            ...
            "\t decision_function svm_c_linear_dcd_trainer::train(x,y,state)"
                << "\n\t The given state object is invalid because the "
                   "training data dimensions have shrunk."
                << "\n\t new_dims:  " << new_dims
                << "\n\t dims:      " << dims);

        DLIB_CASSERT(
            x.size() >= static_cast<long>(alpha.size()),
            "\t decision_function svm_c_linear_dcd_trainer::train(x,y,state)"
                << "\n\t The given state object is invalid because the "
                   "training data has fewer samples than previously."
                << "\n\t x.size():     " << x.size()
        ...
        if (new_dims != dims) {
          ...
          DLIB_CASSERT(
              is_matrix<sample_type>::value == false,
              "\t decision_function svm_c_linear_dcd_trainer::train(x,y,state)"
                  << "\n\t The given state object is invalid because the "
                     "training data dimensions have changed."
                  << "\n\t new_dims:  " << new_dims
                  << "\n\t dims:      " << dims);
          ...
          if (have_bias && !last_weight_1) {
            ...
            w = join_cols(join_cols(colm(w, 0, dims), zeros_matrix<scalar_type>(
                                                          new_dims - dims, 1)),
    ...
    template <typename T>
    typename enable_if<is_matrix<T>, scalar_type>::type
    length_squared(const T &x) const {
      if (!last_weight_1) {
        return dlib::dot(x, x);
      } else {
        // skip the last dimension
        return dlib::dot(colm(x, 0, x.size() - 1), colm(x, 0, x.size() - 1));
      }
    }

    template <typename T>
    typename disable_if<is_matrix<T>, scalar_type>::type
    length_squared(const T &x) const {
      if (!last_weight_1) {
        return dlib::dot(x, x);
      } else {
  ...
  public:
    ...
    friend void deserialize(optimizer_state &item, std::istream &in) {
      ...
      if (version != 1) {
        throw dlib::serialization_error("Error while deserializing "
                                        "dlib::svm_c_linear_dcd_trainer::"
                                        "optimizer_state, unexpected version.");
      }
...
private:
  ...
  template <typename in_sample_vector_type, typename in_scalar_vector_type>
  const decision_function<kernel_type> do_train(const in_sample_vector_type &x,
                                                const in_scalar_vector_type &y,
                                                optimizer_state &state) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(
    ...
    for (unsigned long iter = 0; iter < max_iterations; ++iter) {
      ...
      for (unsigned long i = 0; i < active_size; ++i) {
        ...
        const long j =
            i + state.rnd.get_random_32bit_number() % (active_size - i);
      ...
      for (unsigned long ii = 0; ii < active_size; ++ii) {
        ...
        const scalar_type U =
            do_svm_l2 ? std::numeric_limits<scalar_type>::infinity() : C;
        ...
        if (std::abs(PG) > 1e-12) {
          ...
          alpha[i] = std::min(
              std::max(alpha[i] - G / state.Q[i], (scalar_type)0.0), U);
  ...
  scalar_type dot(const scalar_vector_type &w,
                  const sample_type &sample) const {
    if (have_bias && !last_weight_1) {
  ...
  bool verbose;
  bool have_bias; // having a bias means we pretend all x vectors have an extra
                  // element which is always -1.
  bool last_weight_1;
  bool do_shrinking;
  bool do_svm_l2;
