...
const matrix<double, 1, 3> test_sequence_segmenter(
  ...
  const matrix<double, 1, 3> metrics =
      impl::raw_metrics_test_sequence_segmenter(segmenter, samples, segments);
  ...
  const double precision =
      (total_detections == 0) ? 1 : true_hits / total_detections;
  const double recall =
      (total_true_segments == 0) ? 1 : true_hits / total_true_segments;
...
const matrix<double, 1, 3> cross_validate_sequence_segmenter(
  ...
  std::vector<std::vector<std::pair<unsigned long, unsigned long>>> y_test,
      y_train;
  ...
  for (long i = 0; i < folds; ++i) {
    ...
    metrics += impl::raw_metrics_test_sequence_segmenter(
