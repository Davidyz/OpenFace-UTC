...
template <typename trainer_type, typename in_sample_vector_type> struct job {
...
struct task {
  template <typename trainer_type, typename mem_manager_type,
            typename in_sample_vector_type>
  void operator()(job<trainer_type, in_sample_vector_type> &j,
                  matrix<double, 1, 2, mem_manager_type> &result) {
    try {
      result = test_binary_decision_function(
          j.trainer.train(rowm(*j.x, j.x_train), j.y_train),
          rowm(*j.x, j.x_test), j.y_test);

      // Do this just to make j release it's memory since people might run
      // threaded cross validation on very large datasets.  Every bit of freed
      // memory helps out.
      j = job<trainer_type, in_sample_vector_type>();
    } catch (invalid_nu_error &) {
      // If this is a svm_nu_trainer then we might get this exception if the nu
      // is invalid.  In this case just return a cross validation score of 0.
      result = 0;
    } catch (std::bad_alloc &) {
      std::cerr << "\nstd::bad_alloc thrown while running "
                   "cross_validate_trainer_threaded().  Not enough memory.\n"
                << std::endl;
      throw;
    }
  }
...
cross_validate_trainer_threaded_impl(const trainer_type &trainer,
                                     const in_sample_vector_type &x,
                                     const in_scalar_vector_type &y,
                                     const long folds, const long num_threads) {
  ...
  DLIB_ASSERT(
      is_binary_classification_problem(x, y) == true && 1 < folds &&
          folds <= std::min(sum(y > 0), sum(y < 0)) && num_threads > 0,
...
template <typename trainer_type, typename in_sample_vector_type,
          typename in_scalar_vector_type>
const matrix<double, 1, 2, typename trainer_type::mem_manager_type>
cross_validate_trainer_threaded(const trainer_type &trainer,
                                const in_sample_vector_type &x,
                                const in_scalar_vector_type &y,
                                const long folds, const long num_threads) {
  return cross_validate_trainer_threaded_impl(trainer, mat(x), mat(y), folds,
                                              num_threads);
}
