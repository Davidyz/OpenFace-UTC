...
template <typename kernel_type> class kcentroid {
  /*!
      ...
      feature space.  It can be used to kernelize any algorithm that requires
  only the ability to perform vector addition, subtraction, scalar
  multiplication, and inner products.  It uses the sparsification technique
  described in the paper The Kernel Recursive Least Squares Algorithm by Yaakov
  Engel.

      To understand the code it would also be useful to consult page 114 of the
  book Kernel Methods for Pattern Analysis by Taylor and Cristianini as well as
  page 554 (particularly equation 18.31) of the book Learning with Kernels by
  Scholkopf and Smola.  Everything you really need to know is in the Engel
  paper.  But the other books help give more perspective on the issues involved.
  ...
  my_remove_oldest_first == false) then
              - for all valid 0 < i < dictionary.size():
                  - Let STRENGTHS[i] == the delta you would get for
  dictionary[i] (i.e. Approximately Linearly Dependent value) if you removed
  dictionary[i] from this object and then tried to add it back in.
                  - min_strength == the minimum value from STRENGTHS
                  - min_vect_idx == the index of the element in STRENGTHS with
  the smallest value
...
public:
  ...
  kcentroid()
      : my_remove_oldest_first(false), my_tolerance(0.001),
        my_max_dictionary_size(1000000), bias(0), bias_is_stale(false) {
    clear_dictionary();
  }
  ...
  explicit kcentroid(const kernel_type &kernel_, scalar_type tolerance_ = 0.001,
      ...
      : my_remove_oldest_first(remove_oldest_first_), kernel(kernel_),
        my_tolerance(tolerance_), my_max_dictionary_size(max_dictionary_size_),
        bias(0), bias_is_stale(false) {
  ...
  scalar_type operator()(const kcentroid &x) const {
    ...
    DLIB_ASSERT(x.get_kernel() == get_kernel(),
                "\tscalar_type kcentroid::operator()(const kcentroid& x)"
                    << "\n\tYou can only compare two kcentroid objects if they "
                       "use the same kernel"
                    << "\n\tthis: " << this);
  ...
  unsigned long dictionary_size() const { return dictionary.size(); }
  ...
  distance_function<kernel_type> get_distance_function() const {
    refresh_bias();
    return distance_function<kernel_type>(mat(alpha), bias, kernel,
                                          mat(dictionary));
  }

private:
  void refresh_bias() const {
    if (bias_is_stale) {
      bias_is_stale = false;
  ...
  scalar_type train_and_maybe_test(const sample_type &x, scalar_type cscale,
                                   scalar_type xscale, bool do_test) {
    scalar_type test_result = 0;
    const scalar_type kx = kernel(x, x);
    if (alpha.size() == 0) {
      // just ignore this sample if it is the zero vector (or really close to
      // being zero)
      if (std::abs(kx) > std::numeric_limits<scalar_type>::epsilon()) {
        // set initial state since this is the first training example we have
        // seen
        ...
        // the distance from an empty kcentroid and the zero vector is zero by
        // definition.
        return 0;
      }
    } else {
      ...
      // if this new vector isn't approximately linearly dependent on the
      // vectors in our dictionary.
      if (delta > min_strength && delta > my_tolerance) {
        ...
        if (dictionary.size() >= my_max_dictionary_size) {
          ...
          if (my_remove_oldest_first) {
            ...
            // if we have never computed the min_strength then we should compute
            // it
            if (min_strength == 0)
              recompute_min_strength();

            // select the dictionary vector that is most linearly dependent for
            // removal
        ...
        // update K_inv by computing the new one in the temp matrix
        // (equation 3.14)
        matrix<scalar_type, 0, 0, mem_manager_type> temp(K_inv.nr() + 1,
                                                         K_inv.nc() + 1);
  ...
  void remove_dictionary_vector(long i)
  /*!
      requires
          - 0 <= i < dictionary.size()
      ensures
  ...
  !*/
  {
    ...
    // remove the i'th vector from the inverse kernel matrix.  This formula is
    // basically just the reverse of the way K_inv is updated by equation 3.14
    // during normal training.
    K_inv =
        removerc(K_inv, i, i) - remove_row(colm(K_inv, i) / K_inv(i, i), i) *
                                    remove_col(rowm(K_inv, i), i);

    // now compute the updated alpha values to take account that we just removed
    // one of our dictionary vectors
  ...
  !*/
  {
    ...
    // here we loop over each dictionary vector and compute what its delta would
    // be if we were to remove it from the dictionary and then try to add it
    // back in.
    for (unsigned long i = 0; i < dictionary.size(); ++i) {
      // compute a = K_inv*k but where dictionary vector i has been removed
      a = (removerc(K_inv, i, i) - remove_row(colm(K_inv, i) / K_inv(i, i), i) *
                                       remove_col(rowm(K_inv, i), i)) *
  ...
  // temp variables here just so we don't have to reconstruct them over and
  // over.  Thus, they aren't really part of the state of this object.
