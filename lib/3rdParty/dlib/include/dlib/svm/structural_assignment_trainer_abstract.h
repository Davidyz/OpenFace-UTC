...
template <typename feature_extractor> class structural_assignment_trainer {
  /*!
      REQUIREMENTS ON feature_extractor
          It must be an object that implements an interface compatible with
          the example_feature_extractor defined in
  dlib/svm/assignment_function_abstract.h.

      WHAT THIS OBJECT REPRESENTS
          This object is a tool for learning to solve an assignment problem
  based on a training dataset of example assignments.  The training procedure
  produces an assignment_function object which can be used to predict the
  assignments of new data.
...
public:
  ...
  explicit structural_assignment_trainer(const feature_extractor &fe);
  /*!
      ensures
          - #get_c() == 100
          - this object isn't verbose
  ...
  longer to train.  You can think of this epsilon value as saying "solve the
            optimization problem until the average number of assignment mistakes
  ...
  /*!
      ensures
          - During training, this object basically runs the assignment_function
  on each training sample, over and over.  To speed this up, it is possible to
  ...
  /*!
      ensures
          - returns the amount of loss experienced for associating two objects
            together that shouldn't be associated.  If you care more about
  avoiding accidental associations than ensuring all possible associations are
            identified then then you can increase this value.
  !*/

  void set_loss_per_missed_association(double loss);
  /*!
      requires
          - loss > 0
      ensures
  ...
  double get_loss_per_missed_association() const;
  /*!
      ensures
          - returns the amount of loss experienced for failing to associate two
            ...
            objects that shouldn't be associated then you can increase this
  value.
  !*/
  ...
  void set_forces_assignment(bool new_value);
  /*!
      ensures
  ...
  the get_bias() field of the learned assignment_function will be 0 when
  ...
  const assignment_function<feature_extractor>
  ...
  /*!
      ...
      ensures
          ...
          - returns a function F with the following properties:
              - F(new_sample) == A set of assignments indicating how the
  elements of new_sample.first match up with the elements of new_sample.second.
