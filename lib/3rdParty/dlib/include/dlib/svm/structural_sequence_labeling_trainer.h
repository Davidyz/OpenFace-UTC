...
public:
  ...
  explicit structural_sequence_labeling_trainer(const feature_extractor &fe_)
      : fe(fe_) {
    set_defaults();
  }

  structural_sequence_labeling_trainer() { set_defaults(); }
  ...
  void set_c(double C_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C_ > 0, "\t void structural_sequence_labeling_trainer::set_c()"
  ...
  train(const std::vector<sample_sequence_type> &x,
    ...
    for (unsigned long i = 0; i < y.size(); ++i) {
      for (unsigned long j = 0; j < y[i].size(); ++j) {
        ...
        DLIB_ASSERT(y[i][j] < num_labels(),
                    "\t sequence_labeler "
                    "structural_sequence_labeling_trainer::train(x,y)"
    ...
    structural_svm_sequence_labeling_problem<feature_extractor> prob(
        x, y, fe, num_threads);
