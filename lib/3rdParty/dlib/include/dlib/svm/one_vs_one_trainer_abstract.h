...
class one_vs_one_trainer {
  ...
  std::ostream using operator<<.
      ...
      WHAT THIS OBJECT REPRESENTS
          ...
          classes then it trains N*(N-1)/2 binary classifiers which are then
  used to vote on the identity of a test sample.
...
public:
  ...
  typedef one_vs_one_decision_function<one_vs_one_trainer>
  ...
  void set_trainer(const any_trainer &trainer, const label_type &l1,
                   const label_type &l2);
  /*!
      requires
          - l1 != l2
  ...
  /*!
      ...
      ensures
          - trains a bunch of binary classifiers in a one vs one fashion to
  solve the given multiclass classification problem.
          - returns a one_vs_one_decision_function F with the following
  properties:
  ...
  set_trainer(trainer,l1,l2) form has been used then this exception is thrown if
  not all necessary label pairs have been given a trainer.

            invalid_label::l1 and invalid_label::l2 will contain the label pair
  which is missing a trainer object.  Additionally, the exception will contain
  an informative error message available via invalid_label::what().
  !*/
};
