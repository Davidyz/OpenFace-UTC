...
namespace dlib {
template <typename K> class rr_trainer {
...
public:
  ...
  // non-linear or sparse kernel to the rr_trainer object.  You have to use
  // dlib::linear_kernel with this trainer.
  COMPILE_TIME_ASSERT((is_same_type<K, linear_kernel<sample_type>>::value));

  rr_trainer() : verbose(false), use_regression_loss(true), lambda(0) {
  ...
  void use_regression_loss_for_loo_cv() { use_regression_loss = true; }

  void use_classification_loss_for_loo_cv() { use_regression_loss = false; }
  ...
  const scalar_type get_lambda() const { return lambda; }
...
private:
  ...
  do_train(const in_sample_vector_type &x, const in_scalar_vector_type &y,
           const bool output_loo_values, std::vector<scalar_type> &loo_values,
           scalar_type &the_lambda) const {
    // make sure requires clause is not broken
    ...
    /*
        ...
        There is a trick to compute leave one out cross validation results for
       many different lambda values quickly.  The following paper has a detailed
       discussion of various approaches:

            Notes on Regularized Least Squares by Ryan M. Rifkin and Ross A.
       Lippert.

            In the implementation of the rr_trainer I'm only using two simple
       equations from the above paper.


            First note that inv(C + lambda*I) can be computed for many different
       lambda values in an efficient way by using an eigen decomposition of C.
       So we use the fact that: inv(C + lambda*I) == V*inv(D +
       lambda*I)*trans(V) where V*D*trans(V) == C

            Also, via some simple linear algebra the above paper works out that
       the leave one out value for a sample x(i) is equal to the following: Let
       G = inv(C + lambda*I) let val = trans(x(i))*G*x(i);
        ...
        Finally, note that we will pretend there was a 1 appended to the end of
       each vector in x.  We won't actually do that though because we don't want
       to have to make a copy of all the samples.  So throughout the following
       code I have explicitly dealt with this.
    */
    ...
    // and saving the result so we don't have to recompute it over and over
    // later.
    ...
    if (lambda == 0 || output_loo_values) {
      ...
      // Remember the pretend 1 at the end of x(*).  We want to multiply
      // trans(V)*x(*) so to do this we pull the last column off trans(V) and
      // store it separately.
    ...
    // If we need to automatically select a lambda then do so using the LOOE
    ...
    if (lambda == 0) {
      ...
      // Compute leave one out errors for a bunch of different lambdas and pick
      // the best one.
    ...
    // If we haven't done this already and we are supposed to then compute the
    // LOO error rate for the current lambda and store the result in best_looe.
  ...
  inline scalar_type loss(const scalar_type &a, const scalar_type &b) const {
    if (use_regression_loss) {
      return (a - b) * (a - b);
    } else {
