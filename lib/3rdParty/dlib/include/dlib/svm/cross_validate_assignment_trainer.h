...
double cross_validate_assignment_trainer(
  ...
  if (trainer.forces_assignment()) {
    DLIB_ASSERT(is_forced_assignment_problem(samples, labels) && 1 < folds &&
    ...
    DLIB_ASSERT(is_assignment_problem(samples, labels) && 1 < folds &&
                    folds <= static_cast<long>(samples.size()),
                "\t double cross_validate_assignment_trainer()"
                    << "\n\t invalid inputs were given to this function"
                    << "\n\t samples.size(): " << samples.size()
  ...
  for (long i = 0; i < folds; ++i) {
    ...
    const typename trainer_type::trained_function_type &df =
        trainer.train(samples_train, labels_train);
