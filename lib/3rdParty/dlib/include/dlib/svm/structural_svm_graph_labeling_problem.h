...
bool is_graph_labeling_problem(const dlib::array<graph_type> &samples,
  ...
  // The graph must use all dense vectors or all sparse vectors.  It can't mix
  // the two types together.
  COMPILE_TIME_ASSERT((is_matrix<node_vector_type>::value &&
                       is_matrix<edge_vector_type>::value) ||
                      (!is_matrix<node_vector_type>::value &&
                       !is_matrix<edge_vector_type>::value));
  ...
  for (unsigned long i = 0; i < samples.size(); ++i) {
    if (samples[i].number_of_nodes() != labels[i].size()) {
      sout << "samples[" << i << "].number_of_nodes() doesn't match labels["
           << i << "].size().";
      reason_for_failure = sout.str();
      return false;
    }
    if (graph_contains_length_one_cycle(samples[i])) {
      sout << "graph_contains_length_one_cycle(samples[" << i
           << "]) returned true.";
      reason_for_failure = sout.str();
      return false;
    }
    ...
    for (unsigned long j = 0; j < samples[i].number_of_nodes(); ++j) {
      if (ismat && samples[i].node(j).data.size() == 0) {
        sout << "A graph contains an empty vector at node: samples[" << i
             << "].node(" << j << ").data.";
        reason_for_failure = sout.str();
        return false;
      }
      ...
      if (ismat && (long)samples[i].node(j).data.size() != node_dims) {
        sout << "Not all node vectors in samples[" << i
             << "] are the same dimension.";
        reason_for_failure = sout.str();
        return false;
      }

      for (unsigned long n = 0; n < samples[i].node(j).number_of_neighbors();
           ++n) {
        if (ismat && samples[i].node(j).edge(n).size() == 0) {
          sout << "A graph contains an empty vector at edge: samples[" << i
               << "].node(" << j << ").edge(" << n << ").";
          reason_for_failure = sout.str();
          return false;
        }
        if (min(samples[i].node(j).edge(n)) < 0) {
          sout << "A graph contains negative values on an edge vector at: "
                  "samples["
               << i << "].node(" << j << ").edge(" << n << ").";
          reason_for_failure = sout.str();
          return false;
        ...
        if (ismat && (long)samples[i].node(j).edge(n).size() != edge_dims) {
          sout << "Not all edge vectors in samples[" << i
               << "] are the same dimension.";
          reason_for_failure = sout.str();
          return false;
        }
...
inline bool
all_values_are_nonnegative(const std::vector<std::vector<double>> &x) {
...
namespace impl {
template <typename T, typename enable = void> struct fvect {
...
struct fvect<T, typename enable_if<is_matrix<typename T::type>>::type> {
  ...
  typedef matrix<value_type, NR, 1, typename node_mat::mem_manager_type,
                 typename node_mat::layout_type>
      type;
};
} // namespace impl
...
class structural_svm_graph_labeling_problem
    : noncopyable,
      public structural_svm_problem_threaded<
          matrix<double, 0, 1>, typename dlib::impl::fvect<graph_type>::type> {
public:
  ...
  structural_svm_graph_labeling_problem(
      ...
      : structural_svm_problem_threaded<matrix_type, feature_vector_type>(
            num_threads),
        samples(samples_), labels(labels_), losses(losses_) {
    // make sure requires clause is not broken
#ifdef ENABLE_ASSERTS
    std::string reason_for_failure;
    DLIB_ASSERT(
        is_graph_labeling_problem(samples, labels, reason_for_failure) == true,
        "\t "
        "structural_svm_graph_labeling_problem::structural_svm_graph_labeling_"
        "problem()"
    ...
    DLIB_ASSERT((losses.size() == 0 || sizes_match(labels, losses) == true) &&
                ...
                "\t "
                "structural_svm_graph_labeling_problem::structural_svm_graph_"
                "labeling_problem()"
                    << "\n\t Invalid inputs were given to this function."
                    << "\n\t labels.size():  " << labels.size()
    ...
    for (unsigned long i = 0; i < samples.size(); ++i) {
      for (unsigned long j = 0; j < samples[i].number_of_nodes(); ++j) {
        node_dims = std::max(node_dims,
                             (long)max_index_plus_one(samples[i].node(j).data));
        for (unsigned long n = 0; n < samples[i].node(j).number_of_neighbors();
             ++n) {
          edge_dims = std::max(
              edge_dims, (long)max_index_plus_one(samples[i].node(j).edge(n)));
        }
      }
    }
  }

  const std::vector<std::vector<double>> &get_losses() const { return losses; }

  long get_num_edge_weights() const { return edge_dims; }

  void set_loss_on_positive_class(double loss) {
    // make sure requires clause is not broken
    DLIB_ASSERT(
        loss >= 0 && get_losses().size() == 0,
        "\t void "
  ...
  double get_loss_on_negative_class() const {
    ...
    DLIB_ASSERT(
        get_losses().size() == 0,
        "\t double "
        "structural_svm_graph_labeling_problem::get_loss_on_negative_class()"
            << "\n\t Invalid inputs were given to this function."
            << "\n\t this: " << this);
  ...
  double get_loss_on_positive_class() const {
    ...
    DLIB_ASSERT(
        get_losses().size() == 0,
        "\t double "
        "structural_svm_graph_labeling_problem::get_loss_on_positive_class()"
            << "\n\t Invalid inputs were given to this function."
            << "\n\t this: " << this);
...
private:
  virtual long get_num_dimensions() const {
    // The psi/w vector will begin with all the edge dims and then follow with
    // the node dims.
  ...
  typename enable_if<is_matrix<psi_type>>::type
  get_joint_feature_vector(const sample_type &sample, const label_type &label,
                           psi_type &psi) const {
  ...
  template <typename T>
  void add_to_sparse_vect(T &psi, const T &vect, unsigned long offset) const {
  ...
  template <typename T>
  void subtract_from_sparse_vect(T &psi, const T &vect) const {
    for (typename T::const_iterator i = vect.begin(); i != vect.end(); ++i) {
  ...
  template <typename psi_type>
  typename disable_if<is_matrix<psi_type>>::type
  get_joint_feature_vector(const sample_type &sample, const label_type &label,
                           psi_type &psi) const {
    psi.clear();
    for (unsigned long i = 0; i < sample.number_of_nodes(); ++i) {
  ...
  virtual void separation_oracle(const long idx,
                                 ...
                                 double &loss, feature_vector_type &psi) const {
    ...
    for (unsigned long i = 0; i < g.number_of_nodes(); ++i) {
      g.node(i).data = dot(
          rowm(current_solution, range(edge_dims, current_solution.size() - 1)),
          samp.node(i).data);

      // Include a loss augmentation so that we will get the proper loss
      // augmented max when we use find_max_factor_graph_potts() below.
      if (labels[idx][i])
      ...
      for (unsigned long n = 0; n < g.node(i).number_of_neighbors(); ++n) {
        ...
        if (i < j) {
          g.node(i).edge(n) =
              dot(rowm(current_solution, range(0, edge_dims - 1)),
                  samp.node(i).edge(n));
        }
      }
    }
  ...
  double get_loss_for_sample(long sample_idx, long node_idx,
                             bool predicted_label) const
  /*!
      requires
