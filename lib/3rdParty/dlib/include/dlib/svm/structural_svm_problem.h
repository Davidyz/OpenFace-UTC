...
template <typename structural_svm_problem> class cache_element_structural_svm {
public:
  cache_element_structural_svm()
      : prob(0), sample_idx(0),
        last_true_risk_computed(std::numeric_limits<double>::infinity()) {}
  ...
  typedef
      typename structural_svm_problem::feature_vector_type feature_vector_type;

  void init(const structural_svm_problem *prob_, const long idx)
  /*!
      ensures
  ...
  void get_truth_joint_feature_vector_cached(feature_vector_type &psi) const {
    if (prob->get_max_cache_size() != 0)
      psi = true_psi;
    else
    ...
    if (is_matrix<feature_vector_type>::value) {
      DLIB_CASSERT((long)psi.size() == prob->get_num_dimensions(),
                   "The dimensionality of your PSI vector doesn't match "
                   "get_num_dimensions()");
    }
  }

  void separation_oracle_cached(const bool use_only_cache,
                                const bool skip_cache,
                                const scalar_type &saved_current_risk_gap,
                                const matrix_type &current_solution,
                                scalar_type &out_loss,
                                feature_vector_type &out_psi) const {
    const bool cache_enabled = prob->get_max_cache_size() != 0;

    // Don't waste time computing this if the cache isn't going to be used.
    const scalar_type dot_true_psi =
        cache_enabled ? dot(true_psi, current_solution) : 0;

    scalar_type best_risk = -std::numeric_limits<scalar_type>::infinity();
    unsigned long best_idx = 0;
    long max_lru_count = 0;
    if (cache_enabled) {
      // figure out which element in the cache is the best (i.e. has the biggest
      // risk)
      for (unsigned long i = 0; i < loss.size(); ++i) {
        const scalar_type risk =
            loss[i] + dot(psi[i], current_solution) - dot_true_psi;
        if (risk > best_risk) {
          best_risk = risk;
          out_loss = loss[i];
          best_idx = i;
      ...
      if (!skip_cache) {
        // Check if the best psi vector in the cache is still good enough to use
        // as a proxy for the true separation oracle.  If the risk value has
        // dropped by enough to get into the stopping condition then the best
        // psi isn't good enough.
    ...
    last_true_risk_computed =
        out_loss + dot(out_psi, current_solution) - dot_true_psi;

    // If the separation oracle is only solved approximately then the result
    // might not be as good as just selecting true_psi as the output.  So here
    // we check if that is the case.
    if (last_true_risk_computed < 0 && best_risk < 0) {
      out_psi = true_psi;
      out_loss = 0;
    }
    // Alternatively, an approximate separation oracle might not do as well as
    // just selecting from the cache.  So if that is the case when just take the
    // best element from the cache.
    ...
    // if the cache is full
      ...
      // Make sure this new cache entry has the best lru count since we have
      // used it most recently.
...
private:
  ...
  void compact_sparse_vector(std::vector<std::pair<T, U>, alloc> &vect) const {
    // If the sparse vector has more entires than dimensions then it must have
    // some duplicate elements.  So compact them using
...
public:
  /*!
      CONVENTION
          ...
          - if (skip_cache) then
              - we won't use the oracle cache when we need to evaluate the
  separation oracle. Instead, we will directly call the user supplied
  separation_oracle().
          ...
          - if (cache.size() != 0) then
              ...
              - for all i: cache[i] == the cached results of calls to
  separation_oracle() for the i-th sample.
  !*/
  ...
  structural_svm_problem()
      : saved_current_risk_gap(0), eps(0.001), max_iterations(10000),
        verbose(false), skip_cache(true), count_below_eps(0), max_cache_size(5),
        converged(false), nuclear_norm_part(0),
        cache_based_eps(std::numeric_limits<scalar_type>::infinity()), C(1) {}

  scalar_type get_cache_based_epsilon() const { return cache_based_eps; }

  void set_cache_based_epsilon(scalar_type eps_) {
  ...
  void set_epsilon(scalar_type eps_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(eps_ > 0, "\t void structural_svm_problem::set_epsilon()"
                              << "\n\t eps_ must be greater than 0"
                              << "\n\t eps_: " << eps_
                              << "\n\t this: " << this);
  ...
  void set_max_cache_size(unsigned long max_size) { max_cache_size = max_size; }

  unsigned long get_max_cache_size() const { return max_cache_size; }

  void be_verbose() { verbose = true; }

  void be_quiet() { verbose = false; }

  scalar_type get_c() const { return C; }

  void set_c(scalar_type C_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C_ > 0, "\t void structural_svm_problem::set_c()"
  ...
  void add_nuclear_norm_regularizer(long first_dimension, long rows, long cols,
    ...
    DLIB_ASSERT(0 <= first_dimension &&
                    first_dimension < get_num_dimensions() && 0 <= rows &&
                    0 <= cols &&
                    rows * cols + first_dimension <= get_num_dimensions() &&
  ...
  unsigned long num_nuclear_norm_regularizers() const {
    return nuclear_norm_regularizers.size();
  }

  void clear_nuclear_norm_regularizers() { nuclear_norm_regularizers.clear(); }
...
private:
  ...
  virtual bool optimization_status(scalar_type current_objective_value,
    ...
    if (verbose) {
      ...
      if (nuclear_norm_regularizers.size() != 0) {
        ...
        cout << "risk:                  "
             << current_risk_value - nuclear_norm_part << endl;
    ...
    if (converged) {
      return (current_risk_gap <
              std::max(cache_based_eps,
                       cache_based_eps * current_risk_value)) ||
             (current_risk_gap == 0);
    }
    ...
    if (current_risk_gap < eps) {
      ...
      // calls to the separation oracle and run all subsequent iterations off
      // the cache.
      ...
      // Only disable the cache if we have seen a few consecutive iterations
      // that look to have converged.
      if (count_below_eps > 1) {
        // Instead of stopping we shouldn't use the cache on the next iteration.
        // This way we can be sure to have the best solution rather than
        // assuming the cache is up-to-date enough.
        skip_cache = true;
        count_below_eps = 0;
      }
    } else {
  ...
  virtual void
  call_separation_oracle_on_all_samples(const matrix_type &w,
                                        matrix_type &subgradient,
                                        scalar_type &total_loss) const {
    feature_vector_type ftemp;
    const unsigned long num = get_num_samples();
    for (unsigned long i = 0; i < num; ++i) {
      scalar_type loss;
...
protected:
  void compute_nuclear_norm_parts(const matrix_type &m, matrix_type &grad,
                                  scalar_type &obj) const {
    obj = 0;
    ...
    for (unsigned long i = 0; i < nuclear_norm_regularizers.size(); ++i) {
      ...
      f = matrix_cast<double>(
          reshape(rowm(m, range(idx, idx + size - 1)), nr, nc));
      svd3(f, u, w, v);
      ...
      set_rowm(grad, range(idx, idx + size - 1)) =
          matrix_cast<double>(strength * reshape_to_column_vector(f));
    }
  ...
  void separation_oracle_cached(const long idx,
                                const matrix_type &current_solution,
                                scalar_type &loss,
                                feature_vector_type &psi) const {
    cache[idx].separation_oracle_cached(converged, skip_cache,
                                        saved_current_risk_gap,
                                        current_solution, loss, psi);
  }
  ...
  mutable std::vector<cache_element_structural_svm<structural_svm_problem>>
      cache;
