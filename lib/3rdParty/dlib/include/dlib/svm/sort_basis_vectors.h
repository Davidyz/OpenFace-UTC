...
typename EXP::matrix_type invert(const matrix_exp<EXP> &m) {
  ...
  const scalar_type thresh =
      max_eig * std::sqrt(std::numeric_limits<scalar_type>::epsilon());
  ...
  return make_symmetric(eig.get_pseudo_v() * diagm(reciprocal(vals)) *
                        trans(eig.get_pseudo_v()));
}
...
sort_basis_vectors_impl(const kernel_type &kern, const vect1_type &samples,
                        const vect2_type &labels, const vect3_type &basis,
                        double eps) {
  DLIB_ASSERT(is_binary_classification_problem(samples, labels) && 0 < eps &&
                  eps <= 1 && basis.size() > 0,
  ...
  while (true) {
    ...
    // Apply the permutation.  Doing this gives the same result as permuting all
    // the features and then recomputing the delta and cov_inv from scratch.
    ...
    // Measure how many features it takes to account for eps% of the weights
    // vector.
    ...
    if (size < best_size) {
      ...
      // Give up once we have had 10 rounds where we didn't find a weights
      // vector with a smaller concentration of good features.
      if (misses >= 10)
        break;
    }
  }
  ...
  // permute the basis so that it matches up with the contents of the best
  // weights
...
sort_basis_vectors(const kernel_type &kern, const vect1_type &samples,
                   const vect2_type &labels, const vect3_type &basis,
                   double eps = 0.99) {
  return bs_impl::sort_basis_vectors_impl(kern, mat(samples), mat(labels),
                                          mat(basis), eps);
}
