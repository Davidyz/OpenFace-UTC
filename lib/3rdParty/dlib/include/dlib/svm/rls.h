...
public:
  explicit rls(double forget_factor_, double C_ = 1000,
               bool apply_forget_factor_to_C_ = false) {
    // make sure requires clause is not broken
    DLIB_ASSERT(0 < forget_factor_ && forget_factor_ <= 1 && 0 < C_,
                "\t rls::rls()"
  ...
  double get_forget_factor() const { return forget_factor; }

  bool should_apply_forget_factor_to_C() const {
    return apply_forget_factor_to_C;
  }

  template <typename EXP> void train(const matrix_exp<EXP> &x, double y) {
    // make sure requires clause is not broken
    ...
    // Since we multiplied by the forget factor, we need to add
    // (1-forget_factor) of the identity matrix back in to keep the
    // regularization alive.
  ...
  const matrix<double, 0, 1> &get_w() const { return w; }
...
private:
  void add_eye_to_inv(matrix<double> &m, double C)
  /*!
      ensures
          - Let m == inv(M)
