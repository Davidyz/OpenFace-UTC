...
public:
  ...
  linearly_independent_subset_finder()
      : my_max_dictionary_size(100), min_tolerance(0.001) {
    clear_dictionary();
  }
  ...
  linearly_independent_subset_finder(const kernel_type &kernel_,
      ...
      : kernel(kernel_), my_max_dictionary_size(max_dictionary_size_),
  ...
  scalar_type minimum_tolerance() const { return min_tolerance; }

  void set_minimum_tolerance(scalar_type min_tol) {
    // make sure requires clause is not broken
    ...
    if (dictionary.size() == 0) {
      ...
      if (delta > min_strength && delta > min_tolerance) {
        if (dictionary.size() == my_max_dictionary_size) {
          ...
          // replace the min strength vector with x.  Put the new vector onto
          // the end of dictionary and remove the vector at position i.
          dictionary.erase(dictionary.begin() + i);
          dictionary.push_back(x);

          // compute reduced K_inv.
          // Remove the i'th vector from the inverse kernel matrix.  This
          // formula is basically just the reverse of the way K_inv is updated
          // by equation 3.14 below.
          temp = removerc(K_inv, i, i) -
                 remove_row(colm(K_inv, i) / K_inv(i, i), i) *
  ...
  friend void serialize(const linearly_independent_subset_finder &item,
                        std::ostream &out) {
  ...
  friend void deserialize(linearly_independent_subset_finder &item,
                          std::istream &in) {
...
template <typename T>
const matrix_op<op_array_to_mat<linearly_independent_subset_finder<T>>>
mat(const linearly_independent_subset_finder<T> &m) {
...
void fill_lisf(linearly_independent_subset_finder<kernel_type> &lisf,
               const vector_type &samples, rand_type &rnd, int sampling_size) {
  // make sure requires clause is not broken
  ...
  // Start out by guessing what a reasonable projection error tolerance is. We
  // will use the biggest projection error we see in a small sample.
  scalar_type tol = 0;
  ...
  // run many rounds of random sampling.  In each round we drop the tolerance
  // lower.
  while (tol >= min_tol && lisf.size() < lisf.max_dictionary_size()) {
    ...
    // Keep picking random samples and adding them into the lisf.  Stop when we
    // either fill it up or can't find any more samples with projection error
    // larger than the current tolerance.
    while (lisf.size() < lisf.max_dictionary_size() &&
           add_failures < sampling_size) {
      if (lisf.add(samples(rnd.get_random_32bit_number() % samples.size())) ==
          false) {
        ++add_failures;
      }
    }
...
template <typename kernel_type, typename vector_type, typename rand_type>
typename enable_if<is_rand<rand_type>>::type
fill_lisf(linearly_independent_subset_finder<kernel_type> &lisf,
...
template <typename kernel_type, typename vector_type, typename rand_type>
typename disable_if<is_rand<rand_type>>::type
fill_lisf(linearly_independent_subset_finder<kernel_type> &lisf,
          const vector_type &samples, rand_type random_seed,
          const int sampling_size = 2000) {
  dlib::rand rnd;
