...
const decision_function<kernel_type> convert_to_decision_function(
  ...
  DLIB_ASSERT(
      project_funct.out_vector_size() > 0 && is_vector(vect) &&
      ...
      "\t const decision_function convert_to_decision_function()"
          ...
          << "\n\t project_funct.out_vector_size():    "
          << project_funct.out_vector_size()
          << "\n\t project_funct.weights.nc():         "
          << project_funct.weights.nc()
  ...
  return decision_function<kernel_type>(trans(project_funct.weights) * vect, 0,
...
public:
  ...
  void clear() { empirical_kernel_map().swap(*this); }

  template <typename T>
  void load(const kernel_type &kernel_, const T &basis_samples) {
  ...
  void load(const linearly_independent_subset_finder<kernel_type> &lisf) {
    if (lisf.size() == 0) {
      ...
      sout
          << "An empty linearly_independent_subset_finder was supplied to the\n"
          << "empirical_kernel_map::load() function.  One reason this might "
             "occur\n"
  ...
  const kernel_type get_kernel() const {
    ...
    DLIB_ASSERT(out_vector_size() > 0,
                "\tconst kernel_type empirical_kernel_map::get_kernel()"
                    << "\n\t You have to load this object with a kernel before "
  ...
  unsigned long basis_size() const { return basis.size(); }
  ...
  convert_to_decision_function(const matrix_exp<EXP> &vect) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(out_vector_size() != 0 && is_vector(vect) &&
                    out_vector_size() == vect.size(),
                "\t const decision_function "
                "empirical_kernel_map::convert_to_decision_function()"
                    << "\n\t Invalid inputs to this function."
    ...
    return decision_function<kernel_type>(trans(weights) * vect, 0, kernel,
                                          mat(basis));
  }
  ...
  convert_to_distance_function(const matrix_exp<EXP> &vect) const {
    ...
    return distance_function<kernel_type>(trans(weights) * vect,
                                          dot(vect, vect), kernel, mat(basis));
  }

  const projection_function<kernel_type> get_projection_function() const {
    // make sure requires clause is not broken
    DLIB_ASSERT(out_vector_size() != 0,
                "\tconst projection_function "
                "empirical_kernel_map::get_projection_function()"
                    << "\n\t You have to load this object with data before you "
  ...
  get_transformation_to(const empirical_kernel_map &target) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(
        out_vector_size() != 0 && target.out_vector_size() != 0 &&
    ...
    return target.weights *
           kernel_matrix(target.get_kernel(), target.basis, basis) *
           trans(weights);
  }
  ...
  void get_transformation_to(
    ...
    DLIB_ASSERT(
        ...
        "\t void empirical_kernel_map::get_transformation_to(target, tmat, "
        "partial_projection)"
            << "\n\t Invalid inputs were given to this function"
            << "\n\t out_vector_size():                 " << out_vector_size()
            << "\n\t target.out_vector_size():          "
    ...
    for (unsigned long i = 0; i < basis_size(); ++i) {
      DLIB_ASSERT(
          dlib::equal((*this)[i], target[i]),
          "\t const matrix empirical_kernel_map::get_transformation_to(target, "
          "tmat, partial_projection)"
              << "\n\t target must contain a superset of the basis vectors in "
                 "*this"
              << "\n\t i: " << i << "\n\t this: " << this);
    }
#endif
    ...
    tmat = colm(target.weights, range(0, num1 - 1)) *
           kernel_matrix(kernel, basis) * trans(weights);
  ...
  project(const sample_type &samp) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(out_vector_size() != 0,
  ...
  project(const sample_type &samp, scalar_type &projection_error) const {
    ...
    temp1 = kernel_matrix(kernel, basis, samp);
    temp2 = weights * temp1;
    // This value should never be negative (it measures squared distance) but
    // I'm putting the abs() here just for good measure since rounding error
    // might push it slightly negative.
  ...
  friend void serialize(const empirical_kernel_map &item, std::ostream &out) {
    serialize(item.basis, out);
    serialize(item.weights, out);
    serialize(item.kernel, out);
  }

  friend void deserialize(empirical_kernel_map &item, std::istream &in) {
...
private:
  template <typename T>
  void load_impl(const kernel_type &kernel_, const T &basis_samples) {
    // make sure requires clause is not broken
    ...
    const scalar_type max_norm =
        max(diag(kernel_matrix(kernel, basis_samples)));
    // we will consider anything less than or equal to this number to be 0
    const scalar_type eps =
        max_norm * 100 * std::numeric_limits<scalar_type>::epsilon();

    // Copy all the basis_samples into basis but make sure we don't copy any
    // samples that have length 0
    ...
    if (basis.size() == 0) {
      ...
      throw empirical_kernel_map_error(
          "All basis_samples given to empirical_kernel_map::load() were zero "
          "vectors");
    }
    ...
    if (svd2(false, true, K, U, W, V)) {
      clear();
      throw empirical_kernel_map_error("While loading empirical_kernel_map "
                                       "with data, SVD failed to converge.");
    }
    ...
    if (num_not_zero == 0) {
      clear();
      throw empirical_kernel_map_error(
          "While loading empirical_kernel_map with data, SVD failed");
    }
  ...
  // These members don't contribute to the logical state of this object.  They
  // are just here so that they don't have to be reallocated every time the
  // project() function is called.
