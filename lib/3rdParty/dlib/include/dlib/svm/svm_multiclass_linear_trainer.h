...
template <typename matrix_type, typename sample_type, typename label_type>
class multiclass_svm_problem
    : public structural_svm_problem_threaded<
          matrix_type,
          std::vector<std::pair<unsigned long, typename matrix_type::type>>> {
  /*!
      WHAT THIS OBJECT REPRESENTS
          This object defines the optimization problem for the multiclass SVM
  trainer object at the bottom of this file.

          The joint feature vectors used by this object, the PSI(x,y) vectors,
  are defined as follows: PSI(x,0) = [x,0,0,0,0, ...,0] PSI(x,1) = [0,x,0,0,0,
  ...,0] PSI(x,2) = [0,0,x,0,0, ...,0] That is, if there are N labels then the
  joint feature vector has a dimension that is N times the dimension of a single
  x sample.  Also, note that we append a -1 value onto each x to account for the
  bias term.
  !*/

public:
  typedef typename matrix_type::type scalar_type;
  typedef std::vector<std::pair<unsigned long, scalar_type>>
      feature_vector_type;

  multiclass_svm_problem(const std::vector<sample_type> &samples_,
      ...
      : structural_svm_problem_threaded<
            ...
            std::vector<std::pair<unsigned long, typename matrix_type::type>>>(
            num_threads),
        samples(samples_), labels(labels_), distinct_labels(distinct_labels_),
  ...
  virtual void separation_oracle(const long idx,
    ...
    // LOSS(idx,y) + F(x,y).  Note that y in this case is given by
    // distinct_labels[i].
    for (unsigned long i = 0; i < distinct_labels.size(); ++i) {
      // Compute the F(x,y) part:
      // perform: temp == dot(relevant part of current solution, samples[idx]) -
      // current_bias
      scalar_type temp =
          dot(mat(&current_solution(i * dims), dims - 1), samples[idx]) -
      ...
      // Now temp == LOSS(idx,y) + F(x,y).  Check if it is the biggest we have
      // seen.
      if (temp > best_val) {
        best_val = temp;
        best_idx = i;
...
private:
  void offset_feature_vector(feature_vector_type &sample,
                             const unsigned long val) const {
    if (val != 0) {
      for (typename feature_vector_type::iterator i = sample.begin();
           i != sample.end(); ++i) {
        i->first += val;
      }
    }
...
template <typename K, typename label_type_ = typename K::scalar_type>
...
public:
  ...
  typedef multiclass_linear_decision_function<kernel_type, label_type>
  ...
  svm_multiclass_linear_trainer()
      : num_threads(4), C(1), eps(0.001), max_iterations(10000), verbose(false),
        learn_nonnegative_weights(false) {}

  void set_num_threads(unsigned long num) { num_threads = num; }

  unsigned long get_num_threads() const { return num_threads; }

  void set_epsilon(scalar_type eps_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(eps_ > 0, "\t void svm_multiclass_linear_trainer::set_epsilon()"
  ...
  void set_c(scalar_type C_) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C_ > 0, "\t void svm_multiclass_linear_trainer::set_c()"
  ...
  bool has_prior() const { return prior.labels.size() != 0; }

  trained_function_type train(const std::vector<sample_type> &all_samples,
                              const std::vector<label_type> &all_labels) const {
  ...
  trained_function_type train(const std::vector<sample_type> &all_samples,
    ...
    DLIB_ASSERT(is_learning_problem(all_samples, all_labels),
                "\t trained_function_type "
                "svm_multiclass_linear_trainer::train(all_samples,all_labels)"
    ...
    if (has_prior()) {
      df.labels.insert(df.labels.end(), prior.labels.begin(),
                       prior.labels.end());
    ...
    if (is_matrix<sample_type>::value && has_prior()) {
      DLIB_ASSERT(input_sample_dimensionality == prior.weights.nc(),
                  "\t trained_function_type "
                  "svm_multiclass_linear_trainer::train(all_samples,all_labels)"
                      << "\n\t The training samples given to this function are "
    ...
    w_type weights;
    multiclass_svm_problem<w_type, sample_type, label_type> problem(
        all_samples, all_labels, df.labels, dims, num_threads);
    if (verbose)
      problem.be_verbose();
    ...
    if (!has_prior()) {
      ...
      for (unsigned long i = 0; i < prior.labels.size(); ++i) {
        const long r =
            std::find(df.labels.begin(), df.labels.end(), prior.labels[i]) -
            df.labels.begin();
        set_rowm(temp, r) = join_rows(rowm(prior.weights, i),
                                      zeros_matrix<scalar_type>(1, pad_size));
        b(r) = prior.b(i);
      }
    ...
    df.weights =
        colm(reshape(weights, df.labels.size(), dims + 1), range(0, dims - 1));
