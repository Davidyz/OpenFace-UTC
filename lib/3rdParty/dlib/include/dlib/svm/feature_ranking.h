...
matrix<typename kernel_type::scalar_type, 0, 2,
       typename kernel_type::mem_manager_type>
rank_features_impl(const kcentroid<kernel_type> &kc,
  ...
  */
  ...
  matrix<scalar_type, sample_matrix_type::type::NR, 1, mm> mask(
      samples(0).nr());
  set_all_elements(mask, 1);

  // figure out what the separation is between the two centroids when all the
  ...
  for (long i = results.nr() - 1; i >= 0; --i) {
    ...
    scalar_type worst_feature_score =
  ...
  const scalar_type max_separation =
      std::max(max(colm(results, 1)), first_separation);
...
matrix<typename kernel_type::scalar_type, 0, 2,
       typename kernel_type::mem_manager_type>
rank_features(const kcentroid<kernel_type> &kc,
              const sample_matrix_type &samples,
              const label_matrix_type &labels) {
...
rank_features_impl(const kcentroid<kernel_type> &kc,
                   const sample_matrix_type &samples,
                   const label_matrix_type &labels, const long num_features) {
  /*
  ...
  */
  ...
  for (long i = 0; i < results.nr(); ++i) {
    ...
    scalar_type best_feature_score =
        -std::numeric_limits<scalar_type>::infinity();
...
rank_features(const kcentroid<kernel_type> &kc,
  ...
  if (mat(samples).nr() > 0 && num_features == mat(samples)(0).nr()) {
    // if we are going to rank them all then might as well do the recursive
    // feature elimination version
    return rank_features_impl(kc, mat(samples), mat(labels));
  } else {
...
template <typename K, typename sample_matrix_type, typename label_matrix_type>
typename K::scalar_type centroid_gap(const kcentroid<K> &kc,
                                     const sample_matrix_type &samples,
                                     const label_matrix_type &labels) {
  kcentroid<K> kc1(kc);
  kcentroid<K> kc2(kc);
...
template <typename sample_matrix_type, typename label_matrix_type> class test {
...
public:
  test(const sample_matrix_type &samples_, const label_matrix_type &labels_,
       unsigned long num_sv_, bool verbose_)
  ...
  double operator()(double gamma) const {
    ...
    // we are doing the optimization in log space so don't forget to convert
    // back to normal space
    gamma = std::exp(gamma);

    typedef radial_basis_kernel<sample_type> kernel_type;
    // Make a kcentroid and find out what the gap is at the current gamma.  Try
    // to pick a reasonable tolerance.
    ...
    if (verbose) {
      cout << "\rChecking goodness of gamma = " << gamma
           << ".  Goodness = " << temp << "                    " << flush;
    }
    return temp;
  }
...
double find_gamma_with_big_centroid_gap_impl(const sample_matrix_type &samples,
                                             const label_matrix_type &labels,
  ...
  test<sample_matrix_type, label_matrix_type> funct(samples, labels, num_sv,
                                                    verbose);
  double best_gamma = std::log(initial_gamma);
  double goodness =
      find_max_single_variable(funct, best_gamma, -15, 15, 1e-3, 100);

  if (verbose) {
    cout << "\rBest gamma = " << std::exp(best_gamma)
         << ".  Goodness = " << goodness << "                    " << endl;
  }
...
double find_gamma_with_big_centroid_gap(const sample_matrix_type &samples,
  ...
  DLIB_ASSERT(initial_gamma > 0 && num_sv > 0 &&
  ...
  return rank_features_helpers::find_gamma_with_big_centroid_gap_impl(
      mat(samples), mat(labels), initial_gamma, num_sv, false);
}
...
double verbose_find_gamma_with_big_centroid_gap(
  ...
  return rank_features_helpers::find_gamma_with_big_centroid_gap_impl(
      mat(samples), mat(labels), initial_gamma, num_sv, true);
}
