...
template <typename T> struct radial_basis_kernel {
  ...
  radial_basis_kernel(const radial_basis_kernel &k) : gamma(k.gamma) {}
...
template <typename T> struct kernel_derivative<radial_basis_kernel<T>> {
  typedef typename T::type scalar_type;
  typedef T sample_type;
  typedef typename T::mem_manager_type mem_manager_type;
...
template <typename T> struct kernel_derivative<polynomial_kernel<T>> {
  typedef typename T::type scalar_type;
  typedef T sample_type;
  typedef typename T::mem_manager_type mem_manager_type;
  ...
  const sample_type &operator()(const sample_type &x,
                                const sample_type &y) const {
    // return the derivative of the rbf kernel
    temp = k.degree * k.gamma * x *
           std::pow(k.gamma * (trans(x) * y) + k.coef, k.degree - 1);
    return temp;
  }
...
template <typename T> struct sigmoid_kernel {
  ...
  sigmoid_kernel(const sigmoid_kernel &k) : gamma(k.gamma), coef(k.coef) {}
...
template <typename T> struct kernel_derivative<sigmoid_kernel<T>> {
  typedef typename T::type scalar_type;
  typedef T sample_type;
  typedef typename T::mem_manager_type mem_manager_type;
...
template <typename T>
void serialize(const linear_kernel<T> &, std::ostream &) {}

template <typename T> void deserialize(linear_kernel<T> &, std::istream &) {}

template <typename T> struct kernel_derivative<linear_kernel<T>> {
  typedef typename T::type scalar_type;
  typedef T sample_type;
  typedef typename T::mem_manager_type mem_manager_type;
...
template <typename T> struct histogram_intersection_kernel {
  ...
  bool operator==(const histogram_intersection_kernel &) const { return true; }
};

template <typename T>
void serialize(const histogram_intersection_kernel<T> &, std::ostream &) {}

template <typename T>
void deserialize(histogram_intersection_kernel<T> &, std::istream &) {}
...
template <typename T> struct offset_kernel {
  ...
  offset_kernel(const offset_kernel &k) : kernel(k.kernel), offset(k.offset) {}
...
template <typename T> struct kernel_derivative<offset_kernel<T>> {
  typedef typename T::scalar_type scalar_type;
  typedef typename T::sample_type sample_type;
  typedef typename T::mem_manager_type mem_manager_type;
