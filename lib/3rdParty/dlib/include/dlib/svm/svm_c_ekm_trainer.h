...
namespace dlib {
template <typename K> class svm_c_ekm_trainer {
...
public:
  ...
  explicit svm_c_ekm_trainer(const scalar_type &C) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C > 0, "\t svm_c_ekm_trainer::svm_c_ekm_trainer()"
  ...
  void set_epsilon(scalar_type eps) {
    // make sure requires clause is not broken
    DLIB_ASSERT(eps > 0, "\t void svm_c_ekm_trainer::set_epsilon()"
  ...
  const scalar_type get_epsilon() const { return ocas.get_epsilon(); }

  void set_max_iterations(unsigned long max_iter) {
    ocas.set_max_iterations(max_iter);
  }

  unsigned long get_max_iterations() { return ocas.get_max_iterations(); }
  ...
  void set_oca(const oca &item) { ocas.set_oca(item); }

  const oca get_oca() const { return ocas.get_oca(); }

  const kernel_type get_kernel() const { return kern; }
  ...
  template <typename T> void set_basis(const T &basis_samples) {
    // make sure requires clause is not broken
    DLIB_ASSERT(basis_samples.size() > 0 && is_vector(mat(basis_samples)),
                "\tvoid svm_c_ekm_trainer::set_basis(basis_samples)"
                    << "\n\t You have to give a non-empty set of basis_samples "
                       "and it must be a vector"
                    << "\n\t basis_samples.size():                       "
                    << basis_samples.size()
                    << "\n\t is_vector(mat(basis_samples)): "
                    << is_vector(mat(basis_samples)) << "\n\t this: " << this);
  ...
  bool basis_loaded() const { return (basis.size() != 0); }
  ...
  unsigned long get_max_basis_size() const { return max_basis_size; }

  void set_max_basis_size(unsigned long max_basis_size_) {
    // make sure requires clause is not broken
  ...
  unsigned long get_initial_basis_size() const { return initial_basis_size; }

  void set_initial_basis_size(unsigned long initial_basis_size_) {
    // make sure requires clause is not broken
  ...
  unsigned long get_basis_size_increment() const {
    return basis_size_increment;
  }

  void set_basis_size_increment(unsigned long basis_size_increment_) {
    // make sure requires clause is not broken
  ...
  void set_c(scalar_type C) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C > 0, "\t void svm_c_ekm_trainer::set_c()"
                           << "\n\t C must be greater than 0"
                           << "\n\t C:    " << C << "\n\t this: " << this);
  ...
  const scalar_type get_c_class1() const { return ocas.get_c_class1(); }

  const scalar_type get_c_class2() const { return ocas.get_c_class2(); }

  void set_c_class1(scalar_type C) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C > 0, "\t void svm_c_ekm_trainer::set_c_class1()"
                           << "\n\t C must be greater than 0"
                           << "\n\t C:    " << C << "\n\t this: " << this);
  ...
  void set_c_class2(scalar_type C) {
    // make sure requires clause is not broken
    DLIB_ASSERT(C > 0, "\t void svm_c_ekm_trainer::set_c_class2()"
                           << "\n\t C must be greater than 0"
                           << "\n\t C:    " << C << "\n\t this: " << this);
...
private:
  template <typename in_sample_vector_type, typename in_scalar_vector_type>
  const decision_function<kernel_type>
  do_train_user_basis(const in_sample_vector_type &x,
                      const in_scalar_vector_type &y,
                      scalar_type &svm_objective) const
  /*!
      requires
  ...
  !*/
  {
    ...
    if (verbose) {
      std::cout << "\nMean EKM projection error:                  " << rs.mean()
                << std::endl;
      std::cout << "Standard deviation of EKM projection error: " << rs.stddev()
                << std::endl;
    }

    // now do the training
    decision_function<
        linear_kernel<matrix<scalar_type, 0, 1, mem_manager_type>>>
        df;
  ...
  template <typename in_sample_vector_type, typename in_scalar_vector_type>
  const decision_function<kernel_type>
  do_train_auto_basis(const in_sample_vector_type &x,
                      const in_scalar_vector_type &y,
                      scalar_type &svm_objective) const {
    // make sure requires clause is not broken
    DLIB_ASSERT(is_binary_classification_problem(x, y) == true,
    ...
    linearly_independent_subset_finder<kernel_type> lisf(get_kernel(),
                                                         max_basis_size);
    ...
    for (unsigned long i = 0;
         i < 10 * initial_basis_size && lisf.size() < initial_basis_size; ++i) {
    ...
    // while we are determining what the basis set will be we are going to use a
    // very lose stopping condition.  We will tighten it back up before
    // producing the final decision_function.
    ...
    // do this by repeatedly training the SVM and adding a few points which
    // violate the margin to the basis in each iteration.
    while (true) {
      // if the basis is already as big as it's going to get then just do the
      // most accurate training right now.
      ...
      while (true) {
        ...
        if (trainer.get_epsilon() > min_epsilon) {
          trainer.set_epsilon(
              std::max(trainer.get_epsilon() * 0.5, min_epsilon));
          if (verbose)
            std::cout << " *** Reducing epsilon to " << trainer.get_epsilon()
                      << std::endl;
        } else
          break;
      }
      ...
      for (unsigned long j = 0;
        ...
        // If it is a margin violator then it is useful to add it into the basis
        // set.
        if (df(proj_samples[idx]) * y(idx) < 1) {
          // Add the sample into the basis set if it is linearly independent of
          // all the vectors already in the basis set.
          if (lisf.add(x(idx))) {
            ++count;
          }
        }
      ...
      if (count == 0) {
        if (verbose)
          std::cout << "Stopping, couldn't add more basis vectors."
                    << std::endl;
        break;
      }

      // Project all the samples into the span of our newly enlarged basis.  We
      // will do this using the special transformation in the EKM that lets us
      // project from a smaller basis set to a larger without needing to
      // reevaluate kernel functions we have already computed.
      ekm.swap(prev_ekm);
      ekm.load(lisf);
      ...
      for (long i = 0; i < x.size(); ++i) {
        // assign to temporary to avoid memory allocation that would result if
        // we assigned this expression straight into proj_samples[i]
        temp = prev_to_new * proj_samples[i] + proj_part(x(i));
        proj_samples[i] = temp;
      }
    }

    // Reproject all the data samples using the final basis.  We could just use
    // what we already have but the recursive thing done above to compute the
    // proj_samples might have accumulated a little numerical error.  So lets
    // just be safe.
    ...
    if (verbose) {
      ...
      std::cout << "Standard deviation of EKM projection error: " << rs.stddev()
                << std::endl;
      std::cout
          << "Mean EKM projection error for margin violators:                  "
          << rs_margin.mean() << std::endl;
      std::cout
          << "Standard deviation of EKM projection error for margin violators: "
          << ((rs_margin.current_n() > 1) ? rs_margin.stddev() : 0)
          << std::endl;
