...
// FaceTrackingVidMulti.cpp : Defines the entry point for the multiple face
...
void RemoveOverlapingModels(std::vector<LandmarkDetector::CLNF> &face_models,
  ...
  // Go over the model and eliminate detections that are not informative (there
  // already is a tracker there)
    ...
    if (active_models[model1]) {
      ...
      for (int model2 = model1 + 1; model2 < active_models.size(); ++model2) {
        if (active_models[model2]) {
          ...
          // If the model is already tracking what we're detecting ignore the
          // detection, this is determined by amount of overlap
...
void NonOverlapingDetections(
    const std::vector<LandmarkDetector::CLNF> &clnf_models,
  ...
  for (size_t model = 0; model < clnf_models.size(); ++model) {
    ...
    for (int detection = face_detections.size() - 1; detection >= 0;
         --detection) {
      // If the model is already tracking what we're detecting ignore the
      // detection, this is determined by amount of overlap
...
int main(int argc, char **argv) {
  ...
  face_model.face_detector_HAAR.load(
      det_parameters[0].haar_face_detector_location);
  face_model.haar_face_detector_location =
      det_parameters[0].haar_face_detector_location;
  face_model.face_detector_MTCNN.Read(
      det_parameters[0].mtcnn_face_detector_location);
  face_model.mtcnn_face_detector_location =
      det_parameters[0].mtcnn_face_detector_location;
  ...
  if (det_parameters[0].curr_face_detector ==
    ...
    det_parameters[0].curr_face_detector =
  ...
  // Load facial feature extractor and AU analyser (make sure it is static, as
  // we don't reidentify faces)
  ...
  while (
      true) // this is not a for loop as we might also be reading from a webcam
  {
    // The sequence reader chooses what to open based on command line arguments
    // provided
    ...
    Utilities::RecorderOpenFaceParameters recording_params(
        arguments, true, sequence_reader.IsWebcam(), sequence_reader.fx,
    ...
    if (sequence_reader.IsWebcam()) {
      INFO_STREAM("WARNING: using a webcam in feature extraction, forcing "
                  "visualization of tracking to allow quitting the application "
                  "(press q)");
      visualizer.vis_track = true;
    }

    if (recording_params.outputAUs()) {
      INFO_STREAM("WARNING: using a AU detection in multiple face mode, it "
                  "might not be as accurate and is experimental");
    }
    ...
    while (!rgb_image.empty()) {
      ...
      // Get the detections (every 8th frame and when there are free models
      // available for tracking)
      if (frame_count % 8 == 0 && !all_models_active) {
        if (det_parameters[0].curr_face_detector ==
            LandmarkDetector::FaceModelParameters::HOG_SVM_DETECTOR) {
          std::vector<float> confidences;
          LandmarkDetector::DetectFacesHOG(face_detections, grayscale_image,
                                           face_models[0].face_detector_HOG,
                                           confidences);
        } else if (det_parameters[0].curr_face_detector ==
                   LandmarkDetector::FaceModelParameters::HAAR_DETECTOR) {
          LandmarkDetector::DetectFaces(face_detections, grayscale_image,
                                        face_models[0].face_detector_HAAR);
        } else {
          std::vector<float> confidences;
          LandmarkDetector::DetectFacesMTCNN(face_detections, rgb_image,
                                             face_models[0].face_detector_MTCNN,
                                             confidences);
        }
      }

      // Keep only non overlapping detections (so as not to start tracking where
      // the face is already tracked)
      ...
      for (unsigned int model = 0; model < face_models.size(); ++model) {
        ...
        if (!active_models[model]) {
          ...
          for (size_t detection_ind = 0; detection_ind < face_detections.size();
               ++detection_ind) {
            ...
            if (!face_detections_used[detection_ind]) {
              ...
              // This ensures that a wider window is used for the initial
              // landmark localisation
              face_models[model].detection_success = false;
              detection_success = LandmarkDetector::DetectLandmarksInVideo(
                  rgb_image, face_detections[detection_ind], face_models[model],
                  det_parameters[model], grayscale_image);
          ...
          detection_success = LandmarkDetector::DetectLandmarksInVideo(
              rgb_image, face_models[model], det_parameters[model],
              grayscale_image);
        }
      }

      // Remove models that end up tracking overlapping faces
      // even if initial bounding boxes were not overlapping, they could have
      // ended up converging to the same face
      ...
      // Go through every model and detect eye gaze, record results and
      ...
      for (size_t model = 0; model < face_models.size(); ++model) {
        ...
        if (active_models[model]) {
          ...
          cv::Vec6d pose_estimate = LandmarkDetector::GetPose(
              face_models[model], sequence_reader.fx, sequence_reader.fy,
              sequence_reader.cx, sequence_reader.cy);
          ...
          if (face_models[model].detection_success && face_model.eye_model) {
            GazeAnalysis::EstimateGaze(face_models[model], gaze_direction0,
                                       sequence_reader.fx, sequence_reader.fy,
                                       sequence_reader.cx, sequence_reader.cy,
                                       true);
            GazeAnalysis::EstimateGaze(face_models[model], gaze_direction1,
                                       sequence_reader.fx, sequence_reader.fy,
                                       sequence_reader.cx, sequence_reader.cy,
                                       false);
            gaze_angle =
                GazeAnalysis::GetGazeAngle(gaze_direction0, gaze_direction1);
          }
          ...
          if (recording_params.outputAlignedFaces() ||
            ...
            face_analyser.PredictStaticAUsAndComputeFeatures(
                rgb_image, face_models[model].detected_landmarks);
          ...
          visualizer.SetObservationLandmarks(
              face_models[model].detected_landmarks,
              face_models[model].detection_certainty);
          visualizer.SetObservationPose(
              LandmarkDetector::GetPose(face_models[model], sequence_reader.fx,
                                        sequence_reader.fy, sequence_reader.cx,
                                        sequence_reader.cy),
              face_models[model].detection_certainty);
          visualizer.SetObservationGaze(
              gaze_direction0, gaze_direction1,
              LandmarkDetector::CalculateAllEyeLandmarks(face_models[model]),
              LandmarkDetector::Calculate3DEyeLandmarks(
                  face_models[model], sequence_reader.fx, sequence_reader.fy,
                  sequence_reader.cx, sequence_reader.cy),
              face_models[model].detection_certainty);
          visualizer.SetObservationActionUnits(
              face_analyser.GetCurrentAUsReg(),
              face_analyser.GetCurrentAUsClass());

          // Output features
          open_face_rec.SetObservationHOG(
              face_models[model].detection_success, hog_descriptor,
          ...
          open_face_rec.SetObservationLandmarks(
              face_models[model].detected_landmarks,
              face_models[model].GetShape(
                  sequence_reader.fx, sequence_reader.fy, sequence_reader.cx,
                  sequence_reader.cy),
              face_models[model].params_global, face_models[model].params_local,
              face_models[model].detection_certainty,
              face_models[model].detection_success);
          open_face_rec.SetObservationPose(pose_estimate);
          open_face_rec.SetObservationGaze(
              gaze_direction0, gaze_direction1, gaze_angle,
              LandmarkDetector::CalculateAllEyeLandmarks(face_models[model]),
              LandmarkDetector::Calculate3DEyeLandmarks(
                  face_models[model], sequence_reader.fx, sequence_reader.fy,
                  sequence_reader.cx, sequence_reader.cy));
