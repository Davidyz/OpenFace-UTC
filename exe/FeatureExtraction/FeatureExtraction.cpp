...
// FeatureExtraction.cpp : Defines the entry point for the feature extraction
// console application.
...
int main(int argc, char **argv) {
      ...
      true) // this is not a for loop as we might also be reading from a webcam
  {
    ...
    if (recording_params.outputGaze() && !face_model.eye_model)
      std::cout << "WARNING: no eye model defined, but outputting gaze"
                << std::endl;
    ...
    while (!captured_image.empty()) {
      ...
      if (recording_params.outputAlignedFaces() ||
        ...
        face_analyser.AddNextFrame(
            ...
            face_model.detection_success, getUTCTimestamp(),
      ...
      visualizer.SetImage(captured_image, sequence_reader.fx,
      ...
      open_face_rec.SetObservationGaze(
          ...
          LandmarkDetector::Calculate3DEyeLandmarks(
              face_model, sequence_reader.fx, sequence_reader.fy,
              sequence_reader.cx, sequence_reader.cy));
      open_face_rec.SetObservationTimestamp(getUTCTimestamp());
