...
//
///////////////////////////////////////////////////////////////////////////////
// FaceLandmarkImg.cpp : Defines the entry point for the console application for
...
int main(int argc, char **argv) {
  ...
  if (det_parameters.curr_face_detector ==
    ...
    det_parameters.curr_face_detector =
  ...
  while (!rgb_image.empty()) {
    ...
    Utilities::RecorderOpenFaceParameters recording_params(
        arguments, false, false, image_reader.fx, image_reader.fy,
        image_reader.cx, image_reader.cy);

    if (!face_model.eye_model) {
      recording_params.setOutputGaze(false);
    }
    Utilities::RecorderOpenFace open_face_rec(image_reader.name,
                                              recording_params, arguments);

    visualizer.SetImage(rgb_image, image_reader.fx, image_reader.fy,
                        image_reader.cx, image_reader.cy);

    // Making sure the image is in uchar grayscale (some face detectors use RGB,
    // landmark detector uses grayscale)
    ...
    if (image_reader.has_bounding_boxes) {
      ...
      } else if (det_parameters.curr_face_detector ==
    ...
    for (size_t face = 0; face < face_detections.size(); ++face) {
      ...
      bool success = LandmarkDetector::DetectLandmarksInImage(
          rgb_image, face_detections[face], face_model, det_parameters,
          grayscale_image);
      ...
      if (face_model.eye_model) {
        GazeAnalysis::EstimateGaze(face_model, gaze_direction0, image_reader.fx,
                                   image_reader.fy, image_reader.cx,
                                   image_reader.cy, true);
        GazeAnalysis::EstimateGaze(face_model, gaze_direction1, image_reader.fx,
                                   image_reader.fy, image_reader.cx,
                                   image_reader.cy, false);
        gaze_angle =
      ...
      visualizer.SetObservationLandmarks(
          ...
          face_model.GetVisibilities()); // Set confidence to high to make sure
                                         // we always visualize
      ...
      open_face_rec.SetObservationHOG(
          face_model.detection_success, hog_descriptor, num_hog_rows,
          num_hog_cols, 31); // The number of channels in HOG is fixed at the
                             // moment, as using FHOG
      ...
      open_face_rec.SetObservationLandmarks(
          ...
          face_model.GetShape(image_reader.fx, image_reader.fy, image_reader.cx,
                              image_reader.cy),
