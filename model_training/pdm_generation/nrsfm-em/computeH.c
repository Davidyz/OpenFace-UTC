...
static mxArray *McomputeH(int nargout_, const mxArray *Uc, const mxArray *Vc,
                          const mxArray *E_z, const mxArray *E_zz,
                          const mxArray *RR);

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  if (nlhs > 1) {
    mexErrMsgTxt("Run-time Error: File: computeh Line: 1 Column:"
...
double **getMxArray(IN const mxArray *mxArr, OUT int *rows, OUT int *cols) {
  int i, j, numDim;
  ...
  *cols = mxGetN(mxArr);
  ...
  if (((matrix = (double **)malloc(sizeof(double *) * (*rows))) == NULL) ||
      ((matrix[0] = (double *)malloc(sizeof(double) * (*rows) * (*cols))) ==
       NULL))
    return NULL;
...
double **createMatrix(IN int rows, IN int cols) {
  int i;
  double **matrix;
...
int **createIntMatrix(IN int rows, IN int cols) {
  int i;
  int **matrix;
...
void IO_MLabWriteDoubleImg(char *fileName, char *nameImg, double *img, int rows,
                           int cols) {
  MATFile *fp;
  int i, j, dims[2];
  double *ptr;
...
void kron(IN int rowsA, IN int colsA, IN double **A, IN int rowsB, IN int colsB,
          IN double **B, IN int rowsC, IN int colsC, OUT double **C) {
  int i, j, k, l;
...
double conjgradient(IN int n, IN double **A, IN double **b,
  ...
  double Ax, delta_old, delta_new, delta_0, **r, **d, **q, alpha, alpha_denom,
      beta;
...
double conjgradient2(IN int n, IN double **A, IN double **b,
                     IN int i_max,      /* max # iterations */
                     IN double epsilon, /* error tolerance */
                     IN int k, IN OUT double **x) {
  // my conjugate gradient solver for .5*x'*A*x -b'*x, based on the
  ...
  for (row = 0; row < n; row++) {
    ...
    for (j = 0; j < (k + 1); j++) {
      ...
      Ax += A[row][ind] * x[0][ind] + A[row][ind + 1] * x[0][ind + 1] +
            A[row][ind + 2] * x[0][ind + 2];
    }
  ...
  while ((i < i_max) && (delta_new > epsilon * epsilon * delta_0)) {
    ...
    for (row = 0; row < n; row++) {
      ...
      for (j = 0; j < (k + 1); j++) {
        ind = col_offset + j * sparsity_offset;
        q[0][row] += A[row][ind] * d[0][ind] + A[row][ind + 1] * d[0][ind + 1] +
                     A[row][ind + 2] * d[0][ind + 2];
      }
...
static mxArray *McomputeH(int nargout_, const mxArray *Uc, const mxArray *Vc,
                          const mxArray *E_z, const mxArray *E_zz,
                          const mxArray *RR) {
  double **Ucc, **Vcc, **E_zc, **E_zzc, **RRc, **KK1c, **KK2c, **kk3c, **P_tc,
      **R_tc, **zz_hat_tc, **Tmp1c, **Tmp2c, **vecH_hatc = NULL, delta_err,
                                             epsilon;
  int tc, Fc, Nc, junk1, junk2, kc, ic, jc, ii, jj, kk, ll, i_max,
      **sparsityMap;
...
#define DEBUGFILENAME ".\\debug.mat"
  ...
  for (tc = 0; tc < Fc; tc++) {
...
#if MYDEBUG
    IO_MLabWriteDoubleImg(DEBUGFILENAME, "zz_hat_tc", zz_hat_tc[0], kc + 1,
                          kc + 1);
#endif
    ...
    /*for(ii=0; ii<(kc+1); ii++)
     for(kk=0; kk<(3*Nc); kk++)
       for(jj=0; jj<(kc+1); jj++)
         for(ll=0; ll<(3*Nc); ll++)
           KK2c[(3*Nc)*ii+kk][(3*Nc)*jj+ll] +=
     zz_hat_tc[ii][jj]*Tmp1c[kk][ll];*/
    for (ii = 0; ii < (kc + 1); ii++)
      for (kk = 0; kk < Nc; kk++)
        for (jj = 0; jj < (kc + 1); jj++) {
          KK2c[(3 * Nc) * ii + kk * 3][(3 * Nc) * jj + kk * 3] +=
              zz_hat_tc[ii][jj] * Tmp1c[kk * 3][kk * 3];
          KK2c[(3 * Nc) * ii + kk * 3][(3 * Nc) * jj + kk * 3 + 1] +=
              zz_hat_tc[ii][jj] * Tmp1c[kk * 3][kk * 3 + 1];
          KK2c[(3 * Nc) * ii + kk * 3][(3 * Nc) * jj + kk * 3 + 2] +=
              zz_hat_tc[ii][jj] * Tmp1c[kk * 3][kk * 3 + 2];

          KK2c[(3 * Nc) * ii + kk * 3 + 1][(3 * Nc) * jj + kk * 3] +=
              zz_hat_tc[ii][jj] * Tmp1c[kk * 3 + 1][kk * 3];
          KK2c[(3 * Nc) * ii + kk * 3 + 1][(3 * Nc) * jj + kk * 3 + 1] +=
              zz_hat_tc[ii][jj] * Tmp1c[kk * 3 + 1][kk * 3 + 1];
          KK2c[(3 * Nc) * ii + kk * 3 + 1][(3 * Nc) * jj + kk * 3 + 2] +=
              zz_hat_tc[ii][jj] * Tmp1c[kk * 3 + 1][kk * 3 + 2];

          KK2c[(3 * Nc) * ii + kk * 3 + 2][(3 * Nc) * jj + kk * 3] +=
              zz_hat_tc[ii][jj] * Tmp1c[kk * 3 + 2][kk * 3];
          KK2c[(3 * Nc) * ii + kk * 3 + 2][(3 * Nc) * jj + kk * 3 + 1] +=
              zz_hat_tc[ii][jj] * Tmp1c[kk * 3 + 2][kk * 3 + 1];
          KK2c[(3 * Nc) * ii + kk * 3 + 2][(3 * Nc) * jj + kk * 3 + 2] +=
              zz_hat_tc[ii][jj] * Tmp1c[kk * 3 + 2][kk * 3 + 2];
        }
#if MYDEBUG
    IO_MLabWriteDoubleImg(DEBUGFILENAME, "KK2c", KK2c[0], 3 * Nc * (kc + 1),
                          3 * Nc * (kc + 1));
#endif
    ...
    for (ic = 0; ic < Nc; ic++) {
      Tmp2c[0][3 * ic] = KK1c[2 * ic][3 * ic] * P_tc[0][2 * ic] +
                         KK1c[2 * ic + 1][3 * ic] * P_tc[0][2 * ic + 1];
      Tmp2c[0][3 * ic + 1] = KK1c[2 * ic][3 * ic + 1] * P_tc[0][2 * ic] +
                             KK1c[2 * ic + 1][3 * ic + 1] * P_tc[0][2 * ic + 1];
      Tmp2c[0][3 * ic + 2] = KK1c[2 * ic][3 * ic + 2] * P_tc[0][2 * ic] +
                             KK1c[2 * ic + 1][3 * ic + 2] * P_tc[0][2 * ic + 1];
    }
#if MYDEBUG
    ...
    for (ic = 0; ic < 3 * Nc; ic++) {
      ...
      for (jc = 1; jc < (kc + 1); jc++)
        ...
        kk3c[0][ic + jc * (3 * Nc)] =
            kk3c[0][ic + jc * (3 * Nc)] + Tmp2c[0][ic] * E_zc[jc - 1][tc];
    }
#if MYDEBUG
  ...
  // delta_err = conjgradient(3*Nc*(kc+1), KK2c, kk3c, i_max, epsilon,
  // vecH_hatc);
  delta_err = conjgradient2(3 * Nc * (kc + 1), KK2c, kk3c, i_max, epsilon, kc,
                            vecH_hatc);
